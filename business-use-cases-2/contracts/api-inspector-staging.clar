...
(define-public (execute-api-call (endpoint-id uint) (params uint))
  (if (var-get api-enabled)
    (ok (begin (var-set call-counter (+ (var-get call-counter) u1)) (tuple (endpoint endpoint-id) (params params))))
    (err u1)))
...
(define-public (query-api-status)
  (ok (tuple (calls (var-get call-counter)) (errors (var-get error-counter)) (enabled (var-get api-enabled)))))