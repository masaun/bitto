...
const contractDetails = parseContractIdentifier(
  process.env.MAKERSPACE_ENGAGEMENT_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for MakerspaceEngagement Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  MakerspaceEngagement Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: MAKERSPACE_ENGAGEMENT_CONTRACT_ADDRESS');

  if (!process.env.MAKERSPACE_ENGAGEMENT_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: MAKERSPACE_ENGAGEMENT_CONTRACT_ADDRESS not set in environment.');
