...
const contractDetails = parseContractIdentifier(
  process.env.MICRO_BUSINESS_SUPPORT_GAME_CONTRACT_ADDRESS,
  'micro-business-support-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for MicroBusinessSupportGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  MicroBusinessSupportGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: MICRO_BUSINESS_SUPPORT_GAME_CONTRACT_ADDRESS');

  if (!process.env.MICRO_BUSINESS_SUPPORT_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: MICRO_BUSINESS_SUPPORT_GAME_CONTRACT_ADDRESS not set in environment.');
