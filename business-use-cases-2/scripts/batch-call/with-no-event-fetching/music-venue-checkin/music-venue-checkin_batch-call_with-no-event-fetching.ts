...
const contractDetails = parseContractIdentifier(
  process.env.MUSIC_VENUE_CHECKIN_CONTRACT_ADDRESS,
  'music-venue-checkin'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for MusicVenueCheckin Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  MusicVenueCheckin Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: MUSIC_VENUE_CHECKIN_CONTRACT_ADDRESS');

  if (!process.env.MUSIC_VENUE_CHECKIN_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: MUSIC_VENUE_CHECKIN_CONTRACT_ADDRESS not set in environment.');
