...
const contractDetails = parseContractIdentifier(
  process.env.TOWN_VISIT_REWARD_CONTRACT_ADDRESS,
  'town-visit-reward'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TownVisitReward Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TownVisitReward Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOWN_VISIT_REWARD_CONTRACT_ADDRESS');

  if (!process.env.TOWN_VISIT_REWARD_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOWN_VISIT_REWARD_CONTRACT_ADDRESS not set in environment.');
