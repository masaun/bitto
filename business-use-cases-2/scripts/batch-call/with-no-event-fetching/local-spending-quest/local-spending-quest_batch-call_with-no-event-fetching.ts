...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_SPENDING_QUEST_CONTRACT_ADDRESS,
  'local-spending-quest'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalSpendingQuest Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalSpendingQuest Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_SPENDING_QUEST_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_SPENDING_QUEST_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_SPENDING_QUEST_CONTRACT_ADDRESS not set in environment.');
