...
const contractDetails = parseContractIdentifier(
  process.env.CONCERT_VENUE_ENGAGEMENT_CONTRACT_ADDRESS,
  'concert-venue-engagement'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for ConcertVenueEngagement Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  ConcertVenueEngagement Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CONCERT_VENUE_ENGAGEMENT_CONTRACT_ADDRESS');

  if (!process.env.CONCERT_VENUE_ENGAGEMENT_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CONCERT_VENUE_ENGAGEMENT_CONTRACT_ADDRESS not set in environment.');
