...
const contractDetails = parseContractIdentifier(
  process.env.TOWN_AMBASSADOR_LEAGUE_CONTRACT_ADDRESS,
  'town-ambassador-league'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TownAmbassadorLeague Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TownAmbassadorLeague Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOWN_AMBASSADOR_LEAGUE_CONTRACT_ADDRESS');

  if (!process.env.TOWN_AMBASSADOR_LEAGUE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOWN_AMBASSADOR_LEAGUE_CONTRACT_ADDRESS not set in environment.');
