...
const contractDetails = parseContractIdentifier(
  process.env.CREATIVE_DISTRICT_GAME_CONTRACT_ADDRESS,
  'creative-district-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CreativeDistrictGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CreativeDistrictGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CREATIVE_DISTRICT_GAME_CONTRACT_ADDRESS');

  if (!process.env.CREATIVE_DISTRICT_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CREATIVE_DISTRICT_GAME_CONTRACT_ADDRESS not set in environment.');
