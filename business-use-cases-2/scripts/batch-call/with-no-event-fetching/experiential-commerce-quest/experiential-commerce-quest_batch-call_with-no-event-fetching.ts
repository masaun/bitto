...
const contractDetails = parseContractIdentifier(
  process.env.EXPERIENTIAL_COMMERCE_QUEST_CONTRACT_ADDRESS,
  'experiential-commerce-quest'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for ExperientialCommerceQuest Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  ExperientialCommerceQuest Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: EXPERIENTIAL_COMMERCE_QUEST_CONTRACT_ADDRESS');

  if (!process.env.EXPERIENTIAL_COMMERCE_QUEST_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: EXPERIENTIAL_COMMERCE_QUEST_CONTRACT_ADDRESS not set in environment.');
