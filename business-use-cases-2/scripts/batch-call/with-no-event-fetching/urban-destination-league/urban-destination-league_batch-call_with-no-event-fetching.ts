...
const contractDetails = parseContractIdentifier(
  process.env.URBAN_DESTINATION_LEAGUE_CONTRACT_ADDRESS,
  'urban-destination-league'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for UrbanDestinationLeague Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  UrbanDestinationLeague Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: URBAN_DESTINATION_LEAGUE_CONTRACT_ADDRESS');

  if (!process.env.URBAN_DESTINATION_LEAGUE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: URBAN_DESTINATION_LEAGUE_CONTRACT_ADDRESS not set in environment.');
