...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_INVESTMENT_AWARENESS_CONTRACT_ADDRESS,
  'local-investment-awareness'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalInvestmentAwareness Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalInvestmentAwareness Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_INVESTMENT_AWARENESS_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_INVESTMENT_AWARENESS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_INVESTMENT_AWARENESS_CONTRACT_ADDRESS not set in environment.');
