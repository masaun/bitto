...
const contractDetails = parseContractIdentifier(
  process.env.COMMUNITY_ECONOMIC_QUEST_CONTRACT_ADDRESS,
  'community-economic-quest'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CommunityEconomicQuest Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CommunityEconomicQuest Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: COMMUNITY_ECONOMIC_QUEST_CONTRACT_ADDRESS');

  if (!process.env.COMMUNITY_ECONOMIC_QUEST_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: COMMUNITY_ECONOMIC_QUEST_CONTRACT_ADDRESS not set in environment.');
