...
const contractDetails = parseContractIdentifier(
  process.env.MERCHANT_REVENUE_LEDGER_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for MerchantRevenueLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  MerchantRevenueLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: MERCHANT_REVENUE_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.MERCHANT_REVENUE_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: MERCHANT_REVENUE_LEDGER_CONTRACT_ADDRESS not set in environment.');
