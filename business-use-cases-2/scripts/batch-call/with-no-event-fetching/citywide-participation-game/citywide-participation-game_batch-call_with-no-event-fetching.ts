...
const contractDetails = parseContractIdentifier(
  process.env.CITYWIDE_PARTICIPATION_GAME_CONTRACT_ADDRESS,
  'citywide-participation-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CitywideParticipationGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CitywideParticipationGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITYWIDE_PARTICIPATION_GAME_CONTRACT_ADDRESS');

  if (!process.env.CITYWIDE_PARTICIPATION_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITYWIDE_PARTICIPATION_GAME_CONTRACT_ADDRESS not set in environment.');
