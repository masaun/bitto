...
const contractDetails = parseContractIdentifier(
  process.env.CITY_ENGAGEMENT_SCORE_CONTRACT_ADDRESS,
  'city-engagement-score'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CityEngagementScore Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CityEngagementScore Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_ENGAGEMENT_SCORE_CONTRACT_ADDRESS');

  if (!process.env.CITY_ENGAGEMENT_SCORE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_ENGAGEMENT_SCORE_CONTRACT_ADDRESS not set in environment.');
