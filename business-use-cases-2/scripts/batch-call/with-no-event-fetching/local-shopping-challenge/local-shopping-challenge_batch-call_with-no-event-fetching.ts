...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_SHOPPING_CHALLENGE_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalShoppingChallenge Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalShoppingChallenge Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_SHOPPING_CHALLENGE_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_SHOPPING_CHALLENGE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_SHOPPING_CHALLENGE_CONTRACT_ADDRESS not set in environment.');
