...
const contractDetails = parseContractIdentifier(
  process.env.CITIZEN_DESTINATION_REWARDS_CONTRACT_ADDRESS,
  'citizen-destination-rewards'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CitizenDestinationRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CitizenDestinationRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITIZEN_DESTINATION_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.CITIZEN_DESTINATION_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITIZEN_DESTINATION_REWARDS_CONTRACT_ADDRESS not set in environment.');
