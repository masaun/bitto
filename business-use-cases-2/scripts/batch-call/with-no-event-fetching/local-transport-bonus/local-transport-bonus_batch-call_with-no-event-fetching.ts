...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_TRANSPORT_BONUS_CONTRACT_ADDRESS,
  'local-transport-bonus'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalTransportBonus Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalTransportBonus Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_TRANSPORT_BONUS_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_TRANSPORT_BONUS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_TRANSPORT_BONUS_CONTRACT_ADDRESS not set in environment.');
