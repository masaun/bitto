...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_POINTS_CLEARINGHOUSE_CONTRACT_ADDRESS,
  'local-points-clearinghouse'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalPointsClearinghouse Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalPointsClearinghouse Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_POINTS_CLEARINGHOUSE_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_POINTS_CLEARINGHOUSE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_POINTS_CLEARINGHOUSE_CONTRACT_ADDRESS not set in environment.');
