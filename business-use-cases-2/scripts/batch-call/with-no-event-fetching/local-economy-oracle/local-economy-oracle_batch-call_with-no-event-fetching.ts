...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_ECONOMY_ORACLE_CONTRACT_ADDRESS,
  'local-economy-oracle'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalEconomyOracle Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalEconomyOracle Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_ECONOMY_ORACLE_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_ECONOMY_ORACLE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_ECONOMY_ORACLE_CONTRACT_ADDRESS not set in environment.');
