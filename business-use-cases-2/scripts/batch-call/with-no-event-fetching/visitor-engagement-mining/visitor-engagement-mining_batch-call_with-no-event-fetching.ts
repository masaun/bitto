...
const contractDetails = parseContractIdentifier(
  process.env.VISITOR_ENGAGEMENT_MINING_CONTRACT_ADDRESS,
  'visitor-engagement-mining'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for VisitorEngagementMining Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  VisitorEngagementMining Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: VISITOR_ENGAGEMENT_MINING_CONTRACT_ADDRESS');

  if (!process.env.VISITOR_ENGAGEMENT_MINING_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: VISITOR_ENGAGEMENT_MINING_CONTRACT_ADDRESS not set in environment.');
