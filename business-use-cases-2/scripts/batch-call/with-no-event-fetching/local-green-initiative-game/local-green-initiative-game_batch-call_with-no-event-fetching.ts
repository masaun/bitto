...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_GREEN_INITIATIVE_GAME_CONTRACT_ADDRESS,
  'local-green-initiative-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalGreenInitiativeGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalGreenInitiativeGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_GREEN_INITIATIVE_GAME_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_GREEN_INITIATIVE_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_GREEN_INITIATIVE_GAME_CONTRACT_ADDRESS not set in environment.');
