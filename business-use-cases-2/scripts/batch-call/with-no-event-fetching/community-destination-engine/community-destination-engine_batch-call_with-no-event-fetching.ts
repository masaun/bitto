...
const contractDetails = parseContractIdentifier(
  process.env.COMMUNITY_DESTINATION_ENGINE_CONTRACT_ADDRESS,
  'community-destination-engine'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CommunityDestinationEngine Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CommunityDestinationEngine Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: COMMUNITY_DESTINATION_ENGINE_CONTRACT_ADDRESS');

  if (!process.env.COMMUNITY_DESTINATION_ENGINE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: COMMUNITY_DESTINATION_ENGINE_CONTRACT_ADDRESS not set in environment.');
