...
const contractDetails = parseContractIdentifier(
  process.env.REGIONAL_ACTIVITY_LEDGER_CONTRACT_ADDRESS,
  'regional-activity-ledger'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for RegionalActivityLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  RegionalActivityLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: REGIONAL_ACTIVITY_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.REGIONAL_ACTIVITY_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: REGIONAL_ACTIVITY_LEDGER_CONTRACT_ADDRESS not set in environment.');
