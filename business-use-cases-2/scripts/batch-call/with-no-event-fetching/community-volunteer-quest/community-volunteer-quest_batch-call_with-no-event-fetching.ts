...
const contractDetails = parseContractIdentifier(
  process.env.COMMUNITY_VOLUNTEER_QUEST_CONTRACT_ADDRESS,
  'community-volunteer-quest'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CommunityVolunteerQuest Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CommunityVolunteerQuest Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: COMMUNITY_VOLUNTEER_QUEST_CONTRACT_ADDRESS');

  if (!process.env.COMMUNITY_VOLUNTEER_QUEST_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: COMMUNITY_VOLUNTEER_QUEST_CONTRACT_ADDRESS not set in environment.');
