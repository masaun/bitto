...
const contractDetails = parseContractIdentifier(
  process.env.CIVIC_LEADERBOARD_ENGINE_CONTRACT_ADDRESS,
  'civic-leaderboard-engine'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CivicLeaderboardEngine Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CivicLeaderboardEngine Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CIVIC_LEADERBOARD_ENGINE_CONTRACT_ADDRESS');

  if (!process.env.CIVIC_LEADERBOARD_ENGINE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CIVIC_LEADERBOARD_ENGINE_CONTRACT_ADDRESS not set in environment.');
