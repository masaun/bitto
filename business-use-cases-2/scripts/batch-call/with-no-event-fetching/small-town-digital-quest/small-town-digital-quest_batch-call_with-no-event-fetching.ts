...
const contractDetails = parseContractIdentifier(
  process.env.SMALL_TOWN_DIGITAL_QUEST_CONTRACT_ADDRESS,
  'small-town-digital-quest'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for SmallTownDigitalQuest Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  SmallTownDigitalQuest Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: SMALL_TOWN_DIGITAL_QUEST_CONTRACT_ADDRESS');

  if (!process.env.SMALL_TOWN_DIGITAL_QUEST_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: SMALL_TOWN_DIGITAL_QUEST_CONTRACT_ADDRESS not set in environment.');
