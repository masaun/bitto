...
const contractDetails = parseContractIdentifier(
  process.env.DOWNTOWN_EXPERIENCE_GAME_CONTRACT_ADDRESS,
  'downtown-experience-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for DowntownExperienceGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  DowntownExperienceGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: DOWNTOWN_EXPERIENCE_GAME_CONTRACT_ADDRESS');

  if (!process.env.DOWNTOWN_EXPERIENCE_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: DOWNTOWN_EXPERIENCE_GAME_CONTRACT_ADDRESS not set in environment.');
