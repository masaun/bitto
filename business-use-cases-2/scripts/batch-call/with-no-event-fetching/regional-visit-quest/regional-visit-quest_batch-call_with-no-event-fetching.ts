...
const contractDetails = parseContractIdentifier(
  process.env.REGIONAL_VISIT_QUEST_CONTRACT_ADDRESS,
  'regional-visit-quest'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for RegionalVisitQuest Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  RegionalVisitQuest Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: REGIONAL_VISIT_QUEST_CONTRACT_ADDRESS');

  if (!process.env.REGIONAL_VISIT_QUEST_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: REGIONAL_VISIT_QUEST_CONTRACT_ADDRESS not set in environment.');
