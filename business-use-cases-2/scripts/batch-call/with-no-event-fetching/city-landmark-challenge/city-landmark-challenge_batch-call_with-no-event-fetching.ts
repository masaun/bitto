...
const contractDetails = parseContractIdentifier(
  process.env.CITY_LANDMARK_CHALLENGE_CONTRACT_ADDRESS,
  'city-landmark-challenge'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CityLandmarkChallenge Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CityLandmarkChallenge Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_LANDMARK_CHALLENGE_CONTRACT_ADDRESS');

  if (!process.env.CITY_LANDMARK_CHALLENGE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_LANDMARK_CHALLENGE_CONTRACT_ADDRESS not set in environment.');
