...
const contractDetails = parseContractIdentifier(
  process.env.WATERFRONT_ACTIVITY_LEAGUE_CONTRACT_ADDRESS,
  'waterfront-activity-league'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for WaterfrontActivityLeague Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  WaterfrontActivityLeague Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: WATERFRONT_ACTIVITY_LEAGUE_CONTRACT_ADDRESS');

  if (!process.env.WATERFRONT_ACTIVITY_LEAGUE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: WATERFRONT_ACTIVITY_LEAGUE_CONTRACT_ADDRESS not set in environment.');
