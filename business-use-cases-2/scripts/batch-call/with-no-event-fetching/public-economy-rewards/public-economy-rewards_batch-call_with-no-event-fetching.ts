...
const contractDetails = parseContractIdentifier(
  process.env.PUBLIC_ECONOMY_REWARDS_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for PublicEconomyRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  PublicEconomyRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: PUBLIC_ECONOMY_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.PUBLIC_ECONOMY_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: PUBLIC_ECONOMY_REWARDS_CONTRACT_ADDRESS not set in environment.');
