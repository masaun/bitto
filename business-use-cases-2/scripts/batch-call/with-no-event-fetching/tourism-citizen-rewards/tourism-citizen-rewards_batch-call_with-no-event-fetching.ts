...
const contractDetails = parseContractIdentifier(
  process.env.TOURISM_CITIZEN_REWARDS_CONTRACT_ADDRESS,
  'tourism-citizen-rewards'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TourismCitizenRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TourismCitizenRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOURISM_CITIZEN_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.TOURISM_CITIZEN_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOURISM_CITIZEN_REWARDS_CONTRACT_ADDRESS not set in environment.');
