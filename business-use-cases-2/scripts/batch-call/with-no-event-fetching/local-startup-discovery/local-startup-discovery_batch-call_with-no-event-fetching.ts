...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_STARTUP_DISCOVERY_CONTRACT_ADDRESS,
  'local-startup-discovery'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalStartupDiscovery Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalStartupDiscovery Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_STARTUP_DISCOVERY_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_STARTUP_DISCOVERY_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_STARTUP_DISCOVERY_CONTRACT_ADDRESS not set in environment.');
