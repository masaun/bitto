...
const contractDetails = parseContractIdentifier(
  process.env.CITY_MARKET_GROWTH_CONTRACT_ADDRESS,
  'city-market-growth'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CityMarketGrowth Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CityMarketGrowth Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_MARKET_GROWTH_CONTRACT_ADDRESS');

  if (!process.env.CITY_MARKET_GROWTH_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_MARKET_GROWTH_CONTRACT_ADDRESS not set in environment.');
