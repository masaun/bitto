...
const contractDetails = parseContractIdentifier(
  process.env.DESTINATION_IMPACT_PROTOCOL_CONTRACT_ADDRESS,
  'destination-impact-protocol'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for DestinationImpactProtocol Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  DestinationImpactProtocol Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: DESTINATION_IMPACT_PROTOCOL_CONTRACT_ADDRESS');

  if (!process.env.DESTINATION_IMPACT_PROTOCOL_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: DESTINATION_IMPACT_PROTOCOL_CONTRACT_ADDRESS not set in environment.');
