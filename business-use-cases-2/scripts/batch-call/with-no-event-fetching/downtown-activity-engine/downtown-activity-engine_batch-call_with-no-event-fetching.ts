...
const contractDetails = parseContractIdentifier(
  process.env.DOWNTOWN_ACTIVITY_ENGINE_CONTRACT_ADDRESS,
  'downtown-activity-engine'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for DowntownActivityEngine Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  DowntownActivityEngine Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: DOWNTOWN_ACTIVITY_ENGINE_CONTRACT_ADDRESS');

  if (!process.env.DOWNTOWN_ACTIVITY_ENGINE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: DOWNTOWN_ACTIVITY_ENGINE_CONTRACT_ADDRESS not set in environment.');
