...
const contractDetails = parseContractIdentifier(
  process.env.TOWN_COMMERCE_ENGINE_CONTRACT_ADDRESS,
  'town-commerce-engine'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TownCommerceEngine Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TownCommerceEngine Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOWN_COMMERCE_ENGINE_CONTRACT_ADDRESS');

  if (!process.env.TOWN_COMMERCE_ENGINE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOWN_COMMERCE_ENGINE_CONTRACT_ADDRESS not set in environment.');
