...
const contractDetails = parseContractIdentifier(
  process.env.URBAN_ENGAGEMENT_PROTOCOL_CONTRACT_ADDRESS,
  'urban-engagement-protocol'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for UrbanEngagementProtocol Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  UrbanEngagementProtocol Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: URBAN_ENGAGEMENT_PROTOCOL_CONTRACT_ADDRESS');

  if (!process.env.URBAN_ENGAGEMENT_PROTOCOL_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: URBAN_ENGAGEMENT_PROTOCOL_CONTRACT_ADDRESS not set in environment.');
