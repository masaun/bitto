...
const contractDetails = parseContractIdentifier(
  process.env.DESTINATION_GROWTH_ENGINE_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for DestinationGrowthEngine Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  DestinationGrowthEngine Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: DESTINATION_GROWTH_ENGINE_CONTRACT_ADDRESS');

  if (!process.env.DESTINATION_GROWTH_ENGINE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: DESTINATION_GROWTH_ENGINE_CONTRACT_ADDRESS not set in environment.');
