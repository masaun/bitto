...
const contractDetails = parseContractIdentifier(
  process.env.CITY_ENGAGEMENT_MARKET_CONTRACT_ADDRESS,
  'city-engagement-market'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CityEngagementMarket Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CityEngagementMarket Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_ENGAGEMENT_MARKET_CONTRACT_ADDRESS');

  if (!process.env.CITY_ENGAGEMENT_MARKET_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_ENGAGEMENT_MARKET_CONTRACT_ADDRESS not set in environment.');
