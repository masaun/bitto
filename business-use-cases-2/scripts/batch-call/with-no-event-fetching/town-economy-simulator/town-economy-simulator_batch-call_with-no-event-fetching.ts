...
const contractDetails = parseContractIdentifier(
  process.env.TOWN_ECONOMY_SIMULATOR_CONTRACT_ADDRESS,
  'town-economy-simulator'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TownEconomySimulator Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TownEconomySimulator Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOWN_ECONOMY_SIMULATOR_CONTRACT_ADDRESS');

  if (!process.env.TOWN_ECONOMY_SIMULATOR_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOWN_ECONOMY_SIMULATOR_CONTRACT_ADDRESS not set in environment.');
