...
const contractDetails = parseContractIdentifier(
  process.env.REGIONAL_DISCOVERY_PASS_CONTRACT_ADDRESS,
  'regional-discovery-pass'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for RegionalDiscoveryPass Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  RegionalDiscoveryPass Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: REGIONAL_DISCOVERY_PASS_CONTRACT_ADDRESS');

  if (!process.env.REGIONAL_DISCOVERY_PASS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: REGIONAL_DISCOVERY_PASS_CONTRACT_ADDRESS not set in environment.');
