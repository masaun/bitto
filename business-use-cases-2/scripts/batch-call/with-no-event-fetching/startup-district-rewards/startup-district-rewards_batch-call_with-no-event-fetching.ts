...
const contractDetails = parseContractIdentifier(
  process.env.STARTUP_DISTRICT_REWARDS_CONTRACT_ADDRESS,
  'startup-district-rewards'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for StartupDistrictRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  StartupDistrictRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: STARTUP_DISTRICT_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.STARTUP_DISTRICT_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: STARTUP_DISTRICT_REWARDS_CONTRACT_ADDRESS not set in environment.');
