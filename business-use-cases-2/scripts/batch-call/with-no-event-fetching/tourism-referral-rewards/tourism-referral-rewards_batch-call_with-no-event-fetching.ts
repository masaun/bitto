...
const contractDetails = parseContractIdentifier(
  process.env.TOURISM_REFERRAL_REWARDS_CONTRACT_ADDRESS,
  'tourism-referral-rewards'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TourismReferralRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TourismReferralRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOURISM_REFERRAL_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.TOURISM_REFERRAL_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOURISM_REFERRAL_REWARDS_CONTRACT_ADDRESS not set in environment.');
