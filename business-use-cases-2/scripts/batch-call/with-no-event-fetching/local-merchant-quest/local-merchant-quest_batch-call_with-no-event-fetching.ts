...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_MERCHANT_QUEST_CONTRACT_ADDRESS,
  'local-merchant-quest'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalMerchantQuest Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalMerchantQuest Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_MERCHANT_QUEST_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_MERCHANT_QUEST_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_MERCHANT_QUEST_CONTRACT_ADDRESS not set in environment.');
