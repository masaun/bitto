...
const contractDetails = parseContractIdentifier(
  process.env.CITYWIDE_EXPERIENCE_PROTOCOL_CONTRACT_ADDRESS,
  'citywide-experience-protocol'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CitywideExperienceProtocol Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CitywideExperienceProtocol Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITYWIDE_EXPERIENCE_PROTOCOL_CONTRACT_ADDRESS');

  if (!process.env.CITYWIDE_EXPERIENCE_PROTOCOL_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITYWIDE_EXPERIENCE_PROTOCOL_CONTRACT_ADDRESS not set in environment.');
