...
const contractDetails = parseContractIdentifier(
  process.env.HERITAGE_WALK_LEAGUE_CONTRACT_ADDRESS,
  'heritage-walk-league'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for HeritageWalkLeague Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  HeritageWalkLeague Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: HERITAGE_WALK_LEAGUE_CONTRACT_ADDRESS');

  if (!process.env.HERITAGE_WALK_LEAGUE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: HERITAGE_WALK_LEAGUE_CONTRACT_ADDRESS not set in environment.');
