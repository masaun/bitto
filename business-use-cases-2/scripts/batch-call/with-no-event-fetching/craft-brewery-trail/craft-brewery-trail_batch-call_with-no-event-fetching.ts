...
const contractDetails = parseContractIdentifier(
  process.env.CRAFT_BREWERY_TRAIL_CONTRACT_ADDRESS,
  'craft-brewery-trail'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CraftBreweryTrail Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CraftBreweryTrail Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CRAFT_BREWERY_TRAIL_CONTRACT_ADDRESS');

  if (!process.env.CRAFT_BREWERY_TRAIL_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CRAFT_BREWERY_TRAIL_CONTRACT_ADDRESS not set in environment.');
