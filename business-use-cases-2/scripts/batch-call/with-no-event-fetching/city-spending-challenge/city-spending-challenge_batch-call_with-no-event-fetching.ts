...
const contractDetails = parseContractIdentifier(
  process.env.CITY_SPENDING_CHALLENGE_CONTRACT_ADDRESS,
  'city-spending-challenge'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CitySpendingChallenge Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CitySpendingChallenge Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_SPENDING_CHALLENGE_CONTRACT_ADDRESS');

  if (!process.env.CITY_SPENDING_CHALLENGE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_SPENDING_CHALLENGE_CONTRACT_ADDRESS not set in environment.');
