...
const contractDetails = parseContractIdentifier(
  process.env.HOSPITALITY_GROWTH_GAME_CONTRACT_ADDRESS,
  'hospitality-growth-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for HospitalityGrowthGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  HospitalityGrowthGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: HOSPITALITY_GROWTH_GAME_CONTRACT_ADDRESS');

  if (!process.env.HOSPITALITY_GROWTH_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: HOSPITALITY_GROWTH_GAME_CONTRACT_ADDRESS not set in environment.');
