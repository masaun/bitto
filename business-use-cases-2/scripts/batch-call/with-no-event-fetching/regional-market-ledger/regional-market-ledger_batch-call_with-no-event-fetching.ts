...
const contractDetails = parseContractIdentifier(
  process.env.REGIONAL_MARKET_LEDGER_CONTRACT_ADDRESS,
  'regional-market-ledger'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for RegionalMarketLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  RegionalMarketLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: REGIONAL_MARKET_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.REGIONAL_MARKET_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: REGIONAL_MARKET_LEDGER_CONTRACT_ADDRESS not set in environment.');
