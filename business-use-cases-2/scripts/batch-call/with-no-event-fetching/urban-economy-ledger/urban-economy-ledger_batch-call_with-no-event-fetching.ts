...
const contractDetails = parseContractIdentifier(
  process.env.URBAN_ECONOMY_LEDGER_CONTRACT_ADDRESS,
  'urban-economy-ledger'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for UrbanEconomyLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  UrbanEconomyLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: URBAN_ECONOMY_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.URBAN_ECONOMY_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: URBAN_ECONOMY_LEDGER_CONTRACT_ADDRESS not set in environment.');
