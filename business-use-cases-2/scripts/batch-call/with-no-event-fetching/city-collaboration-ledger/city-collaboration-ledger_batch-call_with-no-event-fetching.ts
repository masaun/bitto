...
const contractDetails = parseContractIdentifier(
  process.env.CITY_COLLABORATION_LEDGER_CONTRACT_ADDRESS,
  'city-collaboration-ledger'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CityCollaborationLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CityCollaborationLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_COLLABORATION_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.CITY_COLLABORATION_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_COLLABORATION_LEDGER_CONTRACT_ADDRESS not set in environment.');
