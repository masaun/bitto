...
const contractDetails = parseContractIdentifier(
  process.env.TOURISM_MARKETPLACE_GAME_CONTRACT_ADDRESS,
  'tourism-marketplace-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TourismMarketplaceGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TourismMarketplaceGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOURISM_MARKETPLACE_GAME_CONTRACT_ADDRESS');

  if (!process.env.TOURISM_MARKETPLACE_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOURISM_MARKETPLACE_GAME_CONTRACT_ADDRESS not set in environment.');
