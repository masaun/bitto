...
const contractDetails = parseContractIdentifier(
  process.env.EVENT_ATTENDANCE_REWARDS_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for EventAttendanceRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  EventAttendanceRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: EVENT_ATTENDANCE_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.EVENT_ATTENDANCE_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: EVENT_ATTENDANCE_REWARDS_CONTRACT_ADDRESS not set in environment.');
