...
const contractDetails = parseContractIdentifier(
  process.env.NEIGHBORHOOD_LOYALTY_GAME_CONTRACT_ADDRESS,
  'neighborhood-loyalty-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for NeighborhoodLoyaltyGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  NeighborhoodLoyaltyGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: NEIGHBORHOOD_LOYALTY_GAME_CONTRACT_ADDRESS');

  if (!process.env.NEIGHBORHOOD_LOYALTY_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: NEIGHBORHOOD_LOYALTY_GAME_CONTRACT_ADDRESS not set in environment.');
