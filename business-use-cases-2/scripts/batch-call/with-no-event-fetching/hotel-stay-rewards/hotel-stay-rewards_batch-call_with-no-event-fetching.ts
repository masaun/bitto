...
const contractDetails = parseContractIdentifier(
  process.env.HOTEL_STAY_REWARDS_CONTRACT_ADDRESS,
  'hotel-stay-rewards'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for HotelStayRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  HotelStayRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: HOTEL_STAY_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.HOTEL_STAY_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: HOTEL_STAY_REWARDS_CONTRACT_ADDRESS not set in environment.');
