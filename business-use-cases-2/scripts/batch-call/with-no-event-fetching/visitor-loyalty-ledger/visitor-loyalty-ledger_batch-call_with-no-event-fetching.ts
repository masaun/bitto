...
const contractDetails = parseContractIdentifier(
  process.env.VISITOR_LOYALTY_LEDGER_CONTRACT_ADDRESS,
  'visitor-loyalty-ledger'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for VisitorLoyaltyLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  VisitorLoyaltyLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: VISITOR_LOYALTY_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.VISITOR_LOYALTY_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: VISITOR_LOYALTY_LEDGER_CONTRACT_ADDRESS not set in environment.');
