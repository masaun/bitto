...
const contractDetails = parseContractIdentifier(
  process.env.TOWN_ACTIVITY_REWARDS_CONTRACT_ADDRESS,
  'town-activity-rewards'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TownActivityRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TownActivityRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOWN_ACTIVITY_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.TOWN_ACTIVITY_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOWN_ACTIVITY_REWARDS_CONTRACT_ADDRESS not set in environment.');
