...
const contractDetails = parseContractIdentifier(
  process.env.PEDESTRIAN_ZONE_GAME_CONTRACT_ADDRESS,
  'pedestrian-zone-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for PedestrianZoneGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  PedestrianZoneGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: PEDESTRIAN_ZONE_GAME_CONTRACT_ADDRESS');

  if (!process.env.PEDESTRIAN_ZONE_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: PEDESTRIAN_ZONE_GAME_CONTRACT_ADDRESS not set in environment.');
