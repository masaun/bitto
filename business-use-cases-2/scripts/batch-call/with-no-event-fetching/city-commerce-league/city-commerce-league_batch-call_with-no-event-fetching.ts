...
const contractDetails = parseContractIdentifier(
  process.env.CITY_COMMERCE_LEAGUE_CONTRACT_ADDRESS,
  'city-commerce-league'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CityCommerceLeague Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CityCommerceLeague Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_COMMERCE_LEAGUE_CONTRACT_ADDRESS');

  if (!process.env.CITY_COMMERCE_LEAGUE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_COMMERCE_LEAGUE_CONTRACT_ADDRESS not set in environment.');
