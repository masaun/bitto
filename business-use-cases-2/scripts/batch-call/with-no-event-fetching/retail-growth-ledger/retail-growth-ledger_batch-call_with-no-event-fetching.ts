...
const contractDetails = parseContractIdentifier(
  process.env.RETAIL_GROWTH_LEDGER_CONTRACT_ADDRESS,
  'retail-growth-ledger'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for RetailGrowthLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  RetailGrowthLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: RETAIL_GROWTH_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.RETAIL_GROWTH_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: RETAIL_GROWTH_LEDGER_CONTRACT_ADDRESS not set in environment.');
