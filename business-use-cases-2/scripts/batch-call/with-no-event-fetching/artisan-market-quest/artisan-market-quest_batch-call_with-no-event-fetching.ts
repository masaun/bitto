...
const contractDetails = parseContractIdentifier(
  process.env.ARTISAN_MARKET_QUEST_CONTRACT_ADDRESS,
  'artisan-market-quest'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for ArtisanMarketQuest Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  ArtisanMarketQuest Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: ARTISAN_MARKET_QUEST_CONTRACT_ADDRESS');

  if (!process.env.ARTISAN_MARKET_QUEST_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: ARTISAN_MARKET_QUEST_CONTRACT_ADDRESS not set in environment.');
