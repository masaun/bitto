...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_PROSPERITY_LEDGER_CONTRACT_ADDRESS,
  'local-prosperity-ledger'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalProsperityLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalProsperityLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_PROSPERITY_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_PROSPERITY_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_PROSPERITY_LEDGER_CONTRACT_ADDRESS not set in environment.');
