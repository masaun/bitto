...
const contractDetails = parseContractIdentifier(
  process.env.LIBRARY_VISIT_REWARDS_CONTRACT_ADDRESS,
  'library-visit-rewards'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LibraryVisitRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LibraryVisitRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LIBRARY_VISIT_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.LIBRARY_VISIT_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LIBRARY_VISIT_REWARDS_CONTRACT_ADDRESS not set in environment.');
