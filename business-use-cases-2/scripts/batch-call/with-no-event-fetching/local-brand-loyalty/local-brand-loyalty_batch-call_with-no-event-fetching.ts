...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_BRAND_LOYALTY_CONTRACT_ADDRESS,
  'local-brand-loyalty'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalBrandLoyalty Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalBrandLoyalty Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_BRAND_LOYALTY_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_BRAND_LOYALTY_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_BRAND_LOYALTY_CONTRACT_ADDRESS not set in environment.');
