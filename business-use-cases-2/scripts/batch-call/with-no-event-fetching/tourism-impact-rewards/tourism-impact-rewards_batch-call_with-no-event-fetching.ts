...
const contractDetails = parseContractIdentifier(
  process.env.TOURISM_IMPACT_REWARDS_CONTRACT_ADDRESS,
  'tourism-impact-rewards'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TourismImpactRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TourismImpactRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOURISM_IMPACT_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.TOURISM_IMPACT_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOURISM_IMPACT_REWARDS_CONTRACT_ADDRESS not set in environment.');
