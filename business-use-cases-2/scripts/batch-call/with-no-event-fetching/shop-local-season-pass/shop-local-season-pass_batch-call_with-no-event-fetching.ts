...
const contractDetails = parseContractIdentifier(
  process.env.SHOP_LOCAL_SEASON_PASS_CONTRACT_ADDRESS,
  'shop-local-season-pass'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for ShopLocalSeasonPass Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  ShopLocalSeasonPass Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: SHOP_LOCAL_SEASON_PASS_CONTRACT_ADDRESS');

  if (!process.env.SHOP_LOCAL_SEASON_PASS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: SHOP_LOCAL_SEASON_PASS_CONTRACT_ADDRESS not set in environment.');
