...
const contractDetails = parseContractIdentifier(
  process.env.VISITOR_EXPERIENCE_LEDGER_CONTRACT_ADDRESS,
  'visitor-experience-ledger'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for VisitorExperienceLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  VisitorExperienceLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: VISITOR_EXPERIENCE_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.VISITOR_EXPERIENCE_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: VISITOR_EXPERIENCE_LEDGER_CONTRACT_ADDRESS not set in environment.');
