...
const contractDetails = parseContractIdentifier(
  process.env.TOWN_ECONOMIC_BOOST_CONTRACT_ADDRESS,
  'town-economic-boost'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TownEconomicBoost Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TownEconomicBoost Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOWN_ECONOMIC_BOOST_CONTRACT_ADDRESS');

  if (!process.env.TOWN_ECONOMIC_BOOST_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOWN_ECONOMIC_BOOST_CONTRACT_ADDRESS not set in environment.');
