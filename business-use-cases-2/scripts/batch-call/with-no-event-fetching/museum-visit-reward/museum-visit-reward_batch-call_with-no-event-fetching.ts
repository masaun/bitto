...
const contractDetails = parseContractIdentifier(
  process.env.MUSEUM_VISIT_REWARD_CONTRACT_ADDRESS,
  'museum-visit-reward'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for MuseumVisitReward Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  MuseumVisitReward Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: MUSEUM_VISIT_REWARD_CONTRACT_ADDRESS');

  if (!process.env.MUSEUM_VISIT_REWARD_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: MUSEUM_VISIT_REWARD_CONTRACT_ADDRESS not set in environment.');
