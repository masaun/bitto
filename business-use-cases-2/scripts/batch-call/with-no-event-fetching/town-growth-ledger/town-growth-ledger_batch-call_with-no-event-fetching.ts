...
const contractDetails = parseContractIdentifier(
  process.env.TOWN_GROWTH_LEDGER_CONTRACT_ADDRESS,
  'town-growth-ledger'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TownGrowthLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TownGrowthLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOWN_GROWTH_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.TOWN_GROWTH_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOWN_GROWTH_LEDGER_CONTRACT_ADDRESS not set in environment.');
