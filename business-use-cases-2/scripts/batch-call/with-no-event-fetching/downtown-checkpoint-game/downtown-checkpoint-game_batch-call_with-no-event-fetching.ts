...
const contractDetails = parseContractIdentifier(
  process.env.DOWNTOWN_CHECKPOINT_GAME_CONTRACT_ADDRESS,
  'downtown-checkpoint-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for DowntownCheckpointGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  DowntownCheckpointGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: DOWNTOWN_CHECKPOINT_GAME_CONTRACT_ADDRESS');

  if (!process.env.DOWNTOWN_CHECKPOINT_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: DOWNTOWN_CHECKPOINT_GAME_CONTRACT_ADDRESS not set in environment.');
