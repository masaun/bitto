...
const contractDetails = parseContractIdentifier(
  process.env.VISITOR_IMPACT_TRACKER_CONTRACT_ADDRESS,
  'visitor-impact-tracker'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for VisitorImpactTracker Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  VisitorImpactTracker Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: VISITOR_IMPACT_TRACKER_CONTRACT_ADDRESS');

  if (!process.env.VISITOR_IMPACT_TRACKER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: VISITOR_IMPACT_TRACKER_CONTRACT_ADDRESS not set in environment.');
