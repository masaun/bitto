...
const contractDetails = parseContractIdentifier(
  process.env.COMMUNITY_PROSPERITY_LEDGER_CONTRACT_ADDRESS,
  'community-prosperity-ledger'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CommunityProsperityLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CommunityProsperityLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: COMMUNITY_PROSPERITY_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.COMMUNITY_PROSPERITY_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: COMMUNITY_PROSPERITY_LEDGER_CONTRACT_ADDRESS not set in environment.');
