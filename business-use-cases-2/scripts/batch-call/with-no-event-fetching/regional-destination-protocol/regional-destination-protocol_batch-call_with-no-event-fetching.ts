...
const contractDetails = parseContractIdentifier(
  process.env.REGIONAL_DESTINATION_PROTOCOL_CONTRACT_ADDRESS,
  'regional-destination-protocol'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for RegionalDestinationProtocol Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  RegionalDestinationProtocol Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: REGIONAL_DESTINATION_PROTOCOL_CONTRACT_ADDRESS');

  if (!process.env.REGIONAL_DESTINATION_PROTOCOL_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: REGIONAL_DESTINATION_PROTOCOL_CONTRACT_ADDRESS not set in environment.');
