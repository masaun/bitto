...
const contractDetails = parseContractIdentifier(
  process.env.MERCHANT_GROWTH_PROTOCOL_CONTRACT_ADDRESS,
  'merchant-growth-protocol'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for MerchantGrowthProtocol Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  MerchantGrowthProtocol Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: MERCHANT_GROWTH_PROTOCOL_CONTRACT_ADDRESS');

  if (!process.env.MERCHANT_GROWTH_PROTOCOL_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: MERCHANT_GROWTH_PROTOCOL_CONTRACT_ADDRESS not set in environment.');
