...
const contractDetails = parseContractIdentifier(
  process.env.CITY_ECONOMIC_LEAGUE_CONTRACT_ADDRESS,
  'city-economic-league'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CityEconomicLeague Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CityEconomicLeague Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_ECONOMIC_LEAGUE_CONTRACT_ADDRESS');

  if (!process.env.CITY_ECONOMIC_LEAGUE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_ECONOMIC_LEAGUE_CONTRACT_ADDRESS not set in environment.');
