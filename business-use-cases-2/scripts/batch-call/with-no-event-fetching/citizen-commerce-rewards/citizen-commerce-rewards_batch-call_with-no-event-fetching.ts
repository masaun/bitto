...
const contractDetails = parseContractIdentifier(
  process.env.CITIZEN_COMMERCE_REWARDS_CONTRACT_ADDRESS,
  'citizen-commerce-rewards'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CitizenCommerceRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CitizenCommerceRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITIZEN_COMMERCE_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.CITIZEN_COMMERCE_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITIZEN_COMMERCE_REWARDS_CONTRACT_ADDRESS not set in environment.');
