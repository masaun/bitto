...
const contractDetails = parseContractIdentifier(
  process.env.DOWNTOWN_LOYALTY_NETWORK_CONTRACT_ADDRESS,
  'downtown-loyalty-network'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for DowntownLoyaltyNetwork Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  DowntownLoyaltyNetwork Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: DOWNTOWN_LOYALTY_NETWORK_CONTRACT_ADDRESS');

  if (!process.env.DOWNTOWN_LOYALTY_NETWORK_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: DOWNTOWN_LOYALTY_NETWORK_CONTRACT_ADDRESS not set in environment.');
