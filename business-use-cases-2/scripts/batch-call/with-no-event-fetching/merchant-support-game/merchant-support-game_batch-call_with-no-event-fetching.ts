...
const contractDetails = parseContractIdentifier(
  process.env.MERCHANT_SUPPORT_GAME_CONTRACT_ADDRESS,
  'merchant-support-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for MerchantSupportGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  MerchantSupportGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: MERCHANT_SUPPORT_GAME_CONTRACT_ADDRESS');

  if (!process.env.MERCHANT_SUPPORT_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: MERCHANT_SUPPORT_GAME_CONTRACT_ADDRESS not set in environment.');
