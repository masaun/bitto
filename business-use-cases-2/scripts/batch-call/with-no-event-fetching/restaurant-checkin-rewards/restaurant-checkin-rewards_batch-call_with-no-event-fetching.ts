...
const contractDetails = parseContractIdentifier(
  process.env.RESTAURANT_CHECKIN_REWARDS_CONTRACT_ADDRESS,
  'restaurant-checkin-rewards'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for RestaurantCheckinRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  RestaurantCheckinRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: RESTAURANT_CHECKIN_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.RESTAURANT_CHECKIN_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: RESTAURANT_CHECKIN_REWARDS_CONTRACT_ADDRESS not set in environment.');
