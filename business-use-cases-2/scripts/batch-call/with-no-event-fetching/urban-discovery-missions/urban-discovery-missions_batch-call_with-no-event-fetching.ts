...
const contractDetails = parseContractIdentifier(
  process.env.URBAN_DISCOVERY_MISSIONS_CONTRACT_ADDRESS,
  'urban-discovery-missions'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for UrbanDiscoveryMissions Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  UrbanDiscoveryMissions Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: URBAN_DISCOVERY_MISSIONS_CONTRACT_ADDRESS');

  if (!process.env.URBAN_DISCOVERY_MISSIONS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: URBAN_DISCOVERY_MISSIONS_CONTRACT_ADDRESS not set in environment.');
