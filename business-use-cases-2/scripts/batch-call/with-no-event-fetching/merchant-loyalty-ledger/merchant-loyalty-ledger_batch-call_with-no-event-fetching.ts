...
const contractDetails = parseContractIdentifier(
  process.env.MERCHANT_LOYALTY_LEDGER_CONTRACT_ADDRESS,
  'merchant-loyalty-ledger'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for MerchantLoyaltyLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  MerchantLoyaltyLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: MERCHANT_LOYALTY_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.MERCHANT_LOYALTY_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: MERCHANT_LOYALTY_LEDGER_CONTRACT_ADDRESS not set in environment.');
