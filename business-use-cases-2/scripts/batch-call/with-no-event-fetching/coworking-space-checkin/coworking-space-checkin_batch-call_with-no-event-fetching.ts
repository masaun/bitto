...
const contractDetails = parseContractIdentifier(
  process.env.COWORKING_SPACE_CHECKIN_CONTRACT_ADDRESS,
  'coworking-space-checkin'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CoworkingSpaceCheckin Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CoworkingSpaceCheckin Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: COWORKING_SPACE_CHECKIN_CONTRACT_ADDRESS');

  if (!process.env.COWORKING_SPACE_CHECKIN_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: COWORKING_SPACE_CHECKIN_CONTRACT_ADDRESS not set in environment.');
