...
const contractDetails = parseContractIdentifier(
  process.env.HOSPITALITY_COMMERCE_GAME_CONTRACT_ADDRESS,
  'hospitality-commerce-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for HospitalityCommerceGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  HospitalityCommerceGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: HOSPITALITY_COMMERCE_GAME_CONTRACT_ADDRESS');

  if (!process.env.HOSPITALITY_COMMERCE_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: HOSPITALITY_COMMERCE_GAME_CONTRACT_ADDRESS not set in environment.');
