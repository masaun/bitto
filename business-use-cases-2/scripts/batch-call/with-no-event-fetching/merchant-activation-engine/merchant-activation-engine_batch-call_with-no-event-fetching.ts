...
const contractDetails = parseContractIdentifier(
  process.env.MERCHANT_ACTIVATION_ENGINE_CONTRACT_ADDRESS,
  'merchant-activation-engine'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for MerchantActivationEngine Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  MerchantActivationEngine Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: MERCHANT_ACTIVATION_ENGINE_CONTRACT_ADDRESS');

  if (!process.env.MERCHANT_ACTIVATION_ENGINE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: MERCHANT_ACTIVATION_ENGINE_CONTRACT_ADDRESS not set in environment.');
