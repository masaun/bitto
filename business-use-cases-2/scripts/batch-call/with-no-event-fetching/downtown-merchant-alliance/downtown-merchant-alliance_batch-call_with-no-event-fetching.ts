...
const contractDetails = parseContractIdentifier(
  process.env.DOWNTOWN_MERCHANT_ALLIANCE_CONTRACT_ADDRESS,
  'downtown-merchant-alliance'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for DowntownMerchantAlliance Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  DowntownMerchantAlliance Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: DOWNTOWN_MERCHANT_ALLIANCE_CONTRACT_ADDRESS');

  if (!process.env.DOWNTOWN_MERCHANT_ALLIANCE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: DOWNTOWN_MERCHANT_ALLIANCE_CONTRACT_ADDRESS not set in environment.');
