...
const contractDetails = parseContractIdentifier(
  process.env.URBAN_MERCHANT_NETWORK_CONTRACT_ADDRESS,
  'urban-merchant-network'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for UrbanMerchantNetwork Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  UrbanMerchantNetwork Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: URBAN_MERCHANT_NETWORK_CONTRACT_ADDRESS');

  if (!process.env.URBAN_MERCHANT_NETWORK_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: URBAN_MERCHANT_NETWORK_CONTRACT_ADDRESS not set in environment.');
