...
const contractDetails = parseContractIdentifier(
  process.env.CITY_HACKATHON_GAME_CONTRACT_ADDRESS,
  'city-hackathon-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CityHackathonGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CityHackathonGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_HACKATHON_GAME_CONTRACT_ADDRESS');

  if (!process.env.CITY_HACKATHON_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_HACKATHON_GAME_CONTRACT_ADDRESS not set in environment.');
