...
const contractDetails = parseContractIdentifier(
  process.env.TOURISM_SEASONAL_QUEST_CONTRACT_ADDRESS,
  'tourism-seasonal-quest'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TourismSeasonalQuest Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TourismSeasonalQuest Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOURISM_SEASONAL_QUEST_CONTRACT_ADDRESS');

  if (!process.env.TOURISM_SEASONAL_QUEST_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOURISM_SEASONAL_QUEST_CONTRACT_ADDRESS not set in environment.');
