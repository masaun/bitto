...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_DESTINATION_REWARDS_CONTRACT_ADDRESS,
  'local-destination-rewards'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalDestinationRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalDestinationRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_DESTINATION_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_DESTINATION_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_DESTINATION_REWARDS_CONTRACT_ADDRESS not set in environment.');
