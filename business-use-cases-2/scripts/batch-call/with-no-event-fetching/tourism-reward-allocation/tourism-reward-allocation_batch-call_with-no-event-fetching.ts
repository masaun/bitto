...
const contractDetails = parseContractIdentifier(
  process.env.TOURISM_REWARD_ALLOCATION_CONTRACT_ADDRESS,
  'tourism-reward-allocation'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TourismRewardAllocation Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TourismRewardAllocation Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOURISM_REWARD_ALLOCATION_CONTRACT_ADDRESS');

  if (!process.env.TOURISM_REWARD_ALLOCATION_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOURISM_REWARD_ALLOCATION_CONTRACT_ADDRESS not set in environment.');
