...
const contractDetails = parseContractIdentifier(
  process.env.TOWN_HISTORY_CHALLENGE_CONTRACT_ADDRESS,
  'town-history-challenge'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TownHistoryChallenge Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TownHistoryChallenge Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOWN_HISTORY_CHALLENGE_CONTRACT_ADDRESS');

  if (!process.env.TOWN_HISTORY_CHALLENGE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOWN_HISTORY_CHALLENGE_CONTRACT_ADDRESS not set in environment.');
