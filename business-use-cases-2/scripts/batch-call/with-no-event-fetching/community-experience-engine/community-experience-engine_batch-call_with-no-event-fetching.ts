...
const contractDetails = parseContractIdentifier(
  process.env.COMMUNITY_EXPERIENCE_ENGINE_CONTRACT_ADDRESS,
  'community-experience-engine'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CommunityExperienceEngine Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CommunityExperienceEngine Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: COMMUNITY_EXPERIENCE_ENGINE_CONTRACT_ADDRESS');

  if (!process.env.COMMUNITY_EXPERIENCE_ENGINE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: COMMUNITY_EXPERIENCE_ENGINE_CONTRACT_ADDRESS not set in environment.');
