...
const contractDetails = parseContractIdentifier(
  process.env.CITY_IMPACT_REWARDS_CONTRACT_ADDRESS,
  'city-impact-rewards'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CityImpactRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CityImpactRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_IMPACT_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.CITY_IMPACT_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_IMPACT_REWARDS_CONTRACT_ADDRESS not set in environment.');
