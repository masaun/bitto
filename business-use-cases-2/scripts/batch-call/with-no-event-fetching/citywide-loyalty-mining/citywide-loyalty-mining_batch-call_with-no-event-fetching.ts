...
const contractDetails = parseContractIdentifier(
  process.env.CITYWIDE_LOYALTY_MINING_CONTRACT_ADDRESS,
  'citywide-loyalty-mining'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CitywideLoyaltyMining Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CitywideLoyaltyMining Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITYWIDE_LOYALTY_MINING_CONTRACT_ADDRESS');

  if (!process.env.CITYWIDE_LOYALTY_MINING_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITYWIDE_LOYALTY_MINING_CONTRACT_ADDRESS not set in environment.');
