...
const contractDetails = parseContractIdentifier(
  process.env.NEIGHBORHOOD_EXPLORATION_LEAGUE_CONTRACT_ADDRESS,
  'neighborhood-exploration-league'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for NeighborhoodExplorationLeague Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  NeighborhoodExplorationLeague Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: NEIGHBORHOOD_EXPLORATION_LEAGUE_CONTRACT_ADDRESS');

  if (!process.env.NEIGHBORHOOD_EXPLORATION_LEAGUE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: NEIGHBORHOOD_EXPLORATION_LEAGUE_CONTRACT_ADDRESS not set in environment.');
