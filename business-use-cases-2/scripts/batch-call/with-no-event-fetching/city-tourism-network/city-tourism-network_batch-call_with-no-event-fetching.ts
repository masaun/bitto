...
const contractDetails = parseContractIdentifier(
  process.env.CITY_TOURISM_NETWORK_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CityTourismNetwork Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CityTourismNetwork Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_TOURISM_NETWORK_CONTRACT_ADDRESS');

  if (!process.env.CITY_TOURISM_NETWORK_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_TOURISM_NETWORK_CONTRACT_ADDRESS not set in environment.');
