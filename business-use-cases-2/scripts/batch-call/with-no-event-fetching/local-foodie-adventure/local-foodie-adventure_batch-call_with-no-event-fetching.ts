...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_FOODIE_ADVENTURE_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalFoodieAdventure Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalFoodieAdventure Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_FOODIE_ADVENTURE_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_FOODIE_ADVENTURE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_FOODIE_ADVENTURE_CONTRACT_ADDRESS not set in environment.');
