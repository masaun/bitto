...
const contractDetails = parseContractIdentifier(
  process.env.COMMUNITY_LOYALTY_LEDGER_CONTRACT_ADDRESS,
  'community-loyalty-ledger'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CommunityLoyaltyLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CommunityLoyaltyLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: COMMUNITY_LOYALTY_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.COMMUNITY_LOYALTY_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: COMMUNITY_LOYALTY_LEDGER_CONTRACT_ADDRESS not set in environment.');
