...
const contractDetails = parseContractIdentifier(
  process.env.ENTREPRENEURSHIP_TRAIL_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for EntrepreneurshipTrail Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  EntrepreneurshipTrail Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: ENTREPRENEURSHIP_TRAIL_CONTRACT_ADDRESS');

  if (!process.env.ENTREPRENEURSHIP_TRAIL_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: ENTREPRENEURSHIP_TRAIL_CONTRACT_ADDRESS not set in environment.');
