...
const contractDetails = parseContractIdentifier(
  process.env.REGIONAL_GROWTH_QUEST_CONTRACT_ADDRESS,
  'regional-growth-quest'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for RegionalGrowthQuest Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  RegionalGrowthQuest Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: REGIONAL_GROWTH_QUEST_CONTRACT_ADDRESS');

  if (!process.env.REGIONAL_GROWTH_QUEST_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: REGIONAL_GROWTH_QUEST_CONTRACT_ADDRESS not set in environment.');
