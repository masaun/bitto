...
const contractDetails = parseContractIdentifier(
  process.env.YOUTH_CITY_EXPLORER_CONTRACT_ADDRESS,
  'youth-city-explorer'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for YouthCityExplorer Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  YouthCityExplorer Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: YOUTH_CITY_EXPLORER_CONTRACT_ADDRESS');

  if (!process.env.YOUTH_CITY_EXPLORER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: YOUTH_CITY_EXPLORER_CONTRACT_ADDRESS not set in environment.');
