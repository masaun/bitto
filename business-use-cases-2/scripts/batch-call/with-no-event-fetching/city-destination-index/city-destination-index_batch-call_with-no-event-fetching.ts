...
const contractDetails = parseContractIdentifier(
  process.env.CITY_DESTINATION_INDEX_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CityDestinationIndex Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CityDestinationIndex Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_DESTINATION_INDEX_CONTRACT_ADDRESS');

  if (!process.env.CITY_DESTINATION_INDEX_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_DESTINATION_INDEX_CONTRACT_ADDRESS not set in environment.');
