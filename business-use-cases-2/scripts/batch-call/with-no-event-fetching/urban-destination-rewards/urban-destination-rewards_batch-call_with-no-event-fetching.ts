...
const contractDetails = parseContractIdentifier(
  process.env.URBAN_DESTINATION_REWARDS_CONTRACT_ADDRESS,
  'urban-destination-rewards'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for UrbanDestinationRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  UrbanDestinationRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: URBAN_DESTINATION_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.URBAN_DESTINATION_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: URBAN_DESTINATION_REWARDS_CONTRACT_ADDRESS not set in environment.');
