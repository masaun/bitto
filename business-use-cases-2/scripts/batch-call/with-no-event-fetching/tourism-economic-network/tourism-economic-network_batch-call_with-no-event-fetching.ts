...
const contractDetails = parseContractIdentifier(
  process.env.TOURISM_ECONOMIC_NETWORK_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TourismEconomicNetwork Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TourismEconomicNetwork Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOURISM_ECONOMIC_NETWORK_CONTRACT_ADDRESS');

  if (!process.env.TOURISM_ECONOMIC_NETWORK_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOURISM_ECONOMIC_NETWORK_CONTRACT_ADDRESS not set in environment.');
