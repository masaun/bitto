...
const contractDetails = parseContractIdentifier(
  process.env.COMMERCE_IMPACT_LEDGER_CONTRACT_ADDRESS,
  'commerce-impact-ledger'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CommerceImpactLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CommerceImpactLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: COMMERCE_IMPACT_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.COMMERCE_IMPACT_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: COMMERCE_IMPACT_LEDGER_CONTRACT_ADDRESS not set in environment.');
