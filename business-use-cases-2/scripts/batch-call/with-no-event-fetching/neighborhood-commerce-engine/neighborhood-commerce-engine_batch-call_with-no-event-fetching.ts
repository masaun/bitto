...
const contractDetails = parseContractIdentifier(
  process.env.NEIGHBORHOOD_COMMERCE_ENGINE_CONTRACT_ADDRESS,
  'neighborhood-commerce-engine'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for NeighborhoodCommerceEngine Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  NeighborhoodCommerceEngine Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: NEIGHBORHOOD_COMMERCE_ENGINE_CONTRACT_ADDRESS');

  if (!process.env.NEIGHBORHOOD_COMMERCE_ENGINE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: NEIGHBORHOOD_COMMERCE_ENGINE_CONTRACT_ADDRESS not set in environment.');
