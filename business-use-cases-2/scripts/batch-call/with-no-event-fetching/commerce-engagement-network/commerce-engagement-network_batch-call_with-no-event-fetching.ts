...
const contractDetails = parseContractIdentifier(
  process.env.COMMERCE_ENGAGEMENT_NETWORK_CONTRACT_ADDRESS,
  'commerce-engagement-network'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CommerceEngagementNetwork Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CommerceEngagementNetwork Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: COMMERCE_ENGAGEMENT_NETWORK_CONTRACT_ADDRESS');

  if (!process.env.COMMERCE_ENGAGEMENT_NETWORK_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: COMMERCE_ENGAGEMENT_NETWORK_CONTRACT_ADDRESS not set in environment.');
