...
const contractDetails = parseContractIdentifier(
  process.env.REGIONAL_CIVIC_GAME_CONTRACT_ADDRESS,
  'regional-civic-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for RegionalCivicGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  RegionalCivicGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: REGIONAL_CIVIC_GAME_CONTRACT_ADDRESS');

  if (!process.env.REGIONAL_CIVIC_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: REGIONAL_CIVIC_GAME_CONTRACT_ADDRESS not set in environment.');
