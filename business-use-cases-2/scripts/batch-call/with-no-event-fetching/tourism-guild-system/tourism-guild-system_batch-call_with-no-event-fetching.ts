...
const contractDetails = parseContractIdentifier(
  process.env.TOURISM_GUILD_SYSTEM_CONTRACT_ADDRESS,
  'tourism-guild-system'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TourismGuildSystem Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TourismGuildSystem Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOURISM_GUILD_SYSTEM_CONTRACT_ADDRESS');

  if (!process.env.TOURISM_GUILD_SYSTEM_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOURISM_GUILD_SYSTEM_CONTRACT_ADDRESS not set in environment.');
