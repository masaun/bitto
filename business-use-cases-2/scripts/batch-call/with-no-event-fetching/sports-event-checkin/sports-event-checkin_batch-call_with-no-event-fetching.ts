...
const contractDetails = parseContractIdentifier(
  process.env.SPORTS_EVENT_CHECKIN_CONTRACT_ADDRESS,
  'sports-event-checkin'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for SportsEventCheckin Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  SportsEventCheckin Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: SPORTS_EVENT_CHECKIN_CONTRACT_ADDRESS');

  if (!process.env.SPORTS_EVENT_CHECKIN_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: SPORTS_EVENT_CHECKIN_CONTRACT_ADDRESS not set in environment.');
