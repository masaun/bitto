...
const contractDetails = parseContractIdentifier(
  process.env.URBAN_ECONOMIC_MINING_CONTRACT_ADDRESS,
  'urban-economic-mining'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for UrbanEconomicMining Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  UrbanEconomicMining Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: URBAN_ECONOMIC_MINING_CONTRACT_ADDRESS');

  if (!process.env.URBAN_ECONOMIC_MINING_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: URBAN_ECONOMIC_MINING_CONTRACT_ADDRESS not set in environment.');
