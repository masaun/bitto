...
const contractDetails = parseContractIdentifier(
  process.env.PUBLIC_PARK_EXPLORATION_GAME_CONTRACT_ADDRESS,
  'public-park-exploration-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for PublicParkExplorationGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  PublicParkExplorationGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: PUBLIC_PARK_EXPLORATION_GAME_CONTRACT_ADDRESS');

  if (!process.env.PUBLIC_PARK_EXPLORATION_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: PUBLIC_PARK_EXPLORATION_GAME_CONTRACT_ADDRESS not set in environment.');
