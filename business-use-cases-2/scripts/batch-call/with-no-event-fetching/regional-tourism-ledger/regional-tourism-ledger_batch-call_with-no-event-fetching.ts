...
const contractDetails = parseContractIdentifier(
  process.env.REGIONAL_TOURISM_LEDGER_CONTRACT_ADDRESS,
  'regional-tourism-ledger'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for RegionalTourismLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  RegionalTourismLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: REGIONAL_TOURISM_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.REGIONAL_TOURISM_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: REGIONAL_TOURISM_LEDGER_CONTRACT_ADDRESS not set in environment.');
