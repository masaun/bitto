...
const contractDetails = parseContractIdentifier(
  process.env.SUSTAINABLE_SHOPPING_QUEST_CONTRACT_ADDRESS,
  'sustainable-shopping-quest'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for SustainableShoppingQuest Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  SustainableShoppingQuest Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: SUSTAINABLE_SHOPPING_QUEST_CONTRACT_ADDRESS');

  if (!process.env.SUSTAINABLE_SHOPPING_QUEST_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: SUSTAINABLE_SHOPPING_QUEST_CONTRACT_ADDRESS not set in environment.');
