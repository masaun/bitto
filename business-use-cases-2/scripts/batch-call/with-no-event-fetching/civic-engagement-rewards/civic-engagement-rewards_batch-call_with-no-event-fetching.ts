...
const contractDetails = parseContractIdentifier(
  process.env.CIVIC_ENGAGEMENT_REWARDS_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CivicEngagementRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CivicEngagementRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CIVIC_ENGAGEMENT_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.CIVIC_ENGAGEMENT_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CIVIC_ENGAGEMENT_REWARDS_CONTRACT_ADDRESS not set in environment.');
