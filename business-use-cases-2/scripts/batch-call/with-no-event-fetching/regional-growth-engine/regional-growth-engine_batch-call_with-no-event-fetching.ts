...
const contractDetails = parseContractIdentifier(
  process.env.REGIONAL_GROWTH_ENGINE_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for RegionalGrowthEngine Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  RegionalGrowthEngine Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: REGIONAL_GROWTH_ENGINE_CONTRACT_ADDRESS');

  if (!process.env.REGIONAL_GROWTH_ENGINE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: REGIONAL_GROWTH_ENGINE_CONTRACT_ADDRESS not set in environment.');
