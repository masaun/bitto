...
const contractDetails = parseContractIdentifier(
  process.env.MERCHANT_DESTINATION_GAME_CONTRACT_ADDRESS,
  'merchant-destination-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for MerchantDestinationGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  MerchantDestinationGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: MERCHANT_DESTINATION_GAME_CONTRACT_ADDRESS');

  if (!process.env.MERCHANT_DESTINATION_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: MERCHANT_DESTINATION_GAME_CONTRACT_ADDRESS not set in environment.');
