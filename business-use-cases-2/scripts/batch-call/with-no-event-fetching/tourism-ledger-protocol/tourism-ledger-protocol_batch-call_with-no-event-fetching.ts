...
const contractDetails = parseContractIdentifier(
  process.env.TOURISM_LEDGER_PROTOCOL_CONTRACT_ADDRESS,
  'tourism-ledger-protocol'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TourismLedgerProtocol Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TourismLedgerProtocol Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOURISM_LEDGER_PROTOCOL_CONTRACT_ADDRESS');

  if (!process.env.TOURISM_LEDGER_PROTOCOL_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOURISM_LEDGER_PROTOCOL_CONTRACT_ADDRESS not set in environment.');
