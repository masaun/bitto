...
const contractDetails = parseContractIdentifier(
  process.env.CITY_PROSPERITY_GAME_CONTRACT_ADDRESS,
  'city-prosperity-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CityProsperityGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CityProsperityGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_PROSPERITY_GAME_CONTRACT_ADDRESS');

  if (!process.env.CITY_PROSPERITY_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_PROSPERITY_GAME_CONTRACT_ADDRESS not set in environment.');
