...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_CRAFT_HUNT_CONTRACT_ADDRESS,
  'local-craft-hunt'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalCraftHunt Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalCraftHunt Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_CRAFT_HUNT_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_CRAFT_HUNT_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_CRAFT_HUNT_CONTRACT_ADDRESS not set in environment.');
