...
const contractDetails = parseContractIdentifier(
  process.env.CITYWIDE_ECONOMIC_REWARDS_CONTRACT_ADDRESS,
  'citywide-economic-rewards'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CitywideEconomicRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CitywideEconomicRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITYWIDE_ECONOMIC_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.CITYWIDE_ECONOMIC_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITYWIDE_ECONOMIC_REWARDS_CONTRACT_ADDRESS not set in environment.');
