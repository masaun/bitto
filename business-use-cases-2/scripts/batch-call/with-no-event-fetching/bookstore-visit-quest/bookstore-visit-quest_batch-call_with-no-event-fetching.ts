...
const contractDetails = parseContractIdentifier(
  process.env.BOOKSTORE_VISIT_QUEST_CONTRACT_ADDRESS,
  'bookstore-visit-quest'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for BookstoreVisitQuest Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  BookstoreVisitQuest Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: BOOKSTORE_VISIT_QUEST_CONTRACT_ADDRESS');

  if (!process.env.BOOKSTORE_VISIT_QUEST_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: BOOKSTORE_VISIT_QUEST_CONTRACT_ADDRESS not set in environment.');
