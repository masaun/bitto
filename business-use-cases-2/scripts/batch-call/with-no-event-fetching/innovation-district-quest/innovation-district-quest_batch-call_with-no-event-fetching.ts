...
const contractDetails = parseContractIdentifier(
  process.env.INNOVATION_DISTRICT_QUEST_CONTRACT_ADDRESS,
  'innovation-district-quest'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for InnovationDistrictQuest Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  InnovationDistrictQuest Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: INNOVATION_DISTRICT_QUEST_CONTRACT_ADDRESS');

  if (!process.env.INNOVATION_DISTRICT_QUEST_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: INNOVATION_DISTRICT_QUEST_CONTRACT_ADDRESS not set in environment.');
