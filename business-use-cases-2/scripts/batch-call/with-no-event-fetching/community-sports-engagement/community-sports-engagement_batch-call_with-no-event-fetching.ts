...
const contractDetails = parseContractIdentifier(
  process.env.COMMUNITY_SPORTS_ENGAGEMENT_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CommunitySportsEngagement Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CommunitySportsEngagement Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: COMMUNITY_SPORTS_ENGAGEMENT_CONTRACT_ADDRESS');

  if (!process.env.COMMUNITY_SPORTS_ENGAGEMENT_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: COMMUNITY_SPORTS_ENGAGEMENT_CONTRACT_ADDRESS not set in environment.');
