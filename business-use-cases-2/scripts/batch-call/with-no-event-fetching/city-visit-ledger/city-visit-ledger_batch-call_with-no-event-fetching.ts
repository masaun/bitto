...
const contractDetails = parseContractIdentifier(
  process.env.CITY_VISIT_LEDGER_CONTRACT_ADDRESS,
  'city-visit-ledger'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CityVisitLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CityVisitLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_VISIT_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.CITY_VISIT_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_VISIT_LEDGER_CONTRACT_ADDRESS not set in environment.');
