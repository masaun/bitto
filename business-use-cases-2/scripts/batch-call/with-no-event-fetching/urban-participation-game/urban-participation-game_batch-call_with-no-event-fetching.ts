...
const contractDetails = parseContractIdentifier(
  process.env.URBAN_PARTICIPATION_GAME_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for UrbanParticipationGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  UrbanParticipationGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: URBAN_PARTICIPATION_GAME_CONTRACT_ADDRESS');

  if (!process.env.URBAN_PARTICIPATION_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: URBAN_PARTICIPATION_GAME_CONTRACT_ADDRESS not set in environment.');
