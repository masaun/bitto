...
const contractDetails = parseContractIdentifier(
  process.env.TOWN_FESTIVAL_GAME_PASS_CONTRACT_ADDRESS,
  'town-festival-game-pass'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TownFestivalGamePass Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TownFestivalGamePass Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOWN_FESTIVAL_GAME_PASS_CONTRACT_ADDRESS');

  if (!process.env.TOWN_FESTIVAL_GAME_PASS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOWN_FESTIVAL_GAME_PASS_CONTRACT_ADDRESS not set in environment.');
