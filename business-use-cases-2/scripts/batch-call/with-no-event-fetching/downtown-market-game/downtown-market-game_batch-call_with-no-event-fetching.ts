...
const contractDetails = parseContractIdentifier(
  process.env.DOWNTOWN_MARKET_GAME_CONTRACT_ADDRESS,
  'downtown-market-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for DowntownMarketGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  DowntownMarketGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: DOWNTOWN_MARKET_GAME_CONTRACT_ADDRESS');

  if (!process.env.DOWNTOWN_MARKET_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: DOWNTOWN_MARKET_GAME_CONTRACT_ADDRESS not set in environment.');
