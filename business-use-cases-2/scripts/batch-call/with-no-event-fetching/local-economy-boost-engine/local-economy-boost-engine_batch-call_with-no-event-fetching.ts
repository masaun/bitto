...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_ECONOMY_BOOST_ENGINE_CONTRACT_ADDRESS,
  'local-economy-boost-engine'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalEconomyBoostEngine Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalEconomyBoostEngine Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_ECONOMY_BOOST_ENGINE_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_ECONOMY_BOOST_ENGINE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_ECONOMY_BOOST_ENGINE_CONTRACT_ADDRESS not set in environment.');
