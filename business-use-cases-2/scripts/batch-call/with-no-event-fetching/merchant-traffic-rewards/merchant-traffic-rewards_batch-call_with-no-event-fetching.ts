...
const contractDetails = parseContractIdentifier(
  process.env.MERCHANT_TRAFFIC_REWARDS_CONTRACT_ADDRESS,
  'merchant-traffic-rewards'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for MerchantTrafficRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  MerchantTrafficRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: MERCHANT_TRAFFIC_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.MERCHANT_TRAFFIC_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: MERCHANT_TRAFFIC_REWARDS_CONTRACT_ADDRESS not set in environment.');
