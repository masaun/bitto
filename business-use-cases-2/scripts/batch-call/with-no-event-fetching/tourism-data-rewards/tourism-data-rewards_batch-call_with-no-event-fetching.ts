...
const contractDetails = parseContractIdentifier(
  process.env.TOURISM_DATA_REWARDS_CONTRACT_ADDRESS,
  'tourism-data-rewards'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TourismDataRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TourismDataRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOURISM_DATA_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.TOURISM_DATA_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOURISM_DATA_REWARDS_CONTRACT_ADDRESS not set in environment.');
