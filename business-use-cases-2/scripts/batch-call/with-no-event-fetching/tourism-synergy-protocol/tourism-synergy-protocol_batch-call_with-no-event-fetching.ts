...
const contractDetails = parseContractIdentifier(
  process.env.TOURISM_SYNERGY_PROTOCOL_CONTRACT_ADDRESS,
  'tourism-synergy-protocol'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TourismSynergyProtocol Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TourismSynergyProtocol Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOURISM_SYNERGY_PROTOCOL_CONTRACT_ADDRESS');

  if (!process.env.TOURISM_SYNERGY_PROTOCOL_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOURISM_SYNERGY_PROTOCOL_CONTRACT_ADDRESS not set in environment.');
