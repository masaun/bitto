...
const contractDetails = parseContractIdentifier(
  process.env.CITY_BIKE_EXPLORER_CONTRACT_ADDRESS,
  'city-bike-explorer'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CityBikeExplorer Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CityBikeExplorer Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_BIKE_EXPLORER_CONTRACT_ADDRESS');

  if (!process.env.CITY_BIKE_EXPLORER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_BIKE_EXPLORER_CONTRACT_ADDRESS not set in environment.');
