...
const contractDetails = parseContractIdentifier(
  process.env.ENTERTAINMENT_DISTRICT_PASS_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for EntertainmentDistrictPass Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  EntertainmentDistrictPass Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: ENTERTAINMENT_DISTRICT_PASS_CONTRACT_ADDRESS');

  if (!process.env.ENTERTAINMENT_DISTRICT_PASS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: ENTERTAINMENT_DISTRICT_PASS_CONTRACT_ADDRESS not set in environment.');
