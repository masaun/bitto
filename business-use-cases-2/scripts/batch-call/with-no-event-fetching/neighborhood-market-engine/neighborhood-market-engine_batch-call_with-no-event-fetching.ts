...
const contractDetails = parseContractIdentifier(
  process.env.NEIGHBORHOOD_MARKET_ENGINE_CONTRACT_ADDRESS,
  'neighborhood-market-engine'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for NeighborhoodMarketEngine Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  NeighborhoodMarketEngine Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: NEIGHBORHOOD_MARKET_ENGINE_CONTRACT_ADDRESS');

  if (!process.env.NEIGHBORHOOD_MARKET_ENGINE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: NEIGHBORHOOD_MARKET_ENGINE_CONTRACT_ADDRESS not set in environment.');
