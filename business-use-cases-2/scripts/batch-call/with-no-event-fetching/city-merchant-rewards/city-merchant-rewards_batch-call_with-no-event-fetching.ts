...
const contractDetails = parseContractIdentifier(
  process.env.CITY_MERCHANT_REWARDS_CONTRACT_ADDRESS,
  'city-merchant-rewards'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CityMerchantRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CityMerchantRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_MERCHANT_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.CITY_MERCHANT_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_MERCHANT_REWARDS_CONTRACT_ADDRESS not set in environment.');
