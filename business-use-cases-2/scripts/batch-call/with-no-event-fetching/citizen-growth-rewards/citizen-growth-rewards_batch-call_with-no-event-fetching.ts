...
const contractDetails = parseContractIdentifier(
  process.env.CITIZEN_GROWTH_REWARDS_CONTRACT_ADDRESS,
  'citizen-growth-rewards'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CitizenGrowthRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CitizenGrowthRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITIZEN_GROWTH_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.CITIZEN_GROWTH_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITIZEN_GROWTH_REWARDS_CONTRACT_ADDRESS not set in environment.');
