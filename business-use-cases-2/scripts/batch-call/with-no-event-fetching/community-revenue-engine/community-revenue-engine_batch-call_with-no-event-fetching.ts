...
const contractDetails = parseContractIdentifier(
  process.env.COMMUNITY_REVENUE_ENGINE_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CommunityRevenueEngine Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CommunityRevenueEngine Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: COMMUNITY_REVENUE_ENGINE_CONTRACT_ADDRESS');

  if (!process.env.COMMUNITY_REVENUE_ENGINE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: COMMUNITY_REVENUE_ENGINE_CONTRACT_ADDRESS not set in environment.');
