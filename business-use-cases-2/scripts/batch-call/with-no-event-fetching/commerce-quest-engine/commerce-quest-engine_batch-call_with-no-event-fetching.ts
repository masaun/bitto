...
const contractDetails = parseContractIdentifier(
  process.env.COMMERCE_QUEST_ENGINE_CONTRACT_ADDRESS,
  'commerce-quest-engine'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CommerceQuestEngine Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CommerceQuestEngine Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: COMMERCE_QUEST_ENGINE_CONTRACT_ADDRESS');

  if (!process.env.COMMERCE_QUEST_ENGINE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: COMMERCE_QUEST_ENGINE_CONTRACT_ADDRESS not set in environment.');
