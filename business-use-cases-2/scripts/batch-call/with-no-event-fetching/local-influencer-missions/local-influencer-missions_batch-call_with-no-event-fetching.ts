...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_INFLUENCER_MISSIONS_CONTRACT_ADDRESS,
  'local-influencer-missions'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalInfluencerMissions Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalInfluencerMissions Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_INFLUENCER_MISSIONS_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_INFLUENCER_MISSIONS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_INFLUENCER_MISSIONS_CONTRACT_ADDRESS not set in environment.');
