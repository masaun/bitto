...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_COLLABORATION_GAME_CONTRACT_ADDRESS,
  'local-collaboration-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalCollaborationGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalCollaborationGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_COLLABORATION_GAME_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_COLLABORATION_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_COLLABORATION_GAME_CONTRACT_ADDRESS not set in environment.');
