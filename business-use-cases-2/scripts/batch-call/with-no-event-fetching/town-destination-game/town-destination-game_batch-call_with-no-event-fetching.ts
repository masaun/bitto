...
const contractDetails = parseContractIdentifier(
  process.env.TOWN_DESTINATION_GAME_CONTRACT_ADDRESS,
  'town-destination-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TownDestinationGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TownDestinationGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOWN_DESTINATION_GAME_CONTRACT_ADDRESS');

  if (!process.env.TOWN_DESTINATION_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOWN_DESTINATION_GAME_CONTRACT_ADDRESS not set in environment.');
