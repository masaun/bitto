...
const contractDetails = parseContractIdentifier(
  process.env.ECO_TOURISM_QUEST_CONTRACT_ADDRESS,
  'eco-tourism-quest'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for EcoTourismQuest Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  EcoTourismQuest Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: ECO_TOURISM_QUEST_CONTRACT_ADDRESS');

  if (!process.env.ECO_TOURISM_QUEST_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: ECO_TOURISM_QUEST_CONTRACT_ADDRESS not set in environment.');
