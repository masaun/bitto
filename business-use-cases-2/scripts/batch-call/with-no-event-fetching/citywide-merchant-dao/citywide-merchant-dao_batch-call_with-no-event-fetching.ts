...
const contractDetails = parseContractIdentifier(
  process.env.CITYWIDE_MERCHANT_DAO_CONTRACT_ADDRESS,
  'citywide-merchant-dao'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CitywideMerchantDao Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CitywideMerchantDao Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITYWIDE_MERCHANT_DAO_CONTRACT_ADDRESS');

  if (!process.env.CITYWIDE_MERCHANT_DAO_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITYWIDE_MERCHANT_DAO_CONTRACT_ADDRESS not set in environment.');
