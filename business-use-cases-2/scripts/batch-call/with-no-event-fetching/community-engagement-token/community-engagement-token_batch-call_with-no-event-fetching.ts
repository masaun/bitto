...
const contractDetails = parseContractIdentifier(
  process.env.COMMUNITY_ENGAGEMENT_TOKEN_CONTRACT_ADDRESS,
  'community-engagement-token'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CommunityEngagementToken Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CommunityEngagementToken Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: COMMUNITY_ENGAGEMENT_TOKEN_CONTRACT_ADDRESS');

  if (!process.env.COMMUNITY_ENGAGEMENT_TOKEN_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: COMMUNITY_ENGAGEMENT_TOKEN_CONTRACT_ADDRESS not set in environment.');
