...
const contractDetails = parseContractIdentifier(
  process.env.FARMERS_MARKET_QUEST_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for FarmersMarketQuest Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  FarmersMarketQuest Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: FARMERS_MARKET_QUEST_CONTRACT_ADDRESS');

  if (!process.env.FARMERS_MARKET_QUEST_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: FARMERS_MARKET_QUEST_CONTRACT_ADDRESS not set in environment.');
