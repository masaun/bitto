...
const contractDetails = parseContractIdentifier(
  process.env.NEIGHBORHOOD_CHALLENGE_LEAGUE_CONTRACT_ADDRESS,
  'neighborhood-challenge-league'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for NeighborhoodChallengeLeague Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  NeighborhoodChallengeLeague Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: NEIGHBORHOOD_CHALLENGE_LEAGUE_CONTRACT_ADDRESS');

  if (!process.env.NEIGHBORHOOD_CHALLENGE_LEAGUE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: NEIGHBORHOOD_CHALLENGE_LEAGUE_CONTRACT_ADDRESS not set in environment.');
