...
const contractDetails = parseContractIdentifier(
  process.env.PUBLIC_DESTINATION_GAME_CONTRACT_ADDRESS,
  'public-destination-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for PublicDestinationGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  PublicDestinationGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: PUBLIC_DESTINATION_GAME_CONTRACT_ADDRESS');

  if (!process.env.PUBLIC_DESTINATION_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: PUBLIC_DESTINATION_GAME_CONTRACT_ADDRESS not set in environment.');
