...
const contractDetails = parseContractIdentifier(
  process.env.CIVIC_POINTS_DISTRIBUTION_CONTRACT_ADDRESS,
  'civic-points-distribution'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CivicPointsDistribution Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CivicPointsDistribution Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CIVIC_POINTS_DISTRIBUTION_CONTRACT_ADDRESS');

  if (!process.env.CIVIC_POINTS_DISTRIBUTION_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CIVIC_POINTS_DISTRIBUTION_CONTRACT_ADDRESS not set in environment.');
