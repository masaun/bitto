...
const contractDetails = parseContractIdentifier(
  process.env.TOURISM_PROSPERITY_LEDGER_CONTRACT_ADDRESS,
  'tourism-prosperity-ledger'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TourismProsperityLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TourismProsperityLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOURISM_PROSPERITY_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.TOURISM_PROSPERITY_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOURISM_PROSPERITY_LEDGER_CONTRACT_ADDRESS not set in environment.');
