...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_ENGAGEMENT_PROTOCOL_CONTRACT_ADDRESS,
  'local-engagement-protocol'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalEngagementProtocol Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalEngagementProtocol Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_ENGAGEMENT_PROTOCOL_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_ENGAGEMENT_PROTOCOL_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_ENGAGEMENT_PROTOCOL_CONTRACT_ADDRESS not set in environment.');
