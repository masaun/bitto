...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_VISIT_PROTOCOL_CONTRACT_ADDRESS,
  'local-visit-protocol'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalVisitProtocol Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalVisitProtocol Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_VISIT_PROTOCOL_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_VISIT_PROTOCOL_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_VISIT_PROTOCOL_CONTRACT_ADDRESS not set in environment.');
