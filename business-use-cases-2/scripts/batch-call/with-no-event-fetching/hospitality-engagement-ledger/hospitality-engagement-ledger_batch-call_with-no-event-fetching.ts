...
const contractDetails = parseContractIdentifier(
  process.env.HOSPITALITY_ENGAGEMENT_LEDGER_CONTRACT_ADDRESS,
  'hospitality-engagement-ledger'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for HospitalityEngagementLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  HospitalityEngagementLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: HOSPITALITY_ENGAGEMENT_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.HOSPITALITY_ENGAGEMENT_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: HOSPITALITY_ENGAGEMENT_LEDGER_CONTRACT_ADDRESS not set in environment.');
