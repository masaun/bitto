...
const contractDetails = parseContractIdentifier(
  process.env.REGIONAL_EVENT_MINING_CONTRACT_ADDRESS,
  'regional-event-mining'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for RegionalEventMining Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  RegionalEventMining Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: REGIONAL_EVENT_MINING_CONTRACT_ADDRESS');

  if (!process.env.REGIONAL_EVENT_MINING_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: REGIONAL_EVENT_MINING_CONTRACT_ADDRESS not set in environment.');
