...
const contractDetails = parseContractIdentifier(
  process.env.CITY_EXPERIENCE_TOKEN_CONTRACT_ADDRESS,
  'city-experience-token'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CityExperienceToken Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CityExperienceToken Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_EXPERIENCE_TOKEN_CONTRACT_ADDRESS');

  if (!process.env.CITY_EXPERIENCE_TOKEN_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_EXPERIENCE_TOKEN_CONTRACT_ADDRESS not set in environment.');
