...
const contractDetails = parseContractIdentifier(
  process.env.STUDENT_DISCOUNT_GAME_CONTRACT_ADDRESS,
  'student-discount-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for StudentDiscountGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  StudentDiscountGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: STUDENT_DISCOUNT_GAME_CONTRACT_ADDRESS');

  if (!process.env.STUDENT_DISCOUNT_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: STUDENT_DISCOUNT_GAME_CONTRACT_ADDRESS not set in environment.');
