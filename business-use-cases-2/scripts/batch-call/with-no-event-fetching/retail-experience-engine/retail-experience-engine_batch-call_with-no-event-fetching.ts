...
const contractDetails = parseContractIdentifier(
  process.env.RETAIL_EXPERIENCE_ENGINE_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for RetailExperienceEngine Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  RetailExperienceEngine Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: RETAIL_EXPERIENCE_ENGINE_CONTRACT_ADDRESS');

  if (!process.env.RETAIL_EXPERIENCE_ENGINE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: RETAIL_EXPERIENCE_ENGINE_CONTRACT_ADDRESS not set in environment.');
