...
const contractDetails = parseContractIdentifier(
  process.env.TOWN_LOYALTY_PROTOCOL_CONTRACT_ADDRESS,
  'town-loyalty-protocol'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TownLoyaltyProtocol Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TownLoyaltyProtocol Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOWN_LOYALTY_PROTOCOL_CONTRACT_ADDRESS');

  if (!process.env.TOWN_LOYALTY_PROTOCOL_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOWN_LOYALTY_PROTOCOL_CONTRACT_ADDRESS not set in environment.');
