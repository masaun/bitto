...
const contractDetails = parseContractIdentifier(
  process.env.CITYWIDE_SCAVENGER_PROTOCOL_CONTRACT_ADDRESS,
  'citywide-scavenger-protocol'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CitywideScavengerProtocol Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CitywideScavengerProtocol Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITYWIDE_SCAVENGER_PROTOCOL_CONTRACT_ADDRESS');

  if (!process.env.CITYWIDE_SCAVENGER_PROTOCOL_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITYWIDE_SCAVENGER_PROTOCOL_CONTRACT_ADDRESS not set in environment.');
