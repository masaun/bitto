...
const contractDetails = parseContractIdentifier(
  process.env.CULTURAL_TRAIL_CHALLENGE_CONTRACT_ADDRESS,
  'cultural-trail-challenge'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CulturalTrailChallenge Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CulturalTrailChallenge Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CULTURAL_TRAIL_CHALLENGE_CONTRACT_ADDRESS');

  if (!process.env.CULTURAL_TRAIL_CHALLENGE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CULTURAL_TRAIL_CHALLENGE_CONTRACT_ADDRESS not set in environment.');
