...
const contractDetails = parseContractIdentifier(
  process.env.SMALL_BUSINESS_STAMP_CARD_CONTRACT_ADDRESS,
  'small-business-stamp-card'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for SmallBusinessStampCard Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  SmallBusinessStampCard Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: SMALL_BUSINESS_STAMP_CARD_CONTRACT_ADDRESS');

  if (!process.env.SMALL_BUSINESS_STAMP_CARD_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: SMALL_BUSINESS_STAMP_CARD_CONTRACT_ADDRESS not set in environment.');
