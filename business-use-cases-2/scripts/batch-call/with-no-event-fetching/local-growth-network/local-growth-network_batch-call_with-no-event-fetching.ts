...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_GROWTH_NETWORK_CONTRACT_ADDRESS,
  'local-growth-network'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalGrowthNetwork Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalGrowthNetwork Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_GROWTH_NETWORK_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_GROWTH_NETWORK_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_GROWTH_NETWORK_CONTRACT_ADDRESS not set in environment.');
