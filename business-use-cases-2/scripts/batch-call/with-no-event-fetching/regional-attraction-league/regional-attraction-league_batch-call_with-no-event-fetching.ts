...
const contractDetails = parseContractIdentifier(
  process.env.REGIONAL_ATTRACTION_LEAGUE_CONTRACT_ADDRESS,
  'regional-attraction-league'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for RegionalAttractionLeague Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  RegionalAttractionLeague Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: REGIONAL_ATTRACTION_LEAGUE_CONTRACT_ADDRESS');

  if (!process.env.REGIONAL_ATTRACTION_LEAGUE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: REGIONAL_ATTRACTION_LEAGUE_CONTRACT_ADDRESS not set in environment.');
