...
const contractDetails = parseContractIdentifier(
  process.env.TOWN_EXPERIENCE_MINING_CONTRACT_ADDRESS,
  'town-experience-mining'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TownExperienceMining Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TownExperienceMining Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOWN_EXPERIENCE_MINING_CONTRACT_ADDRESS');

  if (!process.env.TOWN_EXPERIENCE_MINING_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOWN_EXPERIENCE_MINING_CONTRACT_ADDRESS not set in environment.');
