...
const contractDetails = parseContractIdentifier(
  process.env.CITIZEN_REWARD_DISTRIBUTION_CONTRACT_ADDRESS,
  'citizen-reward-distribution'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CitizenRewardDistribution Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CitizenRewardDistribution Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITIZEN_REWARD_DISTRIBUTION_CONTRACT_ADDRESS');

  if (!process.env.CITIZEN_REWARD_DISTRIBUTION_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITIZEN_REWARD_DISTRIBUTION_CONTRACT_ADDRESS not set in environment.');
