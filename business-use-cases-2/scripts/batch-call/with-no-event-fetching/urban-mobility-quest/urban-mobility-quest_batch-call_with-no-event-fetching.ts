...
const contractDetails = parseContractIdentifier(
  process.env.URBAN_MOBILITY_QUEST_CONTRACT_ADDRESS,
  'urban-mobility-quest'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for UrbanMobilityQuest Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  UrbanMobilityQuest Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: URBAN_MOBILITY_QUEST_CONTRACT_ADDRESS');

  if (!process.env.URBAN_MOBILITY_QUEST_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: URBAN_MOBILITY_QUEST_CONTRACT_ADDRESS not set in environment.');
