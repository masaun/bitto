...
const contractDetails = parseContractIdentifier(
  process.env.CITY_TOUR_COMPETITION_CONTRACT_ADDRESS,
  'city-tour-competition'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CityTourCompetition Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CityTourCompetition Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_TOUR_COMPETITION_CONTRACT_ADDRESS');

  if (!process.env.CITY_TOUR_COMPETITION_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_TOUR_COMPETITION_CONTRACT_ADDRESS not set in environment.');
