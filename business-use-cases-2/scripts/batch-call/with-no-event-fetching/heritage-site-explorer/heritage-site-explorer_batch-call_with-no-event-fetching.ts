...
const contractDetails = parseContractIdentifier(
  process.env.HERITAGE_SITE_EXPLORER_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for HeritageSiteExplorer Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  HeritageSiteExplorer Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: HERITAGE_SITE_EXPLORER_CONTRACT_ADDRESS');

  if (!process.env.HERITAGE_SITE_EXPLORER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: HERITAGE_SITE_EXPLORER_CONTRACT_ADDRESS not set in environment.');
