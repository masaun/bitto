...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_PROSPERITY_ENGINE_CONTRACT_ADDRESS,
  'local-prosperity-engine'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalProsperityEngine Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalProsperityEngine Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_PROSPERITY_ENGINE_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_PROSPERITY_ENGINE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_PROSPERITY_ENGINE_CONTRACT_ADDRESS not set in environment.');
