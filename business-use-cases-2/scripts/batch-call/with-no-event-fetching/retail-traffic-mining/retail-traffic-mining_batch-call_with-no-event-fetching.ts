...
const contractDetails = parseContractIdentifier(
  process.env.RETAIL_TRAFFIC_MINING_CONTRACT_ADDRESS,
  'retail-traffic-mining'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for RetailTrafficMining Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  RetailTrafficMining Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: RETAIL_TRAFFIC_MINING_CONTRACT_ADDRESS');

  if (!process.env.RETAIL_TRAFFIC_MINING_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: RETAIL_TRAFFIC_MINING_CONTRACT_ADDRESS not set in environment.');
