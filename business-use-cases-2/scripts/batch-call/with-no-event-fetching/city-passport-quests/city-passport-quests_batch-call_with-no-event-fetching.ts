...
const contractDetails = parseContractIdentifier(
  process.env.CITY_PASSPORT_QUESTS_CONTRACT_ADDRESS,
  'city-passport-quests'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CityPassportQuests Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CityPassportQuests Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_PASSPORT_QUESTS_CONTRACT_ADDRESS');

  if (!process.env.CITY_PASSPORT_QUESTS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_PASSPORT_QUESTS_CONTRACT_ADDRESS not set in environment.');
