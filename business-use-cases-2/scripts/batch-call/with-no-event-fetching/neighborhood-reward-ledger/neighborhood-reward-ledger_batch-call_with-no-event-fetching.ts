...
const contractDetails = parseContractIdentifier(
  process.env.NEIGHBORHOOD_REWARD_LEDGER_CONTRACT_ADDRESS,
  'neighborhood-reward-ledger'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for NeighborhoodRewardLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  NeighborhoodRewardLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: NEIGHBORHOOD_REWARD_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.NEIGHBORHOOD_REWARD_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: NEIGHBORHOOD_REWARD_LEDGER_CONTRACT_ADDRESS not set in environment.');
