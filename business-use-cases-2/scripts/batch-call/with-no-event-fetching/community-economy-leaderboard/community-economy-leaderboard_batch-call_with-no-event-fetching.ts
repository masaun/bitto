...
const contractDetails = parseContractIdentifier(
  process.env.COMMUNITY_ECONOMY_LEADERBOARD_CONTRACT_ADDRESS,
  'community-economy-leaderboard'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CommunityEconomyLeaderboard Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CommunityEconomyLeaderboard Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: COMMUNITY_ECONOMY_LEADERBOARD_CONTRACT_ADDRESS');

  if (!process.env.COMMUNITY_ECONOMY_LEADERBOARD_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: COMMUNITY_ECONOMY_LEADERBOARD_CONTRACT_ADDRESS not set in environment.');
