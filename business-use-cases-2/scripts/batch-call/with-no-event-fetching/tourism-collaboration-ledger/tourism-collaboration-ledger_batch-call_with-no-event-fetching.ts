...
const contractDetails = parseContractIdentifier(
  process.env.TOURISM_COLLABORATION_LEDGER_CONTRACT_ADDRESS,
  'tourism-collaboration-ledger'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TourismCollaborationLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TourismCollaborationLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOURISM_COLLABORATION_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.TOURISM_COLLABORATION_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOURISM_COLLABORATION_LEDGER_CONTRACT_ADDRESS not set in environment.');
