...
const contractDetails = parseContractIdentifier(
  process.env.URBAN_GROWTH_PROTOCOL_CONTRACT_ADDRESS,
  'urban-growth-protocol'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for UrbanGrowthProtocol Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  UrbanGrowthProtocol Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: URBAN_GROWTH_PROTOCOL_CONTRACT_ADDRESS');

  if (!process.env.URBAN_GROWTH_PROTOCOL_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: URBAN_GROWTH_PROTOCOL_CONTRACT_ADDRESS not set in environment.');
