...
const contractDetails = parseContractIdentifier(
  process.env.CITY_LIFESTYLE_QUEST_CONTRACT_ADDRESS,
  'city-lifestyle-quest'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CityLifestyleQuest Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CityLifestyleQuest Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_LIFESTYLE_QUEST_CONTRACT_ADDRESS');

  if (!process.env.CITY_LIFESTYLE_QUEST_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_LIFESTYLE_QUEST_CONTRACT_ADDRESS not set in environment.');
