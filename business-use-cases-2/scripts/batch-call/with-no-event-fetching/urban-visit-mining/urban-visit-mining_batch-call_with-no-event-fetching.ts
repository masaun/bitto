...
const contractDetails = parseContractIdentifier(
  process.env.URBAN_VISIT_MINING_CONTRACT_ADDRESS,
  'urban-visit-mining'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for UrbanVisitMining Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  UrbanVisitMining Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: URBAN_VISIT_MINING_CONTRACT_ADDRESS');

  if (!process.env.URBAN_VISIT_MINING_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: URBAN_VISIT_MINING_CONTRACT_ADDRESS not set in environment.');
