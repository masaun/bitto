...
const contractDetails = parseContractIdentifier(
  process.env.GUEST_ECONOMY_PROTOCOL_CONTRACT_ADDRESS,
  'guest-economy-protocol'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for GuestEconomyProtocol Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  GuestEconomyProtocol Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: GUEST_ECONOMY_PROTOCOL_CONTRACT_ADDRESS');

  if (!process.env.GUEST_ECONOMY_PROTOCOL_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: GUEST_ECONOMY_PROTOCOL_CONTRACT_ADDRESS not set in environment.');
