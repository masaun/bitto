...
const contractDetails = parseContractIdentifier(
  process.env.CITY_COOPERATION_PROTOCOL_CONTRACT_ADDRESS,
  'city-cooperation-protocol'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CityCooperationProtocol Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CityCooperationProtocol Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_COOPERATION_PROTOCOL_CONTRACT_ADDRESS');

  if (!process.env.CITY_COOPERATION_PROTOCOL_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_COOPERATION_PROTOCOL_CONTRACT_ADDRESS not set in environment.');
