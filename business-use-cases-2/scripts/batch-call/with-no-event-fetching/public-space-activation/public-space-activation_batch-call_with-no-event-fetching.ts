...
const contractDetails = parseContractIdentifier(
  process.env.PUBLIC_SPACE_ACTIVATION_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for PublicSpaceActivation Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  PublicSpaceActivation Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: PUBLIC_SPACE_ACTIVATION_CONTRACT_ADDRESS');

  if (!process.env.PUBLIC_SPACE_ACTIVATION_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: PUBLIC_SPACE_ACTIVATION_CONTRACT_ADDRESS not set in environment.');
