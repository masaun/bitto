...
const contractDetails = parseContractIdentifier(
  process.env.CITIZEN_PARTICIPATION_PROTOCOL_CONTRACT_ADDRESS,
  'citizen-participation-protocol'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CitizenParticipationProtocol Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CitizenParticipationProtocol Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITIZEN_PARTICIPATION_PROTOCOL_CONTRACT_ADDRESS');

  if (!process.env.CITIZEN_PARTICIPATION_PROTOCOL_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITIZEN_PARTICIPATION_PROTOCOL_CONTRACT_ADDRESS not set in environment.');
