...
const contractDetails = parseContractIdentifier(
  process.env.LOCAL_CITY_TREASURE_HUNT_CONTRACT_ADDRESS,
  'local-city-treasure-hunt'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for LocalCityTreasureHunt Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  LocalCityTreasureHunt Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: LOCAL_CITY_TREASURE_HUNT_CONTRACT_ADDRESS');

  if (!process.env.LOCAL_CITY_TREASURE_HUNT_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LOCAL_CITY_TREASURE_HUNT_CONTRACT_ADDRESS not set in environment.');
