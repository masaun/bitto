...
const contractDetails = parseContractIdentifier(
  process.env.TOURISM_ENGAGEMENT_INDEX_CONTRACT_ADDRESS,
  'tourism-engagement-index'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TourismEngagementIndex Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TourismEngagementIndex Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOURISM_ENGAGEMENT_INDEX_CONTRACT_ADDRESS');

  if (!process.env.TOURISM_ENGAGEMENT_INDEX_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOURISM_ENGAGEMENT_INDEX_CONTRACT_ADDRESS not set in environment.');
