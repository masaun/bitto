...
const contractDetails = parseContractIdentifier(
  process.env.DOWNTOWN_FOOT_TRAFFIC_REWARDS_CONTRACT_ADDRESS,
  'downtown-foot-traffic-rewards'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for DowntownFootTrafficRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  DowntownFootTrafficRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: DOWNTOWN_FOOT_TRAFFIC_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.DOWNTOWN_FOOT_TRAFFIC_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: DOWNTOWN_FOOT_TRAFFIC_REWARDS_CONTRACT_ADDRESS not set in environment.');
