...
const contractDetails = parseContractIdentifier(
  process.env.HISTORIC_DISTRICT_ADVENTURE_CONTRACT_ADDRESS,
  'historic-district-adventure'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for HistoricDistrictAdventure Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  HistoricDistrictAdventure Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: HISTORIC_DISTRICT_ADVENTURE_CONTRACT_ADDRESS');

  if (!process.env.HISTORIC_DISTRICT_ADVENTURE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: HISTORIC_DISTRICT_ADVENTURE_CONTRACT_ADDRESS not set in environment.');
