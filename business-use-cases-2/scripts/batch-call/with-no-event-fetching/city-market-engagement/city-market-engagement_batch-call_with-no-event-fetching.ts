...
const contractDetails = parseContractIdentifier(
  process.env.CITY_MARKET_ENGAGEMENT_CONTRACT_ADDRESS,
  'city-market-engagement'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CityMarketEngagement Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CityMarketEngagement Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITY_MARKET_ENGAGEMENT_CONTRACT_ADDRESS');

  if (!process.env.CITY_MARKET_ENGAGEMENT_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITY_MARKET_ENGAGEMENT_CONTRACT_ADDRESS not set in environment.');
