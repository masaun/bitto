...
const contractDetails = parseContractIdentifier(
  process.env.CITIZEN_EXPERIENCE_ENGINE_CONTRACT_ADDRESS,
  'citizen-experience-engine'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CitizenExperienceEngine Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CitizenExperienceEngine Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CITIZEN_EXPERIENCE_ENGINE_CONTRACT_ADDRESS');

  if (!process.env.CITIZEN_EXPERIENCE_ENGINE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CITIZEN_EXPERIENCE_ENGINE_CONTRACT_ADDRESS not set in environment.');
