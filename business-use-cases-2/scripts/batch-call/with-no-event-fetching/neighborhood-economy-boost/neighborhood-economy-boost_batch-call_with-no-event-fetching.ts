...
const contractDetails = parseContractIdentifier(
  process.env.NEIGHBORHOOD_ECONOMY_BOOST_CONTRACT_ADDRESS,
  'neighborhood-economy-boost'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for NeighborhoodEconomyBoost Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  NeighborhoodEconomyBoost Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: NEIGHBORHOOD_ECONOMY_BOOST_CONTRACT_ADDRESS');

  if (!process.env.NEIGHBORHOOD_ECONOMY_BOOST_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: NEIGHBORHOOD_ECONOMY_BOOST_CONTRACT_ADDRESS not set in environment.');
