...
const contractDetails = parseContractIdentifier(
  process.env.TOWN_COMMERCE_LEDGER_CONTRACT_ADDRESS,
  'town-commerce-ledger'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TownCommerceLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TownCommerceLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOWN_COMMERCE_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.TOWN_COMMERCE_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOWN_COMMERCE_LEDGER_CONTRACT_ADDRESS not set in environment.');
