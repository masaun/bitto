...
const contractDetails = parseContractIdentifier(
  process.env.TOURISM_COLLABORATION_ENGINE_CONTRACT_ADDRESS,
  'tourism-collaboration-engine'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TourismCollaborationEngine Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TourismCollaborationEngine Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOURISM_COLLABORATION_ENGINE_CONTRACT_ADDRESS');

  if (!process.env.TOURISM_COLLABORATION_ENGINE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOURISM_COLLABORATION_ENGINE_CONTRACT_ADDRESS not set in environment.');
