...
const contractDetails = parseContractIdentifier(
  process.env.REGIONAL_VISITOR_REWARDS_CONTRACT_ADDRESS,
  'regional-visitor-rewards'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for RegionalVisitorRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  RegionalVisitorRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: REGIONAL_VISITOR_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.REGIONAL_VISITOR_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: REGIONAL_VISITOR_REWARDS_CONTRACT_ADDRESS not set in environment.');
