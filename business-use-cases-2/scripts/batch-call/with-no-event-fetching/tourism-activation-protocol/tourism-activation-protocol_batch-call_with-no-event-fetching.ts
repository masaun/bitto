...
const contractDetails = parseContractIdentifier(
  process.env.TOURISM_ACTIVATION_PROTOCOL_CONTRACT_ADDRESS,
  'tourism-activation-protocol'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TourismActivationProtocol Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TourismActivationProtocol Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TOURISM_ACTIVATION_PROTOCOL_CONTRACT_ADDRESS');

  if (!process.env.TOURISM_ACTIVATION_PROTOCOL_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TOURISM_ACTIVATION_PROTOCOL_CONTRACT_ADDRESS not set in environment.');
