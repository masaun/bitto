...
const contractDetails = parseContractIdentifier(
  process.env.CULTURAL_ECONOMY_LEAGUE_CONTRACT_ADDRESS,
  'cultural-economy-league'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CulturalEconomyLeague Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CulturalEconomyLeague Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CULTURAL_ECONOMY_LEAGUE_CONTRACT_ADDRESS');

  if (!process.env.CULTURAL_ECONOMY_LEAGUE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CULTURAL_ECONOMY_LEAGUE_CONTRACT_ADDRESS not set in environment.');
