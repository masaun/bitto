...
const contractDetails = parseContractIdentifier(
  process.env.TRANSIT_RIDE_REWARDS_CONTRACT_ADDRESS,
  'transit-ride-rewards'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for TransitRideRewards Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  TransitRideRewards Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: TRANSIT_RIDE_REWARDS_CONTRACT_ADDRESS');

  if (!process.env.TRANSIT_RIDE_REWARDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: TRANSIT_RIDE_REWARDS_CONTRACT_ADDRESS not set in environment.');
