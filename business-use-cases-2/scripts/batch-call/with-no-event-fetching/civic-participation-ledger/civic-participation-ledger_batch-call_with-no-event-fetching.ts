...
const contractDetails = parseContractIdentifier(
  process.env.CIVIC_PARTICIPATION_LEDGER_CONTRACT_ADDRESS,
  'civic-participation-ledger'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CivicParticipationLedger Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CivicParticipationLedger Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CIVIC_PARTICIPATION_LEDGER_CONTRACT_ADDRESS');

  if (!process.env.CIVIC_PARTICIPATION_LEDGER_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CIVIC_PARTICIPATION_LEDGER_CONTRACT_ADDRESS not set in environment.');
