...
const contractDetails = parseContractIdentifier(
  process.env.REGIONAL_COMMERCE_CHALLENGE_CONTRACT_ADDRESS,
  'regional-commerce-challenge'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for RegionalCommerceChallenge Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  RegionalCommerceChallenge Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: REGIONAL_COMMERCE_CHALLENGE_CONTRACT_ADDRESS');

  if (!process.env.REGIONAL_COMMERCE_CHALLENGE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: REGIONAL_COMMERCE_CHALLENGE_CONTRACT_ADDRESS not set in environment.');
