...
const contractDetails = parseContractIdentifier(
  process.env.PARK_CLEANUP_REWARD_CONTRACT_ADDRESS,
  'park-cleanup-reward'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for ParkCleanupReward Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  ParkCleanupReward Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: PARK_CLEANUP_REWARD_CONTRACT_ADDRESS');

  if (!process.env.PARK_CLEANUP_REWARD_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: PARK_CLEANUP_REWARD_CONTRACT_ADDRESS not set in environment.');
