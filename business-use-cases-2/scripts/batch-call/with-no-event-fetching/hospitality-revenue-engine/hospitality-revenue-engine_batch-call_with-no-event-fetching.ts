...
const contractDetails = parseContractIdentifier(
  process.env.HOSPITALITY_REVENUE_ENGINE_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for HospitalityRevenueEngine Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  HospitalityRevenueEngine Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: HOSPITALITY_REVENUE_ENGINE_CONTRACT_ADDRESS');

  if (!process.env.HOSPITALITY_REVENUE_ENGINE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: HOSPITALITY_REVENUE_ENGINE_CONTRACT_ADDRESS not set in environment.');
