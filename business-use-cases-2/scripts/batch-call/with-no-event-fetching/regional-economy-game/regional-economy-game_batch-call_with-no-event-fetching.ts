...
const contractDetails = parseContractIdentifier(
  process.env.REGIONAL_ECONOMY_GAME_CONTRACT_ADDRESS,
  'regional-economy-game'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for RegionalEconomyGame Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  RegionalEconomyGame Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: REGIONAL_ECONOMY_GAME_CONTRACT_ADDRESS');

  if (!process.env.REGIONAL_ECONOMY_GAME_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: REGIONAL_ECONOMY_GAME_CONTRACT_ADDRESS not set in environment.');
