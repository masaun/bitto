...
const contractDetails = parseContractIdentifier(
  process.env.PUBLIC_SERVICE_QUEST_CONTRACT_ADDRESS,
  'public-service-quest'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for PublicServiceQuest Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  PublicServiceQuest Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: PUBLIC_SERVICE_QUEST_CONTRACT_ADDRESS');

  if (!process.env.PUBLIC_SERVICE_QUEST_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: PUBLIC_SERVICE_QUEST_CONTRACT_ADDRESS not set in environment.');
