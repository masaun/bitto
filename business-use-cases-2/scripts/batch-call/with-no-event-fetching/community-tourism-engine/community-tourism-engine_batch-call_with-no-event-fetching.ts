...
const contractDetails = parseContractIdentifier(
  process.env.COMMUNITY_TOURISM_ENGINE_CONTRACT_ADDRESS,
  'community-tourism-engine'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for CommunityTourismEngine Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  CommunityTourismEngine Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: COMMUNITY_TOURISM_ENGINE_CONTRACT_ADDRESS');

  if (!process.env.COMMUNITY_TOURISM_ENGINE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: COMMUNITY_TOURISM_ENGINE_CONTRACT_ADDRESS not set in environment.');
