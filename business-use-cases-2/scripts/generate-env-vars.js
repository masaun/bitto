...
contracts.forEach(contract => {
  ...
  envContent += `${envVar}_CONTRACT_ADDRESS=\n`;
});

const mainEnvPath = path.join(__dirname, '..', '.env');
const frontendEnvPath = path.join(__dirname, '..', 'frontend', '.env');

const existingMainEnv = fs.existsSync(mainEnvPath) ? fs.readFileSync(mainEnvPath, 'utf8') : '';
const existingFrontendEnv = fs.existsSync(frontendEnvPath) ? fs.readFileSync(frontendEnvPath, 'utf8') : '';

fs.writeFileSync(mainEnvPath, existingMainEnv + '\n' + envContent);
fs.writeFileSync(frontendEnvPath, existingFrontendEnv + '\n' + envContent);

console.log('✓ Updated .env');
console.log('✓ Updated frontend/.env');
console.log(`Added ${contracts.length} contract address variables`);
