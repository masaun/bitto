...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PYTH_NETWORK_PROXY_ADDRESS || '';

export default function Pyth_Network_ProxyPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.pyth-network-proxy/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'pyth-network-proxy',
