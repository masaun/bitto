...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CIPHERTRAIL_DATA_REFINERY_ADDRESS || '';

export default function Ciphertrail_Data_RefineryPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'ciphertrail-data-refinery',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ciphertrail-data-refinery/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ciphertrail-data-refinery',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>ciphertrail-data-refinery</h1>
