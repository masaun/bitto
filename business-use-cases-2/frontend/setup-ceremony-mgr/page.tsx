...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SETUP_CEREMONY_MGR_ADDRESS || '';

export default function Setup_Ceremony_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.setup-ceremony-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'setup-ceremony-mgr',
