...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_YELLOWCRESTVEIL_SECURE_ADAPTER_ADDRESS || '';

export default function Yellowcrestveil_Secure_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'yellowcrestveil-secure-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.yellowcrestveil-secure-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'yellowcrestveil-secure-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>yellowcrestveil-secure-adapter</h1>
