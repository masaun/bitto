...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ARB_SEQUENCER_CHECK_ADDRESS || '';

export default function Arb_Sequencer_CheckPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.arb-sequencer-check/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'arb-sequencer-check',
