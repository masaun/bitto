...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TREASURY_CAPITAL_ALLOCATOR_ADDRESS || '';

export default function Treasury_Capital_AllocatorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'treasury-capital-allocator',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.treasury-capital-allocator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'treasury-capital-allocator',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>treasury-capital-allocator</h1>
