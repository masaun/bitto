...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SOLVENCY_SCORE_AI_ADDRESS || '';

export default function Solvency_Score_AiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.solvency-score-ai/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'solvency-score-ai',
