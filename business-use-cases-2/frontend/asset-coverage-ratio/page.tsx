...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ASSET_COVERAGE_RATIO_ADDRESS || '';

export default function Asset_Coverage_RatioPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.asset-coverage-ratio/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'asset-coverage-ratio',
