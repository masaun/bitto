...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EXIT_STRATEGY_ROUTER_NEW_ADDRESS || '';

export default function Exit_Strategy_Router_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.exit-strategy-router-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'exit-strategy-router-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>exit-strategy-router-new</h1>
