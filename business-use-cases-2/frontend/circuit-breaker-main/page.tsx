...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CIRCUIT_BREAKER_MAIN_ADDRESS || '';

export default function Circuit_Breaker_MainPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.circuit-breaker-main/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'circuit-breaker-main',
