...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ATTESTATION_ENGINE_V1_ADDRESS || '';

export default function Attestation_Engine_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.attestation-engine-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'attestation-engine-v1',
