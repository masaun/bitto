...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COBALTGUARD_SECRET_STREAM_ADDRESS || '';

export default function Cobaltguard_Secret_StreamPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'cobaltguard-secret-stream',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cobaltguard-secret-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cobaltguard-secret-stream',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>cobaltguard-secret-stream</h1>
