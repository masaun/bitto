...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FACIAL_RECOG_CONSENT_ADDRESS || '';

export default function Facial_Recog_ConsentPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.facial-recog-consent/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'facial-recog-consent',
