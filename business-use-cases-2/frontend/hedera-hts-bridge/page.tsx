...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HEDERA_HTS_BRIDGE_ADDRESS || '';

export default function Hedera_Hts_BridgePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.hedera-hts-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'hedera-hts-bridge',
