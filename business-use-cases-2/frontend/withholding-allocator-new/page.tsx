...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WITHHOLDING_ALLOCATOR_NEW_ADDRESS || '';

export default function Withholding_Allocator_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'withholding-allocator-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.withholding-allocator-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'withholding-allocator-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>withholding-allocator-new</h1>
