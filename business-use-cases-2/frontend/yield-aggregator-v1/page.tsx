...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_YIELD_AGGREGATOR_V1_ADDRESS || '';

export default function Yield_Aggregator_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.yield-aggregator-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'yield-aggregator-v1',
