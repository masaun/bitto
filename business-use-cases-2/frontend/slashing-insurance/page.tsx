...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SLASHING_INSURANCE_ADDRESS || '';

export default function Slashing_InsurancePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.slashing-insurance/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'slashing-insurance',
