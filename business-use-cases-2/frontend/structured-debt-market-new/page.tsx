...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STRUCTURED_DEBT_MARKET_NEW_ADDRESS || '';

export default function Structured_Debt_Market_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'structured-debt-market-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.structured-debt-market-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'structured-debt-market-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>structured-debt-market-new</h1>
