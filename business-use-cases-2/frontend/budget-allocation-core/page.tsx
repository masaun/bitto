...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BUDGET_ALLOCATION_CORE_ADDRESS || '';

export default function Budget_Allocation_CorePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.budget-allocation-core/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'budget-allocation-core',
