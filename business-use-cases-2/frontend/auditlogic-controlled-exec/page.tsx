...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDITLOGIC_CONTROLLED_EXEC_ADDRESS || '';

export default function Auditlogic_Controlled_ExecPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'auditlogic-controlled-exec',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auditlogic-controlled-exec/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auditlogic-controlled-exec',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>auditlogic-controlled-exec</h1>
