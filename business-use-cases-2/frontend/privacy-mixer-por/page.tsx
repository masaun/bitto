...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PRIVACY_MIXER_POR_ADDRESS || '';

export default function Privacy_Mixer_PorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.privacy-mixer-por/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'privacy-mixer-por',
