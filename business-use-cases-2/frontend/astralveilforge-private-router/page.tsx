...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ASTRALVEILFORGE_PRIVATE_ROUTER_ADDRESS || '';

export default function Astralveilforge_Private_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'astralveilforge-private-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.astralveilforge-private-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'astralveilforge-private-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>astralveilforge-private-router</h1>
