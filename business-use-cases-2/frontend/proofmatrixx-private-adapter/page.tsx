...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROOFMATRIXX_PRIVATE_ADAPTER_ADDRESS || '';

export default function Proofmatrixx_Private_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'proofmatrixx-private-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.proofmatrixx-private-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'proofmatrixx-private-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>proofmatrixx-private-adapter</h1>
