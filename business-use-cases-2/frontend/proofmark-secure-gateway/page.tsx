...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROOFMARK_SECURE_GATEWAY_ADDRESS || '';

export default function Proofmark_Secure_GatewayPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.proofmark-secure-gateway/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'proofmark-secure-gateway',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>proofmark-secure-gateway</h1>
