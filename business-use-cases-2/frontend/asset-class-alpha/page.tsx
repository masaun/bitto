...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ASSET_CLASS_ALPHA_ADDRESS || '';

export default function Asset_Class_AlphaPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.asset-class-alpha/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'asset-class-alpha',
