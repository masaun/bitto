...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DATA_LINEAGE_TRACKER_ADDRESS || '';

export default function Data_Lineage_TrackerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.data-lineage-tracker/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'data-lineage-tracker',
