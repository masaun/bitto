...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DAWNFORGE_CONFIDENTIAL_TRANSFORM_ADDRESS || '';

export default function Dawnforge_Confidential_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'dawnforge-confidential-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.dawnforge-confidential-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'dawnforge-confidential-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>dawnforge-confidential-transform</h1>
