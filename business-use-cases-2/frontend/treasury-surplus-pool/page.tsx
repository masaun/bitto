...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TREASURY_SURPLUS_POOL_ADDRESS || '';

export default function Treasury_Surplus_PoolPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.treasury-surplus-pool/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'treasury-surplus-pool',
