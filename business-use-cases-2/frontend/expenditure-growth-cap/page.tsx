...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EXPENDITURE_GROWTH_CAP_ADDRESS || '';

export default function Expenditure_Growth_CapPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.expenditure-growth-cap/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'expenditure-growth-cap',
