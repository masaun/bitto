...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDITCONTROLLER_SECURE_HUB_ADDRESS || '';

export default function Auditcontroller_Secure_HubPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'auditcontroller-secure-hub',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auditcontroller-secure-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auditcontroller-secure-hub',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>auditcontroller-secure-hub</h1>
