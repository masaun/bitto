...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RATING_UPDATE_ADAPTER_NEW_ADDRESS || '';

export default function Rating_Update_Adapter_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'rating-update-adapter-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.rating-update-adapter-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'rating-update-adapter-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>rating-update-adapter-new</h1>
