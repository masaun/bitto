...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BANKRUPTCY_LIQUIDATOR_ADDRESS || '';

export default function Bankruptcy_LiquidatorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bankruptcy-liquidator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bankruptcy-liquidator',
