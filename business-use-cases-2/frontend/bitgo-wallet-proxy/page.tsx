...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BITGO_WALLET_PROXY_ADDRESS || '';

export default function Bitgo_Wallet_ProxyPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bitgo-wallet-proxy/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bitgo-wallet-proxy',
