...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PRIMARY_BALANCE_CALCULATOR_ADDRESS || '';

export default function Primary_Balance_CalculatorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'primary-balance-calculator',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.primary-balance-calculator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'primary-balance-calculator',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>primary-balance-calculator</h1>
