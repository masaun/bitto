...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SECURITY_CAMERA_LOGIC_ADDRESS || '';

export default function Security_Camera_LogicPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.security-camera-logic/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'security-camera-logic',
