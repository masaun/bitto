...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EMBERVAULT_OFFCHAIN_PARSER_ADDRESS || '';

export default function Embervault_Offchain_ParserPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'embervault-offchain-parser',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.embervault-offchain-parser/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'embervault-offchain-parser',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>embervault-offchain-parser</h1>
