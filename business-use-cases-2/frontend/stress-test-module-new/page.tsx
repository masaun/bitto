...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STRESS_TEST_MODULE_NEW_ADDRESS || '';

export default function Stress_Test_Module_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.stress-test-module-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'stress-test-module-new',
