...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TAX_GOVERNANCE_MODULE_ADDRESS || '';

export default function Tax_Governance_ModulePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tax-governance-module/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tax-governance-module',
