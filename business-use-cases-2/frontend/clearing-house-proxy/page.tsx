...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CLEARING_HOUSE_PROXY_ADDRESS || '';

export default function Clearing_House_ProxyPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.clearing-house-proxy/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'clearing-house-proxy',
