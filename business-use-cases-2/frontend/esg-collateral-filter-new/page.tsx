...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ESG_COLLATERAL_FILTER_NEW_ADDRESS || '';

export default function Esg_Collateral_Filter_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'esg-collateral-filter-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.esg-collateral-filter-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'esg-collateral-filter-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>esg-collateral-filter-new</h1>
