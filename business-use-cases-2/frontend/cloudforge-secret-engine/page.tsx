...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CLOUDFORGE_SECRET_ENGINE_ADDRESS || '';

export default function Cloudforge_Secret_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cloudforge-secret-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cloudforge-secret-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>cloudforge-secret-engine</h1>
