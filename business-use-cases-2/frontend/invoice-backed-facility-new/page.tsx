...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INVOICE_BACKED_FACILITY_NEW_ADDRESS || '';

export default function Invoice_Backed_Facility_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'invoice-backed-facility-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.invoice-backed-facility-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'invoice-backed-facility-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>invoice-backed-facility-new</h1>
