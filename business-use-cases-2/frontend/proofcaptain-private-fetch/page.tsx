...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROOFCAPTAIN_PRIVATE_FETCH_ADDRESS || '';

export default function Proofcaptain_Private_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'proofcaptain-private-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.proofcaptain-private-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'proofcaptain-private-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>proofcaptain-private-fetch</h1>
