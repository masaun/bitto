...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SECURITIZE_ID_INT_ADDRESS || '';

export default function Securitize_Id_IntPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.securitize-id-int/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'securitize-id-int',
