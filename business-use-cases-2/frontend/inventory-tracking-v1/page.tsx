...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INVENTORY_TRACKING_V1_ADDRESS || '';

export default function Inventory_Tracking_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.inventory-tracking-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'inventory-tracking-v1',
