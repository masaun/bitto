...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SOLSTICEVEIL_PRIVATE_ENGINE_ADDRESS || '';

export default function Solsticeveil_Private_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'solsticeveil-private-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.solsticeveil-private-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'solsticeveil-private-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>solsticeveil-private-engine</h1>
