...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FEE_DISTRIBUTOR_V2_ADDRESS || '';

export default function Fee_Distributor_V2Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fee-distributor-v2/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fee-distributor-v2',
