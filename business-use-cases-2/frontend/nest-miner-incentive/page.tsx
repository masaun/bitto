...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NEST_MINER_INCENTIVE_ADDRESS || '';

export default function Nest_Miner_IncentivePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.nest-miner-incentive/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'nest-miner-incentive',
