...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SILVERMESH_OFFCHAIN_PARSER_ADDRESS || '';

export default function Silvermesh_Offchain_ParserPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'silvermesh-offchain-parser',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.silvermesh-offchain-parser/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'silvermesh-offchain-parser',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>silvermesh-offchain-parser</h1>
