...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_IONCREST_PROTECTED_SYNC_ADDRESS || '';

export default function Ioncrest_Protected_SyncPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ioncrest-protected-sync/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ioncrest-protected-sync',
