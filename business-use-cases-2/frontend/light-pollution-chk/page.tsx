...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LIGHT_POLLUTION_CHK_ADDRESS || '';

export default function Light_Pollution_ChkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.light-pollution-chk/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'light-pollution-chk',
