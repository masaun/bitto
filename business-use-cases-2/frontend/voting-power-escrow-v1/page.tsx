...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VOTING_POWER_ESCROW_V1_ADDRESS || '';

export default function Voting_Power_Escrow_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.voting-power-escrow-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'voting-power-escrow-v1',
