...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COMMUNITY_SENTIMENT_ADDRESS || '';

export default function Community_SentimentPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.community-sentiment/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'community-sentiment',
