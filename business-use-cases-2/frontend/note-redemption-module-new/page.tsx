...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NOTE_REDEMPTION_MODULE_NEW_ADDRESS || '';

export default function Note_Redemption_Module_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'note-redemption-module-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.note-redemption-module-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'note-redemption-module-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>note-redemption-module-new</h1>
