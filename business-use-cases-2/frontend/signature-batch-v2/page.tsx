...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SIGNATURE_BATCH_V2_ADDRESS || '';

export default function Signature_Batch_V2Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.signature-batch-v2/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'signature-batch-v2',
