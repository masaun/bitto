...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SCHOOL_DISTRICT_EVAL_ADDRESS || '';

export default function School_District_EvalPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.school-district-eval/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'school-district-eval',
