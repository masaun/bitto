...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LOCKED_SUPPLY_CALC_ADDRESS || '';

export default function Locked_Supply_CalcPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.locked-supply-calc/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'locked-supply-calc',
