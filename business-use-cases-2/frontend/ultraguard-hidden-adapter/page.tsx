...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ULTRAGUARD_HIDDEN_ADAPTER_ADDRESS || '';

export default function Ultraguard_Hidden_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'ultraguard-hidden-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ultraguard-hidden-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ultraguard-hidden-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>ultraguard-hidden-adapter</h1>
