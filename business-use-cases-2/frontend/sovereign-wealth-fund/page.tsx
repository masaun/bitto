...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SOVEREIGN_WEALTH_FUND_ADDRESS || '';

export default function Sovereign_Wealth_FundPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sovereign-wealth-fund/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sovereign-wealth-fund',
