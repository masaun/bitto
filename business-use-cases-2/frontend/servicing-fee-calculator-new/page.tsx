...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SERVICING_FEE_CALCULATOR_NEW_ADDRESS || '';

export default function Servicing_Fee_Calculator_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'servicing-fee-calculator-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.servicing-fee-calculator-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'servicing-fee-calculator-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>servicing-fee-calculator-new</h1>
