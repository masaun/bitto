...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FIGI_IDENTIFIER_HUB_ADDRESS || '';

export default function Figi_Identifier_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.figi-identifier-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'figi-identifier-hub',
