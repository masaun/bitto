...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HEDGE_COUNTERPARTY_MODULE_NEW_ADDRESS || '';

export default function Hedge_Counterparty_Module_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'hedge-counterparty-module-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.hedge-counterparty-module-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'hedge-counterparty-module-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>hedge-counterparty-module-new</h1>
