...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AVALANCHE_GATEWAY_ADDRESS || '';

export default function Avalanche_GatewayPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.avalanche-gateway/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'avalanche-gateway',
