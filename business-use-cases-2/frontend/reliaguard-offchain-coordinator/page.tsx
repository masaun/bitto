...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RELIAGUARD_OFFCHAIN_COORDINATOR_ADDRESS || '';

export default function Reliaguard_Offchain_CoordinatorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'reliaguard-offchain-coordinator',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.reliaguard-offchain-coordinator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'reliaguard-offchain-coordinator',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>reliaguard-offchain-coordinator</h1>
