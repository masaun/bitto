...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SUI_OBJECT_BRIDGE_ADDRESS || '';

export default function Sui_Object_BridgePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sui-object-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sui-object-bridge',
