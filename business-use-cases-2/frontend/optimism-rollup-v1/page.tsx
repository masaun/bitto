...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OPTIMISM_ROLLUP_V1_ADDRESS || '';

export default function Optimism_Rollup_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.optimism-rollup-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'optimism-rollup-v1',
