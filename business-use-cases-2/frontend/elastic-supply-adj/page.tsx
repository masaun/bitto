...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ELASTIC_SUPPLY_ADJ_ADDRESS || '';

export default function Elastic_Supply_AdjPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.elastic-supply-adj/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'elastic-supply-adj',
