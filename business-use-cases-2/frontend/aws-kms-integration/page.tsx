...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AWS_KMS_INTEGRATION_ADDRESS || '';

export default function Aws_Kms_IntegrationPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.aws-kms-integration/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'aws-kms-integration',
