...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERIVAULT_CONTROLLED_RUNNER_ADDRESS || '';

export default function Verivault_Controlled_RunnerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'verivault-controlled-runner',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.verivault-controlled-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'verivault-controlled-runner',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>verivault-controlled-runner</h1>
