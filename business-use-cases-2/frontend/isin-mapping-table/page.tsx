...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ISIN_MAPPING_TABLE_ADDRESS || '';

export default function Isin_Mapping_TablePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.isin-mapping-table/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'isin-mapping-table',
