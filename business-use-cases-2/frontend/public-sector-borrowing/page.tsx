...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PUBLIC_SECTOR_BORROWING_ADDRESS || '';

export default function Public_Sector_BorrowingPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.public-sector-borrowing/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'public-sector-borrowing',
