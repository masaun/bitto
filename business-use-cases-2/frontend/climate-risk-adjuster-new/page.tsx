...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CLIMATE_RISK_ADJUSTER_NEW_ADDRESS || '';

export default function Climate_Risk_Adjuster_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'climate-risk-adjuster-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.climate-risk-adjuster-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'climate-risk-adjuster-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>climate-risk-adjuster-new</h1>
