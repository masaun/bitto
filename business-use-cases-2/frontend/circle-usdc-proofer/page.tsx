...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CIRCLE_USDC_PROOFER_ADDRESS || '';

export default function Circle_Usdc_ProoferPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.circle-usdc-proofer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'circle-usdc-proofer',
