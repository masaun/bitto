...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WRAP_UNWRAP_AUDITOR_ADDRESS || '';

export default function Wrap_Unwrap_AuditorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.wrap-unwrap-auditor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'wrap-unwrap-auditor',
