...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ENCRYPTED_DATA_POUCH_ADDRESS || '';

export default function Encrypted_Data_PouchPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.encrypted-data-pouch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'encrypted-data-pouch',
