...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INFLATION_ADJUST_RENT_ADDRESS || '';

export default function Inflation_Adjust_RentPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.inflation-adjust-rent/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'inflation-adjust-rent',
