...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EARLY_AMORTIZATION_TRIGGER_NEW_ADDRESS || '';

export default function Early_Amortization_Trigger_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'early-amortization-trigger-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.early-amortization-trigger-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'early-amortization-trigger-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>early-amortization-trigger-new</h1>
