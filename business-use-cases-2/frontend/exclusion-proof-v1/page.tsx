...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EXCLUSION_PROOF_V1_ADDRESS || '';

export default function Exclusion_Proof_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.exclusion-proof-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'exclusion-proof-v1',
