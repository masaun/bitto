...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PUBLIC_EXPENDITURE_LEDGER_ADDRESS || '';

export default function Public_Expenditure_LedgerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'public-expenditure-ledger',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.public-expenditure-ledger/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'public-expenditure-ledger',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>public-expenditure-ledger</h1>
