...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DELTA_NEUTRAL_BOT_ADDRESS || '';

export default function Delta_Neutral_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.delta-neutral-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'delta-neutral-bot',
