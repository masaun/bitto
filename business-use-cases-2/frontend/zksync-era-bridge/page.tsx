...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ZKSYNC_ERA_BRIDGE_ADDRESS || '';

export default function Zksync_Era_BridgePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.zksync-era-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'zksync-era-bridge',
