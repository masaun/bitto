...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COSMOS_IBC_HANDLER_ADDRESS || '';

export default function Cosmos_Ibc_HandlerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cosmos-ibc-handler/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cosmos-ibc-handler',
