...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_JADECRYPT_CONFIDENTIAL_FETCHER_ADDRESS || '';

export default function Jadecrypt_Confidential_FetcherPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'jadecrypt-confidential-fetcher',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.jadecrypt-confidential-fetcher/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'jadecrypt-confidential-fetcher',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>jadecrypt-confidential-fetcher</h1>
