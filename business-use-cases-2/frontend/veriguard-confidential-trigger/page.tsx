...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERIGUARD_CONFIDENTIAL_TRIGGER_ADDRESS || '';

export default function Veriguard_Confidential_TriggerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'veriguard-confidential-trigger',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veriguard-confidential-trigger/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veriguard-confidential-trigger',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>veriguard-confidential-trigger</h1>
