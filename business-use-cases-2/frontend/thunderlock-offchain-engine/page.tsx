...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_THUNDERLOCK_OFFCHAIN_ENGINE_ADDRESS || '';

export default function Thunderlock_Offchain_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'thunderlock-offchain-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.thunderlock-offchain-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'thunderlock-offchain-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>thunderlock-offchain-engine</h1>
