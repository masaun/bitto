...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AD_SPEND_OPTIMIZER_ADDRESS || '';

export default function Ad_Spend_OptimizerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ad-spend-optimizer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ad-spend-optimizer',
