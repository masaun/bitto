...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ARBITRAGE_FINDER_AI_ADDRESS || '';

export default function Arbitrage_Finder_AiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.arbitrage-finder-ai/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'arbitrage-finder-ai',
