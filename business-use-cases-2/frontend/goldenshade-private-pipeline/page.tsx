...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GOLDENSHADE_PRIVATE_PIPELINE_ADDRESS || '';

export default function Goldenshade_Private_PipelinePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'goldenshade-private-pipeline',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.goldenshade-private-pipeline/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'goldenshade-private-pipeline',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>goldenshade-private-pipeline</h1>
