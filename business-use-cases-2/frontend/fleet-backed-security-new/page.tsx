...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FLEET_BACKED_SECURITY_NEW_ADDRESS || '';

export default function Fleet_Backed_Security_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'fleet-backed-security-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fleet-backed-security-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fleet-backed-security-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>fleet-backed-security-new</h1>
