...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ALGORAND_INDEX_SYNC_ADDRESS || '';

export default function Algorand_Index_SyncPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.algorand-index-sync/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'algorand-index-sync',
