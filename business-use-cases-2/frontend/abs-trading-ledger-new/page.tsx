...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ABS_TRADING_LEDGER_NEW_ADDRESS || '';

export default function Abs_Trading_Ledger_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.abs-trading-ledger-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'abs-trading-ledger-new',
