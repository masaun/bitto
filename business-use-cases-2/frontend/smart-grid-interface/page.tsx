...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SMART_GRID_INTERFACE_ADDRESS || '';

export default function Smart_Grid_InterfacePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.smart-grid-interface/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'smart-grid-interface',
