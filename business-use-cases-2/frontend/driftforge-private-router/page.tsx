...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DRIFTFORGE_PRIVATE_ROUTER_ADDRESS || '';

export default function Driftforge_Private_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'driftforge-private-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.driftforge-private-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'driftforge-private-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>driftforge-private-router</h1>
