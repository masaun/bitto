...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_MONITORING_DASHBOARD_ADDRESS || '';

export default function Fiscal_Monitoring_DashboardPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'fiscal-monitoring-dashboard',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-monitoring-dashboard/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-monitoring-dashboard',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>fiscal-monitoring-dashboard</h1>
