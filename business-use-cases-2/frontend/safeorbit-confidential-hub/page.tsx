...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SAFEORBIT_CONFIDENTIAL_HUB_ADDRESS || '';

export default function Safeorbit_Confidential_HubPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'safeorbit-confidential-hub',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.safeorbit-confidential-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'safeorbit-confidential-hub',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>safeorbit-confidential-hub</h1>
