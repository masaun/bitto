...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BURNER_ADDRESS_MGR_ADDRESS || '';

export default function Burner_Address_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.burner-address-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'burner-address-mgr',
