...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RE_STAKING_MODULE_ADDRESS || '';

export default function Re_Staking_ModulePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.re-staking-module/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 're-staking-module',
