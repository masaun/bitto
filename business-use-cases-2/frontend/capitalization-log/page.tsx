...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CAPITALIZATION_LOG_ADDRESS || '';

export default function Capitalization_LogPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.capitalization-log/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'capitalization-log',
