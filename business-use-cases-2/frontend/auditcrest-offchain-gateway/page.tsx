...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDITCREST_OFFCHAIN_GATEWAY_ADDRESS || '';

export default function Auditcrest_Offchain_GatewayPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'auditcrest-offchain-gateway',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auditcrest-offchain-gateway/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auditcrest-offchain-gateway',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>auditcrest-offchain-gateway</h1>
