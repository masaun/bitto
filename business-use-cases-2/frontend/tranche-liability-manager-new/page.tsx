...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRANCHE_LIABILITY_MANAGER_NEW_ADDRESS || '';

export default function Tranche_Liability_Manager_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tranche-liability-manager-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tranche-liability-manager-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tranche-liability-manager-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tranche-liability-manager-new</h1>
