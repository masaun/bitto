...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SETTLEMENT_FINALIZER_ADDRESS || '';

export default function Settlement_FinalizerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.settlement-finalizer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'settlement-finalizer',
