...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_YELLOWSTONEVEIL_SECURE_FETCH_ADDRESS || '';

export default function Yellowstoneveil_Secure_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'yellowstoneveil-secure-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.yellowstoneveil-secure-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'yellowstoneveil-secure-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>yellowstoneveil-secure-fetch</h1>
