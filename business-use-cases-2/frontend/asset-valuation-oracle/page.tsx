...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ASSET_VALUATION_ORACLE_ADDRESS || '';

export default function Asset_Valuation_OraclePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.asset-valuation-oracle/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'asset-valuation-oracle',
