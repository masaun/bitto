...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AGENT_HANDSHAKE_V1_ADDRESS || '';

export default function Agent_Handshake_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.agent-handshake-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'agent-handshake-v1',
