...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WHOLESALE_DEAL_FINDER_ADDRESS || '';

export default function Wholesale_Deal_FinderPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.wholesale-deal-finder/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'wholesale-deal-finder',
