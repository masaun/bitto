...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REVENUE_SHARING_MODEL_ADDRESS || '';

export default function Revenue_Sharing_ModelPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.revenue-sharing-model/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'revenue-sharing-model',
