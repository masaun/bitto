...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DYNAMIC_PRICING_ENGINE_V1_ADDRESS || '';

export default function Dynamic_Pricing_Engine_V1Page() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'dynamic-pricing-engine-v1',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.dynamic-pricing-engine-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'dynamic-pricing-engine-v1',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>dynamic-pricing-engine-v1</h1>
