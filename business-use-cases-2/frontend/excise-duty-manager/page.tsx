...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EXCISE_DUTY_MANAGER_ADDRESS || '';

export default function Excise_Duty_ManagerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.excise-duty-manager/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'excise-duty-manager',
