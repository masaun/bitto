...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STRUCTURED_ABS_EXCHANGE_NEW_ADDRESS || '';

export default function Structured_Abs_Exchange_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'structured-abs-exchange-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.structured-abs-exchange-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'structured-abs-exchange-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>structured-abs-exchange-new</h1>
