...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ALCHEMY_RPC_PROXY_ADDRESS || '';

export default function Alchemy_Rpc_ProxyPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.alchemy-rpc-proxy/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'alchemy-rpc-proxy',
