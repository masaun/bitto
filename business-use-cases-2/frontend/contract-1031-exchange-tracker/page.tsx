...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_1031_EXCHANGE_TRACKER_ADDRESS || '';

export default function 1031_Exchange_TrackerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.1031-exchange-tracker/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: '1031-exchange-tracker',
