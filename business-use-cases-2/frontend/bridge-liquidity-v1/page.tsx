...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BRIDGE_LIQUIDITY_V1_ADDRESS || '';

export default function Bridge_Liquidity_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bridge-liquidity-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bridge-liquidity-v1',
