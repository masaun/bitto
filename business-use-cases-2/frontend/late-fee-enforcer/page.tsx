...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LATE_FEE_ENFORCER_ADDRESS || '';

export default function Late_Fee_EnforcerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.late-fee-enforcer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'late-fee-enforcer',
