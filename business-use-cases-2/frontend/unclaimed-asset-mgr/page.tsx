...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_UNCLAIMED_ASSET_MGR_ADDRESS || '';

export default function Unclaimed_Asset_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.unclaimed-asset-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'unclaimed-asset-mgr',
