...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RIGHT_OF_FIRST_REF_ADDRESS || '';

export default function Right_Of_First_RefPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.right-of-first-ref/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'right-of-first-ref',
