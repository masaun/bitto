...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SECURITY_AUDIT_CHK_ADDRESS || '';

export default function Security_Audit_ChkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.security-audit-chk/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'security-audit-chk',
