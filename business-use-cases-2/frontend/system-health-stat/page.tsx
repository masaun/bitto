...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SYSTEM_HEALTH_STAT_ADDRESS || '';

export default function System_Health_StatPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.system-health-stat/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'system-health-stat',
