...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CATCH_UP_PROVISION_ADDRESS || '';

export default function Catch_Up_ProvisionPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.catch-up-provision/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'catch-up-provision',
