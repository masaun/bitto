...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AMENDMENT_TRACKER_NEW_ADDRESS || '';

export default function Amendment_Tracker_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.amendment-tracker-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'amendment-tracker-new',
