...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GOVERNMENT_CASHFLOW_LEDGER_ADDRESS || '';

export default function Government_Cashflow_LedgerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'government-cashflow-ledger',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.government-cashflow-ledger/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'government-cashflow-ledger',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>government-cashflow-ledger</h1>
