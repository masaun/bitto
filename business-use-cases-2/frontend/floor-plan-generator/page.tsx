...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FLOOR_PLAN_GENERATOR_ADDRESS || '';

export default function Floor_Plan_GeneratorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.floor-plan-generator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'floor-plan-generator',
