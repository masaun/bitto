...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DATA_ROOM_REGISTRY_NEW_ADDRESS || '';

export default function Data_Room_Registry_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.data-room-registry-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'data-room-registry-new',
