...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BINARY_TREE_WALKER_ADDRESS || '';

export default function Binary_Tree_WalkerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.binary-tree-walker/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'binary-tree-walker',
