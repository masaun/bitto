...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LEASE_ABSTRACTION_AGENT_ADDRESS || '';

export default function Lease_Abstraction_AgentPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.lease-abstraction-agent/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'lease-abstraction-agent',
