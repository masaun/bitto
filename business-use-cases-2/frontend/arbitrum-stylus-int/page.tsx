...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ARBITRUM_STYLUS_INT_ADDRESS || '';

export default function Arbitrum_Stylus_IntPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.arbitrum-stylus-int/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'arbitrum-stylus-int',
