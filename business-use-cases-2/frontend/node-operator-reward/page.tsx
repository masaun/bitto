...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NODE_OPERATOR_REWARD_ADDRESS || '';

export default function Node_Operator_RewardPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.node-operator-reward/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'node-operator-reward',
