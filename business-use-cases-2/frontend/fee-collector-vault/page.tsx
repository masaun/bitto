...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FEE_COLLECTOR_VAULT_ADDRESS || '';

export default function Fee_Collector_VaultPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fee-collector-vault/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fee-collector-vault',
