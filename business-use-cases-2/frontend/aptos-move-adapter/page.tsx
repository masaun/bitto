...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_APTOS_MOVE_ADAPTER_ADDRESS || '';

export default function Aptos_Move_AdapterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.aptos-move-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'aptos-move-adapter',
