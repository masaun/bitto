...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TEAM_ALLOCATION_LOG_ADDRESS || '';

export default function Team_Allocation_LogPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.team-allocation-log/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'team-allocation-log',
