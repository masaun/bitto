...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_API_SPEC_VALIDATOR_ADDRESS || '';

export default function Api_Spec_ValidatorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.api-spec-validator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'api-spec-validator',
