...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_APPRAISAL_MODEL_VALIDATOR_ADDRESS || '';

export default function Appraisal_Model_ValidatorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'appraisal-model-validator',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.appraisal-model-validator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'appraisal-model-validator',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>appraisal-model-validator</h1>
