...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ABS_REPURCHASE_MODULE_NEW_ADDRESS || '';

export default function Abs_Repurchase_Module_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'abs-repurchase-module-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.abs-repurchase-module-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'abs-repurchase-module-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>abs-repurchase-module-new</h1>
