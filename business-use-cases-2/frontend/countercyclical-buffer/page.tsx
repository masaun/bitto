...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COUNTERCYCLICAL_BUFFER_ADDRESS || '';

export default function Countercyclical_BufferPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.countercyclical-buffer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'countercyclical-buffer',
