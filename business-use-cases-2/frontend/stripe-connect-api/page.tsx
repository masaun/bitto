...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STRIPE_CONNECT_API_ADDRESS || '';

export default function Stripe_Connect_ApiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.stripe-connect-api/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'stripe-connect-api',
