...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BUG_REPORT_ANALYZER_ADDRESS || '';

export default function Bug_Report_AnalyzerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bug-report-analyzer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bug-report-analyzer',
