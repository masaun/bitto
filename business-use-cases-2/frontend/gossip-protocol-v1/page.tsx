...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GOSSIP_PROTOCOL_V1_ADDRESS || '';

export default function Gossip_Protocol_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.gossip-protocol-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'gossip-protocol-v1',
