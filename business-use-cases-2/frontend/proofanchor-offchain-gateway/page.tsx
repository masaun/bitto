...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROOFANCHOR_OFFCHAIN_GATEWAY_ADDRESS || '';

export default function Proofanchor_Offchain_GatewayPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'proofanchor-offchain-gateway',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.proofanchor-offchain-gateway/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'proofanchor-offchain-gateway',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>proofanchor-offchain-gateway</h1>
