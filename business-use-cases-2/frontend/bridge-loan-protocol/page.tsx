...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BRIDGE_LOAN_PROTOCOL_ADDRESS || '';

export default function Bridge_Loan_ProtocolPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bridge-loan-protocol/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bridge-loan-protocol',
