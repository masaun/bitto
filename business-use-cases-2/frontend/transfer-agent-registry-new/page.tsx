...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRANSFER_AGENT_REGISTRY_NEW_ADDRESS || '';

export default function Transfer_Agent_Registry_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'transfer-agent-registry-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.transfer-agent-registry-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'transfer-agent-registry-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>transfer-agent-registry-new</h1>
