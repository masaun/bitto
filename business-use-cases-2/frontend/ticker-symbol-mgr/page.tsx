...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TICKER_SYMBOL_MGR_ADDRESS || '';

export default function Ticker_Symbol_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ticker-symbol-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ticker-symbol-mgr',
