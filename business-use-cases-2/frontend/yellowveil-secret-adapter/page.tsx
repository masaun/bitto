...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_YELLOWVEIL_SECRET_ADAPTER_ADDRESS || '';

export default function Yellowveil_Secret_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'yellowveil-secret-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.yellowveil-secret-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'yellowveil-secret-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>yellowveil-secret-adapter</h1>
