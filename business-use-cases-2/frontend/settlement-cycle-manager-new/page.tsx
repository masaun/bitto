...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SETTLEMENT_CYCLE_MANAGER_NEW_ADDRESS || '';

export default function Settlement_Cycle_Manager_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'settlement-cycle-manager-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.settlement-cycle-manager-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'settlement-cycle-manager-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>settlement-cycle-manager-new</h1>
