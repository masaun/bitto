...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDITMONARCH_SECURE_DISPATCH_ADDRESS || '';

export default function Auditmonarch_Secure_DispatchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'auditmonarch-secure-dispatch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auditmonarch-secure-dispatch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auditmonarch-secure-dispatch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>auditmonarch-secure-dispatch</h1>
