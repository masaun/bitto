...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TREASURY_CASH_MANAGER_ADDRESS || '';

export default function Treasury_Cash_ManagerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.treasury-cash-manager/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'treasury-cash-manager',
