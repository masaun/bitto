...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RENT_COLLECTION_AUTO_ADDRESS || '';

export default function Rent_Collection_AutoPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.rent-collection-auto/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'rent-collection-auto',
