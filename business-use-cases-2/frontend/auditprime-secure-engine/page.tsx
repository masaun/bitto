...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDITPRIME_SECURE_ENGINE_ADDRESS || '';

export default function Auditprime_Secure_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auditprime-secure-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auditprime-secure-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>auditprime-secure-engine</h1>
