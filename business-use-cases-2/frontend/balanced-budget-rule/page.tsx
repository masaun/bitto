...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BALANCED_BUDGET_RULE_ADDRESS || '';

export default function Balanced_Budget_RulePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.balanced-budget-rule/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'balanced-budget-rule',
