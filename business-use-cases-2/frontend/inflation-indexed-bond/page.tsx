...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INFLATION_INDEXED_BOND_ADDRESS || '';

export default function Inflation_Indexed_BondPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.inflation-indexed-bond/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'inflation-indexed-bond',
