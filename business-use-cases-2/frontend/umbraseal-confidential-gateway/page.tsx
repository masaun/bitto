...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_UMBRASEAL_CONFIDENTIAL_GATEWAY_ADDRESS || '';

export default function Umbraseal_Confidential_GatewayPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'umbraseal-confidential-gateway',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.umbraseal-confidential-gateway/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'umbraseal-confidential-gateway',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>umbraseal-confidential-gateway</h1>
