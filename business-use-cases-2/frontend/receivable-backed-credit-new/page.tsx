...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RECEIVABLE_BACKED_CREDIT_NEW_ADDRESS || '';

export default function Receivable_Backed_Credit_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'receivable-backed-credit-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.receivable-backed-credit-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'receivable-backed-credit-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>receivable-backed-credit-new</h1>
