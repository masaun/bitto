...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SHARE_REDEMPTION_BOT_ADDRESS || '';

export default function Share_Redemption_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.share-redemption-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'share-redemption-bot',
