...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_EXPENDITURE_CORE_ADDRESS || '';

export default function Fiscal_Expenditure_CorePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-expenditure-core/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-expenditure-core',
