...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ATOMIC_SWAP_STATUS_ADDRESS || '';

export default function Atomic_Swap_StatusPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.atomic-swap-status/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'atomic-swap-status',
