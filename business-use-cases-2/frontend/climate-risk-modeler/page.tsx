...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CLIMATE_RISK_MODELER_ADDRESS || '';

export default function Climate_Risk_ModelerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.climate-risk-modeler/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'climate-risk-modeler',
