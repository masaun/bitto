...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NEAR_SHARD_MONITOR_ADDRESS || '';

export default function Near_Shard_MonitorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.near-shard-monitor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'near-shard-monitor',
