...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ULTRAVEILFORGE_PRIVATE_STREAM_ADDRESS || '';

export default function Ultraveilforge_Private_StreamPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'ultraveilforge-private-stream',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ultraveilforge-private-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ultraveilforge-private-stream',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>ultraveilforge-private-stream</h1>
