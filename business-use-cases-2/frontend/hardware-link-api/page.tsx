...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HARDWARE_LINK_API_ADDRESS || '';

export default function Hardware_Link_ApiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.hardware-link-api/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'hardware-link-api',
