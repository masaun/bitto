...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REVENUE_STABILIZATION_FUND_ADDRESS || '';

export default function Revenue_Stabilization_FundPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.revenue-stabilization-fund/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'revenue-stabilization-fund',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>revenue-stabilization-fund</h1>
