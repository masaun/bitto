...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROOFASSURE_PRIVATE_RUNNER_ADDRESS || '';

export default function Proofassure_Private_RunnerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'proofassure-private-runner',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.proofassure-private-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'proofassure-private-runner',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>proofassure-private-runner</h1>
