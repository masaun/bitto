...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TREASURY_YIELD_CURVE_ADDRESS || '';

export default function Treasury_Yield_CurvePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.treasury-yield-curve/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'treasury-yield-curve',
