...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_UNISWAP_V3_POOL_AUD_ADDRESS || '';

export default function Uniswap_V3_Pool_AudPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.uniswap-v3-pool-aud/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'uniswap-v3-pool-aud',
