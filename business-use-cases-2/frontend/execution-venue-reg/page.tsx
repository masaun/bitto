...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EXECUTION_VENUE_REG_ADDRESS || '';

export default function Execution_Venue_RegPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.execution-venue-reg/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'execution-venue-reg',
