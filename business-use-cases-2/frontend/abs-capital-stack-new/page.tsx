...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ABS_CAPITAL_STACK_NEW_ADDRESS || '';

export default function Abs_Capital_Stack_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.abs-capital-stack-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'abs-capital-stack-new',
