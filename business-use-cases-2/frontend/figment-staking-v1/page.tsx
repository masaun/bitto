...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FIGMENT_STAKING_V1_ADDRESS || '';

export default function Figment_Staking_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.figment-staking-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'figment-staking-v1',
