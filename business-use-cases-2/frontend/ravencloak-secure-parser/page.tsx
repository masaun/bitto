...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RAVENCLOAK_SECURE_PARSER_ADDRESS || '';

export default function Ravencloak_Secure_ParserPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ravencloak-secure-parser/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ravencloak-secure-parser',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>ravencloak-secure-parser</h1>
