...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERIPATH_TRACE_ENGINE_ADDRESS || '';

export default function Veripath_Trace_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veripath-trace-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veripath-trace-engine',
