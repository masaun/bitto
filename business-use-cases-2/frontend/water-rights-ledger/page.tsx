...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WATER_RIGHTS_LEDGER_ADDRESS || '';

export default function Water_Rights_LedgerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.water-rights-ledger/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'water-rights-ledger',
