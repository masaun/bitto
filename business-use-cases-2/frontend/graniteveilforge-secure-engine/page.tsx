...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GRANITEVEILFORGE_SECURE_ENGINE_ADDRESS || '';

export default function Graniteveilforge_Secure_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'graniteveilforge-secure-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.graniteveilforge-secure-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'graniteveilforge-secure-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>graniteveilforge-secure-engine</h1>
