...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_KRYPTONMASK_PRIVATE_BUFFER_ADDRESS || '';

export default function Kryptonmask_Private_BufferPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'kryptonmask-private-buffer',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.kryptonmask-private-buffer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'kryptonmask-private-buffer',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>kryptonmask-private-buffer</h1>
