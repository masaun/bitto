...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LEAD_GENERATION_NEURAL_ADDRESS || '';

export default function Lead_Generation_NeuralPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.lead-generation-neural/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'lead-generation-neural',
