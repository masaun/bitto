...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SIGNATURE_REQUEST_PROXY_ADDRESS || '';

export default function Signature_Request_ProxyPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.signature-request-proxy/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'signature-request-proxy',
