...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACECOMMAND_OFFCHAIN_ADAPTER_ADDRESS || '';

export default function Tracecommand_Offchain_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tracecommand-offchain-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tracecommand-offchain-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tracecommand-offchain-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tracecommand-offchain-adapter</h1>
