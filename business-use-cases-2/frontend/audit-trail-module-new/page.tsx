...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDIT_TRAIL_MODULE_NEW_ADDRESS || '';

export default function Audit_Trail_Module_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.audit-trail-module-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'audit-trail-module-new',
