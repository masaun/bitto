...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_UPLINKCLOAK_SECURE_COLLECTOR_ADDRESS || '';

export default function Uplinkcloak_Secure_CollectorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'uplinkcloak-secure-collector',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.uplinkcloak-secure-collector/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'uplinkcloak-secure-collector',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>uplinkcloak-secure-collector</h1>
