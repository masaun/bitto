...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BONUS_ISSUE_MODULE_ADDRESS || '';

export default function Bonus_Issue_ModulePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bonus-issue-module/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bonus-issue-module',
