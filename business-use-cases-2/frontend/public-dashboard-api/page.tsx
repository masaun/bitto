...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PUBLIC_DASHBOARD_API_ADDRESS || '';

export default function Public_Dashboard_ApiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.public-dashboard-api/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'public-dashboard-api',
