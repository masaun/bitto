...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BLOCK_TRADE_EXECUTOR_ADDRESS || '';

export default function Block_Trade_ExecutorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.block-trade-executor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'block-trade-executor',
