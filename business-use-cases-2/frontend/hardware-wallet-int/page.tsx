...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HARDWARE_WALLET_INT_ADDRESS || '';

export default function Hardware_Wallet_IntPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.hardware-wallet-int/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'hardware-wallet-int',
