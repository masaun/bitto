...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BUDGET_CONTROL_TOWER_ADDRESS || '';

export default function Budget_Control_TowerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.budget-control-tower/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'budget-control-tower',
