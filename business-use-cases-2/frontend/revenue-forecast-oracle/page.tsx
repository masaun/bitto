...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REVENUE_FORECAST_ORACLE_ADDRESS || '';

export default function Revenue_Forecast_OraclePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.revenue-forecast-oracle/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'revenue-forecast-oracle',
