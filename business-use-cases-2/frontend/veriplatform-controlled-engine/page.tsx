...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERIPLATFORM_CONTROLLED_ENGINE_ADDRESS || '';

export default function Veriplatform_Controlled_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'veriplatform-controlled-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veriplatform-controlled-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veriplatform-controlled-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>veriplatform-controlled-engine</h1>
