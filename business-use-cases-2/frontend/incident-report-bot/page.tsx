...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INCIDENT_REPORT_BOT_ADDRESS || '';

export default function Incident_Report_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.incident-report-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'incident-report-bot',
