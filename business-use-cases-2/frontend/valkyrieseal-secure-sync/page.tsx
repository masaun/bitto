...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VALKYRIESEAL_SECURE_SYNC_ADDRESS || '';

export default function Valkyrieseal_Secure_SyncPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.valkyrieseal-secure-sync/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'valkyrieseal-secure-sync',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>valkyrieseal-secure-sync</h1>
