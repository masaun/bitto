...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BLACKSTONECLOAK_SECURE_ENGINE_ADDRESS || '';

export default function Blackstonecloak_Secure_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'blackstonecloak-secure-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.blackstonecloak-secure-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'blackstonecloak-secure-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>blackstonecloak-secure-engine</h1>
