...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MEAN_DEVIATION_CALC_ADDRESS || '';

export default function Mean_Deviation_CalcPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.mean-deviation-calc/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'mean-deviation-calc',
