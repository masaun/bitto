...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDITMARK_CONTROLLED_ORACLE_ADDRESS || '';

export default function Auditmark_Controlled_OraclePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'auditmark-controlled-oracle',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auditmark-controlled-oracle/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auditmark-controlled-oracle',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>auditmark-controlled-oracle</h1>
