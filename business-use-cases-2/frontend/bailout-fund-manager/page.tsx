...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BAILOUT_FUND_MANAGER_ADDRESS || '';

export default function Bailout_Fund_ManagerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bailout-fund-manager/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bailout-fund-manager',
