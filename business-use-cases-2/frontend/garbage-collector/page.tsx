...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GARBAGE_COLLECTOR_ADDRESS || '';

export default function Garbage_CollectorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.garbage-collector/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'garbage-collector',
