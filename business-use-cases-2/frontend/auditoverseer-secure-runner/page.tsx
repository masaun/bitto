...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDITOVERSEER_SECURE_RUNNER_ADDRESS || '';

export default function Auditoverseer_Secure_RunnerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'auditoverseer-secure-runner',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auditoverseer-secure-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auditoverseer-secure-runner',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>auditoverseer-secure-runner</h1>
