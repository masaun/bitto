...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SLASH_EVENT_LOGGER_ADDRESS || '';

export default function Slash_Event_LoggerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.slash-event-logger/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'slash-event-logger',
