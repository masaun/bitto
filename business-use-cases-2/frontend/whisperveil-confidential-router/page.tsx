...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WHISPERVEIL_CONFIDENTIAL_ROUTER_ADDRESS || '';

export default function Whisperveil_Confidential_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'whisperveil-confidential-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.whisperveil-confidential-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'whisperveil-confidential-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>whisperveil-confidential-router</h1>
