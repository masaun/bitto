...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SAFEBEACON_OFFCHAIN_ROUTER_ADDRESS || '';

export default function Safebeacon_Offchain_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'safebeacon-offchain-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.safebeacon-offchain-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'safebeacon-offchain-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>safebeacon-offchain-router</h1>
