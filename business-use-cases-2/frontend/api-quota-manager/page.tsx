...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_API_QUOTA_MANAGER_ADDRESS || '';

export default function Api_Quota_ManagerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.api-quota-manager/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'api-quota-manager',
