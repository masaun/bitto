...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RESOURCE_LIMIT_MGR_ADDRESS || '';

export default function Resource_Limit_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.resource-limit-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'resource-limit-mgr',
