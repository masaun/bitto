...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TWAP_ORACLE_HELPER_ADDRESS || '';

export default function Twap_Oracle_HelperPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.twap-oracle-helper/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'twap-oracle-helper',
