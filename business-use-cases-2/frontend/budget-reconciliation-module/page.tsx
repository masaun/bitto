...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BUDGET_RECONCILIATION_MODULE_ADDRESS || '';

export default function Budget_Reconciliation_ModulePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'budget-reconciliation-module',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.budget-reconciliation-module/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'budget-reconciliation-module',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>budget-reconciliation-module</h1>
