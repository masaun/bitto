...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACEFORGECORE_OFFCHAIN_ROUTER_ADDRESS || '';

export default function Traceforgecore_Offchain_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'traceforgecore-offchain-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.traceforgecore-offchain-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'traceforgecore-offchain-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>traceforgecore-offchain-router</h1>
