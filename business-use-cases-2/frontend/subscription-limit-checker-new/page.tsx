...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SUBSCRIPTION_LIMIT_CHECKER_NEW_ADDRESS || '';

export default function Subscription_Limit_Checker_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'subscription-limit-checker-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.subscription-limit-checker-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'subscription-limit-checker-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>subscription-limit-checker-new</h1>
