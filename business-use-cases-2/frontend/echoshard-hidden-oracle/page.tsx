...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ECHOSHARD_HIDDEN_ORACLE_ADDRESS || '';

export default function Echoshard_Hidden_OraclePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.echoshard-hidden-oracle/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'echoshard-hidden-oracle',
