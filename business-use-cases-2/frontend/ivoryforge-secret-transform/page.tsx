...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_IVORYFORGE_SECRET_TRANSFORM_ADDRESS || '';

export default function Ivoryforge_Secret_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'ivoryforge-secret-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ivoryforge-secret-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ivoryforge-secret-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>ivoryforge-secret-transform</h1>
