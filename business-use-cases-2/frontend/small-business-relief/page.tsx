...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SMALL_BUSINESS_RELIEF_ADDRESS || '';

export default function Small_Business_ReliefPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.small-business-relief/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'small-business-relief',
