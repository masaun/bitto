...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LIQUIDITY_PROVISION_ADDRESS || '';

export default function Liquidity_ProvisionPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.liquidity-provision/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'liquidity-provision',
