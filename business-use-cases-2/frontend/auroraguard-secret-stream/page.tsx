...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AURORAGUARD_SECRET_STREAM_ADDRESS || '';

export default function Auroraguard_Secret_StreamPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'auroraguard-secret-stream',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auroraguard-secret-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auroraguard-secret-stream',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>auroraguard-secret-stream</h1>
