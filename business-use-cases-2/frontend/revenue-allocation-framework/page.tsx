...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REVENUE_ALLOCATION_FRAMEWORK_ADDRESS || '';

export default function Revenue_Allocation_FrameworkPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'revenue-allocation-framework',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.revenue-allocation-framework/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'revenue-allocation-framework',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>revenue-allocation-framework</h1>
