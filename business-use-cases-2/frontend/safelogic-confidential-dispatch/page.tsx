...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SAFELOGIC_CONFIDENTIAL_DISPATCH_ADDRESS || '';

export default function Safelogic_Confidential_DispatchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'safelogic-confidential-dispatch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.safelogic-confidential-dispatch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'safelogic-confidential-dispatch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>safelogic-confidential-dispatch</h1>
