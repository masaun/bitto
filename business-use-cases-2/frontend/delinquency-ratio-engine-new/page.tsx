...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DELINQUENCY_RATIO_ENGINE_NEW_ADDRESS || '';

export default function Delinquency_Ratio_Engine_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'delinquency-ratio-engine-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.delinquency-ratio-engine-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'delinquency-ratio-engine-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>delinquency-ratio-engine-new</h1>
