...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TELLOR_FLEX_PROXY_ADDRESS || '';

export default function Tellor_Flex_ProxyPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tellor-flex-proxy/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tellor-flex-proxy',
