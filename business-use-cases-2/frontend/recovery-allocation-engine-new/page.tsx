...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RECOVERY_ALLOCATION_ENGINE_NEW_ADDRESS || '';

export default function Recovery_Allocation_Engine_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'recovery-allocation-engine-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.recovery-allocation-engine-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'recovery-allocation-engine-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>recovery-allocation-engine-new</h1>
