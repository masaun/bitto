...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRANCHE_PRICING_ORACLE_NEW_ADDRESS || '';

export default function Tranche_Pricing_Oracle_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tranche-pricing-oracle-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tranche-pricing-oracle-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tranche-pricing-oracle-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tranche-pricing-oracle-new</h1>
