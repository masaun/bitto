...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MARBLEVEIL_SECURE_ROUTER_ADDRESS || '';

export default function Marbleveil_Secure_RouterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.marbleveil-secure-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'marbleveil-secure-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>marbleveil-secure-router</h1>
