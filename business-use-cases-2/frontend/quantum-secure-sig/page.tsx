...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_QUANTUM_SECURE_SIG_ADDRESS || '';

export default function Quantum_Secure_SigPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.quantum-secure-sig/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'quantum-secure-sig',
