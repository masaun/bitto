...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACKSPIRE_PRIVATE_DISPATCHER_ADDRESS || '';

export default function Trackspire_Private_DispatcherPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'trackspire-private-dispatcher',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.trackspire-private-dispatcher/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'trackspire-private-dispatcher',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>trackspire-private-dispatcher</h1>
