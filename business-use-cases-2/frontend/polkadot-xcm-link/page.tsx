...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_POLKADOT_XCM_LINK_ADDRESS || '';

export default function Polkadot_Xcm_LinkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.polkadot-xcm-link/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'polkadot-xcm-link',
