...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_KEPLERVEILFORGE_PRIVATE_ENGINE_ADDRESS || '';

export default function Keplerveilforge_Private_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'keplerveilforge-private-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.keplerveilforge-private-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'keplerveilforge-private-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>keplerveilforge-private-engine</h1>
