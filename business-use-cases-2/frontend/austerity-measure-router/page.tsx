...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUSTERITY_MEASURE_ROUTER_ADDRESS || '';

export default function Austerity_Measure_RouterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.austerity-measure-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'austerity-measure-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>austerity-measure-router</h1>
