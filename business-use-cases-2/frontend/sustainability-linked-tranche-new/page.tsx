...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SUSTAINABILITY_LINKED_TRANCHE_NEW_ADDRESS || '';

export default function Sustainability_Linked_Tranche_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'sustainability-linked-tranche-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sustainability-linked-tranche-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sustainability-linked-tranche-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>sustainability-linked-tranche-new</h1>
