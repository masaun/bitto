...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACECERTIFY_OFFCHAIN_ENGINE_ADDRESS || '';

export default function Tracecertify_Offchain_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tracecertify-offchain-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tracecertify-offchain-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tracecertify-offchain-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tracecertify-offchain-engine</h1>
