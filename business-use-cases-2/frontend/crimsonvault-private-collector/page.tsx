...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CRIMSONVAULT_PRIVATE_COLLECTOR_ADDRESS || '';

export default function Crimsonvault_Private_CollectorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'crimsonvault-private-collector',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.crimsonvault-private-collector/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'crimsonvault-private-collector',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>crimsonvault-private-collector</h1>
