...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STORMFORGE_SECRET_COLLECTOR_ADDRESS || '';

export default function Stormforge_Secret_CollectorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'stormforge-secret-collector',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.stormforge-secret-collector/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'stormforge-secret-collector',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>stormforge-secret-collector</h1>
