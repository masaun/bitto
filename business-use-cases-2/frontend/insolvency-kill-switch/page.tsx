...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INSOLVENCY_KILL_SWITCH_ADDRESS || '';

export default function Insolvency_Kill_SwitchPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.insolvency-kill-switch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'insolvency-kill-switch',
