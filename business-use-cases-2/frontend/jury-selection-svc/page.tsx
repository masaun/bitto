...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_JURY_SELECTION_SVC_ADDRESS || '';

export default function Jury_Selection_SvcPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.jury-selection-svc/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'jury-selection-svc',
