...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MORTGAGE_UNDERWRITER_ADDRESS || '';

export default function Mortgage_UnderwriterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.mortgage-underwriter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'mortgage-underwriter',
