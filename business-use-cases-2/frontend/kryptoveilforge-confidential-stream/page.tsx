...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_KRYPTOVEILFORGE_CONFIDENTIAL_STREAM_ADDRESS || '';

export default function Kryptoveilforge_Confidential_StreamPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'kryptoveilforge-confidential-stream',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.kryptoveilforge-confidential-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'kryptoveilforge-confidential-stream',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>kryptoveilforge-confidential-stream</h1>
