...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MULTI_ASSET_POUCH_ADDRESS || '';

export default function Multi_Asset_PouchPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.multi-asset-pouch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'multi-asset-pouch',
