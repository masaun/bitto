...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SOLAR_YIELD_TRACKER_ADDRESS || '';

export default function Solar_Yield_TrackerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.solar-yield-tracker/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'solar-yield-tracker',
