...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_XENOGUARD_SECRET_ADAPTER_ADDRESS || '';

export default function Xenoguard_Secret_AdapterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.xenoguard-secret-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'xenoguard-secret-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>xenoguard-secret-adapter</h1>
