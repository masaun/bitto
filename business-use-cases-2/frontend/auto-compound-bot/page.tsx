...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUTO_COMPOUND_BOT_ADDRESS || '';

export default function Auto_Compound_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auto-compound-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auto-compound-bot',
