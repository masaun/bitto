...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ROOM_TEMP_CONTROL_ADDRESS || '';

export default function Room_Temp_ControlPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.room-temp-control/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'room-temp-control',
