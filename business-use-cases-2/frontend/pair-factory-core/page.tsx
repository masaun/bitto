...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PAIR_FACTORY_CORE_ADDRESS || '';

export default function Pair_Factory_CorePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.pair-factory-core/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'pair-factory-core',
