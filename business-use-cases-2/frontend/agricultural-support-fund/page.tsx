...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AGRICULTURAL_SUPPORT_FUND_ADDRESS || '';

export default function Agricultural_Support_FundPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'agricultural-support-fund',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.agricultural-support-fund/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'agricultural-support-fund',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>agricultural-support-fund</h1>
