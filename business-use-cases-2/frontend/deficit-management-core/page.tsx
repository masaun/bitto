...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEFICIT_MANAGEMENT_CORE_ADDRESS || '';

export default function Deficit_Management_CorePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.deficit-management-core/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'deficit-management-core',
