...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HIGH_WATER_MARK_CALC_ADDRESS || '';

export default function High_Water_Mark_CalcPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.high-water-mark-calc/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'high-water-mark-calc',
