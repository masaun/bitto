...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACKGUARD_CONTROLLED_ORACLE_ADDRESS || '';

export default function Trackguard_Controlled_OraclePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'trackguard-controlled-oracle',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.trackguard-controlled-oracle/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'trackguard-controlled-oracle',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>trackguard-controlled-oracle</h1>
