...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SOVEREIGN_FISCAL_ANCHOR_ADDRESS || '';

export default function Sovereign_Fiscal_AnchorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sovereign-fiscal-anchor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sovereign-fiscal-anchor',
