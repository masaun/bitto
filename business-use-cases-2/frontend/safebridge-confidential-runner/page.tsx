...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SAFEBRIDGE_CONFIDENTIAL_RUNNER_ADDRESS || '';

export default function Safebridge_Confidential_RunnerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'safebridge-confidential-runner',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.safebridge-confidential-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'safebridge-confidential-runner',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>safebridge-confidential-runner</h1>
