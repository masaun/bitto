...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GOLD_BAR_SERIAL_LOG_ADDRESS || '';

export default function Gold_Bar_Serial_LogPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.gold-bar-serial-log/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'gold-bar-serial-log',
