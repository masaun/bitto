...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INVENTORY_COUNT_AUDIT_ADDRESS || '';

export default function Inventory_Count_AuditPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.inventory-count-audit/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'inventory-count-audit',
