...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PLAT_MAP_VALIDATOR_ADDRESS || '';

export default function Plat_Map_ValidatorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.plat-map-validator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'plat-map-validator',
