...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INTERNAL_TRANSFER_AI_ADDRESS || '';

export default function Internal_Transfer_AiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.internal-transfer-ai/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'internal-transfer-ai',
