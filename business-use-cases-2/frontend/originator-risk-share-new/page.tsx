...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ORIGINATOR_RISK_SHARE_NEW_ADDRESS || '';

export default function Originator_Risk_Share_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'originator-risk-share-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.originator-risk-share-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'originator-risk-share-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>originator-risk-share-new</h1>
