...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RING_SIGNATURE_V2_ADDRESS || '';

export default function Ring_Signature_V2Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ring-signature-v2/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ring-signature-v2',
