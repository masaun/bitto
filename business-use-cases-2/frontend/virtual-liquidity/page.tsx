...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VIRTUAL_LIQUIDITY_ADDRESS || '';

export default function Virtual_LiquidityPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.virtual-liquidity/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'virtual-liquidity',
