...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LIQUIDITY_DRAW_MANAGER_NEW_ADDRESS || '';

export default function Liquidity_Draw_Manager_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'liquidity-draw-manager-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.liquidity-draw-manager-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'liquidity-draw-manager-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>liquidity-draw-manager-new</h1>
