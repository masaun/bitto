...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VARIANCE_THRESHOLD_ADDRESS || '';

export default function Variance_ThresholdPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.variance-threshold/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'variance-threshold',
