...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACKSEAL_OFFCHAIN_RUNNER_ADDRESS || '';

export default function Trackseal_Offchain_RunnerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'trackseal-offchain-runner',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.trackseal-offchain-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'trackseal-offchain-runner',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>trackseal-offchain-runner</h1>
