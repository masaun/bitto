...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRANSPARENCY_REPORT_ADDRESS || '';

export default function Transparency_ReportPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.transparency-report/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'transparency-report',
