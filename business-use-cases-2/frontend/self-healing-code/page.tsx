...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SELF_HEALING_CODE_ADDRESS || '';

export default function Self_Healing_CodePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.self-healing-code/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'self-healing-code',
