...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SOVEREIGN_ASSET_MANAGER_ADDRESS || '';

export default function Sovereign_Asset_ManagerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sovereign-asset-manager/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sovereign-asset-manager',
