...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AML_TRANSACTION_GATE_ADDRESS || '';

export default function Aml_Transaction_GatePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.aml-transaction-gate/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'aml-transaction-gate',
