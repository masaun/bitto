...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ACCOUNT_PROOF_CHK_ADDRESS || '';

export default function Account_Proof_ChkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.account-proof-chk/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'account-proof-chk',
