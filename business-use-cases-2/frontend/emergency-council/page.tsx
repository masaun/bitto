...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EMERGENCY_COUNCIL_ADDRESS || '';

export default function Emergency_CouncilPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.emergency-council/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'emergency-council',
