...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROOFADMINISTRATOR_PRIVATE_ADAPTER_ADDRESS || '';

export default function Proofadministrator_Private_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'proofadministrator-private-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.proofadministrator-private-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'proofadministrator-private-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>proofadministrator-private-adapter</h1>
