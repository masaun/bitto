...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MEETING_MINUTES_BOT_ADDRESS || '';

export default function Meeting_Minutes_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.meeting-minutes-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'meeting-minutes-bot',
