...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STORMVEIL_HIDDEN_FETCH_ADDRESS || '';

export default function Stormveil_Hidden_FetchPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.stormveil-hidden-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'stormveil-hidden-fetch',
