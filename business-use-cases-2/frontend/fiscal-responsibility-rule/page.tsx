...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_RESPONSIBILITY_RULE_ADDRESS || '';

export default function Fiscal_Responsibility_RulePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'fiscal-responsibility-rule',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-responsibility-rule/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-responsibility-rule',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>fiscal-responsibility-rule</h1>
