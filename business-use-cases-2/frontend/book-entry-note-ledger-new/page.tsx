...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BOOK_ENTRY_NOTE_LEDGER_NEW_ADDRESS || '';

export default function Book_Entry_Note_Ledger_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'book-entry-note-ledger-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.book-entry-note-ledger-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'book-entry-note-ledger-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>book-entry-note-ledger-new</h1>
