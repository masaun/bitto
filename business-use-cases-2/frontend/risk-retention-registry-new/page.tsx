...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RISK_RETENTION_REGISTRY_NEW_ADDRESS || '';

export default function Risk_Retention_Registry_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'risk-retention-registry-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.risk-retention-registry-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'risk-retention-registry-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>risk-retention-registry-new</h1>
