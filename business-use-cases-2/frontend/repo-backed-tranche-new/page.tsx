...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REPO_BACKED_TRANCHE_NEW_ADDRESS || '';

export default function Repo_Backed_Tranche_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.repo-backed-tranche-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'repo-backed-tranche-new',
