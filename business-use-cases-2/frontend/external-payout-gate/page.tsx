...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EXTERNAL_PAYOUT_GATE_ADDRESS || '';

export default function External_Payout_GatePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.external-payout-gate/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'external-payout-gate',
