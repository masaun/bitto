...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_IRIDESCENTCLOAK_SECRET_REFINERY_ADDRESS || '';

export default function Iridescentcloak_Secret_RefineryPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'iridescentcloak-secret-refinery',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.iridescentcloak-secret-refinery/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'iridescentcloak-secret-refinery',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>iridescentcloak-secret-refinery</h1>
