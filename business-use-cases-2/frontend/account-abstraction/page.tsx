...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ACCOUNT_ABSTRACTION_ADDRESS || '';

export default function Account_AbstractionPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.account-abstraction/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'account-abstraction',
