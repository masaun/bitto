...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MISTFORGE_PRIVATE_PROCESSOR_ADDRESS || '';

export default function Mistforge_Private_ProcessorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'mistforge-private-processor',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.mistforge-private-processor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'mistforge-private-processor',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>mistforge-private-processor</h1>
