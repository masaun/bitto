...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ASSET_DISPOSITION_MODULE_NEW_ADDRESS || '';

export default function Asset_Disposition_Module_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'asset-disposition-module-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.asset-disposition-module-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'asset-disposition-module-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>asset-disposition-module-new</h1>
