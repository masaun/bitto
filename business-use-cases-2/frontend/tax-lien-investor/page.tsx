...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TAX_LIEN_INVESTOR_ADDRESS || '';

export default function Tax_Lien_InvestorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tax-lien-investor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tax-lien-investor',
