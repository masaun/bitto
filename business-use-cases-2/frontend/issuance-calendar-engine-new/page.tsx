...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ISSUANCE_CALENDAR_ENGINE_NEW_ADDRESS || '';

export default function Issuance_Calendar_Engine_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'issuance-calendar-engine-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.issuance-calendar-engine-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'issuance-calendar-engine-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>issuance-calendar-engine-new</h1>
