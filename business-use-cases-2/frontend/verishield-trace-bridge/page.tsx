...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERISHIELD_TRACE_BRIDGE_ADDRESS || '';

export default function Verishield_Trace_BridgePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.verishield-trace-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'verishield-trace-bridge',
