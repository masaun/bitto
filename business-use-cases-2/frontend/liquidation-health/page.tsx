...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LIQUIDATION_HEALTH_ADDRESS || '';

export default function Liquidation_HealthPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.liquidation-health/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'liquidation-health',
