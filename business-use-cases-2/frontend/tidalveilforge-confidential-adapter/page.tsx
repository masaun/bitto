...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TIDALVEILFORGE_CONFIDENTIAL_ADAPTER_ADDRESS || '';

export default function Tidalveilforge_Confidential_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tidalveilforge-confidential-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tidalveilforge-confidential-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tidalveilforge-confidential-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tidalveilforge-confidential-adapter</h1>
