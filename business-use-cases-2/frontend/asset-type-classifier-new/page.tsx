...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ASSET_TYPE_CLASSIFIER_NEW_ADDRESS || '';

export default function Asset_Type_Classifier_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'asset-type-classifier-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.asset-type-classifier-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'asset-type-classifier-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>asset-type-classifier-new</h1>
