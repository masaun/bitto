...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HORIZONLOCK_HIDDEN_TRANSFORM_ADDRESS || '';

export default function Horizonlock_Hidden_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'horizonlock-hidden-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.horizonlock-hidden-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'horizonlock-hidden-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>horizonlock-hidden-transform</h1>
