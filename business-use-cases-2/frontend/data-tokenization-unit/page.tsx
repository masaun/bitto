...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DATA_TOKENIZATION_UNIT_ADDRESS || '';

export default function Data_Tokenization_UnitPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.data-tokenization-unit/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'data-tokenization-unit',
