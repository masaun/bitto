...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RUNESEAL_PRIVATE_STREAM_ADDRESS || '';

export default function Runeseal_Private_StreamPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.runeseal-private-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'runeseal-private-stream',
