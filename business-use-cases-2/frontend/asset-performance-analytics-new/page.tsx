...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ASSET_PERFORMANCE_ANALYTICS_NEW_ADDRESS || '';

export default function Asset_Performance_Analytics_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'asset-performance-analytics-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.asset-performance-analytics-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'asset-performance-analytics-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>asset-performance-analytics-new</h1>
