...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_P2P_VALIDATOR_INT_ADDRESS || '';

export default function P2P_Validator_IntPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.p2p-validator-int/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'p2p-validator-int',
