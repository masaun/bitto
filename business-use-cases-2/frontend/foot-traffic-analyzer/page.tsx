...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FOOT_TRAFFIC_ANALYZER_ADDRESS || '';

export default function Foot_Traffic_AnalyzerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.foot-traffic-analyzer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'foot-traffic-analyzer',
