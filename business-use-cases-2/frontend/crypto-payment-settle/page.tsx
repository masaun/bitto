...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CRYPTO_PAYMENT_SETTLE_ADDRESS || '';

export default function Crypto_Payment_SettlePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.crypto-payment-settle/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'crypto-payment-settle',
