...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FLOATING_RATE_ADJUSTER_NEW_ADDRESS || '';

export default function Floating_Rate_Adjuster_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'floating-rate-adjuster-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.floating-rate-adjuster-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'floating-rate-adjuster-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>floating-rate-adjuster-new</h1>
