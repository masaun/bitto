...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RIVERVEILFORGE_CONFIDENTIAL_ENGINE_ADDRESS || '';

export default function Riverveilforge_Confidential_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'riverveilforge-confidential-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.riverveilforge-confidential-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'riverveilforge-confidential-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>riverveilforge-confidential-engine</h1>
