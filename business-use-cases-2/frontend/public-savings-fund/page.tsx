...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PUBLIC_SAVINGS_FUND_ADDRESS || '';

export default function Public_Savings_FundPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.public-savings-fund/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'public-savings-fund',
