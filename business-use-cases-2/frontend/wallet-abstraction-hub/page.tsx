...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WALLET_ABSTRACTION_HUB_ADDRESS || '';

export default function Wallet_Abstraction_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.wallet-abstraction-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'wallet-abstraction-hub',
