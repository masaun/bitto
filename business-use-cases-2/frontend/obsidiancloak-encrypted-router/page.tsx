...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OBSIDIANCLOAK_ENCRYPTED_ROUTER_ADDRESS || '';

export default function Obsidiancloak_Encrypted_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'obsidiancloak-encrypted-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.obsidiancloak-encrypted-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'obsidiancloak-encrypted-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>obsidiancloak-encrypted-router</h1>
