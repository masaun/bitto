...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CONSTRUCTION_PROGRESS_ADDRESS || '';

export default function Construction_ProgressPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.construction-progress/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'construction-progress',
