...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACEANCHOR_OFFCHAIN_ADAPTER_ADDRESS || '';

export default function Traceanchor_Offchain_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'traceanchor-offchain-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.traceanchor-offchain-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'traceanchor-offchain-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>traceanchor-offchain-adapter</h1>
