...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PATCH_VERSION_MGR_ADDRESS || '';

export default function Patch_Version_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.patch-version-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'patch-version-mgr',
