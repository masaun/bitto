...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_METAMASK_SNAPS_V1_ADDRESS || '';

export default function Metamask_Snaps_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.metamask-snaps-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'metamask-snaps-v1',
