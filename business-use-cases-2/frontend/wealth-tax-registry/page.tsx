...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WEALTH_TAX_REGISTRY_ADDRESS || '';

export default function Wealth_Tax_RegistryPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.wealth-tax-registry/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'wealth-tax-registry',
