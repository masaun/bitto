...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FINDER_FEE_AGREEMENT_ADDRESS || '';

export default function Finder_Fee_AgreementPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.finder-fee-agreement/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'finder-fee-agreement',
