...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_POLYMATH_MESH_LINK_ADDRESS || '';

export default function Polymath_Mesh_LinkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.polymath-mesh-link/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'polymath-mesh-link',
