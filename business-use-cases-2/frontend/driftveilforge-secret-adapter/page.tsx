...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DRIFTVEILFORGE_SECRET_ADAPTER_ADDRESS || '';

export default function Driftveilforge_Secret_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'driftveilforge-secret-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.driftveilforge-secret-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'driftveilforge-secret-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>driftveilforge-secret-adapter</h1>
