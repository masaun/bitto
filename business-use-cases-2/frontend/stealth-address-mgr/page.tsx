...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STEALTH_ADDRESS_MGR_ADDRESS || '';

export default function Stealth_Address_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.stealth-address-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'stealth-address-mgr',
