...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VOTING_POWER_ESCROW_ADDRESS || '';

export default function Voting_Power_EscrowPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.voting-power-escrow/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'voting-power-escrow',
