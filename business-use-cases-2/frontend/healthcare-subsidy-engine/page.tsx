...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HEALTHCARE_SUBSIDY_ENGINE_ADDRESS || '';

export default function Healthcare_Subsidy_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'healthcare-subsidy-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.healthcare-subsidy-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'healthcare-subsidy-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>healthcare-subsidy-engine</h1>
