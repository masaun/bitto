...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CHAINLINK_ADAPTER_ADDRESS || '';

export default function Chainlink_AdapterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.chainlink-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'chainlink-adapter',
