...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CLOUDVEILFORGE_PRIVATE_ROUTER_ADDRESS || '';

export default function Cloudveilforge_Private_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'cloudveilforge-private-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cloudveilforge-private-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cloudveilforge-private-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>cloudveilforge-private-router</h1>
