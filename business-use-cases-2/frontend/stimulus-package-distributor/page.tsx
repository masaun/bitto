...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STIMULUS_PACKAGE_DISTRIBUTOR_ADDRESS || '';

export default function Stimulus_Package_DistributorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'stimulus-package-distributor',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.stimulus-package-distributor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'stimulus-package-distributor',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>stimulus-package-distributor</h1>
