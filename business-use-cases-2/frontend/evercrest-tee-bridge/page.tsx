...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EVERCREST_TEE_BRIDGE_ADDRESS || '';

export default function Evercrest_Tee_BridgePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.evercrest-tee-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'evercrest-tee-bridge',
