...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BANK_OWNED_REO_MGR_ADDRESS || '';

export default function Bank_Owned_Reo_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bank-owned-reo-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bank-owned-reo-mgr',
