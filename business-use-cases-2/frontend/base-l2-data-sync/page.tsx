...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BASE_L2_DATA_SYNC_ADDRESS || '';

export default function Base_L2_Data_SyncPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.base-l2-data-sync/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'base-l2-data-sync',
