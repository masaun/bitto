...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRADE_FINANCE_SECURITIZER_NEW_ADDRESS || '';

export default function Trade_Finance_Securitizer_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'trade-finance-securitizer-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.trade-finance-securitizer-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'trade-finance-securitizer-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>trade-finance-securitizer-new</h1>
