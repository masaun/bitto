...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROOFCRAFT_PRIVATE_DISPATCH_ADDRESS || '';

export default function Proofcraft_Private_DispatchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'proofcraft-private-dispatch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.proofcraft-private-dispatch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'proofcraft-private-dispatch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>proofcraft-private-dispatch</h1>
