...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_KEYSTONESEAL_SECRET_FETCH_ADDRESS || '';

export default function Keystoneseal_Secret_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'keystoneseal-secret-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.keystoneseal-secret-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'keystoneseal-secret-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>keystoneseal-secret-fetch</h1>
