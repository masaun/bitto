...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DURATION_CALCULATOR_NEW_ADDRESS || '';

export default function Duration_Calculator_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.duration-calculator-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'duration-calculator-new',
