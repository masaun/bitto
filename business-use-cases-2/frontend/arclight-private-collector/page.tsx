...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ARCLIGHT_PRIVATE_COLLECTOR_ADDRESS || '';

export default function Arclight_Private_CollectorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'arclight-private-collector',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.arclight-private-collector/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'arclight-private-collector',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>arclight-private-collector</h1>
