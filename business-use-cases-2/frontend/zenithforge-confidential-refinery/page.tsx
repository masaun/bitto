...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ZENITHFORGE_CONFIDENTIAL_REFINERY_ADDRESS || '';

export default function Zenithforge_Confidential_RefineryPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'zenithforge-confidential-refinery',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.zenithforge-confidential-refinery/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'zenithforge-confidential-refinery',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>zenithforge-confidential-refinery</h1>
