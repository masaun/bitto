...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDITDELTA_SECURE_ENGINE_ADDRESS || '';

export default function Auditdelta_Secure_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auditdelta-secure-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auditdelta-secure-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>auditdelta-secure-engine</h1>
