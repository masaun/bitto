...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUTO_LOAN_SECURITIZATION_NEW_ADDRESS || '';

export default function Auto_Loan_Securitization_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'auto-loan-securitization-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auto-loan-securitization-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auto-loan-securitization-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>auto-loan-securitization-new</h1>
