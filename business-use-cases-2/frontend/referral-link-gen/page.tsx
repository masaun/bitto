...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REFERRAL_LINK_GEN_ADDRESS || '';

export default function Referral_Link_GenPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.referral-link-gen/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'referral-link-gen',
