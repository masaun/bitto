...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SWIFT_ISO20022_LINK_ADDRESS || '';

export default function Swift_Iso20022_LinkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.swift-iso20022-link/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'swift-iso20022-link',
