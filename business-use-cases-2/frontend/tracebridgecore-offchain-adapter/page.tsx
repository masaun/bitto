...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACEBRIDGECORE_OFFCHAIN_ADAPTER_ADDRESS || '';

export default function Tracebridgecore_Offchain_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tracebridgecore-offchain-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tracebridgecore-offchain-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tracebridgecore-offchain-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tracebridgecore-offchain-adapter</h1>
