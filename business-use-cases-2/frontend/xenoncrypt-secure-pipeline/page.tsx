...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_XENONCRYPT_SECURE_PIPELINE_ADDRESS || '';

export default function Xenoncrypt_Secure_PipelinePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'xenoncrypt-secure-pipeline',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.xenoncrypt-secure-pipeline/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'xenoncrypt-secure-pipeline',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>xenoncrypt-secure-pipeline</h1>
