...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CHAINAUDIT_PRIVATE_ORCHESTRATOR_ADDRESS || '';

export default function Chainaudit_Private_OrchestratorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'chainaudit-private-orchestrator',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.chainaudit-private-orchestrator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'chainaudit-private-orchestrator',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>chainaudit-private-orchestrator</h1>
