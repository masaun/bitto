...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WATER_USAGE_MONITOR_ADDRESS || '';

export default function Water_Usage_MonitorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.water-usage-monitor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'water-usage-monitor',
