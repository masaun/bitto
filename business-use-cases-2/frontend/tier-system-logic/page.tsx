...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TIER_SYSTEM_LOGIC_ADDRESS || '';

export default function Tier_System_LogicPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tier-system-logic/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tier-system-logic',
