...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DISASTER_RECOVERY_FINANCER_ADDRESS || '';

export default function Disaster_Recovery_FinancerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'disaster-recovery-financer',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.disaster-recovery-financer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'disaster-recovery-financer',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>disaster-recovery-financer</h1>
