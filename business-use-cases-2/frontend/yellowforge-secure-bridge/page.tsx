...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_YELLOWFORGE_SECURE_BRIDGE_ADDRESS || '';

export default function Yellowforge_Secure_BridgePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'yellowforge-secure-bridge',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.yellowforge-secure-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'yellowforge-secure-bridge',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>yellowforge-secure-bridge</h1>
