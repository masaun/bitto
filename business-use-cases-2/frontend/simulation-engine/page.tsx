...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SIMULATION_ENGINE_ADDRESS || '';

export default function Simulation_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.simulation-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'simulation-engine',
