...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SECURE_ELEMENT_LINK_ADDRESS || '';

export default function Secure_Element_LinkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.secure-element-link/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'secure-element-link',
