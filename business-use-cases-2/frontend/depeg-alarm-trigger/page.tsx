...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEPEG_ALARM_TRIGGER_ADDRESS || '';

export default function Depeg_Alarm_TriggerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.depeg-alarm-trigger/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'depeg-alarm-trigger',
