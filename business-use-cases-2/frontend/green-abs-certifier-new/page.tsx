...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GREEN_ABS_CERTIFIER_NEW_ADDRESS || '';

export default function Green_Abs_Certifier_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.green-abs-certifier-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'green-abs-certifier-new',
