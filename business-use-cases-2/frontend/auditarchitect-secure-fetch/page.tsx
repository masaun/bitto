...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDITARCHITECT_SECURE_FETCH_ADDRESS || '';

export default function Auditarchitect_Secure_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'auditarchitect-secure-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auditarchitect-secure-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auditarchitect-secure-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>auditarchitect-secure-fetch</h1>
