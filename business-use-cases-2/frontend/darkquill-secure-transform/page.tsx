...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DARKQUILL_SECURE_TRANSFORM_ADDRESS || '';

export default function Darkquill_Secure_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'darkquill-secure-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.darkquill-secure-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'darkquill-secure-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>darkquill-secure-transform</h1>
