...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GAS_PRICE_PREDICTOR_ADDRESS || '';

export default function Gas_Price_PredictorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.gas-price-predictor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'gas-price-predictor',
