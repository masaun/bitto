...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ACCREDITATION_CHECKER_ADDRESS || '';

export default function Accreditation_CheckerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.accreditation-checker/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'accreditation-checker',
