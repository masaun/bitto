...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OMICRON_ORACLE_V2_ADDRESS || '';

export default function Omicron_Oracle_V2Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.omicron-oracle-v2/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'omicron-oracle-v2',
