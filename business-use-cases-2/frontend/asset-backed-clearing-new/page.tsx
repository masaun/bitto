...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ASSET_BACKED_CLEARING_NEW_ADDRESS || '';

export default function Asset_Backed_Clearing_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'asset-backed-clearing-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.asset-backed-clearing-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'asset-backed-clearing-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>asset-backed-clearing-new</h1>
