...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDITBRIDGE_OFFCHAIN_EXECUTOR_ADDRESS || '';

export default function Auditbridge_Offchain_ExecutorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'auditbridge-offchain-executor',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auditbridge-offchain-executor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auditbridge-offchain-executor',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>auditbridge-offchain-executor</h1>
