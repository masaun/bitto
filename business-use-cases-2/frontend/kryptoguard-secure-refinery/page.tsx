...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_KRYPTOGUARD_SECURE_REFINERY_ADDRESS || '';

export default function Kryptoguard_Secure_RefineryPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'kryptoguard-secure-refinery',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.kryptoguard-secure-refinery/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'kryptoguard-secure-refinery',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>kryptoguard-secure-refinery</h1>
