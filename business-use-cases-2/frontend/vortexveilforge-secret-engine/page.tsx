...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VORTEXVEILFORGE_SECRET_ENGINE_ADDRESS || '';

export default function Vortexveilforge_Secret_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'vortexveilforge-secret-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.vortexveilforge-secret-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'vortexveilforge-secret-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>vortexveilforge-secret-engine</h1>
