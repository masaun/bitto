...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RUNICRYPT_SECRET_REFINERY_ADDRESS || '';

export default function Runicrypt_Secret_RefineryPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'runicrypt-secret-refinery',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.runicrypt-secret-refinery/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'runicrypt-secret-refinery',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>runicrypt-secret-refinery</h1>
