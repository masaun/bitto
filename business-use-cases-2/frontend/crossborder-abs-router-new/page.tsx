...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CROSSBORDER_ABS_ROUTER_NEW_ADDRESS || '';

export default function Crossborder_Abs_Router_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'crossborder-abs-router-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.crossborder-abs-router-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'crossborder-abs-router-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>crossborder-abs-router-new</h1>
