...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TAX_BASE_EXPANSION_ADDRESS || '';

export default function Tax_Base_ExpansionPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tax-base-expansion/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tax-base-expansion',
