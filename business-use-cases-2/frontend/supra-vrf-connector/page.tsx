...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SUPRA_VRF_CONNECTOR_ADDRESS || '';

export default function Supra_Vrf_ConnectorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.supra-vrf-connector/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'supra-vrf-connector',
