...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OPTI_ROLLUP_VERIFIER_ADDRESS || '';

export default function Opti_Rollup_VerifierPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.opti-rollup-verifier/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'opti-rollup-verifier',
