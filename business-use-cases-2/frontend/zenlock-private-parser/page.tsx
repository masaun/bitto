...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ZENLOCK_PRIVATE_PARSER_ADDRESS || '';

export default function Zenlock_Private_ParserPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.zenlock-private-parser/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'zenlock-private-parser',
