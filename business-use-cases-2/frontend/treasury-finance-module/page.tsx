...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TREASURY_FINANCE_MODULE_ADDRESS || '';

export default function Treasury_Finance_ModulePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.treasury-finance-module/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'treasury-finance-module',
