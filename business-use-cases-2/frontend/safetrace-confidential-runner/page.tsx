...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SAFETRACE_CONFIDENTIAL_RUNNER_ADDRESS || '';

export default function Safetrace_Confidential_RunnerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'safetrace-confidential-runner',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.safetrace-confidential-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'safetrace-confidential-runner',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>safetrace-confidential-runner</h1>
