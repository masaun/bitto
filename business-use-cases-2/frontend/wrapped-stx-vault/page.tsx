...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WRAPPED_STX_VAULT_ADDRESS || '';

export default function Wrapped_Stx_VaultPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.wrapped-stx-vault/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'wrapped-stx-vault',
