...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACEARCHIVE_OFFCHAIN_ENGINE_ADDRESS || '';

export default function Tracearchive_Offchain_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tracearchive-offchain-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tracearchive-offchain-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tracearchive-offchain-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tracearchive-offchain-engine</h1>
