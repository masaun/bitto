...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MICRO_PAYMENT_STREAMER_ADDRESS || '';

export default function Micro_Payment_StreamerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.micro-payment-streamer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'micro-payment-streamer',
