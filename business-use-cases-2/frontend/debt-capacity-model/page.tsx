...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEBT_CAPACITY_MODEL_ADDRESS || '';

export default function Debt_Capacity_ModelPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.debt-capacity-model/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'debt-capacity-model',
