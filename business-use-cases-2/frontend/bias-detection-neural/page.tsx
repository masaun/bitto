...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BIAS_DETECTION_NEURAL_ADDRESS || '';

export default function Bias_Detection_NeuralPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bias-detection-neural/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bias-detection-neural',
