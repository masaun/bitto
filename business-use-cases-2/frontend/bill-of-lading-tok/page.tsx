...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BILL_OF_LADING_TOK_ADDRESS || '';

export default function Bill_Of_Lading_TokPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bill-of-lading-tok/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bill-of-lading-tok',
