...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ZK_LEASE_VERIFIER_ADDRESS || '';

export default function Zk_Lease_VerifierPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.zk-lease-verifier/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'zk-lease-verifier',
