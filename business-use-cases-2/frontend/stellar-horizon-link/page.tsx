...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STELLAR_HORIZON_LINK_ADDRESS || '';

export default function Stellar_Horizon_LinkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.stellar-horizon-link/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'stellar-horizon-link',
