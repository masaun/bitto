...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACEBEAM_CONTROLLED_FETCH_ADDRESS || '';

export default function Tracebeam_Controlled_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tracebeam-controlled-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tracebeam-controlled-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tracebeam-controlled-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tracebeam-controlled-fetch</h1>
