...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERISCAN_PRIVATE_ORACLE_ADDRESS || '';

export default function Veriscan_Private_OraclePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veriscan-private-oracle/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veriscan-private-oracle',
