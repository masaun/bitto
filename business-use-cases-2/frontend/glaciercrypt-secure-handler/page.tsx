...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GLACIERCRYPT_SECURE_HANDLER_ADDRESS || '';

export default function Glaciercrypt_Secure_HandlerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'glaciercrypt-secure-handler',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.glaciercrypt-secure-handler/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'glaciercrypt-secure-handler',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>glaciercrypt-secure-handler</h1>
