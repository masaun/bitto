...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WILL_REGISTRY_CORE_ADDRESS || '';

export default function Will_Registry_CorePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.will-registry-core/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'will-registry-core',
