...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CYCLICALLY_ADJUSTED_BALANCE_ADDRESS || '';

export default function Cyclically_Adjusted_BalancePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'cyclically-adjusted-balance',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cyclically-adjusted-balance/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cyclically-adjusted-balance',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>cyclically-adjusted-balance</h1>
