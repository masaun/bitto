...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DRAG_ALONG_RIGHTS_ADDRESS || '';

export default function Drag_Along_RightsPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.drag-along-rights/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'drag-along-rights',
