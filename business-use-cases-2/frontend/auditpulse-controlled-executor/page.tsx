...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDITPULSE_CONTROLLED_EXECUTOR_ADDRESS || '';

export default function Auditpulse_Controlled_ExecutorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'auditpulse-controlled-executor',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auditpulse-controlled-executor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auditpulse-controlled-executor',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>auditpulse-controlled-executor</h1>
