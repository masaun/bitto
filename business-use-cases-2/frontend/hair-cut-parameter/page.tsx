...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HAIR_CUT_PARAMETER_ADDRESS || '';

export default function Hair_Cut_ParameterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.hair-cut-parameter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'hair-cut-parameter',
