...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACECREST_OFFCHAIN_GATEWAY_ADDRESS || '';

export default function Tracecrest_Offchain_GatewayPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tracecrest-offchain-gateway',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tracecrest-offchain-gateway/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tracecrest-offchain-gateway',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tracecrest-offchain-gateway</h1>
