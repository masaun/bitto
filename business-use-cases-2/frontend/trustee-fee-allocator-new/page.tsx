...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRUSTEE_FEE_ALLOCATOR_NEW_ADDRESS || '';

export default function Trustee_Fee_Allocator_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'trustee-fee-allocator-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.trustee-fee-allocator-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'trustee-fee-allocator-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>trustee-fee-allocator-new</h1>
