...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EXPENDITURE_EFFICIENCY_ANALYZER_ADDRESS || '';

export default function Expenditure_Efficiency_AnalyzerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'expenditure-efficiency-analyzer',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.expenditure-efficiency-analyzer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'expenditure-efficiency-analyzer',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>expenditure-efficiency-analyzer</h1>
