...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ABS_CAPITAL_ALLOCATOR_NEW_ADDRESS || '';

export default function Abs_Capital_Allocator_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'abs-capital-allocator-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.abs-capital-allocator-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'abs-capital-allocator-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>abs-capital-allocator-new</h1>
