...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COWORKING_DESK_ACCESS_ADDRESS || '';

export default function Coworking_Desk_AccessPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.coworking-desk-access/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'coworking-desk-access',
