...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SCENARIO_ANALYSIS_ROUTER_NEW_ADDRESS || '';

export default function Scenario_Analysis_Router_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'scenario-analysis-router-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.scenario-analysis-router-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'scenario-analysis-router-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>scenario-analysis-router-new</h1>
