...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COLD_WALLET_MONITOR_ADDRESS || '';

export default function Cold_Wallet_MonitorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cold-wallet-monitor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cold-wallet-monitor',
