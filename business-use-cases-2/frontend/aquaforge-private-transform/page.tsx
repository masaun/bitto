...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AQUAFORGE_PRIVATE_TRANSFORM_ADDRESS || '';

export default function Aquaforge_Private_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'aquaforge-private-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.aquaforge-private-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'aquaforge-private-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>aquaforge-private-transform</h1>
