...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SPREAD_ACCOUNT_REALLOCATOR_NEW_ADDRESS || '';

export default function Spread_Account_Reallocator_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'spread-account-reallocator-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.spread-account-reallocator-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'spread-account-reallocator-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>spread-account-reallocator-new</h1>
