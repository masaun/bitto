...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MESSAGE_BROADCAST_ADDRESS || '';

export default function Message_BroadcastPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.message-broadcast/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'message-broadcast',
