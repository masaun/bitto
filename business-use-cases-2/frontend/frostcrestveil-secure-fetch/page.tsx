...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FROSTCRESTVEIL_SECURE_FETCH_ADDRESS || '';

export default function Frostcrestveil_Secure_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'frostcrestveil-secure-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.frostcrestveil-secure-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'frostcrestveil-secure-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>frostcrestveil-secure-fetch</h1>
