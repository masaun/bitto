...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PLONK_PROOF_SYSTEM_ADDRESS || '';

export default function Plonk_Proof_SystemPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.plonk-proof-system/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'plonk-proof-system',
