...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FOUNDATION_GOVERNOR_ADDRESS || '';

export default function Foundation_GovernorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.foundation-governor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'foundation-governor',
