...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SOVEREIGN_EXPENDITURE_CORE_ADDRESS || '';

export default function Sovereign_Expenditure_CorePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'sovereign-expenditure-core',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sovereign-expenditure-core/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sovereign-expenditure-core',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>sovereign-expenditure-core</h1>
