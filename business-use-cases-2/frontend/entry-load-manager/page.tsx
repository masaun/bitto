...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ENTRY_LOAD_MANAGER_ADDRESS || '';

export default function Entry_Load_ManagerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.entry-load-manager/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'entry-load-manager',
