...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PAYING_AGENT_PROXY_ADDRESS || '';

export default function Paying_Agent_ProxyPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.paying-agent-proxy/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'paying-agent-proxy',
