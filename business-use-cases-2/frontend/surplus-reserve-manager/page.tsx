...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SURPLUS_RESERVE_MANAGER_ADDRESS || '';

export default function Surplus_Reserve_ManagerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.surplus-reserve-manager/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'surplus-reserve-manager',
