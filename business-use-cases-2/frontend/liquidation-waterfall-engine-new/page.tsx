...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LIQUIDATION_WATERFALL_ENGINE_NEW_ADDRESS || '';

export default function Liquidation_Waterfall_Engine_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'liquidation-waterfall-engine-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.liquidation-waterfall-engine-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'liquidation-waterfall-engine-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>liquidation-waterfall-engine-new</h1>
