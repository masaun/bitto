...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RE_ISSUANCE_CONTROLLER_ADDRESS || '';

export default function Re_Issuance_ControllerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.re-issuance-controller/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 're-issuance-controller',
