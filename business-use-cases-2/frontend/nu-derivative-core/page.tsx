...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NU_DERIVATIVE_CORE_ADDRESS || '';

export default function Nu_Derivative_CorePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.nu-derivative-core/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'nu-derivative-core',
