...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PYTH_NETWORK_BRIDGE_ADDRESS || '';

export default function Pyth_Network_BridgePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.pyth-network-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'pyth-network-bridge',
