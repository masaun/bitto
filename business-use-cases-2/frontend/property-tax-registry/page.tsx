...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROPERTY_TAX_REGISTRY_ADDRESS || '';

export default function Property_Tax_RegistryPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.property-tax-registry/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'property-tax-registry',
