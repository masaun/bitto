...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ALERT_THRESHOLD_MGR_ADDRESS || '';

export default function Alert_Threshold_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.alert-threshold-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'alert-threshold-mgr',
