...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERINETWORK_CONTROLLED_HUB_ADDRESS || '';

export default function Verinetwork_Controlled_HubPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'verinetwork-controlled-hub',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.verinetwork-controlled-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'verinetwork-controlled-hub',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>verinetwork-controlled-hub</h1>
