...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INDIGOFORGEVEIL_PRIVATE_ADAPTER_ADDRESS || '';

export default function Indigoforgeveil_Private_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'indigoforgeveil-private-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.indigoforgeveil-private-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'indigoforgeveil-private-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>indigoforgeveil-private-adapter</h1>
