...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FORK_CHOICE_LOGIC_ADDRESS || '';

export default function Fork_Choice_LogicPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fork-choice-logic/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fork-choice-logic',
