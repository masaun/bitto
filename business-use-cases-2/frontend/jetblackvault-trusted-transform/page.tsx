...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_JETBLACKVAULT_TRUSTED_TRANSFORM_ADDRESS || '';

export default function Jetblackvault_Trusted_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'jetblackvault-trusted-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.jetblackvault-trusted-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'jetblackvault-trusted-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>jetblackvault-trusted-transform</h1>
