...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ETH_BRIDGE_ENDPOINT_ADDRESS || '';

export default function Eth_Bridge_EndpointPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.eth-bridge-endpoint/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'eth-bridge-endpoint',
