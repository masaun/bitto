...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TOKENIZED_TRANCHE_EXCHANGE_NEW_ADDRESS || '';

export default function Tokenized_Tranche_Exchange_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tokenized-tranche-exchange-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tokenized-tranche-exchange-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tokenized-tranche-exchange-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tokenized-tranche-exchange-new</h1>
