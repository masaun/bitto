...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SAFEPOINT_CONFIDENTIAL_DISPATCH_ADDRESS || '';

export default function Safepoint_Confidential_DispatchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'safepoint-confidential-dispatch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.safepoint-confidential-dispatch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'safepoint-confidential-dispatch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>safepoint-confidential-dispatch</h1>
