...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ENGLISH_AUCTION_MGR_ADDRESS || '';

export default function English_Auction_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.english-auction-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'english-auction-mgr',
