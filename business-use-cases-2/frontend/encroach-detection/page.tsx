...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ENCROACH_DETECTION_ADDRESS || '';

export default function Encroach_DetectionPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.encroach-detection/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'encroach-detection',
