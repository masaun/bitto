...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BANK_STATEMENT_PROOFER_ADDRESS || '';

export default function Bank_Statement_ProoferPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bank-statement-proofer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bank-statement-proofer',
