...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDITLEDGERX_SECURE_ROUTER_ADDRESS || '';

export default function Auditledgerx_Secure_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'auditledgerx-secure-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auditledgerx-secure-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auditledgerx-secure-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>auditledgerx-secure-router</h1>
