...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FROSTGATE_CONFIDENTIAL_PIPELINE_ADDRESS || '';

export default function Frostgate_Confidential_PipelinePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'frostgate-confidential-pipeline',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.frostgate-confidential-pipeline/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'frostgate-confidential-pipeline',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>frostgate-confidential-pipeline</h1>
