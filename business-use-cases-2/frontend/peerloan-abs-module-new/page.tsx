...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PEERLOAN_ABS_MODULE_NEW_ADDRESS || '';

export default function Peerloan_Abs_Module_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.peerloan-abs-module-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'peerloan-abs-module-new',
