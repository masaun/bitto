...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_THRESHOLD_SIG_NODE_ADDRESS || '';

export default function Threshold_Sig_NodePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.threshold-sig-node/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'threshold-sig-node',
