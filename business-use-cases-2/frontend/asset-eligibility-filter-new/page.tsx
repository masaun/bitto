...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ASSET_ELIGIBILITY_FILTER_NEW_ADDRESS || '';

export default function Asset_Eligibility_Filter_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'asset-eligibility-filter-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.asset-eligibility-filter-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'asset-eligibility-filter-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>asset-eligibility-filter-new</h1>
