...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GRANTS_PROGRAM_POR_ADDRESS || '';

export default function Grants_Program_PorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.grants-program-por/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'grants-program-por',
