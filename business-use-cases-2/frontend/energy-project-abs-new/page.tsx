...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ENERGY_PROJECT_ABS_NEW_ADDRESS || '';

export default function Energy_Project_Abs_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.energy-project-abs-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'energy-project-abs-new',
