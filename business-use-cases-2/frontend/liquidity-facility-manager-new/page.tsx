...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LIQUIDITY_FACILITY_MANAGER_NEW_ADDRESS || '';

export default function Liquidity_Facility_Manager_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'liquidity-facility-manager-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.liquidity-facility-manager-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'liquidity-facility-manager-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>liquidity-facility-manager-new</h1>
