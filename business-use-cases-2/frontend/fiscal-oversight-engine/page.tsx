...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_OVERSIGHT_ENGINE_ADDRESS || '';

export default function Fiscal_Oversight_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-oversight-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-oversight-engine',
