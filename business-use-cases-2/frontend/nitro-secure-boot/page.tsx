...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NITRO_SECURE_BOOT_ADDRESS || '';

export default function Nitro_Secure_BootPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.nitro-secure-boot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'nitro-secure-boot',
