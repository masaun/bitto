...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_APPROPRIATION_BILL_ROUTER_ADDRESS || '';

export default function Appropriation_Bill_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'appropriation-bill-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.appropriation-bill-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'appropriation-bill-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>appropriation-bill-router</h1>
