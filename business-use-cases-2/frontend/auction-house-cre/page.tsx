...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUCTION_HOUSE_CRE_ADDRESS || '';

export default function Auction_House_CrePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auction-house-cre/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auction-house-cre',
