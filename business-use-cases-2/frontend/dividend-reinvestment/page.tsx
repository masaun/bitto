...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DIVIDEND_REINVESTMENT_ADDRESS || '';

export default function Dividend_ReinvestmentPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.dividend-reinvestment/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'dividend-reinvestment',
