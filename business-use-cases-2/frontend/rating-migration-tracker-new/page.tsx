...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RATING_MIGRATION_TRACKER_NEW_ADDRESS || '';

export default function Rating_Migration_Tracker_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'rating-migration-tracker-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.rating-migration-tracker-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'rating-migration-tracker-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>rating-migration-tracker-new</h1>
