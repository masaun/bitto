...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ANTI_DILUTION_ADJ_ADDRESS || '';

export default function Anti_Dilution_AdjPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.anti-dilution-adj/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'anti-dilution-adj',
