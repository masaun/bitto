...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HOME_OFFICE_AUDIT_ADDRESS || '';

export default function Home_Office_AuditPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.home-office-audit/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'home-office-audit',
