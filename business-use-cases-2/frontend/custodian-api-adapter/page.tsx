...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CUSTODIAN_API_ADAPTER_ADDRESS || '';

export default function Custodian_Api_AdapterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.custodian-api-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'custodian-api-adapter',
