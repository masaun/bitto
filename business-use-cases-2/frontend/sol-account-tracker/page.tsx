...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SOL_ACCOUNT_TRACKER_ADDRESS || '';

export default function Sol_Account_TrackerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sol-account-tracker/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sol-account-tracker',
