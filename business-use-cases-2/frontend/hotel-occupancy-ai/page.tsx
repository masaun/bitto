...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HOTEL_OCCUPANCY_AI_ADDRESS || '';

export default function Hotel_Occupancy_AiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.hotel-occupancy-ai/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'hotel-occupancy-ai',
