...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SAFECHAIN_CONFIDENTIAL_FETCH_ADDRESS || '';

export default function Safechain_Confidential_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'safechain-confidential-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.safechain-confidential-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'safechain-confidential-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>safechain-confidential-fetch</h1>
