...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AIR_RIGHTS_TRADING_ADDRESS || '';

export default function Air_Rights_TradingPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.air-rights-trading/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'air-rights-trading',
