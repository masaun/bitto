...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CALENDAR_SYNC_LOGIC_ADDRESS || '';

export default function Calendar_Sync_LogicPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.calendar-sync-logic/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'calendar-sync-logic',
