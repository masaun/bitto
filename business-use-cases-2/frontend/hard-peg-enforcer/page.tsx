...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HARD_PEG_ENFORCER_ADDRESS || '';

export default function Hard_Peg_EnforcerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.hard-peg-enforcer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'hard-peg-enforcer',
