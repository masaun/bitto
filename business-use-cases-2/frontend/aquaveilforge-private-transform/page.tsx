...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AQUAVEILFORGE_PRIVATE_TRANSFORM_ADDRESS || '';

export default function Aquaveilforge_Private_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'aquaveilforge-private-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.aquaveilforge-private-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'aquaveilforge-private-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>aquaveilforge-private-transform</h1>
