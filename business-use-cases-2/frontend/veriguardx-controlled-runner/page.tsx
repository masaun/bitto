...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERIGUARDX_CONTROLLED_RUNNER_ADDRESS || '';

export default function Veriguardx_Controlled_RunnerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'veriguardx-controlled-runner',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veriguardx-controlled-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veriguardx-controlled-runner',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>veriguardx-controlled-runner</h1>
