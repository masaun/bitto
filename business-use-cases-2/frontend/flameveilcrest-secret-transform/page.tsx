...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FLAMEVEILCREST_SECRET_TRANSFORM_ADDRESS || '';

export default function Flameveilcrest_Secret_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'flameveilcrest-secret-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.flameveilcrest-secret-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'flameveilcrest-secret-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>flameveilcrest-secret-transform</h1>
