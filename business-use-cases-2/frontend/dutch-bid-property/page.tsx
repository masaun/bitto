...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DUTCH_BID_PROPERTY_ADDRESS || '';

export default function Dutch_Bid_PropertyPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.dutch-bid-property/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'dutch-bid-property',
