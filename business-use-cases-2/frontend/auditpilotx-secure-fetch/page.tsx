...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDITPILOTX_SECURE_FETCH_ADDRESS || '';

export default function Auditpilotx_Secure_FetchPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auditpilotx-secure-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auditpilotx-secure-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>auditpilotx-secure-fetch</h1>
