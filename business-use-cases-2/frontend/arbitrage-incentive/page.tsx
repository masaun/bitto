...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ARBITRAGE_INCENTIVE_ADDRESS || '';

export default function Arbitrage_IncentivePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.arbitrage-incentive/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'arbitrage-incentive',
