...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ZERO_SLIPPAGE_SWP_ADDRESS || '';

export default function Zero_Slippage_SwpPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.zero-slippage-swp/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'zero-slippage-swp',
