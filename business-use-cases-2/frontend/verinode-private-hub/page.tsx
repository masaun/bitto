...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERINODE_PRIVATE_HUB_ADDRESS || '';

export default function Verinode_Private_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.verinode-private-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'verinode-private-hub',
