...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CARRY_INTEREST_CALC_ADDRESS || '';

export default function Carry_Interest_CalcPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.carry-interest-calc/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'carry-interest-calc',
