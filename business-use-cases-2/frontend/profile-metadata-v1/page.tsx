...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROFILE_METADATA_V1_ADDRESS || '';

export default function Profile_Metadata_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.profile-metadata-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'profile-metadata-v1',
