...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TREASURY_BUDGET_ROUTER_ADDRESS || '';

export default function Treasury_Budget_RouterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.treasury-budget-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'treasury-budget-router',
