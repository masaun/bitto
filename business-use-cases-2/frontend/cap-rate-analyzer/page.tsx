...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CAP_RATE_ANALYZER_ADDRESS || '';

export default function Cap_Rate_AnalyzerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cap-rate-analyzer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cap-rate-analyzer',
