...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EVICTION_PREVENTION_ADDRESS || '';

export default function Eviction_PreventionPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.eviction-prevention/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'eviction-prevention',
