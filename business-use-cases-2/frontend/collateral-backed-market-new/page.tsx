...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COLLATERAL_BACKED_MARKET_NEW_ADDRESS || '';

export default function Collateral_Backed_Market_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'collateral-backed-market-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.collateral-backed-market-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'collateral-backed-market-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>collateral-backed-market-new</h1>
