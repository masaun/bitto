...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SECTION_8_VOUCHER_MGR_ADDRESS || '';

export default function Section_8_Voucher_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.section-8-voucher-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'section-8-voucher-mgr',
