...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SUPPLY_CAP_ENFORCER_ADDRESS || '';

export default function Supply_Cap_EnforcerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.supply-cap-enforcer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'supply-cap-enforcer',
