...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STRUCTURED_INCOME_HUB_NEW_ADDRESS || '';

export default function Structured_Income_Hub_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'structured-income-hub-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.structured-income-hub-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'structured-income-hub-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>structured-income-hub-new</h1>
