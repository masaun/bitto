...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PANDEMIC_RESPONSE_BUDGET_ADDRESS || '';

export default function Pandemic_Response_BudgetPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.pandemic-response-budget/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'pandemic-response-budget',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>pandemic-response-budget</h1>
