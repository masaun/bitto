...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INFURA_GATEWAY_V1_ADDRESS || '';

export default function Infura_Gateway_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.infura-gateway-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'infura-gateway-v1',
