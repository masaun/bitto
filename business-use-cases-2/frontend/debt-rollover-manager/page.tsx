...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEBT_ROLLOVER_MANAGER_ADDRESS || '';

export default function Debt_Rollover_ManagerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.debt-rollover-manager/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'debt-rollover-manager',
