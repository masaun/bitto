...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EQUIPMENT_LEASE_BACKED_NEW_ADDRESS || '';

export default function Equipment_Lease_Backed_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'equipment-lease-backed-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.equipment-lease-backed-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'equipment-lease-backed-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>equipment-lease-backed-new</h1>
