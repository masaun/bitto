...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROOFARCHIVE_PRIVATE_FETCH_ADDRESS || '';

export default function Proofarchive_Private_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'proofarchive-private-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.proofarchive-private-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'proofarchive-private-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>proofarchive-private-fetch</h1>
