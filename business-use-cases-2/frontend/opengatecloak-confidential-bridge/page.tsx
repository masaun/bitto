...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OPENGATECLOAK_CONFIDENTIAL_BRIDGE_ADDRESS || '';

export default function Opengatecloak_Confidential_BridgePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'opengatecloak-confidential-bridge',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.opengatecloak-confidential-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'opengatecloak-confidential-bridge',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>opengatecloak-confidential-bridge</h1>
