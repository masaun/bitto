...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERIFORGE_SECURE_ROUTER_ADDRESS || '';

export default function Veriforge_Secure_RouterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veriforge-secure-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veriforge-secure-router',
