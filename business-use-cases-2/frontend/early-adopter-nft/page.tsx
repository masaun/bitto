...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EARLY_ADOPTER_NFT_ADDRESS || '';

export default function Early_Adopter_NftPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.early-adopter-nft/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'early-adopter-nft',
