...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LANDMARK_STATUS_CHK_ADDRESS || '';

export default function Landmark_Status_ChkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.landmark-status-chk/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'landmark-status-chk',
