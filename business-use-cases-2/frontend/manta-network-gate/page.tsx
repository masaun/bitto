...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MANTA_NETWORK_GATE_ADDRESS || '';

export default function Manta_Network_GatePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.manta-network-gate/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'manta-network-gate',
