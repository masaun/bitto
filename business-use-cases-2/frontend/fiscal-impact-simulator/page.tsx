...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_IMPACT_SIMULATOR_ADDRESS || '';

export default function Fiscal_Impact_SimulatorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-impact-simulator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-impact-simulator',
