...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STEPUP_COUPON_TRIGGER_NEW_ADDRESS || '';

export default function Stepup_Coupon_Trigger_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'stepup-coupon-trigger-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.stepup-coupon-trigger-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'stepup-coupon-trigger-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>stepup-coupon-trigger-new</h1>
