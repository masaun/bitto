...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GOVERNMENT_DEBT_LEDGER_ADDRESS || '';

export default function Government_Debt_LedgerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.government-debt-ledger/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'government-debt-ledger',
