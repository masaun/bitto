...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TIMELOCK_EXECUTION_ADDRESS || '';

export default function Timelock_ExecutionPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.timelock-execution/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'timelock-execution',
