...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REGULATORY_HOOK_V2_ADDRESS || '';

export default function Regulatory_Hook_V2Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.regulatory-hook-v2/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'regulatory-hook-v2',
