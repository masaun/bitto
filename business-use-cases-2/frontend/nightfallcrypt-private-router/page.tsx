...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NIGHTFALLCRYPT_PRIVATE_ROUTER_ADDRESS || '';

export default function Nightfallcrypt_Private_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'nightfallcrypt-private-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.nightfallcrypt-private-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'nightfallcrypt-private-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>nightfallcrypt-private-router</h1>
