...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LIQUIDATION_ENGINE_ADDRESS || '';

export default function Liquidation_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.liquidation-engine/read-only/get-counter`,
        {
          method: 'GET',
