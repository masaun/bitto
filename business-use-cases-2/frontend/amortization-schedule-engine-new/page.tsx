...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AMORTIZATION_SCHEDULE_ENGINE_NEW_ADDRESS || '';

export default function Amortization_Schedule_Engine_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'amortization-schedule-engine-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.amortization-schedule-engine-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'amortization-schedule-engine-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>amortization-schedule-engine-new</h1>
