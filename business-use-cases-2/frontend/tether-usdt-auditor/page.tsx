...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TETHER_USDT_AUDITOR_ADDRESS || '';

export default function Tether_Usdt_AuditorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tether-usdt-auditor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tether-usdt-auditor',
