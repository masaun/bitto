...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_YONDERCRYPT_TRUSTED_FETCH_ADDRESS || '';

export default function Yondercrypt_Trusted_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'yondercrypt-trusted-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.yondercrypt-trusted-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'yondercrypt-trusted-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>yondercrypt-trusted-fetch</h1>
