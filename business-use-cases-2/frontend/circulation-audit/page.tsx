...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CIRCULATION_AUDIT_ADDRESS || '';

export default function Circulation_AuditPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.circulation-audit/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'circulation-audit',
