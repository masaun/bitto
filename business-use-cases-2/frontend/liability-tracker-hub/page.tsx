...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LIABILITY_TRACKER_HUB_ADDRESS || '';

export default function Liability_Tracker_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.liability-tracker-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'liability-tracker-hub',
