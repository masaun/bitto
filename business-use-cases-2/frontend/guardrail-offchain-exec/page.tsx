...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GUARDRAIL_OFFCHAIN_EXEC_ADDRESS || '';

export default function Guardrail_Offchain_ExecPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.guardrail-offchain-exec/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'guardrail-offchain-exec',
