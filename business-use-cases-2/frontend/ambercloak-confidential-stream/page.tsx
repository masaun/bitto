...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AMBERCLOAK_CONFIDENTIAL_STREAM_ADDRESS || '';

export default function Ambercloak_Confidential_StreamPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'ambercloak-confidential-stream',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ambercloak-confidential-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ambercloak-confidential-stream',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>ambercloak-confidential-stream</h1>
