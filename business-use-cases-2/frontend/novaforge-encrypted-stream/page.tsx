...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NOVAFORGE_ENCRYPTED_STREAM_ADDRESS || '';

export default function Novaforge_Encrypted_StreamPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'novaforge-encrypted-stream',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.novaforge-encrypted-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'novaforge-encrypted-stream',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>novaforge-encrypted-stream</h1>
