...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DATA_TAPE_VALIDATOR_NEW_ADDRESS || '';

export default function Data_Tape_Validator_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.data-tape-validator-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'data-tape-validator-new',
