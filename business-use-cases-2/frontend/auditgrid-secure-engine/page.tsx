...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDITGRID_SECURE_ENGINE_ADDRESS || '';

export default function Auditgrid_Secure_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auditgrid-secure-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auditgrid-secure-engine',
