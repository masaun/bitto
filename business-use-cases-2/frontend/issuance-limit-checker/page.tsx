...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ISSUANCE_LIMIT_CHECKER_ADDRESS || '';

export default function Issuance_Limit_CheckerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.issuance-limit-checker/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'issuance-limit-checker',
