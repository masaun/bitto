...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NORTHVEILFORGE_CONFIDENTIAL_ADAPTER_ADDRESS || '';

export default function Northveilforge_Confidential_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'northveilforge-confidential-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.northveilforge-confidential-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'northveilforge-confidential-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>northveilforge-confidential-adapter</h1>
