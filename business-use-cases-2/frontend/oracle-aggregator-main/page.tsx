...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ORACLE_AGGREGATOR_MAIN_ADDRESS || '';

export default function Oracle_Aggregator_MainPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.oracle-aggregator-main/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'oracle-aggregator-main',
