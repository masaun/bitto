...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BLOCK_HEADER_REPO_ADDRESS || '';

export default function Block_Header_RepoPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.block-header-repo/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'block-header-repo',
