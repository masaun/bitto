...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CHECKSUM_VERIFIER_ADDRESS || '';

export default function Checksum_VerifierPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.checksum-verifier/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'checksum-verifier',
