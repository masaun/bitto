...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AIR_QUALITY_SENSOR_ADDRESS || '';

export default function Air_Quality_SensorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.air-quality-sensor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'air-quality-sensor',
