...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ASSET_SWAP_ENGINE_NEW_ADDRESS || '';

export default function Asset_Swap_Engine_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.asset-swap-engine-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'asset-swap-engine-new',
