...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_YIELD_BEARING_STX_ADDRESS || '';

export default function Yield_Bearing_StxPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.yield-bearing-stx/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'yield-bearing-stx',
