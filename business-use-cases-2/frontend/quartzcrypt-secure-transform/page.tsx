...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_QUARTZCRYPT_SECURE_TRANSFORM_ADDRESS || '';

export default function Quartzcrypt_Secure_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'quartzcrypt-secure-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.quartzcrypt-secure-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'quartzcrypt-secure-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>quartzcrypt-secure-transform</h1>
