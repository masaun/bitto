...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REDSTONE_DATA_PROXY_ADDRESS || '';

export default function Redstone_Data_ProxyPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.redstone-data-proxy/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'redstone-data-proxy',
