...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEVICE_FINGERPRINT_ADDRESS || '';

export default function Device_FingerprintPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.device-fingerprint/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'device-fingerprint',
