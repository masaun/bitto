...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OBLIGOR_DIVERSIFICATION_MODULE_NEW_ADDRESS || '';

export default function Obligor_Diversification_Module_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'obligor-diversification-module-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.obligor-diversification-module-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'obligor-diversification-module-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>obligor-diversification-module-new</h1>
