...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RISK_MODELING_POR_ADDRESS || '';

export default function Risk_Modeling_PorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.risk-modeling-por/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'risk-modeling-por',
