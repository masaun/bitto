...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MOONFORGE_SECRET_STREAM_ADDRESS || '';

export default function Moonforge_Secret_StreamPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.moonforge-secret-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'moonforge-secret-stream',
