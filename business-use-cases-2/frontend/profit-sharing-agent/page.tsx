...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROFIT_SHARING_AGENT_ADDRESS || '';

export default function Profit_Sharing_AgentPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.profit-sharing-agent/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'profit-sharing-agent',
