...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDITLINK_CONFIDENTIAL_GATEWAY_ADDRESS || '';

export default function Auditlink_Confidential_GatewayPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'auditlink-confidential-gateway',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auditlink-confidential-gateway/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auditlink-confidential-gateway',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>auditlink-confidential-gateway</h1>
