...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COVENANT_COMPLIANCE_CHECKER_NEW_ADDRESS || '';

export default function Covenant_Compliance_Checker_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'covenant-compliance-checker-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.covenant-compliance-checker-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'covenant-compliance-checker-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>covenant-compliance-checker-new</h1>
