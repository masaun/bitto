...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BURN_BACK_MECHANISM_ADDRESS || '';

export default function Burn_Back_MechanismPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.burn-back-mechanism/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'burn-back-mechanism',
