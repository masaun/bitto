...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BTC_BRIDGE_WATCHER_ADDRESS || '';

export default function Btc_Bridge_WatcherPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.btc-bridge-watcher/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'btc-bridge-watcher',
