...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GRANT_FUNDING_HUB_ADDRESS || '';

export default function Grant_Funding_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.grant-funding-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'grant-funding-hub',
