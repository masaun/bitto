...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ENERGY_AUDIT_AUTOMATED_ADDRESS || '';

export default function Energy_Audit_AutomatedPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.energy-audit-automated/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'energy-audit-automated',
