...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FLAMEVEIL_SECURE_PARSER_ADDRESS || '';

export default function Flameveil_Secure_ParserPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.flameveil-secure-parser/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'flameveil-secure-parser',
