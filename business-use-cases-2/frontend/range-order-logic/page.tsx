...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RANGE_ORDER_LOGIC_ADDRESS || '';

export default function Range_Order_LogicPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.range-order-logic/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'range-order-logic',
