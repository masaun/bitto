...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COLLATERAL_APPRAISAL_ADDRESS || '';

export default function Collateral_AppraisalPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.collateral-appraisal/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'collateral-appraisal',
