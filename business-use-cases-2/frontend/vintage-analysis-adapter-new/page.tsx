...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VINTAGE_ANALYSIS_ADAPTER_NEW_ADDRESS || '';

export default function Vintage_Analysis_Adapter_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'vintage-analysis-adapter-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.vintage-analysis-adapter-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'vintage-analysis-adapter-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>vintage-analysis-adapter-new</h1>
