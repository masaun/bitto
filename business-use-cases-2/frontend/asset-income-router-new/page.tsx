...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ASSET_INCOME_ROUTER_NEW_ADDRESS || '';

export default function Asset_Income_Router_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.asset-income-router-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'asset-income-router-new',
