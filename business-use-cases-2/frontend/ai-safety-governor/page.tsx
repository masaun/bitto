...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AI_SAFETY_GOVERNOR_ADDRESS || '';

export default function Ai_Safety_GovernorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ai-safety-governor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ai-safety-governor',
