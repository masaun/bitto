...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EQUITY_KICKBACK_MGR_ADDRESS || '';

export default function Equity_Kickback_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.equity-kickback-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'equity-kickback-mgr',
