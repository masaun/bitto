...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REVOLVING_PERIOD_CONTROLLER_NEW_ADDRESS || '';

export default function Revolving_Period_Controller_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'revolving-period-controller-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.revolving-period-controller-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'revolving-period-controller-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>revolving-period-controller-new</h1>
