...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ADMIN_CONTROL_HUB_ADDRESS || '';

export default function Admin_Control_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.admin-control-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'admin-control-hub',
