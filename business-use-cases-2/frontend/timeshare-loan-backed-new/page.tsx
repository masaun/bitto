...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TIMESHARE_LOAN_BACKED_NEW_ADDRESS || '';

export default function Timeshare_Loan_Backed_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'timeshare-loan-backed-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.timeshare-loan-backed-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'timeshare-loan-backed-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>timeshare-loan-backed-new</h1>
