...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FLOATING_ADJ_LOGIC_ADDRESS || '';

export default function Floating_Adj_LogicPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.floating-adj-logic/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'floating-adj-logic',
