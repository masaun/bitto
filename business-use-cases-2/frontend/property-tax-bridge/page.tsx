...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROPERTY_TAX_BRIDGE_ADDRESS || '';

export default function Property_Tax_BridgePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.property-tax-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'property-tax-bridge',
