...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEBT_AMORTIZATION_SCHEDULE_ADDRESS || '';

export default function Debt_Amortization_SchedulePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.debt-amortization-schedule/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'debt-amortization-schedule',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>debt-amortization-schedule</h1>
