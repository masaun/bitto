...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STAGING_INVENTORY_ADDRESS || '';

export default function Staging_InventoryPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.staging-inventory/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'staging-inventory',
