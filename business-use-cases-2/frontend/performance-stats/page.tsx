...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PERFORMANCE_STATS_ADDRESS || '';

export default function Performance_StatsPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.performance-stats/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'performance-stats',
