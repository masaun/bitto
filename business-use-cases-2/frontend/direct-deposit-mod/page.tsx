...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DIRECT_DEPOSIT_MOD_ADDRESS || '';

export default function Direct_Deposit_ModPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.direct-deposit-mod/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'direct-deposit-mod',
