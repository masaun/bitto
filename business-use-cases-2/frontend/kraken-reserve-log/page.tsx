...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_KRAKEN_RESERVE_LOG_ADDRESS || '';

export default function Kraken_Reserve_LogPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.kraken-reserve-log/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'kraken-reserve-log',
