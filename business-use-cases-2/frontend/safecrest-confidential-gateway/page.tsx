...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SAFECREST_CONFIDENTIAL_GATEWAY_ADDRESS || '';

export default function Safecrest_Confidential_GatewayPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'safecrest-confidential-gateway',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.safecrest-confidential-gateway/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'safecrest-confidential-gateway',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>safecrest-confidential-gateway</h1>
