...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDITWARDEN_SECURE_RUNNER_ADDRESS || '';

export default function Auditwarden_Secure_RunnerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'auditwarden-secure-runner',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auditwarden-secure-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auditwarden-secure-runner',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>auditwarden-secure-runner</h1>
