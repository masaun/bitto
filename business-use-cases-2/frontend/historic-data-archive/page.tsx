...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HISTORIC_DATA_ARCHIVE_ADDRESS || '';

export default function Historic_Data_ArchivePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.historic-data-archive/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'historic-data-archive',
