...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CHECKMARK_CONFIDENTIAL_ADAPTER_ADDRESS || '';

export default function Checkmark_Confidential_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'checkmark-confidential-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.checkmark-confidential-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'checkmark-confidential-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>checkmark-confidential-adapter</h1>
