...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDITGATE_SECURE_FETCH_ADDRESS || '';

export default function Auditgate_Secure_FetchPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auditgate-secure-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auditgate-secure-fetch',
