...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RENT_ROLL_SCRUBBER_ADDRESS || '';

export default function Rent_Roll_ScrubberPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.rent-roll-scrubber/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'rent-roll-scrubber',
