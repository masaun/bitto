...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REMOTE_REBOOT_LOGIC_ADDRESS || '';

export default function Remote_Reboot_LogicPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.remote-reboot-logic/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'remote-reboot-logic',
