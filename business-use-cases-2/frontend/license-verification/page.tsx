...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LICENSE_VERIFICATION_ADDRESS || '';

export default function License_VerificationPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.license-verification/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'license-verification',
