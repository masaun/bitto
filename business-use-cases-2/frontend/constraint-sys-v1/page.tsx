...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CONSTRAINT_SYS_V1_ADDRESS || '';

export default function Constraint_Sys_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.constraint-sys-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'constraint-sys-v1',
