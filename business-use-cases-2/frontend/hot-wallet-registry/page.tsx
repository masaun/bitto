...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HOT_WALLET_REGISTRY_ADDRESS || '';

export default function Hot_Wallet_RegistryPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.hot-wallet-registry/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'hot-wallet-registry',
