...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ASSET_MANAGEMENT_PRO_ADDRESS || '';

export default function Asset_Management_ProPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.asset-management-pro/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'asset-management-pro',
