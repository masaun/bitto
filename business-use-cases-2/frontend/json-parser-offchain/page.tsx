...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_JSON_PARSER_OFFCHAIN_ADDRESS || '';

export default function Json_Parser_OffchainPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.json-parser-offchain/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'json-parser-offchain',
