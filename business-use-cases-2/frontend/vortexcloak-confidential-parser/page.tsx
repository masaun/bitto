...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VORTEXCLOAK_CONFIDENTIAL_PARSER_ADDRESS || '';

export default function Vortexcloak_Confidential_ParserPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'vortexcloak-confidential-parser',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.vortexcloak-confidential-parser/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'vortexcloak-confidential-parser',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>vortexcloak-confidential-parser</h1>
