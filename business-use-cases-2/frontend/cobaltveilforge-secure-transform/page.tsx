...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COBALTVEILFORGE_SECURE_TRANSFORM_ADDRESS || '';

export default function Cobaltveilforge_Secure_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'cobaltveilforge-secure-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cobaltveilforge-secure-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cobaltveilforge-secure-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>cobaltveilforge-secure-transform</h1>
