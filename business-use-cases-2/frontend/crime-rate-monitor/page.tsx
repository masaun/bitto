...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CRIME_RATE_MONITOR_ADDRESS || '';

export default function Crime_Rate_MonitorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.crime-rate-monitor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'crime-rate-monitor',
