...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_QUARTZVEILCREST_SECURE_ROUTER_ADDRESS || '';

export default function Quartzveilcrest_Secure_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'quartzveilcrest-secure-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.quartzveilcrest-secure-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'quartzveilcrest-secure-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>quartzveilcrest-secure-router</h1>
