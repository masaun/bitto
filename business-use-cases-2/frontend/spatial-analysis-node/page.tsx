...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SPATIAL_ANALYSIS_NODE_ADDRESS || '';

export default function Spatial_Analysis_NodePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.spatial-analysis-node/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'spatial-analysis-node',
