...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EMBERCRESTSEAL_SECRET_FETCH_ADDRESS || '';

export default function Embercrestseal_Secret_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'embercrestseal-secret-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.embercrestseal-secret-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'embercrestseal-secret-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>embercrestseal-secret-fetch</h1>
