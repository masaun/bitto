...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SHORT_SALE_NEGOTIATE_ADDRESS || '';

export default function Short_Sale_NegotiatePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.short-sale-negotiate/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'short-sale-negotiate',
