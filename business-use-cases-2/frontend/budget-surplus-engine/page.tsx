...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BUDGET_SURPLUS_ENGINE_ADDRESS || '';

export default function Budget_Surplus_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.budget-surplus-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'budget-surplus-engine',
