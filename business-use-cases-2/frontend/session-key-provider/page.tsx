...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SESSION_KEY_PROVIDER_ADDRESS || '';

export default function Session_Key_ProviderPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.session-key-provider/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'session-key-provider',
