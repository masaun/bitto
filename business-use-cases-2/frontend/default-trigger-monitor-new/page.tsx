...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEFAULT_TRIGGER_MONITOR_NEW_ADDRESS || '';

export default function Default_Trigger_Monitor_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'default-trigger-monitor-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.default-trigger-monitor-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'default-trigger-monitor-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>default-trigger-monitor-new</h1>
