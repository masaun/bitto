...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WAREHOUSE_FINANCING_CORE_NEW_ADDRESS || '';

export default function Warehouse_Financing_Core_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'warehouse-financing-core-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.warehouse-financing-core-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'warehouse-financing-core-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>warehouse-financing-core-new</h1>
