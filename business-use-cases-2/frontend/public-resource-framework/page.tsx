...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PUBLIC_RESOURCE_FRAMEWORK_ADDRESS || '';

export default function Public_Resource_FrameworkPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'public-resource-framework',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.public-resource-framework/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'public-resource-framework',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>public-resource-framework</h1>
