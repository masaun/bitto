...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_YIELD_CURVE_ORACLE_ADDRESS || '';

export default function Yield_Curve_OraclePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.yield-curve-oracle/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'yield-curve-oracle',
