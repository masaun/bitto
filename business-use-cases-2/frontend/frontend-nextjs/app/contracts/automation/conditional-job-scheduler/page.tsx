...
export default function ConditionalJobSchedulerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('conditional-job-scheduler');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'conditional-job-scheduler',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'conditional-job-scheduler' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Conditional Job Scheduler</h1>
