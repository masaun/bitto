...
export default function ThresholdBasedWorkflowEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('threshold-based-workflow-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'threshold-based-workflow-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'threshold-based-workflow-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Threshold Based Workflow Engine</h1>
