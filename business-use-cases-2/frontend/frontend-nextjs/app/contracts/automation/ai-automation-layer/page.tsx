...
export default function AiAutomationLayerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-automation-layer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-automation-layer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-automation-layer' },
      });
