...
export default function AgentReviewWorkflowPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-review-workflow');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-review-workflow',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-review-workflow' },
      });
