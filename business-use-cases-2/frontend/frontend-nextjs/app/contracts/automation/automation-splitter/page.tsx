...
export default function AutomationSplitterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-splitter');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-splitter',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-splitter' },
      });
