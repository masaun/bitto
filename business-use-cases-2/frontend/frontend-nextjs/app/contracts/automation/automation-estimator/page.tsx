...
export default function AutomationEstimatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-estimator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-estimator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-estimator' },
      });
