...
export default function DelayedJobSchedulerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('delayed-job-scheduler');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'delayed-job-scheduler',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'delayed-job-scheduler' },
      });
