...
export default function MlPoweredWorkflowEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ml-powered-workflow-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ml-powered-workflow-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ml-powered-workflow-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Ml Powered Workflow Engine</h1>
