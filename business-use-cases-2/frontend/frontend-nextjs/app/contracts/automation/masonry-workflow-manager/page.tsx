...
export default function MasonryWorkflowManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('masonry-workflow-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'masonry-workflow-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'masonry-workflow-manager' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Masonry Workflow Manager</h1>
