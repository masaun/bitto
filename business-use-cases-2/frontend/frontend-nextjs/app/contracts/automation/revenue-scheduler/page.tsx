...
export default function RevenueSchedulerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('revenue-scheduler');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'revenue-scheduler',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'revenue-scheduler' },
      });
