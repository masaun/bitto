...
export default function AutomationMergerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-merger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-merger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-merger' },
      });
