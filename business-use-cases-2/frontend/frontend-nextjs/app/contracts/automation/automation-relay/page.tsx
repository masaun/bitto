...
export default function AutomationRelayPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-relay');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-relay',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-relay' },
      });
