...
export default function AutomationVerifierPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-verifier');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-verifier',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-verifier' },
      });
