...
export default function AutomationInspectorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-inspector');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-inspector',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-inspector' },
      });
