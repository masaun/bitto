...
export default function DelayedWorkflowEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('delayed-workflow-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'delayed-workflow-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'delayed-workflow-engine' },
      });
