...
export default function AutomationAllocatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-allocator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-allocator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-allocator' },
      });
