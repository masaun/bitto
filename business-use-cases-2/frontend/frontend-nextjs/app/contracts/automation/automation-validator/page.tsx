...
export default function AutomationValidatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-validator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-validator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-validator' },
      });
