...
export default function AutomationReviewerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-reviewer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-reviewer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-reviewer' },
      });
