...
export default function TimeDrivenTaskSchedulerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('time-driven-task-scheduler');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'time-driven-task-scheduler',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'time-driven-task-scheduler' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Time Driven Task Scheduler</h1>
