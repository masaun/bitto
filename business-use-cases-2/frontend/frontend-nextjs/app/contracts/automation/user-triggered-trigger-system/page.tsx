...
export default function UserTriggeredTriggerSystemPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('user-triggered-trigger-system');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'user-triggered-trigger-system',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'user-triggered-trigger-system' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>User Triggered Trigger System</h1>
