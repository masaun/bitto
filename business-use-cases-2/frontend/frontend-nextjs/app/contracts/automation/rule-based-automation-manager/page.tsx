...
export default function RuleBasedAutomationManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rule-based-automation-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rule-based-automation-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rule-based-automation-manager' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Rule Based Automation Manager</h1>
