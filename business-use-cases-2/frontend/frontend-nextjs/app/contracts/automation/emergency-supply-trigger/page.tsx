...
export default function EmergencySupplyTriggerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('emergency-supply-trigger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'emergency-supply-trigger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'emergency-supply-trigger' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Emergency Supply Trigger</h1>
