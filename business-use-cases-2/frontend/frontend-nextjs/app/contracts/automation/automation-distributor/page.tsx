...
export default function AutomationDistributorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-distributor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-distributor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-distributor' },
      });
