...
export default function AutomationExaminerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-examiner');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-examiner',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-examiner' },
      });
