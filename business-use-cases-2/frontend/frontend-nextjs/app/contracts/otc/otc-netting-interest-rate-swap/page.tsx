...
export default function OtcNettingInterestRateSwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-netting-interest-rate-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-netting-interest-rate-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-netting-interest-rate-swap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Netting Interest Rate Swap</h1>
