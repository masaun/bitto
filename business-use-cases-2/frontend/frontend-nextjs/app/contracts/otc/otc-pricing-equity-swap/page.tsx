...
export default function OtcPricingEquitySwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-pricing-equity-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-pricing-equity-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-pricing-equity-swap' },
      });
