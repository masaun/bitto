...
export default function MessageBrokerStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('message-broker-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'message-broker-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'message-broker-stable' },
      });
