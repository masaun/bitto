...
export default function OtcDeskVarianceSwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-desk-variance-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-desk-variance-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-desk-variance-swap' },
      });
