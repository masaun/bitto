...
export default function OtcHaircutEquitySwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-haircut-equity-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-haircut-equity-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-haircut-equity-swap' },
      });
