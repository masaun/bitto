...
export default function OtcForecasterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-forecaster');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-forecaster',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-forecaster' },
      });
