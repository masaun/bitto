...
export default function OtcDeskCommoditySwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-desk-commodity-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-desk-commodity-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-desk-commodity-swap' },
      });
