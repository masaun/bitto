...
export default function OtcValuationFxOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-valuation-fx-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-valuation-fx-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-valuation-fx-option' },
      });
