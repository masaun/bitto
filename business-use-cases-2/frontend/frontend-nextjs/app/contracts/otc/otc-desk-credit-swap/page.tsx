...
export default function OtcDeskCreditSwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-desk-credit-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-desk-credit-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-desk-credit-swap' },
      });
