...
export default function OtcPricingFxOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-pricing-fx-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-pricing-fx-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-pricing-fx-option' },
      });
