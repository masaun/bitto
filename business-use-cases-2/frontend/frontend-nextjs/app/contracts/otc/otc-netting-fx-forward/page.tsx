...
export default function OtcNettingFxForwardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-netting-fx-forward');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-netting-fx-forward',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-netting-fx-forward' },
      });
