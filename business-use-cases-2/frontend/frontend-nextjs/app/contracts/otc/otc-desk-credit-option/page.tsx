...
export default function OtcDeskCreditOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-desk-credit-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-desk-credit-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-desk-credit-option' },
      });
