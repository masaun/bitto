...
export default function OtcPricingEquityOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-pricing-equity-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-pricing-equity-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-pricing-equity-option' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Pricing Equity Option</h1>
