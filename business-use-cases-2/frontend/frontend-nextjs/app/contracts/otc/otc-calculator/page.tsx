...
export default function OtcCalculatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-calculator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-calculator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-calculator' },
      });
