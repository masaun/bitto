...
export default function AthleteAgentFeeSettlementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('athlete-agent-fee-settlement');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'athlete-agent-fee-settlement',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'athlete-agent-fee-settlement' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Athlete Agent Fee Settlement</h1>
