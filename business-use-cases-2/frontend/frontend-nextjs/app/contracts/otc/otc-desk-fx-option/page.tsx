...
export default function OtcDeskFxOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-desk-fx-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-desk-fx-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-desk-fx-option' },
      });
