...
export default function OtcClearingFxForwardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-clearing-fx-forward');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-clearing-fx-forward',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-clearing-fx-forward' },
      });
