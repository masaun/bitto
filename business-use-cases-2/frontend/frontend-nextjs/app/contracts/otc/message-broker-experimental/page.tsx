...
export default function MessageBrokerExperimentalPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('message-broker-experimental');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'message-broker-experimental',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'message-broker-experimental' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Message Broker Experimental</h1>
