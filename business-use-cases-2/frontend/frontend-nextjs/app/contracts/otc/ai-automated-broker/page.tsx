...
export default function AiAutomatedBrokerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-automated-broker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-automated-broker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-automated-broker' },
      });
