...
export default function OtcMatchingFxForwardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-matching-fx-forward');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-matching-fx-forward',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-matching-fx-forward' },
      });
