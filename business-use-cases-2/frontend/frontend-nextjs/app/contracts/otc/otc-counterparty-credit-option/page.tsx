...
export default function OtcCounterpartyCreditOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-counterparty-credit-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-counterparty-credit-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-counterparty-credit-option' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Counterparty Credit Option</h1>
