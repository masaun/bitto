...
export default function OtcClearingFxOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-clearing-fx-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-clearing-fx-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-clearing-fx-option' },
      });
