...
export default function StadiumConcessionsSettlementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('stadium-concessions-settlement');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'stadium-concessions-settlement',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'stadium-concessions-settlement' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Stadium Concessions Settlement</h1>
