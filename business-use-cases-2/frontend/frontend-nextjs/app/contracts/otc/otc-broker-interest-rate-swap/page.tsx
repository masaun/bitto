...
export default function OtcBrokerInterestRateSwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-broker-interest-rate-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-broker-interest-rate-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-broker-interest-rate-swap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Broker Interest Rate Swap</h1>
