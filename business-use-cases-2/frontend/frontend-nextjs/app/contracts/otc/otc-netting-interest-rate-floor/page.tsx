...
export default function OtcNettingInterestRateFloorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-netting-interest-rate-floor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-netting-interest-rate-floor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-netting-interest-rate-floor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Netting Interest Rate Floor</h1>
