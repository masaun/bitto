...
export default function OtcHaircutInterestRateCollarPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-haircut-interest-rate-collar');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-haircut-interest-rate-collar',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-haircut-interest-rate-collar' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Haircut Interest Rate Collar</h1>
