...
export default function OtcValuationVarianceSwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-valuation-variance-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-valuation-variance-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-valuation-variance-swap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Valuation Variance Swap</h1>
