...
export default function OtcBrokerEquitySwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-broker-equity-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-broker-equity-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-broker-equity-swap' },
      });
