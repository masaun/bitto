...
export default function OtcHaircutCreditSwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-haircut-credit-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-haircut-credit-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-haircut-credit-swap' },
      });
