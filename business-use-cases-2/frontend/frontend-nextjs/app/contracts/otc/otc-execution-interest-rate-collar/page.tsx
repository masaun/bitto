...
export default function OtcExecutionInterestRateCollarPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-execution-interest-rate-collar');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-execution-interest-rate-collar',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-execution-interest-rate-collar' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Execution Interest Rate Collar</h1>
