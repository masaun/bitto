...
export default function OtcMatchingFxOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-matching-fx-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-matching-fx-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-matching-fx-option' },
      });
