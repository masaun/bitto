...
export default function OtcHaircutFxForwardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-haircut-fx-forward');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-haircut-fx-forward',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-haircut-fx-forward' },
      });
