...
export default function OtcHaircutInterestRateSwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-haircut-interest-rate-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-haircut-interest-rate-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-haircut-interest-rate-swap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Haircut Interest Rate Swap</h1>
