...
export default function OtcDeskEquitySwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-desk-equity-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-desk-equity-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-desk-equity-swap' },
      });
