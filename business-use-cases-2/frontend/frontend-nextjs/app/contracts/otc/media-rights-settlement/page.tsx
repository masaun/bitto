...
export default function MediaRightsSettlementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('media-rights-settlement');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'media-rights-settlement',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'media-rights-settlement' },
      });
