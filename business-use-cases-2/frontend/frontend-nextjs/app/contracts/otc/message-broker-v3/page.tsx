...
export default function MessageBrokerV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('message-broker-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'message-broker-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'message-broker-v3' },
      });
