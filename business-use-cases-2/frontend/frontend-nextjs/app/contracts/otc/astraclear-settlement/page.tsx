...
export default function AstraclearSettlementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('astraclear-settlement');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'astraclear-settlement',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'astraclear-settlement' },
      });
