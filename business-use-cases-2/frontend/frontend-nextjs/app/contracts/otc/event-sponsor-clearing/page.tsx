...
export default function EventSponsorClearingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-sponsor-clearing');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-sponsor-clearing',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-sponsor-clearing' },
      });
