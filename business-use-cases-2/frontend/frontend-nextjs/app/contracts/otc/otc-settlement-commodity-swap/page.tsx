...
export default function OtcSettlementCommoditySwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-settlement-commodity-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-settlement-commodity-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-settlement-commodity-swap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Settlement Commodity Swap</h1>
