...
export default function OtcNettingEquitySwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-netting-equity-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-netting-equity-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-netting-equity-swap' },
      });
