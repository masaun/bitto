...
export default function OtcCoordinatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-coordinator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-coordinator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-coordinator' },
      });
