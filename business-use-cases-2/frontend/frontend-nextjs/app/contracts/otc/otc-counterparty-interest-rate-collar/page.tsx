...
export default function OtcCounterpartyInterestRateCollarPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-counterparty-interest-rate-collar');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-counterparty-interest-rate-collar',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-counterparty-interest-rate-collar' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Counterparty Interest Rate Collar</h1>
