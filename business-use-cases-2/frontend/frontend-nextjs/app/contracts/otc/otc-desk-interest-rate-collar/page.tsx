...
export default function OtcDeskInterestRateCollarPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-desk-interest-rate-collar');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-desk-interest-rate-collar',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-desk-interest-rate-collar' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Desk Interest Rate Collar</h1>
