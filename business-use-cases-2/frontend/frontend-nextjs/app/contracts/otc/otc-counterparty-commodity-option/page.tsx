...
export default function OtcCounterpartyCommodityOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-counterparty-commodity-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-counterparty-commodity-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-counterparty-commodity-option' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Counterparty Commodity Option</h1>
