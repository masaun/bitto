...
export default function OtcMarginEquitySwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-margin-equity-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-margin-equity-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-margin-equity-swap' },
      });
