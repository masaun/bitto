...
export default function OtcClearingVarianceSwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-clearing-variance-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-clearing-variance-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-clearing-variance-swap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Clearing Variance Swap</h1>
