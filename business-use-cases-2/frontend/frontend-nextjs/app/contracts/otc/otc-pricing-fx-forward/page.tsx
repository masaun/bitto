...
export default function OtcPricingFxForwardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-pricing-fx-forward');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-pricing-fx-forward',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-pricing-fx-forward' },
      });
