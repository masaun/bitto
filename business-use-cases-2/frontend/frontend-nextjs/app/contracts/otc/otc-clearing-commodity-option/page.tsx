...
export default function OtcClearingCommodityOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-clearing-commodity-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-clearing-commodity-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-clearing-commodity-option' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Clearing Commodity Option</h1>
