...
export default function OtcNettingForwardRateAgreementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-netting-forward-rate-agreement');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-netting-forward-rate-agreement',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-netting-forward-rate-agreement' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Netting Forward Rate Agreement</h1>
