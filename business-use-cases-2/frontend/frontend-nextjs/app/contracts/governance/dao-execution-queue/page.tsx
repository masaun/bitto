...
export default function DaoExecutionQueuePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('dao-execution-queue');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'dao-execution-queue',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'dao-execution-queue' },
      });
