...
export default function SportsTourismDaoPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-tourism-dao');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-tourism-dao',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-tourism-dao' },
      });
