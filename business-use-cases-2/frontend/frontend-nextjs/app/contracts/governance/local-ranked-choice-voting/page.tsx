...
export default function LocalRankedChoiceVotingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-ranked-choice-voting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-ranked-choice-voting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-ranked-choice-voting' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Local Ranked Choice Voting</h1>
