...
export default function DaoMembershipPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('dao-membership');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'dao-membership',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'dao-membership' },
      });
