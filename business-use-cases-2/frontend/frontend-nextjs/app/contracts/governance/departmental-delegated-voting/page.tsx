...
export default function DepartmentalDelegatedVotingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('departmental-delegated-voting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'departmental-delegated-voting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'departmental-delegated-voting' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Departmental Delegated Voting</h1>
