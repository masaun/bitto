...
export default function EventLicensingDaoPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-licensing-dao');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-licensing-dao',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-licensing-dao' },
      });
