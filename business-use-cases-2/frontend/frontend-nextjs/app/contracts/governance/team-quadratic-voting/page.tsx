...
export default function TeamQuadraticVotingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-quadratic-voting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-quadratic-voting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-quadratic-voting' },
      });
