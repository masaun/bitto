...
export default function GovernanceValidatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-validator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-validator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-validator' },
      });
