...
export default function AiMlGovernancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-ml-governance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-ml-governance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-ml-governance' },
      });
