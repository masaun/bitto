...
export default function GovernanceOrganizerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-organizer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-organizer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-organizer' },
      });
