...
export default function DivisionalDelegatedVotingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('divisional-delegated-voting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'divisional-delegated-voting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'divisional-delegated-voting' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Divisional Delegated Voting</h1>
