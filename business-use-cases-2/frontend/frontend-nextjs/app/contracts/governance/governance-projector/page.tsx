...
export default function GovernanceProjectorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-projector');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-projector',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-projector' },
      });
