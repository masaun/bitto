...
export default function StakeholderVotingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('stakeholder-voting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'stakeholder-voting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'stakeholder-voting' },
      });
