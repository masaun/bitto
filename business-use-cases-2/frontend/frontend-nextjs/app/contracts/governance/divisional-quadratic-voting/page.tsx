...
export default function DivisionalQuadraticVotingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('divisional-quadratic-voting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'divisional-quadratic-voting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'divisional-quadratic-voting' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Divisional Quadratic Voting</h1>
