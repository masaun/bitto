...
export default function SponsorTierGovernancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sponsor-tier-governance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sponsor-tier-governance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sponsor-tier-governance' },
      });
