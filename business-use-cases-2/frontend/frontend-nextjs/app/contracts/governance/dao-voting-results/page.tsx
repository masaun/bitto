...
export default function DaoVotingResultsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('dao-voting-results');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'dao-voting-results',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'dao-voting-results' },
      });
