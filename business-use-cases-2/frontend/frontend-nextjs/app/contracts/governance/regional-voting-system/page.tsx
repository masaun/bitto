...
export default function RegionalVotingSystemPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('regional-voting-system');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'regional-voting-system',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'regional-voting-system' },
      });
