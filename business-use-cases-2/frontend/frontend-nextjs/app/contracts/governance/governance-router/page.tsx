...
export default function GovernanceRouterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-router');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-router',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-router' },
      });
