...
export default function OrderingGovernancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ordering-governance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ordering-governance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ordering-governance' },
      });
