...
export default function GovernanceCheckerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-checker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-checker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-checker' },
      });
