...
export default function StakeWeightedVotingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('stake-weighted-voting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'stake-weighted-voting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'stake-weighted-voting' },
      });
