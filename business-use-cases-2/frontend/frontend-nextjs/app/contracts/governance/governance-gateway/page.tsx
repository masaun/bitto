...
export default function GovernanceGatewayPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-gateway');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-gateway',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-gateway' },
      });
