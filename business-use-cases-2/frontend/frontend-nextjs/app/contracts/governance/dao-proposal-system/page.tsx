...
export default function DaoProposalSystemPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('dao-proposal-system');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'dao-proposal-system',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'dao-proposal-system' },
      });
