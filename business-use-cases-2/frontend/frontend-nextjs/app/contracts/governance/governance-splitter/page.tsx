...
export default function GovernanceSplitterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-splitter');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-splitter',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-splitter' },
      });
