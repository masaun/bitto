...
export default function GovernanceMergerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-merger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-merger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-merger' },
      });
