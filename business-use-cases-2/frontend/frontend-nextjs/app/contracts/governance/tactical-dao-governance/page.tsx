...
export default function TacticalDaoGovernancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tactical-dao-governance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tactical-dao-governance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tactical-dao-governance' },
      });
