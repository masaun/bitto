...
export default function GovernanceDistributorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-distributor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-distributor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-distributor' },
      });
