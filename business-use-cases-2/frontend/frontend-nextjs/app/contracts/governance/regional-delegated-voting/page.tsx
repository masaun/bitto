...
export default function RegionalDelegatedVotingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('regional-delegated-voting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'regional-delegated-voting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'regional-delegated-voting' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Regional Delegated Voting</h1>
