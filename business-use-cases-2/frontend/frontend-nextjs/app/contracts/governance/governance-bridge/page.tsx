...
export default function GovernanceBridgePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-bridge');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-bridge',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-bridge' },
      });
