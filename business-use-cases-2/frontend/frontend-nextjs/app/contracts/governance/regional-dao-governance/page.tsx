...
export default function RegionalDaoGovernancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('regional-dao-governance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'regional-dao-governance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'regional-dao-governance' },
      });
