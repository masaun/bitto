...
export default function StrategicVotingSystemPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('strategic-voting-system');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'strategic-voting-system',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'strategic-voting-system' },
      });
