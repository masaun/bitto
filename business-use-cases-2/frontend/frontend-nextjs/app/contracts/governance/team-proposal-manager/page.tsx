...
export default function TeamProposalManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-proposal-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-proposal-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-proposal-manager' },
      });
