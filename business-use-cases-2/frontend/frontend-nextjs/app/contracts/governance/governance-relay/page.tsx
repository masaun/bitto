...
export default function GovernanceRelayPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-relay');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-relay',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-relay' },
      });
