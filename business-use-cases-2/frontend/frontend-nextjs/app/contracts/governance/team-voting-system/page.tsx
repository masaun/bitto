...
export default function TeamVotingSystemPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-voting-system');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-voting-system',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-voting-system' },
      });
