...
export default function ProposalSecrecyLayerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('proposal-secrecy-layer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'proposal-secrecy-layer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'proposal-secrecy-layer' },
      });
