...
export default function DaoExitMechanismPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('dao-exit-mechanism');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'dao-exit-mechanism',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'dao-exit-mechanism' },
      });
