...
export default function LocalVotingSystemPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-voting-system');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-voting-system',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-voting-system' },
      });
