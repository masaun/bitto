...
export default function AmlMonitorFatcaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('aml-monitor-fatca');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'aml-monitor-fatca',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'aml-monitor-fatca' },
      });
