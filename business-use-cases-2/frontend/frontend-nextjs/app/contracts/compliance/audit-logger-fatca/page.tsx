...
export default function AuditLoggerFatcaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('audit-logger-fatca');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'audit-logger-fatca',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'audit-logger-fatca' },
      });
