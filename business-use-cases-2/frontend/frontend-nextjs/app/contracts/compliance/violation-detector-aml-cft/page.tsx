...
export default function ViolationDetectorAmlCftPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('violation-detector-aml-cft');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'violation-detector-aml-cft',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'violation-detector-aml-cft' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Violation Detector Aml Cft</h1>
