...
export default function DefiPrivacyAuditPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('defi-privacy-audit');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'defi-privacy-audit',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'defi-privacy-audit' },
      });
