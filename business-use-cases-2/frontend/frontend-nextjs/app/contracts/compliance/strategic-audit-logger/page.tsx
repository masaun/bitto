...
export default function StrategicAuditLoggerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('strategic-audit-logger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'strategic-audit-logger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'strategic-audit-logger' },
      });
