...
export default function TreatyCompliancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treaty-compliance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treaty-compliance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treaty-compliance' },
      });
