...
export default function FinancePrivacyAuditPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('finance-privacy-audit');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'finance-privacy-audit',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'finance-privacy-audit' },
      });
