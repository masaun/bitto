...
export default function KycCheckerMifidPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('kyc-checker-mifid');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'kyc-checker-mifid',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'kyc-checker-mifid' },
      });
