...
export default function PostconstructionAuditPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('postconstruction-audit');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'postconstruction-audit',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'postconstruction-audit' },
      });
