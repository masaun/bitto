...
export default function ComplianceDistributorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-distributor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-distributor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-distributor' },
      });
