...
export default function AuditReportExportPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('audit-report-export');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'audit-report-export',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'audit-report-export' },
      });
