...
export default function DataProtectorAmlCftPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('data-protector-aml-cft');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'data-protector-aml-cft',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'data-protector-aml-cft' },
      });
