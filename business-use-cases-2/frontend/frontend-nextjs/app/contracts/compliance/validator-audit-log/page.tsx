...
export default function ValidatorAuditLogPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('validator-audit-log');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'validator-audit-log',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'validator-audit-log' },
      });
