...
export default function KycCheckerRiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('kyc-checker-ria');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'kyc-checker-ria',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'kyc-checker-ria' },
      });
