...
export default function DivisionalAuditLoggerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('divisional-audit-logger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'divisional-audit-logger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'divisional-audit-logger' },
      });
