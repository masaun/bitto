...
export default function AuditLoggerFiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('audit-logger-fia');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'audit-logger-fia',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'audit-logger-fia' },
      });
