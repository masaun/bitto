...
export default function RegionalAuditLoggerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('regional-audit-logger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'regional-audit-logger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'regional-audit-logger' },
      });
