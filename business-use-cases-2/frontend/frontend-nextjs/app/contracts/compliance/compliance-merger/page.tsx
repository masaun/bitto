...
export default function ComplianceMergerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-merger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-merger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-merger' },
      });
