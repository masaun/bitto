...
export default function BridgeAuditLogPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bridge-audit-log');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bridge-audit-log',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bridge-audit-log' },
      });
