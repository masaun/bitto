...
export default function InfrastructureAuditLogPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('infrastructure-audit-log');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'infrastructure-audit-log',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'infrastructure-audit-log' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Infrastructure Audit Log</h1>
