...
export default function LeedComplianceTrackerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('leed-compliance-tracker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'leed-compliance-tracker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'leed-compliance-tracker' },
      });
