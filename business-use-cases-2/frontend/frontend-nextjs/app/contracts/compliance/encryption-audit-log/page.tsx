...
export default function EncryptionAuditLogPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encryption-audit-log');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encryption-audit-log',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encryption-audit-log' },
      });
