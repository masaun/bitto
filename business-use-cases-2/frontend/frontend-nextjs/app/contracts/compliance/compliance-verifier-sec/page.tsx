...
export default function ComplianceVerifierSecPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-verifier-sec');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-verifier-sec',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-verifier-sec' },
      });
