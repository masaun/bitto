...
export default function AuditLoggerCftcPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('audit-logger-cftc');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'audit-logger-cftc',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'audit-logger-cftc' },
      });
