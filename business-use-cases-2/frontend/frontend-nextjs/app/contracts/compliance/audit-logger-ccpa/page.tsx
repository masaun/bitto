...
export default function AuditLoggerCcpaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('audit-logger-ccpa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'audit-logger-ccpa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'audit-logger-ccpa' },
      });
