...
export default function AuditLoggerHipaaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('audit-logger-hipaa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'audit-logger-hipaa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'audit-logger-hipaa' },
      });
