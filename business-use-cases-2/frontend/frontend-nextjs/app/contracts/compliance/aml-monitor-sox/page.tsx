...
export default function AmlMonitorSoxPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('aml-monitor-sox');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'aml-monitor-sox',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'aml-monitor-sox' },
      });
