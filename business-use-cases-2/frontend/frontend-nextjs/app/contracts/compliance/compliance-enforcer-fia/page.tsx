...
export default function ComplianceEnforcerFiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-enforcer-fia');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-enforcer-fia',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-enforcer-fia' },
      });
