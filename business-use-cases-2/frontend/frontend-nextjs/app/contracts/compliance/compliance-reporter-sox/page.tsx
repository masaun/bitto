...
export default function ComplianceReporterSoxPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-reporter-sox');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-reporter-sox',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-reporter-sox' },
      });
