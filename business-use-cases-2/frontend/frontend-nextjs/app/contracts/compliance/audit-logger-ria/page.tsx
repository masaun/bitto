...
export default function AuditLoggerRiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('audit-logger-ria');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'audit-logger-ria',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'audit-logger-ria' },
      });
