...
export default function RollupPrivacyAuditPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rollup-privacy-audit');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rollup-privacy-audit',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rollup-privacy-audit' },
      });
