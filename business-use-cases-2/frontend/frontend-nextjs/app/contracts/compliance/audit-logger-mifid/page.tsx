...
export default function AuditLoggerMifidPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('audit-logger-mifid');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'audit-logger-mifid',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'audit-logger-mifid' },
      });
