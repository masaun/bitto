...
export default function TicketResaleCompliancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ticket-resale-compliance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ticket-resale-compliance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ticket-resale-compliance' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Ticket Resale Compliance</h1>
