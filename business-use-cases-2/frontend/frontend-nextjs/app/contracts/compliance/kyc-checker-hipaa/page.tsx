...
export default function KycCheckerHipaaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('kyc-checker-hipaa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'kyc-checker-hipaa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'kyc-checker-hipaa' },
      });
