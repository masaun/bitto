...
export default function ComplianceForecasterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-forecaster');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-forecaster',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-forecaster' },
      });
