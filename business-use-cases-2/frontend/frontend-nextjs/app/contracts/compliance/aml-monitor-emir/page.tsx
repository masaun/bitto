...
export default function AmlMonitorEmirPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('aml-monitor-emir');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'aml-monitor-emir',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'aml-monitor-emir' },
      });
