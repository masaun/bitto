...
export default function KycCheckerEmirPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('kyc-checker-emir');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'kyc-checker-emir',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'kyc-checker-emir' },
      });
