...
export default function AmlMonitorCftcPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('aml-monitor-cftc');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'aml-monitor-cftc',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'aml-monitor-cftc' },
      });
