...
export default function ComplianceOrganizerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-organizer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-organizer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-organizer' },
      });
