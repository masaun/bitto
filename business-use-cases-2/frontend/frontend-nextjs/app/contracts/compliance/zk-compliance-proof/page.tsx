...
export default function ZkComplianceProofPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('zk-compliance-proof');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'zk-compliance-proof',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'zk-compliance-proof' },
      });
