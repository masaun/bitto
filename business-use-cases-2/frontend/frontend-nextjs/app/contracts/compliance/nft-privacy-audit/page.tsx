...
export default function NftPrivacyAuditPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('nft-privacy-audit');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'nft-privacy-audit',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'nft-privacy-audit' },
      });
