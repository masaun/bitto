...
export default function ConsentTrackerAmlCftPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('consent-tracker-aml-cft');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'consent-tracker-aml-cft',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'consent-tracker-aml-cft' },
      });
