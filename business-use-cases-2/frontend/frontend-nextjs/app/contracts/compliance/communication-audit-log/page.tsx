...
export default function CommunicationAuditLogPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('communication-audit-log');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'communication-audit-log',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'communication-audit-log' },
      });
