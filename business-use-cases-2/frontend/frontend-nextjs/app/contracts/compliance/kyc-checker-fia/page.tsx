...
export default function KycCheckerFiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('kyc-checker-fia');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'kyc-checker-fia',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'kyc-checker-fia' },
      });
