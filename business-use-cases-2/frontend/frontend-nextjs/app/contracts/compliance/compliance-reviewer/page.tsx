...
export default function ComplianceReviewerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-reviewer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-reviewer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-reviewer' },
      });
