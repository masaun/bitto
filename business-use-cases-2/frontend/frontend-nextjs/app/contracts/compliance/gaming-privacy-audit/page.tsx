...
export default function GamingPrivacyAuditPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('gaming-privacy-audit');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'gaming-privacy-audit',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'gaming-privacy-audit' },
      });
