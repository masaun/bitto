...
export default function ComplianceSplitterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-splitter');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-splitter',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-splitter' },
      });
