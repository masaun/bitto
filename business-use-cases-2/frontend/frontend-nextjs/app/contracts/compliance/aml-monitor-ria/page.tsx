...
export default function AmlMonitorRiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('aml-monitor-ria');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'aml-monitor-ria',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'aml-monitor-ria' },
      });
