...
export default function AiPrivacyAuditPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-privacy-audit');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-privacy-audit',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-privacy-audit' },
      });
