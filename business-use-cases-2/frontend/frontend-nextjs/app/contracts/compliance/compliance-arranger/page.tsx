...
export default function ComplianceArrangerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-arranger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-arranger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-arranger' },
      });
