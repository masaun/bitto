...
export default function ComplianceEnforcerDoddFrankPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-enforcer-dodd-frank');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-enforcer-dodd-frank',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-enforcer-dodd-frank' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Compliance Enforcer Dodd Frank</h1>
