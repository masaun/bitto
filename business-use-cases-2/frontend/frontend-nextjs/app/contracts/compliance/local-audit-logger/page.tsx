...
export default function LocalAuditLoggerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-audit-logger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-audit-logger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-audit-logger' },
      });
