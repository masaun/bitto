...
export default function TeamAuditLoggerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-audit-logger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-audit-logger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-audit-logger' },
      });
