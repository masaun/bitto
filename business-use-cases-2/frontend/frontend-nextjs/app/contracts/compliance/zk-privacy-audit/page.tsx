...
export default function ZkPrivacyAuditPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('zk-privacy-audit');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'zk-privacy-audit',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'zk-privacy-audit' },
      });
