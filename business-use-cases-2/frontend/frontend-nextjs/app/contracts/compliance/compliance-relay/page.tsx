...
export default function ComplianceRelayPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-relay');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-relay',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-relay' },
      });
