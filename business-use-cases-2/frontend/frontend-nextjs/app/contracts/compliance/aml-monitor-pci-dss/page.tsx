...
export default function AmlMonitorPciDssPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('aml-monitor-pci-dss');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'aml-monitor-pci-dss',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'aml-monitor-pci-dss' },
      });
