...
export default function SiteSafetyCompliancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('site-safety-compliance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'site-safety-compliance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'site-safety-compliance' },
      });
