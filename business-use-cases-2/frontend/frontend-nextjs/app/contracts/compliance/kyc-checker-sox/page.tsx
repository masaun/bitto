...
export default function KycCheckerSoxPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('kyc-checker-sox');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'kyc-checker-sox',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'kyc-checker-sox' },
      });
