...
export default function EnergyEfficiencyAuditPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('energy-efficiency-audit');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'energy-efficiency-audit',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'energy-efficiency-audit' },
      });
