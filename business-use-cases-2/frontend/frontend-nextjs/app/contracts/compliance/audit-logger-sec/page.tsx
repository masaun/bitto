...
export default function AuditLoggerSecPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('audit-logger-sec');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'audit-logger-sec',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'audit-logger-sec' },
      });
