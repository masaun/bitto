...
export default function CrossNetworkAuditPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('cross-network-audit');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'cross-network-audit',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'cross-network-audit' },
      });
