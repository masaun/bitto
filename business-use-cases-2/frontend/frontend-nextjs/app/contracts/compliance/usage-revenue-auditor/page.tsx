...
export default function UsageRevenueAuditorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('usage-revenue-auditor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'usage-revenue-auditor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'usage-revenue-auditor' },
      });
