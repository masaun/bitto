...
export default function AmlMonitorSecPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('aml-monitor-sec');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'aml-monitor-sec',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'aml-monitor-sec' },
      });
