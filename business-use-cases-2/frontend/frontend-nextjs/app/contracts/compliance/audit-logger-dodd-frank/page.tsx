...
export default function AuditLoggerDoddFrankPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('audit-logger-dodd-frank');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'audit-logger-dodd-frank',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'audit-logger-dodd-frank' },
      });
