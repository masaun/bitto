...
export default function ComplianceReporterRiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-reporter-ria');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-reporter-ria',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-reporter-ria' },
      });
