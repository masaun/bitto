...
export default function ComplianceEstimatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-estimator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-estimator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-estimator' },
      });
