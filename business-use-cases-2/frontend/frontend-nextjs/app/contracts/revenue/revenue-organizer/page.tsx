...
export default function RevenueOrganizerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('revenue-organizer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'revenue-organizer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'revenue-organizer' },
      });
