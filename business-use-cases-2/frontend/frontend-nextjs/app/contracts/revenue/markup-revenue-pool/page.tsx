...
export default function MarkupRevenuePoolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('markup-revenue-pool');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'markup-revenue-pool',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'markup-revenue-pool' },
      });
