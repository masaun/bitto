...
export default function DividendFeeManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('dividend-fee-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'dividend-fee-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'dividend-fee-manager' },
      });
