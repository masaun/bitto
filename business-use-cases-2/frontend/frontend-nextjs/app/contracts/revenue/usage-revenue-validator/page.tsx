...
export default function UsageRevenueValidatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('usage-revenue-validator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'usage-revenue-validator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'usage-revenue-validator' },
      });
