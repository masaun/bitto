...
export default function AffiliateRoyaltyManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('affiliate-royalty-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'affiliate-royalty-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'affiliate-royalty-manager' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Affiliate Royalty Manager</h1>
