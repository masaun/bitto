...
export default function AffiliateRevenueTrackerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('affiliate-revenue-tracker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'affiliate-revenue-tracker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'affiliate-revenue-tracker' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Affiliate Revenue Tracker</h1>
