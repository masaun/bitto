...
export default function InsurancePremiumFeeManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('insurance-premium-fee-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'insurance-premium-fee-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'insurance-premium-fee-manager' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Insurance Premium Fee Manager</h1>
