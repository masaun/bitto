...
export default function ShieldedRoyaltyEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('shielded-royalty-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'shielded-royalty-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'shielded-royalty-engine' },
      });
