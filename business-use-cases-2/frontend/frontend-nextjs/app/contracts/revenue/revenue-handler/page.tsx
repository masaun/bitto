...
export default function RevenueHandlerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('revenue-handler');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'revenue-handler',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'revenue-handler' },
      });
