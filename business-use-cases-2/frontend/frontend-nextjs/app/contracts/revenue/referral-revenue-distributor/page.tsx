...
export default function ReferralRevenueDistributorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('referral-revenue-distributor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'referral-revenue-distributor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'referral-revenue-distributor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Referral Revenue Distributor</h1>
