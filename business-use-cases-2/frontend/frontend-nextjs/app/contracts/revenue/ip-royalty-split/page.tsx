...
export default function IpRoyaltySplitPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ip-royalty-split');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ip-royalty-split',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ip-royalty-split' },
      });
