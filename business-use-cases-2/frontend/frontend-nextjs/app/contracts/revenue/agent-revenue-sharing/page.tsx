...
export default function AgentRevenueSharingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-revenue-sharing');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-revenue-sharing',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-revenue-sharing' },
      });
