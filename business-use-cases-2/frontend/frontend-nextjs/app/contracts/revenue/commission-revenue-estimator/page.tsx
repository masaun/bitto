...
export default function CommissionRevenueEstimatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('commission-revenue-estimator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'commission-revenue-estimator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'commission-revenue-estimator' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Commission Revenue Estimator</h1>
