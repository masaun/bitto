...
export default function RevenueCoordinatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('revenue-coordinator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'revenue-coordinator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'revenue-coordinator' },
      });
