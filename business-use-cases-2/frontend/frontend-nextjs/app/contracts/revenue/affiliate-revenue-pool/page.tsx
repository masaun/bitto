...
export default function AffiliateRevenuePoolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('affiliate-revenue-pool');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'affiliate-revenue-pool',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'affiliate-revenue-pool' },
      });
