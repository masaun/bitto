...
export default function MarkupRevenueExaminerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('markup-revenue-examiner');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'markup-revenue-examiner',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'markup-revenue-examiner' },
      });
