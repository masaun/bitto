...
export default function CommissioningReportManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('commissioning-report-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'commissioning-report-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'commissioning-report-manager' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Commissioning Report Manager</h1>
