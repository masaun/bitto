...
export default function RevenueReviewerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('revenue-reviewer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'revenue-reviewer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'revenue-reviewer' },
      });
