...
export default function RevenueReporterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('revenue-reporter');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'revenue-reporter',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'revenue-reporter' },
      });
