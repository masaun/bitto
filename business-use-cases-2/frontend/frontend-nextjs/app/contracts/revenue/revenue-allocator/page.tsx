...
export default function RevenueAllocatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('revenue-allocator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'revenue-allocator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'revenue-allocator' },
      });
