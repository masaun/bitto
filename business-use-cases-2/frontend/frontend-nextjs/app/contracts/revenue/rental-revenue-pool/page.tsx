...
export default function RentalRevenuePoolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rental-revenue-pool');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rental-revenue-pool',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rental-revenue-pool' },
      });
