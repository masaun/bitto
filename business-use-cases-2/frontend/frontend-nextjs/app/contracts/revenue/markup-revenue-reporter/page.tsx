...
export default function MarkupRevenueReporterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('markup-revenue-reporter');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'markup-revenue-reporter',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'markup-revenue-reporter' },
      });
