...
export default function RoyaltyRevenuePoolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('royalty-revenue-pool');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'royalty-revenue-pool',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'royalty-revenue-pool' },
      });
