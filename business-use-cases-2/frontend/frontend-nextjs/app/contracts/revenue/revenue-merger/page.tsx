...
export default function RevenueMergerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('revenue-merger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'revenue-merger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'revenue-merger' },
      });
