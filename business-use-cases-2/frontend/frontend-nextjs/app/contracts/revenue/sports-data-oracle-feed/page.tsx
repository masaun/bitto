...
export default function SportsDataOracleFeedPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-data-oracle-feed');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-data-oracle-feed',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-data-oracle-feed' },
      });
