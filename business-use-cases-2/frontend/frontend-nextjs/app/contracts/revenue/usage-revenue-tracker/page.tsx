...
export default function UsageRevenueTrackerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('usage-revenue-tracker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'usage-revenue-tracker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'usage-revenue-tracker' },
      });
