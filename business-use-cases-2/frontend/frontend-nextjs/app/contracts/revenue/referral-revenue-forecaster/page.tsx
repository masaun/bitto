...
export default function ReferralRevenueForecasterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('referral-revenue-forecaster');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'referral-revenue-forecaster',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'referral-revenue-forecaster' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Referral Revenue Forecaster</h1>
