...
export default function RoyaltyTrackingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('royalty-tracking');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'royalty-tracking',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'royalty-tracking' },
      });
