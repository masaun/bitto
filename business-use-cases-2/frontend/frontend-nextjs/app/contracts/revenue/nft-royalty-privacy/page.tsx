...
export default function NftRoyaltyPrivacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('nft-royalty-privacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'nft-royalty-privacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'nft-royalty-privacy' },
      });
