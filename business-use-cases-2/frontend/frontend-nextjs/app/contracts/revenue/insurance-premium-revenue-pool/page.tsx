...
export default function InsurancePremiumRevenuePoolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('insurance-premium-revenue-pool');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'insurance-premium-revenue-pool',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'insurance-premium-revenue-pool' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Insurance Premium Revenue Pool</h1>
