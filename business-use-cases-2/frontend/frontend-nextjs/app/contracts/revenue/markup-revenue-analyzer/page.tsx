...
export default function MarkupRevenueAnalyzerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('markup-revenue-analyzer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'markup-revenue-analyzer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'markup-revenue-analyzer' },
      });
