...
export default function RoyaltyRevenueReviewerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('royalty-revenue-reviewer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'royalty-revenue-reviewer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'royalty-revenue-reviewer' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Royalty Revenue Reviewer</h1>
