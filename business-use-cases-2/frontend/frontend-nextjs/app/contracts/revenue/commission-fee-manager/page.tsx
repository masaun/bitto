...
export default function CommissionFeeManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('commission-fee-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'commission-fee-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'commission-fee-manager' },
      });
