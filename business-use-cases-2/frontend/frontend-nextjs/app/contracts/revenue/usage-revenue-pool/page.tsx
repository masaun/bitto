...
export default function UsageRevenuePoolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('usage-revenue-pool');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'usage-revenue-pool',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'usage-revenue-pool' },
      });
