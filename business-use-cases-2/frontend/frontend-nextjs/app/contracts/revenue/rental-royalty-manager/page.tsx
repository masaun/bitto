...
export default function RentalRoyaltyManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rental-royalty-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rental-royalty-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rental-royalty-manager' },
      });
