...
export default function AdvertisingFeeManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('advertising-fee-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'advertising-fee-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'advertising-fee-manager' },
      });
