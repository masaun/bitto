...
export default function ClosingShowRevenueSharePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('closing-show-revenue-share');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'closing-show-revenue-share',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'closing-show-revenue-share' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Closing Show Revenue Share</h1>
