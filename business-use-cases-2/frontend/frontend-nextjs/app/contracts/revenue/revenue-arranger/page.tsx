...
export default function RevenueArrangerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('revenue-arranger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'revenue-arranger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'revenue-arranger' },
      });
