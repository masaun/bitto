...
export default function InterestRevenueReporterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('interest-revenue-reporter');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'interest-revenue-reporter',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'interest-revenue-reporter' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Interest Revenue Reporter</h1>
