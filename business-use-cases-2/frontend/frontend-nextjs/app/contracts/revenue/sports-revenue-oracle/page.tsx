...
export default function SportsRevenueOraclePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-revenue-oracle');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-revenue-oracle',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-revenue-oracle' },
      });
