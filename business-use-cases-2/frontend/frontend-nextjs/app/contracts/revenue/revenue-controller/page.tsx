...
export default function RevenueControllerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('revenue-controller');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'revenue-controller',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'revenue-controller' },
      });
