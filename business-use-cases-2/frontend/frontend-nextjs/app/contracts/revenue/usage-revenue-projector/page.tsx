...
export default function UsageRevenueProjectorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('usage-revenue-projector');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'usage-revenue-projector',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'usage-revenue-projector' },
      });
