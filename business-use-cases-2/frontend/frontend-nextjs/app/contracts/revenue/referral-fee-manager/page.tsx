...
export default function ReferralFeeManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('referral-fee-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'referral-fee-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'referral-fee-manager' },
      });
