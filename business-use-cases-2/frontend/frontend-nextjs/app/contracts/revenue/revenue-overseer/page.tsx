...
export default function RevenueOverseerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('revenue-overseer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'revenue-overseer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'revenue-overseer' },
      });
