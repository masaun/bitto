...
export default function DividendRevenueCheckerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('dividend-revenue-checker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'dividend-revenue-checker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'dividend-revenue-checker' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Dividend Revenue Checker</h1>
