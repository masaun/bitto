...
export default function CommissionRoyaltyManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('commission-royalty-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'commission-royalty-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'commission-royalty-manager' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Commission Royalty Manager</h1>
