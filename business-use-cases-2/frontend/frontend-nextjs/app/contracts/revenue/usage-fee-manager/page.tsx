...
export default function UsageFeeManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('usage-fee-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'usage-fee-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'usage-fee-manager' },
      });
