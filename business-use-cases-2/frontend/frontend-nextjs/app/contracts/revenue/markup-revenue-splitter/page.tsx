...
export default function MarkupRevenueSplitterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('markup-revenue-splitter');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'markup-revenue-splitter',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'markup-revenue-splitter' },
      });
