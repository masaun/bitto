...
export default function DividendRevenuePoolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('dividend-revenue-pool');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'dividend-revenue-pool',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'dividend-revenue-pool' },
      });
