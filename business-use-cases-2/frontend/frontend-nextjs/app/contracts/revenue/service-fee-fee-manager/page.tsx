...
export default function ServiceFeeFeeManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('service-fee-fee-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'service-fee-fee-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'service-fee-fee-manager' },
      });
