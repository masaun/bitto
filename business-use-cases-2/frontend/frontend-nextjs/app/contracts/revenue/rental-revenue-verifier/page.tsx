...
export default function RentalRevenueVerifierPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rental-revenue-verifier');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rental-revenue-verifier',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rental-revenue-verifier' },
      });
