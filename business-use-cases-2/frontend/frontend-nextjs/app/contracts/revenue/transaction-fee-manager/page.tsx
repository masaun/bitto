...
export default function TransactionFeeManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('transaction-fee-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'transaction-fee-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'transaction-fee-manager' },
      });
