...
export default function RentalRevenueTrackerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rental-revenue-tracker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rental-revenue-tracker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rental-revenue-tracker' },
      });
