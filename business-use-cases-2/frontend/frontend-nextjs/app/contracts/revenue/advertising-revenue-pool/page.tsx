...
export default function AdvertisingRevenuePoolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('advertising-revenue-pool');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'advertising-revenue-pool',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'advertising-revenue-pool' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Advertising Revenue Pool</h1>
