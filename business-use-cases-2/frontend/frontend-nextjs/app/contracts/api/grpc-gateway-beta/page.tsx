...
export default function GrpcGatewayBetaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('grpc-gateway-beta');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'grpc-gateway-beta',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'grpc-gateway-beta' },
      });
