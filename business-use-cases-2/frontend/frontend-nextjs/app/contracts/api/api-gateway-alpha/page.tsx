...
export default function ApiGatewayAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-gateway-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-gateway-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-gateway-alpha' },
      });
