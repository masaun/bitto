...
export default function ApiDeserializerV2Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-deserializer-v2');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-deserializer-v2',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-deserializer-v2' },
      });
