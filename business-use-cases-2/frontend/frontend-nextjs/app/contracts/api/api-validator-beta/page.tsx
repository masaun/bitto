...
export default function ApiValidatorBetaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-validator-beta');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-validator-beta',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-validator-beta' },
      });
