...
export default function ApiAdapterStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-adapter-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-adapter-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-adapter-stable' },
      });
