...
export default function ApiSerializerV2Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-serializer-v2');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-serializer-v2',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-serializer-v2' },
      });
