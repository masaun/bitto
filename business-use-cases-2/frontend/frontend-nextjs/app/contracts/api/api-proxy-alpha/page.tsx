...
export default function ApiProxyAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-proxy-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-proxy-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-proxy-alpha' },
      });
