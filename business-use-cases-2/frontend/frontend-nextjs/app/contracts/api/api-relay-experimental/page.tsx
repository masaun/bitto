...
export default function ApiRelayExperimentalPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-relay-experimental');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-relay-experimental',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-relay-experimental' },
      });
