...
export default function ApiEncoderProductionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-encoder-production');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-encoder-production',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-encoder-production' },
      });
