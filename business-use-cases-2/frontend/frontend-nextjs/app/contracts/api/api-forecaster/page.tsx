...
export default function ApiForecasterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-forecaster');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-forecaster',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-forecaster' },
      });
