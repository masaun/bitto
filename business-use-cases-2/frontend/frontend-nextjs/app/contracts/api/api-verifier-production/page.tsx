...
export default function ApiVerifierProductionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-verifier-production');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-verifier-production',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-verifier-production' },
      });
