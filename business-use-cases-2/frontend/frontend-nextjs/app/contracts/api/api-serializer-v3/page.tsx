...
export default function ApiSerializerV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-serializer-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-serializer-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-serializer-v3' },
      });
