...
export default function ComplianceAggregatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-aggregator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-aggregator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-aggregator' },
      });
