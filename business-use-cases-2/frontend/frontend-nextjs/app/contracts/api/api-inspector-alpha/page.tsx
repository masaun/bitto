...
export default function ApiInspectorAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-inspector-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-inspector-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-inspector-alpha' },
      });
