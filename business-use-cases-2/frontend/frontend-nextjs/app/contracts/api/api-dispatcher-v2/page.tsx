...
export default function ApiDispatcherV2Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-dispatcher-v2');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-dispatcher-v2',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-dispatcher-v2' },
      });
