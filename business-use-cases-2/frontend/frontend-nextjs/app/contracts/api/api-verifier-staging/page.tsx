...
export default function ApiVerifierStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-verifier-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-verifier-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-verifier-staging' },
      });
