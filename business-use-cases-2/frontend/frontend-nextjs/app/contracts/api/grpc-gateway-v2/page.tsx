...
export default function GrpcGatewayV2Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('grpc-gateway-v2');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'grpc-gateway-v2',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'grpc-gateway-v2' },
      });
