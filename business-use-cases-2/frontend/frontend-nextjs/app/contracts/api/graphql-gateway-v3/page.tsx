...
export default function GraphqlGatewayV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('graphql-gateway-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'graphql-gateway-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'graphql-gateway-v3' },
      });
