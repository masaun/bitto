...
export default function ApiAdapterV2Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-adapter-v2');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-adapter-v2',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-adapter-v2' },
      });
