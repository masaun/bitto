...
export default function WebsocketGatewayV2Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('websocket-gateway-v2');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'websocket-gateway-v2',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'websocket-gateway-v2' },
      });
