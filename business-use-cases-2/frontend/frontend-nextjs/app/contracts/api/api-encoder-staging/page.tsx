...
export default function ApiEncoderStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-encoder-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-encoder-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-encoder-staging' },
      });
