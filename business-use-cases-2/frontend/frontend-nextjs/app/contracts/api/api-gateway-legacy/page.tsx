...
export default function ApiGatewayLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-gateway-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-gateway-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-gateway-legacy' },
      });
