...
export default function ApiConnectorAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-connector-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-connector-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-connector-alpha' },
      });
