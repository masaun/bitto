...
export default function ApiBridgeExperimentalPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-bridge-experimental');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-bridge-experimental',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-bridge-experimental' },
      });
