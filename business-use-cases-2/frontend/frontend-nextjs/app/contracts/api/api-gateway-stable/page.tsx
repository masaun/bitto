...
export default function ApiGatewayStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-gateway-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-gateway-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-gateway-stable' },
      });
