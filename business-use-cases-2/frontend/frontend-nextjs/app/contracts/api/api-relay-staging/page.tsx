...
export default function ApiRelayStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-relay-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-relay-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-relay-staging' },
      });
