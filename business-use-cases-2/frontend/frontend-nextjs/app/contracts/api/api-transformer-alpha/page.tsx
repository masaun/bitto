...
export default function ApiTransformerAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-transformer-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-transformer-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-transformer-alpha' },
      });
