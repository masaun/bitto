...
export default function WebsocketGatewayLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('websocket-gateway-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'websocket-gateway-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'websocket-gateway-legacy' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Websocket Gateway Legacy</h1>
