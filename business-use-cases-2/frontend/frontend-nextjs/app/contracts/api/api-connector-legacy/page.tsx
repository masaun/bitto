...
export default function ApiConnectorLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-connector-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-connector-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-connector-legacy' },
      });
