...
export default function ApiTransformerV2Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-transformer-v2');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-transformer-v2',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-transformer-v2' },
      });
