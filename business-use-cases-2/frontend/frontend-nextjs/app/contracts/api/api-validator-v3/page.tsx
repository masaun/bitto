...
export default function ApiValidatorV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-validator-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-validator-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-validator-v3' },
      });
