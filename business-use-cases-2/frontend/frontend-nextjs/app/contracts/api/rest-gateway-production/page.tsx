...
export default function RestGatewayProductionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rest-gateway-production');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rest-gateway-production',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rest-gateway-production' },
      });
