...
export default function ApiSerializerBetaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-serializer-beta');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-serializer-beta',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-serializer-beta' },
      });
