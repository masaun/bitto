...
export default function ApiValidatorV2Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-validator-v2');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-validator-v2',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-validator-v2' },
      });
