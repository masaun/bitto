...
export default function ApiDeserializerBetaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-deserializer-beta');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-deserializer-beta',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-deserializer-beta' },
      });
