...
export default function ApiInspectorV2Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-inspector-v2');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-inspector-v2',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-inspector-v2' },
      });
