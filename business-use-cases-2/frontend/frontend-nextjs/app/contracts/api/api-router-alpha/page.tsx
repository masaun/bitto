...
export default function ApiRouterAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-router-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-router-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-router-alpha' },
      });
