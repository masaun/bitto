...
export default function WebsocketGatewayStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('websocket-gateway-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'websocket-gateway-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'websocket-gateway-stable' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Websocket Gateway Stable</h1>
