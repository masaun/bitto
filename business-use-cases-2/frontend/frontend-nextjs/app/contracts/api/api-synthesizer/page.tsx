...
export default function ApiSynthesizerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-synthesizer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-synthesizer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-synthesizer' },
      });
