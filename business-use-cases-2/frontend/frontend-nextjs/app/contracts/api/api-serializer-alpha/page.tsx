...
export default function ApiSerializerAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-serializer-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-serializer-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-serializer-alpha' },
      });
