...
export default function ApiConnectorStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-connector-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-connector-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-connector-staging' },
      });
