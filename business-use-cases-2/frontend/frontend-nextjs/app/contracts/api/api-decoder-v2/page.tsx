...
export default function ApiDecoderV2Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-decoder-v2');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-decoder-v2',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-decoder-v2' },
      });
