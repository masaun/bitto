...
export default function AutomationAggregatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-aggregator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-aggregator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-aggregator' },
      });
