...
export default function ApiConnectorV2Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-connector-v2');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-connector-v2',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-connector-v2' },
      });
