...
export default function RevenueAggregatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('revenue-aggregator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'revenue-aggregator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'revenue-aggregator' },
      });
