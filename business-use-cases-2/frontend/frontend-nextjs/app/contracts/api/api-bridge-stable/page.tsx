...
export default function ApiBridgeStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-bridge-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-bridge-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-bridge-stable' },
      });
