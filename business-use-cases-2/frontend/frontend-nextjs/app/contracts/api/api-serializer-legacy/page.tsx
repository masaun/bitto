...
export default function ApiSerializerLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-serializer-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-serializer-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-serializer-legacy' },
      });
