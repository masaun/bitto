...
export default function ApiAuditorBetaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-auditor-beta');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-auditor-beta',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-auditor-beta' },
      });
