...
export default function ApiInspectorBetaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-inspector-beta');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-inspector-beta',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-inspector-beta' },
      });
