...
export default function GraphqlGatewayStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('graphql-gateway-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'graphql-gateway-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'graphql-gateway-staging' },
      });
