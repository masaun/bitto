...
export default function ApiCheckerProductionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-checker-production');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-checker-production',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-checker-production' },
      });
