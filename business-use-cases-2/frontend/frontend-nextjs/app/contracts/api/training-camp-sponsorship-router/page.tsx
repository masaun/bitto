...
export default function TrainingCampSponsorshipRouterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('training-camp-sponsorship-router');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'training-camp-sponsorship-router',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'training-camp-sponsorship-router' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Training Camp Sponsorship Router</h1>
