...
export default function ApiAuditorStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-auditor-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-auditor-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-auditor-staging' },
      });
