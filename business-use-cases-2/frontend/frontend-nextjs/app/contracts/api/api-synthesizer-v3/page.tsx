...
export default function ApiSynthesizerV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-synthesizer-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-synthesizer-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-synthesizer-v3' },
      });
