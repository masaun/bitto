...
export default function SmartTicketNftGatewayPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('smart-ticket-nft-gateway');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'smart-ticket-nft-gateway',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'smart-ticket-nft-gateway' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Smart Ticket Nft Gateway</h1>
