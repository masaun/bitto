...
export default function ApiAuditorProductionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-auditor-production');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-auditor-production',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-auditor-production' },
      });
