...
export default function ApiSerializerStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-serializer-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-serializer-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-serializer-stable' },
      });
