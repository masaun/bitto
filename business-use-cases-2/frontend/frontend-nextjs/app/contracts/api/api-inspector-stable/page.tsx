...
export default function ApiInspectorStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-inspector-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-inspector-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-inspector-stable' },
      });
