...
export default function ApiAggregatorV2Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-aggregator-v2');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-aggregator-v2',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-aggregator-v2' },
      });
