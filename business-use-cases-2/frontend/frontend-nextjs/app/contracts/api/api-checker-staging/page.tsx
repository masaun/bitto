...
export default function ApiCheckerStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-checker-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-checker-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-checker-staging' },
      });
