...
export default function PrivateApiRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('private-api-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'private-api-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'private-api-registry' },
      });
