...
export default function ApiDistributorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-distributor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-distributor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-distributor' },
      });
