...
export default function ApiTransformerLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-transformer-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-transformer-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-transformer-legacy' },
      });
