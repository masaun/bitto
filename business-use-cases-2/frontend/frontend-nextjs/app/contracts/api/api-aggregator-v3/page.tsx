...
export default function ApiAggregatorV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-aggregator-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-aggregator-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-aggregator-v3' },
      });
