...
export default function ApiValidatorLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-validator-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-validator-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-validator-legacy' },
      });
