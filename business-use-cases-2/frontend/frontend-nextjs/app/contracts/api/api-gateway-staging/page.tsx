...
export default function ApiGatewayStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-gateway-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-gateway-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-gateway-staging' },
      });
