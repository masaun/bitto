...
export default function ApiSerializerStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-serializer-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-serializer-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-serializer-staging' },
      });
