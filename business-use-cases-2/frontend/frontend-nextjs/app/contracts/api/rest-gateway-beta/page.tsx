...
export default function RestGatewayBetaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rest-gateway-beta');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rest-gateway-beta',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rest-gateway-beta' },
      });
