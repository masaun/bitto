...
export default function ApiCheckerStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-checker-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-checker-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-checker-stable' },
      });
