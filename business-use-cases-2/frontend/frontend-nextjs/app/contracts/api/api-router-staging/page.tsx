...
export default function ApiRouterStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-router-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-router-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-router-staging' },
      });
