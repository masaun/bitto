...
export default function GraphqlGatewayStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('graphql-gateway-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'graphql-gateway-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'graphql-gateway-stable' },
      });
