...
export default function ApiDecoderBetaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-decoder-beta');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-decoder-beta',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-decoder-beta' },
      });
