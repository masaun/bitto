...
export default function ApiTransformerV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-transformer-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-transformer-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-transformer-v3' },
      });
