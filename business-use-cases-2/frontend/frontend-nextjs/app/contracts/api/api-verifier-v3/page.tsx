...
export default function ApiVerifierV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-verifier-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-verifier-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-verifier-v3' },
      });
