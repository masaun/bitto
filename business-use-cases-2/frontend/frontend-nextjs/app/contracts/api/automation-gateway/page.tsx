...
export default function AutomationGatewayPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-gateway');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-gateway',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-gateway' },
      });
