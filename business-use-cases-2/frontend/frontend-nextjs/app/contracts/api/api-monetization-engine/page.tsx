...
export default function ApiMonetizationEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-monetization-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-monetization-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-monetization-engine' },
      });
