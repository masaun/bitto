...
export default function RestGatewayAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rest-gateway-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rest-gateway-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rest-gateway-alpha' },
      });
