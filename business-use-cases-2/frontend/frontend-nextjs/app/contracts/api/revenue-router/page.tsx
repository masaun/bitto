...
export default function RevenueRouterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('revenue-router');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'revenue-router',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'revenue-router' },
      });
