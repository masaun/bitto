...
export default function ApiEncoderV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-encoder-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-encoder-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-encoder-v3' },
      });
