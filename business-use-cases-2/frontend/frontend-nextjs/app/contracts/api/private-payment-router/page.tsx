...
export default function PrivatePaymentRouterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('private-payment-router');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'private-payment-router',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'private-payment-router' },
      });
