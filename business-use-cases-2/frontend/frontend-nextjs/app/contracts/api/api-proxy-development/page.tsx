...
export default function ApiProxyDevelopmentPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-proxy-development');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-proxy-development',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-proxy-development' },
      });
