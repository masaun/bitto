...
export default function ApiBridgeBetaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-bridge-beta');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-bridge-beta',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-bridge-beta' },
      });
