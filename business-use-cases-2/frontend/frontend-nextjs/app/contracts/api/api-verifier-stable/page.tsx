...
export default function ApiVerifierStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-verifier-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-verifier-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-verifier-stable' },
      });
