...
export default function ApiConnectorStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-connector-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-connector-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-connector-stable' },
      });
