...
export default function LandscapingMaintenanceEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('landscaping-maintenance-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'landscaping-maintenance-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'landscaping-maintenance-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Landscaping Maintenance Engine</h1>
