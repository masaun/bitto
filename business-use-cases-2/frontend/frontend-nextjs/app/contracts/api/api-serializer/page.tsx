...
export default function ApiSerializerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-serializer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-serializer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-serializer' },
      });
