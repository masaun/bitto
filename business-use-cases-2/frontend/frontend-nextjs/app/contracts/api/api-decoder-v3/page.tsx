...
export default function ApiDecoderV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-decoder-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-decoder-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-decoder-v3' },
      });
