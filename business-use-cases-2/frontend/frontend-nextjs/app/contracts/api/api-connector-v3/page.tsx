...
export default function ApiConnectorV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-connector-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-connector-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-connector-v3' },
      });
