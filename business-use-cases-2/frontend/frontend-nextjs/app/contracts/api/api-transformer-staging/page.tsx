...
export default function ApiTransformerStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-transformer-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-transformer-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-transformer-staging' },
      });
