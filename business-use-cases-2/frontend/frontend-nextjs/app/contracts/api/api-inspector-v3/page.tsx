...
export default function ApiInspectorV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-inspector-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-inspector-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-inspector-v3' },
      });
