...
export default function RegulatorAccessGatewayPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('regulator-access-gateway');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'regulator-access-gateway',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'regulator-access-gateway' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Regulator Access Gateway</h1>
