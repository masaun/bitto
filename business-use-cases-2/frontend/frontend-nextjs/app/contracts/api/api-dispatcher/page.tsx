...
export default function ApiDispatcherPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-dispatcher');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-dispatcher',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-dispatcher' },
      });
