...
export default function ApiAuditorStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-auditor-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-auditor-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-auditor-stable' },
      });
