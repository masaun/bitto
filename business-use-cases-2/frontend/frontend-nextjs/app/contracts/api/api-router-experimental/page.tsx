...
export default function ApiRouterExperimentalPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-router-experimental');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-router-experimental',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-router-experimental' },
      });
