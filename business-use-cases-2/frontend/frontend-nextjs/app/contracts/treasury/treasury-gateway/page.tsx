...
export default function TreasuryGatewayPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-gateway');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-gateway',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-gateway' },
      });
