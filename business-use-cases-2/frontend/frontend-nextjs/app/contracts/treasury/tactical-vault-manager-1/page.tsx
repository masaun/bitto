...
export default function TacticalVaultManager1Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tactical-vault-manager-1');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tactical-vault-manager-1',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tactical-vault-manager-1' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Tactical Vault Manager 1</h1>
