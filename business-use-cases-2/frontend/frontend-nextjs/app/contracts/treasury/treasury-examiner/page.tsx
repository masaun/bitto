...
export default function TreasuryExaminerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-examiner');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-examiner',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-examiner' },
      });
