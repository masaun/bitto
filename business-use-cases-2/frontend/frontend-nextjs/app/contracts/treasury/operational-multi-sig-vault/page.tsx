...
export default function OperationalMultiSigVaultPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('operational-multi-sig-vault');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'operational-multi-sig-vault',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'operational-multi-sig-vault' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Operational Multi Sig Vault</h1>
