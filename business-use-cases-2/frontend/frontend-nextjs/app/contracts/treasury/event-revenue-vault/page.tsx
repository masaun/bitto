...
export default function EventRevenueVaultPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-revenue-vault');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-revenue-vault',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-revenue-vault' },
      });
