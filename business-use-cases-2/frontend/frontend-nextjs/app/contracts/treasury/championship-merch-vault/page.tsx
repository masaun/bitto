...
export default function ChampionshipMerchVaultPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('championship-merch-vault');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'championship-merch-vault',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'championship-merch-vault' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Championship Merch Vault</h1>
