...
export default function TreasuryReviewerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-reviewer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-reviewer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-reviewer' },
      });
