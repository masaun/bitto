...
export default function TreasuryCalculatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-calculator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-calculator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-calculator' },
      });
