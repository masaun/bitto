...
export default function EncryptedRefundPolicyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-refund-policy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-refund-policy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-refund-policy' },
      });
