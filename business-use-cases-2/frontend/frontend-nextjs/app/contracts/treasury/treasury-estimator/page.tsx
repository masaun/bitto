...
export default function TreasuryEstimatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-estimator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-estimator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-estimator' },
      });
