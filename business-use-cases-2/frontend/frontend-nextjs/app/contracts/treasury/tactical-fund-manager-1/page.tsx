...
export default function TacticalFundManager1Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tactical-fund-manager-1');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tactical-fund-manager-1',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tactical-fund-manager-1' },
      });
