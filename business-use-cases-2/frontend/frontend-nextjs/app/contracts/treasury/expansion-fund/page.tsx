...
export default function ExpansionFundPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('expansion-fund');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'expansion-fund',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'expansion-fund' },
      });
