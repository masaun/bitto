...
export default function MunicipalGrowthFund7Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('municipal-growth-fund-7');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'municipal-growth-fund-7',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'municipal-growth-fund-7' },
      });
