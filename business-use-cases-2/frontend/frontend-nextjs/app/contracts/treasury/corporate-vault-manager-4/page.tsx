...
export default function CorporateVaultManager4Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('corporate-vault-manager-4');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'corporate-vault-manager-4',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'corporate-vault-manager-4' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Corporate Vault Manager 4</h1>
