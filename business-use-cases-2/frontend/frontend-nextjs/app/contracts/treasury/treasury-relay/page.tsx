...
export default function TreasuryRelayPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-relay');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-relay',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-relay' },
      });
