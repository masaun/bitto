...
export default function TacticalVaultBridge1Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tactical-vault-bridge-1');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tactical-vault-bridge-1',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tactical-vault-bridge-1' },
      });
