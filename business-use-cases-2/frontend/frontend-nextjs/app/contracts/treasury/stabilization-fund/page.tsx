...
export default function StabilizationFundPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('stabilization-fund');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'stabilization-fund',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'stabilization-fund' },
      });
