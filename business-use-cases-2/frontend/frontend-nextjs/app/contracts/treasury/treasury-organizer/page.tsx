...
export default function TreasuryOrganizerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-organizer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-organizer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-organizer' },
      });
