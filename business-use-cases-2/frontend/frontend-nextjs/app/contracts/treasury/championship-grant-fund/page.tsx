...
export default function ChampionshipGrantFundPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('championship-grant-fund');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'championship-grant-fund',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'championship-grant-fund' },
      });
