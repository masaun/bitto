...
export default function VaultCombinerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('vault-combiner');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'vault-combiner',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'vault-combiner' },
      });
