...
export default function ArtisanalFurnitureVaultPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('artisanal-furniture-vault');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'artisanal-furniture-vault',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'artisanal-furniture-vault' },
      });
