...
export default function CorporateVaultRelay4Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('corporate-vault-relay-4');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'corporate-vault-relay-4',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'corporate-vault-relay-4' },
      });
