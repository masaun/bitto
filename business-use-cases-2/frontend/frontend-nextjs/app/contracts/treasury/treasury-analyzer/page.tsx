...
export default function TreasuryAnalyzerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-analyzer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-analyzer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-analyzer' },
      });
