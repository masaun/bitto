...
export default function HiddenTreasuryManagementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('hidden-treasury-management');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'hidden-treasury-management',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'hidden-treasury-management' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Hidden Treasury Management</h1>
