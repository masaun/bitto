...
export default function ChampionshipSponsorVaultPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('championship-sponsor-vault');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'championship-sponsor-vault',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'championship-sponsor-vault' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Championship Sponsor Vault</h1>
