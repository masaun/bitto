...
export default function TreasuryCoordinatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-coordinator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-coordinator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-coordinator' },
      });
