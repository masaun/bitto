...
export default function TreasuryValidatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-validator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-validator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-validator' },
      });
