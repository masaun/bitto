...
export default function InstitutionalVaultProxy5Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('institutional-vault-proxy-5');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'institutional-vault-proxy-5',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'institutional-vault-proxy-5' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Institutional Vault Proxy 5</h1>
