...
export default function TreasuryCombinerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-combiner');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-combiner',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-combiner' },
      });
