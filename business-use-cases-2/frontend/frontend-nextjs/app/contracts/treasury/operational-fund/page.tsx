...
export default function OperationalFundPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('operational-fund');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'operational-fund',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'operational-fund' },
      });
