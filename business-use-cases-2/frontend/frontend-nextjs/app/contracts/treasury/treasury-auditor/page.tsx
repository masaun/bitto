...
export default function TreasuryAuditorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-auditor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-auditor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-auditor' },
      });
