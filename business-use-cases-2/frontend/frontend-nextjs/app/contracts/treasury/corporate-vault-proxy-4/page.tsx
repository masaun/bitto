...
export default function CorporateVaultProxy4Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('corporate-vault-proxy-4');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'corporate-vault-proxy-4',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'corporate-vault-proxy-4' },
      });
