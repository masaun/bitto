...
export default function FinancialGrowthFund3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('financial-growth-fund-3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'financial-growth-fund-3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'financial-growth-fund-3' },
      });
