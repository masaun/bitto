...
export default function EventPrizeVaultPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-prize-vault');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-prize-vault',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-prize-vault' },
      });
