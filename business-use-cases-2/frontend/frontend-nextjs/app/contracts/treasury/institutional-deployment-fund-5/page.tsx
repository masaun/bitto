...
export default function InstitutionalDeploymentFund5Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('institutional-deployment-fund-5');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'institutional-deployment-fund-5',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'institutional-deployment-fund-5' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Institutional Deployment Fund 5</h1>
