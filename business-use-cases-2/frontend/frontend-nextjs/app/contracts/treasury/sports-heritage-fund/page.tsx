...
export default function SportsHeritageFundPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-heritage-fund');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-heritage-fund',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-heritage-fund' },
      });
