...
export default function DaoEncryptedTreasuryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('dao-encrypted-treasury');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'dao-encrypted-treasury',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'dao-encrypted-treasury' },
      });
