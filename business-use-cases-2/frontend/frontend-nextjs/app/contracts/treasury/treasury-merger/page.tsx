...
export default function TreasuryMergerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-merger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-merger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-merger' },
      });
