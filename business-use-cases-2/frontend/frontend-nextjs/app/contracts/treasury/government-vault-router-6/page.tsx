...
export default function GovernmentVaultRouter6Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('government-vault-router-6');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'government-vault-router-6',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'government-vault-router-6' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Government Vault Router 6</h1>
