...
export default function TreasuryBridgePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-bridge');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-bridge',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-bridge' },
      });
