...
export default function SecureFundReleasePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('secure-fund-release');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'secure-fund-release',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'secure-fund-release' },
      });
