...
export default function ChampionshipLegacyVaultPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('championship-legacy-vault');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'championship-legacy-vault',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'championship-legacy-vault' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Championship Legacy Vault</h1>
