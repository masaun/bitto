...
export default function EncryptedAiInferencePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-ai-inference');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-ai-inference',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-ai-inference' },
      });
