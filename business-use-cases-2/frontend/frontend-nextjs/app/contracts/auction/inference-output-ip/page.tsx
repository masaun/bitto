...
export default function InferenceOutputIpPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('inference-output-ip');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'inference-output-ip',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'inference-output-ip' },
      });
