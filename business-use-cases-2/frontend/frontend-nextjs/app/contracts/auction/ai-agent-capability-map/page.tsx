...
export default function AiAgentCapabilityMapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-agent-capability-map');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-agent-capability-map',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-agent-capability-map' },
      });
