...
export default function SpotMarketFxForwardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('spot-market-fx-forward');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'spot-market-fx-forward',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'spot-market-fx-forward' },
      });
