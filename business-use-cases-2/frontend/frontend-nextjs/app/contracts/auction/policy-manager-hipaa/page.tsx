...
export default function PolicyManagerHipaaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('policy-manager-hipaa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'policy-manager-hipaa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'policy-manager-hipaa' },
      });
