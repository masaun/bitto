...
export default function NftConfidentialMetadataPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('nft-confidential-metadata');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'nft-confidential-metadata',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'nft-confidential-metadata' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Nft Confidential Metadata</h1>
