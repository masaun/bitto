...
export default function EthicsRemediationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ethics-remediation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ethics-remediation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ethics-remediation' },
      });
