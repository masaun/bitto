...
export default function AuctionRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-registry' },
      });
