...
export default function SubcontractorPaymentEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('subcontractor-payment-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'subcontractor-payment-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'subcontractor-payment-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Subcontractor Payment Engine</h1>
