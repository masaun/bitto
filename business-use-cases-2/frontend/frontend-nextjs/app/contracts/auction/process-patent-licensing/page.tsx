...
export default function ProcessPatentLicensingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('process-patent-licensing');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'process-patent-licensing',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'process-patent-licensing' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Process Patent Licensing</h1>
