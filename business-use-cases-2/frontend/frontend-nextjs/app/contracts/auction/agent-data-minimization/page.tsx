...
export default function AgentDataMinimizationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-data-minimization');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-data-minimization',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-data-minimization' },
      });
