...
export default function RiskAssessorFiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('risk-assessor-fia');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'risk-assessor-fia',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'risk-assessor-fia' },
      });
