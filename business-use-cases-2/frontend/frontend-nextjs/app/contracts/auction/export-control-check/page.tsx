...
export default function ExportControlCheckPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('export-control-check');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'export-control-check',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'export-control-check' },
      });
