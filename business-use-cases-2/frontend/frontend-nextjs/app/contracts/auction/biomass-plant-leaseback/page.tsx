...
export default function BiomassPlantLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('biomass-plant-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'biomass-plant-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'biomass-plant-leaseback' },
      });
