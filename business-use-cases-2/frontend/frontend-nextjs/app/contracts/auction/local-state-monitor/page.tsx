...
export default function LocalStateMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-state-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-state-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-state-monitor' },
      });
