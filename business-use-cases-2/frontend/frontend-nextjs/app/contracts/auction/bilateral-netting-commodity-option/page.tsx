...
export default function BilateralNettingCommodityOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bilateral-netting-commodity-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bilateral-netting-commodity-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bilateral-netting-commodity-option' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Bilateral Netting Commodity Option</h1>
