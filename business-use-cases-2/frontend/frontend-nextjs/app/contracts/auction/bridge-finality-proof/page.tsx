...
export default function BridgeFinalityProofPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bridge-finality-proof');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bridge-finality-proof',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bridge-finality-proof' },
      });
