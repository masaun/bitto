...
export default function StadiumSmartAccessPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('stadium-smart-access');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'stadium-smart-access',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'stadium-smart-access' },
      });
