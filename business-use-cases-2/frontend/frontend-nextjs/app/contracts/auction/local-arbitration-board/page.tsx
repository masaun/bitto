...
export default function LocalArbitrationBoardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-arbitration-board');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-arbitration-board',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-arbitration-board' },
      });
