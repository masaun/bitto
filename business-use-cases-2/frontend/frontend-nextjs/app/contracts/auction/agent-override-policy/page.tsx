...
export default function AgentOverridePolicyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-override-policy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-override-policy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-override-policy' },
      });
