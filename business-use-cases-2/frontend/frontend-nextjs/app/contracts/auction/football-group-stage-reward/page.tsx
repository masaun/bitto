...
export default function FootballGroupStageRewardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('football-group-stage-reward');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'football-group-stage-reward',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'football-group-stage-reward' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Football Group Stage Reward</h1>
