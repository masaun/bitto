...
export default function AuctionReducerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-reducer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-reducer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-reducer' },
      });
