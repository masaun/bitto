...
export default function PpeDistributionLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ppe-distribution-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ppe-distribution-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ppe-distribution-ledger' },
      });
