...
export default function ChampionshipGroupRewardsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('championship-group-rewards');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'championship-group-rewards',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'championship-group-rewards' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Championship Group Rewards</h1>
