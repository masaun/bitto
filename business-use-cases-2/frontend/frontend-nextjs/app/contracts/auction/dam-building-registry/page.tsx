...
export default function DamBuildingRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('dam-building-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'dam-building-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'dam-building-registry' },
      });
