...
export default function VenueDigitalTwinPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('venue-digital-twin');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'venue-digital-twin',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'venue-digital-twin' },
      });
