...
export default function AgentTrustScorePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-trust-score');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-trust-score',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-trust-score' },
      });
