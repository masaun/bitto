...
export default function ViolationDetectorFatcaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('violation-detector-fatca');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'violation-detector-fatca',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'violation-detector-fatca' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Violation Detector Fatca</h1>
