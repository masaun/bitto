...
export default function SportsRightsTokenPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-rights-token');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-rights-token',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-rights-token' },
      });
