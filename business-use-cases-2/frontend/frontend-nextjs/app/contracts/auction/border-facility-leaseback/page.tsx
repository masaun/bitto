...
export default function BorderFacilityLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('border-facility-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'border-facility-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'border-facility-leaseback' },
      });
