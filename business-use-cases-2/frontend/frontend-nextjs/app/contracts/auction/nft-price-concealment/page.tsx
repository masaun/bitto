...
export default function NftPriceConcealmentPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('nft-price-concealment');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'nft-price-concealment',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'nft-price-concealment' },
      });
