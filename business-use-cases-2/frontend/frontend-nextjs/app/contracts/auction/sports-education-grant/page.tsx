...
export default function SportsEducationGrantPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-education-grant');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-education-grant',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-education-grant' },
      });
