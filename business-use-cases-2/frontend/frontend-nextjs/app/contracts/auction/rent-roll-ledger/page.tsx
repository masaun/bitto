...
export default function RentRollLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rent-roll-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rent-roll-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rent-roll-ledger' },
      });
