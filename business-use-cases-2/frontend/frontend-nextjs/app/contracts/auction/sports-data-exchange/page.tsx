...
export default function SportsDataExchangePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-data-exchange');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-data-exchange',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-data-exchange' },
      });
