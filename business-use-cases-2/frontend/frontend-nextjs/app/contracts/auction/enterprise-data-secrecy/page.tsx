...
export default function EnterpriseDataSecrecyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('enterprise-data-secrecy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'enterprise-data-secrecy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'enterprise-data-secrecy' },
      });
