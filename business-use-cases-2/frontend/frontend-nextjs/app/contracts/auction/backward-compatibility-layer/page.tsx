...
export default function BackwardCompatibilityLayerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('backward-compatibility-layer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'backward-compatibility-layer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'backward-compatibility-layer' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Backward Compatibility Layer</h1>
