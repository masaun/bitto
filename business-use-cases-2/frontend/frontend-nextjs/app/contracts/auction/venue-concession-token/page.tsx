...
export default function VenueConcessionTokenPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('venue-concession-token');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'venue-concession-token',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'venue-concession-token' },
      });
