...
export default function TimeDrivenRuleEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('time-driven-rule-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'time-driven-rule-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'time-driven-rule-engine' },
      });
