...
export default function FiberNetworkInstallationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('fiber-network-installation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'fiber-network-installation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'fiber-network-installation' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Fiber Network Installation</h1>
