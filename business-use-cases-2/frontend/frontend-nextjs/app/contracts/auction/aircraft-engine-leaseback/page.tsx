...
export default function AircraftEngineLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('aircraft-engine-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'aircraft-engine-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'aircraft-engine-leaseback' },
      });
