...
export default function GlobalSportsIndexPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('global-sports-index');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'global-sports-index',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'global-sports-index' },
      });
