...
export default function AiSovereigntyLayerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-sovereignty-layer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-sovereignty-layer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-sovereignty-layer' },
      });
