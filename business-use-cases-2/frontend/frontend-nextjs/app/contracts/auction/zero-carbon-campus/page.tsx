...
export default function ZeroCarbonCampusPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('zero-carbon-campus');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'zero-carbon-campus',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'zero-carbon-campus' },
      });
