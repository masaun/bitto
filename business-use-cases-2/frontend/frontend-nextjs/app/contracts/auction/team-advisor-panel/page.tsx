...
export default function TeamAdvisorPanelPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-advisor-panel');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-advisor-panel',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-advisor-panel' },
      });
