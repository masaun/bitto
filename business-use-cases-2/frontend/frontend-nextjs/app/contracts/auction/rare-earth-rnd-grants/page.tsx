...
export default function RareEarthRndGrantsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rare-earth-rnd-grants');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rare-earth-rnd-grants',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rare-earth-rnd-grants' },
      });
