...
export default function ZkValidatorAttestationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('zk-validator-attestation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'zk-validator-attestation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'zk-validator-attestation' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Zk Validator Attestation</h1>
