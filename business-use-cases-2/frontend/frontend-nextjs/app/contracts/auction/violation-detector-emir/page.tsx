...
export default function ViolationDetectorEmirPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('violation-detector-emir');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'violation-detector-emir',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'violation-detector-emir' },
      });
