...
export default function SportsTravelBookingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-travel-booking');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-travel-booking',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-travel-booking' },
      });
