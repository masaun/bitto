...
export default function RuleBasedRuleEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rule-based-rule-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rule-based-rule-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rule-based-rule-engine' },
      });
