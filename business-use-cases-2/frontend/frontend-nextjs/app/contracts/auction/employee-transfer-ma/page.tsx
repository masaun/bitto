...
export default function EmployeeTransferMaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('employee-transfer-ma');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'employee-transfer-ma',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'employee-transfer-ma' },
      });
