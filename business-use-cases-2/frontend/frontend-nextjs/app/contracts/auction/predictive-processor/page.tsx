...
export default function PredictiveProcessorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('predictive-processor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'predictive-processor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'predictive-processor' },
      });
