...
export default function DelayedProcessMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('delayed-process-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'delayed-process-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'delayed-process-monitor' },
      });
