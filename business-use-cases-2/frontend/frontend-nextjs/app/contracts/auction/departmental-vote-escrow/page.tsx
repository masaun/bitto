...
export default function DepartmentalVoteEscrowPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('departmental-vote-escrow');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'departmental-vote-escrow',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'departmental-vote-escrow' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Departmental Vote Escrow</h1>
