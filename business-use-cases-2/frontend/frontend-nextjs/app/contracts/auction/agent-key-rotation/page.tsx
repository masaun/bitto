...
export default function AgentKeyRotationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-key-rotation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-key-rotation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-key-rotation' },
      });
