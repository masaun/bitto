...
export default function SyncProcessorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sync-processor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sync-processor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sync-processor' },
      });
