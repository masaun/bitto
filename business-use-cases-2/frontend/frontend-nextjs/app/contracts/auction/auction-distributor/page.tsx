...
export default function AuctionDistributorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-distributor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-distributor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-distributor' },
      });
