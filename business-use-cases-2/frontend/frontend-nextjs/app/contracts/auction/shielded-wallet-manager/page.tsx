...
export default function ShieldedWalletManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('shielded-wallet-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'shielded-wallet-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'shielded-wallet-manager' },
      });
