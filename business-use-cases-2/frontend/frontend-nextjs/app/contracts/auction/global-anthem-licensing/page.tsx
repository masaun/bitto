...
export default function GlobalAnthemLicensingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('global-anthem-licensing');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'global-anthem-licensing',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'global-anthem-licensing' },
      });
