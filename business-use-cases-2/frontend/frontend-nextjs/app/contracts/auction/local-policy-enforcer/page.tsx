...
export default function LocalPolicyEnforcerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-policy-enforcer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-policy-enforcer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-policy-enforcer' },
      });
