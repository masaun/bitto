...
export default function DelayedStateMachinePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('delayed-state-machine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'delayed-state-machine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'delayed-state-machine' },
      });
