...
export default function RagIndexRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rag-index-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rag-index-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rag-index-registry' },
      });
