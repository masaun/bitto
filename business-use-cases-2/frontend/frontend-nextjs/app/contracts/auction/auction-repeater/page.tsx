...
export default function AuctionRepeaterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-repeater');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-repeater',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-repeater' },
      });
