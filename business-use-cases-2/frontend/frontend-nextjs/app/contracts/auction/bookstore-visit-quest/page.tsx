...
export default function BookstoreVisitQuestPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bookstore-visit-quest');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bookstore-visit-quest',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bookstore-visit-quest' },
      });
