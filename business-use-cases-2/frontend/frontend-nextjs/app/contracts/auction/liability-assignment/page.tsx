...
export default function LiabilityAssignmentPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('liability-assignment');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'liability-assignment',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'liability-assignment' },
      });
