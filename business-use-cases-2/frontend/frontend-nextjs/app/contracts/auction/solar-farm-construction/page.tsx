...
export default function SolarFarmConstructionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('solar-farm-construction');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'solar-farm-construction',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'solar-farm-construction' },
      });
