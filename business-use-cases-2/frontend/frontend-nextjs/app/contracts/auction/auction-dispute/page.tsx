...
export default function AuctionDisputePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-dispute');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-dispute',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-dispute' },
      });
