...
export default function ZeroKnowledgeSequencingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('zero-knowledge-sequencing');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'zero-knowledge-sequencing',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'zero-knowledge-sequencing' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Zero Knowledge Sequencing</h1>
