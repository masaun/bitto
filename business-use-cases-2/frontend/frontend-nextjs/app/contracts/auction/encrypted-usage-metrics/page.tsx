...
export default function EncryptedUsageMetricsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-usage-metrics');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-usage-metrics',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-usage-metrics' },
      });
