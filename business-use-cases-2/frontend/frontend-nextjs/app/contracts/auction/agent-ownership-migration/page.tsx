...
export default function AgentOwnershipMigrationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-ownership-migration');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-ownership-migration',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-ownership-migration' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Agent Ownership Migration</h1>
