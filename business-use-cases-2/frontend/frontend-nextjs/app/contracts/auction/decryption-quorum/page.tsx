...
export default function DecryptionQuorumPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('decryption-quorum');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'decryption-quorum',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'decryption-quorum' },
      });
