...
export default function FlowControllerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('flow-controller');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'flow-controller',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'flow-controller' },
      });
