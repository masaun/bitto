...
export default function AssetMonetizationEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('asset-monetization-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'asset-monetization-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'asset-monetization-engine' },
      });
