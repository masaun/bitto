...
export default function EnterpriseChargebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('enterprise-chargeback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'enterprise-chargeback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'enterprise-chargeback' },
      });
