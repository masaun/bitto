...
export default function EncryptedDataWarehousePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-data-warehouse');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-data-warehouse',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-data-warehouse' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Encrypted Data Warehouse</h1>
