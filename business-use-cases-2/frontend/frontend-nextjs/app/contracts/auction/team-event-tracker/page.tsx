...
export default function TeamEventTrackerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-event-tracker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-event-tracker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-event-tracker' },
      });
