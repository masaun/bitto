...
export default function PrivacyStakingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-staking');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-staking',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-staking' },
      });
