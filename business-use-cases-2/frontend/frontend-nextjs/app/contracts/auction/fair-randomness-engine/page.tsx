...
export default function FairRandomnessEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('fair-randomness-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'fair-randomness-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'fair-randomness-engine' },
      });
