...
export default function MultiChainPrivacyPolicyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('multi-chain-privacy-policy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'multi-chain-privacy-policy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'multi-chain-privacy-policy' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Multi Chain Privacy Policy</h1>
