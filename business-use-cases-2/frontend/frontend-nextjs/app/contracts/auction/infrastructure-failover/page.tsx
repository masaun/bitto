...
export default function InfrastructureFailoverPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('infrastructure-failover');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'infrastructure-failover',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'infrastructure-failover' },
      });
