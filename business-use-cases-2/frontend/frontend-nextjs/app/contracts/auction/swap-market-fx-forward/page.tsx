...
export default function SwapMarketFxForwardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('swap-market-fx-forward');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'swap-market-fx-forward',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'swap-market-fx-forward' },
      });
