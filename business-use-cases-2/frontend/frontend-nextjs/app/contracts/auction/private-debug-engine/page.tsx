...
export default function PrivateDebugEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('private-debug-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'private-debug-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'private-debug-engine' },
      });
