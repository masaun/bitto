...
export default function CarbonCreditOffsetsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('carbon-credit-offsets');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'carbon-credit-offsets',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'carbon-credit-offsets' },
      });
