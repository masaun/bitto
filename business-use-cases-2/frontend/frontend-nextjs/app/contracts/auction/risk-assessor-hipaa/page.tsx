...
export default function RiskAssessorHipaaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('risk-assessor-hipaa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'risk-assessor-hipaa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'risk-assessor-hipaa' },
      });
