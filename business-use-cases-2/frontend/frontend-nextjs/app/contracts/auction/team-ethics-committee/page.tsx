...
export default function TeamEthicsCommitteePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-ethics-committee');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-ethics-committee',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-ethics-committee' },
      });
