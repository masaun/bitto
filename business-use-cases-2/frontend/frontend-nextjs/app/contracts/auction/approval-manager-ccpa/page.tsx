...
export default function ApprovalManagerCcpaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('approval-manager-ccpa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'approval-manager-ccpa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'approval-manager-ccpa' },
      });
