...
export default function AgentUptimeSlaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-uptime-sla');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-uptime-sla',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-uptime-sla' },
      });
