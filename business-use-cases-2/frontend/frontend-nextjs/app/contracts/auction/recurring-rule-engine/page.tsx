...
export default function RecurringRuleEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('recurring-rule-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'recurring-rule-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'recurring-rule-engine' },
      });
