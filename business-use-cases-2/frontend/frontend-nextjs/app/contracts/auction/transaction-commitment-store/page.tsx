...
export default function TransactionCommitmentStorePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('transaction-commitment-store');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'transaction-commitment-store',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'transaction-commitment-store' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Transaction Commitment Store</h1>
