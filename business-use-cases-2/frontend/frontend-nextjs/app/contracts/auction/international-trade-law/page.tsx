...
export default function InternationalTradeLawPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('international-trade-law');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'international-trade-law',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'international-trade-law' },
      });
