...
export default function CulturalAssessmentPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('cultural-assessment');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'cultural-assessment',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'cultural-assessment' },
      });
