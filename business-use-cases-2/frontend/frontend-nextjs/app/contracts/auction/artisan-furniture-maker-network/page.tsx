...
export default function ArtisanFurnitureMakerNetworkPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('artisan-furniture-maker-network');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'artisan-furniture-maker-network',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'artisan-furniture-maker-network' },
      });
