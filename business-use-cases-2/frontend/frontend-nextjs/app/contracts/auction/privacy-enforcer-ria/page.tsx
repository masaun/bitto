...
export default function PrivacyEnforcerRiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-enforcer-ria');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-enforcer-ria',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-enforcer-ria' },
      });
