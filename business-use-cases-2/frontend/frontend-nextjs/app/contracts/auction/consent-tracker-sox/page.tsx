...
export default function ConsentTrackerSoxPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('consent-tracker-sox');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'consent-tracker-sox',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'consent-tracker-sox' },
      });
