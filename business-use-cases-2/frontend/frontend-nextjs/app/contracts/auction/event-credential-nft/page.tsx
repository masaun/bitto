...
export default function EventCredentialNftPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-credential-nft');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-credential-nft',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-credential-nft' },
      });
