...
export default function IdentityValidatorEmirPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('identity-validator-emir');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'identity-validator-emir',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'identity-validator-emir' },
      });
