...
export default function QueueManagerStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('queue-manager-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'queue-manager-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'queue-manager-staging' },
      });
