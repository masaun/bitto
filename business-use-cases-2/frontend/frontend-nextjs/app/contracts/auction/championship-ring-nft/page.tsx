...
export default function ChampionshipRingNftPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('championship-ring-nft');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'championship-ring-nft',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'championship-ring-nft' },
      });
