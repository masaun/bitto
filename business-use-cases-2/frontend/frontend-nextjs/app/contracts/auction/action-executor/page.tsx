...
export default function ActionExecutorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('action-executor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'action-executor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'action-executor' },
      });
