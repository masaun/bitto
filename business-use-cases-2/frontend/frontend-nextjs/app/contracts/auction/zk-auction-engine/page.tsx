...
export default function ZkAuctionEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('zk-auction-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'zk-auction-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'zk-auction-engine' },
      });
