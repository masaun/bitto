...
export default function EventCarbonOffsetRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-carbon-offset-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-carbon-offset-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-carbon-offset-registry' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Event Carbon Offset Registry</h1>
