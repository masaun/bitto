...
export default function IdentityValidatorCcpaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('identity-validator-ccpa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'identity-validator-ccpa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'identity-validator-ccpa' },
      });
