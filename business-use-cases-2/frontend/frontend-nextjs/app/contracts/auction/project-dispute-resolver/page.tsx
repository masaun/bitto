...
export default function ProjectDisputeResolverPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('project-dispute-resolver');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'project-dispute-resolver',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'project-dispute-resolver' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Project Dispute Resolver</h1>
