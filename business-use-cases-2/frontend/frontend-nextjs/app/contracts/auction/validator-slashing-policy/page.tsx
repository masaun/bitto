...
export default function ValidatorSlashingPolicyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('validator-slashing-policy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'validator-slashing-policy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'validator-slashing-policy' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Validator Slashing Policy</h1>
