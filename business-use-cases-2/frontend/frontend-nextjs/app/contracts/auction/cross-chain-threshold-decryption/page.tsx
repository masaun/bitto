...
export default function CrossChainThresholdDecryptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('cross-chain-threshold-decryption');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'cross-chain-threshold-decryption',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'cross-chain-threshold-decryption' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Cross Chain Threshold Decryption</h1>
