...
export default function AgentGoalDecompositionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-goal-decomposition');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-goal-decomposition',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-goal-decomposition' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Agent Goal Decomposition</h1>
