...
export default function ExportQuotaAllocationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('export-quota-allocation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'export-quota-allocation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'export-quota-allocation' },
      });
