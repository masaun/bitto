...
export default function KnowledgeCitationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('knowledge-citation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'knowledge-citation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'knowledge-citation' },
      });
