...
export default function AthleteLoyaltyProgramPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('athlete-loyalty-program');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'athlete-loyalty-program',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'athlete-loyalty-program' },
      });
