...
export default function AgentAlertingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-alerting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-alerting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-alerting' },
      });
