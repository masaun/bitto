...
export default function ElevatorInstallationEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('elevator-installation-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'elevator-installation-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'elevator-installation-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Elevator Installation Engine</h1>
