...
export default function MiningSiteRestorationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('mining-site-restoration');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'mining-site-restoration',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'mining-site-restoration' },
      });
