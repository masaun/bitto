...
export default function EventFanEngagementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-fan-engagement');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-fan-engagement',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-fan-engagement' },
      });
