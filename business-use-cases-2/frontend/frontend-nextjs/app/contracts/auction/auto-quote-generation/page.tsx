...
export default function AutoQuoteGenerationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auto-quote-generation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auto-quote-generation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auto-quote-generation' },
      });
