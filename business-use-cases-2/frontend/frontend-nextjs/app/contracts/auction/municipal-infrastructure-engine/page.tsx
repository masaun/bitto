...
export default function MunicipalInfrastructureEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('municipal-infrastructure-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'municipal-infrastructure-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'municipal-infrastructure-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Municipal Infrastructure Engine</h1>
