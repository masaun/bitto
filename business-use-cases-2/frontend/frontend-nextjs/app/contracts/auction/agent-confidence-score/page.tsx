...
export default function AgentConfidenceScorePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-confidence-score');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-confidence-score',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-confidence-score' },
      });
