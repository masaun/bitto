...
export default function WebhookManagerStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('webhook-manager-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'webhook-manager-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'webhook-manager-staging' },
      });
