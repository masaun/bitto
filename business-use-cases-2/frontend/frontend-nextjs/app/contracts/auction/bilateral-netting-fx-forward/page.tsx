...
export default function BilateralNettingFxForwardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bilateral-netting-fx-forward');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bilateral-netting-fx-forward',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bilateral-netting-fx-forward' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Bilateral Netting Fx Forward</h1>
