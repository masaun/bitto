...
export default function RegionalAiPolicyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('regional-ai-policy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'regional-ai-policy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'regional-ai-policy' },
      });
