...
export default function BreweryLeasebackEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('brewery-leaseback-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'brewery-leaseback-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'brewery-leaseback-engine' },
      });
