...
export default function SocialImpactAssessmentPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('social-impact-assessment');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'social-impact-assessment',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'social-impact-assessment' },
      });
