...
export default function IdentityValidatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('identity-validator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'identity-validator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'identity-validator' },
      });
