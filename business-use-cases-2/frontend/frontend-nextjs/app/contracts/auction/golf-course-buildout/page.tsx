...
export default function GolfCourseBuildoutPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('golf-course-buildout');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'golf-course-buildout',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'golf-course-buildout' },
      });
