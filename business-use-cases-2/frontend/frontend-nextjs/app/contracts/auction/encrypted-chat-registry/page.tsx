...
export default function EncryptedChatRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-chat-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-chat-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-chat-registry' },
      });
