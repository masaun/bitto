...
export default function DocumentProcessorEmirPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('document-processor-emir');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'document-processor-emir',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'document-processor-emir' },
      });
