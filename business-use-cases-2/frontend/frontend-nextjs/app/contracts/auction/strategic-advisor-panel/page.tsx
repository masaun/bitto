...
export default function StrategicAdvisorPanelPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('strategic-advisor-panel');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'strategic-advisor-panel',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'strategic-advisor-panel' },
      });
