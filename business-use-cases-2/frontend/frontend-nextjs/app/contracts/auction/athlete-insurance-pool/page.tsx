...
export default function AthleteInsurancePoolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('athlete-insurance-pool');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'athlete-insurance-pool',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'athlete-insurance-pool' },
      });
