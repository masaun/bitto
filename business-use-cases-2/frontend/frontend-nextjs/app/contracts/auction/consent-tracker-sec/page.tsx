...
export default function ConsentTrackerSecPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('consent-tracker-sec');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'consent-tracker-sec',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'consent-tracker-sec' },
      });
