...
export default function AuctionDispatcherPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-dispatcher');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-dispatcher',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-dispatcher' },
      });
