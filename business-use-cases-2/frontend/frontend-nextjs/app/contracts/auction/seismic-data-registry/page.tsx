...
export default function SeismicDataRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('seismic-data-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'seismic-data-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'seismic-data-registry' },
      });
