...
export default function PromptLeakagePreventionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('prompt-leakage-prevention');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'prompt-leakage-prevention',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'prompt-leakage-prevention' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Prompt Leakage Prevention</h1>
