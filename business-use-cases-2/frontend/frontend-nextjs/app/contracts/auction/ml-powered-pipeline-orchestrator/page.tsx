...
export default function MlPoweredPipelineOrchestratorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ml-powered-pipeline-orchestrator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ml-powered-pipeline-orchestrator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ml-powered-pipeline-orchestrator' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Ml Powered Pipeline Orchestrator</h1>
