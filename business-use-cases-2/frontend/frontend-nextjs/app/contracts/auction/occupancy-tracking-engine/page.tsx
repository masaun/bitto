...
export default function OccupancyTrackingEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('occupancy-tracking-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'occupancy-tracking-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'occupancy-tracking-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Occupancy Tracking Engine</h1>
