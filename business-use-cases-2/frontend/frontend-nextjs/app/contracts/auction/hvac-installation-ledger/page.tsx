...
export default function HvacInstallationLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('hvac-installation-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'hvac-installation-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'hvac-installation-ledger' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Hvac Installation Ledger</h1>
