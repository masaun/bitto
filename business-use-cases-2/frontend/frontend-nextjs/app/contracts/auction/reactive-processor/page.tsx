...
export default function ReactiveProcessorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('reactive-processor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'reactive-processor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'reactive-processor' },
      });
