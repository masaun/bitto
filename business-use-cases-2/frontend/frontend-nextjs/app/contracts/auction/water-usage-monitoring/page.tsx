...
export default function WaterUsageMonitoringPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('water-usage-monitoring');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'water-usage-monitoring',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'water-usage-monitoring' },
      });
