...
export default function ContinuousAuctionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('continuous-auction');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'continuous-auction',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'continuous-auction' },
      });
