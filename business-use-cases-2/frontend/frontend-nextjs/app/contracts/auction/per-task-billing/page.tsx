...
export default function PerTaskBillingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('per-task-billing');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'per-task-billing',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'per-task-billing' },
      });
