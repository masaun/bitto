...
export default function DelayedQueueManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('delayed-queue-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'delayed-queue-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'delayed-queue-manager' },
      });
