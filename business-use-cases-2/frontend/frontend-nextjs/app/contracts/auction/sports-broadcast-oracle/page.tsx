...
export default function SportsBroadcastOraclePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-broadcast-oracle');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-broadcast-oracle',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-broadcast-oracle' },
      });
