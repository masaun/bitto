...
export default function SportsStadiumBuilderPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-stadium-builder');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-stadium-builder',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-stadium-builder' },
      });
