...
export default function MevAnalyticsDashboardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('mev-analytics-dashboard');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'mev-analytics-dashboard',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'mev-analytics-dashboard' },
      });
