...
export default function MlPoweredRuleEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ml-powered-rule-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ml-powered-rule-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ml-powered-rule-engine' },
      });
