...
export default function TunnelExcavationEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tunnel-excavation-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tunnel-excavation-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tunnel-excavation-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Tunnel Excavation Engine</h1>
