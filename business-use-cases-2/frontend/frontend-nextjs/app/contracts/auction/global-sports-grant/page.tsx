...
export default function GlobalSportsGrantPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('global-sports-grant');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'global-sports-grant',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'global-sports-grant' },
      });
