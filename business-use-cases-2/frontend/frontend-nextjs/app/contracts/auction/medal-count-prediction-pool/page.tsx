...
export default function MedalCountPredictionPoolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('medal-count-prediction-pool');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'medal-count-prediction-pool',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'medal-count-prediction-pool' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Medal Count Prediction Pool</h1>
