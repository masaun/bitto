...
export default function PrivatePriceOraclePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('private-price-oracle');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'private-price-oracle',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'private-price-oracle' },
      });
