...
export default function AstroproofRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('astroproof-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'astroproof-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'astroproof-registry' },
      });
