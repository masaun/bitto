...
export default function AiOperationsOptimizerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-operations-optimizer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-operations-optimizer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-operations-optimizer' },
      });
