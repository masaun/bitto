...
export default function ShiftSchedulingSystemPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('shift-scheduling-system');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'shift-scheduling-system',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'shift-scheduling-system' },
      });
