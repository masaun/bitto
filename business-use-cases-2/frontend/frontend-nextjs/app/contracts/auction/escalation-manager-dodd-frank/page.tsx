...
export default function EscalationManagerDoddFrankPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('escalation-manager-dodd-frank');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'escalation-manager-dodd-frank',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'escalation-manager-dodd-frank' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Escalation Manager Dodd Frank</h1>
