...
export default function DocumentProcessorRiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('document-processor-ria');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'document-processor-ria',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'document-processor-ria' },
      });
