...
export default function RareEarthSpotMarketPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rare-earth-spot-market');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rare-earth-spot-market',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rare-earth-spot-market' },
      });
