...
export default function GlobalAthleticsMarketPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('global-athletics-market');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'global-athletics-market',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'global-athletics-market' },
      });
