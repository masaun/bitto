...
export default function AstropoolLiquidityPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('astropool-liquidity');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'astropool-liquidity',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'astropool-liquidity' },
      });
