...
export default function PromptExpiryPolicyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('prompt-expiry-policy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'prompt-expiry-policy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'prompt-expiry-policy' },
      });
