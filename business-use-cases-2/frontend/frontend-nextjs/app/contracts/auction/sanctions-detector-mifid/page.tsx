...
export default function SanctionsDetectorMifidPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sanctions-detector-mifid');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sanctions-detector-mifid',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sanctions-detector-mifid' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Sanctions Detector Mifid</h1>
