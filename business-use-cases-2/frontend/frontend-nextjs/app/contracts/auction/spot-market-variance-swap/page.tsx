...
export default function SpotMarketVarianceSwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('spot-market-variance-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'spot-market-variance-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'spot-market-variance-swap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Spot Market Variance Swap</h1>
