...
export default function TransactionDecryptionCoordinatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('transaction-decryption-coordinator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'transaction-decryption-coordinator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'transaction-decryption-coordinator' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Transaction Decryption Coordinator</h1>
