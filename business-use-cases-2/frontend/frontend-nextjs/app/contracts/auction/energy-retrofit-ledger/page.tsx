...
export default function EnergyRetrofitLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('energy-retrofit-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'energy-retrofit-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'energy-retrofit-ledger' },
      });
