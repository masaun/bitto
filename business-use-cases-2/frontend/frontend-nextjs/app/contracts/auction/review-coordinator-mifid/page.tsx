...
export default function ReviewCoordinatorMifidPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('review-coordinator-mifid');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'review-coordinator-mifid',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'review-coordinator-mifid' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Review Coordinator Mifid</h1>
