...
export default function ModelDriftDetectionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('model-drift-detection');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'model-drift-detection',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'model-drift-detection' },
      });
