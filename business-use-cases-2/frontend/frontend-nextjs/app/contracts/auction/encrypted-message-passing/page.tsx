...
export default function EncryptedMessagePassingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-message-passing');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-message-passing',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-message-passing' },
      });
