...
export default function CustodyChainTrackingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('custody-chain-tracking');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'custody-chain-tracking',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'custody-chain-tracking' },
      });
