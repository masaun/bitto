...
export default function DataDrivenRuleEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('data-driven-rule-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'data-driven-rule-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'data-driven-rule-engine' },
      });
