...
export default function ArtisanFurnishingsLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('artisan-furnishings-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'artisan-furnishings-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'artisan-furnishings-ledger' },
      });
