...
export default function BiotechEquipmentLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('biotech-equipment-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'biotech-equipment-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'biotech-equipment-leaseback' },
      });
