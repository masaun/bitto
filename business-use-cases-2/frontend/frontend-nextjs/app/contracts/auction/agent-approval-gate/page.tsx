...
export default function AgentApprovalGatePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-approval-gate');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-approval-gate',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-approval-gate' },
      });
