...
export default function AirQualityMeasurementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('air-quality-measurement');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'air-quality-measurement',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'air-quality-measurement' },
      });
