...
export default function EncryptedAssetRoutingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-asset-routing');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-asset-routing',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-asset-routing' },
      });
