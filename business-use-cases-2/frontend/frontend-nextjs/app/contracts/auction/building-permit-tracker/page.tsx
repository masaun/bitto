...
export default function BuildingPermitTrackerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('building-permit-tracker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'building-permit-tracker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'building-permit-tracker' },
      });
