...
export default function AgentAlignmentCheckPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-alignment-check');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-alignment-check',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-alignment-check' },
      });
