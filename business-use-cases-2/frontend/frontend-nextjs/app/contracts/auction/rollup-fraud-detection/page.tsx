...
export default function RollupFraudDetectionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rollup-fraud-detection');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rollup-fraud-detection',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rollup-fraud-detection' },
      });
