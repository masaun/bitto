...
export default function LocalDisputeResolverPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-dispute-resolver');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-dispute-resolver',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-dispute-resolver' },
      });
