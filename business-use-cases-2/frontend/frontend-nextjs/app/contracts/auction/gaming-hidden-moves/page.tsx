...
export default function GamingHiddenMovesPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('gaming-hidden-moves');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'gaming-hidden-moves',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'gaming-hidden-moves' },
      });
