...
export default function ShieldedTokenWrapperPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('shielded-token-wrapper');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'shielded-token-wrapper',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'shielded-token-wrapper' },
      });
