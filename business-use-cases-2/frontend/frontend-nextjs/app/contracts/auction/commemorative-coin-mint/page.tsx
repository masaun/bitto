...
export default function CommemorativeCoinMintPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('commemorative-coin-mint');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'commemorative-coin-mint',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'commemorative-coin-mint' },
      });
