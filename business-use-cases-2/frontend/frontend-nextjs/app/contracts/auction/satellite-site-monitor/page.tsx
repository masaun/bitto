...
export default function SatelliteSiteMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('satellite-site-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'satellite-site-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'satellite-site-monitor' },
      });
