...
export default function SecureContractExecutionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('secure-contract-execution');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'secure-contract-execution',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'secure-contract-execution' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Secure Contract Execution</h1>
