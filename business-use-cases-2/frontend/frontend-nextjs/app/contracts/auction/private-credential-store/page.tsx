...
export default function PrivateCredentialStorePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('private-credential-store');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'private-credential-store',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'private-credential-store' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Private Credential Store</h1>
