...
export default function RecurringQueueManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('recurring-queue-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'recurring-queue-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'recurring-queue-manager' },
      });
