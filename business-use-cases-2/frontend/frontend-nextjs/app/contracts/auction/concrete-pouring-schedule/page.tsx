...
export default function ConcretePouringSchedulePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('concrete-pouring-schedule');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'concrete-pouring-schedule',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'concrete-pouring-schedule' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Concrete Pouring Schedule</h1>
