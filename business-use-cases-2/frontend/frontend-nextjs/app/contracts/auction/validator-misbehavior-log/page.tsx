...
export default function ValidatorMisbehaviorLogPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('validator-misbehavior-log');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'validator-misbehavior-log',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'validator-misbehavior-log' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Validator Misbehavior Log</h1>
