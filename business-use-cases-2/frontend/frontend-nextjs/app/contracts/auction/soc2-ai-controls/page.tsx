...
export default function Soc2AiControlsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('soc2-ai-controls');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'soc2-ai-controls',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'soc2-ai-controls' },
      });
