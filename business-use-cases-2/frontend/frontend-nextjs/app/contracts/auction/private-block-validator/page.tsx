...
export default function PrivateBlockValidatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('private-block-validator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'private-block-validator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'private-block-validator' },
      });
