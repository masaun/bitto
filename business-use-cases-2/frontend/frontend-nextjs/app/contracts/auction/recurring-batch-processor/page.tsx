...
export default function RecurringBatchProcessorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('recurring-batch-processor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'recurring-batch-processor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'recurring-batch-processor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Recurring Batch Processor</h1>
