...
export default function StrategicVoteEscrowPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('strategic-vote-escrow');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'strategic-vote-escrow',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'strategic-vote-escrow' },
      });
