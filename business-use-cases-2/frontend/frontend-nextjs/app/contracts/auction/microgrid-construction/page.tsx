...
export default function MicrogridConstructionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('microgrid-construction');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'microgrid-construction',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'microgrid-construction' },
      });
