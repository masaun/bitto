...
export default function InstitutionalDistributionEngine5Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('institutional-distribution-engine-5');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'institutional-distribution-engine-5',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'institutional-distribution-engine-5' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Institutional Distribution Engine 5</h1>
