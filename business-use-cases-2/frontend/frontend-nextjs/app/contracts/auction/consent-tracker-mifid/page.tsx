...
export default function ConsentTrackerMifidPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('consent-tracker-mifid');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'consent-tracker-mifid',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'consent-tracker-mifid' },
      });
