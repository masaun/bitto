...
export default function ProjectStateMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('project-state-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'project-state-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'project-state-monitor' },
      });
