...
export default function BridgeSlashingPolicyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bridge-slashing-policy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bridge-slashing-policy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bridge-slashing-policy' },
      });
