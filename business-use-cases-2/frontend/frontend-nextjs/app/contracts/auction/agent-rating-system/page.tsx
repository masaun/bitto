...
export default function AgentRatingSystemPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-rating-system');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-rating-system',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-rating-system' },
      });
