...
export default function DataProtectorPciDssPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('data-protector-pci-dss');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'data-protector-pci-dss',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'data-protector-pci-dss' },
      });
