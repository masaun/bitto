...
export default function TimeDrivenProcessMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('time-driven-process-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'time-driven-process-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'time-driven-process-monitor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Time Driven Process Monitor</h1>
