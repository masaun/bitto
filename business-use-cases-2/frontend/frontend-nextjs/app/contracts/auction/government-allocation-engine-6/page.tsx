...
export default function GovernmentAllocationEngine6Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('government-allocation-engine-6');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'government-allocation-engine-6',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'government-allocation-engine-6' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Government Allocation Engine 6</h1>
