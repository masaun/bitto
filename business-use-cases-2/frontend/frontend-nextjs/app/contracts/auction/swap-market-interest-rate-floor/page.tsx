...
export default function SwapMarketInterestRateFloorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('swap-market-interest-rate-floor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'swap-market-interest-rate-floor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'swap-market-interest-rate-floor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Swap Market Interest Rate Floor</h1>
