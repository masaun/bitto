...
export default function BaseballFinalPrizePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('baseball-final-prize');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'baseball-final-prize',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'baseball-final-prize' },
      });
