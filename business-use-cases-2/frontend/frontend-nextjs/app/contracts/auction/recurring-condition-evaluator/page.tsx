...
export default function RecurringConditionEvaluatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('recurring-condition-evaluator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'recurring-condition-evaluator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'recurring-condition-evaluator' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Recurring Condition Evaluator</h1>
