...
export default function HostCityBidBondPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('host-city-bid-bond');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'host-city-bid-bond',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'host-city-bid-bond' },
      });
