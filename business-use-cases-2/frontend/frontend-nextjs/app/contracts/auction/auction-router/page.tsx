...
export default function AuctionRouterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-router');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-router',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-router' },
      });
