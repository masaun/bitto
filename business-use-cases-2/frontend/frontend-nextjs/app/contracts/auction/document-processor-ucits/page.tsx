...
export default function DocumentProcessorUcitsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('document-processor-ucits');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'document-processor-ucits',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'document-processor-ucits' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Document Processor Ucits</h1>
