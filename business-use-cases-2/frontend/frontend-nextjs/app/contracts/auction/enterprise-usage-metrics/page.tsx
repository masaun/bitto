...
export default function EnterpriseUsageMetricsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('enterprise-usage-metrics');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'enterprise-usage-metrics',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'enterprise-usage-metrics' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Enterprise Usage Metrics</h1>
