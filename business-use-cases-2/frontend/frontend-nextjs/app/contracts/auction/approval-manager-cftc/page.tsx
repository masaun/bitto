...
export default function ApprovalManagerCftcPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('approval-manager-cftc');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'approval-manager-cftc',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'approval-manager-cftc' },
      });
