...
export default function DataProtectorHipaaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('data-protector-hipaa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'data-protector-hipaa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'data-protector-hipaa' },
      });
