...
export default function EncryptedBlockStoragePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-block-storage');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-block-storage',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-block-storage' },
      });
