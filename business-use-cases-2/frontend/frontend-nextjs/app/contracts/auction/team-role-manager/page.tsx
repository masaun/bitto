...
export default function TeamRoleManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-role-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-role-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-role-manager' },
      });
