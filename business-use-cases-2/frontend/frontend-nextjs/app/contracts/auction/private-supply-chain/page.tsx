...
export default function PrivateSupplyChainPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('private-supply-chain');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'private-supply-chain',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'private-supply-chain' },
      });
