...
export default function SimulationApprovalPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('simulation-approval');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'simulation-approval',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'simulation-approval' },
      });
