...
export default function PlayerBonusEscrowPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('player-bonus-escrow');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'player-bonus-escrow',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'player-bonus-escrow' },
      });
