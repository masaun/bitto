...
export default function EncryptedEventStreamPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-event-stream');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-event-stream',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-event-stream' },
      });
