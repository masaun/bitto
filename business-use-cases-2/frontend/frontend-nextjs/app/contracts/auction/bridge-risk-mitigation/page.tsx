...
export default function BridgeRiskMitigationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bridge-risk-mitigation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bridge-risk-mitigation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bridge-risk-mitigation' },
      });
