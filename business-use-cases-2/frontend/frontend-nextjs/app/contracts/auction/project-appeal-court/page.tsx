...
export default function ProjectAppealCourtPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('project-appeal-court');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'project-appeal-court',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'project-appeal-court' },
      });
