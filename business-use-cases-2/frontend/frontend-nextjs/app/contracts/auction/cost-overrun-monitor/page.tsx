...
export default function CostOverrunMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('cost-overrun-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'cost-overrun-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'cost-overrun-monitor' },
      });
