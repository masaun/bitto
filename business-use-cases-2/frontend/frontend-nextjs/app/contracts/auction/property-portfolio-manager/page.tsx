...
export default function PropertyPortfolioManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('property-portfolio-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'property-portfolio-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'property-portfolio-manager' },
      });
