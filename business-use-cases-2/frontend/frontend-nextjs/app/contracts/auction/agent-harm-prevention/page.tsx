...
export default function AgentHarmPreventionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-harm-prevention');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-harm-prevention',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-harm-prevention' },
      });
