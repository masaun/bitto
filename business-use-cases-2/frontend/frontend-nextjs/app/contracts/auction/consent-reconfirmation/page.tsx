...
export default function ConsentReconfirmationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('consent-reconfirmation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'consent-reconfirmation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'consent-reconfirmation' },
      });
