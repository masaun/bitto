...
export default function BondGuaranteeLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bond-guarantee-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bond-guarantee-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bond-guarantee-ledger' },
      });
