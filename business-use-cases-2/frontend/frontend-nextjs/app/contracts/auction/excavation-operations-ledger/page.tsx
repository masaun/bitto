...
export default function ExcavationOperationsLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('excavation-operations-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'excavation-operations-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'excavation-operations-ledger' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Excavation Operations Ledger</h1>
