...
export default function LandReclamationEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('land-reclamation-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'land-reclamation-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'land-reclamation-engine' },
      });
