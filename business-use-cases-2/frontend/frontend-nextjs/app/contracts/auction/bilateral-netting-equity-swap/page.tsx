...
export default function BilateralNettingEquitySwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bilateral-netting-equity-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bilateral-netting-equity-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bilateral-netting-equity-swap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Bilateral Netting Equity Swap</h1>
