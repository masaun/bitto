...
export default function AuctionConnectorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-connector');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-connector',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-connector' },
      });
