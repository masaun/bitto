...
export default function SportsTicketAuctionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-ticket-auction');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-ticket-auction',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-ticket-auction' },
      });
