...
export default function IncidentContainmentPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('incident-containment');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'incident-containment',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'incident-containment' },
      });
