...
export default function UtilityGridExpansionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('utility-grid-expansion');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'utility-grid-expansion',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'utility-grid-expansion' },
      });
