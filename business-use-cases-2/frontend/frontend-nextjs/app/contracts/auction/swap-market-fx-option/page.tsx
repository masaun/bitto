...
export default function SwapMarketFxOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('swap-market-fx-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'swap-market-fx-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'swap-market-fx-option' },
      });
