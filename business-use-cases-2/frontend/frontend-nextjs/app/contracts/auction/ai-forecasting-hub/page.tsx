...
export default function AiForecastingHubPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-forecasting-hub');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-forecasting-hub',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-forecasting-hub' },
      });
