...
export default function AutonomousEnterpriseAgentPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('autonomous-enterprise-agent');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'autonomous-enterprise-agent',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'autonomous-enterprise-agent' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Autonomous Enterprise Agent</h1>
