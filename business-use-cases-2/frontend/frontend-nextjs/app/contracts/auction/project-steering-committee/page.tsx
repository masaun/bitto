...
export default function ProjectSteeringCommitteePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('project-steering-committee');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'project-steering-committee',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'project-steering-committee' },
      });
