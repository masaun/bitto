...
export default function AiSequencingOptimizerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-sequencing-optimizer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-sequencing-optimizer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-sequencing-optimizer' },
      });
