...
export default function ThresholdBasedPipelineOrchestratorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('threshold-based-pipeline-orchestrator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'threshold-based-pipeline-orchestrator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'threshold-based-pipeline-orchestrator' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Threshold Based Pipeline Orchestrator</h1>
