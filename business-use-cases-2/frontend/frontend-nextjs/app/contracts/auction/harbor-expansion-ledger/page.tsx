...
export default function HarborExpansionLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('harbor-expansion-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'harbor-expansion-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'harbor-expansion-ledger' },
      });
