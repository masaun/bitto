...
export default function ApprovalManagerRiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('approval-manager-ria');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'approval-manager-ria',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'approval-manager-ria' },
      });
