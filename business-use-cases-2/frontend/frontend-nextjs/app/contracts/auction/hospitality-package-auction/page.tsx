...
export default function HospitalityPackageAuctionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('hospitality-package-auction');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'hospitality-package-auction',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'hospitality-package-auction' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Hospitality Package Auction</h1>
