...
export default function IncentiveDistributionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('incentive-distribution');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'incentive-distribution',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'incentive-distribution' },
      });
