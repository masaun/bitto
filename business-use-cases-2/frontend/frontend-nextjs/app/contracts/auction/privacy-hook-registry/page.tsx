...
export default function PrivacyHookRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-hook-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-hook-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-hook-registry' },
      });
