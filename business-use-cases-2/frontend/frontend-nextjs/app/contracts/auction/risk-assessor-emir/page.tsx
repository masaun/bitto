...
export default function RiskAssessorEmirPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('risk-assessor-emir');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'risk-assessor-emir',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'risk-assessor-emir' },
      });
