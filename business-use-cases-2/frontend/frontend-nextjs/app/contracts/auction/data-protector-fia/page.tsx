...
export default function DataProtectorFiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('data-protector-fia');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'data-protector-fia',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'data-protector-fia' },
      });
