...
export default function VoteRevealManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('vote-reveal-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'vote-reveal-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'vote-reveal-manager' },
      });
