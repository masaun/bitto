...
export default function AffordableHousingEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('affordable-housing-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'affordable-housing-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'affordable-housing-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Affordable Housing Engine</h1>
