...
export default function CrossBorderBroadcastAuctionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('cross-border-broadcast-auction');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'cross-border-broadcast-auction',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'cross-border-broadcast-auction' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Cross Border Broadcast Auction</h1>
