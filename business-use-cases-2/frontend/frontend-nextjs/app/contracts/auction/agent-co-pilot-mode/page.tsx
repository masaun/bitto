...
export default function AgentCoPilotModePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-co-pilot-mode');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-co-pilot-mode',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-co-pilot-mode' },
      });
