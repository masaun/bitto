...
export default function AgentDeprecationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-deprecation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-deprecation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-deprecation' },
      });
