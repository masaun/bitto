...
export default function IdentityValidatorRiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('identity-validator-ria');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'identity-validator-ria',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'identity-validator-ria' },
      });
