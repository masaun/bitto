...
export default function SanctionsDetectorSecPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sanctions-detector-sec');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sanctions-detector-sec',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sanctions-detector-sec' },
      });
