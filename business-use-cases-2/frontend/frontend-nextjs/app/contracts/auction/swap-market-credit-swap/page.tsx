...
export default function SwapMarketCreditSwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('swap-market-credit-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'swap-market-credit-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'swap-market-credit-swap' },
      });
