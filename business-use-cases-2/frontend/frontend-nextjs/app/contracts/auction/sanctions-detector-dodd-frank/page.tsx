...
export default function SanctionsDetectorDoddFrankPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sanctions-detector-dodd-frank');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sanctions-detector-dodd-frank',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sanctions-detector-dodd-frank' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Sanctions Detector Dodd Frank</h1>
