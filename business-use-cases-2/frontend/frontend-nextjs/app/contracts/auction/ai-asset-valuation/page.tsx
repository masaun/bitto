...
export default function AiAssetValuationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-asset-valuation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-asset-valuation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-asset-valuation' },
      });
