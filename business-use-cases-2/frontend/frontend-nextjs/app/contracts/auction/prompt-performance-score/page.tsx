...
export default function PromptPerformanceScorePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('prompt-performance-score');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'prompt-performance-score',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'prompt-performance-score' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Prompt Performance Score</h1>
