...
export default function StrategicRoleManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('strategic-role-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'strategic-role-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'strategic-role-manager' },
      });
