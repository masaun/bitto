...
export default function LiftingPlanApprovalPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('lifting-plan-approval');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'lifting-plan-approval',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'lifting-plan-approval' },
      });
