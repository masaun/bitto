...
export default function KnowledgeAccessControlPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('knowledge-access-control');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'knowledge-access-control',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'knowledge-access-control' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Knowledge Access Control</h1>
