...
export default function MevRiskScorePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('mev-risk-score');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'mev-risk-score',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'mev-risk-score' },
      });
