...
export default function PoolingSystemPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('pooling-system');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'pooling-system',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'pooling-system' },
      });
