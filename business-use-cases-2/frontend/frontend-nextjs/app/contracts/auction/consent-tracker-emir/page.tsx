...
export default function ConsentTrackerEmirPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('consent-tracker-emir');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'consent-tracker-emir',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'consent-tracker-emir' },
      });
