...
export default function DisputeResolutionEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('dispute-resolution-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'dispute-resolution-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'dispute-resolution-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Dispute Resolution Engine</h1>
