...
export default function PolicyManagerCcpaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('policy-manager-ccpa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'policy-manager-ccpa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'policy-manager-ccpa' },
      });
