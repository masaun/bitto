...
export default function EmergencyUpgradePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('emergency-upgrade');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'emergency-upgrade',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'emergency-upgrade' },
      });
