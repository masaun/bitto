...
export default function PrivacyEnforcerFiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-enforcer-fia');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-enforcer-fia',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-enforcer-fia' },
      });
