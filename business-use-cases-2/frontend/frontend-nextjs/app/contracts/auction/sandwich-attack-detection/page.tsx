...
export default function SandwichAttackDetectionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sandwich-attack-detection');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sandwich-attack-detection',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sandwich-attack-detection' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Sandwich Attack Detection</h1>
