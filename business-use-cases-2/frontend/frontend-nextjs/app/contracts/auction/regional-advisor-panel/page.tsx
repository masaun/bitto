...
export default function RegionalAdvisorPanelPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('regional-advisor-panel');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'regional-advisor-panel',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'regional-advisor-panel' },
      });
