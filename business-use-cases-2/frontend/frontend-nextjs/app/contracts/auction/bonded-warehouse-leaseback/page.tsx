...
export default function BondedWarehouseLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bonded-warehouse-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bonded-warehouse-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bonded-warehouse-leaseback' },
      });
