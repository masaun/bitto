...
export default function AutoDisputeRoutingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auto-dispute-routing');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auto-dispute-routing',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auto-dispute-routing' },
      });
