...
export default function AgentTaskOrchestratorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-task-orchestrator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-task-orchestrator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-task-orchestrator' },
      });
