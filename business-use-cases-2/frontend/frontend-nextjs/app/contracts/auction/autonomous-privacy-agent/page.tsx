...
export default function AutonomousPrivacyAgentPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('autonomous-privacy-agent');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'autonomous-privacy-agent',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'autonomous-privacy-agent' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Autonomous Privacy Agent</h1>
