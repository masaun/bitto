...
export default function DataDrivenActionExecutorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('data-driven-action-executor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'data-driven-action-executor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'data-driven-action-executor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Data Driven Action Executor</h1>
