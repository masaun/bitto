...
export default function DocumentProcessorSoxPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('document-processor-sox');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'document-processor-sox',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'document-processor-sox' },
      });
