...
export default function KnowledgeLeakPreventionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('knowledge-leak-prevention');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'knowledge-leak-prevention',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'knowledge-leak-prevention' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Knowledge Leak Prevention</h1>
