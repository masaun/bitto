...
export default function PipelineInstallationLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('pipeline-installation-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'pipeline-installation-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'pipeline-installation-ledger' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Pipeline Installation Ledger</h1>
