...
export default function RecurringCronExecutorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('recurring-cron-executor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'recurring-cron-executor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'recurring-cron-executor' },
      });
