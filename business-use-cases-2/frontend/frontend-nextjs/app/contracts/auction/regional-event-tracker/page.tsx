...
export default function RegionalEventTrackerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('regional-event-tracker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'regional-event-tracker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'regional-event-tracker' },
      });
