...
export default function GlobalScoutingBountyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('global-scouting-bounty');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'global-scouting-bounty',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'global-scouting-bounty' },
      });
