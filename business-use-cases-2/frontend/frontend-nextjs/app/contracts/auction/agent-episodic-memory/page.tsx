...
export default function AgentEpisodicMemoryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-episodic-memory');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-episodic-memory',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-episodic-memory' },
      });
