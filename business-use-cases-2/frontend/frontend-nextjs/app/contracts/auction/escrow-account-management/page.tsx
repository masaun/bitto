...
export default function EscrowAccountManagementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('escrow-account-management');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'escrow-account-management',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'escrow-account-management' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Escrow Account Management</h1>
