...
export default function AgentDataValidationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-data-validation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-data-validation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-data-validation' },
      });
