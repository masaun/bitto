...
export default function InternationalBroadcastTokenPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('international-broadcast-token');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'international-broadcast-token',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'international-broadcast-token' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>International Broadcast Token</h1>
