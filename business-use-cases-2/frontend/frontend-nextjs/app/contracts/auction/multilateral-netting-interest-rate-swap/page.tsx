...
export default function MultilateralNettingInterestRateSwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('multilateral-netting-interest-rate-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'multilateral-netting-interest-rate-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'multilateral-netting-interest-rate-swap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Multilateral Netting Interest Rate Swap</h1>
