...
export default function FairBlockConstructionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('fair-block-construction');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'fair-block-construction',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'fair-block-construction' },
      });
