...
export default function AccountabilityTrackingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('accountability-tracking');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'accountability-tracking',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'accountability-tracking' },
      });
