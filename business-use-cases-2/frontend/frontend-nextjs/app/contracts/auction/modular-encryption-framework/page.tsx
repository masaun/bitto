...
export default function ModularEncryptionFrameworkPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('modular-encryption-framework');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'modular-encryption-framework',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'modular-encryption-framework' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Modular Encryption Framework</h1>
