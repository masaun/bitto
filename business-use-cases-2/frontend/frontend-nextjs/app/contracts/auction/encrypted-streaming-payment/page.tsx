...
export default function EncryptedStreamingPaymentPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-streaming-payment');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-streaming-payment',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-streaming-payment' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Encrypted Streaming Payment</h1>
