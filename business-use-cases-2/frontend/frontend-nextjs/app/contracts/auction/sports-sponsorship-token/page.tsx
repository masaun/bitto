...
export default function SportsSponsorshipTokenPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-sponsorship-token');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-sponsorship-token',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-sponsorship-token' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Sports Sponsorship Token</h1>
