...
export default function AgentFallbackHandlerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-fallback-handler');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-fallback-handler',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-fallback-handler' },
      });
