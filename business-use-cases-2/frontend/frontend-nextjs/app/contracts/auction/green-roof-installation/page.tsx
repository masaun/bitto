...
export default function GreenRoofInstallationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('green-roof-installation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'green-roof-installation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'green-roof-installation' },
      });
