...
export default function HiddenLootDistributionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('hidden-loot-distribution');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'hidden-loot-distribution',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'hidden-loot-distribution' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Hidden Loot Distribution</h1>
