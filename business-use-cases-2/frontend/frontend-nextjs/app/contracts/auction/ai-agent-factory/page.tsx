...
export default function AiAgentFactoryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-agent-factory');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-agent-factory',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-agent-factory' },
      });
