...
export default function CrossBorderDataSharingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('cross-border-data-sharing');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'cross-border-data-sharing',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'cross-border-data-sharing' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Cross Border Data Sharing</h1>
