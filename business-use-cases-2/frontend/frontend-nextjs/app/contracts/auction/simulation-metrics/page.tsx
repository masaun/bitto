...
export default function SimulationMetricsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('simulation-metrics');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'simulation-metrics',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'simulation-metrics' },
      });
