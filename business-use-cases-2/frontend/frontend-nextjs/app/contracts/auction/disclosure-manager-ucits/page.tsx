...
export default function DisclosureManagerUcitsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('disclosure-manager-ucits');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'disclosure-manager-ucits',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'disclosure-manager-ucits' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Disclosure Manager Ucits</h1>
