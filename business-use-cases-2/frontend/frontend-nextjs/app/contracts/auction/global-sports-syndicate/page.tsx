...
export default function GlobalSportsSyndicatePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('global-sports-syndicate');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'global-sports-syndicate',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'global-sports-syndicate' },
      });
