...
export default function AnomalyDetectionRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('anomaly-detection-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'anomaly-detection-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'anomaly-detection-registry' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Anomaly Detection Registry</h1>
