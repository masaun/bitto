...
export default function RiskAssessorRiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('risk-assessor-ria');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'risk-assessor-ria',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'risk-assessor-ria' },
      });
