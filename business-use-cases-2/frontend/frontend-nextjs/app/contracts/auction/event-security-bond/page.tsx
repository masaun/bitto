...
export default function EventSecurityBondPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-security-bond');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-security-bond',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-security-bond' },
      });
