...
export default function RuleBasedEventHandlerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rule-based-event-handler');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rule-based-event-handler',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rule-based-event-handler' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Rule Based Event Handler</h1>
