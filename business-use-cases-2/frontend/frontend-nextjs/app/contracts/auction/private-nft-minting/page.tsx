...
export default function PrivateNftMintingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('private-nft-minting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'private-nft-minting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'private-nft-minting' },
      });
