...
export default function TransactionSequencingPolicyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('transaction-sequencing-policy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'transaction-sequencing-policy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'transaction-sequencing-policy' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Transaction Sequencing Policy</h1>
