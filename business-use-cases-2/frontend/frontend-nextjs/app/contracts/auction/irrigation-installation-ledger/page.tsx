...
export default function IrrigationInstallationLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('irrigation-installation-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'irrigation-installation-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'irrigation-installation-ledger' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Irrigation Installation Ledger</h1>
