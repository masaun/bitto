...
export default function AiServerLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-server-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-server-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-server-leaseback' },
      });
