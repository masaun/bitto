...
export default function BaseballKnockoutStageRewardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('baseball-knockout-stage-reward');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'baseball-knockout-stage-reward',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'baseball-knockout-stage-reward' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Baseball Knockout Stage Reward</h1>
