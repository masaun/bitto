...
export default function SwapMarketVarianceSwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('swap-market-variance-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'swap-market-variance-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'swap-market-variance-swap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Swap Market Variance Swap</h1>
