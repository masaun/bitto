...
export default function AirportTerminalLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('airport-terminal-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'airport-terminal-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'airport-terminal-leaseback' },
      });
