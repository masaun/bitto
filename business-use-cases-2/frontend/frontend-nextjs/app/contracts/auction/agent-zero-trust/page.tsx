...
export default function AgentZeroTrustPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-zero-trust');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-zero-trust',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-zero-trust' },
      });
