...
export default function AstrobridgeLiquidityPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('astrobridge-liquidity');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'astrobridge-liquidity',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'astrobridge-liquidity' },
      });
