...
export default function PrivacyAlertSystemPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-alert-system');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-alert-system',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-alert-system' },
      });
