...
export default function ForwardMarketCommoditySwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('forward-market-commodity-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'forward-market-commodity-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'forward-market-commodity-swap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Forward Market Commodity Swap</h1>
