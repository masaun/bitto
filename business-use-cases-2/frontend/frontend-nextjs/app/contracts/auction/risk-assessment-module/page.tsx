...
export default function RiskAssessmentModulePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('risk-assessment-module');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'risk-assessment-module',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'risk-assessment-module' },
      });
