...
export default function AgentIpProtectionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-ip-protection');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-ip-protection',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-ip-protection' },
      });
