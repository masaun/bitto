...
export default function AgentVectorIndexPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-vector-index');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-vector-index',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-vector-index' },
      });
