...
export default function PipelineRehabilitationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('pipeline-rehabilitation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'pipeline-rehabilitation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'pipeline-rehabilitation' },
      });
