...
export default function DisputeWithholdingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('dispute-withholding');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'dispute-withholding',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'dispute-withholding' },
      });
