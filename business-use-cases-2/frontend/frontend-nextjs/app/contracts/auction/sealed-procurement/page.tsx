...
export default function SealedProcurementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sealed-procurement');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sealed-procurement',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sealed-procurement' },
      });
