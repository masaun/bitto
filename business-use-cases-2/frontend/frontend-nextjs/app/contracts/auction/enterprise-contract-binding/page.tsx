...
export default function EnterpriseContractBindingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('enterprise-contract-binding');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'enterprise-contract-binding',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'enterprise-contract-binding' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Enterprise Contract Binding</h1>
