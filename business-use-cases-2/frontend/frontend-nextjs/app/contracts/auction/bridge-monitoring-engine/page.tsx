...
export default function BridgeMonitoringEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bridge-monitoring-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bridge-monitoring-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bridge-monitoring-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Bridge Monitoring Engine</h1>
