...
export default function SealedBidEscrowPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sealed-bid-escrow');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sealed-bid-escrow',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sealed-bid-escrow' },
      });
