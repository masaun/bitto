...
export default function SecureBroadcastLayerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('secure-broadcast-layer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'secure-broadcast-layer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'secure-broadcast-layer' },
      });
