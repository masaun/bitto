...
export default function DelayedAsyncProcessorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('delayed-async-processor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'delayed-async-processor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'delayed-async-processor' },
      });
