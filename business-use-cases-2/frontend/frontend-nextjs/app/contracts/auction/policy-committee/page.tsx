...
export default function PolicyCommitteePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('policy-committee');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'policy-committee',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'policy-committee' },
      });
