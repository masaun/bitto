...
export default function GeotechnicalSurveyLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('geotechnical-survey-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'geotechnical-survey-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'geotechnical-survey-ledger' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Geotechnical Survey Ledger</h1>
