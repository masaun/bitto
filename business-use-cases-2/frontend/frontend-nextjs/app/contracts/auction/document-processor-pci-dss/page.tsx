...
export default function DocumentProcessorPciDssPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('document-processor-pci-dss');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'document-processor-pci-dss',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'document-processor-pci-dss' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Document Processor Pci Dss</h1>
