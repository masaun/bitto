...
export default function ForwardMarketEquityOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('forward-market-equity-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'forward-market-equity-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'forward-market-equity-option' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Forward Market Equity Option</h1>
