...
export default function AiClusterLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-cluster-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-cluster-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-cluster-leaseback' },
      });
