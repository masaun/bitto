...
export default function AiDrivenCronExecutorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-driven-cron-executor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-driven-cron-executor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-driven-cron-executor' },
      });
