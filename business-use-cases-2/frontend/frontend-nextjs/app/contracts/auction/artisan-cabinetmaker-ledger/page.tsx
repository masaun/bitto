...
export default function ArtisanCabinetmakerLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('artisan-cabinetmaker-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'artisan-cabinetmaker-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'artisan-cabinetmaker-ledger' },
      });
