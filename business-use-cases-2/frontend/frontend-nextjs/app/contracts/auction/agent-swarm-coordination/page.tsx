...
export default function AgentSwarmCoordinationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-swarm-coordination');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-swarm-coordination',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-swarm-coordination' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Agent Swarm Coordination</h1>
