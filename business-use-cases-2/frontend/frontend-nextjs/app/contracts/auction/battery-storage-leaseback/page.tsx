...
export default function BatteryStorageLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('battery-storage-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'battery-storage-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'battery-storage-leaseback' },
      });
