...
export default function AccessRightsManagementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('access-rights-management');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'access-rights-management',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'access-rights-management' },
      });
