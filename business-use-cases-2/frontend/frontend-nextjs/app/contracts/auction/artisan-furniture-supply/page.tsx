...
export default function ArtisanFurnitureSupplyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('artisan-furniture-supply');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'artisan-furniture-supply',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'artisan-furniture-supply' },
      });
