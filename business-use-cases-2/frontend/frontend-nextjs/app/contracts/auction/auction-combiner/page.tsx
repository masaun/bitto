...
export default function AuctionCombinerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-combiner');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-combiner',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-combiner' },
      });
