...
export default function SanctionsDetectorCcpaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sanctions-detector-ccpa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sanctions-detector-ccpa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sanctions-detector-ccpa' },
      });
