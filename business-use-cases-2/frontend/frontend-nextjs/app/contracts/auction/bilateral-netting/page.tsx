...
export default function BilateralNettingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bilateral-netting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bilateral-netting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bilateral-netting' },
      });
