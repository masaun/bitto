...
export default function ConsentTrackerPciDssPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('consent-tracker-pci-dss');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'consent-tracker-pci-dss',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'consent-tracker-pci-dss' },
      });
