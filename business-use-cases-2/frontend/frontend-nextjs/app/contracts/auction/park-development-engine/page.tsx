...
export default function ParkDevelopmentEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('park-development-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'park-development-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'park-development-engine' },
      });
