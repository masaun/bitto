...
export default function LocalEthicsCommitteePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-ethics-committee');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-ethics-committee',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-ethics-committee' },
      });
