...
export default function RecurringStateMachinePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('recurring-state-machine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'recurring-state-machine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'recurring-state-machine' },
      });
