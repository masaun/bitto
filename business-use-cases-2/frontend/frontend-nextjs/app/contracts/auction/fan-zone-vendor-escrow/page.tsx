...
export default function FanZoneVendorEscrowPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('fan-zone-vendor-escrow');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'fan-zone-vendor-escrow',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'fan-zone-vendor-escrow' },
      });
