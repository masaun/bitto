...
export default function EnterpriseCostAllocationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('enterprise-cost-allocation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'enterprise-cost-allocation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'enterprise-cost-allocation' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Enterprise Cost Allocation</h1>
