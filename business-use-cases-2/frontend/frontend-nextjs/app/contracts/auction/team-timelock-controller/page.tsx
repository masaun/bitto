...
export default function TeamTimelockControllerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-timelock-controller');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-timelock-controller',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-timelock-controller' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Team Timelock Controller</h1>
