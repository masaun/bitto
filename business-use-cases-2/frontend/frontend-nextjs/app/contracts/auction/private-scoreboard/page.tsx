...
export default function PrivateScoreboardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('private-scoreboard');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'private-scoreboard',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'private-scoreboard' },
      });
