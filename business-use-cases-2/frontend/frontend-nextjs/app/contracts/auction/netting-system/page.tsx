...
export default function NettingSystemPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('netting-system');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'netting-system',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'netting-system' },
      });
