...
export default function ArtisanFurnitureAuctionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('artisan-furniture-auction');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'artisan-furniture-auction',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'artisan-furniture-auction' },
      });
