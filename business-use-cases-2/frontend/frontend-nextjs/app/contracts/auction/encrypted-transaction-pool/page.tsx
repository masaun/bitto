...
export default function EncryptedTransactionPoolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-transaction-pool');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-transaction-pool',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-transaction-pool' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Encrypted Transaction Pool</h1>
