...
export default function TeamDisputeResolverPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-dispute-resolver');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-dispute-resolver',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-dispute-resolver' },
      });
