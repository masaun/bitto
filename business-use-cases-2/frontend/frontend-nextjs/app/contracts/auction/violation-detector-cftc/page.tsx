...
export default function ViolationDetectorCftcPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('violation-detector-cftc');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'violation-detector-cftc',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'violation-detector-cftc' },
      });
