...
export default function ReputationEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('reputation-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'reputation-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'reputation-engine' },
      });
