...
export default function ConditionalProcessMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('conditional-process-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'conditional-process-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'conditional-process-monitor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Conditional Process Monitor</h1>
