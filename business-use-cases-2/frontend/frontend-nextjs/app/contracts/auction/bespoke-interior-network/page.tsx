...
export default function BespokeInteriorNetworkPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bespoke-interior-network');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bespoke-interior-network',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bespoke-interior-network' },
      });
