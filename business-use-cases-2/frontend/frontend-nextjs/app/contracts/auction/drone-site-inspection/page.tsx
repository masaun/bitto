...
export default function DroneSiteInspectionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('drone-site-inspection');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'drone-site-inspection',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'drone-site-inspection' },
      });
