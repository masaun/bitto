...
export default function HazardReportingEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('hazard-reporting-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'hazard-reporting-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'hazard-reporting-engine' },
      });
