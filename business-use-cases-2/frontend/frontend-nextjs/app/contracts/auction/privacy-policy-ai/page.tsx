...
export default function PrivacyPolicyAiPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-policy-ai');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-policy-ai',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-policy-ai' },
      });
