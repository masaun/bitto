...
export default function EliteSponsorshipTierPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('elite-sponsorship-tier');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'elite-sponsorship-tier',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'elite-sponsorship-tier' },
      });
