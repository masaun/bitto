...
export default function UrbanPlanningProtocolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('urban-planning-protocol');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'urban-planning-protocol',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'urban-planning-protocol' },
      });
