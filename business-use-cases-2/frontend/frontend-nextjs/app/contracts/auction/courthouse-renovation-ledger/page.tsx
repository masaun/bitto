...
export default function CourthouseRenovationLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('courthouse-renovation-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'courthouse-renovation-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'courthouse-renovation-ledger' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Courthouse Renovation Ledger</h1>
