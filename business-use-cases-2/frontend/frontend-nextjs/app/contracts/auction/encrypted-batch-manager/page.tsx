...
export default function EncryptedBatchManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-batch-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-batch-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-batch-manager' },
      });
