...
export default function ApprovalManagerFiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('approval-manager-fia');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'approval-manager-fia',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'approval-manager-fia' },
      });
