...
export default function ComputeResourceRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compute-resource-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compute-resource-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compute-resource-registry' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Compute Resource Registry</h1>
