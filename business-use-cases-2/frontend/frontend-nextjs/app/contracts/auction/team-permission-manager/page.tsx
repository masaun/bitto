...
export default function TeamPermissionManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-permission-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-permission-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-permission-manager' },
      });
