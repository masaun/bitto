...
export default function ConsentTrackerHipaaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('consent-tracker-hipaa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'consent-tracker-hipaa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'consent-tracker-hipaa' },
      });
