...
export default function ConsentTrackerCftcPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('consent-tracker-cftc');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'consent-tracker-cftc',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'consent-tracker-cftc' },
      });
