...
export default function EventDrivenFlowControllerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-driven-flow-controller');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-driven-flow-controller',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-driven-flow-controller' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Event Driven Flow Controller</h1>
