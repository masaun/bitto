...
export default function DataProtectorRiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('data-protector-ria');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'data-protector-ria',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'data-protector-ria' },
      });
