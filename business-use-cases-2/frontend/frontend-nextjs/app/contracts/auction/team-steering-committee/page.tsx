...
export default function TeamSteeringCommitteePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-steering-committee');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-steering-committee',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-steering-committee' },
      });
