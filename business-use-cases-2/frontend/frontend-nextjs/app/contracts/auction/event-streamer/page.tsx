...
export default function EventStreamerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-streamer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-streamer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-streamer' },
      });
