...
export default function WebhookManagerLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('webhook-manager-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'webhook-manager-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'webhook-manager-legacy' },
      });
