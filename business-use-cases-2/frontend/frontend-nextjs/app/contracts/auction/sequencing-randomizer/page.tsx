...
export default function SequencingRandomizerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sequencing-randomizer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sequencing-randomizer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sequencing-randomizer' },
      });
