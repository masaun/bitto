...
export default function AthleteBrandCollaborationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('athlete-brand-collaboration');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'athlete-brand-collaboration',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'athlete-brand-collaboration' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Athlete Brand Collaboration</h1>
