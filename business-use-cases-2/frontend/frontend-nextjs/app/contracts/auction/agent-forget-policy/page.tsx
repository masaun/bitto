...
export default function AgentForgetPolicyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-forget-policy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-forget-policy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-forget-policy' },
      });
