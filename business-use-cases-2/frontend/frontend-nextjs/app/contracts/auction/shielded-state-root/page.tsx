...
export default function ShieldedStateRootPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('shielded-state-root');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'shielded-state-root',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'shielded-state-root' },
      });
