...
export default function SecureMessageArchivePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('secure-message-archive');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'secure-message-archive',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'secure-message-archive' },
      });
