...
export default function SecureBroadcastChannelPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('secure-broadcast-channel');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'secure-broadcast-channel',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'secure-broadcast-channel' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Secure Broadcast Channel</h1>
