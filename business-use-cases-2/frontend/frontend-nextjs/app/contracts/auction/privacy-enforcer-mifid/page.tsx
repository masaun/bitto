...
export default function PrivacyEnforcerMifidPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-enforcer-mifid');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-enforcer-mifid',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-enforcer-mifid' },
      });
