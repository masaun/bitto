...
export default function AgentDecisionProofPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-decision-proof');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-decision-proof',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-decision-proof' },
      });
