...
export default function SportsTicketStakingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-ticket-staking');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-ticket-staking',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-ticket-staking' },
      });
