...
export default function ModelRiskScorePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('model-risk-score');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'model-risk-score',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'model-risk-score' },
      });
