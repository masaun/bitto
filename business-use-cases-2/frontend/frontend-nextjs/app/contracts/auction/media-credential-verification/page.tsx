...
export default function MediaCredentialVerificationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('media-credential-verification');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'media-credential-verification',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'media-credential-verification' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Media Credential Verification</h1>
