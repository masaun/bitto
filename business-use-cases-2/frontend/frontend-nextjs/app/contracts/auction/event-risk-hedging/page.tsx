...
export default function EventRiskHedgingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-risk-hedging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-risk-hedging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-risk-hedging' },
      });
