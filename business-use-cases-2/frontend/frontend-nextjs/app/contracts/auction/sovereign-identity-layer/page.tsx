...
export default function SovereignIdentityLayerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sovereign-identity-layer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sovereign-identity-layer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sovereign-identity-layer' },
      });
