...
export default function IdentityValidatorSoxPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('identity-validator-sox');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'identity-validator-sox',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'identity-validator-sox' },
      });
