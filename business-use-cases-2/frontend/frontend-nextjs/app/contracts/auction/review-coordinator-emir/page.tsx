...
export default function ReviewCoordinatorEmirPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('review-coordinator-emir');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'review-coordinator-emir',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'review-coordinator-emir' },
      });
