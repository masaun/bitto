...
export default function AiIdentityResolutionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-identity-resolution');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-identity-resolution',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-identity-resolution' },
      });
