...
export default function AgentIncidentReportingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-incident-reporting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-incident-reporting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-incident-reporting' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Agent Incident Reporting</h1>
