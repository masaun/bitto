...
export default function BridgeConstructionLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bridge-construction-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bridge-construction-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bridge-construction-ledger' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Bridge Construction Ledger</h1>
