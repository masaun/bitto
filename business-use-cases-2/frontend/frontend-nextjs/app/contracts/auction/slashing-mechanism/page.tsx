...
export default function SlashingMechanismPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('slashing-mechanism');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'slashing-mechanism',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'slashing-mechanism' },
      });
