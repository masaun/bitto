...
export default function AgentMemoryExpiryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-memory-expiry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-memory-expiry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-memory-expiry' },
      });
