...
export default function AntiCensorshipQueuePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('anti-censorship-queue');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'anti-censorship-queue',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'anti-censorship-queue' },
      });
