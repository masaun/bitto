...
export default function MultilateralNettingFxOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('multilateral-netting-fx-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'multilateral-netting-fx-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'multilateral-netting-fx-option' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Multilateral Netting Fx Option</h1>
