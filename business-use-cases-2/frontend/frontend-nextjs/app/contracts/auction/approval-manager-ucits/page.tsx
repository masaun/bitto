...
export default function ApprovalManagerUcitsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('approval-manager-ucits');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'approval-manager-ucits',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'approval-manager-ucits' },
      });
