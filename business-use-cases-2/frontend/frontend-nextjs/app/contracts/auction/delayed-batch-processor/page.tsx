...
export default function DelayedBatchProcessorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('delayed-batch-processor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'delayed-batch-processor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'delayed-batch-processor' },
      });
