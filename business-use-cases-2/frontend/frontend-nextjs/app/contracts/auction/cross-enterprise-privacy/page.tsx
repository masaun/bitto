...
export default function CrossEnterprisePrivacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('cross-enterprise-privacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'cross-enterprise-privacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'cross-enterprise-privacy' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Cross Enterprise Privacy</h1>
