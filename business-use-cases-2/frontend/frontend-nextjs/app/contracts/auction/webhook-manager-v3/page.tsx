...
export default function WebhookManagerV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('webhook-manager-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'webhook-manager-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'webhook-manager-v3' },
      });
