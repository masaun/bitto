...
export default function VibrationMonitoringSystemPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('vibration-monitoring-system');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'vibration-monitoring-system',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'vibration-monitoring-system' },
      });
