...
export default function CustomOrderingPolicyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('custom-ordering-policy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'custom-ordering-policy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'custom-ordering-policy' },
      });
