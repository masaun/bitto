...
export default function BespokeStorageSolutionsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bespoke-storage-solutions');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bespoke-storage-solutions',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bespoke-storage-solutions' },
      });
