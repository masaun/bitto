...
export default function GamingCommitRevealPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('gaming-commit-reveal');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'gaming-commit-reveal',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'gaming-commit-reveal' },
      });
