...
export default function DivestitureHandlingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('divestiture-handling');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'divestiture-handling',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'divestiture-handling' },
      });
