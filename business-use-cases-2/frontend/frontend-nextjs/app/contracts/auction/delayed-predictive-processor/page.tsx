...
export default function DelayedPredictiveProcessorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('delayed-predictive-processor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'delayed-predictive-processor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'delayed-predictive-processor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Delayed Predictive Processor</h1>
