...
export default function PolicyManagerFiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('policy-manager-fia');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'policy-manager-fia',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'policy-manager-fia' },
      });
