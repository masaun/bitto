...
export default function BridgeLeasebackArrangementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bridge-leaseback-arrangement');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bridge-leaseback-arrangement',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bridge-leaseback-arrangement' },
      });
