...
export default function AgritechSensorNetworkPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agritech-sensor-network');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agritech-sensor-network',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agritech-sensor-network' },
      });
