...
export default function SecureMessagingLayerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('secure-messaging-layer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'secure-messaging-layer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'secure-messaging-layer' },
      });
