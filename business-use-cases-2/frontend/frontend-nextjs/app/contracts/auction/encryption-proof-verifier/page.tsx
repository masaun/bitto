...
export default function EncryptionProofVerifierPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encryption-proof-verifier');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encryption-proof-verifier',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encryption-proof-verifier' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Encryption Proof Verifier</h1>
