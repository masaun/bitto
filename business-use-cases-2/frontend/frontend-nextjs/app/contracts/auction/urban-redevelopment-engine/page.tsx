...
export default function UrbanRedevelopmentEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('urban-redevelopment-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'urban-redevelopment-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'urban-redevelopment-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Urban Redevelopment Engine</h1>
