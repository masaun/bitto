...
export default function DataProtectorEmirPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('data-protector-emir');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'data-protector-emir',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'data-protector-emir' },
      });
