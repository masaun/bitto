...
export default function SanctionsDetectorFiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sanctions-detector-fia');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sanctions-detector-fia',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sanctions-detector-fia' },
      });
