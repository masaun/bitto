...
export default function EncryptedArchiveNodePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-archive-node');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-archive-node',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-archive-node' },
      });
