...
export default function FanLoyaltyAirdropPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('fan-loyalty-airdrop');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'fan-loyalty-airdrop',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'fan-loyalty-airdrop' },
      });
