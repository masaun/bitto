...
export default function MevProtectionRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('mev-protection-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'mev-protection-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'mev-protection-registry' },
      });
