...
export default function DocumentProcessorSecPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('document-processor-sec');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'document-processor-sec',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'document-processor-sec' },
      });
