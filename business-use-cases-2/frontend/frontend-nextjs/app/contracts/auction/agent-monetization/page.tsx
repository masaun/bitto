...
export default function AgentMonetizationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-monetization');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-monetization',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-monetization' },
      });
