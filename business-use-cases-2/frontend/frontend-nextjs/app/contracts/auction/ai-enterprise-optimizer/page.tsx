...
export default function AiEnterpriseOptimizerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-enterprise-optimizer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-enterprise-optimizer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-enterprise-optimizer' },
      });
