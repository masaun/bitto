...
export default function IdentityValidatorSecPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('identity-validator-sec');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'identity-validator-sec',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'identity-validator-sec' },
      });
