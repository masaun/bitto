...
export default function CraneOperationsMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('crane-operations-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'crane-operations-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'crane-operations-monitor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Crane Operations Monitor</h1>
