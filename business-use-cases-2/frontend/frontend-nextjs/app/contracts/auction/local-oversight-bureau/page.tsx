...
export default function LocalOversightBureauPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-oversight-bureau');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-oversight-bureau',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-oversight-bureau' },
      });
