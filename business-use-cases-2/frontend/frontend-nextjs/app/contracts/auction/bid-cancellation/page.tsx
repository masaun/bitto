...
export default function BidCancellationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bid-cancellation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bid-cancellation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bid-cancellation' },
      });
