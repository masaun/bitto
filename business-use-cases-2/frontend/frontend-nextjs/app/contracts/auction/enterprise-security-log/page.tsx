...
export default function EnterpriseSecurityLogPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('enterprise-security-log');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'enterprise-security-log',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'enterprise-security-log' },
      });
