...
export default function KnowledgeRetirementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('knowledge-retirement');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'knowledge-retirement',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'knowledge-retirement' },
      });
