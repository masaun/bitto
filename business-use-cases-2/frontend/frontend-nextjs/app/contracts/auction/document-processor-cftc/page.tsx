...
export default function DocumentProcessorCftcPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('document-processor-cftc');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'document-processor-cftc',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'document-processor-cftc' },
      });
