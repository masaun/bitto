...
export default function HospitalityFitoutManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('hospitality-fitout-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'hospitality-fitout-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'hospitality-fitout-manager' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Hospitality Fitout Manager</h1>
