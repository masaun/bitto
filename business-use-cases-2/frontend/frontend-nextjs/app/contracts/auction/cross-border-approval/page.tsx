...
export default function CrossBorderApprovalPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('cross-border-approval');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'cross-border-approval',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'cross-border-approval' },
      });
