...
export default function ModelRollbackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('model-rollback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'model-rollback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'model-rollback' },
      });
