...
export default function BidAnonymityLayerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bid-anonymity-layer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bid-anonymity-layer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bid-anonymity-layer' },
      });
