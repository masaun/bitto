...
export default function AiDrivenRuleEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-driven-rule-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-driven-rule-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-driven-rule-engine' },
      });
