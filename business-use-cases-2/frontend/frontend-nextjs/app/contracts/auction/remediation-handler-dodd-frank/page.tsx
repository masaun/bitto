...
export default function RemediationHandlerDoddFrankPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('remediation-handler-dodd-frank');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'remediation-handler-dodd-frank',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'remediation-handler-dodd-frank' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Remediation Handler Dodd Frank</h1>
