...
export default function HiddenAuctionEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('hidden-auction-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'hidden-auction-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'hidden-auction-engine' },
      });
