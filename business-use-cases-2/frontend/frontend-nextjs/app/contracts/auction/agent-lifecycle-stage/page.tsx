...
export default function AgentLifecycleStagePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-lifecycle-stage');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-lifecycle-stage',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-lifecycle-stage' },
      });
