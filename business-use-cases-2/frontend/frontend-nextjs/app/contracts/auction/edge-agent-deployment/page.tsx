...
export default function EdgeAgentDeploymentPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('edge-agent-deployment');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'edge-agent-deployment',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'edge-agent-deployment' },
      });
