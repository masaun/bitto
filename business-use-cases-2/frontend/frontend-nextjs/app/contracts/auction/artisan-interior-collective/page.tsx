...
export default function ArtisanInteriorCollectivePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('artisan-interior-collective');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'artisan-interior-collective',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'artisan-interior-collective' },
      });
