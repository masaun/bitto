...
export default function ChampionshipBroadcastTokenPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('championship-broadcast-token');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'championship-broadcast-token',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'championship-broadcast-token' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Championship Broadcast Token</h1>
