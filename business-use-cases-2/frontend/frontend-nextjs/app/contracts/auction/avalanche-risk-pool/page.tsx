...
export default function AvalancheRiskPoolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('avalanche-risk-pool');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'avalanche-risk-pool',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'avalanche-risk-pool' },
      });
