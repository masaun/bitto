...
export default function LocalStakeholderCouncilPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-stakeholder-council');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-stakeholder-council',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-stakeholder-council' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Local Stakeholder Council</h1>
