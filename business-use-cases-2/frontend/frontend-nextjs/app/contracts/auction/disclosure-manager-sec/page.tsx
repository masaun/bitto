...
export default function DisclosureManagerSecPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('disclosure-manager-sec');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'disclosure-manager-sec',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'disclosure-manager-sec' },
      });
