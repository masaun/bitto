...
export default function FrameworkAgreementLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('framework-agreement-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'framework-agreement-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'framework-agreement-ledger' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Framework Agreement Ledger</h1>
