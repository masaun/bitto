...
export default function RegionalRoleManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('regional-role-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'regional-role-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'regional-role-manager' },
      });
