...
export default function AiSecurityFirewallPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-security-firewall');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-security-firewall',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-security-firewall' },
      });
