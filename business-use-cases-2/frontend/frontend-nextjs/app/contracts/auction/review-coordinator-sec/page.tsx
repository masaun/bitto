...
export default function ReviewCoordinatorSecPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('review-coordinator-sec');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'review-coordinator-sec',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'review-coordinator-sec' },
      });
