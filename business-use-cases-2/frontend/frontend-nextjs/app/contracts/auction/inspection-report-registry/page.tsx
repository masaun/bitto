...
export default function InspectionReportRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('inspection-report-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'inspection-report-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'inspection-report-registry' },
      });
