...
export default function DelayedSyncProcessorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('delayed-sync-processor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'delayed-sync-processor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'delayed-sync-processor' },
      });
