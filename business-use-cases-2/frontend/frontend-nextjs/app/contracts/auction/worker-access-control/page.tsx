...
export default function WorkerAccessControlPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('worker-access-control');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'worker-access-control',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'worker-access-control' },
      });
