...
export default function MultiAgentEconomyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('multi-agent-economy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'multi-agent-economy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'multi-agent-economy' },
      });
