...
export default function PolicyManagerEmirPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('policy-manager-emir');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'policy-manager-emir',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'policy-manager-emir' },
      });
