...
export default function ArtisanCarpenterMarketPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('artisan-carpenter-market');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'artisan-carpenter-market',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'artisan-carpenter-market' },
      });
