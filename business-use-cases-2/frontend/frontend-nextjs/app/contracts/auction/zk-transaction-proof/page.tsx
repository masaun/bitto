...
export default function ZkTransactionProofPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('zk-transaction-proof');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'zk-transaction-proof',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'zk-transaction-proof' },
      });
