...
export default function AiAgentSuspensionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-agent-suspension');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-agent-suspension',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-agent-suspension' },
      });
