...
export default function SummerGymnasticsBonusPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('summer-gymnastics-bonus');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'summer-gymnastics-bonus',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'summer-gymnastics-bonus' },
      });
