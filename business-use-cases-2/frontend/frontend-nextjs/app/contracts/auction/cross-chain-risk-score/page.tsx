...
export default function CrossChainRiskScorePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('cross-chain-risk-score');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'cross-chain-risk-score',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'cross-chain-risk-score' },
      });
