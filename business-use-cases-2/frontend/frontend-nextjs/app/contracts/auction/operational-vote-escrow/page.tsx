...
export default function OperationalVoteEscrowPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('operational-vote-escrow');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'operational-vote-escrow',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'operational-vote-escrow' },
      });
