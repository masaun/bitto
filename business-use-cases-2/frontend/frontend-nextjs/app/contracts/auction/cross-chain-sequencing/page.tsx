...
export default function CrossChainSequencingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('cross-chain-sequencing');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'cross-chain-sequencing',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'cross-chain-sequencing' },
      });
