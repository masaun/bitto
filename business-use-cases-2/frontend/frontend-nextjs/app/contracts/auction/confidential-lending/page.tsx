...
export default function ConfidentialLendingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('confidential-lending');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'confidential-lending',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'confidential-lending' },
      });
