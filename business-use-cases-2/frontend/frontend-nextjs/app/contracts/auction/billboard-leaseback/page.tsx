...
export default function BillboardLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('billboard-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'billboard-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'billboard-leaseback' },
      });
