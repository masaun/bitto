...
export default function DisclosureManagerSoxPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('disclosure-manager-sox');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'disclosure-manager-sox',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'disclosure-manager-sox' },
      });
