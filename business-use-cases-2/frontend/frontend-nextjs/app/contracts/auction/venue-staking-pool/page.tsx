...
export default function VenueStakingPoolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('venue-staking-pool');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'venue-staking-pool',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'venue-staking-pool' },
      });
