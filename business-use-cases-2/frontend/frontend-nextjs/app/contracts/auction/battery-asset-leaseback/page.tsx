...
export default function BatteryAssetLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('battery-asset-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'battery-asset-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'battery-asset-leaseback' },
      });
