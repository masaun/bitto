...
export default function IdentityRevocationLogPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('identity-revocation-log');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'identity-revocation-log',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'identity-revocation-log' },
      });
