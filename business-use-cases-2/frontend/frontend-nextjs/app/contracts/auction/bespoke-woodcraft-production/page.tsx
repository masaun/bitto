...
export default function BespokeWoodcraftProductionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bespoke-woodcraft-production');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bespoke-woodcraft-production',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bespoke-woodcraft-production' },
      });
