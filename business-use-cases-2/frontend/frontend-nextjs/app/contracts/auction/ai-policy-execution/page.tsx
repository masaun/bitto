...
export default function AiPolicyExecutionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-policy-execution');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-policy-execution',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-policy-execution' },
      });
