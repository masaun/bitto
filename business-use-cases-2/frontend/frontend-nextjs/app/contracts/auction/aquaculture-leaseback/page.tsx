...
export default function AquacultureLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('aquaculture-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'aquaculture-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'aquaculture-leaseback' },
      });
