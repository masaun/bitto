...
export default function LienWaiverRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('lien-waiver-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'lien-waiver-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'lien-waiver-registry' },
      });
