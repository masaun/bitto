...
export default function AviationAssetLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('aviation-asset-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'aviation-asset-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'aviation-asset-leaseback' },
      });
