...
export default function RemediationHandlerHipaaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('remediation-handler-hipaa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'remediation-handler-hipaa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'remediation-handler-hipaa' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Remediation Handler Hipaa</h1>
