...
export default function RainwaterHarvestingBuildPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rainwater-harvesting-build');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rainwater-harvesting-build',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rainwater-harvesting-build' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Rainwater Harvesting Build</h1>
