...
export default function PolicyManagerUcitsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('policy-manager-ucits');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'policy-manager-ucits',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'policy-manager-ucits' },
      });
