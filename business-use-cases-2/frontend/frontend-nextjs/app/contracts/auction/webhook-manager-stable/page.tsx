...
export default function WebhookManagerStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('webhook-manager-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'webhook-manager-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'webhook-manager-stable' },
      });
