...
export default function ConcreteRecyclingFacilityPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('concrete-recycling-facility');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'concrete-recycling-facility',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'concrete-recycling-facility' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Concrete Recycling Facility</h1>
