...
export default function ConsentTrackerFiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('consent-tracker-fia');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'consent-tracker-fia',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'consent-tracker-fia' },
      });
