...
export default function LocalAdvisorPanelPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-advisor-panel');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-advisor-panel',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-advisor-panel' },
      });
