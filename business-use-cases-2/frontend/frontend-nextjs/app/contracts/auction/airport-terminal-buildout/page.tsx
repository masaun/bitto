...
export default function AirportTerminalBuildoutPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('airport-terminal-buildout');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'airport-terminal-buildout',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'airport-terminal-buildout' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Airport Terminal Buildout</h1>
