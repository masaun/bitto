...
export default function WebhookManagerAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('webhook-manager-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'webhook-manager-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'webhook-manager-alpha' },
      });
