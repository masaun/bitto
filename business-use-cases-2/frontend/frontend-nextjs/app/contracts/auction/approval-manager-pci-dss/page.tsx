...
export default function ApprovalManagerPciDssPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('approval-manager-pci-dss');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'approval-manager-pci-dss',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'approval-manager-pci-dss' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Approval Manager Pci Dss</h1>
