...
export default function MachineryRentalNetworkPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('machinery-rental-network');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'machinery-rental-network',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'machinery-rental-network' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Machinery Rental Network</h1>
