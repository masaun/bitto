...
export default function TacticalStateMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tactical-state-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tactical-state-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tactical-state-monitor' },
      });
