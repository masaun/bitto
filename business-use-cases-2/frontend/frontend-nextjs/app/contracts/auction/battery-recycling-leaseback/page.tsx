...
export default function BatteryRecyclingLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('battery-recycling-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'battery-recycling-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'battery-recycling-leaseback' },
      });
