...
export default function LocalRoleManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-role-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-role-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-role-manager' },
      });
