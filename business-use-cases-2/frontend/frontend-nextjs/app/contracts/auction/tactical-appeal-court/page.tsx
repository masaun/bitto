...
export default function TacticalAppealCourtPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tactical-appeal-court');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tactical-appeal-court',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tactical-appeal-court' },
      });
