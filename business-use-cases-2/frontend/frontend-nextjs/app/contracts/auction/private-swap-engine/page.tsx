...
export default function PrivateSwapEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('private-swap-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'private-swap-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'private-swap-engine' },
      });
