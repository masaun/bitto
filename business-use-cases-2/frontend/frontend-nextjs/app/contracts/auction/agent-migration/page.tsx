...
export default function AgentMigrationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-migration');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-migration',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-migration' },
      });
