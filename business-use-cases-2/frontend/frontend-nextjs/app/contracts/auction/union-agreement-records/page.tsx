...
export default function UnionAgreementRecordsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('union-agreement-records');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'union-agreement-records',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'union-agreement-records' },
      });
