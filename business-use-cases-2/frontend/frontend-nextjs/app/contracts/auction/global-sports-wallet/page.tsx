...
export default function GlobalSportsWalletPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('global-sports-wallet');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'global-sports-wallet',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'global-sports-wallet' },
      });
