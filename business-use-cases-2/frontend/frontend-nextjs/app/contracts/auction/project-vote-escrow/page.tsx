...
export default function ProjectVoteEscrowPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('project-vote-escrow');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'project-vote-escrow',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'project-vote-escrow' },
      });
