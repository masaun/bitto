...
export default function PrivacyEnforcerCftcPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-enforcer-cftc');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-enforcer-cftc',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-enforcer-cftc' },
      });
