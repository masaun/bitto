...
export default function PrivacyHealthScorePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-health-score');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-health-score',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-health-score' },
      });
