...
export default function HospitalityLoyaltyProgramPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('hospitality-loyalty-program');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'hospitality-loyalty-program',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'hospitality-loyalty-program' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Hospitality Loyalty Program</h1>
