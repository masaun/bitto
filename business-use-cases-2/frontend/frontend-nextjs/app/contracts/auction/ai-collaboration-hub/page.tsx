...
export default function AiCollaborationHubPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-collaboration-hub');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-collaboration-hub',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-collaboration-hub' },
      });
