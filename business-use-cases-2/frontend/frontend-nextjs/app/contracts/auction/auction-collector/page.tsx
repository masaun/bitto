...
export default function AuctionCollectorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-collector');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-collector',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-collector' },
      });
