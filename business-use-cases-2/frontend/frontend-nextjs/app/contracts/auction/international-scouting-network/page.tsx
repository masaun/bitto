...
export default function InternationalScoutingNetworkPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('international-scouting-network');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'international-scouting-network',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'international-scouting-network' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>International Scouting Network</h1>
