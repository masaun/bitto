...
export default function TeamChangeManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-change-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-change-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-change-manager' },
      });
