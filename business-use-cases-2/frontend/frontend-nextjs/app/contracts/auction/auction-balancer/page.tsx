...
export default function AuctionBalancerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-balancer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-balancer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-balancer' },
      });
