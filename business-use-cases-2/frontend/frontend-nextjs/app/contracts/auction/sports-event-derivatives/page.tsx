...
export default function SportsEventDerivativesPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-event-derivatives');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-event-derivatives',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-event-derivatives' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Sports Event Derivatives</h1>
