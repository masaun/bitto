...
export default function IpInfringementClaimsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ip-infringement-claims');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ip-infringement-claims',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ip-infringement-claims' },
      });
