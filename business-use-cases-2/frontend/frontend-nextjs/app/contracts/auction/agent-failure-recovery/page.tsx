...
export default function AgentFailureRecoveryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-failure-recovery');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-failure-recovery',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-failure-recovery' },
      });
