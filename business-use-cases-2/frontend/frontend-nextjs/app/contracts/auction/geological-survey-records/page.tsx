...
export default function GeologicalSurveyRecordsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('geological-survey-records');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'geological-survey-records',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'geological-survey-records' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Geological Survey Records</h1>
