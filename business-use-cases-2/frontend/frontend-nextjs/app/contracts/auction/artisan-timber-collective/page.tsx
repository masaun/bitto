...
export default function ArtisanTimberCollectivePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('artisan-timber-collective');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'artisan-timber-collective',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'artisan-timber-collective' },
      });
