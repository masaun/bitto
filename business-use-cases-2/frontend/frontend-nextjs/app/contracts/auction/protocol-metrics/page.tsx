...
export default function ProtocolMetricsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('protocol-metrics');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'protocol-metrics',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'protocol-metrics' },
      });
