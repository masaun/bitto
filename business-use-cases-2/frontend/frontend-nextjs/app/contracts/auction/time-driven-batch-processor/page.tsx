...
export default function TimeDrivenBatchProcessorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('time-driven-batch-processor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'time-driven-batch-processor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'time-driven-batch-processor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Time Driven Batch Processor</h1>
