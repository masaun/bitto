...
export default function FrontrunPreventionEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('frontrun-prevention-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'frontrun-prevention-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'frontrun-prevention-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Frontrun Prevention Engine</h1>
