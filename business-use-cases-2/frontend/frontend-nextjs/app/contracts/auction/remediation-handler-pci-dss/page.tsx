...
export default function RemediationHandlerPciDssPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('remediation-handler-pci-dss');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'remediation-handler-pci-dss',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'remediation-handler-pci-dss' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Remediation Handler Pci Dss</h1>
