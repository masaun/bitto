...
export default function SteelErectionCoordinatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('steel-erection-coordinator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'steel-erection-coordinator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'steel-erection-coordinator' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Steel Erection Coordinator</h1>
