...
export default function WinterIceRinkLeasingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('winter-ice-rink-leasing');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'winter-ice-rink-leasing',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'winter-ice-rink-leasing' },
      });
