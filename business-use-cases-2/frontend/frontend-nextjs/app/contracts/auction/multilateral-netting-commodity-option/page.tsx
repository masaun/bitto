...
export default function MultilateralNettingCommodityOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('multilateral-netting-commodity-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'multilateral-netting-commodity-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'multilateral-netting-commodity-option' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Multilateral Netting Commodity Option</h1>
