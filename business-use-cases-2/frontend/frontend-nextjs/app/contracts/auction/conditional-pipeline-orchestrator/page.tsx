...
export default function ConditionalPipelineOrchestratorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('conditional-pipeline-orchestrator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'conditional-pipeline-orchestrator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'conditional-pipeline-orchestrator' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Conditional Pipeline Orchestrator</h1>
