...
export default function EncryptedTransactionReplayPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-transaction-replay');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-transaction-replay',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-transaction-replay' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Encrypted Transaction Replay</h1>
