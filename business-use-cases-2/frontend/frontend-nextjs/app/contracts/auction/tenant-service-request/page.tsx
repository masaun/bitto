...
export default function TenantServiceRequestPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tenant-service-request');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tenant-service-request',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tenant-service-request' },
      });
