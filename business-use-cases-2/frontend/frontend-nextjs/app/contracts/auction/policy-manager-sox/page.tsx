...
export default function PolicyManagerSoxPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('policy-manager-sox');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'policy-manager-sox',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'policy-manager-sox' },
      });
