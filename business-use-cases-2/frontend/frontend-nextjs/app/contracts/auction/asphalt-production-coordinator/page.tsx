...
export default function AsphaltProductionCoordinatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('asphalt-production-coordinator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'asphalt-production-coordinator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'asphalt-production-coordinator' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Asphalt Production Coordinator</h1>
