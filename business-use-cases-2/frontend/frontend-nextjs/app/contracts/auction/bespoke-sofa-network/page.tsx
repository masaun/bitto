...
export default function BespokeSofaNetworkPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bespoke-sofa-network');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bespoke-sofa-network',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bespoke-sofa-network' },
      });
