...
export default function DelayedFlowControllerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('delayed-flow-controller');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'delayed-flow-controller',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'delayed-flow-controller' },
      });
