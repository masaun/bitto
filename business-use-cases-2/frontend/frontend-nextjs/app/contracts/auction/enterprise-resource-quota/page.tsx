...
export default function EnterpriseResourceQuotaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('enterprise-resource-quota');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'enterprise-resource-quota',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'enterprise-resource-quota' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Enterprise Resource Quota</h1>
