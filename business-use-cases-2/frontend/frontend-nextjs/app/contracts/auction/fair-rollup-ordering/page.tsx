...
export default function FairRollupOrderingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('fair-rollup-ordering');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'fair-rollup-ordering',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'fair-rollup-ordering' },
      });
