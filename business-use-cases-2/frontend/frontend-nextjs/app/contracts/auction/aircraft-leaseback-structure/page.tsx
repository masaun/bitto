...
export default function AircraftLeasebackStructurePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('aircraft-leaseback-structure');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'aircraft-leaseback-structure',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'aircraft-leaseback-structure' },
      });
