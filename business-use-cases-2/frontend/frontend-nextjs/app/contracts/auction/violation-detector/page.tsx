...
export default function ViolationDetectorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('violation-detector');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'violation-detector',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'violation-detector' },
      });
