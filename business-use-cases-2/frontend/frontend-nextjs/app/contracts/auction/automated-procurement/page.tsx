...
export default function AutomatedProcurementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automated-procurement');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automated-procurement',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automated-procurement' },
      });
