...
export default function EncryptedStateSyncPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-state-sync');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-state-sync',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-state-sync' },
      });
