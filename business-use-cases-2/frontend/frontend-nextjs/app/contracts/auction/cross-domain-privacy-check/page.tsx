...
export default function CrossDomainPrivacyCheckPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('cross-domain-privacy-check');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'cross-domain-privacy-check',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'cross-domain-privacy-check' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Cross Domain Privacy Check</h1>
