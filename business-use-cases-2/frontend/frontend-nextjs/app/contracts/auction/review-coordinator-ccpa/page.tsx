...
export default function ReviewCoordinatorCcpaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('review-coordinator-ccpa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'review-coordinator-ccpa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'review-coordinator-ccpa' },
      });
