...
export default function ArtisanWoodcraftAuctionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('artisan-woodcraft-auction');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'artisan-woodcraft-auction',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'artisan-woodcraft-auction' },
      });
