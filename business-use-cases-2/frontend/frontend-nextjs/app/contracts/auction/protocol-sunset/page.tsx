...
export default function ProtocolSunsetPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('protocol-sunset');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'protocol-sunset',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'protocol-sunset' },
      });
