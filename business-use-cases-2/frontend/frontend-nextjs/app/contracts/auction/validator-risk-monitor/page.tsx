...
export default function ValidatorRiskMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('validator-risk-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'validator-risk-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'validator-risk-monitor' },
      });
