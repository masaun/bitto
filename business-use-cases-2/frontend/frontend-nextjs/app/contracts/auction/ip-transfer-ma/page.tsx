...
export default function IpTransferMaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ip-transfer-ma');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ip-transfer-ma',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ip-transfer-ma' },
      });
