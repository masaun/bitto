...
export default function OversightBureauPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('oversight-bureau');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'oversight-bureau',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'oversight-bureau' },
      });
