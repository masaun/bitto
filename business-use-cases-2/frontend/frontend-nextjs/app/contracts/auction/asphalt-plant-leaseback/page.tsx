...
export default function AsphaltPlantLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('asphalt-plant-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'asphalt-plant-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'asphalt-plant-leaseback' },
      });
