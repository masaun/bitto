...
export default function OvertimePaymentLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('overtime-payment-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'overtime-payment-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'overtime-payment-ledger' },
      });
