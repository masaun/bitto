...
export default function AgentIdentityGraphPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-identity-graph');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-identity-graph',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-identity-graph' },
      });
