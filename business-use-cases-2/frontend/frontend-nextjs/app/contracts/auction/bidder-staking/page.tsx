...
export default function BidderStakingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bidder-staking');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bidder-staking',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bidder-staking' },
      });
