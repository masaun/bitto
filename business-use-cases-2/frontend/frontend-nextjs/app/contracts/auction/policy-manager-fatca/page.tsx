...
export default function PolicyManagerFatcaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('policy-manager-fatca');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'policy-manager-fatca',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'policy-manager-fatca' },
      });
