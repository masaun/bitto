...
export default function GlobalTicketLotteryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('global-ticket-lottery');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'global-ticket-lottery',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'global-ticket-lottery' },
      });
