...
export default function EncryptedPushServicePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-push-service');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-push-service',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-push-service' },
      });
