...
export default function SportsEventMicroinsurancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-event-microinsurance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-event-microinsurance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-event-microinsurance' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Sports Event Microinsurance</h1>
