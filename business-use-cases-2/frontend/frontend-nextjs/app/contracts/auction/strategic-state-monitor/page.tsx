...
export default function StrategicStateMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('strategic-state-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'strategic-state-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'strategic-state-monitor' },
      });
