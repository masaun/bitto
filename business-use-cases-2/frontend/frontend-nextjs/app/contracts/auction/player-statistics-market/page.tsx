...
export default function PlayerStatisticsMarketPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('player-statistics-market');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'player-statistics-market',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'player-statistics-market' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Player Statistics Market</h1>
