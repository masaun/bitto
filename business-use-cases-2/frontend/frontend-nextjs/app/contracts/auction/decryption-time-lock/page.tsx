...
export default function DecryptionTimeLockPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('decryption-time-lock');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'decryption-time-lock',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'decryption-time-lock' },
      });
