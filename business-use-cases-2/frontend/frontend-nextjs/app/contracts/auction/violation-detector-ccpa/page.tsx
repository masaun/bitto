...
export default function ViolationDetectorCcpaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('violation-detector-ccpa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'violation-detector-ccpa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'violation-detector-ccpa' },
      });
