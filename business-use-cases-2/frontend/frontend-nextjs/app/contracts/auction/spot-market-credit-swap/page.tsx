...
export default function SpotMarketCreditSwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('spot-market-credit-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'spot-market-credit-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'spot-market-credit-swap' },
      });
