...
export default function LifecycleMaintenanceLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('lifecycle-maintenance-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'lifecycle-maintenance-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'lifecycle-maintenance-ledger' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Lifecycle Maintenance Ledger</h1>
