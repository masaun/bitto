...
export default function AiToAiContractingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-to-ai-contracting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-to-ai-contracting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-to-ai-contracting' },
      });
