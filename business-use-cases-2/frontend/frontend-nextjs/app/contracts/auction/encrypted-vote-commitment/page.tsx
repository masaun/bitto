...
export default function EncryptedVoteCommitmentPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-vote-commitment');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-vote-commitment',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-vote-commitment' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Encrypted Vote Commitment</h1>
