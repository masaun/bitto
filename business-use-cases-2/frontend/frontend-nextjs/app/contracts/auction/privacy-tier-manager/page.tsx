...
export default function PrivacyTierManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-tier-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-tier-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-tier-manager' },
      });
