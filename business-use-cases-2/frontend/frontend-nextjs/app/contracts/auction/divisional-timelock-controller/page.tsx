...
export default function DivisionalTimelockControllerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('divisional-timelock-controller');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'divisional-timelock-controller',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'divisional-timelock-controller' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Divisional Timelock Controller</h1>
