...
export default function AiIpRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-ip-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-ip-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-ip-registry' },
      });
