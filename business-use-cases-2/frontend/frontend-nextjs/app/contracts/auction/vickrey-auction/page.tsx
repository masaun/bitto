...
export default function VickreyAuctionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('vickrey-auction');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'vickrey-auction',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'vickrey-auction' },
      });
