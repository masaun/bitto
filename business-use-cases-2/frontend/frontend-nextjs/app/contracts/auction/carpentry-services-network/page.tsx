...
export default function CarpentryServicesNetworkPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('carpentry-services-network');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'carpentry-services-network',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'carpentry-services-network' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Carpentry Services Network</h1>
