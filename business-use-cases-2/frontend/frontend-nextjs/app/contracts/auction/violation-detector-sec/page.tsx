...
export default function ViolationDetectorSecPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('violation-detector-sec');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'violation-detector-sec',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'violation-detector-sec' },
      });
