...
export default function ChampionshipHospitalityTokenPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('championship-hospitality-token');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'championship-hospitality-token',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'championship-hospitality-token' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Championship Hospitality Token</h1>
