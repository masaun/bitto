...
export default function EventStreamerExperimentalPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-streamer-experimental');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-streamer-experimental',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-streamer-experimental' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Event Streamer Experimental</h1>
