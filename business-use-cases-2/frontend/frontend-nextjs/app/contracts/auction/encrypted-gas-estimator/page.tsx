...
export default function EncryptedGasEstimatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-gas-estimator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-gas-estimator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-gas-estimator' },
      });
