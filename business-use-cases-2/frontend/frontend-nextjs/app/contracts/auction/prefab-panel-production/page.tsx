...
export default function PrefabPanelProductionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('prefab-panel-production');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'prefab-panel-production',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'prefab-panel-production' },
      });
