...
export default function VenueAccessNftPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('venue-access-nft');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'venue-access-nft',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'venue-access-nft' },
      });
