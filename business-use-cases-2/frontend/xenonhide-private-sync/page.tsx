...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_XENONHIDE_PRIVATE_SYNC_ADDRESS || '';

export default function Xenonhide_Private_SyncPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.xenonhide-private-sync/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'xenonhide-private-sync',
