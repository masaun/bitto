...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACEVANTAGE_OFFCHAIN_RUNNER_ADDRESS || '';

export default function Tracevantage_Offchain_RunnerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tracevantage-offchain-runner',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tracevantage-offchain-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tracevantage-offchain-runner',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tracevantage-offchain-runner</h1>
