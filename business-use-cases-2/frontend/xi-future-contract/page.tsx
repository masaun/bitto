...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_XI_FUTURE_CONTRACT_ADDRESS || '';

export default function Xi_Future_ContractPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.xi-future-contract/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'xi-future-contract',
