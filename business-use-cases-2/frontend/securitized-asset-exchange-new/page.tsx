...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SECURITIZED_ASSET_EXCHANGE_NEW_ADDRESS || '';

export default function Securitized_Asset_Exchange_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'securitized-asset-exchange-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.securitized-asset-exchange-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'securitized-asset-exchange-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>securitized-asset-exchange-new</h1>
