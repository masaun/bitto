...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CONTRIBUTOR_BADGE_ADDRESS || '';

export default function Contributor_BadgePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.contributor-badge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'contributor-badge',
