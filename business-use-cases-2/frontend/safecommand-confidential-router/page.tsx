...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SAFECOMMAND_CONFIDENTIAL_ROUTER_ADDRESS || '';

export default function Safecommand_Confidential_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'safecommand-confidential-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.safecommand-confidential-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'safecommand-confidential-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>safecommand-confidential-router</h1>
