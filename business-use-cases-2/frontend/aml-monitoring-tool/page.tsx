...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AML_MONITORING_TOOL_ADDRESS || '';

export default function Aml_Monitoring_ToolPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.aml-monitoring-tool/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'aml-monitoring-tool',
