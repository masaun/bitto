...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDITCHAIN_SECURE_EXECUTOR_ADDRESS || '';

export default function Auditchain_Secure_ExecutorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'auditchain-secure-executor',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auditchain-secure-executor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auditchain-secure-executor',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>auditchain-secure-executor</h1>
