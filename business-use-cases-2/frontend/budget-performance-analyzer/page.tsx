...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BUDGET_PERFORMANCE_ANALYZER_ADDRESS || '';

export default function Budget_Performance_AnalyzerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.budget-performance-analyzer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'budget-performance-analyzer',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>budget-performance-analyzer</h1>
