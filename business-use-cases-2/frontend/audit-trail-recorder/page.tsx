...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDIT_TRAIL_RECORDER_ADDRESS || '';

export default function Audit_Trail_RecorderPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.audit-trail-recorder/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'audit-trail-recorder',
