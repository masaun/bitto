...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AQUACREST_HIDDEN_PIPELINE_ADDRESS || '';

export default function Aquacrest_Hidden_PipelinePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'aquacrest-hidden-pipeline',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.aquacrest-hidden-pipeline/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'aquacrest-hidden-pipeline',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>aquacrest-hidden-pipeline</h1>
