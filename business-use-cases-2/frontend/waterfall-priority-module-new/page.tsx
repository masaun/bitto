...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WATERFALL_PRIORITY_MODULE_NEW_ADDRESS || '';

export default function Waterfall_Priority_Module_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'waterfall-priority-module-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.waterfall-priority-module-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'waterfall-priority-module-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>waterfall-priority-module-new</h1>
