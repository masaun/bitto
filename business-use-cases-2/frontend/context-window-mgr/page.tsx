...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CONTEXT_WINDOW_MGR_ADDRESS || '';

export default function Context_Window_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.context-window-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'context-window-mgr',
