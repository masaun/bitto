...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CIRCUIT_BREAKER_V2_ADDRESS || '';

export default function Circuit_Breaker_V2Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.circuit-breaker-v2/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'circuit-breaker-v2',
