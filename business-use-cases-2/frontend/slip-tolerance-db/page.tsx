...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SLIP_TOLERANCE_DB_ADDRESS || '';

export default function Slip_Tolerance_DbPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.slip-tolerance-db/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'slip-tolerance-db',
