...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PHASE_1_ENVIRON_AI_ADDRESS || '';

export default function Phase_1_Environ_AiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.phase-1-environ-ai/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'phase-1-environ-ai',
