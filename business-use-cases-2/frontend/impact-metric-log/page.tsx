...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_IMPACT_METRIC_LOG_ADDRESS || '';

export default function Impact_Metric_LogPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.impact-metric-log/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'impact-metric-log',
