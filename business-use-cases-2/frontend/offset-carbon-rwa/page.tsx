...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OFFSET_CARBON_RWA_ADDRESS || '';

export default function Offset_Carbon_RwaPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.offset-carbon-rwa/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'offset-carbon-rwa',
