...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MEZZANINE_TRANCHE_ROUTER_NEW_ADDRESS || '';

export default function Mezzanine_Tranche_Router_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'mezzanine-tranche-router-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.mezzanine-tranche-router-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'mezzanine-tranche-router-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>mezzanine-tranche-router-new</h1>
