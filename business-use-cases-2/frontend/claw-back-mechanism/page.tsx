...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CLAW_BACK_MECHANISM_ADDRESS || '';

export default function Claw_Back_MechanismPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.claw-back-mechanism/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'claw-back-mechanism',
