...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MEDIUM_TERM_BUDGET_ADDRESS || '';

export default function Medium_Term_BudgetPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.medium-term-budget/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'medium-term-budget',
