...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERICREST_CONFIDENTIAL_EXECUTOR_ADDRESS || '';

export default function Vericrest_Confidential_ExecutorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'vericrest-confidential-executor',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.vericrest-confidential-executor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'vericrest-confidential-executor',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>vericrest-confidential-executor</h1>
