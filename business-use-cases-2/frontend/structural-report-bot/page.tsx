...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STRUCTURAL_REPORT_BOT_ADDRESS || '';

export default function Structural_Report_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.structural-report-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'structural-report-bot',
