...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EXIT_PENALTY_LOGIC_ADDRESS || '';

export default function Exit_Penalty_LogicPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.exit-penalty-logic/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'exit-penalty-logic',
