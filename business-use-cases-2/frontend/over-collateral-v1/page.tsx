...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OVER_COLLATERAL_V1_ADDRESS || '';

export default function Over_Collateral_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.over-collateral-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'over-collateral-v1',
