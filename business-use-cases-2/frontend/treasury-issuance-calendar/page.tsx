...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TREASURY_ISSUANCE_CALENDAR_ADDRESS || '';

export default function Treasury_Issuance_CalendarPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'treasury-issuance-calendar',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.treasury-issuance-calendar/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'treasury-issuance-calendar',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>treasury-issuance-calendar</h1>
