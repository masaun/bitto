...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GOVERNMENT_GUARANTEE_LEDGER_ADDRESS || '';

export default function Government_Guarantee_LedgerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'government-guarantee-ledger',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.government-guarantee-ledger/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'government-guarantee-ledger',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>government-guarantee-ledger</h1>
