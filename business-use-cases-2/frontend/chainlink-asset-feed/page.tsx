...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CHAINLINK_ASSET_FEED_ADDRESS || '';

export default function Chainlink_Asset_FeedPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.chainlink-asset-feed/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'chainlink-asset-feed',
