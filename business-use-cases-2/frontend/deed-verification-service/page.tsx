...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEED_VERIFICATION_SERVICE_ADDRESS || '';

export default function Deed_Verification_ServicePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'deed-verification-service',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.deed-verification-service/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'deed-verification-service',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>deed-verification-service</h1>
