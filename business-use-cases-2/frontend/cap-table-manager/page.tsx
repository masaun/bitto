...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CAP_TABLE_MANAGER_ADDRESS || '';

export default function Cap_Table_ManagerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cap-table-manager/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cap-table-manager',
