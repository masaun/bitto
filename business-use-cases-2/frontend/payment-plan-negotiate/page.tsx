...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PAYMENT_PLAN_NEGOTIATE_ADDRESS || '';

export default function Payment_Plan_NegotiatePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.payment-plan-negotiate/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'payment-plan-negotiate',
