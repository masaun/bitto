...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PRIORITY_FEE_CALC_ADDRESS || '';

export default function Priority_Fee_CalcPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.priority-fee-calc/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'priority-fee-calc',
