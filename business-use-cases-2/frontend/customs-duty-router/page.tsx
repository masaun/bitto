...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CUSTOMS_DUTY_ROUTER_ADDRESS || '';

export default function Customs_Duty_RouterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.customs-duty-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'customs-duty-router',
