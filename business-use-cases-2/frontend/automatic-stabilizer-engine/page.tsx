...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUTOMATIC_STABILIZER_ENGINE_ADDRESS || '';

export default function Automatic_Stabilizer_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'automatic-stabilizer-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.automatic-stabilizer-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'automatic-stabilizer-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>automatic-stabilizer-engine</h1>
