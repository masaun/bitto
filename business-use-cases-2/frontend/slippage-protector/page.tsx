...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SLIPPAGE_PROTECTOR_ADDRESS || '';

export default function Slippage_ProtectorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.slippage-protector/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'slippage-protector',
