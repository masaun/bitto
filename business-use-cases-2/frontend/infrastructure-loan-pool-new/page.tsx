...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INFRASTRUCTURE_LOAN_POOL_NEW_ADDRESS || '';

export default function Infrastructure_Loan_Pool_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'infrastructure-loan-pool-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.infrastructure-loan-pool-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'infrastructure-loan-pool-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>infrastructure-loan-pool-new</h1>
