...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_URI_POINTER_REGISTRY_ADDRESS || '';

export default function Uri_Pointer_RegistryPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.uri-pointer-registry/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'uri-pointer-registry',
