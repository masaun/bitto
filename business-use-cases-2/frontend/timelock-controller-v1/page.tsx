...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TIMELOCK_CONTROLLER_V1_ADDRESS || '';

export default function Timelock_Controller_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.timelock-controller-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'timelock-controller-v1',
