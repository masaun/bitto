...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MARKETPLACE_LOAN_SECURITIZER_NEW_ADDRESS || '';

export default function Marketplace_Loan_Securitizer_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'marketplace-loan-securitizer-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.marketplace-loan-securitizer-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'marketplace-loan-securitizer-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>marketplace-loan-securitizer-new</h1>
