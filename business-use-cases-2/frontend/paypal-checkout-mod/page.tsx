...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PAYPAL_CHECKOUT_MOD_ADDRESS || '';

export default function Paypal_Checkout_ModPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.paypal-checkout-mod/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'paypal-checkout-mod',
