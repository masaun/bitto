...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_DISCIPLINE_RULE_ADDRESS || '';

export default function Fiscal_Discipline_RulePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-discipline-rule/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-discipline-rule',
