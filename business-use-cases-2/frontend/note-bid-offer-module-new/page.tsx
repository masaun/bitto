...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NOTE_BID_OFFER_MODULE_NEW_ADDRESS || '';

export default function Note_Bid_Offer_Module_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'note-bid-offer-module-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.note-bid-offer-module-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'note-bid-offer-module-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>note-bid-offer-module-new</h1>
