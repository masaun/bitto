...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EVENT_EMITTER_LOG_ADDRESS || '';

export default function Event_Emitter_LogPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.event-emitter-log/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'event-emitter-log',
