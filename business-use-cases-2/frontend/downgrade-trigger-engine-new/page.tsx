...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DOWNGRADE_TRIGGER_ENGINE_NEW_ADDRESS || '';

export default function Downgrade_Trigger_Engine_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'downgrade-trigger-engine-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.downgrade-trigger-engine-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'downgrade-trigger-engine-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>downgrade-trigger-engine-new</h1>
