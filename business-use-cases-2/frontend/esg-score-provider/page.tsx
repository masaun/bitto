...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ESG_SCORE_PROVIDER_ADDRESS || '';

export default function Esg_Score_ProviderPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.esg-score-provider/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'esg-score-provider',
