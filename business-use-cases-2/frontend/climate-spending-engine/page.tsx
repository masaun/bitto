...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CLIMATE_SPENDING_ENGINE_ADDRESS || '';

export default function Climate_Spending_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.climate-spending-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'climate-spending-engine',
