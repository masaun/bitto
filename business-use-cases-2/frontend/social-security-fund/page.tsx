...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SOCIAL_SECURITY_FUND_ADDRESS || '';

export default function Social_Security_FundPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.social-security-fund/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'social-security-fund',
