...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INFRASTRUCTURE_BOND_ISSUER_ADDRESS || '';

export default function Infrastructure_Bond_IssuerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'infrastructure-bond-issuer',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.infrastructure-bond-issuer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'infrastructure-bond-issuer',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>infrastructure-bond-issuer</h1>
