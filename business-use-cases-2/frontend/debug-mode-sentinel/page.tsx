...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEBUG_MODE_SENTINEL_ADDRESS || '';

export default function Debug_Mode_SentinelPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.debug-mode-sentinel/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'debug-mode-sentinel',
