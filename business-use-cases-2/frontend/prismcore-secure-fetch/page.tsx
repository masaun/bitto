...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PRISMCORE_SECURE_FETCH_ADDRESS || '';

export default function Prismcore_Secure_FetchPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.prismcore-secure-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'prismcore-secure-fetch',
