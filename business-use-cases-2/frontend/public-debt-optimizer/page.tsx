...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PUBLIC_DEBT_OPTIMIZER_ADDRESS || '';

export default function Public_Debt_OptimizerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.public-debt-optimizer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'public-debt-optimizer',
