...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HOUSING_SUBSIDY_ENGINE_ADDRESS || '';

export default function Housing_Subsidy_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.housing-subsidy-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'housing-subsidy-engine',
