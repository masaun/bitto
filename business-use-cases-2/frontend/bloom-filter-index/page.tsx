...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BLOOM_FILTER_INDEX_ADDRESS || '';

export default function Bloom_Filter_IndexPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bloom-filter-index/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bloom-filter-index',
