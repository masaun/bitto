...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NETWORK_CONGESTION_AI_ADDRESS || '';

export default function Network_Congestion_AiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.network-congestion-ai/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'network-congestion-ai',
