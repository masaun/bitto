...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WASTE_MANAGEMENT_BOT_ADDRESS || '';

export default function Waste_Management_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.waste-management-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'waste-management-bot',
