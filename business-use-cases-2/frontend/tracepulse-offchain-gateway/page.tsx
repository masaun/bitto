...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACEPULSE_OFFCHAIN_GATEWAY_ADDRESS || '';

export default function Tracepulse_Offchain_GatewayPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tracepulse-offchain-gateway',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tracepulse-offchain-gateway/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tracepulse-offchain-gateway',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tracepulse-offchain-gateway</h1>
