...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OMNIBUS_WALLET_V1_ADDRESS || '';

export default function Omnibus_Wallet_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.omnibus-wallet-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'omnibus-wallet-v1',
