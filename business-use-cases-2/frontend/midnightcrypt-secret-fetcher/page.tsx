...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MIDNIGHTCRYPT_SECRET_FETCHER_ADDRESS || '';

export default function Midnightcrypt_Secret_FetcherPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'midnightcrypt-secret-fetcher',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.midnightcrypt-secret-fetcher/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'midnightcrypt-secret-fetcher',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>midnightcrypt-secret-fetcher</h1>
