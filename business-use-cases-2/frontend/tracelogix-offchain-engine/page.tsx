...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACELOGIX_OFFCHAIN_ENGINE_ADDRESS || '';

export default function Tracelogix_Offchain_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tracelogix-offchain-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tracelogix-offchain-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tracelogix-offchain-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tracelogix-offchain-engine</h1>
