...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_XENOENGINEVAULT_SECRET_ADAPTER_ADDRESS || '';

export default function Xenoenginevault_Secret_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'xenoenginevault-secret-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.xenoenginevault-secret-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'xenoenginevault-secret-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>xenoenginevault-secret-adapter</h1>
