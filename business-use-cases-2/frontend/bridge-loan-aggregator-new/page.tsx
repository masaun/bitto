...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BRIDGE_LOAN_AGGREGATOR_NEW_ADDRESS || '';

export default function Bridge_Loan_Aggregator_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'bridge-loan-aggregator-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bridge-loan-aggregator-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bridge-loan-aggregator-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>bridge-loan-aggregator-new</h1>
