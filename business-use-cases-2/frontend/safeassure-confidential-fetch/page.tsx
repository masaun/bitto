...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SAFEASSURE_CONFIDENTIAL_FETCH_ADDRESS || '';

export default function Safeassure_Confidential_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'safeassure-confidential-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.safeassure-confidential-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'safeassure-confidential-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>safeassure-confidential-fetch</h1>
