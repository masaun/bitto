...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_FORECAST_HUB_ADDRESS || '';

export default function Fiscal_Forecast_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-forecast-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-forecast-hub',
