...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STABLECOIN_BASKET_ADDRESS || '';

export default function Stablecoin_BasketPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.stablecoin-basket/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'stablecoin-basket',
