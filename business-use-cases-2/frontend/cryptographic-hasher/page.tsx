...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CRYPTOGRAPHIC_HASHER_ADDRESS || '';

export default function Cryptographic_HasherPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cryptographic-hasher/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cryptographic-hasher',
