...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ENTROPY_SOURCE_V1_ADDRESS || '';

export default function Entropy_Source_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.entropy-source-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'entropy-source-v1',
