...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HORIZONFORGE_CONFIDENTIAL_STREAM_ADDRESS || '';

export default function Horizonforge_Confidential_StreamPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'horizonforge-confidential-stream',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.horizonforge-confidential-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'horizonforge-confidential-stream',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>horizonforge-confidential-stream</h1>
