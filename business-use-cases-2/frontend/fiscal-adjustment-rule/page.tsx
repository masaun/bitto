...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_ADJUSTMENT_RULE_ADDRESS || '';

export default function Fiscal_Adjustment_RulePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-adjustment-rule/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-adjustment-rule',
