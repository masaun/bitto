...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SANDSTORMGUARD_CONFIDENTIAL_PARSER_ADDRESS || '';

export default function Sandstormguard_Confidential_ParserPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'sandstormguard-confidential-parser',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sandstormguard-confidential-parser/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sandstormguard-confidential-parser',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>sandstormguard-confidential-parser</h1>
