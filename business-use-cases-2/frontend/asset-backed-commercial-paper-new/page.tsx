...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ASSET_BACKED_COMMERCIAL_PAPER_NEW_ADDRESS || '';

export default function Asset_Backed_Commercial_Paper_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'asset-backed-commercial-paper-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.asset-backed-commercial-paper-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'asset-backed-commercial-paper-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>asset-backed-commercial-paper-new</h1>
