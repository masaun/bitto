...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_UPTIME_PERFORMANCE_ADDRESS || '';

export default function Uptime_PerformancePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.uptime-performance/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'uptime-performance',
