...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_UNDERWRITER_SIGNATURE_ADDRESS || '';

export default function Underwriter_SignaturePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.underwriter-signature/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'underwriter-signature',
