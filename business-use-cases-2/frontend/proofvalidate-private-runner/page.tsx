...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROOFVALIDATE_PRIVATE_RUNNER_ADDRESS || '';

export default function Proofvalidate_Private_RunnerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'proofvalidate-private-runner',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.proofvalidate-private-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'proofvalidate-private-runner',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>proofvalidate-private-runner</h1>
