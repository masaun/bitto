...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PRICE_IMPACT_CALC_ADDRESS || '';

export default function Price_Impact_CalcPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.price-impact-calc/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'price-impact-calc',
