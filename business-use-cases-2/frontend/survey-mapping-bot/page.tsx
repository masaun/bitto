...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SURVEY_MAPPING_BOT_ADDRESS || '';

export default function Survey_Mapping_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.survey-mapping-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'survey-mapping-bot',
