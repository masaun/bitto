...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PINECRESTVAULT_PRIVATE_ADAPTER_ADDRESS || '';

export default function Pinecrestvault_Private_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'pinecrestvault-private-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.pinecrestvault-private-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'pinecrestvault-private-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>pinecrestvault-private-adapter</h1>
