...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SAFEGUARDIAN_CONFIDENTIAL_RUNNER_ADDRESS || '';

export default function Safeguardian_Confidential_RunnerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'safeguardian-confidential-runner',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.safeguardian-confidential-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'safeguardian-confidential-runner',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>safeguardian-confidential-runner</h1>
