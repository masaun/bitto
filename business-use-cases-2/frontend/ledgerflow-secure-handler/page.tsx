...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LEDGERFLOW_SECURE_HANDLER_ADDRESS || '';

export default function Ledgerflow_Secure_HandlerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'ledgerflow-secure-handler',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ledgerflow-secure-handler/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ledgerflow-secure-handler',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>ledgerflow-secure-handler</h1>
