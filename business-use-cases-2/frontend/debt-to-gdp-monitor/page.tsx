...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEBT_TO_GDP_MONITOR_ADDRESS || '';

export default function Debt_To_Gdp_MonitorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.debt-to-gdp-monitor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'debt-to-gdp-monitor',
