...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_SPACE_CALCULATOR_ADDRESS || '';

export default function Fiscal_Space_CalculatorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-space-calculator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-space-calculator',
