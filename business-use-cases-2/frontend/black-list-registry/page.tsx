...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BLACK_LIST_REGISTRY_ADDRESS || '';

export default function Black_List_RegistryPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.black-list-registry/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'black-list-registry',
