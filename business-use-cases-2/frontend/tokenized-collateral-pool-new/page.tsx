...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TOKENIZED_COLLATERAL_POOL_NEW_ADDRESS || '';

export default function Tokenized_Collateral_Pool_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tokenized-collateral-pool-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tokenized-collateral-pool-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tokenized-collateral-pool-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tokenized-collateral-pool-new</h1>
