...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERITRACKLINE_CONTROLLED_ENGINE_ADDRESS || '';

export default function Veritrackline_Controlled_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'veritrackline-controlled-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veritrackline-controlled-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veritrackline-controlled-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>veritrackline-controlled-engine</h1>
