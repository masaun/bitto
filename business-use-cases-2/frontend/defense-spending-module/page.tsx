...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEFENSE_SPENDING_MODULE_ADDRESS || '';

export default function Defense_Spending_ModulePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.defense-spending-module/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'defense-spending-module',
