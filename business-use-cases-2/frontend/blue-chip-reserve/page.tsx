...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BLUE_CHIP_RESERVE_ADDRESS || '';

export default function Blue_Chip_ReservePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.blue-chip-reserve/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'blue-chip-reserve',
