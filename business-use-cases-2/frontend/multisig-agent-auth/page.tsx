...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MULTISIG_AGENT_AUTH_ADDRESS || '';

export default function Multisig_Agent_AuthPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.multisig-agent-auth/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'multisig-agent-auth',
