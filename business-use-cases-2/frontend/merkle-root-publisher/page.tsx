...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MERKLE_ROOT_PUBLISHER_ADDRESS || '';

export default function Merkle_Root_PublisherPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.merkle-root-publisher/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'merkle-root-publisher',
