...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDITBEAM_PRIVATE_FETCH_ADDRESS || '';

export default function Auditbeam_Private_FetchPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auditbeam-private-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auditbeam-private-fetch',
