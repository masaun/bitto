...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROOFTRACKX_PRIVATE_ENGINE_ADDRESS || '';

export default function Prooftrackx_Private_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'prooftrackx-private-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.prooftrackx-private-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'prooftrackx-private-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>prooftrackx-private-engine</h1>
