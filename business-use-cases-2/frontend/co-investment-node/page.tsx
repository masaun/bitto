...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CO_INVESTMENT_NODE_ADDRESS || '';

export default function Co_Investment_NodePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.co-investment-node/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'co-investment-node',
