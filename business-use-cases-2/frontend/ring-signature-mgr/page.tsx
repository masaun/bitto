...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RING_SIGNATURE_MGR_ADDRESS || '';

export default function Ring_Signature_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ring-signature-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ring-signature-mgr',
