...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDITSPIRE_OFFCHAIN_RUNNER_ADDRESS || '';

export default function Auditspire_Offchain_RunnerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'auditspire-offchain-runner',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auditspire-offchain-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auditspire-offchain-runner',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>auditspire-offchain-runner</h1>
