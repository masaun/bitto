...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SBTC_RESERVE_CHECK_ADDRESS || '';

export default function Sbtc_Reserve_CheckPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sbtc-reserve-check/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sbtc-reserve-check',
