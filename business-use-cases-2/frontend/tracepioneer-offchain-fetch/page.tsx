...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACEPIONEER_OFFCHAIN_FETCH_ADDRESS || '';

export default function Tracepioneer_Offchain_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tracepioneer-offchain-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tracepioneer-offchain-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tracepioneer-offchain-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tracepioneer-offchain-fetch</h1>
