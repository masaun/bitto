...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BUDGET_LIABILITY_TRACKER_ADDRESS || '';

export default function Budget_Liability_TrackerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.budget-liability-tracker/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'budget-liability-tracker',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>budget-liability-tracker</h1>
