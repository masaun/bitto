...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_UPLINKVEIL_SECURE_STREAM_ADDRESS || '';

export default function Uplinkveil_Secure_StreamPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.uplinkveil-secure-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'uplinkveil-secure-stream',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>uplinkveil-secure-stream</h1>
