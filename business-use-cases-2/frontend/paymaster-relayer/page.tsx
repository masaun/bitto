...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PAYMASTER_RELAYER_ADDRESS || '';

export default function Paymaster_RelayerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.paymaster-relayer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'paymaster-relayer',
