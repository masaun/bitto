...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ZERO_KNOWLEDGE_V3_ADDRESS || '';

export default function Zero_Knowledge_V3Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.zero-knowledge-v3/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'zero-knowledge-v3',
