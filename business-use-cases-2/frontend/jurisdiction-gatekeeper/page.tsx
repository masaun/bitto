...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_JURISDICTION_GATEKEEPER_ADDRESS || '';

export default function Jurisdiction_GatekeeperPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.jurisdiction-gatekeeper/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'jurisdiction-gatekeeper',
