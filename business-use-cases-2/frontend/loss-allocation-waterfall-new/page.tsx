...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LOSS_ALLOCATION_WATERFALL_NEW_ADDRESS || '';

export default function Loss_Allocation_Waterfall_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'loss-allocation-waterfall-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.loss-allocation-waterfall-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'loss-allocation-waterfall-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>loss-allocation-waterfall-new</h1>
