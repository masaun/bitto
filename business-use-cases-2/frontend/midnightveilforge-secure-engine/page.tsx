...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MIDNIGHTVEILFORGE_SECURE_ENGINE_ADDRESS || '';

export default function Midnightveilforge_Secure_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'midnightveilforge-secure-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.midnightveilforge-secure-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'midnightveilforge-secure-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>midnightveilforge-secure-engine</h1>
