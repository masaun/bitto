...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_BUFFER_FUND_ADDRESS || '';

export default function Fiscal_Buffer_FundPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-buffer-fund/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-buffer-fund',
