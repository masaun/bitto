...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MUNICIPAL_BOND_ISSUER_ADDRESS || '';

export default function Municipal_Bond_IssuerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.municipal-bond-issuer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'municipal-bond-issuer',
