...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUTONOMOUS_TASK_ROUTER_ADDRESS || '';

export default function Autonomous_Task_RouterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.autonomous-task-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'autonomous-task-router',
