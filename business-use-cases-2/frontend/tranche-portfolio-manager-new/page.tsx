...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRANCHE_PORTFOLIO_MANAGER_NEW_ADDRESS || '';

export default function Tranche_Portfolio_Manager_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tranche-portfolio-manager-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tranche-portfolio-manager-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tranche-portfolio-manager-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tranche-portfolio-manager-new</h1>
