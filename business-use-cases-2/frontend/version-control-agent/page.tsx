...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERSION_CONTROL_AGENT_ADDRESS || '';

export default function Version_Control_AgentPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.version-control-agent/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'version-control-agent',
