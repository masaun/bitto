...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACEGUARD_AUDIT_RUNNER_ADDRESS || '';

export default function Traceguard_Audit_RunnerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.traceguard-audit-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'traceguard-audit-runner',
