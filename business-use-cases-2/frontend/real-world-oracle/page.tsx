...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REAL_WORLD_ORACLE_ADDRESS || '';

export default function Real_World_OraclePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.real-world-oracle/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'real-world-oracle',
