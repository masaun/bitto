...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERISTREAM_SECURE_GATEWAY_ADDRESS || '';

export default function Veristream_Secure_GatewayPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'veristream-secure-gateway',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veristream-secure-gateway/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veristream-secure-gateway',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>veristream-secure-gateway</h1>
