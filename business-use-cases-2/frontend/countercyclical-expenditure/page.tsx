...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COUNTERCYCLICAL_EXPENDITURE_ADDRESS || '';

export default function Countercyclical_ExpenditurePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'countercyclical-expenditure',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.countercyclical-expenditure/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'countercyclical-expenditure',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>countercyclical-expenditure</h1>
