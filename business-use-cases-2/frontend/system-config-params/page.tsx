...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SYSTEM_CONFIG_PARAMS_ADDRESS || '';

export default function System_Config_ParamsPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.system-config-params/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'system-config-params',
