...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROMPT_ENGINEERING_VAULT_ADDRESS || '';

export default function Prompt_Engineering_VaultPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.prompt-engineering-vault/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'prompt-engineering-vault',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>prompt-engineering-vault</h1>
