...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VAULTSHIELD_DATA_INGESTOR_ADDRESS || '';

export default function Vaultshield_Data_IngestorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'vaultshield-data-ingestor',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.vaultshield-data-ingestor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'vaultshield-data-ingestor',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>vaultshield-data-ingestor</h1>
