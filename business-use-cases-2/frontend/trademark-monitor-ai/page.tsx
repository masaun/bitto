...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRADEMARK_MONITOR_AI_ADDRESS || '';

export default function Trademark_Monitor_AiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.trademark-monitor-ai/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'trademark-monitor-ai',
