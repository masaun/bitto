...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_QUARTZFORGE_CONFIDENTIAL_FETCHER_ADDRESS || '';

export default function Quartzforge_Confidential_FetcherPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'quartzforge-confidential-fetcher',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.quartzforge-confidential-fetcher/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'quartzforge-confidential-fetcher',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>quartzforge-confidential-fetcher</h1>
