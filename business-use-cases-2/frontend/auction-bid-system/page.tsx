...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUCTION_BID_SYSTEM_ADDRESS || '';

export default function Auction_Bid_SystemPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auction-bid-system/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auction-bid-system',
