...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INSURANCE_FUND_AUD_ADDRESS || '';

export default function Insurance_Fund_AudPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.insurance-fund-aud/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'insurance-fund-aud',
