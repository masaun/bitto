...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACEPATH_SECURE_EXECUTOR_ADDRESS || '';

export default function Tracepath_Secure_ExecutorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tracepath-secure-executor',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tracepath-secure-executor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tracepath-secure-executor',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tracepath-secure-executor</h1>
