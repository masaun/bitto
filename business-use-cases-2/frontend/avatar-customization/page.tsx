...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AVATAR_CUSTOMIZATION_ADDRESS || '';

export default function Avatar_CustomizationPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.avatar-customization/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'avatar-customization',
