...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_YIELD_CURVE_ADAPTER_NEW_ADDRESS || '';

export default function Yield_Curve_Adapter_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.yield-curve-adapter-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'yield-curve-adapter-new',
