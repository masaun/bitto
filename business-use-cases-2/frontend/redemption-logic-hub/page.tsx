...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REDEMPTION_LOGIC_HUB_ADDRESS || '';

export default function Redemption_Logic_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.redemption-logic-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'redemption-logic-hub',
