...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ROYALTY_STREAM_BACKED_NEW_ADDRESS || '';

export default function Royalty_Stream_Backed_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'royalty-stream-backed-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.royalty-stream-backed-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'royalty-stream-backed-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>royalty-stream-backed-new</h1>
