...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_UNDERWRITING_ASSESSMENT_ENGINE_NEW_ADDRESS || '';

export default function Underwriting_Assessment_Engine_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'underwriting-assessment-engine-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.underwriting-assessment-engine-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'underwriting-assessment-engine-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>underwriting-assessment-engine-new</h1>
