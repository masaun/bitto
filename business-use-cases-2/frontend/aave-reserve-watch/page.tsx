...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AAVE_RESERVE_WATCH_ADDRESS || '';

export default function Aave_Reserve_WatchPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.aave-reserve-watch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'aave-reserve-watch',
