...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BTC_LIGHTNING_X402_ADDRESS || '';

export default function Btc_Lightning_X402Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.btc-lightning-x402/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'btc-lightning-x402',
