...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PARAMETRIC_TRIGGER_ADDRESS || '';

export default function Parametric_TriggerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.parametric-trigger/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'parametric-trigger',
