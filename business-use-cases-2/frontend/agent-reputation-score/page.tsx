...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AGENT_REPUTATION_SCORE_ADDRESS || '';

export default function Agent_Reputation_ScorePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.agent-reputation-score/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'agent-reputation-score',
