...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PORTFOLIO_SALE_ADAPTER_NEW_ADDRESS || '';

export default function Portfolio_Sale_Adapter_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'portfolio-sale-adapter-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.portfolio-sale-adapter-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'portfolio-sale-adapter-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>portfolio-sale-adapter-new</h1>
