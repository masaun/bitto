...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_KEYSTONECLOAK_DATA_BRIDGE_ADDRESS || '';

export default function Keystonecloak_Data_BridgePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'keystonecloak-data-bridge',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.keystonecloak-data-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'keystonecloak-data-bridge',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>keystonecloak-data-bridge</h1>
