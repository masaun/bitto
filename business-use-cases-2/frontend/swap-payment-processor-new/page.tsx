...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SWAP_PAYMENT_PROCESSOR_NEW_ADDRESS || '';

export default function Swap_Payment_Processor_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'swap-payment-processor-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.swap-payment-processor-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'swap-payment-processor-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>swap-payment-processor-new</h1>
