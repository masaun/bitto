...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NOTARITRACE_CONTROLLED_HUB_ADDRESS || '';

export default function Notaritrace_Controlled_HubPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'notaritrace-controlled-hub',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.notaritrace-controlled-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'notaritrace-controlled-hub',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>notaritrace-controlled-hub</h1>
