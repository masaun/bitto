...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUTONOMOUS_DELIVERY_ADDRESS || '';

export default function Autonomous_DeliveryPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.autonomous-delivery/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'autonomous-delivery',
