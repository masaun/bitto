...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ETA_CARBON_CREDIT_ADDRESS || '';

export default function Eta_Carbon_CreditPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.eta-carbon-credit/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'eta-carbon-credit',
