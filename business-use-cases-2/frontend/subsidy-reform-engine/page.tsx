...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SUBSIDY_REFORM_ENGINE_ADDRESS || '';

export default function Subsidy_Reform_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.subsidy-reform-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'subsidy-reform-engine',
