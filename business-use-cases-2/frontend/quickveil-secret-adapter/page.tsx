...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_QUICKVEIL_SECRET_ADAPTER_ADDRESS || '';

export default function Quickveil_Secret_AdapterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.quickveil-secret-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'quickveil-secret-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>quickveil-secret-adapter</h1>
