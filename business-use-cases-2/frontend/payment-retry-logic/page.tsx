...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PAYMENT_RETRY_LOGIC_ADDRESS || '';

export default function Payment_Retry_LogicPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.payment-retry-logic/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'payment-retry-logic',
