...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SENTINELPATH_TRACE_HUB_ADDRESS || '';

export default function Sentinelpath_Trace_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sentinelpath-trace-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sentinelpath-trace-hub',
