...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERIGUIDE_CONTROLLED_ROUTER_ADDRESS || '';

export default function Veriguide_Controlled_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'veriguide-controlled-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veriguide-controlled-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veriguide-controlled-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>veriguide-controlled-router</h1>
