...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REBASE_LOGIC_CORE_ADDRESS || '';

export default function Rebase_Logic_CorePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.rebase-logic-core/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'rebase-logic-core',
