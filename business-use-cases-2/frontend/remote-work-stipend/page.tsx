...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REMOTE_WORK_STIPEND_ADDRESS || '';

export default function Remote_Work_StipendPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.remote-work-stipend/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'remote-work-stipend',
