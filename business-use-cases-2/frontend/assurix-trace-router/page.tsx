...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ASSURIX_TRACE_ROUTER_ADDRESS || '';

export default function Assurix_Trace_RouterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.assurix-trace-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'assurix-trace-router',
