...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TREZOR_BRIDGE_MOD_ADDRESS || '';

export default function Trezor_Bridge_ModPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.trezor-bridge-mod/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'trezor-bridge-mod',
