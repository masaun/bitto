...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GOLDENVEILFORGE_SECURE_ROUTER_ADDRESS || '';

export default function Goldenveilforge_Secure_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'goldenveilforge-secure-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.goldenveilforge-secure-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'goldenveilforge-secure-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>goldenveilforge-secure-router</h1>
