...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BACKGROUND_CHECK_NODE_ADDRESS || '';

export default function Background_Check_NodePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.background-check-node/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'background-check-node',
