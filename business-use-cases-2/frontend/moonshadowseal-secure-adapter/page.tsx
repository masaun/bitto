...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MOONSHADOWSEAL_SECURE_ADAPTER_ADDRESS || '';

export default function Moonshadowseal_Secure_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'moonshadowseal-secure-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.moonshadowseal-secure-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'moonshadowseal-secure-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>moonshadowseal-secure-adapter</h1>
