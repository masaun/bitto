...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MARGIN_TRADING_V1_ADDRESS || '';

export default function Margin_Trading_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.margin-trading-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'margin-trading-v1',
