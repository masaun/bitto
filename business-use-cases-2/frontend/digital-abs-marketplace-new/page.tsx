...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DIGITAL_ABS_MARKETPLACE_NEW_ADDRESS || '';

export default function Digital_Abs_Marketplace_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'digital-abs-marketplace-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.digital-abs-marketplace-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'digital-abs-marketplace-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>digital-abs-marketplace-new</h1>
