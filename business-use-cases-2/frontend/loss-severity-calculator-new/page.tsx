...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LOSS_SEVERITY_CALCULATOR_NEW_ADDRESS || '';

export default function Loss_Severity_Calculator_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'loss-severity-calculator-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.loss-severity-calculator-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'loss-severity-calculator-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>loss-severity-calculator-new</h1>
