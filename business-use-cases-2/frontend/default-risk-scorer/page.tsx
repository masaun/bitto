...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEFAULT_RISK_SCORER_ADDRESS || '';

export default function Default_Risk_ScorerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.default-risk-scorer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'default-risk-scorer',
