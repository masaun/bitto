...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_POLY_STATE_RECEIVER_ADDRESS || '';

export default function Poly_State_ReceiverPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.poly-state-receiver/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'poly-state-receiver',
