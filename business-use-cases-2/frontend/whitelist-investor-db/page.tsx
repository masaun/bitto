...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WHITELIST_INVESTOR_DB_ADDRESS || '';

export default function Whitelist_Investor_DbPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.whitelist-investor-db/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'whitelist-investor-db',
