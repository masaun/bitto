...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SMART_WALLET_CORE_ADDRESS || '';

export default function Smart_Wallet_CorePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.smart-wallet-core/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'smart-wallet-core',
