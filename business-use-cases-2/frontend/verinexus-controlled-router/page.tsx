...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERINEXUS_CONTROLLED_ROUTER_ADDRESS || '';

export default function Verinexus_Controlled_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'verinexus-controlled-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.verinexus-controlled-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'verinexus-controlled-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>verinexus-controlled-router</h1>
