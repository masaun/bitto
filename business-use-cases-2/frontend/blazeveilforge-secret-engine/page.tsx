...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BLAZEVEILFORGE_SECRET_ENGINE_ADDRESS || '';

export default function Blazeveilforge_Secret_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'blazeveilforge-secret-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.blazeveilforge-secret-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'blazeveilforge-secret-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>blazeveilforge-secret-engine</h1>
