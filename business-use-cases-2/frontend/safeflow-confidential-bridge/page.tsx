...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SAFEFLOW_CONFIDENTIAL_BRIDGE_ADDRESS || '';

export default function Safeflow_Confidential_BridgePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'safeflow-confidential-bridge',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.safeflow-confidential-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'safeflow-confidential-bridge',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>safeflow-confidential-bridge</h1>
