...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STATE_ROOT_UPDATE_ADDRESS || '';

export default function State_Root_UpdatePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.state-root-update/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'state-root-update',
