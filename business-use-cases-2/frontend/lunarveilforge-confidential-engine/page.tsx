...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LUNARVEILFORGE_CONFIDENTIAL_ENGINE_ADDRESS || '';

export default function Lunarveilforge_Confidential_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'lunarveilforge-confidential-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.lunarveilforge-confidential-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'lunarveilforge-confidential-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>lunarveilforge-confidential-engine</h1>
