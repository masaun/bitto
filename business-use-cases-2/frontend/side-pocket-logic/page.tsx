...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SIDE_POCKET_LOGIC_ADDRESS || '';

export default function Side_Pocket_LogicPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.side-pocket-logic/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'side-pocket-logic',
