...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRANSFER_RESTRICTION_ADDRESS || '';

export default function Transfer_RestrictionPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.transfer-restriction/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'transfer-restriction',
