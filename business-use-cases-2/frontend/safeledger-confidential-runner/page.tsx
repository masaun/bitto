...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SAFELEDGER_CONFIDENTIAL_RUNNER_ADDRESS || '';

export default function Safeledger_Confidential_RunnerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'safeledger-confidential-runner',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.safeledger-confidential-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'safeledger-confidential-runner',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>safeledger-confidential-runner</h1>
