...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PI_SWAP_AGREEMENT_ADDRESS || '';

export default function Pi_Swap_AgreementPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.pi-swap-agreement/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'pi-swap-agreement',
