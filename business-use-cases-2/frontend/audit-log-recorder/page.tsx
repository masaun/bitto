...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDIT_LOG_RECORDER_ADDRESS || '';

export default function Audit_Log_RecorderPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.audit-log-recorder/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'audit-log-recorder',
