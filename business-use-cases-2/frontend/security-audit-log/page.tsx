...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SECURITY_AUDIT_LOG_ADDRESS || '';

export default function Security_Audit_LogPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.security-audit-log/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'security-audit-log',
