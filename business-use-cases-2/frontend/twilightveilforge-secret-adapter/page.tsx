...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TWILIGHTVEILFORGE_SECRET_ADAPTER_ADDRESS || '';

export default function Twilightveilforge_Secret_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'twilightveilforge-secret-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.twilightveilforge-secret-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'twilightveilforge-secret-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>twilightveilforge-secret-adapter</h1>
