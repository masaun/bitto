...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ACQUISITION_ESCROW_ADDRESS || '';

export default function Acquisition_EscrowPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.acquisition-escrow/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'acquisition-escrow',
