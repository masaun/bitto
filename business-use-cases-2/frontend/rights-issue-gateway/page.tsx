...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RIGHTS_ISSUE_GATEWAY_ADDRESS || '';

export default function Rights_Issue_GatewayPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.rights-issue-gateway/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'rights-issue-gateway',
