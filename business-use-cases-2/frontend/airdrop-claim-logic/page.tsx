...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AIRDROP_CLAIM_LOGIC_ADDRESS || '';

export default function Airdrop_Claim_LogicPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.airdrop-claim-logic/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'airdrop-claim-logic',
