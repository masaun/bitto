...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TREASURY_MANAGEMENT_V1_ADDRESS || '';

export default function Treasury_Management_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.treasury-management-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'treasury-management-v1',
