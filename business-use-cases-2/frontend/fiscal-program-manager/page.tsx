...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_PROGRAM_MANAGER_ADDRESS || '';

export default function Fiscal_Program_ManagerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-program-manager/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-program-manager',
