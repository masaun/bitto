...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LEGAL_CLAUSE_LIBRARY_ADDRESS || '';

export default function Legal_Clause_LibraryPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.legal-clause-library/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'legal-clause-library',
