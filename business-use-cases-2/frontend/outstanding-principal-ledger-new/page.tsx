...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OUTSTANDING_PRINCIPAL_LEDGER_NEW_ADDRESS || '';

export default function Outstanding_Principal_Ledger_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'outstanding-principal-ledger-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.outstanding-principal-ledger-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'outstanding-principal-ledger-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>outstanding-principal-ledger-new</h1>
