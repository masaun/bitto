...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RETAIL_NOTE_ACCESS_NEW_ADDRESS || '';

export default function Retail_Note_Access_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.retail-note-access-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'retail-note-access-new',
