...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DUTCH_AUCTION_BID_ADDRESS || '';

export default function Dutch_Auction_BidPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.dutch-auction-bid/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'dutch-auction-bid',
