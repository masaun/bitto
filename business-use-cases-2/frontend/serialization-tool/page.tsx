...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SERIALIZATION_TOOL_ADDRESS || '';

export default function Serialization_ToolPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.serialization-tool/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'serialization-tool',
