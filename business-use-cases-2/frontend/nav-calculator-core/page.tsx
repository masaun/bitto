...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NAV_CALCULATOR_CORE_ADDRESS || '';

export default function Nav_Calculator_CorePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.nav-calculator-core/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'nav-calculator-core',
