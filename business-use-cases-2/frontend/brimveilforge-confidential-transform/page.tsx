...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BRIMVEILFORGE_CONFIDENTIAL_TRANSFORM_ADDRESS || '';

export default function Brimveilforge_Confidential_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'brimveilforge-confidential-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.brimveilforge-confidential-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'brimveilforge-confidential-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>brimveilforge-confidential-transform</h1>
