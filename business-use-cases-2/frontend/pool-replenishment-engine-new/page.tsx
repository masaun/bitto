...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_POOL_REPLENISHMENT_ENGINE_NEW_ADDRESS || '';

export default function Pool_Replenishment_Engine_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'pool-replenishment-engine-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.pool-replenishment-engine-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'pool-replenishment-engine-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>pool-replenishment-engine-new</h1>
