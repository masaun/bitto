...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WHITELIST_VERIFIER_ADDRESS || '';

export default function Whitelist_VerifierPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.whitelist-verifier/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'whitelist-verifier',
