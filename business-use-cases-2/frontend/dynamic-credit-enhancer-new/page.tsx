...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DYNAMIC_CREDIT_ENHANCER_NEW_ADDRESS || '';

export default function Dynamic_Credit_Enhancer_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'dynamic-credit-enhancer-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.dynamic-credit-enhancer-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'dynamic-credit-enhancer-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>dynamic-credit-enhancer-new</h1>
