...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LEGAL_ENTITY_BINDING_ADDRESS || '';

export default function Legal_Entity_BindingPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.legal-entity-binding/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'legal-entity-binding',
