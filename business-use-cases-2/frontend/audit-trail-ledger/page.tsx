...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDIT_TRAIL_LEDGER_ADDRESS || '';

export default function Audit_Trail_LedgerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.audit-trail-ledger/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'audit-trail-ledger',
