...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OAKENSAFE_TRUSTED_HANDLER_ADDRESS || '';

export default function Oakensafe_Trusted_HandlerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'oakensafe-trusted-handler',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.oakensafe-trusted-handler/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'oakensafe-trusted-handler',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>oakensafe-trusted-handler</h1>
