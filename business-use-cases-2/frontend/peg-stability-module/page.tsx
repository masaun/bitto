...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PEG_STABILITY_MODULE_ADDRESS || '';

export default function Peg_Stability_ModulePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.peg-stability-module/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'peg-stability-module',
