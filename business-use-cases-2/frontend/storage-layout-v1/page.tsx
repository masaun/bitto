...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STORAGE_LAYOUT_V1_ADDRESS || '';

export default function Storage_Layout_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.storage-layout-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'storage-layout-v1',
