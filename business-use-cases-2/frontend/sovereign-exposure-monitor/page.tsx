...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SOVEREIGN_EXPOSURE_MONITOR_ADDRESS || '';

export default function Sovereign_Exposure_MonitorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'sovereign-exposure-monitor',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sovereign-exposure-monitor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sovereign-exposure-monitor',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>sovereign-exposure-monitor</h1>
