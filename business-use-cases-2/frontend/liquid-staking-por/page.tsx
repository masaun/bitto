...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LIQUID_STAKING_POR_ADDRESS || '';

export default function Liquid_Staking_PorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.liquid-staking-por/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'liquid-staking-por',
