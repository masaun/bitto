...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TOKEN_USAGE_AUDITOR_ADDRESS || '';

export default function Token_Usage_AuditorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.token-usage-auditor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'token-usage-auditor',
