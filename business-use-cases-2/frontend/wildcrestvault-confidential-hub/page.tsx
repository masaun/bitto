...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WILDCRESTVAULT_CONFIDENTIAL_HUB_ADDRESS || '';

export default function Wildcrestvault_Confidential_HubPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'wildcrestvault-confidential-hub',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.wildcrestvault-confidential-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'wildcrestvault-confidential-hub',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>wildcrestvault-confidential-hub</h1>
