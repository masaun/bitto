...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ASSET_BACKED_NOTE_VAULT_NEW_ADDRESS || '';

export default function Asset_Backed_Note_Vault_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'asset-backed-note-vault-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.asset-backed-note-vault-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'asset-backed-note-vault-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>asset-backed-note-vault-new</h1>
