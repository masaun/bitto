...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VIRTUAL_TOUR_STAGING_ADDRESS || '';

export default function Virtual_Tour_StagingPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.virtual-tour-staging/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'virtual-tour-staging',
