...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AFFORDABLE_HOUSING_AI_ADDRESS || '';

export default function Affordable_Housing_AiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.affordable-housing-ai/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'affordable-housing-ai',
