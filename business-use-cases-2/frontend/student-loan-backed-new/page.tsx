...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STUDENT_LOAN_BACKED_NEW_ADDRESS || '';

export default function Student_Loan_Backed_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.student-loan-backed-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'student-loan-backed-new',
