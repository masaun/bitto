...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CAPITAL_GAINS_TAX_MOD_ADDRESS || '';

export default function Capital_Gains_Tax_ModPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.capital-gains-tax-mod/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'capital-gains-tax-mod',
