...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TIMEOUT_HANDLER_V1_ADDRESS || '';

export default function Timeout_Handler_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.timeout-handler-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'timeout-handler-v1',
