...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WILDFIRECLOAK_SECRET_HANDLER_ADDRESS || '';

export default function Wildfirecloak_Secret_HandlerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'wildfirecloak-secret-handler',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.wildfirecloak-secret-handler/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'wildfirecloak-secret-handler',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>wildfirecloak-secret-handler</h1>
