...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RWA_VAULT_STORAGE_ADDRESS || '';

export default function Rwa_Vault_StoragePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.rwa-vault-storage/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'rwa-vault-storage',
