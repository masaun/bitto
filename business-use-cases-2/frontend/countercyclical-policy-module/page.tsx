...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COUNTERCYCLICAL_POLICY_MODULE_ADDRESS || '';

export default function Countercyclical_Policy_ModulePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'countercyclical-policy-module',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.countercyclical-policy-module/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'countercyclical-policy-module',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>countercyclical-policy-module</h1>
