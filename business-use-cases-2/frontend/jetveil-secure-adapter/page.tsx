...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_JETVEIL_SECURE_ADAPTER_ADDRESS || '';

export default function Jetveil_Secure_AdapterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.jetveil-secure-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'jetveil-secure-adapter',
