...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SECURITIZED_LOAN_MARKET_NEW_ADDRESS || '';

export default function Securitized_Loan_Market_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'securitized-loan-market-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.securitized-loan-market-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'securitized-loan-market-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>securitized-loan-market-new</h1>
