...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HORIZONVEILFORGE_CONFIDENTIAL_TRANSFORM_ADDRESS || '';

export default function Horizonveilforge_Confidential_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'horizonveilforge-confidential-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.horizonveilforge-confidential-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'horizonveilforge-confidential-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>horizonveilforge-confidential-transform</h1>
