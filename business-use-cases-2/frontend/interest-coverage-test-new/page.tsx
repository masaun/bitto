...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INTEREST_COVERAGE_TEST_NEW_ADDRESS || '';

export default function Interest_Coverage_Test_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'interest-coverage-test-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.interest-coverage-test-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'interest-coverage-test-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>interest-coverage-test-new</h1>
