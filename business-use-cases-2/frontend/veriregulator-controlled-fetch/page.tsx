...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERIREGULATOR_CONTROLLED_FETCH_ADDRESS || '';

export default function Veriregulator_Controlled_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'veriregulator-controlled-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veriregulator-controlled-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veriregulator-controlled-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>veriregulator-controlled-fetch</h1>
