...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BASE_CURRENCY_REF_ADDRESS || '';

export default function Base_Currency_RefPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.base-currency-ref/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'base-currency-ref',
