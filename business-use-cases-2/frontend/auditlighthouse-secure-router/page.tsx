...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUDITLIGHTHOUSE_SECURE_ROUTER_ADDRESS || '';

export default function Auditlighthouse_Secure_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'auditlighthouse-secure-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auditlighthouse-secure-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auditlighthouse-secure-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>auditlighthouse-secure-router</h1>
