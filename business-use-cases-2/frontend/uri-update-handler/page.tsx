...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_URI_UPDATE_HANDLER_ADDRESS || '';

export default function Uri_Update_HandlerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.uri-update-handler/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'uri-update-handler',
