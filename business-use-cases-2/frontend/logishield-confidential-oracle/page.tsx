...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LOGISHIELD_CONFIDENTIAL_ORACLE_ADDRESS || '';

export default function Logishield_Confidential_OraclePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'logishield-confidential-oracle',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.logishield-confidential-oracle/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'logishield-confidential-oracle',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>logishield-confidential-oracle</h1>
