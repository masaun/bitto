...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CARDANO_MILKOMEDA_ADDRESS || '';

export default function Cardano_MilkomedaPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cardano-milkomeda/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cardano-milkomeda',
