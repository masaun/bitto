...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INTERTEMPORAL_BUDGET_MODEL_ADDRESS || '';

export default function Intertemporal_Budget_ModelPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'intertemporal-budget-model',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.intertemporal-budget-model/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'intertemporal-budget-model',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>intertemporal-budget-model</h1>
