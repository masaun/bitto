...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ACCESSIBILITY_AUDIT_ADDRESS || '';

export default function Accessibility_AuditPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.accessibility-audit/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'accessibility-audit',
