...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ENTROPY_BEACON_API_ADDRESS || '';

export default function Entropy_Beacon_ApiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.entropy-beacon-api/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'entropy-beacon-api',
