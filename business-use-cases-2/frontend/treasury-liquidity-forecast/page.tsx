...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TREASURY_LIQUIDITY_FORECAST_ADDRESS || '';

export default function Treasury_Liquidity_ForecastPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'treasury-liquidity-forecast',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.treasury-liquidity-forecast/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'treasury-liquidity-forecast',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>treasury-liquidity-forecast</h1>
