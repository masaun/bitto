...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HEALTH_SAFETY_CHECKER_ADDRESS || '';

export default function Health_Safety_CheckerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.health-safety-checker/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'health-safety-checker',
