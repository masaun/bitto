...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DISTRIBUTION_WATERFALL_ADDRESS || '';

export default function Distribution_WaterfallPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.distribution-waterfall/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'distribution-waterfall',
