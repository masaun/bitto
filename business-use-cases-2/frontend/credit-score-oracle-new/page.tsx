...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CREDIT_SCORE_ORACLE_NEW_ADDRESS || '';

export default function Credit_Score_Oracle_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.credit-score-oracle-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'credit-score-oracle-new',
