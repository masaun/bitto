...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_JUNIOR_TRANCHE_DISTRIBUTOR_NEW_ADDRESS || '';

export default function Junior_Tranche_Distributor_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'junior-tranche-distributor-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.junior-tranche-distributor-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'junior-tranche-distributor-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>junior-tranche-distributor-new</h1>
