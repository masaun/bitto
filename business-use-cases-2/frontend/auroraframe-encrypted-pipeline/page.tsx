...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AURORAFRAME_ENCRYPTED_PIPELINE_ADDRESS || '';

export default function Auroraframe_Encrypted_PipelinePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'auroraframe-encrypted-pipeline',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auroraframe-encrypted-pipeline/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auroraframe-encrypted-pipeline',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>auroraframe-encrypted-pipeline</h1>
