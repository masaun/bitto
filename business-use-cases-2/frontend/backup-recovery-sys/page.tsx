...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BACKUP_RECOVERY_SYS_ADDRESS || '';

export default function Backup_Recovery_SysPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.backup-recovery-sys/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'backup-recovery-sys',
