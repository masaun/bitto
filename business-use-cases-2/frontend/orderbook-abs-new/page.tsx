...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ORDERBOOK_ABS_NEW_ADDRESS || '';

export default function Orderbook_Abs_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.orderbook-abs-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'orderbook-abs-new',
