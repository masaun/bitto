...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SOCIAL_RECOVERY_V2_ADDRESS || '';

export default function Social_Recovery_V2Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.social-recovery-v2/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'social-recovery-v2',
