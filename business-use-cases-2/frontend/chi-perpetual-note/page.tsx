...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CHI_PERPETUAL_NOTE_ADDRESS || '';

export default function Chi_Perpetual_NotePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.chi-perpetual-note/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'chi-perpetual-note',
