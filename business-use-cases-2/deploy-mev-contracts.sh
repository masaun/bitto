...
echo "=========================================="
echo "MEV/Privacy Contracts Deployment Script"
echo "300 Contracts to Mainnet"
echo "=========================================="
echo ""
...
cp Clarinet.toml Clarinet.toml.backup.$(date +%Y%m%d%H%M%S)
...
for CONTRACT in "${CONTRACTS[@]}"; do
  CURRENT=$((CURRENT + 1))
  echo "=========================================="
  echo "Deploying contract $CURRENT/$TOTAL: $CONTRACT"
  echo "=========================================="
  
  head -n 17 Clarinet.toml.backup.* | tail -1 > Clarinet.toml.temp
  
  echo "" >> Clarinet.toml.temp
  echo "[contracts.$CONTRACT]" >> Clarinet.toml.temp
  echo "path = \"contracts/${CONTRACT}.clar\"" >> Clarinet.toml.temp
  echo "clarity_version = 4" >> Clarinet.toml.temp
  ...
  CONTRACT_ADDRESS=$(clarinet deployments apply --mainnet 2>&1 | grep -oP "(?<=Contract deployed: ).*" | head -1)
  
  if [ -n "$CONTRACT_ADDRESS" ]; then
    ENV_VAR_NAME=$(echo "$CONTRACT" | tr '[:lower:]-' '[:upper:]_')
    echo "${ENV_VAR_NAME}_CONTRACT_ADDRESS=$CONTRACT_ADDRESS" >> "$ENV_FILE"
    echo "${ENV_VAR_NAME}_CONTRACT_ADDRESS=$CONTRACT_ADDRESS" >> "$FRONTEND_ENV_FILE"
...
echo "=========================================="
echo "All 300 MEV/Privacy contracts deployed!"
