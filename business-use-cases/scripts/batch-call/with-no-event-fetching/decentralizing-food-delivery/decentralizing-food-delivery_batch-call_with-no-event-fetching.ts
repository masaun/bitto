...
const contractDetails = parseContractIdentifier(
  process.env.DECENTRALIZING_FOOD_DELIVERY_CONTRACT_ADDRESS,
  'decentralizing-food-delivery'
);
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Decentralizing Food Delivery Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Decentralizing Food Delivery Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log(`Environment Variable: DECENTRALIZING_FOOD_DELIVERY_CONTRACT_ADDRESS`);

  if (!process.env.DECENTRALIZING_FOOD_DELIVERY_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: DECENTRALIZING_FOOD_DELIVERY_CONTRACT_ADDRESS not set in environment.');
