...
const contractDetails = parseContractIdentifier(
  process.env.MICROGRIDS_CONTRACT_ADDRESS,
  'microgrids'
);
...
function getWriteFunctions(senderAddress: string): FunctionCall[] {
  return [
    ...
    },
    {
      ...
      description: 'Join microgrid',
    },
    {
      name: 'contribute-energy',
      args: (index: number, sender: string) => [
        uintCV(1000000),
        uintCV(1000000),
      ],
      description: 'Contribute energy',
    },
    {
      name: 'toggle-island-mode',
      args: (index: number, sender: string) => [
        uintCV(1000000),
        boolCV(true),
      ],
      description: 'Toggle island mode',
    },
  ];
}
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Microgrids Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Microgrids Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log(`Environment Variable: MICROGRIDS_CONTRACT_ADDRESS`);

  if (!process.env.MICROGRIDS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: MICROGRIDS_CONTRACT_ADDRESS not set in environment.');
