...
const contractDetails = parseContractIdentifier(
  process.env.FIXED_WIRELESS_ACCESS_NETWORK_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Fixed Wireless Access Network Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Fixed Wireless Access Network Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log(`Environment Variable: FIXED_WIRELESS_ACCESS_NETWORK_CONTRACT_ADDRESS`);

  if (!process.env.FIXED_WIRELESS_ACCESS_NETWORK_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: FIXED_WIRELESS_ACCESS_NETWORK_CONTRACT_ADDRESS not set in environment.');
