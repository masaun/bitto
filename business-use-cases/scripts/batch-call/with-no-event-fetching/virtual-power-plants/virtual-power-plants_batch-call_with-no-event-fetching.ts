...
const contractDetails = parseContractIdentifier(
  process.env.VIRTUAL_POWER_PLANTS_CONTRACT_ADDRESS,
  ...
  return [
    {
      ...
      description: 'Create virtual power plant',
    },
    {
      ...
      description: 'Register energy device',
    },
    {
      ...
      description: 'Purchase energy',
    },
    {
      name: 'update-device-contribution',
      args: (index: number, sender: string) => [
        uintCV(1000000),
        uintCV(1000000),
        uintCV(1000000),
      ],
      description: 'Update device contribution',
    },
  ];
}
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Virtual Power Plants Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Virtual Power Plants Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log(`Environment Variable: VIRTUAL_POWER_PLANTS_CONTRACT_ADDRESS`);

  if (!process.env.VIRTUAL_POWER_PLANTS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: VIRTUAL_POWER_PLANTS_CONTRACT_ADDRESS not set in environment.');
