...
const contractDetails = parseContractIdentifier(
  process.env.CASH_FLOW_MODELING_CONTRACT_ADDRESS,
  'cash-flow-modeling'
);
  ...
  return [
    {
      ...
      description: 'Create cash flow model',
    },
    {
      name: 'add-cash-entry',
      args: (index: number, sender: string) => [
      ...
      description: 'Add cash entry to model',
    },
    {
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Cash Flow Modeling Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Cash Flow Modeling Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log(`Environment Variable: CASH_FLOW_MODELING_CONTRACT_ADDRESS`);

  if (!process.env.CASH_FLOW_MODELING_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CASH_FLOW_MODELING_CONTRACT_ADDRESS not set in environment.');
