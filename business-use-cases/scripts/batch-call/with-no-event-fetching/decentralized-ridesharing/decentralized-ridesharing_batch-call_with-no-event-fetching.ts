...
const contractDetails = parseContractIdentifier(
  process.env.DECENTRALIZED_RIDESHARING_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Decentralized Ridesharing Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Decentralized Ridesharing Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log(`Environment Variable: DECENTRALIZED_RIDESHARING_CONTRACT_ADDRESS`);

  if (!process.env.DECENTRALIZED_RIDESHARING_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: DECENTRALIZED_RIDESHARING_CONTRACT_ADDRESS not set in environment.');
