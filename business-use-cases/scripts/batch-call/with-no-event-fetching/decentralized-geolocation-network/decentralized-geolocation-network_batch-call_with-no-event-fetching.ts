...
const contractDetails = parseContractIdentifier(
  process.env.DECENTRALIZED_GEOLOCATION_NETWORK_CONTRACT_ADDRESS,
  'decentralized-geolocation-network'
);
  ...
  return [
    {
      ...
      description: 'Deploy location beacon',
    },
    {
    ...
    {
      name: 'update-beacon-location',
      args: (index: number, sender: string) => [
        uintCV(1000000),
        bufferCV(Buffer.from("000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f", "hex")),
        uintCV(1000000),
      ],
      description: 'Update beacon location',
    },
    {
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Decentralized Geolocation Network Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Decentralized Geolocation Network Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log(`Environment Variable: DECENTRALIZED_GEOLOCATION_NETWORK_CONTRACT_ADDRESS`);

  if (!process.env.DECENTRALIZED_GEOLOCATION_NETWORK_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: DECENTRALIZED_GEOLOCATION_NETWORK_CONTRACT_ADDRESS not set in environment.');
