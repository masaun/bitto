...
const contractDetails = parseContractIdentifier(
  process.env.CARBON_CREDIT_TOKENIZATION_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Carbon Credit Tokenization Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Carbon Credit Tokenization Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log(`Environment Variable: CARBON_CREDIT_TOKENIZATION_CONTRACT_ADDRESS`);

  if (!process.env.CARBON_CREDIT_TOKENIZATION_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CARBON_CREDIT_TOKENIZATION_CONTRACT_ADDRESS not set in environment.');
