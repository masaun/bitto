...
const contractDetails = parseContractIdentifier(
  process.env.INFRASTRUCTURE_DEBT_CONTRACT_ADDRESS,
  'infrastructure-debt'
);
  ...
  return [
    {
      ...
      args: (index: number, sender: string) => [
        principalCV(sender),
        uintCV(1000000 + (index * 10000)),
        uintCV(500 + (index * 10)),
        uintCV(100000 + (index * 1000)),
      ],
      description: 'Issue infrastructure debt',
    },
    {
      name: 'make-payment',
      args: (index: number) => [
      ...
      description: 'Make payment on debt',
    },
    {
      name: 'repay-full',
      args: (index: number) => [
      ...
      description: 'Repay full debt amount',
    },
    {
      name: 'mark-default',
      args: (index: number) => [
      ...
      description: 'Mark debt as defaulted',
    },
  ];
}
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Infrastructure Debt Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Infrastructure Debt Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log(`Environment Variable: INFRASTRUCTURE_DEBT_CONTRACT_ADDRESS`);

  if (!process.env.INFRASTRUCTURE_DEBT_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: INFRASTRUCTURE_DEBT_CONTRACT_ADDRESS not set in environment.');
