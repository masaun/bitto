...
const contractDetails = parseContractIdentifier(
  process.env.DCF_VALUATION_MODELING_CONTRACT_ADDRESS,
  ...
  return [
    {
      ...
      description: 'Create DCF valuation',
    },
    {
      name: 'add-cash-flow',
      args: (index: number, sender: string) => [
      ...
      description: 'Add cash flow to valuation',
    },
    {
      name: 'update-terminal-value',
      args: (index: number, sender: string) => [
        uintCV(1000000),
        uintCV(1000000),
      ],
      description: 'Update terminal value',
    },
    {
      name: 'finalize-valuation',
      args: (index: number, sender: string) => [
        uintCV(1000000),
      ],
      description: 'Finalize valuation',
    },
  ];
}
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Dcf Valuation Modeling Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Dcf Valuation Modeling Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log(`Environment Variable: DCF_VALUATION_MODELING_CONTRACT_ADDRESS`);

  if (!process.env.DCF_VALUATION_MODELING_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: DCF_VALUATION_MODELING_CONTRACT_ADDRESS not set in environment.');
