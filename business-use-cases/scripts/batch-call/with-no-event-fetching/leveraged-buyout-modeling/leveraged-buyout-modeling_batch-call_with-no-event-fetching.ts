...
const contractDetails = parseContractIdentifier(
  process.env.LEVERAGED_BUYOUT_MODELING_CONTRACT_ADDRESS,
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Leveraged Buyout Modeling Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Leveraged Buyout Modeling Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log(`Environment Variable: LEVERAGED_BUYOUT_MODELING_CONTRACT_ADDRESS`);

  if (!process.env.LEVERAGED_BUYOUT_MODELING_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LEVERAGED_BUYOUT_MODELING_CONTRACT_ADDRESS not set in environment.');
