...
const contractDetails = parseContractIdentifier(
  process.env.DECENTRALIZED_SATELLITE_NETWORK_CONTRACT_ADDRESS,
...
function getWriteFunctions(senderAddress: string): FunctionCall[] {
  return [
    ...
    },
    {
      ...
      description: 'Add ground station',
    },
    {
      name: 'transmit-data',
      args: (index: number, sender: string) => [
      ...
      description: 'Transmit satellite data',
    },
    {
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Decentralized Satellite Network Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Decentralized Satellite Network Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log(`Environment Variable: DECENTRALIZED_SATELLITE_NETWORK_CONTRACT_ADDRESS`);

  if (!process.env.DECENTRALIZED_SATELLITE_NETWORK_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: DECENTRALIZED_SATELLITE_NETWORK_CONTRACT_ADDRESS not set in environment.');
