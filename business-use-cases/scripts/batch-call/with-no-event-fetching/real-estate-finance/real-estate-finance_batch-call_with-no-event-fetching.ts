...
const contractDetails = parseContractIdentifier(
  process.env.REAL_ESTATE_FINANCE_CONTRACT_ADDRESS,
  'real-estate-finance'
);
  ...
  return [
    {
      ...
      args: (index: number) => [
        uintCV(5000000 + (index * 250000)),
        uintCV(4000000 + (index * 200000)),
        uintCV(1000000 + (index * 50000)),
        uintCV(450 + (index * 25)),
        uintCV(360000 + (index * 18000)),
      ],
    ...
    },
    {
      ...
      description: 'Finance property',
    },
    {
      name: 'make-payment',
      args: (index: number) => [
        uintCV(index + 1),
        uintCV(50000 + (index * 5000)),
      ],
    ...
    },
    {
      ...
      description: 'Refinance property',
    },
  ];
}
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Real Estate Finance Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Real Estate Finance Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log(`Environment Variable: REAL_ESTATE_FINANCE_CONTRACT_ADDRESS`);

  if (!process.env.REAL_ESTATE_FINANCE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: REAL_ESTATE_FINANCE_CONTRACT_ADDRESS not set in environment.');
