...
const CONTRACT_ADDRESS = process.env.ON_CHAIN_ETF_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'on-chain-etf';
...
export async function set_asset_weight() {
  const functionArgs = [
...
export async function get_asset_weight() {
  const response = await fetch(
    `https://api.mainnet.hiro.so/v2/contracts/call-read/${CONTRACT_ADDRESS.split('.')[0]}/${CONTRACT_NAME}/get-asset-weight`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
