...
const CONTRACT_ADDRESS = process.env.GENOMIC_DATA_REGISTRY_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'genomic-data-registry';

export async function connectWallet() {
  showConnect({
    appDetails: {
      name: 'Genomic Data Registry',
...
export async function update_access_level() {
  const functionArgs = [
  ...
  const options = {
    ...
    contractAddress: CONTRACT_ADDRESS.split('.')[0],
    contractName: CONTRACT_NAME,
    functionName: 'update-access-level',
    functionArgs,
    postConditionMode: PostConditionMode.Deny,
    postConditions: [],
    onFinish: (data: any) => {
