...
const CONTRACT_ADDRESS = process.env.RTK_NETWORK_FOR_AGRICULTURE_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'rtk-network-for-agriculture';

export async function connectWallet() {
  showConnect({
    appDetails: {
      name: 'Rtk Network For Agriculture',
...
export async function register_rtk_station() {
  const functionArgs = [
...
export async function subscribe_to_station() {
  const functionArgs = [
...
export async function update_station_status() {
  const functionArgs = [
