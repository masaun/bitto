...
const CONTRACT_ADDRESS = process.env.IDENTITY_DRIVEN_ATTENDANCE_EVENT_MARKETPLACES_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'identity-driven-attendance-event-marketplaces';

export async function connectWallet() {
  showConnect({
    appDetails: {
      name: 'Identity Driven Attendance Event Marketplaces',
...
export async function create_verified_event() {
  const functionArgs = [
  ...
  const options = {
    ...
    contractAddress: CONTRACT_ADDRESS.split('.')[0],
    contractName: CONTRACT_NAME,
    functionName: 'create-verified-event',
    functionArgs,
    postConditionMode: PostConditionMode.Deny,
    postConditions: [],
    onFinish: (data: any) => {
...
export async function purchase_verified_ticket() {
  const functionArgs = [
  ...
  const options = {
    ...
    contractAddress: CONTRACT_ADDRESS.split('.')[0],
    contractName: CONTRACT_NAME,
    functionName: 'purchase-verified-ticket',
    functionArgs,
    postConditionMode: PostConditionMode.Deny,
    postConditions: [],
    onFinish: (data: any) => {
...
export async function check_in_attendee() {
  const functionArgs = [
