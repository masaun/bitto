...
const CONTRACT_ADDRESS = process.env.ROBOT_TRAINING_DATA_MARKETPLACE_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'robot-training-data-marketplace';

export async function connectWallet() {
  showConnect({
    appDetails: {
      name: 'Robot Training Data Marketplace',
...
export async function list_dataset() {
  const functionArgs = [
...
export async function submit_quality_review() {
  const functionArgs = [
  ...
  const options = {
    ...
    contractAddress: CONTRACT_ADDRESS.split('.')[0],
    contractName: CONTRACT_NAME,
    functionName: 'submit-quality-review',
    functionArgs,
    postConditionMode: PostConditionMode.Deny,
    postConditions: [],
    onFinish: (data: any) => {
...
export async function get_quality_review() {
  const response = await fetch(
    `https://api.mainnet.hiro.so/v2/contracts/call-read/${CONTRACT_ADDRESS.split('.')[0]}/${CONTRACT_NAME}/get-quality-review`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
