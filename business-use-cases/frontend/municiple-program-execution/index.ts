...
const CONTRACT_ADDRESS = process.env.MUNICIPLE_PROGRAM_EXECUTION_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'municiple-program-execution';
...
export async function record_expenditure() {
  const functionArgs = [
...
export async function update_program_status() {
  const functionArgs = [
...
export async function get_expenditure() {
  const response = await fetch(
    `https://api.mainnet.hiro.so/v2/contracts/call-read/${CONTRACT_ADDRESS.split('.')[0]}/${CONTRACT_NAME}/get-expenditure`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
