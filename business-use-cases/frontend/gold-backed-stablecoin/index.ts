...
const CONTRACT_ADDRESS = process.env.GOLD_BACKED_STABLECOIN_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'gold-backed-stablecoin';
...
export async function update_gold_reserves() {
  const functionArgs = [
...
export async function remove_auditor() {
  const functionArgs = [
...
export async function get_gold_reserves)() {
  const response = await fetch(
    `https://api.mainnet.hiro.so/v2/contracts/call-read/${CONTRACT_ADDRESS.split('.')[0]}/${CONTRACT_NAME}/get-gold-reserves)`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
export async function is_auditor() {
  const response = await fetch(
    `https://api.mainnet.hiro.so/v2/contracts/call-read/${CONTRACT_ADDRESS.split('.')[0]}/${CONTRACT_NAME}/is-auditor`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
