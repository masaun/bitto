...
const CONTRACT_ADDRESS = process.env.DECENTRALIZED_BANDWIDTH_ACCESS_MARKET_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'decentralized-bandwidth-access-market';

export async function connectWallet() {
  showConnect({
    appDetails: {
      name: 'Decentralized Bandwidth Access Market',
...
export async function purchase_bandwidth() {
  const functionArgs = [
  ...
  const options = {
    ...
    contractAddress: CONTRACT_ADDRESS.split('.')[0],
    contractName: CONTRACT_NAME,
    functionName: 'purchase-bandwidth',
    functionArgs,
    postConditionMode: PostConditionMode.Deny,
    postConditions: [],
    onFinish: (data: any) => {
