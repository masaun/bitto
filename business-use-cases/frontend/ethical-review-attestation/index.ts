...
const CONTRACT_ADDRESS = process.env.ETHICAL_REVIEW_ATTESTATION_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'ethical-review-attestation';

export async function connectWallet() {
  showConnect({
    appDetails: {
      name: 'Ethical Review Attestation',
...
export async function record_approval() {
  const functionArgs = [
...
export async function get_review() {
  const response = await fetch(
    `https://api.mainnet.hiro.so/v2/contracts/call-read/${CONTRACT_ADDRESS.split('.')[0]}/${CONTRACT_NAME}/get-review`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
