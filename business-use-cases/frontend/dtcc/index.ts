...
const CONTRACT_ADDRESS = process.env.DTCC_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'dtcc';
...
export async function register_asset() {
  const functionArgs = [
...
export async function tokenize_asset() {
  const functionArgs = [
...
export async function transfer_asset() {
  const functionArgs = [
...
export async function get_asset_info() {
  const response = await fetch(
    `https://api.mainnet.hiro.so/v2/contracts/call-read/${CONTRACT_ADDRESS.split('.')[0]}/${CONTRACT_NAME}/get-asset-info`,
    {
      method: 'POST',
      headers: {
