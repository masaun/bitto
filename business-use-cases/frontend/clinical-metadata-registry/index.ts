...
const CONTRACT_ADDRESS = process.env.CLINICAL_METADATA_REGISTRY_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'clinical-metadata-registry';

export async function connectWallet() {
  showConnect({
    appDetails: {
      name: 'Clinical Metadata Registry',
...
export async function register_metadata() {
  const functionArgs = [
  ...
  const options = {
    ...
    contractAddress: CONTRACT_ADDRESS.split('.')[0],
    contractName: CONTRACT_NAME,
    functionName: 'register-metadata',
    functionArgs,
    postConditionMode: PostConditionMode.Deny,
    postConditions: [],
    onFinish: (data: any) => {
...
export async function get_metadata() {
  const response = await fetch(
    `https://api.mainnet.hiro.so/v2/contracts/call-read/${CONTRACT_ADDRESS.split('.')[0]}/${CONTRACT_NAME}/get-metadata`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
