...
const CONTRACT_ADDRESS = process.env.DONOR_CONSENT_ATTESTATION_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'donor-consent-attestation';

export async function connectWallet() {
  showConnect({
    appDetails: {
      name: 'Donor Consent Attestation',
...
export async function record_consent() {
  const functionArgs = [
...
export async function revoke_consent() {
  const functionArgs = [
...
export async function get_consent() {
  const response = await fetch(
    `https://api.mainnet.hiro.so/v2/contracts/call-read/${CONTRACT_ADDRESS.split('.')[0]}/${CONTRACT_NAME}/get-consent`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
