...
const CONTRACT_ADDRESS = process.env.OUTCOME_LINKED_PUBLIC_SPENDING_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'outcome-linked-public-spending';
...
export async function record_spending() {
  const functionArgs = [
...
export async function add_verifier() {
  const functionArgs = [
...
export async function remove_verifier() {
  const functionArgs = [
...
export async function is_verifier() {
  const response = await fetch(
    `https://api.mainnet.hiro.so/v2/contracts/call-read/${CONTRACT_ADDRESS.split('.')[0]}/${CONTRACT_NAME}/is-verifier`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
