...
const CONTRACT_ADDRESS = process.env.AID_DISTRIBUTION_MANAGEMENT_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'aid-distribution-management';

export async function connectWallet() {
  showConnect({
    appDetails: {
      name: 'Aid Distribution Management',
...
export async function distribute_aid() {
  const functionArgs = [
...
export async function get_distribution() {
  const response = await fetch(
    `https://api.mainnet.hiro.so/v2/contracts/call-read/${CONTRACT_ADDRESS.split('.')[0]}/${CONTRACT_NAME}/get-distribution`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
