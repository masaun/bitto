...
const CONTRACT_ADDRESS = process.env.SAMPLE_REGISTRY_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'sample-registry';
...
export async function register_sample() {
  const functionArgs = [
...
export async function update_sample_status() {
  const functionArgs = [
  ...
  const options = {
    ...
    contractAddress: CONTRACT_ADDRESS.split('.')[0],
    contractName: CONTRACT_NAME,
    functionName: 'update-sample-status',
    functionArgs,
    postConditionMode: PostConditionMode.Deny,
    postConditions: [],
    onFinish: (data: any) => {
...
export async function get_sample() {
  const response = await fetch(
    `https://api.mainnet.hiro.so/v2/contracts/call-read/${CONTRACT_ADDRESS.split('.')[0]}/${CONTRACT_NAME}/get-sample`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
