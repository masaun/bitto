...
const CONTRACT_ADDRESS = process.env.DISPUTE_RESOLUTION_BIOBANK_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'dispute-resolution-biobank';

export async function connectWallet() {
  showConnect({
    appDetails: {
      name: 'Dispute Resolution Biobank',
...
export async function file_dispute() {
  const functionArgs = [
...
export async function get_dispute() {
  const response = await fetch(
    `https://api.mainnet.hiro.so/v2/contracts/call-read/${CONTRACT_ADDRESS.split('.')[0]}/${CONTRACT_NAME}/get-dispute`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
