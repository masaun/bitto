...
(define-public (fund-loan (loan-id uint))
  (let
    ...
    (map-set loans loan-id (merge loan {funded: true, active: true, start-block: stacks-stacks-block-height}))
    (ok true)
  )
)
...
(define-public (make-repayment (loan-id uint) (amount uint))
  (let
    ...
    (map-set loan-payments {loan-id: loan-id, payment-id: pid} {amount: amount, block: stacks-stacks-block-height, type: "repayment"})
...
(define-read-only (calculate-payoff (loan-id uint))
  (let
    (
      ...
      (elapsed (- stacks-stacks-block-height (get start-block loan)))
    )
