...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BUDGET_ADJUSTMENT_ENGINE_ADDRESS || '';

export default function Budget_Adjustment_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.budget-adjustment-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'budget-adjustment-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>budget-adjustment-engine</h1>
