...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CIRCUIT_BREAKER_MOD_ADDRESS || '';

export default function Circuit_Breaker_ModPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.circuit-breaker-mod/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'circuit-breaker-mod',
