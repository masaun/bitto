...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CARBON_FOOTPRINT_DB_ADDRESS || '';

export default function Carbon_Footprint_DbPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.carbon-footprint-db/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'carbon-footprint-db',
