...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BANKRUPTCY_REMOTE_SPV_ADDRESS || '';

export default function Bankruptcy_Remote_SpvPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bankruptcy-remote-spv/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bankruptcy-remote-spv',
