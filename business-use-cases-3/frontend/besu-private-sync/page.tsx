...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BESU_PRIVATE_SYNC_ADDRESS || '';

export default function Besu_Private_SyncPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.besu-private-sync/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'besu-private-sync',
