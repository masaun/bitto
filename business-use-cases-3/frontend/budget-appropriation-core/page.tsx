...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BUDGET_APPROPRIATION_CORE_ADDRESS || '';

export default function Budget_Appropriation_CorePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'budget-appropriation-core',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.budget-appropriation-core/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'budget-appropriation-core',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>budget-appropriation-core</h1>
