...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CAPITAL_BUDGET_FRAMEWORK_ADDRESS || '';

export default function Capital_Budget_FrameworkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.capital-budget-framework/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'capital-budget-framework',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>capital-budget-framework</h1>
