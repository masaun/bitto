...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BLIND_AUCTION_BID_ADDRESS || '';

export default function Blind_Auction_BidPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.blind-auction-bid/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'blind-auction-bid',
