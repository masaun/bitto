...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CHORUS_ONE_ADAPTER_ADDRESS || '';

export default function Chorus_One_AdapterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.chorus-one-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'chorus-one-adapter',
