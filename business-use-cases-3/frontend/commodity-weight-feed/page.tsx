...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COMMODITY_WEIGHT_FEED_ADDRESS || '';

export default function Commodity_Weight_FeedPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.commodity-weight-feed/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'commodity-weight-feed',
