...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BUDGET_DEFICIT_FINANCER_ADDRESS || '';

export default function Budget_Deficit_FinancerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.budget-deficit-financer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'budget-deficit-financer',
