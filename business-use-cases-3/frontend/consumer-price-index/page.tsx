...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CONSUMER_PRICE_INDEX_ADDRESS || '';

export default function Consumer_Price_IndexPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.consumer-price-index/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'consumer-price-index',
