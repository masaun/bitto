...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BOND_YIELD_TRACKER_ADDRESS || '';

export default function Bond_Yield_TrackerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bond-yield-tracker/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bond-yield-tracker',
