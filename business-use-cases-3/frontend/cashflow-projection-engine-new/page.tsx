...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CASHFLOW_PROJECTION_ENGINE_NEW_ADDRESS || '';

export default function Cashflow_Projection_Engine_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'cashflow-projection-engine-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cashflow-projection-engine-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cashflow-projection-engine-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>cashflow-projection-engine-new</h1>
