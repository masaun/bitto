...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BENEFICIAL_OWNER_DB_ADDRESS || '';

export default function Beneficial_Owner_DbPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.beneficial-owner-db/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'beneficial-owner-db',
