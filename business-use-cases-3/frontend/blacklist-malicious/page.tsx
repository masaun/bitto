...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BLACKLIST_MALICIOUS_ADDRESS || '';

export default function Blacklist_MaliciousPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.blacklist-malicious/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'blacklist-malicious',
