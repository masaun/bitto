...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COMPLIANCE_CERTIFICATION_LEDGER_NEW_ADDRESS || '';

export default function Compliance_Certification_Ledger_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'compliance-certification-ledger-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.compliance-certification-ledger-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'compliance-certification-ledger-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>compliance-certification-ledger-new</h1>
