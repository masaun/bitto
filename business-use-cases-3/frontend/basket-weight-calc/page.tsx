...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BASKET_WEIGHT_CALC_ADDRESS || '';

export default function Basket_Weight_CalcPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.basket-weight-calc/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'basket-weight-calc',
