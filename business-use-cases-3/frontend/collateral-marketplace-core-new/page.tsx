...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COLLATERAL_MARKETPLACE_CORE_NEW_ADDRESS || '';

export default function Collateral_Marketplace_Core_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'collateral-marketplace-core-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.collateral-marketplace-core-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'collateral-marketplace-core-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>collateral-marketplace-core-new</h1>
