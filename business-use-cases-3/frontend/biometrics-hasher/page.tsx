...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BIOMETRICS_HASHER_ADDRESS || '';

export default function Biometrics_HasherPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.biometrics-hasher/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'biometrics-hasher',
