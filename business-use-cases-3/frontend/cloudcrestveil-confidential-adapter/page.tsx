...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CLOUDCRESTVEIL_CONFIDENTIAL_ADAPTER_ADDRESS || '';

export default function Cloudcrestveil_Confidential_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'cloudcrestveil-confidential-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cloudcrestveil-confidential-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cloudcrestveil-confidential-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>cloudcrestveil-confidential-adapter</h1>
