...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BUDGET_CAP_ENFORCER_ADDRESS || '';

export default function Budget_Cap_EnforcerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.budget-cap-enforcer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'budget-cap-enforcer',
