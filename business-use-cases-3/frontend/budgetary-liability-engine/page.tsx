...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BUDGETARY_LIABILITY_ENGINE_ADDRESS || '';

export default function Budgetary_Liability_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'budgetary-liability-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.budgetary-liability-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'budgetary-liability-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>budgetary-liability-engine</h1>
