...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CARBON_CREDIT_MINTER_ADDRESS || '';

export default function Carbon_Credit_MinterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.carbon-credit-minter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'carbon-credit-minter',
