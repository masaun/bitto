...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BLAZECRYPT_SECURE_COLLECTOR_ADDRESS || '';

export default function Blazecrypt_Secure_CollectorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'blazecrypt-secure-collector',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.blazecrypt-secure-collector/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'blazecrypt-secure-collector',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>blazecrypt-secure-collector</h1>
