...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COMMUNITY_POOL_CHK_ADDRESS || '';

export default function Community_Pool_ChkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.community-pool-chk/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'community-pool-chk',
