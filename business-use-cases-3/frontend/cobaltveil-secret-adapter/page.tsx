...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COBALTVEIL_SECRET_ADAPTER_ADDRESS || '';

export default function Cobaltveil_Secret_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'cobaltveil-secret-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cobaltveil-secret-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cobaltveil-secret-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>cobaltveil-secret-adapter</h1>
