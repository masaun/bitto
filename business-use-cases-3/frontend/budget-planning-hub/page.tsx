...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BUDGET_PLANNING_HUB_ADDRESS || '';

export default function Budget_Planning_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.budget-planning-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'budget-planning-hub',
