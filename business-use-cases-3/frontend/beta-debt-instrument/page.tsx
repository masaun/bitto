...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BETA_DEBT_INSTRUMENT_ADDRESS || '';

export default function Beta_Debt_InstrumentPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.beta-debt-instrument/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'beta-debt-instrument',
