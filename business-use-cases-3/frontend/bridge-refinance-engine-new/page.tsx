...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BRIDGE_REFINANCE_ENGINE_NEW_ADDRESS || '';

export default function Bridge_Refinance_Engine_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'bridge-refinance-engine-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bridge-refinance-engine-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bridge-refinance-engine-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>bridge-refinance-engine-new</h1>
