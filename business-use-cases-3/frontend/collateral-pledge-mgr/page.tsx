...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COLLATERAL_PLEDGE_MGR_ADDRESS || '';

export default function Collateral_Pledge_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.collateral-pledge-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'collateral-pledge-mgr',
