...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CASHFLOW_WATERFALL_ROUTER_NEW_ADDRESS || '';

export default function Cashflow_Waterfall_Router_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'cashflow-waterfall-router-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cashflow-waterfall-router-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cashflow-waterfall-router-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>cashflow-waterfall-router-new</h1>
