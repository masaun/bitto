...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COLLATERALIZED_LIQUIDITY_NEW_ADDRESS || '';

export default function Collateralized_Liquidity_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'collateralized-liquidity-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.collateralized-liquidity-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'collateralized-liquidity-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>collateralized-liquidity-new</h1>
