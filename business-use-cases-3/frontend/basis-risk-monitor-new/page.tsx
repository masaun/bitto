...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BASIS_RISK_MONITOR_NEW_ADDRESS || '';

export default function Basis_Risk_Monitor_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.basis-risk-monitor-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'basis-risk-monitor-new',
