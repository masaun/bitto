...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BUNDLER_AGGREGATOR_ADDRESS || '';

export default function Bundler_AggregatorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bundler-aggregator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bundler-aggregator',
