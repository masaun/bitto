...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CONSENSUS_MECHANISM_ADDRESS || '';

export default function Consensus_MechanismPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.consensus-mechanism/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'consensus-mechanism',
