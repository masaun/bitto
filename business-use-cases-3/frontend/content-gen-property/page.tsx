...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CONTENT_GEN_PROPERTY_ADDRESS || '';

export default function Content_Gen_PropertyPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.content-gen-property/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'content-gen-property',
