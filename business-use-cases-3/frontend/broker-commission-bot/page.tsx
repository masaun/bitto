...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BROKER_COMMISSION_BOT_ADDRESS || '';

export default function Broker_Commission_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.broker-commission-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'broker-commission-bot',
