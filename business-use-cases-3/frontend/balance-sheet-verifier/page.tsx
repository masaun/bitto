...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BALANCE_SHEET_VERIFIER_ADDRESS || '';

export default function Balance_Sheet_VerifierPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.balance-sheet-verifier/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'balance-sheet-verifier',
