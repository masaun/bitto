...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BOUNTY_HUNTING_LOG_ADDRESS || '';

export default function Bounty_Hunting_LogPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bounty-hunting-log/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bounty-hunting-log',
