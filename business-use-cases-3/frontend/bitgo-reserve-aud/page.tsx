...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BITGO_RESERVE_AUD_ADDRESS || '';

export default function Bitgo_Reserve_AudPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bitgo-reserve-aud/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bitgo-reserve-aud',
