...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CONTRACTIONARY_POLICY_MODULE_ADDRESS || '';

export default function Contractionary_Policy_ModulePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'contractionary-policy-module',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.contractionary-policy-module/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'contractionary-policy-module',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>contractionary-policy-module</h1>
