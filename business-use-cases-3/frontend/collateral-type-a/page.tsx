...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COLLATERAL_TYPE_A_ADDRESS || '';

export default function Collateral_Type_APage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.collateral-type-a/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'collateral-type-a',
