...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BUDGETARY_BALANCE_ENGINE_ADDRESS || '';

export default function Budgetary_Balance_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.budgetary-balance-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'budgetary-balance-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>budgetary-balance-engine</h1>
