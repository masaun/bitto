...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BLOCK_SPACE_MARKET_ADDRESS || '';

export default function Block_Space_MarketPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.block-space-market/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'block-space-market',
