...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COLLATERAL_VALUATION_ORACLE_NEW_ADDRESS || '';

export default function Collateral_Valuation_Oracle_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'collateral-valuation-oracle-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.collateral-valuation-oracle-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'collateral-valuation-oracle-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>collateral-valuation-oracle-new</h1>
