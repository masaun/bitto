...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CLAIM_ADJUSTER_V1_ADDRESS || '';

export default function Claim_Adjuster_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.claim-adjuster-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'claim-adjuster-v1',
