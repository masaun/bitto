...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BANKRUPTCY_REMOTE_STRUCTURE_NEW_ADDRESS || '';

export default function Bankruptcy_Remote_Structure_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'bankruptcy-remote-structure-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bankruptcy-remote-structure-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bankruptcy-remote-structure-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>bankruptcy-remote-structure-new</h1>
