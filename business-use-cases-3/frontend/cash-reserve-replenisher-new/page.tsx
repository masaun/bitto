...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CASH_RESERVE_REPLENISHER_NEW_ADDRESS || '';

export default function Cash_Reserve_Replenisher_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'cash-reserve-replenisher-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cash-reserve-replenisher-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cash-reserve-replenisher-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>cash-reserve-replenisher-new</h1>
