...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BENCHMARK_RATE_ORACLE_NEW_ADDRESS || '';

export default function Benchmark_Rate_Oracle_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'benchmark-rate-oracle-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.benchmark-rate-oracle-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'benchmark-rate-oracle-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>benchmark-rate-oracle-new</h1>
