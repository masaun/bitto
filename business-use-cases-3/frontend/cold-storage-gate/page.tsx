...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COLD_STORAGE_GATE_ADDRESS || '';

export default function Cold_Storage_GatePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cold-storage-gate/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cold-storage-gate',
