...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CERULEANVEIL_SECURE_ADAPTER_ADDRESS || '';

export default function Ceruleanveil_Secure_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'ceruleanveil-secure-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ceruleanveil-secure-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ceruleanveil-secure-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>ceruleanveil-secure-adapter</h1>
