...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BRIDGE_INFLOW_OUT_ADDRESS || '';

export default function Bridge_Inflow_OutPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bridge-inflow-out/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bridge-inflow-out',
