...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BLOCK_HEIGHT_FILTER_ADDRESS || '';

export default function Block_Height_FilterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.block-height-filter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'block-height-filter',
