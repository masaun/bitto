...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BULLETPROOF_RANGE_ADDRESS || '';

export default function Bulletproof_RangePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bulletproof-range/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bulletproof-range',
