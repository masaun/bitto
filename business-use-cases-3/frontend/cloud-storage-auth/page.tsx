...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CLOUD_STORAGE_AUTH_ADDRESS || '';

export default function Cloud_Storage_AuthPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cloud-storage-auth/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cloud-storage-auth',
