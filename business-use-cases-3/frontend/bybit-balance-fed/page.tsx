...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BYBIT_BALANCE_FED_ADDRESS || '';

export default function Bybit_Balance_FedPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.bybit-balance-fed/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'bybit-balance-fed',
