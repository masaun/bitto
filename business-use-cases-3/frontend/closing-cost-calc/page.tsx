...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CLOSING_COST_CALC_ADDRESS || '';

export default function Closing_Cost_CalcPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.closing-cost-calc/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'closing-cost-calc',
