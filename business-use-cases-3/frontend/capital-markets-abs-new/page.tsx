...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CAPITAL_MARKETS_ABS_NEW_ADDRESS || '';

export default function Capital_Markets_Abs_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.capital-markets-abs-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'capital-markets-abs-new',
