...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BUDGET_CYCLE_ENGINE_ADDRESS || '';

export default function Budget_Cycle_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.budget-cycle-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'budget-cycle-engine',
