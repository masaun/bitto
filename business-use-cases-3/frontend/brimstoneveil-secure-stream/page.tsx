...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BRIMSTONEVEIL_SECURE_STREAM_ADDRESS || '';

export default function Brimstoneveil_Secure_StreamPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'brimstoneveil-secure-stream',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.brimstoneveil-secure-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'brimstoneveil-secure-stream',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>brimstoneveil-secure-stream</h1>
