...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CHRONOLOCK_TRACE_ADAPTER_ADDRESS || '';

export default function Chronolock_Trace_AdapterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.chronolock-trace-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'chronolock-trace-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>chronolock-trace-adapter</h1>
