...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CLEAN_UP_CALL_TRIGGER_NEW_ADDRESS || '';

export default function Clean_Up_Call_Trigger_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'clean-up-call-trigger-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.clean-up-call-trigger-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'clean-up-call-trigger-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>clean-up-call-trigger-new</h1>
