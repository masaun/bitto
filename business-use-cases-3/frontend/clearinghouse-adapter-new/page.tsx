...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CLEARINGHOUSE_ADAPTER_NEW_ADDRESS || '';

export default function Clearinghouse_Adapter_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'clearinghouse-adapter-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.clearinghouse-adapter-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'clearinghouse-adapter-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>clearinghouse-adapter-new</h1>
