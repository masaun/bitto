...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CONSENT_SOLICITATION_ENGINE_NEW_ADDRESS || '';

export default function Consent_Solicitation_Engine_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'consent-solicitation-engine-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.consent-solicitation-engine-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'consent-solicitation-engine-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>consent-solicitation-engine-new</h1>
