...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_BRIGHTFORGE_SECURE_BRIDGE_ADDRESS || '';

export default function Brightforge_Secure_BridgePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'brightforge-secure-bridge',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.brightforge-secure-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'brightforge-secure-bridge',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>brightforge-secure-bridge</h1>
