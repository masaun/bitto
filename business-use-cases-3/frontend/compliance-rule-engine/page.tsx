...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COMPLIANCE_RULE_ENGINE_ADDRESS || '';

export default function Compliance_Rule_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.compliance-rule-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'compliance-rule-engine',
