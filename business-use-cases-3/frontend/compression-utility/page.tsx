...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COMPRESSION_UTILITY_ADDRESS || '';

export default function Compression_UtilityPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.compression-utility/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'compression-utility',
