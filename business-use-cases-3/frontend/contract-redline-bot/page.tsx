...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CONTRACT_REDLINE_BOT_ADDRESS || '';

export default function Contract_Redline_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.contract-redline-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'contract-redline-bot',
