...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ESCROW_AUTO_TRIGGER_ADDRESS || '';

export default function Escrow_Auto_TriggerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.escrow-auto-trigger/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'escrow-auto-trigger',
