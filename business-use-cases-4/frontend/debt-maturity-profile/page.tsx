...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEBT_MATURITY_PROFILE_ADDRESS || '';

export default function Debt_Maturity_ProfilePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.debt-maturity-profile/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'debt-maturity-profile',
