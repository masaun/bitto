...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEBT_SERVICE_COVERAGE_ADDRESS || '';

export default function Debt_Service_CoveragePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.debt-service-coverage/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'debt-service-coverage',
