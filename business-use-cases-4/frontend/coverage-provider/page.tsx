...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COVERAGE_PROVIDER_ADDRESS || '';

export default function Coverage_ProviderPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.coverage-provider/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'coverage-provider',
