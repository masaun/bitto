...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CURRENCY_HEDGE_AGENT_ADDRESS || '';

export default function Currency_Hedge_AgentPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.currency-hedge-agent/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'currency-hedge-agent',
