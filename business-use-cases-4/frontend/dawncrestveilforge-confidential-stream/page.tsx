...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DAWNCRESTVEILFORGE_CONFIDENTIAL_STREAM_ADDRESS || '';

export default function Dawncrestveilforge_Confidential_StreamPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'dawncrestveilforge-confidential-stream',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.dawncrestveilforge-confidential-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'dawncrestveilforge-confidential-stream',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>dawncrestveilforge-confidential-stream</h1>
