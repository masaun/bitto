...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DELINQUENCY_REPORTING_HUB_NEW_ADDRESS || '';

export default function Delinquency_Reporting_Hub_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'delinquency-reporting-hub-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.delinquency-reporting-hub-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'delinquency-reporting-hub-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>delinquency-reporting-hub-new</h1>
