...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEPRECIATION_CALC_AI_ADDRESS || '';

export default function Depreciation_Calc_AiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.depreciation-calc-ai/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'depreciation-calc-ai',
