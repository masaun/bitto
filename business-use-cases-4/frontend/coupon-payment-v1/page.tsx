...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COUPON_PAYMENT_V1_ADDRESS || '';

export default function Coupon_Payment_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.coupon-payment-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'coupon-payment-v1',
