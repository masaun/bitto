...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEBT_SERVICE_TRACKER_V1_ADDRESS || '';

export default function Debt_Service_Tracker_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.debt-service-tracker-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'debt-service-tracker-v1',
