...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CURVE_GAUGE_MONITOR_ADDRESS || '';

export default function Curve_Gauge_MonitorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.curve-gauge-monitor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'curve-gauge-monitor',
