...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COVERAGE_RATIO_MONITOR_NEW_ADDRESS || '';

export default function Coverage_Ratio_Monitor_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'coverage-ratio-monitor-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.coverage-ratio-monitor-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'coverage-ratio-monitor-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>coverage-ratio-monitor-new</h1>
