...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EPSILON_MARKET_CAP_ADDRESS || '';

export default function Epsilon_Market_CapPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.epsilon-market-cap/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'epsilon-market-cap',
