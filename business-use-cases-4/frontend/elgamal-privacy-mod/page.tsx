...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ELGAMAL_PRIVACY_MOD_ADDRESS || '';

export default function Elgamal_Privacy_ModPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.elgamal-privacy-mod/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'elgamal-privacy-mod',
