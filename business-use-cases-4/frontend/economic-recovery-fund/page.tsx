...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ECONOMIC_RECOVERY_FUND_ADDRESS || '';

export default function Economic_Recovery_FundPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.economic-recovery-fund/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'economic-recovery-fund',
