...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEBT_TRANSFER_SYS_ADDRESS || '';

export default function Debt_Transfer_SysPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.debt-transfer-sys/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'debt-transfer-sys',
