...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DOT_PARACHAIN_INFO_ADDRESS || '';

export default function Dot_Parachain_InfoPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.dot-parachain-info/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'dot-parachain-info',
