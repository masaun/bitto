...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CRIMSONVEILFORGE_SECURE_ADAPTER_ADDRESS || '';

export default function Crimsonveilforge_Secure_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'crimsonveilforge-secure-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.crimsonveilforge-secure-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'crimsonveilforge-secure-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>crimsonveilforge-secure-adapter</h1>
