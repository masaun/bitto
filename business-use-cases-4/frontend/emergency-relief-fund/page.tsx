...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EMERGENCY_RELIEF_FUND_ADDRESS || '';

export default function Emergency_Relief_FundPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.emergency-relief-fund/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'emergency-relief-fund',
