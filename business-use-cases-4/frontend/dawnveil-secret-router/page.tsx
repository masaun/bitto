...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DAWNVEIL_SECRET_ROUTER_ADDRESS || '';

export default function Dawnveil_Secret_RouterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.dawnveil-secret-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'dawnveil-secret-router',
