...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CUSIP_DATA_PROVIDER_ADDRESS || '';

export default function Cusip_Data_ProviderPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cusip-data-provider/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cusip-data-provider',
