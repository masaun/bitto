...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CSV_BATCH_UPLOADER_ADDRESS || '';

export default function Csv_Batch_UploaderPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.csv-batch-uploader/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'csv-batch-uploader',
