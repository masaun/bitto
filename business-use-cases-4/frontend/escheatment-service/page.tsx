...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ESCHEATMENT_SERVICE_ADDRESS || '';

export default function Escheatment_ServicePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.escheatment-service/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'escheatment-service',
