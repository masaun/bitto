...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EMBERLINK_TEE_PROCESSOR_ADDRESS || '';

export default function Emberlink_Tee_ProcessorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.emberlink-tee-processor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'emberlink-tee-processor',
