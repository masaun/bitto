...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DIGITAL_TWIN_SYNC_ADDRESS || '';

export default function Digital_Twin_SyncPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.digital-twin-sync/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'digital-twin-sync',
