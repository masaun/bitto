...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEMOGRAPHIC_TREND_BOT_ADDRESS || '';

export default function Demographic_Trend_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.demographic-trend-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'demographic-trend-bot',
