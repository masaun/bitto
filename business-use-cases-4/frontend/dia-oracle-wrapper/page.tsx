...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DIA_ORACLE_WRAPPER_ADDRESS || '';

export default function Dia_Oracle_WrapperPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.dia-oracle-wrapper/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'dia-oracle-wrapper',
