...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DUE_DILIGENCE_ROUTER_NEW_ADDRESS || '';

export default function Due_Diligence_Router_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.due-diligence-router-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'due-diligence-router-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>due-diligence-router-new</h1>
