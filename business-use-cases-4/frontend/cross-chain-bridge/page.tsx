...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CROSS_CHAIN_BRIDGE_ADDRESS || '';

export default function Cross_Chain_BridgePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cross-chain-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cross-chain-bridge',
