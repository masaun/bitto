...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ENTITY_STRUCTURE_AI_ADDRESS || '';

export default function Entity_Structure_AiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.entity-structure-ai/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'entity-structure-ai',
