...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DATA_INTEGRITY_CHK_ADDRESS || '';

export default function Data_Integrity_ChkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.data-integrity-chk/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'data-integrity-chk',
