...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DUSKSHIELD_PRIVATE_REFINERY_ADDRESS || '';

export default function Duskshield_Private_RefineryPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'duskshield-private-refinery',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.duskshield-private-refinery/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'duskshield-private-refinery',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>duskshield-private-refinery</h1>
