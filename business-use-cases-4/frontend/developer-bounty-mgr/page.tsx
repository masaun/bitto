...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEVELOPER_BOUNTY_MGR_ADDRESS || '';

export default function Developer_Bounty_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.developer-bounty-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'developer-bounty-mgr',
