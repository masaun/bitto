...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CRYPTOSPAN_TRUSTED_BRIDGE_ADDRESS || '';

export default function Cryptospan_Trusted_BridgePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'cryptospan-trusted-bridge',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cryptospan-trusted-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cryptospan-trusted-bridge',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>cryptospan-trusted-bridge</h1>
