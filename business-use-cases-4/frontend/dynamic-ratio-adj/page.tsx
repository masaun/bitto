...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DYNAMIC_RATIO_ADJ_ADDRESS || '';

export default function Dynamic_Ratio_AdjPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.dynamic-ratio-adj/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'dynamic-ratio-adj',
