...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEFICIT_SPENDING_ENGINE_ADDRESS || '';

export default function Deficit_Spending_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.deficit-spending-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'deficit-spending-engine',
