...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DVP_ATOMIC_PROCESSOR_ADDRESS || '';

export default function Dvp_Atomic_ProcessorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.dvp-atomic-processor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'dvp-atomic-processor',
