...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DIVIDEND_DISTRO_GATE_ADDRESS || '';

export default function Dividend_Distro_GatePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.dividend-distro-gate/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'dividend-distro-gate',
