...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DAWNVEILFORGE_CONFIDENTIAL_ROUTER_ADDRESS || '';

export default function Dawnveilforge_Confidential_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'dawnveilforge-confidential-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.dawnveilforge-confidential-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'dawnveilforge-confidential-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>dawnveilforge-confidential-router</h1>
