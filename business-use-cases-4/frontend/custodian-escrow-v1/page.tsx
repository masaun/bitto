...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CUSTODIAN_ESCROW_V1_ADDRESS || '';

export default function Custodian_Escrow_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.custodian-escrow-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'custodian-escrow-v1',
