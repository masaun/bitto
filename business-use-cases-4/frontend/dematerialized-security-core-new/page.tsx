...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEMATERIALIZED_SECURITY_CORE_NEW_ADDRESS || '';

export default function Dematerialized_Security_Core_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'dematerialized-security-core-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.dematerialized-security-core-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'dematerialized-security-core-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>dematerialized-security-core-new</h1>
