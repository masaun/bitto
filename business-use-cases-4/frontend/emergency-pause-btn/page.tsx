...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EMERGENCY_PAUSE_BTN_ADDRESS || '';

export default function Emergency_Pause_BtnPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.emergency-pause-btn/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'emergency-pause-btn',
