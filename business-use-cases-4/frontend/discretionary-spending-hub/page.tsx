...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DISCRETIONARY_SPENDING_HUB_ADDRESS || '';

export default function Discretionary_Spending_HubPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'discretionary-spending-hub',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.discretionary-spending-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'discretionary-spending-hub',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>discretionary-spending-hub</h1>
