...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ELEVATOR_MAINTENANCE_ADDRESS || '';

export default function Elevator_MaintenancePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.elevator-maintenance/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'elevator-maintenance',
