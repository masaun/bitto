...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEBT_STABILIZATION_RULE_ADDRESS || '';

export default function Debt_Stabilization_RulePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.debt-stabilization-rule/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'debt-stabilization-rule',
