...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DECENTRALIZED_ID_CRE_ADDRESS || '';

export default function Decentralized_Id_CrePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.decentralized-id-cre/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'decentralized-id-cre',
