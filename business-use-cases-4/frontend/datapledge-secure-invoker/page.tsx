...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DATAPLEDGE_SECURE_INVOKER_ADDRESS || '';

export default function Datapledge_Secure_InvokerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'datapledge-secure-invoker',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.datapledge-secure-invoker/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'datapledge-secure-invoker',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>datapledge-secure-invoker</h1>
