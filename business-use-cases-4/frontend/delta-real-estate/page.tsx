...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DELTA_REAL_ESTATE_ADDRESS || '';

export default function Delta_Real_EstatePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.delta-real-estate/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'delta-real-estate',
