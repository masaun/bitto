...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CORPORATE_ACTION_HUB_ADDRESS || '';

export default function Corporate_Action_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.corporate-action-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'corporate-action-hub',
