...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ECLIPSEFORGE_PRIVATE_ADAPTER_ADDRESS || '';

export default function Eclipseforge_Private_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'eclipseforge-private-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.eclipseforge-private-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'eclipseforge-private-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>eclipseforge-private-adapter</h1>
