...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEFLATION_BURN_LOG_ADDRESS || '';

export default function Deflation_Burn_LogPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.deflation-burn-log/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'deflation-burn-log',
