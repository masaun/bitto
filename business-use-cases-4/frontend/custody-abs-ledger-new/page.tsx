...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CUSTODY_ABS_LEDGER_NEW_ADDRESS || '';

export default function Custody_Abs_Ledger_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.custody-abs-ledger-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'custody-abs-ledger-new',
