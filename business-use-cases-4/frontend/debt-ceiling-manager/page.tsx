...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEBT_CEILING_MANAGER_ADDRESS || '';

export default function Debt_Ceiling_ManagerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.debt-ceiling-manager/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'debt-ceiling-manager',
