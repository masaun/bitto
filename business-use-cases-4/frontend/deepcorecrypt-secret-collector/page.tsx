...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DEEPCORECRYPT_SECRET_COLLECTOR_ADDRESS || '';

export default function Deepcorecrypt_Secret_CollectorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'deepcorecrypt-secret-collector',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.deepcorecrypt-secret-collector/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'deepcorecrypt-secret-collector',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>deepcorecrypt-secret-collector</h1>
