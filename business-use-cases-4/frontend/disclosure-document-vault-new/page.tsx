...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DISCLOSURE_DOCUMENT_VAULT_NEW_ADDRESS || '';

export default function Disclosure_Document_Vault_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'disclosure-document-vault-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.disclosure-document-vault-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'disclosure-document-vault-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>disclosure-document-vault-new</h1>
