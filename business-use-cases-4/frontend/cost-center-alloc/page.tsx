...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_COST_CENTER_ALLOC_ADDRESS || '';

export default function Cost_Center_AllocPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cost-center-alloc/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cost-center-alloc',
