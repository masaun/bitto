...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CREDIT_RATING_HUB_ADDRESS || '';

export default function Credit_Rating_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.credit-rating-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'credit-rating-hub',
