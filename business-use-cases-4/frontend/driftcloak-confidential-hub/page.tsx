...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_DRIFTCLOAK_CONFIDENTIAL_HUB_ADDRESS || '';

export default function Driftcloak_Confidential_HubPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'driftcloak-confidential-hub',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.driftcloak-confidential-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'driftcloak-confidential-hub',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>driftcloak-confidential-hub</h1>
