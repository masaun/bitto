...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EMBERVEILFORGE_SECURE_STREAM_ADDRESS || '';

export default function Emberveilforge_Secure_StreamPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'emberveilforge-secure-stream',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.emberveilforge-secure-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'emberveilforge-secure-stream',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>emberveilforge-secure-stream</h1>
