...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EIN_LOOKUP_ADAPTER_ADDRESS || '';

export default function Ein_Lookup_AdapterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ein-lookup-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ein-lookup-adapter',
