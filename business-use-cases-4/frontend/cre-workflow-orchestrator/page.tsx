...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_CRE_WORKFLOW_ORCHESTRATOR_ADDRESS || '';

export default function Cre_Workflow_OrchestratorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'cre-workflow-orchestrator',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.cre-workflow-orchestrator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'cre-workflow-orchestrator',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>cre-workflow-orchestrator</h1>
