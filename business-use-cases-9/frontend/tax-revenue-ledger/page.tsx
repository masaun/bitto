...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TAX_REVENUE_LEDGER_ADDRESS || '';

export default function Tax_Revenue_LedgerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tax-revenue-ledger/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tax-revenue-ledger',
