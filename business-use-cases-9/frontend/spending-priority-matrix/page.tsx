...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SPENDING_PRIORITY_MATRIX_ADDRESS || '';

export default function Spending_Priority_MatrixPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.spending-priority-matrix/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'spending-priority-matrix',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>spending-priority-matrix</h1>
