...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STELLAR_SOROBAN_INT_ADDRESS || '';

export default function Stellar_Soroban_IntPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.stellar-soroban-int/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'stellar-soroban-int',
