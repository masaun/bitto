...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STACKS_SIP010_PAY_ADDRESS || '';

export default function Stacks_Sip010_PayPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.stacks-sip010-pay/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'stacks-sip010-pay',
