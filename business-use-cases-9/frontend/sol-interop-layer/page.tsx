...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SOL_INTEROP_LAYER_ADDRESS || '';

export default function Sol_Interop_LayerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sol-interop-layer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sol-interop-layer',
