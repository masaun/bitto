...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SGX_REMOTE_VERIFY_ADDRESS || '';

export default function Sgx_Remote_VerifyPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sgx-remote-verify/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sgx-remote-verify',
