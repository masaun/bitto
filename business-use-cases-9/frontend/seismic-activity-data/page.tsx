...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SEISMIC_ACTIVITY_DATA_ADDRESS || '';

export default function Seismic_Activity_DataPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.seismic-activity-data/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'seismic-activity-data',
