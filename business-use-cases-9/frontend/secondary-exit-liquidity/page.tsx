...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SECONDARY_EXIT_LIQUIDITY_ADDRESS || '';

export default function Secondary_Exit_LiquidityPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.secondary-exit-liquidity/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'secondary-exit-liquidity',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>secondary-exit-liquidity</h1>
