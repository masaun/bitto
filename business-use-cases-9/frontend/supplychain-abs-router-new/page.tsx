...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SUPPLYCHAIN_ABS_ROUTER_NEW_ADDRESS || '';

export default function Supplychain_Abs_Router_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'supplychain-abs-router-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.supplychain-abs-router-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'supplychain-abs-router-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>supplychain-abs-router-new</h1>
