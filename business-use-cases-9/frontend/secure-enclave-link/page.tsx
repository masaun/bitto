...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SECURE_ENCLAVE_LINK_ADDRESS || '';

export default function Secure_Enclave_LinkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.secure-enclave-link/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'secure-enclave-link',
