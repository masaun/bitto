...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SOVEREIGN_BUDGET_MANAGER_ADDRESS || '';

export default function Sovereign_Budget_ManagerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sovereign-budget-manager/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sovereign-budget-manager',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>sovereign-budget-manager</h1>
