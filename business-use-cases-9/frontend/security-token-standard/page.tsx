...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SECURITY_TOKEN_STANDARD_ADDRESS || '';

export default function Security_Token_StandardPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.security-token-standard/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'security-token-standard',
