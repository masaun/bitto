...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SEPA_INSTANT_BRIDGE_ADDRESS || '';

export default function Sepa_Instant_BridgePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sepa-instant-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sepa-instant-bridge',
