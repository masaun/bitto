...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STABILIZATION_FUND_MANAGER_ADDRESS || '';

export default function Stabilization_Fund_ManagerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'stabilization-fund-manager',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.stabilization-fund-manager/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'stabilization-fund-manager',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>stabilization-fund-manager</h1>
