...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STABLE_SWAP_CURVE_ADDRESS || '';

export default function Stable_Swap_CurvePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.stable-swap-curve/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'stable-swap-curve',
