...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SNAPSHOT_FREQUENCY_MGR_ADDRESS || '';

export default function Snapshot_Frequency_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.snapshot-frequency-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'snapshot-frequency-mgr',
