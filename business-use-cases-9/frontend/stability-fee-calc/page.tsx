...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STABILITY_FEE_CALC_ADDRESS || '';

export default function Stability_Fee_CalcPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.stability-fee-calc/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'stability-fee-calc',
