...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STRUCTURAL_DEFICIT_ANALYZER_ADDRESS || '';

export default function Structural_Deficit_AnalyzerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'structural-deficit-analyzer',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.structural-deficit-analyzer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'structural-deficit-analyzer',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>structural-deficit-analyzer</h1>
