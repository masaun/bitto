...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SAFEWATCH_CONFIDENTIAL_RUNNER_ADDRESS || '';

export default function Safewatch_Confidential_RunnerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'safewatch-confidential-runner',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.safewatch-confidential-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'safewatch-confidential-runner',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>safewatch-confidential-runner</h1>
