...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SSI_CREDENTIAL_MGR_ADDRESS || '';

export default function Ssi_Credential_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ssi-credential-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ssi-credential-mgr',
