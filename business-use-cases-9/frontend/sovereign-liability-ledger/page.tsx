...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SOVEREIGN_LIABILITY_LEDGER_ADDRESS || '';

export default function Sovereign_Liability_LedgerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'sovereign-liability-ledger',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sovereign-liability-ledger/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sovereign-liability-ledger',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>sovereign-liability-ledger</h1>
