...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TAKEOUT_FINANCING_MODULE_NEW_ADDRESS || '';

export default function Takeout_Financing_Module_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'takeout-financing-module-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.takeout-financing-module-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'takeout-financing-module-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>takeout-financing-module-new</h1>
