...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SIGNATURE_VALIDATOR_ADDRESS || '';

export default function Signature_ValidatorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.signature-validator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'signature-validator',
