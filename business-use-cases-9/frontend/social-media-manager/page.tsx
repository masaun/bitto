...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SOCIAL_MEDIA_MANAGER_ADDRESS || '';

export default function Social_Media_ManagerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.social-media-manager/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'social-media-manager',
