...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SPIN_OFF_CONTROLLER_ADDRESS || '';

export default function Spin_Off_ControllerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.spin-off-controller/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'spin-off-controller',
