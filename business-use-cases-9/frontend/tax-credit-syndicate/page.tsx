...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TAX_CREDIT_SYNDICATE_ADDRESS || '';

export default function Tax_Credit_SyndicatePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tax-credit-syndicate/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tax-credit-syndicate',
