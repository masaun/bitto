...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STEPDOWN_TRIGGER_MANAGER_NEW_ADDRESS || '';

export default function Stepdown_Trigger_Manager_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'stepdown-trigger-manager-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.stepdown-trigger-manager-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'stepdown-trigger-manager-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>stepdown-trigger-manager-new</h1>
