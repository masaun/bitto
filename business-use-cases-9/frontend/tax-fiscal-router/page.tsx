...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TAX_FISCAL_ROUTER_ADDRESS || '';

export default function Tax_Fiscal_RouterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tax-fiscal-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tax-fiscal-router',
