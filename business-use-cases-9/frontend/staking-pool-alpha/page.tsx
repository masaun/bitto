...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STAKING_POOL_ALPHA_ADDRESS || '';

export default function Staking_Pool_AlphaPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.staking-pool-alpha/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'staking-pool-alpha',
