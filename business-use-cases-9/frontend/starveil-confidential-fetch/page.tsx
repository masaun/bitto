...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STARVEIL_CONFIDENTIAL_FETCH_ADDRESS || '';

export default function Starveil_Confidential_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'starveil-confidential-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.starveil-confidential-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'starveil-confidential-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>starveil-confidential-fetch</h1>
