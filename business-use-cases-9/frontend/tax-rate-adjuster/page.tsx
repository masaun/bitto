...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TAX_RATE_ADJUSTER_ADDRESS || '';

export default function Tax_Rate_AdjusterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tax-rate-adjuster/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tax-rate-adjuster',
