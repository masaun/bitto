...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TARIFF_REVENUE_MODULE_ADDRESS || '';

export default function Tariff_Revenue_ModulePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tariff-revenue-module/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tariff-revenue-module',
