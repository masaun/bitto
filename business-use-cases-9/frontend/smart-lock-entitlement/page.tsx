...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SMART_LOCK_ENTITLEMENT_ADDRESS || '';

export default function Smart_Lock_EntitlementPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.smart-lock-entitlement/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'smart-lock-entitlement',
