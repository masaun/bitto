...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SOVEREIGN_CONTINGENT_LIABILITY_ADDRESS || '';

export default function Sovereign_Contingent_LiabilityPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'sovereign-contingent-liability',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sovereign-contingent-liability/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sovereign-contingent-liability',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>sovereign-contingent-liability</h1>
