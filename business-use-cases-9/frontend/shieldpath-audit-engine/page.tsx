...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SHIELDPATH_AUDIT_ENGINE_ADDRESS || '';

export default function Shieldpath_Audit_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.shieldpath-audit-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'shieldpath-audit-engine',
