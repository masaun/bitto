...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STRUCTURED_LIABILITY_HUB_NEW_ADDRESS || '';

export default function Structured_Liability_Hub_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'structured-liability-hub-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.structured-liability-hub-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'structured-liability-hub-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>structured-liability-hub-new</h1>
