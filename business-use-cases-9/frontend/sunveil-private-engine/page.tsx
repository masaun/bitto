...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SUNVEIL_PRIVATE_ENGINE_ADDRESS || '';

export default function Sunveil_Private_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sunveil-private-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sunveil-private-engine',
