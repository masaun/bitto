...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SLIPPAGE_IMPACT_MOD_ADDRESS || '';

export default function Slippage_Impact_ModPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.slippage-impact-mod/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'slippage-impact-mod',
