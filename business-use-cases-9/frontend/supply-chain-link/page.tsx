...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SUPPLY_CHAIN_LINK_ADDRESS || '';

export default function Supply_Chain_LinkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.supply-chain-link/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'supply-chain-link',
