...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STATE_PRUNING_LOGIC_ADDRESS || '';

export default function State_Pruning_LogicPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.state-pruning-logic/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'state-pruning-logic',
