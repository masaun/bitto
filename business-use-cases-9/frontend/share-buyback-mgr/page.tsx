...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SHARE_BUYBACK_MGR_ADDRESS || '';

export default function Share_Buyback_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.share-buyback-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'share-buyback-mgr',
