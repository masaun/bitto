...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TAX_WITHHOLDING_CALC_ADDRESS || '';

export default function Tax_Withholding_CalcPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tax-withholding-calc/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tax-withholding-calc',
