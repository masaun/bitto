...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SECUREFLUX_AUDIT_PROXY_ADDRESS || '';

export default function Secureflux_Audit_ProxyPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.secureflux-audit-proxy/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'secureflux-audit-proxy',
