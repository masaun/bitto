...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SPECIAL_PURPOSE_VEHICLE_NEW_ADDRESS || '';

export default function Special_Purpose_Vehicle_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'special-purpose-vehicle-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.special-purpose-vehicle-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'special-purpose-vehicle-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>special-purpose-vehicle-new</h1>
