...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STEALTH_ADDRESS_GEN_ADDRESS || '';

export default function Stealth_Address_GenPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.stealth-address-gen/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'stealth-address-gen',
