...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SATELLITE_OFFICE_MGR_ADDRESS || '';

export default function Satellite_Office_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.satellite-office-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'satellite-office-mgr',
