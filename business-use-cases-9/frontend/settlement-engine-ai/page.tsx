...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SETTLEMENT_ENGINE_AI_ADDRESS || '';

export default function Settlement_Engine_AiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.settlement-engine-ai/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'settlement-engine-ai',
