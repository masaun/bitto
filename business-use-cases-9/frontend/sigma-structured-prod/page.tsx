...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SIGMA_STRUCTURED_PROD_ADDRESS || '';

export default function Sigma_Structured_ProdPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sigma-structured-prod/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sigma-structured-prod',
