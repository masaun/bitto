...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STORAGE_PROOF_EVAL_ADDRESS || '';

export default function Storage_Proof_EvalPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.storage-proof-eval/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'storage-proof-eval',
