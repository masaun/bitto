...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TAX_ADMINISTRATION_CORE_ADDRESS || '';

export default function Tax_Administration_CorePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tax-administration-core/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tax-administration-core',
