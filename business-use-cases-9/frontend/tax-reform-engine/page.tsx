...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TAX_REFORM_ENGINE_ADDRESS || '';

export default function Tax_Reform_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tax-reform-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tax-reform-engine',
