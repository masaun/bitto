...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SUBSCRIPTION_TIER_ADDRESS || '';

export default function Subscription_TierPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.subscription-tier/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'subscription-tier',
