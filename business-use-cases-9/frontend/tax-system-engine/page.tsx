...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TAX_SYSTEM_ENGINE_ADDRESS || '';

export default function Tax_System_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tax-system-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tax-system-engine',
