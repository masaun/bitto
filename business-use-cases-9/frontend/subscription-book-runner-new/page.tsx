...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SUBSCRIPTION_BOOK_RUNNER_NEW_ADDRESS || '';

export default function Subscription_Book_Runner_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'subscription-book-runner-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.subscription-book-runner-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'subscription-book-runner-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>subscription-book-runner-new</h1>
