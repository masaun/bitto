...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SAFETRACKR_CONFIDENTIAL_DISPATCH_ADDRESS || '';

export default function Safetrackr_Confidential_DispatchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'safetrackr-confidential-dispatch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.safetrackr-confidential-dispatch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'safetrackr-confidential-dispatch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>safetrackr-confidential-dispatch</h1>
