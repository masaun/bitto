...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STARKNET_MESSAGING_ADDRESS || '';

export default function Starknet_MessagingPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.starknet-messaging/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'starknet-messaging',
