...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SYNTHETIC_MKT_MGR_ADDRESS || '';

export default function Synthetic_Mkt_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.synthetic-mkt-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'synthetic-mkt-mgr',
