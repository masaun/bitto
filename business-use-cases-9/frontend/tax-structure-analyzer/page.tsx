...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TAX_STRUCTURE_ANALYZER_ADDRESS || '';

export default function Tax_Structure_AnalyzerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tax-structure-analyzer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tax-structure-analyzer',
