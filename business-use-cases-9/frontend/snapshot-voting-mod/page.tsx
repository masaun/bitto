...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SNAPSHOT_VOTING_MOD_ADDRESS || '';

export default function Snapshot_Voting_ModPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.snapshot-voting-mod/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'snapshot-voting-mod',
