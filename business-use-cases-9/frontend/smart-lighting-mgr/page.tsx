...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SMART_LIGHTING_MGR_ADDRESS || '';

export default function Smart_Lighting_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.smart-lighting-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'smart-lighting-mgr',
