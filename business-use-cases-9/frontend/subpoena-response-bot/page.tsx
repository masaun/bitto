...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SUBPOENA_RESPONSE_BOT_ADDRESS || '';

export default function Subpoena_Response_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.subpoena-response-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'subpoena-response-bot',
