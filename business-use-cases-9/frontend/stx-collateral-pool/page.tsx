...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STX_COLLATERAL_POOL_ADDRESS || '';

export default function Stx_Collateral_PoolPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.stx-collateral-pool/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'stx-collateral-pool',
