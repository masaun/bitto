...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TAX_POLICY_EVALUATOR_ADDRESS || '';

export default function Tax_Policy_EvaluatorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tax-policy-evaluator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tax-policy-evaluator',
