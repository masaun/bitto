...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TAXATION_POLICY_REGISTRY_ADDRESS || '';

export default function Taxation_Policy_RegistryPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.taxation-policy-registry/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'taxation-policy-registry',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>taxation-policy-registry</h1>
