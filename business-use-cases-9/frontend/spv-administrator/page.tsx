...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SPV_ADMINISTRATOR_ADDRESS || '';

export default function Spv_AdministratorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.spv-administrator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'spv-administrator',
