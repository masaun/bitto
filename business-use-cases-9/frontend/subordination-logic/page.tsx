...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SUBORDINATION_LOGIC_ADDRESS || '';

export default function Subordination_LogicPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.subordination-logic/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'subordination-logic',
