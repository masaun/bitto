...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SEALED_ENVELOPE_BID_ADDRESS || '';

export default function Sealed_Envelope_BidPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sealed-envelope-bid/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sealed-envelope-bid',
