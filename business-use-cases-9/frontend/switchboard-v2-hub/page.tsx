...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SWITCHBOARD_V2_HUB_ADDRESS || '';

export default function Switchboard_V2_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.switchboard-v2-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'switchboard-v2-hub',
