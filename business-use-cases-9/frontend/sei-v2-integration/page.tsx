...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SEI_V2_INTEGRATION_ADDRESS || '';

export default function Sei_V2_IntegrationPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sei-v2-integration/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sei-v2-integration',
