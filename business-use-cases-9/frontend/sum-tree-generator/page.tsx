...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SUM_TREE_GENERATOR_ADDRESS || '';

export default function Sum_Tree_GeneratorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sum-tree-generator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sum-tree-generator',
