...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SOVEREIGN_BUDGET_FRAMEWORK_ADDRESS || '';

export default function Sovereign_Budget_FrameworkPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'sovereign-budget-framework',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sovereign-budget-framework/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sovereign-budget-framework',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>sovereign-budget-framework</h1>
