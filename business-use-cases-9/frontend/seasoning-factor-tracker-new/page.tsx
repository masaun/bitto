...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SEASONING_FACTOR_TRACKER_NEW_ADDRESS || '';

export default function Seasoning_Factor_Tracker_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'seasoning-factor-tracker-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.seasoning-factor-tracker-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'seasoning-factor-tracker-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>seasoning-factor-tracker-new</h1>
