...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SURETY_BOND_LOGIC_ADDRESS || '';

export default function Surety_Bond_LogicPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.surety-bond-logic/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'surety-bond-logic',
