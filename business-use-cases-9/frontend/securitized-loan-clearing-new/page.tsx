...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SECURITIZED_LOAN_CLEARING_NEW_ADDRESS || '';

export default function Securitized_Loan_Clearing_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'securitized-loan-clearing-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.securitized-loan-clearing-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'securitized-loan-clearing-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>securitized-loan-clearing-new</h1>
