...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SECRET_SHARING_SCHEME_ADDRESS || '';

export default function Secret_Sharing_SchemePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.secret-sharing-scheme/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'secret-sharing-scheme',
