...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SAFEVECTOR_CONFIDENTIAL_HUB_ADDRESS || '';

export default function Safevector_Confidential_HubPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'safevector-confidential-hub',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.safevector-confidential-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'safevector-confidential-hub',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>safevector-confidential-hub</h1>
