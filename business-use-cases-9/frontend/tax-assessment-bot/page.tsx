...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TAX_ASSESSMENT_BOT_ADDRESS || '';

export default function Tax_Assessment_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tax-assessment-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tax-assessment-bot',
