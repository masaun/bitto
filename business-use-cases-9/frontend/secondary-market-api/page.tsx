...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SECONDARY_MARKET_API_ADDRESS || '';

export default function Secondary_Market_ApiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.secondary-market-api/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'secondary-market-api',
