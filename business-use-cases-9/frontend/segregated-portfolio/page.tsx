...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SEGREGATED_PORTFOLIO_ADDRESS || '';

export default function Segregated_PortfolioPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.segregated-portfolio/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'segregated-portfolio',
