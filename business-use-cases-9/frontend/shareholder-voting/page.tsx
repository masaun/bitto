...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SHAREHOLDER_VOTING_ADDRESS || '';

export default function Shareholder_VotingPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.shareholder-voting/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'shareholder-voting',
