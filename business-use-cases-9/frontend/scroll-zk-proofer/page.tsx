...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SCROLL_ZK_PROOFER_ADDRESS || '';

export default function Scroll_Zk_ProoferPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.scroll-zk-proofer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'scroll-zk-proofer',
