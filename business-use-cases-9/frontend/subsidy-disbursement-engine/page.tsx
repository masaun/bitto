...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SUBSIDY_DISBURSEMENT_ENGINE_ADDRESS || '';

export default function Subsidy_Disbursement_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'subsidy-disbursement-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.subsidy-disbursement-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'subsidy-disbursement-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>subsidy-disbursement-engine</h1>
