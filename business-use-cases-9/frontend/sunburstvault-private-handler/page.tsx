...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SUNBURSTVAULT_PRIVATE_HANDLER_ADDRESS || '';

export default function Sunburstvault_Private_HandlerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'sunburstvault-private-handler',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sunburstvault-private-handler/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sunburstvault-private-handler',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>sunburstvault-private-handler</h1>
