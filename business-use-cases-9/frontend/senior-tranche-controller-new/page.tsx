...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SENIOR_TRANCHE_CONTROLLER_NEW_ADDRESS || '';

export default function Senior_Tranche_Controller_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'senior-tranche-controller-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.senior-tranche-controller-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'senior-tranche-controller-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>senior-tranche-controller-new</h1>
