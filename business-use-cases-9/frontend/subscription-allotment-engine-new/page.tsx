...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SUBSCRIPTION_ALLOTMENT_ENGINE_NEW_ADDRESS || '';

export default function Subscription_Allotment_Engine_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'subscription-allotment-engine-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.subscription-allotment-engine-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'subscription-allotment-engine-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>subscription-allotment-engine-new</h1>
