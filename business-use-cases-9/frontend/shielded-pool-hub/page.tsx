...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SHIELDED_POOL_HUB_ADDRESS || '';

export default function Shielded_Pool_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.shielded-pool-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'shielded-pool-hub',
