...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SETTLEMENT_LAYER_V1_ADDRESS || '';

export default function Settlement_Layer_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.settlement-layer-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'settlement-layer-v1',
