...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SANDWICH_DETECTOR_ADDRESS || '';

export default function Sandwich_DetectorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sandwich-detector/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sandwich-detector',
