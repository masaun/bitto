...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SPARSE_MERKLE_TREE_ADDRESS || '';

export default function Sparse_Merkle_TreePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.sparse-merkle-tree/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'sparse-merkle-tree',
