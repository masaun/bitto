...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SHROUDED_TRANSFER_ADDRESS || '';

export default function Shrouded_TransferPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.shrouded-transfer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'shrouded-transfer',
