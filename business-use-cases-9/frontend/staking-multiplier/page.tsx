...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STAKING_MULTIPLIER_ADDRESS || '';

export default function Staking_MultiplierPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.staking-multiplier/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'staking-multiplier',
