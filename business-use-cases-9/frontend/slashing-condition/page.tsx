...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SLASHING_CONDITION_ADDRESS || '';

export default function Slashing_ConditionPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.slashing-condition/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'slashing-condition',
