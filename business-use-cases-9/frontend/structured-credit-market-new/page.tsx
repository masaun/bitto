...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_STRUCTURED_CREDIT_MARKET_NEW_ADDRESS || '';

export default function Structured_Credit_Market_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'structured-credit-market-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.structured-credit-market-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'structured-credit-market-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>structured-credit-market-new</h1>
