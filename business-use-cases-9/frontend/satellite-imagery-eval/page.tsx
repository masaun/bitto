...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SATELLITE_IMAGERY_EVAL_ADDRESS || '';

export default function Satellite_Imagery_EvalPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.satellite-imagery-eval/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'satellite-imagery-eval',
