...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SURPLUS_DISTRIBUTION_ENGINE_ADDRESS || '';

export default function Surplus_Distribution_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'surplus-distribution-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.surplus-distribution-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'surplus-distribution-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>surplus-distribution-engine</h1>
