...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_UNITYVAULT_SECURE_INGESTOR_ADDRESS || '';

export default function Unityvault_Secure_IngestorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'unityvault-secure-ingestor',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.unityvault-secure-ingestor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'unityvault-secure-ingestor',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>unityvault-secure-ingestor</h1>
