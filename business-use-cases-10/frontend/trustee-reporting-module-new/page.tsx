...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRUSTEE_REPORTING_MODULE_NEW_ADDRESS || '';

export default function Trustee_Reporting_Module_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'trustee-reporting-module-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.trustee-reporting-module-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'trustee-reporting-module-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>trustee-reporting-module-new</h1>
