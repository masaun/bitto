...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRANCHE_BALANCE_TRACKER_NEW_ADDRESS || '';

export default function Tranche_Balance_Tracker_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tranche-balance-tracker-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tranche-balance-tracker-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tranche-balance-tracker-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tranche-balance-tracker-new</h1>
