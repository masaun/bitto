...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WATERFALL_PAYMENT_LOGIC_NEW_ADDRESS || '';

export default function Waterfall_Payment_Logic_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'waterfall-payment-logic-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.waterfall-payment-logic-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'waterfall-payment-logic-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>waterfall-payment-logic-new</h1>
