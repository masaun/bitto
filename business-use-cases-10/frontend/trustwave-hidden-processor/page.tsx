...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRUSTWAVE_HIDDEN_PROCESSOR_ADDRESS || '';

export default function Trustwave_Hidden_ProcessorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'trustwave-hidden-processor',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.trustwave-hidden-processor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'trustwave-hidden-processor',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>trustwave-hidden-processor</h1>
