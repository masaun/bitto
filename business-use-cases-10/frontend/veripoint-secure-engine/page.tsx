...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERIPOINT_SECURE_ENGINE_ADDRESS || '';

export default function Veripoint_Secure_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veripoint-secure-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veripoint-secure-engine',
