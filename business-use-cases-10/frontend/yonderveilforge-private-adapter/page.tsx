...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_YONDERVEILFORGE_PRIVATE_ADAPTER_ADDRESS || '';

export default function Yonderveilforge_Private_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'yonderveilforge-private-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.yonderveilforge-private-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'yonderveilforge-private-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>yonderveilforge-private-adapter</h1>
