...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERDANTVEILFORGE_CONFIDENTIAL_ROUTER_ADDRESS || '';

export default function Verdantveilforge_Confidential_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'verdantveilforge-confidential-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.verdantveilforge-confidential-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'verdantveilforge-confidential-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>verdantveilforge-confidential-router</h1>
