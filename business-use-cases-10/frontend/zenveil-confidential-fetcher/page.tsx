...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ZENVEIL_CONFIDENTIAL_FETCHER_ADDRESS || '';

export default function Zenveil_Confidential_FetcherPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'zenveil-confidential-fetcher',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.zenveil-confidential-fetcher/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'zenveil-confidential-fetcher',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>zenveil-confidential-fetcher</h1>
