...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TOKENY_TREX_STANDARD_ADDRESS || '';

export default function Tokeny_Trex_StandardPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tokeny-trex-standard/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tokeny-trex-standard',
