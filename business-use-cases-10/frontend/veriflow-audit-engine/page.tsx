...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERIFLOW_AUDIT_ENGINE_ADDRESS || '';

export default function Veriflow_Audit_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veriflow-audit-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veriflow-audit-engine',
