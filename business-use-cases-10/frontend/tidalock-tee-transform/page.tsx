...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TIDALOCK_TEE_TRANSFORM_ADDRESS || '';

export default function Tidalock_Tee_TransformPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tidalock-tee-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tidalock-tee-transform',
