...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VAULT_TRANSIT_KEY_ADDRESS || '';

export default function Vault_Transit_KeyPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.vault-transit-key/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'vault-transit-key',
