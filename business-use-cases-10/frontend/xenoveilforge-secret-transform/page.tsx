...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_XENOVEILFORGE_SECRET_TRANSFORM_ADDRESS || '';

export default function Xenoveilforge_Secret_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'xenoveilforge-secret-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.xenoveilforge-secret-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'xenoveilforge-secret-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>xenoveilforge-secret-transform</h1>
