...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRUSTEE_OVERSIGHT_ADAPTER_NEW_ADDRESS || '';

export default function Trustee_Oversight_Adapter_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'trustee-oversight-adapter-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.trustee-oversight-adapter-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'trustee-oversight-adapter-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>trustee-oversight-adapter-new</h1>
