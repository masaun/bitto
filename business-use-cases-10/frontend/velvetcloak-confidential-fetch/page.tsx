...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VELVETCLOAK_CONFIDENTIAL_FETCH_ADDRESS || '';

export default function Velvetcloak_Confidential_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'velvetcloak-confidential-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.velvetcloak-confidential-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'velvetcloak-confidential-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>velvetcloak-confidential-fetch</h1>
