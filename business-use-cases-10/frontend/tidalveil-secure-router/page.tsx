...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TIDALVEIL_SECURE_ROUTER_ADDRESS || '';

export default function Tidalveil_Secure_RouterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tidalveil-secure-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tidalveil-secure-router',
