...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WELLNESS_INCENTIVE_ADDRESS || '';

export default function Wellness_IncentivePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.wellness-incentive/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'wellness-incentive',
