...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERIBEACON_CONTROLLED_ENGINE_ADDRESS || '';

export default function Veribeacon_Controlled_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'veribeacon-controlled-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veribeacon-controlled-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veribeacon-controlled-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>veribeacon-controlled-engine</h1>
