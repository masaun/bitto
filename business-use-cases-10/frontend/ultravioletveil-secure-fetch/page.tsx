...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ULTRAVIOLETVEIL_SECURE_FETCH_ADDRESS || '';

export default function Ultravioletveil_Secure_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'ultravioletveil-secure-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ultravioletveil-secure-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ultravioletveil-secure-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>ultravioletveil-secure-fetch</h1>
