...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACEOPERATOR_OFFCHAIN_DISPATCH_ADDRESS || '';

export default function Traceoperator_Offchain_DispatchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'traceoperator-offchain-dispatch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.traceoperator-offchain-dispatch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'traceoperator-offchain-dispatch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>traceoperator-offchain-dispatch</h1>
