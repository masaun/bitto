...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WATER_FALL_PAYOUT_ADDRESS || '';

export default function Water_Fall_PayoutPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.water-fall-payout/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'water-fall-payout',
