...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TENDER_OFFER_PORTAL_ADDRESS || '';

export default function Tender_Offer_PortalPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tender-offer-portal/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tender-offer-portal',
