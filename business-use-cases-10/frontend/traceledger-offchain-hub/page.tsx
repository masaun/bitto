...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACELEDGER_OFFCHAIN_HUB_ADDRESS || '';

export default function Traceledger_Offchain_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.traceledger-offchain-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'traceledger-offchain-hub',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>traceledger-offchain-hub</h1>
