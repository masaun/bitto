...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUTO_REBALANCE_BOT_ADDRESS || '';

export default function Auto_Rebalance_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.auto-rebalance-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'auto-rebalance-bot',
