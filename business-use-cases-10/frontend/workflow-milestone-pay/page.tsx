...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WORKFLOW_MILESTONE_PAY_ADDRESS || '';

export default function Workflow_Milestone_PayPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.workflow-milestone-pay/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'workflow-milestone-pay',
