...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TEMPESTVEILFORGE_SECRET_STREAM_ADDRESS || '';

export default function Tempestveilforge_Secret_StreamPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tempestveilforge-secret-stream',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tempestveilforge-secret-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tempestveilforge-secret-stream',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tempestveilforge-secret-stream</h1>
