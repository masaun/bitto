...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WINDFORGE_SECRET_ENGINE_ADDRESS || '';

export default function Windforge_Secret_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.windforge-secret-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'windforge-secret-engine',
