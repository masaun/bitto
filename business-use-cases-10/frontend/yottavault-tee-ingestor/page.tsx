...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_YOTTAVAULT_TEE_INGESTOR_ADDRESS || '';

export default function Yottavault_Tee_IngestorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.yottavault-tee-ingestor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'yottavault-tee-ingestor',
