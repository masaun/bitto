...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VENDOR_BIDDING_ENGINE_ADDRESS || '';

export default function Vendor_Bidding_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.vendor-bidding-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'vendor-bidding-engine',
