...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_UNDER_WRAPPED_STX_ADDRESS || '';

export default function Under_Wrapped_StxPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.under-wrapped-stx/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'under-wrapped-stx',
