...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERICOMPLY_CONTROLLED_ROUTER_ADDRESS || '';

export default function Vericomply_Controlled_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'vericomply-controlled-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.vericomply-controlled-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'vericomply-controlled-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>vericomply-controlled-router</h1>
