...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACEVAULT_API_RUNNER_ADDRESS || '';

export default function Tracevault_Api_RunnerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tracevault-api-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tracevault-api-runner',
