...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACEVIGIL_OFFCHAIN_DISPATCH_ADDRESS || '';

export default function Tracevigil_Offchain_DispatchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tracevigil-offchain-dispatch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tracevigil-offchain-dispatch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tracevigil-offchain-dispatch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tracevigil-offchain-dispatch</h1>
