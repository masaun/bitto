...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACESTAMP_OFFCHAIN_ROUTER_ADDRESS || '';

export default function Tracestamp_Offchain_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tracestamp-offchain-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tracestamp-offchain-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tracestamp-offchain-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tracestamp-offchain-router</h1>
