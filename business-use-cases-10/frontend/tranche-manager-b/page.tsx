...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRANCHE_MANAGER_B_ADDRESS || '';

export default function Tranche_Manager_BPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tranche-manager-b/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tranche-manager-b',
