...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRADING_VOLUME_AUD_ADDRESS || '';

export default function Trading_Volume_AudPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.trading-volume-aud/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'trading-volume-aud',
