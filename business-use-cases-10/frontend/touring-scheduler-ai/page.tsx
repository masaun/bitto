...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TOURING_SCHEDULER_AI_ADDRESS || '';

export default function Touring_Scheduler_AiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.touring-scheduler-ai/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'touring-scheduler-ai',
