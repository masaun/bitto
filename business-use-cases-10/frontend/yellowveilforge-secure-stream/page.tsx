...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_YELLOWVEILFORGE_SECURE_STREAM_ADDRESS || '';

export default function Yellowveilforge_Secure_StreamPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'yellowveilforge-secure-stream',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.yellowveilforge-secure-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'yellowveilforge-secure-stream',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>yellowveilforge-secure-stream</h1>
