...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERIRECORD_CONTROLLED_RUNNER_ADDRESS || '';

export default function Verirecord_Controlled_RunnerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'verirecord-controlled-runner',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.verirecord-controlled-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'verirecord-controlled-runner',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>verirecord-controlled-runner</h1>
