...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERIGUARDCORE_CONTROLLED_ENGINE_ADDRESS || '';

export default function Veriguardcore_Controlled_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'veriguardcore-controlled-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veriguardcore-controlled-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veriguardcore-controlled-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>veriguardcore-controlled-engine</h1>
