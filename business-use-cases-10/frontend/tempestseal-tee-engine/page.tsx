...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TEMPESTSEAL_TEE_ENGINE_ADDRESS || '';

export default function Tempestseal_Tee_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tempestseal-tee-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tempestseal-tee-engine',
