...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERIWAVE_CONTROLLED_ORACLE_ADDRESS || '';

export default function Veriwave_Controlled_OraclePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'veriwave-controlled-oracle',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veriwave-controlled-oracle/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veriwave-controlled-oracle',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>veriwave-controlled-oracle</h1>
