...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_UTILITY_BILL_PARSER_ADDRESS || '';

export default function Utility_Bill_ParserPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.utility-bill-parser/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'utility-bill-parser',
