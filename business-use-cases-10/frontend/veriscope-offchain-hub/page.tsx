...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERISCOPE_OFFCHAIN_HUB_ADDRESS || '';

export default function Veriscope_Offchain_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veriscope-offchain-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veriscope-offchain-hub',
