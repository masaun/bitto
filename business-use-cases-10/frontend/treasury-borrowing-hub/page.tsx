...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TREASURY_BORROWING_HUB_ADDRESS || '';

export default function Treasury_Borrowing_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.treasury-borrowing-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'treasury-borrowing-hub',
