...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WITHHOLDING_TAX_ROUTER_ADDRESS || '';

export default function Withholding_Tax_RouterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.withholding-tax-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'withholding-tax-router',
