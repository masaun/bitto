...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_THETA_INTELLECTUAL_PROP_ADDRESS || '';

export default function Theta_Intellectual_PropPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.theta-intellectual-prop/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'theta-intellectual-prop',
