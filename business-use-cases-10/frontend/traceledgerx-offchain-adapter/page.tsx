...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACELEDGERX_OFFCHAIN_ADAPTER_ADDRESS || '';

export default function Traceledgerx_Offchain_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'traceledgerx-offchain-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.traceledgerx-offchain-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'traceledgerx-offchain-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>traceledgerx-offchain-adapter</h1>
