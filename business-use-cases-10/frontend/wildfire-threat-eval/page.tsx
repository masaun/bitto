...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WILDFIRE_THREAT_EVAL_ADDRESS || '';

export default function Wildfire_Threat_EvalPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.wildfire-threat-eval/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'wildfire-threat-eval',
