...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ZEPHYRCLOAK_TRUSTED_PROCESSOR_ADDRESS || '';

export default function Zephyrcloak_Trusted_ProcessorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'zephyrcloak-trusted-processor',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.zephyrcloak-trusted-processor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'zephyrcloak-trusted-processor',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>zephyrcloak-trusted-processor</h1>
