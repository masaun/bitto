...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ZENFORGE_PRIVATE_TRANSFORM_ADDRESS || '';

export default function Zenforge_Private_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'zenforge-private-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.zenforge-private-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'zenforge-private-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>zenforge-private-transform</h1>
