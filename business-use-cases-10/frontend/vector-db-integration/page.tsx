...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VECTOR_DB_INTEGRATION_ADDRESS || '';

export default function Vector_Db_IntegrationPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.vector-db-integration/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'vector-db-integration',
