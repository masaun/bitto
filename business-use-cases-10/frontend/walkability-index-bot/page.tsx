...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WALKABILITY_INDEX_BOT_ADDRESS || '';

export default function Walkability_Index_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.walkability-index-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'walkability-index-bot',
