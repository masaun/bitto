...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACEOBSERVER_OFFCHAIN_FETCH_ADDRESS || '';

export default function Traceobserver_Offchain_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'traceobserver-offchain-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.traceobserver-offchain-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'traceobserver-offchain-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>traceobserver-offchain-fetch</h1>
