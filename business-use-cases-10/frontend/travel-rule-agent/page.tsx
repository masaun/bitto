...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRAVEL_RULE_AGENT_ADDRESS || '';

export default function Travel_Rule_AgentPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.travel-rule-agent/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'travel-rule-agent',
