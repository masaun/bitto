...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERILIGHT_CONTROLLED_FETCH_ADDRESS || '';

export default function Verilight_Controlled_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'verilight-controlled-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.verilight-controlled-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'verilight-controlled-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>verilight-controlled-fetch</h1>
