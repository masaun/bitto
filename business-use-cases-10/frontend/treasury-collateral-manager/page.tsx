...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TREASURY_COLLATERAL_MANAGER_ADDRESS || '';

export default function Treasury_Collateral_ManagerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.treasury-collateral-manager/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'treasury-collateral-manager',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>treasury-collateral-manager</h1>
