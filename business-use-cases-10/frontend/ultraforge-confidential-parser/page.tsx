...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ULTRAFORGE_CONFIDENTIAL_PARSER_ADDRESS || '';

export default function Ultraforge_Confidential_ParserPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'ultraforge-confidential-parser',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ultraforge-confidential-parser/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ultraforge-confidential-parser',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>ultraforge-confidential-parser</h1>
