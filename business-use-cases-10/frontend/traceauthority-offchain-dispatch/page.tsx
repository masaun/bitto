...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACEAUTHORITY_OFFCHAIN_DISPATCH_ADDRESS || '';

export default function Traceauthority_Offchain_DispatchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'traceauthority-offchain-dispatch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.traceauthority-offchain-dispatch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'traceauthority-offchain-dispatch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>traceauthority-offchain-dispatch</h1>
