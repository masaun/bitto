...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERIARCHIVE_CONTROLLED_RUNNER_ADDRESS || '';

export default function Veriarchive_Controlled_RunnerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'veriarchive-controlled-runner',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veriarchive-controlled-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veriarchive-controlled-runner',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>veriarchive-controlled-runner</h1>
