...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACECONTROL_OFFCHAIN_RUNNER_ADDRESS || '';

export default function Tracecontrol_Offchain_RunnerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tracecontrol-offchain-runner',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tracecontrol-offchain-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tracecontrol-offchain-runner',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tracecontrol-offchain-runner</h1>
