...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WINDOW_SHADE_AUTO_ADDRESS || '';

export default function Window_Shade_AutoPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.window-shade-auto/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'window-shade-auto',
