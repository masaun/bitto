...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VORTEXCRESTVEILFORGE_CONFIDENTIAL_TRANSFORM_ADDRESS || '';

export default function Vortexcrestveilforge_Confidential_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'vortexcrestveilforge-confidential-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.vortexcrestveilforge-confidential-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'vortexcrestveilforge-confidential-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>vortexcrestveilforge-confidential-transform</h1>
