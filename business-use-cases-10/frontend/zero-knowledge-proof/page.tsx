...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ZERO_KNOWLEDGE_PROOF_ADDRESS || '';

export default function Zero_Knowledge_ProofPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.zero-knowledge-proof/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'zero-knowledge-proof',
