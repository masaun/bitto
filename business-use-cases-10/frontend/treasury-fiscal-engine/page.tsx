...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TREASURY_FISCAL_ENGINE_ADDRESS || '';

export default function Treasury_Fiscal_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.treasury-fiscal-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'treasury-fiscal-engine',
