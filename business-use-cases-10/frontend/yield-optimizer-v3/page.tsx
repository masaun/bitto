...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_YIELD_OPTIMIZER_V3_ADDRESS || '';

export default function Yield_Optimizer_V3Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.yield-optimizer-v3/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'yield-optimizer-v3',
