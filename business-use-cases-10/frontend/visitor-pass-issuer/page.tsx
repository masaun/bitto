...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VISITOR_PASS_ISSUER_ADDRESS || '';

export default function Visitor_Pass_IssuerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.visitor-pass-issuer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'visitor-pass-issuer',
