...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ULTRAVIOLETSEAL_SECURE_ROUTER_ADDRESS || '';

export default function Ultravioletseal_Secure_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'ultravioletseal-secure-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ultravioletseal-secure-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ultravioletseal-secure-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>ultravioletseal-secure-router</h1>
