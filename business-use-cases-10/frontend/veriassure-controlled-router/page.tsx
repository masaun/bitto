...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERIASSURE_CONTROLLED_ROUTER_ADDRESS || '';

export default function Veriassure_Controlled_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'veriassure-controlled-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veriassure-controlled-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veriassure-controlled-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>veriassure-controlled-router</h1>
