...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TOKEN_SALE_LAUNCH_ADDRESS || '';

export default function Token_Sale_LaunchPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.token-sale-launch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'token-sale-launch',
