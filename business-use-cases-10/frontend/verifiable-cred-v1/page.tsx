...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERIFIABLE_CRED_V1_ADDRESS || '';

export default function Verifiable_Cred_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.verifiable-cred-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'verifiable-cred-v1',
