...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACENODE_CONTROLLED_EXEC_ADDRESS || '';

export default function Tracenode_Controlled_ExecPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tracenode-controlled-exec',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tracenode-controlled-exec/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tracenode-controlled-exec',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tracenode-controlled-exec</h1>
