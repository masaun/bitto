...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ZENVEILFORGE_SECRET_STREAM_ADDRESS || '';

export default function Zenveilforge_Secret_StreamPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'zenveilforge-secret-stream',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.zenveilforge-secret-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'zenveilforge-secret-stream',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>zenveilforge-secret-stream</h1>
