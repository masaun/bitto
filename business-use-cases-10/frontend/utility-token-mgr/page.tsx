...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_UTILITY_TOKEN_MGR_ADDRESS || '';

export default function Utility_Token_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.utility-token-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'utility-token-mgr',
