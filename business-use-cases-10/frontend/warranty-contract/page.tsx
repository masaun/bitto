...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WARRANTY_CONTRACT_ADDRESS || '';

export default function Warranty_ContractPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.warranty-contract/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'warranty-contract',
