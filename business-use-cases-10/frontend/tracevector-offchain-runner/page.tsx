...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACEVECTOR_OFFCHAIN_RUNNER_ADDRESS || '';

export default function Tracevector_Offchain_RunnerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tracevector-offchain-runner',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tracevector-offchain-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tracevector-offchain-runner',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tracevector-offchain-runner</h1>
