...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERILEDGER_CONTROLLED_RUNNER_ADDRESS || '';

export default function Veriledger_Controlled_RunnerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'veriledger-controlled-runner',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veriledger-controlled-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veriledger-controlled-runner',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>veriledger-controlled-runner</h1>
