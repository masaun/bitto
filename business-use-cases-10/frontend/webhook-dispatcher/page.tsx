...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WEBHOOK_DISPATCHER_ADDRESS || '';

export default function Webhook_DispatcherPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.webhook-dispatcher/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'webhook-dispatcher',
