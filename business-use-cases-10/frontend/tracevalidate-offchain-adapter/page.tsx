...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACEVALIDATE_OFFCHAIN_ADAPTER_ADDRESS || '';

export default function Tracevalidate_Offchain_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tracevalidate-offchain-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tracevalidate-offchain-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tracevalidate-offchain-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tracevalidate-offchain-adapter</h1>
