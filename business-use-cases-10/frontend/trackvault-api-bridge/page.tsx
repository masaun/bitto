...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACKVAULT_API_BRIDGE_ADDRESS || '';

export default function Trackvault_Api_BridgePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.trackvault-api-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'trackvault-api-bridge',
