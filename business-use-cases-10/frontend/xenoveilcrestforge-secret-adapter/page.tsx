...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_XENOVEILCRESTFORGE_SECRET_ADAPTER_ADDRESS || '';

export default function Xenoveilcrestforge_Secret_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'xenoveilcrestforge-secret-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.xenoveilcrestforge-secret-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'xenoveilcrestforge-secret-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>xenoveilcrestforge-secret-adapter</h1>
