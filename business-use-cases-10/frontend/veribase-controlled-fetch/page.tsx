...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERIBASE_CONTROLLED_FETCH_ADDRESS || '';

export default function Veribase_Controlled_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'veribase-controlled-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veribase-controlled-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veribase-controlled-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>veribase-controlled-fetch</h1>
