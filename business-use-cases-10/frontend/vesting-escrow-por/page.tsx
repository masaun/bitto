...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VESTING_ESCROW_POR_ADDRESS || '';

export default function Vesting_Escrow_PorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.vesting-escrow-por/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'vesting-escrow-por',
