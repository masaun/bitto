...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_XBTC_BRIDGE_ADAPTER_ADDRESS || '';

export default function Xbtc_Bridge_AdapterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.xbtc-bridge-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'xbtc-bridge-adapter',
