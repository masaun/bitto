...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_XENOCRESTVEIL_SECURE_HUB_ADDRESS || '';

export default function Xenocrestveil_Secure_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.xenocrestveil-secure-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'xenocrestveil-secure-hub',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>xenocrestveil-secure-hub</h1>
