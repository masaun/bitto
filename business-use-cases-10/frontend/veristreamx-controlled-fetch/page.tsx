...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERISTREAMX_CONTROLLED_FETCH_ADDRESS || '';

export default function Veristreamx_Controlled_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'veristreamx-controlled-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veristreamx-controlled-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veristreamx-controlled-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>veristreamx-controlled-fetch</h1>
