...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TITLE_SEARCH_CRAWLER_ADDRESS || '';

export default function Title_Search_CrawlerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.title-search-crawler/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'title-search-crawler',
