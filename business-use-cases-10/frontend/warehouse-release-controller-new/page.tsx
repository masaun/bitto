...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WAREHOUSE_RELEASE_CONTROLLER_NEW_ADDRESS || '';

export default function Warehouse_Release_Controller_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'warehouse-release-controller-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.warehouse-release-controller-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'warehouse-release-controller-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>warehouse-release-controller-new</h1>
