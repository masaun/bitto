...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_AUTOMATED_MARKET_MAKER_ADDRESS || '';

export default function Automated_Market_MakerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.automated-market-maker/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'automated-market-maker',
