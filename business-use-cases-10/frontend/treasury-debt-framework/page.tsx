...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TREASURY_DEBT_FRAMEWORK_ADDRESS || '';

export default function Treasury_Debt_FrameworkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.treasury-debt-framework/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'treasury-debt-framework',
