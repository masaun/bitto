...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_XML_DATA_EXTRACTOR_ADDRESS || '';

export default function Xml_Data_ExtractorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.xml-data-extractor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'xml-data-extractor',
