...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ZENITHLOCK_CREDENTIAL_GUARD_ADDRESS || '';

export default function Zenithlock_Credential_GuardPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'zenithlock-credential-guard',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.zenithlock-credential-guard/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'zenithlock-credential-guard',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>zenithlock-credential-guard</h1>
