...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRANCHE_YIELD_OPTIMIZER_NEW_ADDRESS || '';

export default function Tranche_Yield_Optimizer_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tranche-yield-optimizer-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tranche-yield-optimizer-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tranche-yield-optimizer-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tranche-yield-optimizer-new</h1>
