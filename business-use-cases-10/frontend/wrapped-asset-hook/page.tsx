...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WRAPPED_ASSET_HOOK_ADDRESS || '';

export default function Wrapped_Asset_HookPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.wrapped-asset-hook/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'wrapped-asset-hook',
