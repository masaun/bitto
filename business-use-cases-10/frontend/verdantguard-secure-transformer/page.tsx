...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERDANTGUARD_SECURE_TRANSFORMER_ADDRESS || '';

export default function Verdantguard_Secure_TransformerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'verdantguard-secure-transformer',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.verdantguard-secure-transformer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'verdantguard-secure-transformer',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>verdantguard-secure-transformer</h1>
