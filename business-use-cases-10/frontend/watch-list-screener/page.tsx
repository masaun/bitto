...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WATCH_LIST_SCREENER_ADDRESS || '';

export default function Watch_List_ScreenerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.watch-list-screener/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'watch-list-screener',
