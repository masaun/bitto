...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WEIGHT_ADJ_FACTOR_ADDRESS || '';

export default function Weight_Adj_FactorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.weight-adj-factor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'weight-adj-factor',
