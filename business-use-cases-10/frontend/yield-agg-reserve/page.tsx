...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_YIELD_AGG_RESERVE_ADDRESS || '';

export default function Yield_Agg_ReservePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.yield-agg-reserve/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'yield-agg-reserve',
