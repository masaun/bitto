...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WELFARE_ALLOCATION_ROUTER_ADDRESS || '';

export default function Welfare_Allocation_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'welfare-allocation-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.welfare-allocation-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'welfare-allocation-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>welfare-allocation-router</h1>
