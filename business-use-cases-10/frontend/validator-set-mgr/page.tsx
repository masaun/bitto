...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VALIDATOR_SET_MGR_ADDRESS || '';

export default function Validator_Set_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.validator-set-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'validator-set-mgr',
