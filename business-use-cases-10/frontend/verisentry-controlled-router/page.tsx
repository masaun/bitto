...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERISENTRY_CONTROLLED_ROUTER_ADDRESS || '';

export default function Verisentry_Controlled_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'verisentry-controlled-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.verisentry-controlled-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'verisentry-controlled-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>verisentry-controlled-router</h1>
