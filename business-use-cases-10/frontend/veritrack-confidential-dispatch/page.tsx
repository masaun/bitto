...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_VERITRACK_CONFIDENTIAL_DISPATCH_ADDRESS || '';

export default function Veritrack_Confidential_DispatchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'veritrack-confidential-dispatch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.veritrack-confidential-dispatch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'veritrack-confidential-dispatch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>veritrack-confidential-dispatch</h1>
