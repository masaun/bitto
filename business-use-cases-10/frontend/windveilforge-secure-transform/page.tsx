...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WINDVEILFORGE_SECURE_TRANSFORM_ADDRESS || '';

export default function Windveilforge_Secure_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'windveilforge-secure-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.windveilforge-secure-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'windveilforge-secure-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>windveilforge-secure-transform</h1>
