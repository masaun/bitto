...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACEWATCH_OFFCHAIN_FETCH_ADDRESS || '';

export default function Tracewatch_Offchain_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tracewatch-offchain-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tracewatch-offchain-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tracewatch-offchain-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tracewatch-offchain-fetch</h1>
