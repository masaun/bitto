...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TENANT_SCREENING_LOGIC_ADDRESS || '';

export default function Tenant_Screening_LogicPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tenant-screening-logic/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tenant-screening-logic',
