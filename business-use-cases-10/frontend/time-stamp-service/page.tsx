...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TIME_STAMP_SERVICE_ADDRESS || '';

export default function Time_Stamp_ServicePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.time-stamp-service/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'time-stamp-service',
