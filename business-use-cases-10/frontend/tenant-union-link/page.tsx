...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TENANT_UNION_LINK_ADDRESS || '';

export default function Tenant_Union_LinkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tenant-union-link/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tenant-union-link',
