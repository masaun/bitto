...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACEBEACON_OFFCHAIN_ENGINE_ADDRESS || '';

export default function Tracebeacon_Offchain_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'tracebeacon-offchain-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.tracebeacon-offchain-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'tracebeacon-offchain-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>tracebeacon-offchain-engine</h1>
