...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_TRACKFORGE_CONFIDENTIAL_BRIDGE_ADDRESS || '';

export default function Trackforge_Confidential_BridgePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'trackforge-confidential-bridge',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.trackforge-confidential-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'trackforge-confidential-bridge',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>trackforge-confidential-bridge</h1>
