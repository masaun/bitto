...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_UPSILON_PREFERRED_STK_ADDRESS || '';

export default function Upsilon_Preferred_StkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.upsilon-preferred-stk/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'upsilon-preferred-stk',
