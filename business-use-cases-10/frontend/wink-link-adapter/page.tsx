...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_WINK_LINK_ADAPTER_ADDRESS || '';

export default function Wink_Link_AdapterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.wink-link-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'wink-link-adapter',
