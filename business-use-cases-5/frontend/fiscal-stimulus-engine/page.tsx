...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_STIMULUS_ENGINE_ADDRESS || '';

export default function Fiscal_Stimulus_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-stimulus-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-stimulus-engine',
