...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FORBEARANCE_TRACKER_NEW_ADDRESS || '';

export default function Forbearance_Tracker_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.forbearance-tracker-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'forbearance-tracker-new',
