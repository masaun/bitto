...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_MULTIPLIER_ENGINE_ADDRESS || '';

export default function Fiscal_Multiplier_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-multiplier-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-multiplier-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>fiscal-multiplier-engine</h1>
