...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GOVERNMENT_LIABILITY_TRACKER_ADDRESS || '';

export default function Government_Liability_TrackerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'government-liability-tracker',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.government-liability-tracker/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'government-liability-tracker',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>government-liability-tracker</h1>
