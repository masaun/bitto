...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_FINANCE_CORE_ADDRESS || '';

export default function Fiscal_Finance_CorePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-finance-core/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-finance-core',
