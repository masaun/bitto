...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FLASH_LOAN_REAL_ESTATE_ADDRESS || '';

export default function Flash_Loan_Real_EstatePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.flash-loan-real-estate/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'flash-loan-real-estate',
