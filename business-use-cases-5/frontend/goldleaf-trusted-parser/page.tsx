...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GOLDLEAF_TRUSTED_PARSER_ADDRESS || '';

export default function Goldleaf_Trusted_ParserPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.goldleaf-trusted-parser/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'goldleaf-trusted-parser',
