...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FEDNOW_INSTANT_PAY_ADDRESS || '';

export default function Fednow_Instant_PayPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fednow-instant-pay/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fednow-instant-pay',
