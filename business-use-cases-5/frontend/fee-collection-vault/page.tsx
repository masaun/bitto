...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FEE_COLLECTION_VAULT_ADDRESS || '';

export default function Fee_Collection_VaultPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fee-collection-vault/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fee-collection-vault',
