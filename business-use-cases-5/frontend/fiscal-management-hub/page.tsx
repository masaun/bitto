...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_MANAGEMENT_HUB_ADDRESS || '';

export default function Fiscal_Management_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-management-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-management-hub',
