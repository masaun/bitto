...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FROSTVEIL_TEE_ADAPTER_ADDRESS || '';

export default function Frostveil_Tee_AdapterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.frostveil-tee-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'frostveil-tee-adapter',
