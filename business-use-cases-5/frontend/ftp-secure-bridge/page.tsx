...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FTP_SECURE_BRIDGE_ADDRESS || '';

export default function Ftp_Secure_BridgePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ftp-secure-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ftp-secure-bridge',
