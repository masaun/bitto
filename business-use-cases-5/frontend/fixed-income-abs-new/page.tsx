...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FIXED_INCOME_ABS_NEW_ADDRESS || '';

export default function Fixed_Income_Abs_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fixed-income-abs-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fixed-income-abs-new',
