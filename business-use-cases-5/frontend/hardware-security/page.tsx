...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HARDWARE_SECURITY_ADDRESS || '';

export default function Hardware_SecurityPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.hardware-security/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'hardware-security',
