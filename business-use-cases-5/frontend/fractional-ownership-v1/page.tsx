...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FRACTIONAL_OWNERSHIP_V1_ADDRESS || '';

export default function Fractional_Ownership_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fractional-ownership-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fractional-ownership-v1',
