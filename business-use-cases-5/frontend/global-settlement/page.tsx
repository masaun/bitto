...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GLOBAL_SETTLEMENT_ADDRESS || '';

export default function Global_SettlementPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.global-settlement/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'global-settlement',
