...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HALCYONVEILFORGE_SECRET_ROUTER_ADDRESS || '';

export default function Halcyonveilforge_Secret_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'halcyonveilforge-secret-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.halcyonveilforge-secret-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'halcyonveilforge-secret-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>halcyonveilforge-secret-router</h1>
