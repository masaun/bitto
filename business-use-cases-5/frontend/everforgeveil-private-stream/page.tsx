...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EVERFORGEVEIL_PRIVATE_STREAM_ADDRESS || '';

export default function Everforgeveil_Private_StreamPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'everforgeveil-private-stream',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.everforgeveil-private-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'everforgeveil-private-stream',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>everforgeveil-private-stream</h1>
