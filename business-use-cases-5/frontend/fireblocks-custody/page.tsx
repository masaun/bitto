...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FIREBLOCKS_CUSTODY_ADDRESS || '';

export default function Fireblocks_CustodyPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fireblocks-custody/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fireblocks-custody',
