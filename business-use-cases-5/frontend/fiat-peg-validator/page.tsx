...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FIAT_PEG_VALIDATOR_ADDRESS || '';

export default function Fiat_Peg_ValidatorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiat-peg-validator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiat-peg-validator',
