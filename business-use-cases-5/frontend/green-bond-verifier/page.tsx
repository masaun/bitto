...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GREEN_BOND_VERIFIER_ADDRESS || '';

export default function Green_Bond_VerifierPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.green-bond-verifier/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'green-bond-verifier',
