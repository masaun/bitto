...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FLAMEGUARD_PRIVATE_TRANSFORM_ADDRESS || '';

export default function Flameguard_Private_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'flameguard-private-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.flameguard-private-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'flameguard-private-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>flameguard-private-transform</h1>
