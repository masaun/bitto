...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INCOME_TAX_COLLECTOR_ADDRESS || '';

export default function Income_Tax_CollectorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.income-tax-collector/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'income-tax-collector',
