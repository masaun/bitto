...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_PLANNING_CORE_ADDRESS || '';

export default function Fiscal_Planning_CorePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-planning-core/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-planning-core',
