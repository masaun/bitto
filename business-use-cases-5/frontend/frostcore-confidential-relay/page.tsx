...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FROSTCORE_CONFIDENTIAL_RELAY_ADDRESS || '';

export default function Frostcore_Confidential_RelayPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'frostcore-confidential-relay',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.frostcore-confidential-relay/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'frostcore-confidential-relay',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>frostcore-confidential-relay</h1>
