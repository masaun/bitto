...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FUEL_NETWORK_LINK_ADDRESS || '';

export default function Fuel_Network_LinkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fuel-network-link/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fuel-network-link',
