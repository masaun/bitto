...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EXTENDABLE_MATURITY_ENGINE_NEW_ADDRESS || '';

export default function Extendable_Maturity_Engine_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'extendable-maturity-engine-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.extendable-maturity-engine-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'extendable-maturity-engine-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>extendable-maturity-engine-new</h1>
