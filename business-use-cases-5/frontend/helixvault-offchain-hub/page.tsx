...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HELIXVAULT_OFFCHAIN_HUB_ADDRESS || '';

export default function Helixvault_Offchain_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.helixvault-offchain-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'helixvault-offchain-hub',
