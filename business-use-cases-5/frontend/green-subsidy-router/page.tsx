...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GREEN_SUBSIDY_ROUTER_ADDRESS || '';

export default function Green_Subsidy_RouterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.green-subsidy-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'green-subsidy-router',
