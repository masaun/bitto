...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FINAL_WORKFLOW_CLOSER_ADDRESS || '';

export default function Final_Workflow_CloserPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.final-workflow-closer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'final-workflow-closer',
