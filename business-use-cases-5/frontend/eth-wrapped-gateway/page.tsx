...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ETH_WRAPPED_GATEWAY_ADDRESS || '';

export default function Eth_Wrapped_GatewayPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.eth-wrapped-gateway/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'eth-wrapped-gateway',
