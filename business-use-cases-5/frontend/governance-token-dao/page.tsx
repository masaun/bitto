...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GOVERNANCE_TOKEN_DAO_ADDRESS || '';

export default function Governance_Token_DaoPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.governance-token-dao/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'governance-token-dao',
