...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HISTORICAL_PRESERV_ADDRESS || '';

export default function Historical_PreservPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.historical-preserv/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'historical-preserv',
