...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EVERCLOAK_SECURE_HARVESTER_ADDRESS || '';

export default function Evercloak_Secure_HarvesterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'evercloak-secure-harvester',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.evercloak-secure-harvester/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'evercloak-secure-harvester',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>evercloak-secure-harvester</h1>
