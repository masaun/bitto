...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_SPENDING_ENGINE_ADDRESS || '';

export default function Fiscal_Spending_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-spending-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-spending-engine',
