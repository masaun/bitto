...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GOLD_BACKED_ASSET_ADDRESS || '';

export default function Gold_Backed_AssetPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.gold-backed-asset/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'gold-backed-asset',
