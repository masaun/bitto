...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GOVERNANCE_V2_EXT_ADDRESS || '';

export default function Governance_V2_ExtPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.governance-v2-ext/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'governance-v2-ext',
