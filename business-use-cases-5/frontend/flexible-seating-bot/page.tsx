...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FLEXIBLE_SEATING_BOT_ADDRESS || '';

export default function Flexible_Seating_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.flexible-seating-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'flexible-seating-bot',
