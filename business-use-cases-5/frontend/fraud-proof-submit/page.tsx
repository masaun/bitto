...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FRAUD_PROOF_SUBMIT_ADDRESS || '';

export default function Fraud_Proof_SubmitPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fraud-proof-submit/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fraud-proof-submit',
