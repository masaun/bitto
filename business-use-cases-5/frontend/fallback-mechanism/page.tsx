...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FALLBACK_MECHANISM_ADDRESS || '';

export default function Fallback_MechanismPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fallback-mechanism/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fallback-mechanism',
