...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GAS_REFUND_RELAYER_ADDRESS || '';

export default function Gas_Refund_RelayerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.gas-refund-relayer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'gas-refund-relayer',
