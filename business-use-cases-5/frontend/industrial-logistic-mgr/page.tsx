...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INDUSTRIAL_LOGISTIC_MGR_ADDRESS || '';

export default function Industrial_Logistic_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.industrial-logistic-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'industrial-logistic-mgr',
