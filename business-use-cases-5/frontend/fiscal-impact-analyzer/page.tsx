...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_IMPACT_ANALYZER_ADDRESS || '';

export default function Fiscal_Impact_AnalyzerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-impact-analyzer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-impact-analyzer',
