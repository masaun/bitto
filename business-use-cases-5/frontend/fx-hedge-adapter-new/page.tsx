...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FX_HEDGE_ADAPTER_NEW_ADDRESS || '';

export default function Fx_Hedge_Adapter_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fx-hedge-adapter-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fx-hedge-adapter-new',
