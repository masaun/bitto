...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ETHICAL_USAGE_AUDIT_ADDRESS || '';

export default function Ethical_Usage_AuditPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ethical-usage-audit/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ethical-usage-audit',
