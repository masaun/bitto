...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GAMMA_EQUITY_TOKEN_ADDRESS || '';

export default function Gamma_Equity_TokenPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.gamma-equity-token/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'gamma-equity-token',
