...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GENESIS_BLOCK_INF_ADDRESS || '';

export default function Genesis_Block_InfPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.genesis-block-inf/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'genesis-block-inf',
