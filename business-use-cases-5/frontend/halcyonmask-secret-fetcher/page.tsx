...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HALCYONMASK_SECRET_FETCHER_ADDRESS || '';

export default function Halcyonmask_Secret_FetcherPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'halcyonmask-secret-fetcher',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.halcyonmask-secret-fetcher/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'halcyonmask-secret-fetcher',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>halcyonmask-secret-fetcher</h1>
