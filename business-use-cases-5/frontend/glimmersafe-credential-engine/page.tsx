...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GLIMMERSAFE_CREDENTIAL_ENGINE_ADDRESS || '';

export default function Glimmersafe_Credential_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'glimmersafe-credential-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.glimmersafe-credential-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'glimmersafe-credential-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>glimmersafe-credential-engine</h1>
