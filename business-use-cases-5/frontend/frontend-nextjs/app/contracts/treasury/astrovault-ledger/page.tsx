...
export default function AstrovaultLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('astrovault-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'astrovault-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'astrovault-ledger' },
      });
