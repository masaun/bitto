...
export default function TreasurySchedulerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-scheduler');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-scheduler',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-scheduler' },
      });
