...
export default function TournamentPrizeVaultPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tournament-prize-vault');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tournament-prize-vault',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tournament-prize-vault' },
      });
