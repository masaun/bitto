...
export default function InfrastructureFundManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('infrastructure-fund-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'infrastructure-fund-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'infrastructure-fund-manager' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Infrastructure Fund Manager</h1>
