...
export default function TreasuryAllocatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-allocator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-allocator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-allocator' },
      });
