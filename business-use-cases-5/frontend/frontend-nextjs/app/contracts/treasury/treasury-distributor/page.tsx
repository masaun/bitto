...
export default function TreasuryDistributorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-distributor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-distributor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-distributor' },
      });
