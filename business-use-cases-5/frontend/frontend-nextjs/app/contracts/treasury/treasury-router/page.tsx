...
export default function TreasuryRouterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-router');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-router',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-router' },
      });
