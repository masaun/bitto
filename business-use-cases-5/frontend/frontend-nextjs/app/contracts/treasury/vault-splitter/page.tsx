...
export default function VaultSplitterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('vault-splitter');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'vault-splitter',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'vault-splitter' },
      });
