...
export default function LocalMultiSigVaultPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-multi-sig-vault');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-multi-sig-vault',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-multi-sig-vault' },
      });
