...
export default function LegacyInfrastructureFundPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('legacy-infrastructure-fund');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'legacy-infrastructure-fund',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'legacy-infrastructure-fund' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Legacy Infrastructure Fund</h1>
