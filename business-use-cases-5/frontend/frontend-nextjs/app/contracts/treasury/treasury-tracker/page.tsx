...
export default function TreasuryTrackerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-tracker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-tracker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-tracker' },
      });
