...
export default function FinancialVaultRouter3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('financial-vault-router-3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'financial-vault-router-3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'financial-vault-router-3' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Financial Vault Router 3</h1>
