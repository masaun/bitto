...
export default function InstitutionalVaultGateway5Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('institutional-vault-gateway-5');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'institutional-vault-gateway-5',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'institutional-vault-gateway-5' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Institutional Vault Gateway 5</h1>
