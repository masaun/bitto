...
export default function TreasuryInspectorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-inspector');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-inspector',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-inspector' },
      });
