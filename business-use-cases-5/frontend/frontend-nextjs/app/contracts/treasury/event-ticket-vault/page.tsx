...
export default function EventTicketVaultPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-ticket-vault');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-ticket-vault',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-ticket-vault' },
      });
