...
export default function EventSustainabilityFundPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-sustainability-fund');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-sustainability-fund',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-sustainability-fund' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Event Sustainability Fund</h1>
