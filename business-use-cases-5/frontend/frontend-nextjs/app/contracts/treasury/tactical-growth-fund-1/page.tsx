...
export default function TacticalGrowthFund1Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tactical-growth-fund-1');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tactical-growth-fund-1',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tactical-growth-fund-1' },
      });
