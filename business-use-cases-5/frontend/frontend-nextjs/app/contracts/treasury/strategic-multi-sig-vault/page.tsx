...
export default function StrategicMultiSigVaultPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('strategic-multi-sig-vault');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'strategic-multi-sig-vault',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'strategic-multi-sig-vault' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Strategic Multi Sig Vault</h1>
