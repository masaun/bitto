...
export default function TacticalVaultRouter1Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tactical-vault-router-1');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tactical-vault-router-1',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tactical-vault-router-1' },
      });
