...
export default function FinancialVaultRelay3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('financial-vault-relay-3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'financial-vault-relay-3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'financial-vault-relay-3' },
      });
