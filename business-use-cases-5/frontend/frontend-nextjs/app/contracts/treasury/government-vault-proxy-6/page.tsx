...
export default function GovernmentVaultProxy6Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('government-vault-proxy-6');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'government-vault-proxy-6',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'government-vault-proxy-6' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Government Vault Proxy 6</h1>
