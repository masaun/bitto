...
export default function OpeningShowTicketVaultPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('opening-show-ticket-vault');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'opening-show-ticket-vault',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'opening-show-ticket-vault' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Opening Show Ticket Vault</h1>
