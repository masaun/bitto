...
export default function CorporateGrowthFund4Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('corporate-growth-fund-4');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'corporate-growth-fund-4',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'corporate-growth-fund-4' },
      });
