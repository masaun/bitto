...
export default function TacticalCapitalPool1Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tactical-capital-pool-1');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tactical-capital-pool-1',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tactical-capital-pool-1' },
      });
