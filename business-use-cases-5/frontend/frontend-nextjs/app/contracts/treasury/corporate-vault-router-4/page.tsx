...
export default function CorporateVaultRouter4Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('corporate-vault-router-4');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'corporate-vault-router-4',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'corporate-vault-router-4' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Corporate Vault Router 4</h1>
