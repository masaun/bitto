...
export default function ChampionshipYouthFundPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('championship-youth-fund');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'championship-youth-fund',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'championship-youth-fund' },
      });
