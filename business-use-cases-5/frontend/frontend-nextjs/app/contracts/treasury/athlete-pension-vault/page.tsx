...
export default function AthletePensionVaultPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('athlete-pension-vault');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'athlete-pension-vault',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'athlete-pension-vault' },
      });
