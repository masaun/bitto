...
export default function MultiSigVaultPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('multi-sig-vault');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'multi-sig-vault',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'multi-sig-vault' },
      });
