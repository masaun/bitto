...
export default function TreasuryCheckerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-checker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-checker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-checker' },
      });
