...
export default function TacticalReserveFund1Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tactical-reserve-fund-1');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tactical-reserve-fund-1',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tactical-reserve-fund-1' },
      });
