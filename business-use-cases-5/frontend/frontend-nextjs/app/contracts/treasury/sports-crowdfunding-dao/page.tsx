...
export default function SportsCrowdfundingDaoPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-crowdfunding-dao');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-crowdfunding-dao',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-crowdfunding-dao' },
      });
