...
export default function InstitutionalVaultAggregator5Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('institutional-vault-aggregator-5');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'institutional-vault-aggregator-5',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'institutional-vault-aggregator-5' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Institutional Vault Aggregator 5</h1>
