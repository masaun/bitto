...
export default function SportsBonusVaultPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-bonus-vault');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-bonus-vault',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-bonus-vault' },
      });
