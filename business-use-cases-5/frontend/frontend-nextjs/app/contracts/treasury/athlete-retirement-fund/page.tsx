...
export default function AthleteRetirementFundPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('athlete-retirement-fund');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'athlete-retirement-fund',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'athlete-retirement-fund' },
      });
