...
export default function OperationalVaultProxy2Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('operational-vault-proxy-2');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'operational-vault-proxy-2',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'operational-vault-proxy-2' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Operational Vault Proxy 2</h1>
