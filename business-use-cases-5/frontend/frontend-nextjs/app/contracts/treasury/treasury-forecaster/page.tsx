...
export default function TreasuryForecasterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-forecaster');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-forecaster',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-forecaster' },
      });
