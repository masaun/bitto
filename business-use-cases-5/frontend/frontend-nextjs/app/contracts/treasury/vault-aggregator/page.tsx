...
export default function VaultAggregatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('vault-aggregator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'vault-aggregator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'vault-aggregator' },
      });
