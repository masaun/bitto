...
export default function DeploymentFundPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('deployment-fund');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'deployment-fund',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'deployment-fund' },
      });
