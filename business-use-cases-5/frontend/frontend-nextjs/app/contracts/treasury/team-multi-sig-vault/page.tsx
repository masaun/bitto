...
export default function TeamMultiSigVaultPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-multi-sig-vault');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-multi-sig-vault',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-multi-sig-vault' },
      });
