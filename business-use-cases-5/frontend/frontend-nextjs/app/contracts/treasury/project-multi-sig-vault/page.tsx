...
export default function ProjectMultiSigVaultPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('project-multi-sig-vault');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'project-multi-sig-vault',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'project-multi-sig-vault' },
      });
