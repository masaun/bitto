...
export default function TreasuryAggregatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-aggregator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-aggregator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-aggregator' },
      });
