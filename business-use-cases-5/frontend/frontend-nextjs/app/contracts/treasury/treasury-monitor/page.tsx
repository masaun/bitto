...
export default function TreasuryMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-monitor' },
      });
