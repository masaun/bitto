...
export default function VenueHospitalityVaultPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('venue-hospitality-vault');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'venue-hospitality-vault',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'venue-hospitality-vault' },
      });
