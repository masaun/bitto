...
export default function TreasuryProxyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-proxy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-proxy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-proxy' },
      });
