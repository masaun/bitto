...
export default function TreasuryReporterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-reporter');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-reporter',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-reporter' },
      });
