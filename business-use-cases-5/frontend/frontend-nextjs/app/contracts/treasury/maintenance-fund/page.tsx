...
export default function MaintenanceFundPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('maintenance-fund');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'maintenance-fund',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'maintenance-fund' },
      });
