...
export default function DivisionalMultiSigVaultPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('divisional-multi-sig-vault');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'divisional-multi-sig-vault',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'divisional-multi-sig-vault' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Divisional Multi Sig Vault</h1>
