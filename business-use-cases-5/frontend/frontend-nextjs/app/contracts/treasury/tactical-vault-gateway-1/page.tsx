...
export default function TacticalVaultGateway1Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tactical-vault-gateway-1');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tactical-vault-gateway-1',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tactical-vault-gateway-1' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Tactical Vault Gateway 1</h1>
