...
export default function StrategicFundPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('strategic-fund');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'strategic-fund',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'strategic-fund' },
      });
