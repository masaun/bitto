...
export default function QualifyingRoundFundPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('qualifying-round-fund');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'qualifying-round-fund',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'qualifying-round-fund' },
      });
