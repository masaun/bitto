...
export default function TreasurySplitterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('treasury-splitter');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'treasury-splitter',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'treasury-splitter' },
      });
