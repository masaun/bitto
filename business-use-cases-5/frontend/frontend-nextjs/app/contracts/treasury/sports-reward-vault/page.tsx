...
export default function SportsRewardVaultPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-reward-vault');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-reward-vault',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-reward-vault' },
      });
