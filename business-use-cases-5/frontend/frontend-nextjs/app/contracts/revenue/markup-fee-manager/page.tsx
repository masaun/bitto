...
export default function MarkupFeeManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('markup-fee-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'markup-fee-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'markup-fee-manager' },
      });
