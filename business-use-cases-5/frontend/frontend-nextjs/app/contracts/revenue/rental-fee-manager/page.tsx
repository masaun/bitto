...
export default function RentalFeeManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rental-fee-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rental-fee-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rental-fee-manager' },
      });
