...
export default function RentalRevenueAnalyzerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rental-revenue-analyzer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rental-revenue-analyzer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rental-revenue-analyzer' },
      });
