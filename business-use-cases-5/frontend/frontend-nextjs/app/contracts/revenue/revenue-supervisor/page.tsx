...
export default function RevenueSupervisorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('revenue-supervisor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'revenue-supervisor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'revenue-supervisor' },
      });
