...
export default function RoyaltyRevenueTrackerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('royalty-revenue-tracker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'royalty-revenue-tracker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'royalty-revenue-tracker' },
      });
