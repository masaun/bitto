...
export default function RoyaltyPaymentDistributionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('royalty-payment-distribution');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'royalty-payment-distribution',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'royalty-payment-distribution' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Royalty Payment Distribution</h1>
