...
export default function RentalRevenueSplitterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rental-revenue-splitter');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rental-revenue-splitter',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rental-revenue-splitter' },
      });
