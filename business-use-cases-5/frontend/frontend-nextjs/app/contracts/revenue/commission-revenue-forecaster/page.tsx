...
export default function CommissionRevenueForecasterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('commission-revenue-forecaster');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'commission-revenue-forecaster',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'commission-revenue-forecaster' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Commission Revenue Forecaster</h1>
