...
export default function ReferralRevenueReporterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('referral-revenue-reporter');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'referral-revenue-reporter',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'referral-revenue-reporter' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Referral Revenue Reporter</h1>
