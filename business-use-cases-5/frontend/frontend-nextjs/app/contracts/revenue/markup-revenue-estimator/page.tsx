...
export default function MarkupRevenueEstimatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('markup-revenue-estimator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'markup-revenue-estimator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'markup-revenue-estimator' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Markup Revenue Estimator</h1>
