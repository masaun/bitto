...
export default function MarkupRevenueCheckerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('markup-revenue-checker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'markup-revenue-checker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'markup-revenue-checker' },
      });
