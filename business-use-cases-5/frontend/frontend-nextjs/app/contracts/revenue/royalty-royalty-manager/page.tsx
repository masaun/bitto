...
export default function RoyaltyRoyaltyManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('royalty-royalty-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'royalty-royalty-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'royalty-royalty-manager' },
      });
