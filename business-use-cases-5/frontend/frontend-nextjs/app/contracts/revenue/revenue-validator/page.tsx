...
export default function RevenueValidatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('revenue-validator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'revenue-validator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'revenue-validator' },
      });
