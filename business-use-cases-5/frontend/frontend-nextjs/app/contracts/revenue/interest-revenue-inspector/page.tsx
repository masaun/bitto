...
export default function InterestRevenueInspectorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('interest-revenue-inspector');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'interest-revenue-inspector',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'interest-revenue-inspector' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Interest Revenue Inspector</h1>
