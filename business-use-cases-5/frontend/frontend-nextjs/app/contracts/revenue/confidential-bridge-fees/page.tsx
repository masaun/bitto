...
export default function ConfidentialBridgeFeesPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('confidential-bridge-fees');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'confidential-bridge-fees',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'confidential-bridge-fees' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Confidential Bridge Fees</h1>
