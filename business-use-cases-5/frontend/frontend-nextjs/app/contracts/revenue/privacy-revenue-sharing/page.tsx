...
export default function PrivacyRevenueSharingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-revenue-sharing');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-revenue-sharing',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-revenue-sharing' },
      });
