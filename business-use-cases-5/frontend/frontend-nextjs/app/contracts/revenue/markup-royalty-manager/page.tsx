...
export default function MarkupRoyaltyManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('markup-royalty-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'markup-royalty-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'markup-royalty-manager' },
      });
