...
export default function ReferralRevenueExaminerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('referral-revenue-examiner');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'referral-revenue-examiner',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'referral-revenue-examiner' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Referral Revenue Examiner</h1>
