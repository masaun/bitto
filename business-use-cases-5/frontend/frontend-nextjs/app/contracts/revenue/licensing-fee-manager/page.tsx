...
export default function LicensingFeeManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('licensing-fee-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'licensing-fee-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'licensing-fee-manager' },
      });
