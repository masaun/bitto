...
export default function UsageRevenueAnalyzerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('usage-revenue-analyzer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'usage-revenue-analyzer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'usage-revenue-analyzer' },
      });
