...
export default function RentalRevenueReporterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rental-revenue-reporter');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rental-revenue-reporter',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rental-revenue-reporter' },
      });
