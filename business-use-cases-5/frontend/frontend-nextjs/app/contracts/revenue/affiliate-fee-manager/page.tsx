...
export default function AffiliateFeeManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('affiliate-fee-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'affiliate-fee-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'affiliate-fee-manager' },
      });
