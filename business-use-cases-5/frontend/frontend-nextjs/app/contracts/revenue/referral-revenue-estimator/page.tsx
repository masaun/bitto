...
export default function ReferralRevenueEstimatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('referral-revenue-estimator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'referral-revenue-estimator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'referral-revenue-estimator' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Referral Revenue Estimator</h1>
