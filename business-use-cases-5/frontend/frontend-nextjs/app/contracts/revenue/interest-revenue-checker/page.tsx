...
export default function InterestRevenueCheckerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('interest-revenue-checker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'interest-revenue-checker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'interest-revenue-checker' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Interest Revenue Checker</h1>
