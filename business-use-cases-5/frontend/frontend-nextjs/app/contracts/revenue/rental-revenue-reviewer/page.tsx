...
export default function RentalRevenueReviewerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rental-revenue-reviewer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rental-revenue-reviewer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rental-revenue-reviewer' },
      });
