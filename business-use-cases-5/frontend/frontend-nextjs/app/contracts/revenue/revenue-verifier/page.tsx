...
export default function RevenueVerifierPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('revenue-verifier');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'revenue-verifier',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'revenue-verifier' },
      });
