...
export default function RevenueCombinerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('revenue-combiner');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'revenue-combiner',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'revenue-combiner' },
      });
