...
export default function InterestFeeManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('interest-fee-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'interest-fee-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'interest-fee-manager' },
      });
