...
export default function ServiceFeeRevenueInspectorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('service-fee-revenue-inspector');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'service-fee-revenue-inspector',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'service-fee-revenue-inspector' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Service Fee Revenue Inspector</h1>
