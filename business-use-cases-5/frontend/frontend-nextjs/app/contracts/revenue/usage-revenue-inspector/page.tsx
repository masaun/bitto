...
export default function UsageRevenueInspectorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('usage-revenue-inspector');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'usage-revenue-inspector',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'usage-revenue-inspector' },
      });
