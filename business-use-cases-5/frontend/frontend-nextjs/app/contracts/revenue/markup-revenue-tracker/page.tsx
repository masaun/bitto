...
export default function MarkupRevenueTrackerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('markup-revenue-tracker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'markup-revenue-tracker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'markup-revenue-tracker' },
      });
