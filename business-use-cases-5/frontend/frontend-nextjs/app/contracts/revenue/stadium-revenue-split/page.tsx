...
export default function StadiumRevenueSplitPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('stadium-revenue-split');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'stadium-revenue-split',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'stadium-revenue-split' },
      });
