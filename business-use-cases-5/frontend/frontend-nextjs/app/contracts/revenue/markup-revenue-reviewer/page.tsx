...
export default function MarkupRevenueReviewerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('markup-revenue-reviewer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'markup-revenue-reviewer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'markup-revenue-reviewer' },
      });
