...
export default function GlobalSportsRevenuePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('global-sports-revenue');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'global-sports-revenue',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'global-sports-revenue' },
      });
