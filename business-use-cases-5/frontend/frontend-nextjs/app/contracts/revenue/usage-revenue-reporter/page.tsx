...
export default function UsageRevenueReporterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('usage-revenue-reporter');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'usage-revenue-reporter',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'usage-revenue-reporter' },
      });
