...
export default function CommissionRevenueInspectorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('commission-revenue-inspector');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'commission-revenue-inspector',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'commission-revenue-inspector' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Commission Revenue Inspector</h1>
