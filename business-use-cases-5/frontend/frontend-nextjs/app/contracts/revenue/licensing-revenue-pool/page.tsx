...
export default function LicensingRevenuePoolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('licensing-revenue-pool');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'licensing-revenue-pool',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'licensing-revenue-pool' },
      });
