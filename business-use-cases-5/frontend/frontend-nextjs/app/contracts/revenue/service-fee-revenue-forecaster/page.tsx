...
export default function ServiceFeeRevenueForecasterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('service-fee-revenue-forecaster');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'service-fee-revenue-forecaster',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'service-fee-revenue-forecaster' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Service Fee Revenue Forecaster</h1>
