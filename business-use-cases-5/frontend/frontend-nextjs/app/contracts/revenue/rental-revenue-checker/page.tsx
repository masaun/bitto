...
export default function RentalRevenueCheckerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rental-revenue-checker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rental-revenue-checker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rental-revenue-checker' },
      });
