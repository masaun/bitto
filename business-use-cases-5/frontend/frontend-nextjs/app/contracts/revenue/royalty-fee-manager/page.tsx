...
export default function RoyaltyFeeManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('royalty-fee-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'royalty-fee-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'royalty-fee-manager' },
      });
