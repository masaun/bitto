...
export default function UsageRevenueForecasterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('usage-revenue-forecaster');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'usage-revenue-forecaster',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'usage-revenue-forecaster' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Usage Revenue Forecaster</h1>
