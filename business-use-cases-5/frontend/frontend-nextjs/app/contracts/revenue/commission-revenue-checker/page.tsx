...
export default function CommissionRevenueCheckerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('commission-revenue-checker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'commission-revenue-checker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'commission-revenue-checker' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Commission Revenue Checker</h1>
