...
export default function UsageRevenueCalculatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('usage-revenue-calculator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'usage-revenue-calculator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'usage-revenue-calculator' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Usage Revenue Calculator</h1>
