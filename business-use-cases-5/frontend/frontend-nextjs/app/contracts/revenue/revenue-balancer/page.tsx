...
export default function RevenueBalancerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('revenue-balancer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'revenue-balancer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'revenue-balancer' },
      });
