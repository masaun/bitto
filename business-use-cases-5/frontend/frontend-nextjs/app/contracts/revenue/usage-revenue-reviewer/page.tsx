...
export default function UsageRevenueReviewerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('usage-revenue-reviewer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'usage-revenue-reviewer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'usage-revenue-reviewer' },
      });
