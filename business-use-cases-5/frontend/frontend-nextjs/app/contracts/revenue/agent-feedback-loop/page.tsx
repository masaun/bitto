...
export default function AgentFeedbackLoopPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-feedback-loop');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-feedback-loop',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-feedback-loop' },
      });
