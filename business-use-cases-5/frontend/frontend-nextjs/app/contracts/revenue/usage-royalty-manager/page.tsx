...
export default function UsageRoyaltyManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('usage-royalty-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'usage-royalty-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'usage-royalty-manager' },
      });
