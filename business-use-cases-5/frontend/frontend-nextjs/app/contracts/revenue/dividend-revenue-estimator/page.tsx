...
export default function DividendRevenueEstimatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('dividend-revenue-estimator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'dividend-revenue-estimator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'dividend-revenue-estimator' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Dividend Revenue Estimator</h1>
