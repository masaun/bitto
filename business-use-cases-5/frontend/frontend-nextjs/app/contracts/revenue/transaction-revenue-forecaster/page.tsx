...
export default function TransactionRevenueForecasterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('transaction-revenue-forecaster');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'transaction-revenue-forecaster',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'transaction-revenue-forecaster' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Transaction Revenue Forecaster</h1>
