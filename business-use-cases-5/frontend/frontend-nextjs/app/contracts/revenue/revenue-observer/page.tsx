...
export default function RevenueObserverPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('revenue-observer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'revenue-observer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'revenue-observer' },
      });
