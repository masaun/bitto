...
export default function InterestRevenueTrackerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('interest-revenue-tracker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'interest-revenue-tracker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'interest-revenue-tracker' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Interest Revenue Tracker</h1>
