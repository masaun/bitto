...
export default function EventMerchRoyaltyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-merch-royalty');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-merch-royalty',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-merch-royalty' },
      });
