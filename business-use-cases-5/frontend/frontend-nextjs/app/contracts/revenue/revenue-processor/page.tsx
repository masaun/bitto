...
export default function RevenueProcessorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('revenue-processor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'revenue-processor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'revenue-processor' },
      });
