...
export default function TransactionRevenueSplitterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('transaction-revenue-splitter');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'transaction-revenue-splitter',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'transaction-revenue-splitter' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Transaction Revenue Splitter</h1>
