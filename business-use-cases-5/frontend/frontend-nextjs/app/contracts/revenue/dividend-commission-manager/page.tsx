...
export default function DividendCommissionManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('dividend-commission-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'dividend-commission-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'dividend-commission-manager' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Dividend Commission Manager</h1>
