...
export default function ScheduledStateMachinePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('scheduled-state-machine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'scheduled-state-machine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'scheduled-state-machine' },
      });
