...
export default function AuctionSplitterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-splitter');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-splitter',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-splitter' },
      });
