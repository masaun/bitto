...
export default function ApprovalManagerMifidPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('approval-manager-mifid');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'approval-manager-mifid',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'approval-manager-mifid' },
      });
