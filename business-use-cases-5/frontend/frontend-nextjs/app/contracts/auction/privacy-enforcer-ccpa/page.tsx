...
export default function PrivacyEnforcerCcpaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-enforcer-ccpa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-enforcer-ccpa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-enforcer-ccpa' },
      });
