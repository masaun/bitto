...
export default function FairOrderingEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('fair-ordering-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'fair-ordering-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'fair-ordering-engine' },
      });
