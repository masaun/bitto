...
export default function AgentDataDeletionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-data-deletion');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-data-deletion',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-data-deletion' },
      });
