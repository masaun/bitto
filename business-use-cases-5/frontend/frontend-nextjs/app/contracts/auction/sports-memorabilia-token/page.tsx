...
export default function SportsMemorabiliaTokenPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-memorabilia-token');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-memorabilia-token',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-memorabilia-token' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Sports Memorabilia Token</h1>
