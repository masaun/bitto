...
export default function ReorderProtectionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('reorder-protection');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'reorder-protection',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'reorder-protection' },
      });
