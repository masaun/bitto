...
export default function InstitutionalCollectionManager5Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('institutional-collection-manager-5');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'institutional-collection-manager-5',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'institutional-collection-manager-5' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Institutional Collection Manager 5</h1>
