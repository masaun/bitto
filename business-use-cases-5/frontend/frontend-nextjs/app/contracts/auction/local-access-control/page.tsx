...
export default function LocalAccessControlPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-access-control');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-access-control',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-access-control' },
      });
