...
export default function BespokeDiningTableMakerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bespoke-dining-table-maker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bespoke-dining-table-maker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bespoke-dining-table-maker' },
      });
