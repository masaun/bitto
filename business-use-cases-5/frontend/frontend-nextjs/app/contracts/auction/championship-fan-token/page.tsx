...
export default function ChampionshipFanTokenPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('championship-fan-token');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'championship-fan-token',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'championship-fan-token' },
      });
