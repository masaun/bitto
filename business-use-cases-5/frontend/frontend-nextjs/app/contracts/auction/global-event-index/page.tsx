...
export default function GlobalEventIndexPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('global-event-index');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'global-event-index',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'global-event-index' },
      });
