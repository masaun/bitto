...
export default function ForwardMarketEquitySwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('forward-market-equity-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'forward-market-equity-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'forward-market-equity-swap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Forward Market Equity Swap</h1>
