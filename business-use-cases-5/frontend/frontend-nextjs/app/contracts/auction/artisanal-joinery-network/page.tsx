...
export default function ArtisanalJoineryNetworkPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('artisanal-joinery-network');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'artisanal-joinery-network',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'artisanal-joinery-network' },
      });
