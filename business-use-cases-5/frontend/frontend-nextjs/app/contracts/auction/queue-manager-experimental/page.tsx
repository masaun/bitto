...
export default function QueueManagerExperimentalPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('queue-manager-experimental');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'queue-manager-experimental',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'queue-manager-experimental' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Queue Manager Experimental</h1>
