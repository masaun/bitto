...
export default function AiPrivacyShieldPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-privacy-shield');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-privacy-shield',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-privacy-shield' },
      });
