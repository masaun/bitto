...
export default function AgentLatencyMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-latency-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-latency-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-latency-monitor' },
      });
