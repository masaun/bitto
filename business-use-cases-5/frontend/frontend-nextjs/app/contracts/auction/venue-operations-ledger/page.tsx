...
export default function VenueOperationsLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('venue-operations-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'venue-operations-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'venue-operations-ledger' },
      });
