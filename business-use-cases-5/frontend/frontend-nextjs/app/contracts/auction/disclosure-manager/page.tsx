...
export default function DisclosureManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('disclosure-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'disclosure-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'disclosure-manager' },
      });
