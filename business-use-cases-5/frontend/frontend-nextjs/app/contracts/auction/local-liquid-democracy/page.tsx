...
export default function LocalLiquidDemocracyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-liquid-democracy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-liquid-democracy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-liquid-democracy' },
      });
