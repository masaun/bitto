...
export default function SummerCyclingPrizePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('summer-cycling-prize');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'summer-cycling-prize',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'summer-cycling-prize' },
      });
