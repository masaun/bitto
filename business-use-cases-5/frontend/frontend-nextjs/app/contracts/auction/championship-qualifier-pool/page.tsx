...
export default function ChampionshipQualifierPoolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('championship-qualifier-pool');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'championship-qualifier-pool',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'championship-qualifier-pool' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Championship Qualifier Pool</h1>
