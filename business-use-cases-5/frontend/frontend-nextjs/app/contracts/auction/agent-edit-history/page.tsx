...
export default function AgentEditHistoryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-edit-history');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-edit-history',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-edit-history' },
      });
