...
export default function EncryptedStateProofPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-state-proof');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-state-proof',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-state-proof' },
      });
