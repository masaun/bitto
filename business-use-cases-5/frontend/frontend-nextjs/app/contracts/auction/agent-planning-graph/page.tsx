...
export default function AgentPlanningGraphPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-planning-graph');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-planning-graph',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-planning-graph' },
      });
