...
export default function PrivacySunsetPolicyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-sunset-policy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-sunset-policy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-sunset-policy' },
      });
