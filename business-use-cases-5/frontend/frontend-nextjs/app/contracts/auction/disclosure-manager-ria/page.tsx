...
export default function DisclosureManagerRiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('disclosure-manager-ria');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'disclosure-manager-ria',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'disclosure-manager-ria' },
      });
