...
export default function ReviewCoordinatorRiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('review-coordinator-ria');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'review-coordinator-ria',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'review-coordinator-ria' },
      });
