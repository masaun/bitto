...
export default function AirqualityMonitorExchangePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('airquality-monitor-exchange');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'airquality-monitor-exchange',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'airquality-monitor-exchange' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Airquality Monitor Exchange</h1>
