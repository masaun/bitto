...
export default function MasterServiceContractPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('master-service-contract');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'master-service-contract',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'master-service-contract' },
      });
