...
export default function PrivateNodeRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('private-node-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'private-node-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'private-node-registry' },
      });
