...
export default function VenueCommercializationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('venue-commercialization');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'venue-commercialization',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'venue-commercialization' },
      });
