...
export default function QueueManagerV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('queue-manager-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'queue-manager-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'queue-manager-v3' },
      });
