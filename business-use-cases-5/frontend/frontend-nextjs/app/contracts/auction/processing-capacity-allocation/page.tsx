...
export default function ProcessingCapacityAllocationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('processing-capacity-allocation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'processing-capacity-allocation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'processing-capacity-allocation' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Processing Capacity Allocation</h1>
