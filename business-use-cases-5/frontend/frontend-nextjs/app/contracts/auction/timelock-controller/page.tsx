...
export default function TimelockControllerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('timelock-controller');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'timelock-controller',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'timelock-controller' },
      });
