...
export default function PrivacyEnforcerSoxPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-enforcer-sox');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-enforcer-sox',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-enforcer-sox' },
      });
