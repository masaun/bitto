...
export default function DataDrivenEventHandlerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('data-driven-event-handler');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'data-driven-event-handler',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'data-driven-event-handler' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Data Driven Event Handler</h1>
