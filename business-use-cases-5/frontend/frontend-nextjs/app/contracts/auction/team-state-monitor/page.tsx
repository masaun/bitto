...
export default function TeamStateMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-state-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-state-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-state-monitor' },
      });
