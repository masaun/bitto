...
export default function AccountabilityMappingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('accountability-mapping');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'accountability-mapping',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'accountability-mapping' },
      });
