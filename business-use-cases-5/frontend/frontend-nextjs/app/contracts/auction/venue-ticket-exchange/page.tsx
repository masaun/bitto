...
export default function VenueTicketExchangePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('venue-ticket-exchange');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'venue-ticket-exchange',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'venue-ticket-exchange' },
      });
