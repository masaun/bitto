...
export default function LightingInstallationLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('lighting-installation-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'lighting-installation-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'lighting-installation-ledger' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Lighting Installation Ledger</h1>
