...
export default function GlobalMatchSchedulingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('global-match-scheduling');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'global-match-scheduling',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'global-match-scheduling' },
      });
