...
export default function DivisionalDisputeResolverPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('divisional-dispute-resolver');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'divisional-dispute-resolver',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'divisional-dispute-resolver' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Divisional Dispute Resolver</h1>
