...
export default function ReviewCoordinatorSoxPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('review-coordinator-sox');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'review-coordinator-sox',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'review-coordinator-sox' },
      });
