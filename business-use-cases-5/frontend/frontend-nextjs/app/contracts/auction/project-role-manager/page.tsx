...
export default function ProjectRoleManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('project-role-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'project-role-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'project-role-manager' },
      });
