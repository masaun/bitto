...
export default function ArchivalManagementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('archival-management');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'archival-management',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'archival-management' },
      });
