...
export default function BaseballGroupStageRewardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('baseball-group-stage-reward');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'baseball-group-stage-reward',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'baseball-group-stage-reward' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Baseball Group Stage Reward</h1>
