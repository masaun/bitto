...
export default function AirportExpansionCoordinatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('airport-expansion-coordinator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'airport-expansion-coordinator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'airport-expansion-coordinator' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Airport Expansion Coordinator</h1>
