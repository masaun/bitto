...
export default function BespokeLoungeProductionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bespoke-lounge-production');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bespoke-lounge-production',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bespoke-lounge-production' },
      });
