...
export default function WeatherRiskMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('weather-risk-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'weather-risk-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'weather-risk-monitor' },
      });
