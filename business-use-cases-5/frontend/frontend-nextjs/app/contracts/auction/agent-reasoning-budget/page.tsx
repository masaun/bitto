...
export default function AgentReasoningBudgetPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-reasoning-budget');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-reasoning-budget',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-reasoning-budget' },
      });
