...
export default function AthleteBrandLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('athlete-brand-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'athlete-brand-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'athlete-brand-ledger' },
      });
