...
export default function DelayedPipelineOrchestratorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('delayed-pipeline-orchestrator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'delayed-pipeline-orchestrator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'delayed-pipeline-orchestrator' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Delayed Pipeline Orchestrator</h1>
