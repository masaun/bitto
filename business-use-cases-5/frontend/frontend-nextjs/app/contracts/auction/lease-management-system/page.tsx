...
export default function LeaseManagementSystemPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('lease-management-system');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'lease-management-system',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'lease-management-system' },
      });
