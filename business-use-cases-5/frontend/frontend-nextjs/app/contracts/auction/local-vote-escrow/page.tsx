...
export default function LocalVoteEscrowPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-vote-escrow');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-vote-escrow',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-vote-escrow' },
      });
