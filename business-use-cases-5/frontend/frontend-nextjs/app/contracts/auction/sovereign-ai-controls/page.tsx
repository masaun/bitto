...
export default function SovereignAiControlsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sovereign-ai-controls');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sovereign-ai-controls',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sovereign-ai-controls' },
      });
