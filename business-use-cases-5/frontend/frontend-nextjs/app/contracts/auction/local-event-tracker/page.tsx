...
export default function LocalEventTrackerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-event-tracker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-event-tracker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-event-tracker' },
      });
