...
export default function AiAgentLifecyclePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-agent-lifecycle');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-agent-lifecycle',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-agent-lifecycle' },
      });
