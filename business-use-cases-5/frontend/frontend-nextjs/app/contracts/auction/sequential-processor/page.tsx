...
export default function SequentialProcessorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sequential-processor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sequential-processor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sequential-processor' },
      });
