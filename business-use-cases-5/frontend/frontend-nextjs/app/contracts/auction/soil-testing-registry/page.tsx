...
export default function SoilTestingRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('soil-testing-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'soil-testing-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'soil-testing-registry' },
      });
