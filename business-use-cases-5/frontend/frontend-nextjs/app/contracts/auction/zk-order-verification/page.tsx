...
export default function ZkOrderVerificationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('zk-order-verification');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'zk-order-verification',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'zk-order-verification' },
      });
