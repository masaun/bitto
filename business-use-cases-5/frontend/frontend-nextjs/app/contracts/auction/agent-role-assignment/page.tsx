...
export default function AgentRoleAssignmentPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-role-assignment');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-role-assignment',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-role-assignment' },
      });
