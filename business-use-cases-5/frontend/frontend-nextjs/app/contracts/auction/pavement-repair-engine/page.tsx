...
export default function PavementRepairEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('pavement-repair-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'pavement-repair-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'pavement-repair-engine' },
      });
