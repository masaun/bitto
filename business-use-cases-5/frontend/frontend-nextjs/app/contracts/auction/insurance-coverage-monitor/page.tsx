...
export default function InsuranceCoverageMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('insurance-coverage-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'insurance-coverage-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'insurance-coverage-monitor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Insurance Coverage Monitor</h1>
