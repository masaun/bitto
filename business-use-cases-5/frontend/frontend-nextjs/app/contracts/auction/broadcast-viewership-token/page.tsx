...
export default function BroadcastViewershipTokenPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('broadcast-viewership-token');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'broadcast-viewership-token',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'broadcast-viewership-token' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Broadcast Viewership Token</h1>
