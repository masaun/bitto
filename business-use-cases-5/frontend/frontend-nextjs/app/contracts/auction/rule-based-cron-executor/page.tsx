...
export default function RuleBasedCronExecutorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rule-based-cron-executor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rule-based-cron-executor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rule-based-cron-executor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Rule Based Cron Executor</h1>
