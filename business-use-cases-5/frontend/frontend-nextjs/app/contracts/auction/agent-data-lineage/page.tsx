...
export default function AgentDataLineagePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-data-lineage');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-data-lineage',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-data-lineage' },
      });
