...
export default function LocalStrategyCommitteePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-strategy-committee');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-strategy-committee',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-strategy-committee' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Local Strategy Committee</h1>
