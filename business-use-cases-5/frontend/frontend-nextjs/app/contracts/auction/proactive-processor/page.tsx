...
export default function ProactiveProcessorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('proactive-processor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'proactive-processor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'proactive-processor' },
      });
