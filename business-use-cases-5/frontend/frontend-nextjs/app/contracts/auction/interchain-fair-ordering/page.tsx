...
export default function InterchainFairOrderingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('interchain-fair-ordering');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'interchain-fair-ordering',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'interchain-fair-ordering' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Interchain Fair Ordering</h1>
