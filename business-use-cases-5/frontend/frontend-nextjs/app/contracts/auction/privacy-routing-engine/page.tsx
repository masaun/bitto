...
export default function PrivacyRoutingEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-routing-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-routing-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-routing-engine' },
      });
