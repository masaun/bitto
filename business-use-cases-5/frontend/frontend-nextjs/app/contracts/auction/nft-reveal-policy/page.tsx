...
export default function NftRevealPolicyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('nft-reveal-policy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'nft-reveal-policy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'nft-reveal-policy' },
      });
