...
export default function SportsFanTokenPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-fan-token');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-fan-token',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-fan-token' },
      });
