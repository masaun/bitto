...
export default function DataLiquidityPoolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('data-liquidity-pool');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'data-liquidity-pool',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'data-liquidity-pool' },
      });
