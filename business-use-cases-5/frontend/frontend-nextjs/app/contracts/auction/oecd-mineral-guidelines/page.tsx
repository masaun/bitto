...
export default function OecdMineralGuidelinesPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('oecd-mineral-guidelines');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'oecd-mineral-guidelines',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'oecd-mineral-guidelines' },
      });
