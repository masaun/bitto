...
export default function AgentSunsetPolicyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-sunset-policy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-sunset-policy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-sunset-policy' },
      });
