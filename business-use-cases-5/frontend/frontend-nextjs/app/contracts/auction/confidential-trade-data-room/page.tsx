...
export default function ConfidentialTradeDataRoomPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('confidential-trade-data-room');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'confidential-trade-data-room',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'confidential-trade-data-room' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Confidential Trade Data Room</h1>
