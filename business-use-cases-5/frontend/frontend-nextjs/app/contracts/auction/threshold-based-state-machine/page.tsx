...
export default function ThresholdBasedStateMachinePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('threshold-based-state-machine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'threshold-based-state-machine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'threshold-based-state-machine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Threshold Based State Machine</h1>
