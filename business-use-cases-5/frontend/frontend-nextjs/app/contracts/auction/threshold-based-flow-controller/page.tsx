...
export default function ThresholdBasedFlowControllerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('threshold-based-flow-controller');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'threshold-based-flow-controller',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'threshold-based-flow-controller' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Threshold Based Flow Controller</h1>
