...
export default function StrategicAppealCourtPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('strategic-appeal-court');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'strategic-appeal-court',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'strategic-appeal-court' },
      });
