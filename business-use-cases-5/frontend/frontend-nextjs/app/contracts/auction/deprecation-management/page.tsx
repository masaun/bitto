...
export default function DeprecationManagementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('deprecation-management');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'deprecation-management',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'deprecation-management' },
      });
