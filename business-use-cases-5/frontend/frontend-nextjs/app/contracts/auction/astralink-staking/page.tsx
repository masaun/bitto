...
export default function AstralinkStakingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('astralink-staking');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'astralink-staking',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'astralink-staking' },
      });
