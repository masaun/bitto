...
export default function PromptInjectionDetectionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('prompt-injection-detection');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'prompt-injection-detection',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'prompt-injection-detection' },
      });
