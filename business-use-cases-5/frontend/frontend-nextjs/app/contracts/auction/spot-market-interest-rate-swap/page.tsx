...
export default function SpotMarketInterestRateSwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('spot-market-interest-rate-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'spot-market-interest-rate-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'spot-market-interest-rate-swap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Spot Market Interest Rate Swap</h1>
