...
export default function ContractVersionControlPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('contract-version-control');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'contract-version-control',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'contract-version-control' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Contract Version Control</h1>
