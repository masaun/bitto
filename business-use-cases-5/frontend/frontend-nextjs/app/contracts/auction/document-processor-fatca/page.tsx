...
export default function DocumentProcessorFatcaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('document-processor-fatca');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'document-processor-fatca',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'document-processor-fatca' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Document Processor Fatca</h1>
