...
export default function BespokeInteriorFabricatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bespoke-interior-fabricator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bespoke-interior-fabricator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bespoke-interior-fabricator' },
      });
