...
export default function RoadMaintenanceLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('road-maintenance-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'road-maintenance-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'road-maintenance-ledger' },
      });
