...
export default function CementSupplyRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('cement-supply-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'cement-supply-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'cement-supply-registry' },
      });
