...
export default function RiskMitigationRegisterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('risk-mitigation-register');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'risk-mitigation-register',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'risk-mitigation-register' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Risk Mitigation Register</h1>
