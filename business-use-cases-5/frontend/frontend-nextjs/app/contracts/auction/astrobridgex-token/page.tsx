...
export default function AstrobridgexTokenPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('astrobridgex-token');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'astrobridgex-token',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'astrobridgex-token' },
      });
