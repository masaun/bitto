...
export default function AmbulanceFleetLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ambulance-fleet-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ambulance-fleet-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ambulance-fleet-leaseback' },
      });
