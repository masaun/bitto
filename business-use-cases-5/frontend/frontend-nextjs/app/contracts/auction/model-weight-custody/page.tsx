...
export default function ModelWeightCustodyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('model-weight-custody');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'model-weight-custody',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'model-weight-custody' },
      });
