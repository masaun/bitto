...
export default function VenueSponsorRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('venue-sponsor-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'venue-sponsor-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'venue-sponsor-registry' },
      });
