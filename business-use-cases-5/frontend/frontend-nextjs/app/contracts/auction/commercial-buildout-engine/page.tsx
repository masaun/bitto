...
export default function CommercialBuildoutEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('commercial-buildout-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'commercial-buildout-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'commercial-buildout-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Commercial Buildout Engine</h1>
