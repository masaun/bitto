...
export default function TacticalVoteEscrowPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tactical-vote-escrow');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tactical-vote-escrow',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tactical-vote-escrow' },
      });
