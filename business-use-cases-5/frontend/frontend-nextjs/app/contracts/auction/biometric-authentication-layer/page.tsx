...
export default function BiometricAuthenticationLayerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('biometric-authentication-layer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'biometric-authentication-layer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'biometric-authentication-layer' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Biometric Authentication Layer</h1>
