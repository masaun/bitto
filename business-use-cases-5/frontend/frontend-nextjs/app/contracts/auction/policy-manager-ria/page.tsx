...
export default function PolicyManagerRiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('policy-manager-ria');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'policy-manager-ria',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'policy-manager-ria' },
      });
