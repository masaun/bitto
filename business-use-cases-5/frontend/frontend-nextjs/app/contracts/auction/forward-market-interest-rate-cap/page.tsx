...
export default function ForwardMarketInterestRateCapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('forward-market-interest-rate-cap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'forward-market-interest-rate-cap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'forward-market-interest-rate-cap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Forward Market Interest Rate Cap</h1>
