...
export default function SpotMarketFxOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('spot-market-fx-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'spot-market-fx-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'spot-market-fx-option' },
      });
