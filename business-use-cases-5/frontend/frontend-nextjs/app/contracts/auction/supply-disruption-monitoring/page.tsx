...
export default function SupplyDisruptionMonitoringPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('supply-disruption-monitoring');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'supply-disruption-monitoring',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'supply-disruption-monitoring' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Supply Disruption Monitoring</h1>
