...
export default function ProjectChangeManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('project-change-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'project-change-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'project-change-manager' },
      });
