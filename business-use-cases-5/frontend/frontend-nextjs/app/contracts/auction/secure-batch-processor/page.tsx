...
export default function SecureBatchProcessorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('secure-batch-processor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'secure-batch-processor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'secure-batch-processor' },
      });
