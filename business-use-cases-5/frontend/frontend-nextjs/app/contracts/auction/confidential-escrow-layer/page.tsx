...
export default function ConfidentialEscrowLayerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('confidential-escrow-layer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'confidential-escrow-layer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'confidential-escrow-layer' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Confidential Escrow Layer</h1>
