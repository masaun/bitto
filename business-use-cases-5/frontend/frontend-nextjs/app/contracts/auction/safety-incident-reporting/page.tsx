...
export default function SafetyIncidentReportingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('safety-incident-reporting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'safety-incident-reporting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'safety-incident-reporting' },
      });
