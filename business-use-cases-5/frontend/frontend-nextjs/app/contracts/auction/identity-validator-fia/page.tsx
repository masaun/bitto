...
export default function IdentityValidatorFiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('identity-validator-fia');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'identity-validator-fia',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'identity-validator-fia' },
      });
