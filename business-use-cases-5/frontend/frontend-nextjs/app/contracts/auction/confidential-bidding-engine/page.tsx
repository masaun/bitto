...
export default function ConfidentialBiddingEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('confidential-bidding-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'confidential-bidding-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'confidential-bidding-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Confidential Bidding Engine</h1>
