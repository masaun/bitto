...
export default function TeamVoteEscrowPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-vote-escrow');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-vote-escrow',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-vote-escrow' },
      });
