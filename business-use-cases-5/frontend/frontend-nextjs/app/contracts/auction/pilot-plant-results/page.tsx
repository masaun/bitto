...
export default function PilotPlantResultsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('pilot-plant-results');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'pilot-plant-results',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'pilot-plant-results' },
      });
