...
export default function StrategyCommitteePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('strategy-committee');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'strategy-committee',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'strategy-committee' },
      });
