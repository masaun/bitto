...
export default function ZkExecutionProofPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('zk-execution-proof');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'zk-execution-proof',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'zk-execution-proof' },
      });
