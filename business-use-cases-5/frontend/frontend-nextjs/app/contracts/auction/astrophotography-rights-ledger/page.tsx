...
export default function AstrophotographyRightsLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('astrophotography-rights-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'astrophotography-rights-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'astrophotography-rights-ledger' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Astrophotography Rights Ledger</h1>
