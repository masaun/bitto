...
export default function DataProtectorSoxPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('data-protector-sox');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'data-protector-sox',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'data-protector-sox' },
      });
