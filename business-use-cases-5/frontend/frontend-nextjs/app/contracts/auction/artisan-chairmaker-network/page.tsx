...
export default function ArtisanChairmakerNetworkPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('artisan-chairmaker-network');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'artisan-chairmaker-network',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'artisan-chairmaker-network' },
      });
