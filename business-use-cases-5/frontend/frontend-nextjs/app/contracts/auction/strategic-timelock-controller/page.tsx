...
export default function StrategicTimelockControllerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('strategic-timelock-controller');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'strategic-timelock-controller',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'strategic-timelock-controller' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Strategic Timelock Controller</h1>
