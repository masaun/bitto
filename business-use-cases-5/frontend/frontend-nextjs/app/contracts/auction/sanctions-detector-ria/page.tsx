...
export default function SanctionsDetectorRiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sanctions-detector-ria');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sanctions-detector-ria',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sanctions-detector-ria' },
      });
