...
export default function RegulatoryApprovalMaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('regulatory-approval-ma');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'regulatory-approval-ma',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'regulatory-approval-ma' },
      });
