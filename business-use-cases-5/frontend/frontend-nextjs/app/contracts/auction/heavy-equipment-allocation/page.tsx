...
export default function HeavyEquipmentAllocationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('heavy-equipment-allocation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'heavy-equipment-allocation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'heavy-equipment-allocation' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Heavy Equipment Allocation</h1>
