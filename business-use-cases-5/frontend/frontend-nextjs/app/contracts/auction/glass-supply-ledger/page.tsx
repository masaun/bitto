...
export default function GlassSupplyLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('glass-supply-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'glass-supply-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'glass-supply-ledger' },
      });
