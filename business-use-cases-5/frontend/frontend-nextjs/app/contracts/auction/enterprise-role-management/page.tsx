...
export default function EnterpriseRoleManagementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('enterprise-role-management');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'enterprise-role-management',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'enterprise-role-management' },
      });
