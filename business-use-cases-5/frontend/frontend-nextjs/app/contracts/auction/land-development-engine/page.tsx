...
export default function LandDevelopmentEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('land-development-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'land-development-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'land-development-engine' },
      });
