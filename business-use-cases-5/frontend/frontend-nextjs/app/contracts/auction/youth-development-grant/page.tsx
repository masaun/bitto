...
export default function YouthDevelopmentGrantPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('youth-development-grant');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'youth-development-grant',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'youth-development-grant' },
      });
