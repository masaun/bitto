...
export default function ViolationDetectorSoxPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('violation-detector-sox');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'violation-detector-sox',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'violation-detector-sox' },
      });
