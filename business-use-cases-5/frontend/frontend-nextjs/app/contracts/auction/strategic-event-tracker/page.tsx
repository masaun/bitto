...
export default function StrategicEventTrackerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('strategic-event-tracker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'strategic-event-tracker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'strategic-event-tracker' },
      });
