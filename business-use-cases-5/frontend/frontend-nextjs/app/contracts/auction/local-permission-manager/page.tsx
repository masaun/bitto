...
export default function LocalPermissionManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-permission-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-permission-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-permission-manager' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Local Permission Manager</h1>
