...
export default function TeamKitLicensingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-kit-licensing');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-kit-licensing',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-kit-licensing' },
      });
