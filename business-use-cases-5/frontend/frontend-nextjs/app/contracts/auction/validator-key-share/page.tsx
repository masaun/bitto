...
export default function ValidatorKeySharePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('validator-key-share');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'validator-key-share',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'validator-key-share' },
      });
