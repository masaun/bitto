...
export default function TorchRelaySponsorshipLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('torch-relay-sponsorship-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'torch-relay-sponsorship-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'torch-relay-sponsorship-ledger' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Torch Relay Sponsorship Ledger</h1>
