...
export default function DataProtectorUcitsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('data-protector-ucits');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'data-protector-ucits',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'data-protector-ucits' },
      });
