...
export default function OfficeFitoutLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('office-fitout-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'office-fitout-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'office-fitout-ledger' },
      });
