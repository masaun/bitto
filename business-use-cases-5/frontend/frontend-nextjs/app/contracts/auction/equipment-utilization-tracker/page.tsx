...
export default function EquipmentUtilizationTrackerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('equipment-utilization-tracker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'equipment-utilization-tracker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'equipment-utilization-tracker' },
      });
