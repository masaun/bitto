...
export default function AgentAttackSurfaceMapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-attack-surface-map');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-attack-surface-map',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-attack-surface-map' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Agent Attack Surface Map</h1>
