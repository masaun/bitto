...
export default function AuctionDecoderPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-decoder');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-decoder',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-decoder' },
      });
