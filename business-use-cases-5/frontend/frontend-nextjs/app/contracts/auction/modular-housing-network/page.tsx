...
export default function ModularHousingNetworkPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('modular-housing-network');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'modular-housing-network',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'modular-housing-network' },
      });
