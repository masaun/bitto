...
export default function MetroStationBuildoutPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('metro-station-buildout');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'metro-station-buildout',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'metro-station-buildout' },
      });
