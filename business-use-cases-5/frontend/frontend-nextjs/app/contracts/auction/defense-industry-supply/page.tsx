...
export default function DefenseIndustrySupplyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('defense-industry-supply');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'defense-industry-supply',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'defense-industry-supply' },
      });
