...
export default function AgentDataVersioningPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-data-versioning');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-data-versioning',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-data-versioning' },
      });
