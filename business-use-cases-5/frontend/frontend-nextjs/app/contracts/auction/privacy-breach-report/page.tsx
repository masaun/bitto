...
export default function PrivacyBreachReportPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-breach-report');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-breach-report',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-breach-report' },
      });
