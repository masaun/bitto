...
export default function SlashingEnforcementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('slashing-enforcement');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'slashing-enforcement',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'slashing-enforcement' },
      });
