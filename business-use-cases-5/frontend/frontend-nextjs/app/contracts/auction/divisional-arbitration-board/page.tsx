...
export default function DivisionalArbitrationBoardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('divisional-arbitration-board');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'divisional-arbitration-board',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'divisional-arbitration-board' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Divisional Arbitration Board</h1>
