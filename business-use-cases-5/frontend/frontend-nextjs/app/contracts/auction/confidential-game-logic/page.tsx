...
export default function ConfidentialGameLogicPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('confidential-game-logic');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'confidential-game-logic',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'confidential-game-logic' },
      });
