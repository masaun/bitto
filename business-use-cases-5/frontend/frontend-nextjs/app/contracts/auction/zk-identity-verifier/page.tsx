...
export default function ZkIdentityVerifierPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('zk-identity-verifier');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'zk-identity-verifier',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'zk-identity-verifier' },
      });
