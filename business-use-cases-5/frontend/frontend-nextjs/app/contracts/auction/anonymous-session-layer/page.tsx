...
export default function AnonymousSessionLayerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('anonymous-session-layer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'anonymous-session-layer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'anonymous-session-layer' },
      });
