...
export default function AgentBiasDetectionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-bias-detection');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-bias-detection',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-bias-detection' },
      });
