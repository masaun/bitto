...
export default function AiEnterpriseRoutingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-enterprise-routing');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-enterprise-routing',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-enterprise-routing' },
      });
