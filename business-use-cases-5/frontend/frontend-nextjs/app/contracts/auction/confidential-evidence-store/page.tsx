...
export default function ConfidentialEvidenceStorePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('confidential-evidence-store');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'confidential-evidence-store',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'confidential-evidence-store' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Confidential Evidence Store</h1>
