...
export default function ConditionalParallelProcessorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('conditional-parallel-processor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'conditional-parallel-processor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'conditional-parallel-processor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Conditional Parallel Processor</h1>
