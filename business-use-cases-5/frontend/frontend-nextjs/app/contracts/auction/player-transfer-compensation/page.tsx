...
export default function PlayerTransferCompensationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('player-transfer-compensation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'player-transfer-compensation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'player-transfer-compensation' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Player Transfer Compensation</h1>
