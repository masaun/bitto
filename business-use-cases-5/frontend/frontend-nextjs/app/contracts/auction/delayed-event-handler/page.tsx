...
export default function DelayedEventHandlerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('delayed-event-handler');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'delayed-event-handler',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'delayed-event-handler' },
      });
