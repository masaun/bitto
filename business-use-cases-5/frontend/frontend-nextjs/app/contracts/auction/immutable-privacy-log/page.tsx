...
export default function ImmutablePrivacyLogPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('immutable-privacy-log');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'immutable-privacy-log',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'immutable-privacy-log' },
      });
