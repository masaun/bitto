...
export default function RiskAssessorFatcaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('risk-assessor-fatca');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'risk-assessor-fatca',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'risk-assessor-fatca' },
      });
