...
export default function SecureOracleBridgePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('secure-oracle-bridge');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'secure-oracle-bridge',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'secure-oracle-bridge' },
      });
