...
export default function BroadcastSignalDistributionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('broadcast-signal-distribution');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'broadcast-signal-distribution',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'broadcast-signal-distribution' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Broadcast Signal Distribution</h1>
