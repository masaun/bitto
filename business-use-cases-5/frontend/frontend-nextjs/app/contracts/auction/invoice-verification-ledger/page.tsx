...
export default function InvoiceVerificationLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('invoice-verification-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'invoice-verification-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'invoice-verification-ledger' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Invoice Verification Ledger</h1>
