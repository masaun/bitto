...
export default function EarnoutTrackingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('earnout-tracking');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'earnout-tracking',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'earnout-tracking' },
      });
