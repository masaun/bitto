...
export default function PrivateAiTrainingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('private-ai-training');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'private-ai-training',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'private-ai-training' },
      });
