...
export default function AiPredictiveDiagnosticsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-predictive-diagnostics');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-predictive-diagnostics',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-predictive-diagnostics' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Ai Predictive Diagnostics</h1>
