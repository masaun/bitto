...
export default function RebarDeliveryMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rebar-delivery-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rebar-delivery-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rebar-delivery-monitor' },
      });
