...
export default function ArtisanUpholsteryMarketPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('artisan-upholstery-market');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'artisan-upholstery-market',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'artisan-upholstery-market' },
      });
