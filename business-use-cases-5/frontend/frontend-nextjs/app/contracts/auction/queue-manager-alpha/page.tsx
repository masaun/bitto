...
export default function QueueManagerAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('queue-manager-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'queue-manager-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'queue-manager-alpha' },
      });
