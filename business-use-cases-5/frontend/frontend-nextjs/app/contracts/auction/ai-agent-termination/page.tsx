...
export default function AiAgentTerminationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-agent-termination');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-agent-termination',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-agent-termination' },
      });
