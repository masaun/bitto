...
export default function QueueManagerStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('queue-manager-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'queue-manager-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'queue-manager-stable' },
      });
