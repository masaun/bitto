...
export default function MultilateralNettingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('multilateral-netting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'multilateral-netting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'multilateral-netting' },
      });
