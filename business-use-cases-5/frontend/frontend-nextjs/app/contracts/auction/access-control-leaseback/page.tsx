...
export default function AccessControlLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('access-control-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'access-control-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'access-control-leaseback' },
      });
