...
export default function ValueAlignmentPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('value-alignment');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'value-alignment',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'value-alignment' },
      });
