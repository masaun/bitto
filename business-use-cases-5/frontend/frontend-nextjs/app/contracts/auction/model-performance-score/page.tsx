...
export default function ModelPerformanceScorePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('model-performance-score');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'model-performance-score',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'model-performance-score' },
      });
