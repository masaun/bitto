...
export default function AgentDataConsentPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-data-consent');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-data-consent',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-data-consent' },
      });
