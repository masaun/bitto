...
export default function FanEngagementBountyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('fan-engagement-bounty');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'fan-engagement-bounty',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'fan-engagement-bounty' },
      });
