...
export default function BiomarkerDiscoveryPoolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('biomarker-discovery-pool');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'biomarker-discovery-pool',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'biomarker-discovery-pool' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Biomarker Discovery Pool</h1>
