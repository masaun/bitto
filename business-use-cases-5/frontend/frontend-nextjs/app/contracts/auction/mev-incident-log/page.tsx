...
export default function MevIncidentLogPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('mev-incident-log');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'mev-incident-log',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'mev-incident-log' },
      });
