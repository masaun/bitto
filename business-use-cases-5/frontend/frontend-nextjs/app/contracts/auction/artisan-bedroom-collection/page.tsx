...
export default function ArtisanBedroomCollectionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('artisan-bedroom-collection');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'artisan-bedroom-collection',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'artisan-bedroom-collection' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Artisan Bedroom Collection</h1>
