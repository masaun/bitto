...
export default function EscalationManagerSoxPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('escalation-manager-sox');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'escalation-manager-sox',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'escalation-manager-sox' },
      });
