...
export default function EncryptedValidatorBridgePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-validator-bridge');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-validator-bridge',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-validator-bridge' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Encrypted Validator Bridge</h1>
