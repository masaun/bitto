...
export default function PublicHousingProjectPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('public-housing-project');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'public-housing-project',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'public-housing-project' },
      });
