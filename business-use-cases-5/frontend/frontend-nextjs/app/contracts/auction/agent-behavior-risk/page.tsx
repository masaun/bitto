...
export default function AgentBehaviorRiskPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-behavior-risk');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-behavior-risk',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-behavior-risk' },
      });
