...
export default function AthleteTransferMarketPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('athlete-transfer-market');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'athlete-transfer-market',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'athlete-transfer-market' },
      });
