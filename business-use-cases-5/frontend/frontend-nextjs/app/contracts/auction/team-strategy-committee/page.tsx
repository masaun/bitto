...
export default function TeamStrategyCommitteePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-strategy-committee');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-strategy-committee',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-strategy-committee' },
      });
