...
export default function AiAgentVersioningPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-agent-versioning');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-agent-versioning',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-agent-versioning' },
      });
