...
export default function RegionalAccessControlPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('regional-access-control');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'regional-access-control',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'regional-access-control' },
      });
