...
export default function QueueManagerBetaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('queue-manager-beta');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'queue-manager-beta',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'queue-manager-beta' },
      });
