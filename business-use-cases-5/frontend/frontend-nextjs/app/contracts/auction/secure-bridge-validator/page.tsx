...
export default function SecureBridgeValidatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('secure-bridge-validator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'secure-bridge-validator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'secure-bridge-validator' },
      });
