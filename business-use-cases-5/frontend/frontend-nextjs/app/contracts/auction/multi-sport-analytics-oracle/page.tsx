...
export default function MultiSportAnalyticsOraclePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('multi-sport-analytics-oracle');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'multi-sport-analytics-oracle',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'multi-sport-analytics-oracle' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Multi Sport Analytics Oracle</h1>
