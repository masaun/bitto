...
export default function CrossChainSecurityLogPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('cross-chain-security-log');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'cross-chain-security-log',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'cross-chain-security-log' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Cross Chain Security Log</h1>
