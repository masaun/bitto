...
export default function BidCommitmentHashPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bid-commitment-hash');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bid-commitment-hash',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bid-commitment-hash' },
      });
