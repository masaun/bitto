...
export default function AiDatacenterLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-datacenter-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-datacenter-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-datacenter-leaseback' },
      });
