...
export default function AiRiskMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-risk-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-risk-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-risk-monitor' },
      });
