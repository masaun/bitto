...
export default function LocalExecutiveBoardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-executive-board');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-executive-board',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-executive-board' },
      });
