...
export default function SecureP2pChannelPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('secure-p2p-channel');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'secure-p2p-channel',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'secure-p2p-channel' },
      });
