...
export default function ZkEsgAttestationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('zk-esg-attestation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'zk-esg-attestation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'zk-esg-attestation' },
      });
