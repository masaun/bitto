...
export default function DeterministicOrderingPolicyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('deterministic-ordering-policy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'deterministic-ordering-policy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'deterministic-ordering-policy' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Deterministic Ordering Policy</h1>
