...
export default function SwapMarketEquitySwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('swap-market-equity-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'swap-market-equity-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'swap-market-equity-swap' },
      });
