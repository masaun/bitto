...
export default function WorkforceProductivityEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('workforce-productivity-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'workforce-productivity-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'workforce-productivity-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Workforce Productivity Engine</h1>
