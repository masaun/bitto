...
export default function AgentFairnessScorePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-fairness-score');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-fairness-score',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-fairness-score' },
      });
