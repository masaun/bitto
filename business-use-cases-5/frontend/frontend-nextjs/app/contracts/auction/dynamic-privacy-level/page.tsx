...
export default function DynamicPrivacyLevelPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('dynamic-privacy-level');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'dynamic-privacy-level',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'dynamic-privacy-level' },
      });
