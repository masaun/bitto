...
export default function AgentPerformanceReviewPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-performance-review');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-performance-review',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-performance-review' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Agent Performance Review</h1>
