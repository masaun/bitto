...
export default function ContractVersioningPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('contract-versioning');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'contract-versioning',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'contract-versioning' },
      });
