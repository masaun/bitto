...
export default function NoiseControlMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('noise-control-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'noise-control-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'noise-control-monitor' },
      });
