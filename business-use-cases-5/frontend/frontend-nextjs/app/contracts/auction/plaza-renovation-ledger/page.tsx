...
export default function PlazaRenovationLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('plaza-renovation-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'plaza-renovation-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'plaza-renovation-ledger' },
      });
