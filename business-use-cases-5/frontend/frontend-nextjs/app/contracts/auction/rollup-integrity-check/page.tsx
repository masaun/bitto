...
export default function RollupIntegrityCheckPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rollup-integrity-check');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rollup-integrity-check',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rollup-integrity-check' },
      });
