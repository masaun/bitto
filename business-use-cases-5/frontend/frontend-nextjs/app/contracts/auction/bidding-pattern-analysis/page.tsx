...
export default function BiddingPatternAnalysisPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bidding-pattern-analysis');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bidding-pattern-analysis',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bidding-pattern-analysis' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Bidding Pattern Analysis</h1>
