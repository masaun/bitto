...
export default function ScheduledCronExecutorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('scheduled-cron-executor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'scheduled-cron-executor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'scheduled-cron-executor' },
      });
