...
export default function ProjectBudgetControlPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('project-budget-control');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'project-budget-control',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'project-budget-control' },
      });
