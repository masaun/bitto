...
export default function EventSecurityProcurementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-security-procurement');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-security-procurement',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-security-procurement' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Event Security Procurement</h1>
