...
export default function TacticalRoleManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tactical-role-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tactical-role-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tactical-role-manager' },
      });
