...
export default function StadiumAccessControlPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('stadium-access-control');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'stadium-access-control',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'stadium-access-control' },
      });
