...
export default function LumberDistributionNetworkPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('lumber-distribution-network');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'lumber-distribution-network',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'lumber-distribution-network' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Lumber Distribution Network</h1>
