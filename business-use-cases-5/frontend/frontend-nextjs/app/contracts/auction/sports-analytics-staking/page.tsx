...
export default function SportsAnalyticsStakingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-analytics-staking');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-analytics-staking',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-analytics-staking' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Sports Analytics Staking</h1>
