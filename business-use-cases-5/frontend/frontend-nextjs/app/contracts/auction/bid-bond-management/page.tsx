...
export default function BidBondManagementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bid-bond-management');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bid-bond-management',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bid-bond-management' },
      });
