...
export default function AuctionEncoderPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-encoder');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-encoder',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-encoder' },
      });
