...
export default function OccupancyPermitRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('occupancy-permit-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'occupancy-permit-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'occupancy-permit-registry' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Occupancy Permit Registry</h1>
