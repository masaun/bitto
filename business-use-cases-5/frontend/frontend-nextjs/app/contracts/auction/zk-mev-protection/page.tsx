...
export default function ZkMevProtectionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('zk-mev-protection');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'zk-mev-protection',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'zk-mev-protection' },
      });
