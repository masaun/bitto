...
export default function PredictiveMevDefensePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('predictive-mev-defense');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'predictive-mev-defense',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'predictive-mev-defense' },
      });
