...
export default function AcceleratorHubLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('accelerator-hub-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'accelerator-hub-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'accelerator-hub-leaseback' },
      });
