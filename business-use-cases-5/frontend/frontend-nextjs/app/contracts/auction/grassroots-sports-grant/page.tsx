...
export default function GrassrootsSportsGrantPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('grassroots-sports-grant');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'grassroots-sports-grant',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'grassroots-sports-grant' },
      });
