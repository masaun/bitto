...
export default function AutomatedRenewalEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automated-renewal-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automated-renewal-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automated-renewal-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Automated Renewal Engine</h1>
