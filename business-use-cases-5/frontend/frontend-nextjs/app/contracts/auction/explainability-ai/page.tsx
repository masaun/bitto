...
export default function ExplainabilityAiPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('explainability-ai');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'explainability-ai',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'explainability-ai' },
      });
