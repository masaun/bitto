...
export default function SmartSensorDeploymentPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('smart-sensor-deployment');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'smart-sensor-deployment',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'smart-sensor-deployment' },
      });
