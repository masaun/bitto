...
export default function SystemHealthIndexPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('system-health-index');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'system-health-index',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'system-health-index' },
      });
