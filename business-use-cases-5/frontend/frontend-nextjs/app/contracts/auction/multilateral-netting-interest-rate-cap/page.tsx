...
export default function MultilateralNettingInterestRateCapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('multilateral-netting-interest-rate-cap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'multilateral-netting-interest-rate-cap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'multilateral-netting-interest-rate-cap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Multilateral Netting Interest Rate Cap</h1>
