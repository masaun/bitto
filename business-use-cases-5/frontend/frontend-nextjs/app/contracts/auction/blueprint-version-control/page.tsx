...
export default function BlueprintVersionControlPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('blueprint-version-control');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'blueprint-version-control',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'blueprint-version-control' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Blueprint Version Control</h1>
