...
export default function MixeduseDevelopmentEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('mixeduse-development-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'mixeduse-development-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'mixeduse-development-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Mixeduse Development Engine</h1>
