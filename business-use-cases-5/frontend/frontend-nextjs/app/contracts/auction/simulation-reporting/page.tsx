...
export default function SimulationReportingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('simulation-reporting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'simulation-reporting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'simulation-reporting' },
      });
