...
export default function ContingencyBudgetLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('contingency-budget-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'contingency-budget-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'contingency-budget-ledger' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Contingency Budget Ledger</h1>
