...
export default function GuaranteeMonitoringEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('guarantee-monitoring-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'guarantee-monitoring-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'guarantee-monitoring-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Guarantee Monitoring Engine</h1>
