...
export default function AgentTaskRoutingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-task-routing');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-task-routing',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-task-routing' },
      });
