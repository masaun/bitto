...
export default function AuctionOraclePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-oracle');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-oracle',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-oracle' },
      });
