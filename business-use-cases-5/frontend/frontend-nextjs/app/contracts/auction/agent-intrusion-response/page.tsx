...
export default function AgentIntrusionResponsePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-intrusion-response');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-intrusion-response',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-intrusion-response' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Agent Intrusion Response</h1>
