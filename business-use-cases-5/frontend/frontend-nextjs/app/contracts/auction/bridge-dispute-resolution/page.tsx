...
export default function BridgeDisputeResolutionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bridge-dispute-resolution');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bridge-dispute-resolution',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bridge-dispute-resolution' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Bridge Dispute Resolution</h1>
