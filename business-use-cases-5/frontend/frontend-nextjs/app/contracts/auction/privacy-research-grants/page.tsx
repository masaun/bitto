...
export default function PrivacyResearchGrantsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-research-grants');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-research-grants',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-research-grants' },
      });
