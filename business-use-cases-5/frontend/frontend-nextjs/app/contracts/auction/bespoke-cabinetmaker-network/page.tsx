...
export default function BespokeCabinetmakerNetworkPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bespoke-cabinetmaker-network');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bespoke-cabinetmaker-network',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bespoke-cabinetmaker-network' },
      });
