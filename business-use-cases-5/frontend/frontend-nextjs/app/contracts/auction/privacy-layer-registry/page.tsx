...
export default function PrivacyLayerRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-layer-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-layer-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-layer-registry' },
      });
