...
export default function GlazingSystemManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('glazing-system-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'glazing-system-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'glazing-system-manager' },
      });
