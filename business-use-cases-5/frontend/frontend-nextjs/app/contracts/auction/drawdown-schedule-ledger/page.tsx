...
export default function DrawdownScheduleLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('drawdown-schedule-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'drawdown-schedule-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'drawdown-schedule-ledger' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Drawdown Schedule Ledger</h1>
