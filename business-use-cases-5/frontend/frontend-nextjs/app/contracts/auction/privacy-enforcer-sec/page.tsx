...
export default function PrivacyEnforcerSecPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-enforcer-sec');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-enforcer-sec',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-enforcer-sec' },
      });
