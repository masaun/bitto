...
export default function ArbitratorSelectionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('arbitrator-selection');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'arbitrator-selection',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'arbitrator-selection' },
      });
