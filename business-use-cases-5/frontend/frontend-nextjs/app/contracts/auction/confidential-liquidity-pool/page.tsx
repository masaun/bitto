...
export default function ConfidentialLiquidityPoolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('confidential-liquidity-pool');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'confidential-liquidity-pool',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'confidential-liquidity-pool' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Confidential Liquidity Pool</h1>
