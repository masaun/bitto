...
export default function PrivacySlaMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-sla-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-sla-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-sla-monitor' },
      });
