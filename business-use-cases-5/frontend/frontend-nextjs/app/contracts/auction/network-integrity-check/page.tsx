...
export default function NetworkIntegrityCheckPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('network-integrity-check');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'network-integrity-check',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'network-integrity-check' },
      });
