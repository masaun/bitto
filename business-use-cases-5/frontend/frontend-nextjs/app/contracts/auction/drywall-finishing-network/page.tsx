...
export default function DrywallFinishingNetworkPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('drywall-finishing-network');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'drywall-finishing-network',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'drywall-finishing-network' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Drywall Finishing Network</h1>
