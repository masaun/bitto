...
export default function OptionMarketForwardRateAgreementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('option-market-forward-rate-agreement');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'option-market-forward-rate-agreement',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'option-market-forward-rate-agreement' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Option Market Forward Rate Agreement</h1>
