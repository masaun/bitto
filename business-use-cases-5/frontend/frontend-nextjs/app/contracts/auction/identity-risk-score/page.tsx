...
export default function IdentityRiskScorePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('identity-risk-score');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'identity-risk-score',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'identity-risk-score' },
      });
