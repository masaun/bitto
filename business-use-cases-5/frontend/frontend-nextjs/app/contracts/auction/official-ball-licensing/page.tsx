...
export default function OfficialBallLicensingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('official-ball-licensing');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'official-ball-licensing',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'official-ball-licensing' },
      });
