...
export default function ArtisanMarketQuestPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('artisan-market-quest');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'artisan-market-quest',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'artisan-market-quest' },
      });
