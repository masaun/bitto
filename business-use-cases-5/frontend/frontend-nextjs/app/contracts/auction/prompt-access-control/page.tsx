...
export default function PromptAccessControlPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('prompt-access-control');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'prompt-access-control',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'prompt-access-control' },
      });
