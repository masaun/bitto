...
export default function VenueEventStakingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('venue-event-staking');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'venue-event-staking',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'venue-event-staking' },
      });
