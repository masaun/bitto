...
export default function BiotechLabLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('biotech-lab-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'biotech-lab-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'biotech-lab-leaseback' },
      });
