...
export default function SecureRelayManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('secure-relay-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'secure-relay-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'secure-relay-manager' },
      });
