...
export default function CrossBorderDataTransferPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('cross-border-data-transfer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'cross-border-data-transfer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'cross-border-data-transfer' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Cross Border Data Transfer</h1>
