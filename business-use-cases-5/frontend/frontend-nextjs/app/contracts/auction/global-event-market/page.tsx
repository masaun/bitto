...
export default function GlobalEventMarketPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('global-event-market');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'global-event-market',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'global-event-market' },
      });
