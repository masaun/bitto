...
export default function PremiumSeatAuctionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('premium-seat-auction');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'premium-seat-auction',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'premium-seat-auction' },
      });
