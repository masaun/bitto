...
export default function PrefabricationCoordinationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('prefabrication-coordination');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'prefabrication-coordination',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'prefabrication-coordination' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Prefabrication Coordination</h1>
