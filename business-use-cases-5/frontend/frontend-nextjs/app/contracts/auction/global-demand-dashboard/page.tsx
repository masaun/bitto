...
export default function GlobalDemandDashboardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('global-demand-dashboard');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'global-demand-dashboard',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'global-demand-dashboard' },
      });
