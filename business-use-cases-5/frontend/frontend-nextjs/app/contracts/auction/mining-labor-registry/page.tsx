...
export default function MiningLaborRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('mining-labor-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'mining-labor-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'mining-labor-registry' },
      });
