...
export default function VenueNamingAuctionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('venue-naming-auction');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'venue-naming-auction',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'venue-naming-auction' },
      });
