...
export default function ProjectEventTrackerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('project-event-tracker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'project-event-tracker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'project-event-tracker' },
      });
