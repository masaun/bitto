...
export default function AgentDataIngestionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-data-ingestion');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-data-ingestion',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-data-ingestion' },
      });
