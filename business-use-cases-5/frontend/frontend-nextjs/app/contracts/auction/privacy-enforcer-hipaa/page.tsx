...
export default function PrivacyEnforcerHipaaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-enforcer-hipaa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-enforcer-hipaa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-enforcer-hipaa' },
      });
