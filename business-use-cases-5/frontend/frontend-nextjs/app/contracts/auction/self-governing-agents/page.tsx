...
export default function SelfGoverningAgentsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('self-governing-agents');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'self-governing-agents',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'self-governing-agents' },
      });
