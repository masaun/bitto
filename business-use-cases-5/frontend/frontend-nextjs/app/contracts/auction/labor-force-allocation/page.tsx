...
export default function LaborForceAllocationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('labor-force-allocation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'labor-force-allocation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'labor-force-allocation' },
      });
