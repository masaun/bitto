...
export default function ConsentTrackerRiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('consent-tracker-ria');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'consent-tracker-ria',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'consent-tracker-ria' },
      });
