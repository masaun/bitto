...
export default function PortConstructionNetworkPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('port-construction-network');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'port-construction-network',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'port-construction-network' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Port Construction Network</h1>
