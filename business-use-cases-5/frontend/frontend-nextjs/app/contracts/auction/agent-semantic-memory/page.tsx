...
export default function AgentSemanticMemoryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-semantic-memory');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-semantic-memory',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-semantic-memory' },
      });
