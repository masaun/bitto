...
export default function RemediationHandlerCftcPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('remediation-handler-cftc');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'remediation-handler-cftc',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'remediation-handler-cftc' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Remediation Handler Cftc</h1>
