...
export default function AgentHealthScorePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-health-score');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-health-score',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-health-score' },
      });
