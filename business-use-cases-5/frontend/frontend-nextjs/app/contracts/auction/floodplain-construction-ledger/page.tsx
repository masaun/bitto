...
export default function FloodplainConstructionLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('floodplain-construction-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'floodplain-construction-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'floodplain-construction-ledger' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Floodplain Construction Ledger</h1>
