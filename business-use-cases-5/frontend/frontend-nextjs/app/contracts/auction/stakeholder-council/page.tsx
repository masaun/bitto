...
export default function StakeholderCouncilPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('stakeholder-council');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'stakeholder-council',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'stakeholder-council' },
      });
