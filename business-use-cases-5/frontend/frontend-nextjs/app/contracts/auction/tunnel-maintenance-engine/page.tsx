...
export default function TunnelMaintenanceEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tunnel-maintenance-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tunnel-maintenance-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tunnel-maintenance-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Tunnel Maintenance Engine</h1>
