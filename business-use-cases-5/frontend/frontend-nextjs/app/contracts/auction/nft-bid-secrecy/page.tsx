...
export default function NftBidSecrecyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('nft-bid-secrecy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'nft-bid-secrecy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'nft-bid-secrecy' },
      });
