...
export default function PrivacyMarketplacePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-marketplace');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-marketplace',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-marketplace' },
      });
