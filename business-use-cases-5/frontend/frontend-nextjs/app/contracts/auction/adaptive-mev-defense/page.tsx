...
export default function AdaptiveMevDefensePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('adaptive-mev-defense');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'adaptive-mev-defense',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'adaptive-mev-defense' },
      });
