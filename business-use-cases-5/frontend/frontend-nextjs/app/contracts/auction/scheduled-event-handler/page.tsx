...
export default function ScheduledEventHandlerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('scheduled-event-handler');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'scheduled-event-handler',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'scheduled-event-handler' },
      });
