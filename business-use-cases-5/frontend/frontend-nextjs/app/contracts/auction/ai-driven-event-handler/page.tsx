...
export default function AiDrivenEventHandlerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-driven-event-handler');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-driven-event-handler',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-driven-event-handler' },
      });
