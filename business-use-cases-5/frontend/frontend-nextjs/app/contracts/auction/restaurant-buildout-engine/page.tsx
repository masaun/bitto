...
export default function RestaurantBuildoutEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('restaurant-buildout-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'restaurant-buildout-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'restaurant-buildout-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Restaurant Buildout Engine</h1>
