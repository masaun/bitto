...
export default function SolarRoofDeploymentPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('solar-roof-deployment');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'solar-roof-deployment',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'solar-roof-deployment' },
      });
