...
export default function ApprovalManagerSoxPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('approval-manager-sox');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'approval-manager-sox',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'approval-manager-sox' },
      });
