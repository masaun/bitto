...
export default function RiskAssessorPciDssPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('risk-assessor-pci-dss');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'risk-assessor-pci-dss',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'risk-assessor-pci-dss' },
      });
