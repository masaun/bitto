...
export default function TransparencyDisclosurePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('transparency-disclosure');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'transparency-disclosure',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'transparency-disclosure' },
      });
