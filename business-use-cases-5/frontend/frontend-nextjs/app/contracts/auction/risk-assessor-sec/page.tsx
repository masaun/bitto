...
export default function RiskAssessorSecPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('risk-assessor-sec');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'risk-assessor-sec',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'risk-assessor-sec' },
      });
