...
export default function SelfHealingAgentsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('self-healing-agents');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'self-healing-agents',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'self-healing-agents' },
      });
