...
export default function AiTrustLayerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-trust-layer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-trust-layer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-trust-layer' },
      });
