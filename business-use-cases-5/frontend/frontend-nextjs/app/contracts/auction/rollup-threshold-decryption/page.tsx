...
export default function RollupThresholdDecryptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rollup-threshold-decryption');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rollup-threshold-decryption',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rollup-threshold-decryption' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Rollup Threshold Decryption</h1>
