...
export default function TimeDrivenConditionEvaluatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('time-driven-condition-evaluator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'time-driven-condition-evaluator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'time-driven-condition-evaluator' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Time Driven Condition Evaluator</h1>
