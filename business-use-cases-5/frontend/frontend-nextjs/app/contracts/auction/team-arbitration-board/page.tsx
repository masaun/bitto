...
export default function TeamArbitrationBoardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-arbitration-board');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-arbitration-board',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-arbitration-board' },
      });
