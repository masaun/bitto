...
export default function BespokeWardrobeStudioPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bespoke-wardrobe-studio');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bespoke-wardrobe-studio',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bespoke-wardrobe-studio' },
      });
