...
export default function AgentLicensingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-licensing');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-licensing',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-licensing' },
      });
