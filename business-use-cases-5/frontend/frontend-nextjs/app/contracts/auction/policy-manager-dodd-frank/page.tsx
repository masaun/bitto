...
export default function PolicyManagerDoddFrankPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('policy-manager-dodd-frank');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'policy-manager-dodd-frank',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'policy-manager-dodd-frank' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Policy Manager Dodd Frank</h1>
