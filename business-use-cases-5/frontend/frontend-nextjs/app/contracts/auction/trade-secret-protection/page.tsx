...
export default function TradeSecretProtectionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('trade-secret-protection');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'trade-secret-protection',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'trade-secret-protection' },
      });
