...
export default function HazardMitigationProgramPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('hazard-mitigation-program');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'hazard-mitigation-program',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'hazard-mitigation-program' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Hazard Mitigation Program</h1>
