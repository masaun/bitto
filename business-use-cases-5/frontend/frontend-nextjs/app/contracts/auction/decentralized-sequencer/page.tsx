...
export default function DecentralizedSequencerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('decentralized-sequencer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'decentralized-sequencer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'decentralized-sequencer' },
      });
