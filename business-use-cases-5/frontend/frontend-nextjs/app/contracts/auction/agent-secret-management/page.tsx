...
export default function AgentSecretManagementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-secret-management');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-secret-management',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-secret-management' },
      });
