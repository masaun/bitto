...
export default function WinterSnowboardPrizePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('winter-snowboard-prize');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'winter-snowboard-prize',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'winter-snowboard-prize' },
      });
