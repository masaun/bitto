...
export default function AgentMemoryRetentionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-memory-retention');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-memory-retention',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-memory-retention' },
      });
