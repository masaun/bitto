...
export default function ConditionalRuleEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('conditional-rule-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'conditional-rule-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'conditional-rule-engine' },
      });
