...
export default function AiDrivenProcessMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-driven-process-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-driven-process-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-driven-process-monitor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Ai Driven Process Monitor</h1>
