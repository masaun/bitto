...
export default function ShieldedLimitOrderPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('shielded-limit-order');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'shielded-limit-order',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'shielded-limit-order' },
      });
