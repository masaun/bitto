...
export default function AiSimulationRunnerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-simulation-runner');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-simulation-runner',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-simulation-runner' },
      });
