...
export default function ViolationDetectorFiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('violation-detector-fia');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'violation-detector-fia',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'violation-detector-fia' },
      });
