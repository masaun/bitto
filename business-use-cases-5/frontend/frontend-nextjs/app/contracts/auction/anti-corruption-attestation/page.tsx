...
export default function AntiCorruptionAttestationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('anti-corruption-attestation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'anti-corruption-attestation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'anti-corruption-attestation' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Anti Corruption Attestation</h1>
