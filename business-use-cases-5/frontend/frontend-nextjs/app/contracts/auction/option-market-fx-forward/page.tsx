...
export default function OptionMarketFxForwardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('option-market-fx-forward');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'option-market-fx-forward',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'option-market-fx-forward' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Option Market Fx Forward</h1>
