...
export default function RiskAssessorMifidPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('risk-assessor-mifid');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'risk-assessor-mifid',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'risk-assessor-mifid' },
      });
