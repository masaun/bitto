...
export default function PrivacyLiquidityPoolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-liquidity-pool');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-liquidity-pool',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-liquidity-pool' },
      });
