...
export default function SanctionsDetectorEmirPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sanctions-detector-emir');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sanctions-detector-emir',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sanctions-detector-emir' },
      });
