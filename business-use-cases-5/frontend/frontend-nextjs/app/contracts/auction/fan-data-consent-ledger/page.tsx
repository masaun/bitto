...
export default function FanDataConsentLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('fan-data-consent-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'fan-data-consent-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'fan-data-consent-ledger' },
      });
