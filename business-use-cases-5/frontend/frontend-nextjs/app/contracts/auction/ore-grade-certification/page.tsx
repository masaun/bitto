...
export default function OreGradeCertificationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ore-grade-certification');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ore-grade-certification',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ore-grade-certification' },
      });
