...
export default function BlockAssemblyLogPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('block-assembly-log');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'block-assembly-log',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'block-assembly-log' },
      });
