...
export default function PrivacyEnforcerUcitsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-enforcer-ucits');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-enforcer-ucits',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-enforcer-ucits' },
      });
