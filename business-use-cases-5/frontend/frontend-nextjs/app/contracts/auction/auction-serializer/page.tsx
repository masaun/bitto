...
export default function AuctionSerializerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-serializer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-serializer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-serializer' },
      });
