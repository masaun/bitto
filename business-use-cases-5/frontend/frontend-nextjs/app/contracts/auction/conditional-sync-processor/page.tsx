...
export default function ConditionalSyncProcessorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('conditional-sync-processor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'conditional-sync-processor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'conditional-sync-processor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Conditional Sync Processor</h1>
