...
export default function BilateralNettingCommoditySwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bilateral-netting-commodity-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bilateral-netting-commodity-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bilateral-netting-commodity-swap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Bilateral Netting Commodity Swap</h1>
