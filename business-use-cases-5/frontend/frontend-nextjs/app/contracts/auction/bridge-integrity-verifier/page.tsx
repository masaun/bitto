...
export default function BridgeIntegrityVerifierPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bridge-integrity-verifier');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bridge-integrity-verifier',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bridge-integrity-verifier' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Bridge Integrity Verifier</h1>
