...
export default function DistributedKeyGenerationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('distributed-key-generation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'distributed-key-generation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'distributed-key-generation' },
      });
