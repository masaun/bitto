...
export default function AgentMemoryRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-memory-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-memory-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-memory-registry' },
      });
