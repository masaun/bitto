...
export default function AiDrivenSyncProcessorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-driven-sync-processor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-driven-sync-processor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-driven-sync-processor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Ai Driven Sync Processor</h1>
