...
export default function InfraAccessControlPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('infra-access-control');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'infra-access-control',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'infra-access-control' },
      });
