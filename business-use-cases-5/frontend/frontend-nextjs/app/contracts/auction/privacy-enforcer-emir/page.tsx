...
export default function PrivacyEnforcerEmirPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-enforcer-emir');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-enforcer-emir',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-enforcer-emir' },
      });
