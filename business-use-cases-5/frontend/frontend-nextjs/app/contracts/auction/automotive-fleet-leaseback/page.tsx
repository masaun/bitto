...
export default function AutomotiveFleetLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automotive-fleet-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automotive-fleet-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automotive-fleet-leaseback' },
      });
