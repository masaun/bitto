...
export default function ModelTransferPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('model-transfer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'model-transfer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'model-transfer' },
      });
