...
export default function PortOfEntryClearancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('port-of-entry-clearance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'port-of-entry-clearance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'port-of-entry-clearance' },
      });
