...
export default function RegionalStateMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('regional-state-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'regional-state-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'regional-state-monitor' },
      });
