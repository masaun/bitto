...
export default function EventDrivenSequentialProcessorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-driven-sequential-processor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-driven-sequential-processor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-driven-sequential-processor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Event Driven Sequential Processor</h1>
