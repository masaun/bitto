...
export default function SubcontractorRetainageEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('subcontractor-retainage-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'subcontractor-retainage-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'subcontractor-retainage-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Subcontractor Retainage Engine</h1>
