...
export default function SportsAnalyticsMarketplacePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-analytics-marketplace');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-analytics-marketplace',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-analytics-marketplace' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Sports Analytics Marketplace</h1>
