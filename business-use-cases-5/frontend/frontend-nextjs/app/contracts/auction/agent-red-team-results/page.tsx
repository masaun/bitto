...
export default function AgentRedTeamResultsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-red-team-results');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-red-team-results',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-red-team-results' },
      });
