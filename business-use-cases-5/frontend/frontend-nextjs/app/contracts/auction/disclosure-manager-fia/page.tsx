...
export default function DisclosureManagerFiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('disclosure-manager-fia');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'disclosure-manager-fia',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'disclosure-manager-fia' },
      });
