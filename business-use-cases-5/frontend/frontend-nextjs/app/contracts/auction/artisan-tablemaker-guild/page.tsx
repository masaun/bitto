...
export default function ArtisanTablemakerGuildPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('artisan-tablemaker-guild');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'artisan-tablemaker-guild',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'artisan-tablemaker-guild' },
      });
