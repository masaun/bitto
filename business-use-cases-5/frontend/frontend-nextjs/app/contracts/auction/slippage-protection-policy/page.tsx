...
export default function SlippageProtectionPolicyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('slippage-protection-policy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'slippage-protection-policy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'slippage-protection-policy' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Slippage Protection Policy</h1>
