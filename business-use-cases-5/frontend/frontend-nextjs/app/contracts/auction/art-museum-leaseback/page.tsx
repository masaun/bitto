...
export default function ArtMuseumLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('art-museum-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'art-museum-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'art-museum-leaseback' },
      });
