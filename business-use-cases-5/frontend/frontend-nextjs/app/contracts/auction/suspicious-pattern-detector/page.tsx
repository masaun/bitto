...
export default function SuspiciousPatternDetectorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('suspicious-pattern-detector');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'suspicious-pattern-detector',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'suspicious-pattern-detector' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Suspicious Pattern Detector</h1>
