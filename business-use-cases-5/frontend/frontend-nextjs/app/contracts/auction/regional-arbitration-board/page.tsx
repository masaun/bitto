...
export default function RegionalArbitrationBoardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('regional-arbitration-board');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'regional-arbitration-board',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'regional-arbitration-board' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Regional Arbitration Board</h1>
