...
export default function EthicsReviewBoardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ethics-review-board');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ethics-review-board',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ethics-review-board' },
      });
