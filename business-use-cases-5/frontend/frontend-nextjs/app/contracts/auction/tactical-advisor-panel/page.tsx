...
export default function TacticalAdvisorPanelPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tactical-advisor-panel');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tactical-advisor-panel',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tactical-advisor-panel' },
      });
