...
export default function DisclosureManagerCcpaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('disclosure-manager-ccpa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'disclosure-manager-ccpa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'disclosure-manager-ccpa' },
      });
