...
export default function PrivateAssetTransferPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('private-asset-transfer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'private-asset-transfer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'private-asset-transfer' },
      });
