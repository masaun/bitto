...
export default function AsphaltRecyclingPlantPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('asphalt-recycling-plant');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'asphalt-recycling-plant',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'asphalt-recycling-plant' },
      });
