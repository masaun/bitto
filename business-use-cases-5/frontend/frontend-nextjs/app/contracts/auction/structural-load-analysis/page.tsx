...
export default function StructuralLoadAnalysisPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('structural-load-analysis');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'structural-load-analysis',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'structural-load-analysis' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Structural Load Analysis</h1>
