...
export default function TeamPolicyCommitteePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-policy-committee');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-policy-committee',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-policy-committee' },
      });
