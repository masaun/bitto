...
export default function DivisionalRoleManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('divisional-role-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'divisional-role-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'divisional-role-manager' },
      });
