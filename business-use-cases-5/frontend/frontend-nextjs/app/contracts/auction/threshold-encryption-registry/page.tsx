...
export default function ThresholdEncryptionRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('threshold-encryption-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'threshold-encryption-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'threshold-encryption-registry' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Threshold Encryption Registry</h1>
