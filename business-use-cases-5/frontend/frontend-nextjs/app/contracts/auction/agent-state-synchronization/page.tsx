...
export default function AgentStateSynchronizationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-state-synchronization');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-state-synchronization',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-state-synchronization' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Agent State Synchronization</h1>
