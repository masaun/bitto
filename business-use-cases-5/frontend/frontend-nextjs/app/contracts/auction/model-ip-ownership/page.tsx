...
export default function ModelIpOwnershipPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('model-ip-ownership');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'model-ip-ownership',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'model-ip-ownership' },
      });
