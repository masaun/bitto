...
export default function SealedGameBiddingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sealed-game-bidding');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sealed-game-bidding',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sealed-game-bidding' },
      });
