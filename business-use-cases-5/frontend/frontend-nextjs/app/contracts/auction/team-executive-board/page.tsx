...
export default function TeamExecutiveBoardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-executive-board');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-executive-board',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-executive-board' },
      });
