...
export default function AgentAlignmentMarketPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-alignment-market');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-alignment-market',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-alignment-market' },
      });
