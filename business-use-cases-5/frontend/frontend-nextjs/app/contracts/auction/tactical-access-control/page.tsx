...
export default function TacticalAccessControlPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tactical-access-control');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tactical-access-control',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tactical-access-control' },
      });
