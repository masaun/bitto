...
export default function AgentPriorityQueuePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-priority-queue');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-priority-queue',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-priority-queue' },
      });
