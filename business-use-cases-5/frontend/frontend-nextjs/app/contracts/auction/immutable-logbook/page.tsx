...
export default function ImmutableLogbookPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('immutable-logbook');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'immutable-logbook',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'immutable-logbook' },
      });
