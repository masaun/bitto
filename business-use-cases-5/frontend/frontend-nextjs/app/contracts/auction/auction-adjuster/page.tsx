...
export default function AuctionAdjusterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-adjuster');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-adjuster',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-adjuster' },
      });
