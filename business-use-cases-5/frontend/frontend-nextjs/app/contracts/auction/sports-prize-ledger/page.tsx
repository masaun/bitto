...
export default function SportsPrizeLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-prize-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-prize-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-prize-ledger' },
      });
