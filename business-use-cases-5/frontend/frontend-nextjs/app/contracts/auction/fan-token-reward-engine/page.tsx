...
export default function FanTokenRewardEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('fan-token-reward-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'fan-token-reward-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'fan-token-reward-engine' },
      });
