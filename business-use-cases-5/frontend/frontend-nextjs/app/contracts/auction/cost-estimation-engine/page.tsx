...
export default function CostEstimationEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('cost-estimation-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'cost-estimation-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'cost-estimation-engine' },
      });
