...
export default function ProtocolPrivacyScorePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('protocol-privacy-score');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'protocol-privacy-score',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'protocol-privacy-score' },
      });
