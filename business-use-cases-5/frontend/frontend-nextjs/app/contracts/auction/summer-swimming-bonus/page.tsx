...
export default function SummerSwimmingBonusPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('summer-swimming-bonus');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'summer-swimming-bonus',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'summer-swimming-bonus' },
      });
