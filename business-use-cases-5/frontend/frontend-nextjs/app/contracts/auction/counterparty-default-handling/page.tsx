...
export default function CounterpartyDefaultHandlingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('counterparty-default-handling');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'counterparty-default-handling',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'counterparty-default-handling' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Counterparty Default Handling</h1>
