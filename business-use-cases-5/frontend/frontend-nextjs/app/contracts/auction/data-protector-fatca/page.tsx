...
export default function DataProtectorFatcaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('data-protector-fatca');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'data-protector-fatca',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'data-protector-fatca' },
      });
