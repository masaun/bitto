...
export default function DocumentProcessorCcpaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('document-processor-ccpa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'document-processor-ccpa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'document-processor-ccpa' },
      });
