...
export default function BridgeRollbackEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bridge-rollback-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bridge-rollback-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bridge-rollback-engine' },
      });
