...
export default function TacticalChangeManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tactical-change-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tactical-change-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tactical-change-manager' },
      });
