...
export default function AgentKillSwitchPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-kill-switch');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-kill-switch',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-kill-switch' },
      });
