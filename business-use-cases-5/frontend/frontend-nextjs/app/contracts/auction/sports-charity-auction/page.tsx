...
export default function SportsCharityAuctionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-charity-auction');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-charity-auction',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-charity-auction' },
      });
