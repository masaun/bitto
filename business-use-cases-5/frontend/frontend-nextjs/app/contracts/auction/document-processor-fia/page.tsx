...
export default function DocumentProcessorFiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('document-processor-fia');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'document-processor-fia',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'document-processor-fia' },
      });
