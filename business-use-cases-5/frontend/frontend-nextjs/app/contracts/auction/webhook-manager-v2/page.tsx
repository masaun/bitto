...
export default function WebhookManagerV2Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('webhook-manager-v2');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'webhook-manager-v2',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'webhook-manager-v2' },
      });
