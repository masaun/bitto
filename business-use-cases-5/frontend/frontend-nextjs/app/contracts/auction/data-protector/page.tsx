...
export default function DataProtectorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('data-protector');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'data-protector',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'data-protector' },
      });
