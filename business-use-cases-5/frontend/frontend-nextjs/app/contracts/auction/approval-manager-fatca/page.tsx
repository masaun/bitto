...
export default function ApprovalManagerFatcaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('approval-manager-fatca');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'approval-manager-fatca',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'approval-manager-fatca' },
      });
