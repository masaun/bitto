...
export default function FacadeRenovationEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('facade-renovation-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'facade-renovation-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'facade-renovation-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Facade Renovation Engine</h1>
