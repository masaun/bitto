...
export default function ScheduledQueueManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('scheduled-queue-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'scheduled-queue-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'scheduled-queue-manager' },
      });
