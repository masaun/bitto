...
export default function AuctionAggregatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-aggregator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-aggregator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-aggregator' },
      });
