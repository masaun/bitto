...
export default function ParallelProcessorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('parallel-processor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'parallel-processor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'parallel-processor' },
      });
