...
export default function VenueBroadcastRightsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('venue-broadcast-rights');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'venue-broadcast-rights',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'venue-broadcast-rights' },
      });
