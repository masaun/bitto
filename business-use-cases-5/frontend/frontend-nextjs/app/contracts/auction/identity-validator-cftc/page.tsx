...
export default function IdentityValidatorCftcPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('identity-validator-cftc');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'identity-validator-cftc',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'identity-validator-cftc' },
      });
