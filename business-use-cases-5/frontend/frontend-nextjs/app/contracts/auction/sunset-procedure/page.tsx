...
export default function SunsetProcedurePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sunset-procedure');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sunset-procedure',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sunset-procedure' },
      });
