...
export default function GlobalRankingPointsLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('global-ranking-points-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'global-ranking-points-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'global-ranking-points-ledger' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Global Ranking Points Ledger</h1>
