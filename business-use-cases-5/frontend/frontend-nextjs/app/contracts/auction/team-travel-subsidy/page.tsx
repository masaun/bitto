...
export default function TeamTravelSubsidyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-travel-subsidy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-travel-subsidy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-travel-subsidy' },
      });
