...
export default function AiCyberDefenseLayerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-cyber-defense-layer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-cyber-defense-layer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-cyber-defense-layer' },
      });
