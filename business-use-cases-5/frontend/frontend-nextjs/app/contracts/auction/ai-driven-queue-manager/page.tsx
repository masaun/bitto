...
export default function AiDrivenQueueManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-driven-queue-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-driven-queue-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-driven-queue-manager' },
      });
