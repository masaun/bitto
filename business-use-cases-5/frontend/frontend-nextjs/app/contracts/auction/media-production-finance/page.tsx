...
export default function MediaProductionFinancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('media-production-finance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'media-production-finance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'media-production-finance' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Media Production Finance</h1>
