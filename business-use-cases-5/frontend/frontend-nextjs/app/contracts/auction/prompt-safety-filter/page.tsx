...
export default function PromptSafetyFilterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('prompt-safety-filter');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'prompt-safety-filter',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'prompt-safety-filter' },
      });
