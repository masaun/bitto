...
export default function CrossChainStateManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('cross-chain-state-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'cross-chain-state-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'cross-chain-state-manager' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Cross Chain State Manager</h1>
