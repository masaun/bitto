...
export default function QueueManagerLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('queue-manager-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'queue-manager-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'queue-manager-legacy' },
      });
