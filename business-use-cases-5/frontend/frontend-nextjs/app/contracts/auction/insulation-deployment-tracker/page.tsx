...
export default function InsulationDeploymentTrackerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('insulation-deployment-tracker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'insulation-deployment-tracker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'insulation-deployment-tracker' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Insulation Deployment Tracker</h1>
