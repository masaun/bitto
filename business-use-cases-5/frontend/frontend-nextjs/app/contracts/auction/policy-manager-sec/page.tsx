...
export default function PolicyManagerSecPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('policy-manager-sec');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'policy-manager-sec',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'policy-manager-sec' },
      });
