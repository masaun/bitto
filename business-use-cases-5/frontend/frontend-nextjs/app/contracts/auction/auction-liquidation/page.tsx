...
export default function AuctionLiquidationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-liquidation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-liquidation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-liquidation' },
      });
