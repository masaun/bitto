...
export default function RiskMitigationPlanPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('risk-mitigation-plan');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'risk-mitigation-plan',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'risk-mitigation-plan' },
      });
