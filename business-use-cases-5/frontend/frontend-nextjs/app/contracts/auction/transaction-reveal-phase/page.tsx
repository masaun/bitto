...
export default function TransactionRevealPhasePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('transaction-reveal-phase');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'transaction-reveal-phase',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'transaction-reveal-phase' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Transaction Reveal Phase</h1>
