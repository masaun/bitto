...
export default function AiPoweredFraudDetectionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-powered-fraud-detection');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-powered-fraud-detection',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-powered-fraud-detection' },
      });
