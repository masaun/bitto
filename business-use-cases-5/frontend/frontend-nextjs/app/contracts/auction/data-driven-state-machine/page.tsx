...
export default function DataDrivenStateMachinePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('data-driven-state-machine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'data-driven-state-machine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'data-driven-state-machine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Data Driven State Machine</h1>
