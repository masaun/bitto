...
export default function WebhookManagerExperimentalPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('webhook-manager-experimental');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'webhook-manager-experimental',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'webhook-manager-experimental' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Webhook Manager Experimental</h1>
