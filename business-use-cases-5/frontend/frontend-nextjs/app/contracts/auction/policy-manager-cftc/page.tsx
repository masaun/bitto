...
export default function PolicyManagerCftcPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('policy-manager-cftc');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'policy-manager-cftc',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'policy-manager-cftc' },
      });
