...
export default function SlopeStabilizationProgramPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('slope-stabilization-program');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'slope-stabilization-program',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'slope-stabilization-program' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Slope Stabilization Program</h1>
