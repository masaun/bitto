...
export default function AiAgentComposabilityPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-agent-composability');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-agent-composability',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-agent-composability' },
      });
