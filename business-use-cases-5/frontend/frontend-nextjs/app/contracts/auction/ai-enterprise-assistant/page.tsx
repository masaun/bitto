...
export default function AiEnterpriseAssistantPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-enterprise-assistant');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-enterprise-assistant',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-enterprise-assistant' },
      });
