...
export default function RemediationHandlerUcitsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('remediation-handler-ucits');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'remediation-handler-ucits',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'remediation-handler-ucits' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Remediation Handler Ucits</h1>
