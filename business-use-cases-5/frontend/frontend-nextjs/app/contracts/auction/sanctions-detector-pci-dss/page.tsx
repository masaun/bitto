...
export default function SanctionsDetectorPciDssPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sanctions-detector-pci-dss');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sanctions-detector-pci-dss',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sanctions-detector-pci-dss' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Sanctions Detector Pci Dss</h1>
