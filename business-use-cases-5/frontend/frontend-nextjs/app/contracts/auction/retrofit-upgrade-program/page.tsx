...
export default function RetrofitUpgradeProgramPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('retrofit-upgrade-program');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'retrofit-upgrade-program',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'retrofit-upgrade-program' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Retrofit Upgrade Program</h1>
