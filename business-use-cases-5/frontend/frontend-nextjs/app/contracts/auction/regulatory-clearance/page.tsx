...
export default function RegulatoryClearancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('regulatory-clearance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'regulatory-clearance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'regulatory-clearance' },
      });
