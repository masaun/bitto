...
export default function EnergyPerformanceCertPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('energy-performance-cert');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'energy-performance-cert',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'energy-performance-cert' },
      });
