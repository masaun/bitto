...
export default function NewSeparationTechIpPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('new-separation-tech-ip');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'new-separation-tech-ip',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'new-separation-tech-ip' },
      });
