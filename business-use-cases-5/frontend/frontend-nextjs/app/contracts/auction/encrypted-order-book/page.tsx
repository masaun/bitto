...
export default function EncryptedOrderBookPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-order-book');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-order-book',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-order-book' },
      });
