...
export default function AuctionSequencerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-sequencer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-sequencer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-sequencer' },
      });
