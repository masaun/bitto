...
export default function RegionalVoteEscrowPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('regional-vote-escrow');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'regional-vote-escrow',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'regional-vote-escrow' },
      });
