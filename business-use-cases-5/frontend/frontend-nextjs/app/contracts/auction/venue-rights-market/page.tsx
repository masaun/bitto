...
export default function VenueRightsMarketPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('venue-rights-market');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'venue-rights-market',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'venue-rights-market' },
      });
