...
export default function AgentTrialAccessPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-trial-access');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-trial-access',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-trial-access' },
      });
