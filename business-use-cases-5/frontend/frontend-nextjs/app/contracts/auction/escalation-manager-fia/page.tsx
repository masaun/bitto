...
export default function EscalationManagerFiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('escalation-manager-fia');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'escalation-manager-fia',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'escalation-manager-fia' },
      });
