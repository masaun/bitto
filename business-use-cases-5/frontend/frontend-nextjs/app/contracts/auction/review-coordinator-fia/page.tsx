...
export default function ReviewCoordinatorFiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('review-coordinator-fia');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'review-coordinator-fia',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'review-coordinator-fia' },
      });
