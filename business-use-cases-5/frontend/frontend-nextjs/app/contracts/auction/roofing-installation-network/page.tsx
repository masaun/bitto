...
export default function RoofingInstallationNetworkPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('roofing-installation-network');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'roofing-installation-network',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'roofing-installation-network' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Roofing Installation Network</h1>
