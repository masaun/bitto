...
export default function OrderingProofPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ordering-proof');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ordering-proof',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ordering-proof' },
      });
