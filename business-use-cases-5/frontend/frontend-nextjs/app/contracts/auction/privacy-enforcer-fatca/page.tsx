...
export default function PrivacyEnforcerFatcaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-enforcer-fatca');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-enforcer-fatca',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-enforcer-fatca' },
      });
