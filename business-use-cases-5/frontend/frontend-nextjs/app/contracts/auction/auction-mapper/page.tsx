...
export default function AuctionMapperPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-mapper');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-mapper',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-mapper' },
      });
