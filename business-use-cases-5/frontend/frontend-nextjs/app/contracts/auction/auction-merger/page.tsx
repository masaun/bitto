...
export default function AuctionMergerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-merger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-merger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-merger' },
      });
