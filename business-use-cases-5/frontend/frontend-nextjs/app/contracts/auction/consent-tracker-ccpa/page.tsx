...
export default function ConsentTrackerCcpaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('consent-tracker-ccpa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'consent-tracker-ccpa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'consent-tracker-ccpa' },
      });
