...
export default function RiskAssessorUcitsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('risk-assessor-ucits');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'risk-assessor-ucits',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'risk-assessor-ucits' },
      });
