...
export default function EventDrivenConditionEvaluatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-driven-condition-evaluator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-driven-condition-evaluator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-driven-condition-evaluator' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Event Driven Condition Evaluator</h1>
