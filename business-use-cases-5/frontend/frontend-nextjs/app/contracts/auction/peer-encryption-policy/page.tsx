...
export default function PeerEncryptionPolicyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('peer-encryption-policy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'peer-encryption-policy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'peer-encryption-policy' },
      });
