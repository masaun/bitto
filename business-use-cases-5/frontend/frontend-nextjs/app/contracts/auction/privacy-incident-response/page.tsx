...
export default function PrivacyIncidentResponsePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-incident-response');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-incident-response',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-incident-response' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Privacy Incident Response</h1>
