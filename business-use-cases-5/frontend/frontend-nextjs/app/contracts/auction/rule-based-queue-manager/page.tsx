...
export default function RuleBasedQueueManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rule-based-queue-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rule-based-queue-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rule-based-queue-manager' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Rule Based Queue Manager</h1>
