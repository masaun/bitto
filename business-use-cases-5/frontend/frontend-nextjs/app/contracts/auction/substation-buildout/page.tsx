...
export default function SubstationBuildoutPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('substation-buildout');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'substation-buildout',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'substation-buildout' },
      });
