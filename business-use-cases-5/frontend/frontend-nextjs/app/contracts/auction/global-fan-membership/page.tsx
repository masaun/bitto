...
export default function GlobalFanMembershipPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('global-fan-membership');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'global-fan-membership',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'global-fan-membership' },
      });
