...
export default function TimeDrivenStateMachinePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('time-driven-state-machine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'time-driven-state-machine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'time-driven-state-machine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Time Driven State Machine</h1>
