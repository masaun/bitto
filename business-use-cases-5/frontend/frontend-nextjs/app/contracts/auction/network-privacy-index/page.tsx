...
export default function NetworkPrivacyIndexPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('network-privacy-index');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'network-privacy-index',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'network-privacy-index' },
      });
