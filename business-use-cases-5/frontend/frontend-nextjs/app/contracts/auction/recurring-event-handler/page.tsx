...
export default function RecurringEventHandlerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('recurring-event-handler');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'recurring-event-handler',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'recurring-event-handler' },
      });
