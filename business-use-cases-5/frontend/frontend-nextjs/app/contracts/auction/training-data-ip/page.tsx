...
export default function TrainingDataIpPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('training-data-ip');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'training-data-ip',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'training-data-ip' },
      });
