...
export default function SportsMediaTokenPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-media-token');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-media-token',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-media-token' },
      });
