...
export default function EventAppLoyaltyTokenPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-app-loyalty-token');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-app-loyalty-token',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-app-loyalty-token' },
      });
