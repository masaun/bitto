...
export default function PrizeMoneySplitterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('prize-money-splitter');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'prize-money-splitter',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'prize-money-splitter' },
      });
