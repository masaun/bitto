...
export default function AuctionBridgePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-bridge');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-bridge',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-bridge' },
      });
