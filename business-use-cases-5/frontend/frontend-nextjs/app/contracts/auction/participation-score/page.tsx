...
export default function ParticipationScorePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('participation-score');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'participation-score',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'participation-score' },
      });
