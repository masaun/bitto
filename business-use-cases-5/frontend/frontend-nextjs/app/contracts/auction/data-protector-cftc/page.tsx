...
export default function DataProtectorCftcPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('data-protector-cftc');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'data-protector-cftc',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'data-protector-cftc' },
      });
