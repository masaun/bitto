...
export default function MultiItemAuctionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('multi-item-auction');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'multi-item-auction',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'multi-item-auction' },
      });
