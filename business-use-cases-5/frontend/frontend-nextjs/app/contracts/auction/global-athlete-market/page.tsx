...
export default function GlobalAthleteMarketPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('global-athlete-market');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'global-athlete-market',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'global-athlete-market' },
      });
