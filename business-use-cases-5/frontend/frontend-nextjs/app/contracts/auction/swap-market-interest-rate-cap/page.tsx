...
export default function SwapMarketInterestRateCapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('swap-market-interest-rate-cap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'swap-market-interest-rate-cap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'swap-market-interest-rate-cap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Swap Market Interest Rate Cap</h1>
