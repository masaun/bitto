...
export default function AgentRemediationTrackingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-remediation-tracking');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-remediation-tracking',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-remediation-tracking' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Agent Remediation Tracking</h1>
