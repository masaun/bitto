...
export default function RegionalChangeManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('regional-change-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'regional-change-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'regional-change-manager' },
      });
