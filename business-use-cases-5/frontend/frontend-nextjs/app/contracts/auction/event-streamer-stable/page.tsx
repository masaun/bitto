...
export default function EventStreamerStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-streamer-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-streamer-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-streamer-stable' },
      });
