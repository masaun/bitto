...
export default function SecureTimeSyncPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('secure-time-sync');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'secure-time-sync',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'secure-time-sync' },
      });
