...
export default function FanRewardStakingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('fan-reward-staking');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'fan-reward-staking',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'fan-reward-staking' },
      });
