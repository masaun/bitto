...
export default function DataProtectorCcpaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('data-protector-ccpa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'data-protector-ccpa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'data-protector-ccpa' },
      });
