...
export default function EventSponsorshipLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-sponsorship-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-sponsorship-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-sponsorship-ledger' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Event Sponsorship Ledger</h1>
