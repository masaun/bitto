...
export default function AthleteBonusTokenPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('athlete-bonus-token');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'athlete-bonus-token',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'athlete-bonus-token' },
      });
