...
export default function EventStreamerAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-streamer-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-streamer-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-streamer-alpha' },
      });
