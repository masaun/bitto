...
export default function SecureMlOraclePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('secure-ml-oracle');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'secure-ml-oracle',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'secure-ml-oracle' },
      });
