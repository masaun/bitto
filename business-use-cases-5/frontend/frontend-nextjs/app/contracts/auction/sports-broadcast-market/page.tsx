...
export default function SportsBroadcastMarketPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-broadcast-market');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-broadcast-market',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-broadcast-market' },
      });
