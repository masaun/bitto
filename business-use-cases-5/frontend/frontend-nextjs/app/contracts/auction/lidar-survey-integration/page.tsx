...
export default function LidarSurveyIntegrationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('lidar-survey-integration');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'lidar-survey-integration',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'lidar-survey-integration' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Lidar Survey Integration</h1>
