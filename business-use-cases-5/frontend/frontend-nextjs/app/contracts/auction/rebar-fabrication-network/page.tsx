...
export default function RebarFabricationNetworkPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rebar-fabrication-network');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rebar-fabrication-network',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rebar-fabrication-network' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Rebar Fabrication Network</h1>
