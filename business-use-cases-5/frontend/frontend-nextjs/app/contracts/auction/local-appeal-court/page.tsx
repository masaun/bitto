...
export default function LocalAppealCourtPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-appeal-court');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-appeal-court',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-appeal-court' },
      });
