...
export default function PricingIndexOraclePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('pricing-index-oracle');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'pricing-index-oracle',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'pricing-index-oracle' },
      });
