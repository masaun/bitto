...
export default function LocalPolicyCommitteePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-policy-committee');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-policy-committee',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-policy-committee' },
      });
