...
export default function TransitHubDevelopmentPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('transit-hub-development');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'transit-hub-development',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'transit-hub-development' },
      });
