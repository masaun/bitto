...
export default function AgentEvaluationMetricsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-evaluation-metrics');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-evaluation-metrics',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-evaluation-metrics' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Agent Evaluation Metrics</h1>
