...
export default function EscalationManagerFatcaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('escalation-manager-fatca');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'escalation-manager-fatca',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'escalation-manager-fatca' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Escalation Manager Fatca</h1>
