...
export default function AppealProcessPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('appeal-process');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'appeal-process',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'appeal-process' },
      });
