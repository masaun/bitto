...
export default function AgentErrorTrackingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-error-tracking');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-error-tracking',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-error-tracking' },
      });
