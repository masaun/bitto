...
export default function ProjectAdvisorPanelPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('project-advisor-panel');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'project-advisor-panel',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'project-advisor-panel' },
      });
