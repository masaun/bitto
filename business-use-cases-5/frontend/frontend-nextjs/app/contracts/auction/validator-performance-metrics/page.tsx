...
export default function ValidatorPerformanceMetricsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('validator-performance-metrics');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'validator-performance-metrics',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'validator-performance-metrics' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Validator Performance Metrics</h1>
