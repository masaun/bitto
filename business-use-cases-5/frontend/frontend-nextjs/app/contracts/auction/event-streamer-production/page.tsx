...
export default function EventStreamerProductionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-streamer-production');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-streamer-production',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-streamer-production' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Event Streamer Production</h1>
