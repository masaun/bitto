...
export default function BidEvaluationEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bid-evaluation-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bid-evaluation-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bid-evaluation-engine' },
      });
