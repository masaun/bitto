...
export default function HistoricalProductionLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('historical-production-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'historical-production-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'historical-production-ledger' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Historical Production Ledger</h1>
