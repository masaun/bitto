...
export default function SeismicRetrofitSystemPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('seismic-retrofit-system');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'seismic-retrofit-system',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'seismic-retrofit-system' },
      });
