...
export default function LocalChangeManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-change-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-change-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-change-manager' },
      });
