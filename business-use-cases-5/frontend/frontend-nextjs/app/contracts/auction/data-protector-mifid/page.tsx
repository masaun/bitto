...
export default function DataProtectorMifidPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('data-protector-mifid');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'data-protector-mifid',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'data-protector-mifid' },
      });
