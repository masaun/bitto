...
export default function ModelVersionControlPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('model-version-control');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'model-version-control',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'model-version-control' },
      });
