...
export default function AgentDataAnonymizationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-data-anonymization');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-data-anonymization',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-data-anonymization' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Agent Data Anonymization</h1>
