...
export default function AgentConsentCapturePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-consent-capture');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-consent-capture',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-consent-capture' },
      });
