...
export default function SupplierBidEvaluationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('supplier-bid-evaluation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'supplier-bid-evaluation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'supplier-bid-evaluation' },
      });
