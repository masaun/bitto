...
export default function PrivacyEnforcerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-enforcer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-enforcer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-enforcer' },
      });
