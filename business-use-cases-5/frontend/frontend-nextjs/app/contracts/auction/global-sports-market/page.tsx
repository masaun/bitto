...
export default function GlobalSportsMarketPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('global-sports-market');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'global-sports-market',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'global-sports-market' },
      });
