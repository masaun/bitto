...
export default function IpExpiryPolicyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ip-expiry-policy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ip-expiry-policy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ip-expiry-policy' },
      });
