...
export default function KnowledgeVersioningPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('knowledge-versioning');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'knowledge-versioning',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'knowledge-versioning' },
      });
