...
export default function DelayedCronExecutorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('delayed-cron-executor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'delayed-cron-executor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'delayed-cron-executor' },
      });
