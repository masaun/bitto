...
export default function RiskIncidentLogPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('risk-incident-log');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'risk-incident-log',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'risk-incident-log' },
      });
