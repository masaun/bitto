...
export default function DisclosureManagerEmirPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('disclosure-manager-emir');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'disclosure-manager-emir',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'disclosure-manager-emir' },
      });
