...
export default function NodeAttestationRegistryPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('node-attestation-registry');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'node-attestation-registry',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'node-attestation-registry' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Node Attestation Registry</h1>
