...
export default function DepartmentalStrategyCommitteePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('departmental-strategy-committee');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'departmental-strategy-committee',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'departmental-strategy-committee' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Departmental Strategy Committee</h1>
