...
export default function EcoIndustrialParkPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('eco-industrial-park');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'eco-industrial-park',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'eco-industrial-park' },
      });
