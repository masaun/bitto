...
export default function OptionMarketFxOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('option-market-fx-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'option-market-fx-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'option-market-fx-option' },
      });
