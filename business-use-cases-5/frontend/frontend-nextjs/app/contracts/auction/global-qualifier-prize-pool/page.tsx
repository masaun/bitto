...
export default function GlobalQualifierPrizePoolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('global-qualifier-prize-pool');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'global-qualifier-prize-pool',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'global-qualifier-prize-pool' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Global Qualifier Prize Pool</h1>
