...
export default function BilateralNettingInterestRateCapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bilateral-netting-interest-rate-cap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bilateral-netting-interest-rate-cap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bilateral-netting-interest-rate-cap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Bilateral Netting Interest Rate Cap</h1>
