...
export default function PolicyManagerPciDssPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('policy-manager-pci-dss');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'policy-manager-pci-dss',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'policy-manager-pci-dss' },
      });
