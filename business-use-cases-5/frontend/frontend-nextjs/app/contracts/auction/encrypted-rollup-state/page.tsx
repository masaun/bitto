...
export default function EncryptedRollupStatePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('encrypted-rollup-state');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'encrypted-rollup-state',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'encrypted-rollup-state' },
      });
