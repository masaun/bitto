...
export default function EthicalReviewBoardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ethical-review-board');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ethical-review-board',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ethical-review-board' },
      });
