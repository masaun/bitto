...
export default function InferenceRoutingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('inference-routing');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'inference-routing',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'inference-routing' },
      });
