...
export default function SanctionsDetectorCftcPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sanctions-detector-cftc');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sanctions-detector-cftc',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sanctions-detector-cftc' },
      });
