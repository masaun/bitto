...
export default function SportsTicketLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-ticket-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-ticket-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-ticket-ledger' },
      });
