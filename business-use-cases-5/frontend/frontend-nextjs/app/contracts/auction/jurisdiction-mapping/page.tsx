...
export default function JurisdictionMappingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('jurisdiction-mapping');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'jurisdiction-mapping',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'jurisdiction-mapping' },
      });
