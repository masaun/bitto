...
export default function BespokeSeatingEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bespoke-seating-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bespoke-seating-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bespoke-seating-engine' },
      });
