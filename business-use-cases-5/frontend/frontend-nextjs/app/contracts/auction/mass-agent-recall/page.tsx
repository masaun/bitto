...
export default function MassAgentRecallPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('mass-agent-recall');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'mass-agent-recall',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'mass-agent-recall' },
      });
