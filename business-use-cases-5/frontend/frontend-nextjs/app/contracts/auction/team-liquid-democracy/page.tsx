...
export default function TeamLiquidDemocracyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-liquid-democracy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-liquid-democracy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-liquid-democracy' },
      });
