...
export default function PrivacyUpgradeEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('privacy-upgrade-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'privacy-upgrade-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'privacy-upgrade-engine' },
      });
