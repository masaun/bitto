...
export default function EvSupplyContractsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ev-supply-contracts');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ev-supply-contracts',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ev-supply-contracts' },
      });
