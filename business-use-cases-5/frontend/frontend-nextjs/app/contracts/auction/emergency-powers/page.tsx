...
export default function EmergencyPowersPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('emergency-powers');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'emergency-powers',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'emergency-powers' },
      });
