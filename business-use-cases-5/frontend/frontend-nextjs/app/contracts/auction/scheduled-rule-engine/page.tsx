...
export default function ScheduledRuleEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('scheduled-rule-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'scheduled-rule-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'scheduled-rule-engine' },
      });
