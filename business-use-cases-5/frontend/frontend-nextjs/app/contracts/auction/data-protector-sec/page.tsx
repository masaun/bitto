...
export default function DataProtectorSecPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('data-protector-sec');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'data-protector-sec',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'data-protector-sec' },
      });
