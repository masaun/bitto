...
export default function AutonomousProcurementAgentPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('autonomous-procurement-agent');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'autonomous-procurement-agent',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'autonomous-procurement-agent' },
      });
