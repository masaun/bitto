...
export default function SportsInsuranceRiskPoolPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-insurance-risk-pool');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-insurance-risk-pool',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-insurance-risk-pool' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Sports Insurance Risk Pool</h1>
