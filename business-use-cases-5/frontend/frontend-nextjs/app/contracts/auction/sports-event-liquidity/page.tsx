...
export default function SportsEventLiquidityPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-event-liquidity');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-event-liquidity',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-event-liquidity' },
      });
