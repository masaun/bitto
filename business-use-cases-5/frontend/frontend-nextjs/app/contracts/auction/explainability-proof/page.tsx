...
export default function ExplainabilityProofPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('explainability-proof');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'explainability-proof',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'explainability-proof' },
      });
