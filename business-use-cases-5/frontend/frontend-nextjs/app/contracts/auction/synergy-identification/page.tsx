...
export default function SynergyIdentificationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('synergy-identification');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'synergy-identification',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'synergy-identification' },
      });
