...
export default function AgentDataQualityScorePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-data-quality-score');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-data-quality-score',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-data-quality-score' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Agent Data Quality Score</h1>
