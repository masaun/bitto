...
export default function AutoPolicyEnforcementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auto-policy-enforcement');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auto-policy-enforcement',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auto-policy-enforcement' },
      });
