...
export default function ProjectPolicyEnforcerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('project-policy-enforcer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'project-policy-enforcer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'project-policy-enforcer' },
      });
