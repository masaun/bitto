...
export default function OptionMarketInterestRateCollarPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('option-market-interest-rate-collar');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'option-market-interest-rate-collar',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'option-market-interest-rate-collar' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Option Market Interest Rate Collar</h1>
