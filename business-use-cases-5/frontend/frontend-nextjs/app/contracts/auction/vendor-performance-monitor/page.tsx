...
export default function VendorPerformanceMonitorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('vendor-performance-monitor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'vendor-performance-monitor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'vendor-performance-monitor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Vendor Performance Monitor</h1>
