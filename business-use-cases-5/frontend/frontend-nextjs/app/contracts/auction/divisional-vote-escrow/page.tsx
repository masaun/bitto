...
export default function DivisionalVoteEscrowPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('divisional-vote-escrow');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'divisional-vote-escrow',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'divisional-vote-escrow' },
      });
