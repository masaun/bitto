...
export default function AiAssetValuationMaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-asset-valuation-ma');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-asset-valuation-ma',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-asset-valuation-ma' },
      });
