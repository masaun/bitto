...
export default function EthicsViolationLogPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ethics-violation-log');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ethics-violation-log',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ethics-violation-log' },
      });
