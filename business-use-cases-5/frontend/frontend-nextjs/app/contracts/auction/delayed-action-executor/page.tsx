...
export default function DelayedActionExecutorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('delayed-action-executor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'delayed-action-executor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'delayed-action-executor' },
      });
