...
export default function AgentMemoryEncryptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-memory-encryption');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-memory-encryption',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-memory-encryption' },
      });
