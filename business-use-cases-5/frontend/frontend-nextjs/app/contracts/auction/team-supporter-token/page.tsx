...
export default function TeamSupporterTokenPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-supporter-token');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-supporter-token',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-supporter-token' },
      });
