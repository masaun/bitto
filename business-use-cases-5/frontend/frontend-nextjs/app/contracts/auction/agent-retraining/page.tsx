...
export default function AgentRetrainingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-retraining');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-retraining',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-retraining' },
      });
