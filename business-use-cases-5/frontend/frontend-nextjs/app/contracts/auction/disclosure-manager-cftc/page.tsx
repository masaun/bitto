...
export default function DisclosureManagerCftcPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('disclosure-manager-cftc');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'disclosure-manager-cftc',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'disclosure-manager-cftc' },
      });
