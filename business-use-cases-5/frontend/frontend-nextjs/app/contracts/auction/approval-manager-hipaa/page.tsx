...
export default function ApprovalManagerHipaaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('approval-manager-hipaa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'approval-manager-hipaa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'approval-manager-hipaa' },
      });
