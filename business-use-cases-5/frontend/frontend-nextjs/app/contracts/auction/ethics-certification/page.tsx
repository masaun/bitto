...
export default function EthicsCertificationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ethics-certification');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ethics-certification',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ethics-certification' },
      });
