...
export default function AgentLearningLoopPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-learning-loop');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-learning-loop',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-learning-loop' },
      });
