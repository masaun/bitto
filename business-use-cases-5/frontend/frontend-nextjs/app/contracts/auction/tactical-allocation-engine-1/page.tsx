...
export default function TacticalAllocationEngine1Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tactical-allocation-engine-1');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tactical-allocation-engine-1',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tactical-allocation-engine-1' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Tactical Allocation Engine 1</h1>
