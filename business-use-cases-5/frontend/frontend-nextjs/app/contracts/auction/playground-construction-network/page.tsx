...
export default function PlaygroundConstructionNetworkPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('playground-construction-network');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'playground-construction-network',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'playground-construction-network' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Playground Construction Network</h1>
