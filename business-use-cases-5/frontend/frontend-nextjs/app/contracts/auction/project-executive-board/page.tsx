...
export default function ProjectExecutiveBoardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('project-executive-board');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'project-executive-board',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'project-executive-board' },
      });
