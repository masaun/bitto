...
export default function ConfidentialSealedBidPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('confidential-sealed-bid');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'confidential-sealed-bid',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'confidential-sealed-bid' },
      });
