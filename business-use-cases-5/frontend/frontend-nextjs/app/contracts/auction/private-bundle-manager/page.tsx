...
export default function PrivateBundleManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('private-bundle-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'private-bundle-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'private-bundle-manager' },
      });
