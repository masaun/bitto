...
export default function AstroResearchGrantPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('astro-research-grant');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'astro-research-grant',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'astro-research-grant' },
      });
