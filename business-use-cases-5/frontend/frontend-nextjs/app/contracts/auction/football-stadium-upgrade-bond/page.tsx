...
export default function FootballStadiumUpgradeBondPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('football-stadium-upgrade-bond');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'football-stadium-upgrade-bond',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'football-stadium-upgrade-bond' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Football Stadium Upgrade Bond</h1>
