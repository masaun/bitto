...
export default function AgentRootCausePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-root-cause');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-root-cause',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-root-cause' },
      });
