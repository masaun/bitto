...
export default function DefenseDemandForecastPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('defense-demand-forecast');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'defense-demand-forecast',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'defense-demand-forecast' },
      });
