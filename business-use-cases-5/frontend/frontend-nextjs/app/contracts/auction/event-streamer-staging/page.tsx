...
export default function EventStreamerStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-streamer-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-streamer-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-streamer-staging' },
      });
