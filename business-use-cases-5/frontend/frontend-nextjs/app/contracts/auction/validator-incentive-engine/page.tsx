...
export default function ValidatorIncentiveEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('validator-incentive-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'validator-incentive-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'validator-incentive-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Validator Incentive Engine</h1>
