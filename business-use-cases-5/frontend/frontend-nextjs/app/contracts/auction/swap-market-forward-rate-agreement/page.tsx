...
export default function SwapMarketForwardRateAgreementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('swap-market-forward-rate-agreement');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'swap-market-forward-rate-agreement',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'swap-market-forward-rate-agreement' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Swap Market Forward Rate Agreement</h1>
