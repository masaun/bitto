...
export default function SimulationArchivePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('simulation-archive');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'simulation-archive',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'simulation-archive' },
      });
