...
export default function AgentAutonomyLevelPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-autonomy-level');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-autonomy-level',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-autonomy-level' },
      });
