...
export default function PolicyManagerMifidPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('policy-manager-mifid');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'policy-manager-mifid',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'policy-manager-mifid' },
      });
