...
export default function ContractAdministrationLedgerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('contract-administration-ledger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'contract-administration-ledger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'contract-administration-ledger' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Contract Administration Ledger</h1>
