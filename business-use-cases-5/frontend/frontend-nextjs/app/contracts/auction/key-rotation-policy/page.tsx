...
export default function KeyRotationPolicyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('key-rotation-policy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'key-rotation-policy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'key-rotation-policy' },
      });
