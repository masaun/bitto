...
export default function AuctionTransformerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-transformer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-transformer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-transformer' },
      });
