...
export default function EscalationManagerCftcPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('escalation-manager-cftc');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'escalation-manager-cftc',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'escalation-manager-cftc' },
      });
