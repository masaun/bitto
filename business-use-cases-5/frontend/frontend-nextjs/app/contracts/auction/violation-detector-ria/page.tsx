...
export default function ViolationDetectorRiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('violation-detector-ria');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'violation-detector-ria',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'violation-detector-ria' },
      });
