...
export default function AutoBidEvaluationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auto-bid-evaluation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auto-bid-evaluation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auto-bid-evaluation' },
      });
