...
export default function AiSecureCollaborationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-secure-collaboration');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-secure-collaboration',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-secure-collaboration' },
      });
