...
export default function SportsInnovationGrantPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-innovation-grant');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-innovation-grant',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-innovation-grant' },
      });
