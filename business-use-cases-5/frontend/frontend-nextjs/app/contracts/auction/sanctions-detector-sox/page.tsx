...
export default function SanctionsDetectorSoxPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sanctions-detector-sox');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sanctions-detector-sox',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sanctions-detector-sox' },
      });
