...
export default function DataCenterBuildoutPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('data-center-buildout');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'data-center-buildout',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'data-center-buildout' },
      });
