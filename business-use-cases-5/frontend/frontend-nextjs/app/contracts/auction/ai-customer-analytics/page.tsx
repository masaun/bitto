...
export default function AiCustomerAnalyticsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-customer-analytics');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-customer-analytics',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-customer-analytics' },
      });
