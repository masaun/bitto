...
export default function BankBranchLeasebackPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bank-branch-leaseback');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bank-branch-leaseback',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bank-branch-leaseback' },
      });
