...
export default function EventStreamerLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('event-streamer-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'event-streamer-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'event-streamer-legacy' },
      });
