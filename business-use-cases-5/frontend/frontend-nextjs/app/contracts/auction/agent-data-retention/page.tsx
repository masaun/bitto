...
export default function AgentDataRetentionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-data-retention');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-data-retention',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-data-retention' },
      });
