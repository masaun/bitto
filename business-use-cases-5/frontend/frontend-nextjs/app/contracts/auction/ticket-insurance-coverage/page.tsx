...
export default function TicketInsuranceCoveragePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ticket-insurance-coverage');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ticket-insurance-coverage',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ticket-insurance-coverage' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Ticket Insurance Coverage</h1>
