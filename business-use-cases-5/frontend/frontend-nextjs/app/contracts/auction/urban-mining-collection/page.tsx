...
export default function UrbanMiningCollectionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('urban-mining-collection');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'urban-mining-collection',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'urban-mining-collection' },
      });
