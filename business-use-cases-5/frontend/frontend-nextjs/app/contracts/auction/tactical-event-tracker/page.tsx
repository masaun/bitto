...
export default function TacticalEventTrackerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tactical-event-tracker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tactical-event-tracker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tactical-event-tracker' },
      });
