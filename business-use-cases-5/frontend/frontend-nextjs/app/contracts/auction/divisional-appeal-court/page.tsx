...
export default function DivisionalAppealCourtPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('divisional-appeal-court');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'divisional-appeal-court',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'divisional-appeal-court' },
      });
