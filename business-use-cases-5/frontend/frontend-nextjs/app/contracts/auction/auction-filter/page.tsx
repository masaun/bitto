...
export default function AuctionFilterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-filter');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-filter',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-filter' },
      });
