...
export default function AiDrivenStateMachinePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-driven-state-machine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-driven-state-machine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-driven-state-machine' },
      });
