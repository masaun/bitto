...
export default function ApprenticeshipProgramTrackerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('apprenticeship-program-tracker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'apprenticeship-program-tracker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'apprenticeship-program-tracker' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Apprenticeship Program Tracker</h1>
