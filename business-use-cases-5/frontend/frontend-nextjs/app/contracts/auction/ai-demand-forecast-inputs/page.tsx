...
export default function AiDemandForecastInputsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-demand-forecast-inputs');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-demand-forecast-inputs',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-demand-forecast-inputs' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Ai Demand Forecast Inputs</h1>
