...
export default function SpotMarketEquitySwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('spot-market-equity-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'spot-market-equity-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'spot-market-equity-swap' },
      });
