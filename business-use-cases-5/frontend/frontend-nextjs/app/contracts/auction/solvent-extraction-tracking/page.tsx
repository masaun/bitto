...
export default function SolventExtractionTrackingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('solvent-extraction-tracking');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'solvent-extraction-tracking',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'solvent-extraction-tracking' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Solvent Extraction Tracking</h1>
