...
export default function TransactionEncryptionManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('transaction-encryption-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'transaction-encryption-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'transaction-encryption-manager' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Transaction Encryption Manager</h1>
