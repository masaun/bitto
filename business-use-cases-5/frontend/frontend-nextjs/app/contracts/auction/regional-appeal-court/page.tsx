...
export default function RegionalAppealCourtPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('regional-appeal-court');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'regional-appeal-court',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'regional-appeal-court' },
      });
