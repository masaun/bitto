...
export default function AgentAnomalyDetectionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-anomaly-detection');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-anomaly-detection',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-anomaly-detection' },
      });
