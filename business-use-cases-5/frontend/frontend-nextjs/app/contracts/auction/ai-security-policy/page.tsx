...
export default function AiSecurityPolicyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-security-policy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-security-policy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-security-policy' },
      });
