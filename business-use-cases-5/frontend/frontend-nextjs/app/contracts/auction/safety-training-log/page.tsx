...
export default function SafetyTrainingLogPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('safety-training-log');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'safety-training-log',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'safety-training-log' },
      });
