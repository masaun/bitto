...
export default function AuctionAppealPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('auction-appeal');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'auction-appeal',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'auction-appeal' },
      });
