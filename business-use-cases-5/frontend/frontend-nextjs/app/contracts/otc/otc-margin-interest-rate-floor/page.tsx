...
export default function OtcMarginInterestRateFloorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-margin-interest-rate-floor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-margin-interest-rate-floor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-margin-interest-rate-floor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Margin Interest Rate Floor</h1>
