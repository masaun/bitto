...
export default function OtcMarginInterestRateCapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-margin-interest-rate-cap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-margin-interest-rate-cap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-margin-interest-rate-cap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Margin Interest Rate Cap</h1>
