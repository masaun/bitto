...
export default function OtcMatchingForwardRateAgreementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-matching-forward-rate-agreement');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-matching-forward-rate-agreement',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-matching-forward-rate-agreement' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Matching Forward Rate Agreement</h1>
