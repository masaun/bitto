...
export default function EnterpriseConfidentialSettlementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('enterprise-confidential-settlement');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'enterprise-confidential-settlement',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'enterprise-confidential-settlement' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Enterprise Confidential Settlement</h1>
