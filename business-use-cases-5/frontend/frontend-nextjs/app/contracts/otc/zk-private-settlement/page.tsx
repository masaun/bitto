...
export default function ZkPrivateSettlementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('zk-private-settlement');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'zk-private-settlement',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'zk-private-settlement' },
      });
