...
export default function OtcExecutionCommodityOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-execution-commodity-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-execution-commodity-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-execution-commodity-option' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Execution Commodity Option</h1>
