...
export default function OtcBrokerCreditOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-broker-credit-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-broker-credit-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-broker-credit-option' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Broker Credit Option</h1>
