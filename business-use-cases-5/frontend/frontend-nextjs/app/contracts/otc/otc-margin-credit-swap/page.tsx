...
export default function OtcMarginCreditSwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-margin-credit-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-margin-credit-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-margin-credit-swap' },
      });
