...
export default function OtcValuationInterestRateCapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-valuation-interest-rate-cap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-valuation-interest-rate-cap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-valuation-interest-rate-cap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Valuation Interest Rate Cap</h1>
