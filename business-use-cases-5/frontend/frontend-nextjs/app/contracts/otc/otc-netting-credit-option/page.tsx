...
export default function OtcNettingCreditOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-netting-credit-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-netting-credit-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-netting-credit-option' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Netting Credit Option</h1>
