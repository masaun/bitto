...
export default function OtcNettingCommodityOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-netting-commodity-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-netting-commodity-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-netting-commodity-option' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Netting Commodity Option</h1>
