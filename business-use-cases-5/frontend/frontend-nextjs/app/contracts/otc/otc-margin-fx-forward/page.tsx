...
export default function OtcMarginFxForwardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-margin-fx-forward');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-margin-fx-forward',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-margin-fx-forward' },
      });
