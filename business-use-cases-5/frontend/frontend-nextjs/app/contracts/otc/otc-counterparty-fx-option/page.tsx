...
export default function OtcCounterpartyFxOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-counterparty-fx-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-counterparty-fx-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-counterparty-fx-option' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Counterparty Fx Option</h1>
