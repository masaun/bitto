...
export default function OtcSettlementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-settlement');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-settlement',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-settlement' },
      });
