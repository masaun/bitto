...
export default function MessageBrokerBetaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('message-broker-beta');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'message-broker-beta',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'message-broker-beta' },
      });
