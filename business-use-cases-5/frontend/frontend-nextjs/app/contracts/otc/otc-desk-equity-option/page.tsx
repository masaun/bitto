...
export default function OtcDeskEquityOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-desk-equity-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-desk-equity-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-desk-equity-option' },
      });
