...
export default function OtcBrokerVarianceSwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-broker-variance-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-broker-variance-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-broker-variance-swap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Broker Variance Swap</h1>
