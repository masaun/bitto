...
export default function OtcDeskForwardRateAgreementPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-desk-forward-rate-agreement');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-desk-forward-rate-agreement',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-desk-forward-rate-agreement' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Desk Forward Rate Agreement</h1>
