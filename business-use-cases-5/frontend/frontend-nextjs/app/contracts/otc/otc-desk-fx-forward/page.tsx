...
export default function OtcDeskFxForwardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-desk-fx-forward');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-desk-fx-forward',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-desk-fx-forward' },
      });
