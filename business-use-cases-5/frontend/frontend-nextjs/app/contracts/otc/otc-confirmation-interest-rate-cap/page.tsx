...
export default function OtcConfirmationInterestRateCapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-confirmation-interest-rate-cap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-confirmation-interest-rate-cap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-confirmation-interest-rate-cap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Confirmation Interest Rate Cap</h1>
