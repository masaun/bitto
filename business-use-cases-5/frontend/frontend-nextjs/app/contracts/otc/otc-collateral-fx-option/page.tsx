...
export default function OtcCollateralFxOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-collateral-fx-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-collateral-fx-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-collateral-fx-option' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Collateral Fx Option</h1>
