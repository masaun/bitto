...
export default function OtcBrokerFxOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-broker-fx-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-broker-fx-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-broker-fx-option' },
      });
