...
export default function OtcPricingCreditSwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-pricing-credit-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-pricing-credit-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-pricing-credit-swap' },
      });
