...
export default function OtcBrokerCommoditySwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-broker-commodity-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-broker-commodity-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-broker-commodity-swap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Broker Commodity Swap</h1>
