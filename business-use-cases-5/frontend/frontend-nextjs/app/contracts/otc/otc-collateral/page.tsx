...
export default function OtcCollateralPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-collateral');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-collateral',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-collateral' },
      });
