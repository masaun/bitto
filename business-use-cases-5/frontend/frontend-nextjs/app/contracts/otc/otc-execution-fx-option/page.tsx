...
export default function OtcExecutionFxOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-execution-fx-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-execution-fx-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-execution-fx-option' },
      });
