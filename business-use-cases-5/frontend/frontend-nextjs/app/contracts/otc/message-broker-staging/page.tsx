...
export default function MessageBrokerStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('message-broker-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'message-broker-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'message-broker-staging' },
      });
