...
export default function OtcConfirmationPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-confirmation');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-confirmation',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-confirmation' },
      });
