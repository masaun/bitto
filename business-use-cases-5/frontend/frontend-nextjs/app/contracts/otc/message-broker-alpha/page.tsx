...
export default function MessageBrokerAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('message-broker-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'message-broker-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'message-broker-alpha' },
      });
