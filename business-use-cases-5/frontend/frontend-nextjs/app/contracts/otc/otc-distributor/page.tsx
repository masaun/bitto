...
export default function OtcDistributorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-distributor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-distributor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-distributor' },
      });
