...
export default function OtcNettingFxOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-netting-fx-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-netting-fx-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-netting-fx-option' },
      });
