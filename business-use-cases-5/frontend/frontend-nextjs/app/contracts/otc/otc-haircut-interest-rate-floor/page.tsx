...
export default function OtcHaircutInterestRateFloorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-haircut-interest-rate-floor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-haircut-interest-rate-floor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-haircut-interest-rate-floor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Haircut Interest Rate Floor</h1>
