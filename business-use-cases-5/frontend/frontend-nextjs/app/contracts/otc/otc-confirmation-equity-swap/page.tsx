...
export default function OtcConfirmationEquitySwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-confirmation-equity-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-confirmation-equity-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-confirmation-equity-swap' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Confirmation Equity Swap</h1>
