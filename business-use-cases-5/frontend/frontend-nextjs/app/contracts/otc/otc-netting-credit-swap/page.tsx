...
export default function OtcNettingCreditSwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-netting-credit-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-netting-credit-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-netting-credit-swap' },
      });
