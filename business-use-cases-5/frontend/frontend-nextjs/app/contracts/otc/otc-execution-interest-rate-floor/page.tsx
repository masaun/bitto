...
export default function OtcExecutionInterestRateFloorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-execution-interest-rate-floor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-execution-interest-rate-floor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-execution-interest-rate-floor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Execution Interest Rate Floor</h1>
