...
export default function MessageBrokerLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('message-broker-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'message-broker-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'message-broker-legacy' },
      });
