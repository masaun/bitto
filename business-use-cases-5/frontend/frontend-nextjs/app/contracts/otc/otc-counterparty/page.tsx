...
export default function OtcCounterpartyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-counterparty');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-counterparty',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-counterparty' },
      });
