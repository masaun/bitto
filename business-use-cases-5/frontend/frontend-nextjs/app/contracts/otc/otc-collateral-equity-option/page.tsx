...
export default function OtcCollateralEquityOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-collateral-equity-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-collateral-equity-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-collateral-equity-option' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Collateral Equity Option</h1>
