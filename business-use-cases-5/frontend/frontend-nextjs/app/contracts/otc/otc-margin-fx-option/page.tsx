...
export default function OtcMarginFxOptionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-margin-fx-option');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-margin-fx-option',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-margin-fx-option' },
      });
