...
export default function OtcExecutionFxForwardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-execution-fx-forward');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-execution-fx-forward',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-execution-fx-forward' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Otc Execution Fx Forward</h1>
