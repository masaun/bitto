...
export default function SportsTravelDaoPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sports-travel-dao');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sports-travel-dao',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sports-travel-dao' },
      });
