...
export default function ProjectVotingSystemPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('project-voting-system');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'project-voting-system',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'project-voting-system' },
      });
