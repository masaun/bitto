...
export default function TeamMerchandiseDaoPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-merchandise-dao');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-merchandise-dao',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-merchandise-dao' },
      });
