...
export default function AiGovernanceDaoPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-governance-dao');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-governance-dao',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-governance-dao' },
      });
