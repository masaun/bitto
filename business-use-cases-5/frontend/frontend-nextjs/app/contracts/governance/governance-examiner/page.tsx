...
export default function GovernanceExaminerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-examiner');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-examiner',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-examiner' },
      });
