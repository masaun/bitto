...
export default function IsoAiGovernancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('iso-ai-governance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'iso-ai-governance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'iso-ai-governance' },
      });
