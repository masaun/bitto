...
export default function GovernanceAggregatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-aggregator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-aggregator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-aggregator' },
      });
