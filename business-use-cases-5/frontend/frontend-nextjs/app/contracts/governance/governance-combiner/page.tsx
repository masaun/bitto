...
export default function GovernanceCombinerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-combiner');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-combiner',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-combiner' },
      });
