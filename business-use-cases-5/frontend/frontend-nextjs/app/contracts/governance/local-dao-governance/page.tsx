...
export default function LocalDaoGovernancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-dao-governance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-dao-governance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-dao-governance' },
      });
