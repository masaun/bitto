...
export default function LocalGovernanceTokenPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-governance-token');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-governance-token',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-governance-token' },
      });
