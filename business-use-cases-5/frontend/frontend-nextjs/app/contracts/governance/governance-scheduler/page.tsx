...
export default function GovernanceSchedulerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-scheduler');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-scheduler',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-scheduler' },
      });
