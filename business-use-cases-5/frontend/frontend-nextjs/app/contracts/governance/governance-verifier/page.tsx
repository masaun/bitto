...
export default function GovernanceVerifierPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-verifier');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-verifier',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-verifier' },
      });
