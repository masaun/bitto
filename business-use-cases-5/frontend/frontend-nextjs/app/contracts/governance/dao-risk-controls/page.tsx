...
export default function DaoRiskControlsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('dao-risk-controls');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'dao-risk-controls',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'dao-risk-controls' },
      });
