...
export default function VotingMechanismPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('voting-mechanism');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'voting-mechanism',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'voting-mechanism' },
      });
