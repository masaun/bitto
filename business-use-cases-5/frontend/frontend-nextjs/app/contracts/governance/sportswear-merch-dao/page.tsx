...
export default function SportswearMerchDaoPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sportswear-merch-dao');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sportswear-merch-dao',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sportswear-merch-dao' },
      });
