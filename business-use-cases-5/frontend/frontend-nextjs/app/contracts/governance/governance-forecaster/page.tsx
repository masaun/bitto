...
export default function GovernanceForecasterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-forecaster');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-forecaster',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-forecaster' },
      });
