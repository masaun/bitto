...
export default function DepartmentalQuadraticVotingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('departmental-quadratic-voting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'departmental-quadratic-voting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'departmental-quadratic-voting' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Departmental Quadratic Voting</h1>
