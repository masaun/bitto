...
export default function GovernanceProxyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-proxy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-proxy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-proxy' },
      });
