...
export default function BridgeGovernancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('bridge-governance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'bridge-governance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'bridge-governance' },
      });
