...
export default function GovernanceEstimatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-estimator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-estimator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-estimator' },
      });
