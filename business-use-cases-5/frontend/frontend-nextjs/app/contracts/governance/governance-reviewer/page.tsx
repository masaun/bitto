...
export default function GovernanceReviewerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-reviewer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-reviewer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-reviewer' },
      });
