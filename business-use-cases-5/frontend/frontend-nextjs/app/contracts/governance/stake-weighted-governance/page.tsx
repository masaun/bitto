...
export default function StakeWeightedGovernancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('stake-weighted-governance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'stake-weighted-governance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'stake-weighted-governance' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Stake Weighted Governance</h1>
