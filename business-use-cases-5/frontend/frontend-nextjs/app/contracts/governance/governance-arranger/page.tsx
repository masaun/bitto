...
export default function GovernanceArrangerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-arranger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-arranger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-arranger' },
      });
