...
export default function OperationalGovernanceTokenPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('operational-governance-token');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'operational-governance-token',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'operational-governance-token' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Operational Governance Token</h1>
