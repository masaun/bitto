...
export default function TeamDaoGovernancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-dao-governance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-dao-governance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-dao-governance' },
      });
