...
export default function FanVotingGovernancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('fan-voting-governance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'fan-voting-governance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'fan-voting-governance' },
      });
