...
export default function AiGovernanceAuditorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-governance-auditor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-governance-auditor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-governance-auditor' },
      });
