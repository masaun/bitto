...
export default function RollupGovernancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rollup-governance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rollup-governance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rollup-governance' },
      });
