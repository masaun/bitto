...
export default function TeamDelegatedVotingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-delegated-voting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-delegated-voting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-delegated-voting' },
      });
