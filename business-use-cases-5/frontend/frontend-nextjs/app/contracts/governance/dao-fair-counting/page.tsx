...
export default function DaoFairCountingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('dao-fair-counting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'dao-fair-counting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'dao-fair-counting' },
      });
