...
export default function ProjectGovernanceTokenPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('project-governance-token');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'project-governance-token',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'project-governance-token' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Project Governance Token</h1>
