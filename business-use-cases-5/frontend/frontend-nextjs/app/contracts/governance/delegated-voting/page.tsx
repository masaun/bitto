...
export default function DelegatedVotingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('delegated-voting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'delegated-voting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'delegated-voting' },
      });
