...
export default function LocalQuadraticVotingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-quadratic-voting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-quadratic-voting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-quadratic-voting' },
      });
