...
export default function GovernanceAllocatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-allocator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-allocator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-allocator' },
      });
