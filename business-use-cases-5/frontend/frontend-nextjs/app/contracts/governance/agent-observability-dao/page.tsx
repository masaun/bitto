...
export default function AgentObservabilityDaoPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-observability-dao');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-observability-dao',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-observability-dao' },
      });
