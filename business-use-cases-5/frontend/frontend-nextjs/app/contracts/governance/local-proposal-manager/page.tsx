...
export default function LocalProposalManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-proposal-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-proposal-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-proposal-manager' },
      });
