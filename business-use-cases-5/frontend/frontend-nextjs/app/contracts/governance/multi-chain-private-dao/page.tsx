...
export default function MultiChainPrivateDaoPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('multi-chain-private-dao');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'multi-chain-private-dao',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'multi-chain-private-dao' },
      });
