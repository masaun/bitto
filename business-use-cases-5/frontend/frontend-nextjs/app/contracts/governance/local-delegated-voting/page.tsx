...
export default function LocalDelegatedVotingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-delegated-voting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-delegated-voting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-delegated-voting' },
      });
