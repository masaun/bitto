...
export default function GovernanceCoordinatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-coordinator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-coordinator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-coordinator' },
      });
