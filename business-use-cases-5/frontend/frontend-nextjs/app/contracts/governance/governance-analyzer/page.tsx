...
export default function GovernanceAnalyzerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-analyzer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-analyzer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-analyzer' },
      });
