...
export default function TacticalVotingSystemPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('tactical-voting-system');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'tactical-voting-system',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'tactical-voting-system' },
      });
