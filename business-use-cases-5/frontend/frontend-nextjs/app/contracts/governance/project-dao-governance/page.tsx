...
export default function ProjectDaoGovernancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('project-dao-governance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'project-dao-governance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'project-dao-governance' },
      });
