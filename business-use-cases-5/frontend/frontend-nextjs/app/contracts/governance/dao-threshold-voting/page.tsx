...
export default function DaoThresholdVotingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('dao-threshold-voting');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'dao-threshold-voting',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'dao-threshold-voting' },
      });
