...
export default function GovernanceQuorumPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-quorum');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-quorum',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-quorum' },
      });
