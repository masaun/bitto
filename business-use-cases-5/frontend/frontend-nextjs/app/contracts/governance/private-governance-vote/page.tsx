...
export default function PrivateGovernanceVotePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('private-governance-vote');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'private-governance-vote',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'private-governance-vote' },
      });
