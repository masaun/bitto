...
export default function GovernancePlannerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-planner');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-planner',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-planner' },
      });
