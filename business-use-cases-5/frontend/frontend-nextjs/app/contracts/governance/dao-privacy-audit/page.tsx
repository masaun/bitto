...
export default function DaoPrivacyAuditPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('dao-privacy-audit');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'dao-privacy-audit',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'dao-privacy-audit' },
      });
