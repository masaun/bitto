...
export default function GovernanceInspectorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('governance-inspector');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'governance-inspector',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'governance-inspector' },
      });
