...
export default function AgentMemoryAuditPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('agent-memory-audit');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'agent-memory-audit',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'agent-memory-audit' },
      });
