...
export default function KycCheckerSecPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('kyc-checker-sec');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'kyc-checker-sec',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'kyc-checker-sec' },
      });
