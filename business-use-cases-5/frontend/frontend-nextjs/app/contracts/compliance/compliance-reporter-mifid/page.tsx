...
export default function ComplianceReporterMifidPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-reporter-mifid');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-reporter-mifid',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-reporter-mifid' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Compliance Reporter Mifid</h1>
