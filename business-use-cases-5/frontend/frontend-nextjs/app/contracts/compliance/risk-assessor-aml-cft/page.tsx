...
export default function RiskAssessorAmlCftPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('risk-assessor-aml-cft');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'risk-assessor-aml-cft',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'risk-assessor-aml-cft' },
      });
