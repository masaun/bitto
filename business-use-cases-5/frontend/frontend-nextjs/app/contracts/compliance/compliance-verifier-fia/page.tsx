...
export default function ComplianceVerifierFiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-verifier-fia');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-verifier-fia',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-verifier-fia' },
      });
