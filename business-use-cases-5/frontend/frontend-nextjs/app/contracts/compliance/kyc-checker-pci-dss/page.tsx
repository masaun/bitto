...
export default function KycCheckerPciDssPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('kyc-checker-pci-dss');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'kyc-checker-pci-dss',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'kyc-checker-pci-dss' },
      });
