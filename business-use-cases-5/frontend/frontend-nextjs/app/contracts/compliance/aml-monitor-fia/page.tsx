...
export default function AmlMonitorFiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('aml-monitor-fia');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'aml-monitor-fia',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'aml-monitor-fia' },
      });
