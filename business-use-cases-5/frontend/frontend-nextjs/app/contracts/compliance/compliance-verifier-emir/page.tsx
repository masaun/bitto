...
export default function ComplianceVerifierEmirPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-verifier-emir');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-verifier-emir',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-verifier-emir' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Compliance Verifier Emir</h1>
