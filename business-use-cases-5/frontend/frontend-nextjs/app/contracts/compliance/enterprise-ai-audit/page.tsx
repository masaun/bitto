...
export default function EnterpriseAiAuditPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('enterprise-ai-audit');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'enterprise-ai-audit',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'enterprise-ai-audit' },
      });
