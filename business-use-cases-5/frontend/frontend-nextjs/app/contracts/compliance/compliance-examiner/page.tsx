...
export default function ComplianceExaminerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-examiner');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-examiner',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-examiner' },
      });
