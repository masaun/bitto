...
export default function ComplianceReporterUcitsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-reporter-ucits');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-reporter-ucits',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-reporter-ucits' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Compliance Reporter Ucits</h1>
