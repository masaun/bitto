...
export default function QualityAssuranceAuditPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('quality-assurance-audit');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'quality-assurance-audit',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'quality-assurance-audit' },
      });
