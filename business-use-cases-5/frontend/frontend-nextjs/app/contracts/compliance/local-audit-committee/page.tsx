...
export default function LocalAuditCommitteePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-audit-committee');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-audit-committee',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-audit-committee' },
      });
