...
export default function ComplianceCombinerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-combiner');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-combiner',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-combiner' },
      });
