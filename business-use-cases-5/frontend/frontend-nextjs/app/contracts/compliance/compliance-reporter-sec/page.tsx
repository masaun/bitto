...
export default function ComplianceReporterSecPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-reporter-sec');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-reporter-sec',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-reporter-sec' },
      });
