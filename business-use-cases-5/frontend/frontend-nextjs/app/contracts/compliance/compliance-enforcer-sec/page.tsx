...
export default function ComplianceEnforcerSecPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-enforcer-sec');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-enforcer-sec',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-enforcer-sec' },
      });
