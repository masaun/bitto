...
export default function OrderingAuditPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ordering-audit');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ordering-audit',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ordering-audit' },
      });
