...
export default function KycCheckerFatcaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('kyc-checker-fatca');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'kyc-checker-fatca',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'kyc-checker-fatca' },
      });
