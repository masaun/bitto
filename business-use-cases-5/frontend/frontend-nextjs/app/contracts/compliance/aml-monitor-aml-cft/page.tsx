...
export default function AmlMonitorAmlCftPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('aml-monitor-aml-cft');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'aml-monitor-aml-cft',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'aml-monitor-aml-cft' },
      });
