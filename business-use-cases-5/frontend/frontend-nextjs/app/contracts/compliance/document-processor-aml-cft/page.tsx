...
export default function DocumentProcessorAmlCftPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('document-processor-aml-cft');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'document-processor-aml-cft',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'document-processor-aml-cft' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Document Processor Aml Cft</h1>
