...
export default function PostMergerAuditPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('post-merger-audit');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'post-merger-audit',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'post-merger-audit' },
      });
