...
export default function ComplianceProofExportPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-proof-export');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-proof-export',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-proof-export' },
      });
