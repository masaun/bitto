...
export default function WorkerSafetyCompliancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('worker-safety-compliance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'worker-safety-compliance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'worker-safety-compliance' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Worker Safety Compliance</h1>
