...
export default function ProjectAuditLoggerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('project-audit-logger');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'project-audit-logger',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'project-audit-logger' },
      });
