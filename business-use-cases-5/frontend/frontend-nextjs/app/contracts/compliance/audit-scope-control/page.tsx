...
export default function AuditScopeControlPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('audit-scope-control');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'audit-scope-control',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'audit-scope-control' },
      });
