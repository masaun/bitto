...
export default function RentalRevenueAuditorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rental-revenue-auditor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rental-revenue-auditor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rental-revenue-auditor' },
      });
