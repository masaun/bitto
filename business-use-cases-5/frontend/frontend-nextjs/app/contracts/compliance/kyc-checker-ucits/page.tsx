...
export default function KycCheckerUcitsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('kyc-checker-ucits');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'kyc-checker-ucits',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'kyc-checker-ucits' },
      });
