...
export default function IsoStandardsCompliancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('iso-standards-compliance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'iso-standards-compliance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'iso-standards-compliance' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Iso Standards Compliance</h1>
