...
export default function AuditLoggerEmirPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('audit-logger-emir');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'audit-logger-emir',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'audit-logger-emir' },
      });
