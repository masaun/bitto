...
export default function ComplianceEnforcerMifidPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-enforcer-mifid');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-enforcer-mifid',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-enforcer-mifid' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Compliance Enforcer Mifid</h1>
