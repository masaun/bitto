...
export default function TeamComplianceCheckerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-compliance-checker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-compliance-checker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-compliance-checker' },
      });
