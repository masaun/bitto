...
export default function ComplianceAllocatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-allocator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-allocator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-allocator' },
      });
