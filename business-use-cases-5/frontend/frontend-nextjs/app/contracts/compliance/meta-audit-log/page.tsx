...
export default function MetaAuditLogPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('meta-audit-log');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'meta-audit-log',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'meta-audit-log' },
      });
