...
export default function AuditScorecardPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('audit-scorecard');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'audit-scorecard',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'audit-scorecard' },
      });
