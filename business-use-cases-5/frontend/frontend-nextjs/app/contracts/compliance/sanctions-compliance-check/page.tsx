...
export default function SanctionsComplianceCheckPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('sanctions-compliance-check');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'sanctions-compliance-check',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'sanctions-compliance-check' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Sanctions Compliance Check</h1>
