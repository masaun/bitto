...
export default function KycCheckerAmlCftPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('kyc-checker-aml-cft');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'kyc-checker-aml-cft',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'kyc-checker-aml-cft' },
      });
