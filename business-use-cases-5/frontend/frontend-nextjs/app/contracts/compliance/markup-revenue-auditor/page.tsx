...
export default function MarkupRevenueAuditorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('markup-revenue-auditor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'markup-revenue-auditor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'markup-revenue-auditor' },
      });
