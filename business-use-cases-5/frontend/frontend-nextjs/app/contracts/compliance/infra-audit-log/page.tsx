...
export default function InfraAuditLogPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('infra-audit-log');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'infra-audit-log',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'infra-audit-log' },
      });
