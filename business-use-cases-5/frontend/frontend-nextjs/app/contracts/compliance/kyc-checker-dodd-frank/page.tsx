...
export default function KycCheckerDoddFrankPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('kyc-checker-dodd-frank');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'kyc-checker-dodd-frank',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'kyc-checker-dodd-frank' },
      });
