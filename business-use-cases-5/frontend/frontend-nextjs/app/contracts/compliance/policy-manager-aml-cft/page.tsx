...
export default function PolicyManagerAmlCftPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('policy-manager-aml-cft');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'policy-manager-aml-cft',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'policy-manager-aml-cft' },
      });
