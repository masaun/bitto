...
export default function InterestRevenueAuditorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('interest-revenue-auditor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'interest-revenue-auditor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'interest-revenue-auditor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Interest Revenue Auditor</h1>
