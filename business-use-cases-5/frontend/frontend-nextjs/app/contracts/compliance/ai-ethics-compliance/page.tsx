...
export default function AiEthicsCompliancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-ethics-compliance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-ethics-compliance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-ethics-compliance' },
      });
