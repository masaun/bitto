...
export default function AuditEvidenceStorePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('audit-evidence-store');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'audit-evidence-store',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'audit-evidence-store' },
      });
