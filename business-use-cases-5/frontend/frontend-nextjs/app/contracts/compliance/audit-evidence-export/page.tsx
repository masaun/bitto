...
export default function AuditEvidenceExportPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('audit-evidence-export');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'audit-evidence-export',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'audit-evidence-export' },
      });
