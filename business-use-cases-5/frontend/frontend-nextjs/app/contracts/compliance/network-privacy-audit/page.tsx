...
export default function NetworkPrivacyAuditPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('network-privacy-audit');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'network-privacy-audit',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'network-privacy-audit' },
      });
