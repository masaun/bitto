...
export default function ComplianceVerifierRiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-verifier-ria');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-verifier-ria',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-verifier-ria' },
      });
