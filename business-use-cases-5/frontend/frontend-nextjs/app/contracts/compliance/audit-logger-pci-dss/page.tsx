...
export default function AuditLoggerPciDssPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('audit-logger-pci-dss');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'audit-logger-pci-dss',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'audit-logger-pci-dss' },
      });
