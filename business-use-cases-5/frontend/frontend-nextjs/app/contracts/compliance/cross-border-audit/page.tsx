...
export default function CrossBorderAuditPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('cross-border-audit');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'cross-border-audit',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'cross-border-audit' },
      });
