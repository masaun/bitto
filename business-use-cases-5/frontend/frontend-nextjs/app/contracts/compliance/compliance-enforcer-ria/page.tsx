...
export default function ComplianceEnforcerRiaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-enforcer-ria');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-enforcer-ria',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-enforcer-ria' },
      });
