...
export default function AmlMonitorCcpaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('aml-monitor-ccpa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'aml-monitor-ccpa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'aml-monitor-ccpa' },
      });
