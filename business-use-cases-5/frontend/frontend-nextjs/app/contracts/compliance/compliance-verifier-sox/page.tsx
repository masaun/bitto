...
export default function ComplianceVerifierSoxPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-verifier-sox');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-verifier-sox',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-verifier-sox' },
      });
