...
export default function MevAuditTrailPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('mev-audit-trail');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'mev-audit-trail',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'mev-audit-trail' },
      });
