...
export default function LocalComplianceCheckerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('local-compliance-checker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'local-compliance-checker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'local-compliance-checker' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Local Compliance Checker</h1>
