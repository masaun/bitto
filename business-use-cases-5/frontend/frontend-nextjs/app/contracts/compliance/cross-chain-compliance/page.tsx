...
export default function CrossChainCompliancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('cross-chain-compliance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'cross-chain-compliance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'cross-chain-compliance' },
      });
