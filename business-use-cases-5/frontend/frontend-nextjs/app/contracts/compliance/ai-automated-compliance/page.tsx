...
export default function AiAutomatedCompliancePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-automated-compliance');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-automated-compliance',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-automated-compliance' },
      });
