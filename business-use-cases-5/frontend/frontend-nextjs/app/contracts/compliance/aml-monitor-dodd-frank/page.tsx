...
export default function AmlMonitorDoddFrankPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('aml-monitor-dodd-frank');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'aml-monitor-dodd-frank',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'aml-monitor-dodd-frank' },
      });
