...
export default function KycCheckerCftcPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('kyc-checker-cftc');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'kyc-checker-cftc',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'kyc-checker-cftc' },
      });
