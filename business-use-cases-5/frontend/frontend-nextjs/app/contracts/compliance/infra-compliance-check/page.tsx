...
export default function InfraComplianceCheckPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('infra-compliance-check');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'infra-compliance-check',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'infra-compliance-check' },
      });
