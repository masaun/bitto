...
export default function KycCheckerCcpaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('kyc-checker-ccpa');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'kyc-checker-ccpa',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'kyc-checker-ccpa' },
      });
