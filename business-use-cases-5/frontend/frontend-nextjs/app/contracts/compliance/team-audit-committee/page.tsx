...
export default function TeamAuditCommitteePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('team-audit-committee');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'team-audit-committee',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'team-audit-committee' },
      });
