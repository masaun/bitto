...
export default function ComplianceProjectorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('compliance-projector');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'compliance-projector',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'compliance-projector' },
      });
