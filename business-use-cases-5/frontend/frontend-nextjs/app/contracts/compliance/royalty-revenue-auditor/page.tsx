...
export default function RoyaltyRevenueAuditorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('royalty-revenue-auditor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'royalty-revenue-auditor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'royalty-revenue-auditor' },
      });
