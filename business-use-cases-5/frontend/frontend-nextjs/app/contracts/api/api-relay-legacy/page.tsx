...
export default function ApiRelayLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-relay-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-relay-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-relay-legacy' },
      });
