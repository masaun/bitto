...
export default function GrpcGatewayAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('grpc-gateway-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'grpc-gateway-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'grpc-gateway-alpha' },
      });
