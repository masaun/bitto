...
export default function ApiAdapterProductionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-adapter-production');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-adapter-production',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-adapter-production' },
      });
