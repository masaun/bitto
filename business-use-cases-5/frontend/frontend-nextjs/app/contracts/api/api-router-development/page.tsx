...
export default function ApiRouterDevelopmentPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-router-development');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-router-development',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-router-development' },
      });
