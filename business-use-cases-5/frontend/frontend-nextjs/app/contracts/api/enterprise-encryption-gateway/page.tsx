...
export default function EnterpriseEncryptionGatewayPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('enterprise-encryption-gateway');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'enterprise-encryption-gateway',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'enterprise-encryption-gateway' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Enterprise Encryption Gateway</h1>
