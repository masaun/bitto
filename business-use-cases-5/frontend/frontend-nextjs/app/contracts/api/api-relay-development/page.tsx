...
export default function ApiRelayDevelopmentPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-relay-development');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-relay-development',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-relay-development' },
      });
