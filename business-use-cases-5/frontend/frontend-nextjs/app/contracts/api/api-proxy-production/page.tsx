...
export default function ApiProxyProductionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-proxy-production');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-proxy-production',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-proxy-production' },
      });
