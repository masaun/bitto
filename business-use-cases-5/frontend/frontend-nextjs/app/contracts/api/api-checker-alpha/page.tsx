...
export default function ApiCheckerAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-checker-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-checker-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-checker-alpha' },
      });
