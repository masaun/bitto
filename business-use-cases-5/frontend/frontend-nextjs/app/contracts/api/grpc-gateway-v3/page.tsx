...
export default function GrpcGatewayV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('grpc-gateway-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'grpc-gateway-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'grpc-gateway-v3' },
      });
