...
export default function ApiAggregatorLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-aggregator-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-aggregator-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-aggregator-legacy' },
      });
