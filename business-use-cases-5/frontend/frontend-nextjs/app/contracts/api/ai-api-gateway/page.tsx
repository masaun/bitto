...
export default function AiApiGatewayPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-api-gateway');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-api-gateway',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-api-gateway' },
      });
