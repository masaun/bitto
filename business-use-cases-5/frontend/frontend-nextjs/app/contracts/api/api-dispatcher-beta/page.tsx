...
export default function ApiDispatcherBetaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-dispatcher-beta');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-dispatcher-beta',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-dispatcher-beta' },
      });
