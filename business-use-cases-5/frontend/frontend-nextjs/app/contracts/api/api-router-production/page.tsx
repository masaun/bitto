...
export default function ApiRouterProductionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-router-production');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-router-production',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-router-production' },
      });
