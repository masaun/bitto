...
export default function ApiEncoderV2Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-encoder-v2');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-encoder-v2',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-encoder-v2' },
      });
