...
export default function ApiCheckerV2Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-checker-v2');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-checker-v2',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-checker-v2' },
      });
