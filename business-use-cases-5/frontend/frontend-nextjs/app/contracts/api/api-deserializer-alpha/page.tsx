...
export default function ApiDeserializerAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-deserializer-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-deserializer-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-deserializer-alpha' },
      });
