...
export default function ApiAuditorAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-auditor-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-auditor-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-auditor-alpha' },
      });
