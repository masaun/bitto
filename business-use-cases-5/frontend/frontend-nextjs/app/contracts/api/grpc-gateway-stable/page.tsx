...
export default function GrpcGatewayStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('grpc-gateway-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'grpc-gateway-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'grpc-gateway-stable' },
      });
