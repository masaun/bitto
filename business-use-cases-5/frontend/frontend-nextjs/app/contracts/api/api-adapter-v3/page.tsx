...
export default function ApiAdapterV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-adapter-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-adapter-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-adapter-v3' },
      });
