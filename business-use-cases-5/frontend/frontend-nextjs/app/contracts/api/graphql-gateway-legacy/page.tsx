...
export default function GraphqlGatewayLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('graphql-gateway-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'graphql-gateway-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'graphql-gateway-legacy' },
      });
