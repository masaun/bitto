...
export default function GraphqlGatewayAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('graphql-gateway-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'graphql-gateway-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'graphql-gateway-alpha' },
      });
