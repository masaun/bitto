...
export default function WebsocketGatewayV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('websocket-gateway-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'websocket-gateway-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'websocket-gateway-v3' },
      });
