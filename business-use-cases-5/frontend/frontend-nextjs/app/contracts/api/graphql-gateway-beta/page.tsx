...
export default function GraphqlGatewayBetaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('graphql-gateway-beta');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'graphql-gateway-beta',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'graphql-gateway-beta' },
      });
