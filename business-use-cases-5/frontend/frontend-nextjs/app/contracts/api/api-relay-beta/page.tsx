...
export default function ApiRelayBetaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-relay-beta');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-relay-beta',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-relay-beta' },
      });
