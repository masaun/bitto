...
export default function RestGatewayLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rest-gateway-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rest-gateway-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rest-gateway-legacy' },
      });
