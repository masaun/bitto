...
export default function ApiSynthesizerStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-synthesizer-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-synthesizer-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-synthesizer-staging' },
      });
