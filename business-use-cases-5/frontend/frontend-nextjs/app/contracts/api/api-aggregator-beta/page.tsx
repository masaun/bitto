...
export default function ApiAggregatorBetaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-aggregator-beta');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-aggregator-beta',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-aggregator-beta' },
      });
