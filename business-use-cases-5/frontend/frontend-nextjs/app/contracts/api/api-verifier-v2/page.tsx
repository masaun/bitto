...
export default function ApiVerifierV2Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-verifier-v2');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-verifier-v2',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-verifier-v2' },
      });
