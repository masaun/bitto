...
export default function GrpcGatewayStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('grpc-gateway-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'grpc-gateway-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'grpc-gateway-staging' },
      });
