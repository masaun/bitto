...
export default function ApiRelayAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-relay-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-relay-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-relay-alpha' },
      });
