...
export default function ApiAggregatorStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-aggregator-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-aggregator-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-aggregator-stable' },
      });
