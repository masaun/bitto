...
export default function ApiGatewayDevelopmentPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-gateway-development');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-gateway-development',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-gateway-development' },
      });
