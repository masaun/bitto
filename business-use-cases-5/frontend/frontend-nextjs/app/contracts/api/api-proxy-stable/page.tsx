...
export default function ApiProxyStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-proxy-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-proxy-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-proxy-stable' },
      });
