...
export default function ApiAuditorV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-auditor-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-auditor-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-auditor-v3' },
      });
