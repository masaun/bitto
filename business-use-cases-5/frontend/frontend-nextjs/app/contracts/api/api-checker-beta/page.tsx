...
export default function ApiCheckerBetaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-checker-beta');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-checker-beta',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-checker-beta' },
      });
