...
export default function ApiSynthesizerLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-synthesizer-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-synthesizer-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-synthesizer-legacy' },
      });
