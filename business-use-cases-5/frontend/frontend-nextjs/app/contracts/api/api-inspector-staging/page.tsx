...
export default function ApiInspectorStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-inspector-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-inspector-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-inspector-staging' },
      });
