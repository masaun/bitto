...
export default function ApiVerifierAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-verifier-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-verifier-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-verifier-alpha' },
      });
