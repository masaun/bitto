...
export default function WebsocketGatewayBetaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('websocket-gateway-beta');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'websocket-gateway-beta',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'websocket-gateway-beta' },
      });
