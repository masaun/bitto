...
export default function ApiProxyStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-proxy-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-proxy-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-proxy-staging' },
      });
