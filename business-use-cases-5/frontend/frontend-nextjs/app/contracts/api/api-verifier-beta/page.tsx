...
export default function ApiVerifierBetaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-verifier-beta');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-verifier-beta',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-verifier-beta' },
      });
