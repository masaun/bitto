...
export default function ApiRelayProductionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-relay-production');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-relay-production',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-relay-production' },
      });
