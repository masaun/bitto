...
export default function ApiBridgeLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-bridge-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-bridge-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-bridge-legacy' },
      });
