...
export default function ApiProxyExperimentalPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-proxy-experimental');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-proxy-experimental',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-proxy-experimental' },
      });
