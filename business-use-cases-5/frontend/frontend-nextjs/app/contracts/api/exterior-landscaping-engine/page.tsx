...
export default function ExteriorLandscapingEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('exterior-landscaping-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'exterior-landscaping-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'exterior-landscaping-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Exterior Landscaping Engine</h1>
