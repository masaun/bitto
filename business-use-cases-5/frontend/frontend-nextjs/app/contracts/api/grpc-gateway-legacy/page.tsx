...
export default function GrpcGatewayLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('grpc-gateway-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'grpc-gateway-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'grpc-gateway-legacy' },
      });
