...
export default function ApiDispatcherStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-dispatcher-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-dispatcher-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-dispatcher-stable' },
      });
