...
export default function ApiAuditorV2Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-auditor-v2');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-auditor-v2',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-auditor-v2' },
      });
