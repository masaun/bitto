...
export default function GraphqlGatewayV2Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('graphql-gateway-v2');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'graphql-gateway-v2',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'graphql-gateway-v2' },
      });
