...
export default function RestGatewayV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rest-gateway-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rest-gateway-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rest-gateway-v3' },
      });
