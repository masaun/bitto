...
export default function WebsocketGatewayAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('websocket-gateway-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'websocket-gateway-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'websocket-gateway-alpha' },
      });
