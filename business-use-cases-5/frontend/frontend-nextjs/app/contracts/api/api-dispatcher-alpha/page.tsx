...
export default function ApiDispatcherAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-dispatcher-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-dispatcher-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-dispatcher-alpha' },
      });
