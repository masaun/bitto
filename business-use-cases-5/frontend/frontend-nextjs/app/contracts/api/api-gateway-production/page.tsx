...
export default function ApiGatewayProductionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-gateway-production');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-gateway-production',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-gateway-production' },
      });
