...
export default function ApiCheckerV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-checker-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-checker-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-checker-v3' },
      });
