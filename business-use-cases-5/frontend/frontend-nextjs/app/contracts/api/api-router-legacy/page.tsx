...
export default function ApiRouterLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-router-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-router-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-router-legacy' },
      });
