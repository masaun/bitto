...
export default function ApiCheckerLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-checker-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-checker-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-checker-legacy' },
      });
