...
export default function ApiValidatorStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-validator-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-validator-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-validator-staging' },
      });
