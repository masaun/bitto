...
export default function ApiAccumulatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-accumulator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-accumulator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-accumulator' },
      });
