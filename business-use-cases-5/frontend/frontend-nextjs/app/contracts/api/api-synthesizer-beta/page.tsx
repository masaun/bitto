...
export default function ApiSynthesizerBetaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-synthesizer-beta');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-synthesizer-beta',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-synthesizer-beta' },
      });
