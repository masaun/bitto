...
export default function ApiAuditorLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-auditor-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-auditor-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-auditor-legacy' },
      });
