...
export default function ApiValidatorAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-validator-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-validator-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-validator-alpha' },
      });
