...
export default function GrpcGatewayProductionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('grpc-gateway-production');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'grpc-gateway-production',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'grpc-gateway-production' },
      });
