...
export default function ApiDecoderProductionPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-decoder-production');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-decoder-production',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-decoder-production' },
      });
