...
export default function ConfidentialApiLayerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('confidential-api-layer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'confidential-api-layer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'confidential-api-layer' },
      });
