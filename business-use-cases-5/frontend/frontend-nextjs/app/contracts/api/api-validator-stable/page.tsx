...
export default function ApiValidatorStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-validator-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-validator-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-validator-stable' },
      });
