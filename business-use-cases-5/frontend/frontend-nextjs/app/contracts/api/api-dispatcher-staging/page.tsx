...
export default function ApiDispatcherStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-dispatcher-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-dispatcher-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-dispatcher-staging' },
      });
