...
export default function ApiProxyBetaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-proxy-beta');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-proxy-beta',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-proxy-beta' },
      });
