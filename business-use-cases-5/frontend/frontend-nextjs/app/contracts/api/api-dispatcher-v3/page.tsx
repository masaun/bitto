...
export default function ApiDispatcherV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-dispatcher-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-dispatcher-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-dispatcher-v3' },
      });
