...
export default function ApiRelayStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-relay-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-relay-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-relay-stable' },
      });
