...
export default function ApiDeserializerV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-deserializer-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-deserializer-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-deserializer-v3' },
      });
