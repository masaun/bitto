...
export default function ApiRouterStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-router-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-router-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-router-stable' },
      });
