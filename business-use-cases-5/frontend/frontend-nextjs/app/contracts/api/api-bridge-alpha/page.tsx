...
export default function ApiBridgeAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-bridge-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-bridge-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-bridge-alpha' },
      });
