...
export default function ApiRouterBetaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-router-beta');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-router-beta',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-router-beta' },
      });
