...
export default function ApiDecoderLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-decoder-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-decoder-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-decoder-legacy' },
      });
