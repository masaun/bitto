...
export default function OtcAggregatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('otc-aggregator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'otc-aggregator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'otc-aggregator' },
      });
