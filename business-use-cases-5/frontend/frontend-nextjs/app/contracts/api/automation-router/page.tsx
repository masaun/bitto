...
export default function AutomationRouterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-router');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-router',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-router' },
      });
