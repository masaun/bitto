...
export default function ApiTransformerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-transformer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-transformer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-transformer' },
      });
