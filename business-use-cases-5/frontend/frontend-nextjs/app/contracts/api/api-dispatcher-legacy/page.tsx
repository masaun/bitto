...
export default function ApiDispatcherLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-dispatcher-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-dispatcher-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-dispatcher-legacy' },
      });
