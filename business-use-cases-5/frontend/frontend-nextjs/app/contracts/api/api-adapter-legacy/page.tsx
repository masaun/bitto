...
export default function ApiAdapterLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-adapter-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-adapter-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-adapter-legacy' },
      });
