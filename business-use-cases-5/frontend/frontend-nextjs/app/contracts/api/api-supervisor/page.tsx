...
export default function ApiSupervisorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-supervisor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-supervisor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-supervisor' },
      });
