...
export default function ApiProxyLegacyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-proxy-legacy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-proxy-legacy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-proxy-legacy' },
      });
