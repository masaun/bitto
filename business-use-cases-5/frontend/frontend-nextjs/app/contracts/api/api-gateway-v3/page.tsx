...
export default function ApiGatewayV3Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-gateway-v3');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-gateway-v3',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-gateway-v3' },
      });
