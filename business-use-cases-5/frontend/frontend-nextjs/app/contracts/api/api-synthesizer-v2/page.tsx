...
export default function ApiSynthesizerV2Page() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-synthesizer-v2');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-synthesizer-v2',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-synthesizer-v2' },
      });
