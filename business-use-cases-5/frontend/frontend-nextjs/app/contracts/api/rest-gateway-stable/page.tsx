...
export default function RestGatewayStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rest-gateway-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rest-gateway-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rest-gateway-stable' },
      });
