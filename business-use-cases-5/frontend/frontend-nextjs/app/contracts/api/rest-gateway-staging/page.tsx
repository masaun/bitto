...
export default function RestGatewayStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('rest-gateway-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'rest-gateway-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'rest-gateway-staging' },
      });
