...
export default function BlockchainApiAdapterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('blockchain-api-adapter');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'blockchain-api-adapter',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'blockchain-api-adapter' },
      });
