...
export default function ApiAdapterAlphaPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-adapter-alpha');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-adapter-alpha',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-adapter-alpha' },
      });
