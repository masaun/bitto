...
export default function ApiEncoderStablePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-encoder-stable');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-encoder-stable',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-encoder-stable' },
      });
