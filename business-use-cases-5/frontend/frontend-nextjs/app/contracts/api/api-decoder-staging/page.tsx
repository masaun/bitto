...
export default function ApiDecoderStagingPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('api-decoder-staging');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'api-decoder-staging',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'api-decoder-staging' },
      });
