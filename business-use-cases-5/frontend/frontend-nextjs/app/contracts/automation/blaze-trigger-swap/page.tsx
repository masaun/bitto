...
export default function BlazeTriggerSwapPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('blaze-trigger-swap');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'blaze-trigger-swap',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'blaze-trigger-swap' },
      });
