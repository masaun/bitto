...
export default function MlPoweredJobSchedulerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ml-powered-job-scheduler');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ml-powered-job-scheduler',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ml-powered-job-scheduler' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Ml Powered Job Scheduler</h1>
