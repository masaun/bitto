...
export default function DataDrivenJobSchedulerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('data-driven-job-scheduler');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'data-driven-job-scheduler',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'data-driven-job-scheduler' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Data Driven Job Scheduler</h1>
