...
export default function UserTriggeredPipelineOrchestratorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('user-triggered-pipeline-orchestrator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'user-triggered-pipeline-orchestrator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'user-triggered-pipeline-orchestrator' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>User Triggered Pipeline Orchestrator</h1>
