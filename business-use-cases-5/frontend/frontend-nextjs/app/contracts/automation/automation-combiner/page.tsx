...
export default function AutomationCombinerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-combiner');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-combiner',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-combiner' },
      });
