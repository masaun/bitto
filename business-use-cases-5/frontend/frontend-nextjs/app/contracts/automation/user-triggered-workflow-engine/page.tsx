...
export default function UserTriggeredWorkflowEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('user-triggered-workflow-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'user-triggered-workflow-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'user-triggered-workflow-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>User Triggered Workflow Engine</h1>
