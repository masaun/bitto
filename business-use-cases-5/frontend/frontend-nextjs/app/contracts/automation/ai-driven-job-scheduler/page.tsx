...
export default function AiDrivenJobSchedulerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('ai-driven-job-scheduler');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'ai-driven-job-scheduler',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'ai-driven-job-scheduler' },
      });
