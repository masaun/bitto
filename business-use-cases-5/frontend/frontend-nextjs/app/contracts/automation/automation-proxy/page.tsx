...
export default function AutomationProxyPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-proxy');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-proxy',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-proxy' },
      });
