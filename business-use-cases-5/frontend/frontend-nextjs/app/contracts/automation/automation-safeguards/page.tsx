...
export default function AutomationSafeguardsPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-safeguards');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-safeguards',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-safeguards' },
      });
