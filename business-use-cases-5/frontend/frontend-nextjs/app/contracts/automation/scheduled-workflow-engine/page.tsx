...
export default function ScheduledWorkflowEnginePage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('scheduled-workflow-engine');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'scheduled-workflow-engine',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'scheduled-workflow-engine' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Scheduled Workflow Engine</h1>
