...
export default function AutomationForecasterPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-forecaster');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-forecaster',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-forecaster' },
      });
