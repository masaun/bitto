...
export default function UserTriggeredBatchProcessorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('user-triggered-batch-processor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'user-triggered-batch-processor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'user-triggered-batch-processor' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>User Triggered Batch Processor</h1>
