...
export default function AutomationOrganizerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-organizer');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-organizer',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-organizer' },
      });
