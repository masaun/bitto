...
export default function AutomationCoordinatorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-coordinator');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-coordinator',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-coordinator' },
      });
