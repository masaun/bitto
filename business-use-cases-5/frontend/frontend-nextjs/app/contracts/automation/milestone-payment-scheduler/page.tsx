...
export default function MilestonePaymentSchedulerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('milestone-payment-scheduler');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'milestone-payment-scheduler',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'milestone-payment-scheduler' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>Milestone Payment Scheduler</h1>
