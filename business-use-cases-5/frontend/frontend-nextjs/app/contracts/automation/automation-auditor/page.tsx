...
export default function AutomationAuditorPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-auditor');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-auditor',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-auditor' },
      });
