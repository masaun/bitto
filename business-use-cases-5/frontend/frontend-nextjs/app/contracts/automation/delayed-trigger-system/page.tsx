...
export default function DelayedTriggerSystemPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('delayed-trigger-system');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'delayed-trigger-system',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'delayed-trigger-system' },
      });
