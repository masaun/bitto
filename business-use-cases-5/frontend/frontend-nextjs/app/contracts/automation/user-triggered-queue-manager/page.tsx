...
export default function UserTriggeredQueueManagerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('user-triggered-queue-manager');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'user-triggered-queue-manager',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'user-triggered-queue-manager' },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1>User Triggered Queue Manager</h1>
