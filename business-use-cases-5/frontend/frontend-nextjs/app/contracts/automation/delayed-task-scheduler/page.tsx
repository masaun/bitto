...
export default function DelayedTaskSchedulerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('delayed-task-scheduler');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'delayed-task-scheduler',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'delayed-task-scheduler' },
      });
