...
export default function TriggerSystemPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('trigger-system');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'trigger-system',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'trigger-system' },
      });
