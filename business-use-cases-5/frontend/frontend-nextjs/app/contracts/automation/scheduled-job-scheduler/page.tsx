...
export default function ScheduledJobSchedulerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('scheduled-job-scheduler');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'scheduled-job-scheduler',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'scheduled-job-scheduler' },
      });
