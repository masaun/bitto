...
export default function AutomationCheckerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('automation-checker');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'automation-checker',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'automation-checker' },
      });
