...
export default function RecurringJobSchedulerPage() {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  const contractAddress = getContractAddress('recurring-job-scheduler');
  ...
  const handleAction = async () => {
    ...
    try {
      await openContractCall({
        ...
        contractAddress,
        contractName: 'recurring-job-scheduler',
        functionName: 'initialize',
        functionArgs: [],
        appDetails: { name: 'recurring-job-scheduler' },
      });
