...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GOVERNMENT_REVENUE_CORE_ADDRESS || '';

export default function Government_Revenue_CorePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.government-revenue-core/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'government-revenue-core',
