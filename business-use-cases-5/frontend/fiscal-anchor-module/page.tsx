...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_ANCHOR_MODULE_ADDRESS || '';

export default function Fiscal_Anchor_ModulePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-anchor-module/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-anchor-module',
