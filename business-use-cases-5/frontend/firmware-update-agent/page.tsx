...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FIRMWARE_UPDATE_AGENT_ADDRESS || '';

export default function Firmware_Update_AgentPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.firmware-update-agent/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'firmware-update-agent',
