...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EXCHANGE_RESERVE_FEED_ADDRESS || '';

export default function Exchange_Reserve_FeedPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.exchange-reserve-feed/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'exchange-reserve-feed',
