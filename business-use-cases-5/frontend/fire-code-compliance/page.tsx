...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FIRE_CODE_COMPLIANCE_ADDRESS || '';

export default function Fire_Code_CompliancePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fire-code-compliance/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fire-code-compliance',
