...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EXPENDITURE_REPRIORITIZER_ADDRESS || '';

export default function Expenditure_ReprioritizerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'expenditure-reprioritizer',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.expenditure-reprioritizer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'expenditure-reprioritizer',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>expenditure-reprioritizer</h1>
