...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_OVERSIGHT_REGISTRY_ADDRESS || '';

export default function Fiscal_Oversight_RegistryPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'fiscal-oversight-registry',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-oversight-registry/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-oversight-registry',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>fiscal-oversight-registry</h1>
