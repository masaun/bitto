...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ETH_ERC20_GATEWAY_ADDRESS || '';

export default function Eth_Erc20_GatewayPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.eth-erc20-gateway/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'eth-erc20-gateway',
