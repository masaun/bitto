...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GALACTIC_TREASURY_ADDRESS || '';

export default function Galactic_TreasuryPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.galactic-treasury/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'galactic-treasury',
