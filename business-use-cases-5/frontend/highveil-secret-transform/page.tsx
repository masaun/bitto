...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HIGHVEIL_SECRET_TRANSFORM_ADDRESS || '';

export default function Highveil_Secret_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'highveil-secret-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.highveil-secret-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'highveil-secret-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>highveil-secret-transform</h1>
