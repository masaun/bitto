...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HYPERLEDGER_FABRIC_ADDRESS || '';

export default function Hyperledger_FabricPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.hyperledger-fabric/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'hyperledger-fabric',
