...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EVERVEIL_SECURE_TRANSFORMER_ADDRESS || '';

export default function Everveil_Secure_TransformerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'everveil-secure-transformer',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.everveil-secure-transformer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'everveil-secure-transformer',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>everveil-secure-transformer</h1>
