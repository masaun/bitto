...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GRAVITONSEAL_SECURE_REFINERY_ADDRESS || '';

export default function Gravitonseal_Secure_RefineryPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'gravitonseal-secure-refinery',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.gravitonseal-secure-refinery/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'gravitonseal-secure-refinery',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>gravitonseal-secure-refinery</h1>
