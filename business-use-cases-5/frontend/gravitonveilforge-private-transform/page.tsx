...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GRAVITONVEILFORGE_PRIVATE_TRANSFORM_ADDRESS || '';

export default function Gravitonveilforge_Private_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'gravitonveilforge-private-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.gravitonveilforge-private-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'gravitonveilforge-private-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>gravitonveilforge-private-transform</h1>
