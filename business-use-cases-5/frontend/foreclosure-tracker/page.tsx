...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FORECLOSURE_TRACKER_ADDRESS || '';

export default function Foreclosure_TrackerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.foreclosure-tracker/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'foreclosure-tracker',
