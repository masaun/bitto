...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EXPENSE_REIMBURSEMENT_ADDRESS || '';

export default function Expense_ReimbursementPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.expense-reimbursement/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'expense-reimbursement',
