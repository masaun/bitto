...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FOREIGN_INVESTOR_TAX_ADDRESS || '';

export default function Foreign_Investor_TaxPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.foreign-investor-tax/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'foreign-investor-tax',
