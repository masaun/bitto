...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HURDLE_RATE_CHECKER_ADDRESS || '';

export default function Hurdle_Rate_CheckerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.hurdle-rate-checker/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'hurdle-rate-checker',
