...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HIGHLANDSEAL_PRIVATE_SYNC_ADDRESS || '';

export default function Highlandseal_Private_SyncPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'highlandseal-private-sync',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.highlandseal-private-sync/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'highlandseal-private-sync',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>highlandseal-private-sync</h1>
