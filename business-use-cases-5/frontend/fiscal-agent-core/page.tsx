...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_AGENT_CORE_ADDRESS || '';

export default function Fiscal_Agent_CorePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-agent-core/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-agent-core',
