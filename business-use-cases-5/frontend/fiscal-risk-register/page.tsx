...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_RISK_REGISTER_ADDRESS || '';

export default function Fiscal_Risk_RegisterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-risk-register/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-risk-register',
