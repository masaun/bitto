...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FINALITY_PROVIDER_ADDRESS || '';

export default function Finality_ProviderPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.finality-provider/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'finality-provider',
