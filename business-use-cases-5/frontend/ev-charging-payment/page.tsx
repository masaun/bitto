...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EV_CHARGING_PAYMENT_ADDRESS || '';

export default function Ev_Charging_PaymentPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ev-charging-payment/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ev-charging-payment',
