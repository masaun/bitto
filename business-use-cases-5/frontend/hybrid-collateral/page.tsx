...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HYBRID_COLLATERAL_ADDRESS || '';

export default function Hybrid_CollateralPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.hybrid-collateral/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'hybrid-collateral',
