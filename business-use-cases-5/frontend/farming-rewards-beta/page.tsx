...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FARMING_REWARDS_BETA_ADDRESS || '';

export default function Farming_Rewards_BetaPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.farming-rewards-beta/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'farming-rewards-beta',
