...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GRANT_PROGRAM_FUND_ADDRESS || '';

export default function Grant_Program_FundPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.grant-program-fund/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'grant-program-fund',
