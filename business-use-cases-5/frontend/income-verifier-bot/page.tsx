...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INCOME_VERIFIER_BOT_ADDRESS || '';

export default function Income_Verifier_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.income-verifier-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'income-verifier-bot',
