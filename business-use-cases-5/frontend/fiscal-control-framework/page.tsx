...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_CONTROL_FRAMEWORK_ADDRESS || '';

export default function Fiscal_Control_FrameworkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-control-framework/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-control-framework',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>fiscal-control-framework</h1>
