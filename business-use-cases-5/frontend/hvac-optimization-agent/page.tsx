...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HVAC_OPTIMIZATION_AGENT_ADDRESS || '';

export default function Hvac_Optimization_AgentPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.hvac-optimization-agent/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'hvac-optimization-agent',
