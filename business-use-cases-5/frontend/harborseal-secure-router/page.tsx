...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HARBORSEAL_SECURE_ROUTER_ADDRESS || '';

export default function Harborseal_Secure_RouterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.harborseal-secure-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'harborseal-secure-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>harborseal-secure-router</h1>
