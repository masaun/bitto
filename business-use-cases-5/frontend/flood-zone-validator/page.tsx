...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FLOOD_ZONE_VALIDATOR_ADDRESS || '';

export default function Flood_Zone_ValidatorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.flood-zone-validator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'flood-zone-validator',
