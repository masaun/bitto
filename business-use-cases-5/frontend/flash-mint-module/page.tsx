...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FLASH_MINT_MODULE_ADDRESS || '';

export default function Flash_Mint_ModulePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.flash-mint-module/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'flash-mint-module',
