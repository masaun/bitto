...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EXECUTION_DELAY_V1_ADDRESS || '';

export default function Execution_Delay_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.execution-delay-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'execution-delay-v1',
