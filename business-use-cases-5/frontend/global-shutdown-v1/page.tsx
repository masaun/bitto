...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GLOBAL_SHUTDOWN_V1_ADDRESS || '';

export default function Global_Shutdown_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.global-shutdown-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'global-shutdown-v1',
