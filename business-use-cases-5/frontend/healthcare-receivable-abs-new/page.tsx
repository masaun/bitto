...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_HEALTHCARE_RECEIVABLE_ABS_NEW_ADDRESS || '';

export default function Healthcare_Receivable_Abs_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'healthcare-receivable-abs-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.healthcare-receivable-abs-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'healthcare-receivable-abs-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>healthcare-receivable-abs-new</h1>
