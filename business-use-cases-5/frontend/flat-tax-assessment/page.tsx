...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FLAT_TAX_ASSESSMENT_ADDRESS || '';

export default function Flat_Tax_AssessmentPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.flat-tax-assessment/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'flat-tax-assessment',
