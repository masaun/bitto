...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FOREIGN_ASSET_REG_ADDRESS || '';

export default function Foreign_Asset_RegPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.foreign-asset-reg/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'foreign-asset-reg',
