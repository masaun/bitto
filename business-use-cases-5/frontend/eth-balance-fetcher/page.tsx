...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ETH_BALANCE_FETCHER_ADDRESS || '';

export default function Eth_Balance_FetcherPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.eth-balance-fetcher/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'eth-balance-fetcher',
