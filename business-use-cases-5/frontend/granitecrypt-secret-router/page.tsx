...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_GRANITECRYPT_SECRET_ROUTER_ADDRESS || '';

export default function Granitecrypt_Secret_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'granitecrypt-secret-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.granitecrypt-secret-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'granitecrypt-secret-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>granitecrypt-secret-router</h1>
