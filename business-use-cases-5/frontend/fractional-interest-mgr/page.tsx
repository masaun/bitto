...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FRACTIONAL_INTEREST_MGR_ADDRESS || '';

export default function Fractional_Interest_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fractional-interest-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fractional-interest-mgr',
