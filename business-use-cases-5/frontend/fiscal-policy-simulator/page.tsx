...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_POLICY_SIMULATOR_ADDRESS || '';

export default function Fiscal_Policy_SimulatorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-policy-simulator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-policy-simulator',
