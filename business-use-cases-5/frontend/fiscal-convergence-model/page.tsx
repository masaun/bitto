...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_CONVERGENCE_MODEL_ADDRESS || '';

export default function Fiscal_Convergence_ModelPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-convergence-model/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-convergence-model',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>fiscal-convergence-model</h1>
