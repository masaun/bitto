...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_DEBT_RATIO_ADDRESS || '';

export default function Fiscal_Debt_RatioPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-debt-ratio/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-debt-ratio',
