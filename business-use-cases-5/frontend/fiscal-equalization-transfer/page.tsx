...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_EQUALIZATION_TRANSFER_ADDRESS || '';

export default function Fiscal_Equalization_TransferPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'fiscal-equalization-transfer',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-equalization-transfer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-equalization-transfer',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>fiscal-equalization-transfer</h1>
