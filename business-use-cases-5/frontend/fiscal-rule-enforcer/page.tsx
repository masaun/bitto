...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FISCAL_RULE_ENFORCER_ADDRESS || '';

export default function Fiscal_Rule_EnforcerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fiscal-rule-enforcer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fiscal-rule-enforcer',
