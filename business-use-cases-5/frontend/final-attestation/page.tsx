...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FINAL_ATTESTATION_ADDRESS || '';

export default function Final_AttestationPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.final-attestation/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'final-attestation',
