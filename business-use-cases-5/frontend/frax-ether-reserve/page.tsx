...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FRAX_ETHER_RESERVE_ADDRESS || '';

export default function Frax_Ether_ReservePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.frax-ether-reserve/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'frax-ether-reserve',
