...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_FIDELITY_DIGITAL_V_ADDRESS || '';

export default function Fidelity_Digital_VPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.fidelity-digital-v/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'fidelity-digital-v',
