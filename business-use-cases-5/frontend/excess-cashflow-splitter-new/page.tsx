...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_EXCESS_CASHFLOW_SPLITTER_NEW_ADDRESS || '';

export default function Excess_Cashflow_Splitter_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'excess-cashflow-splitter-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.excess-cashflow-splitter-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'excess-cashflow-splitter-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>excess-cashflow-splitter-new</h1>
