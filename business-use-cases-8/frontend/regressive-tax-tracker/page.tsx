...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REGRESSIVE_TAX_TRACKER_ADDRESS || '';

export default function Regressive_Tax_TrackerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.regressive-tax-tracker/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'regressive-tax-tracker',
