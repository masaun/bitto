...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RENOVATION_BUDGET_AI_ADDRESS || '';

export default function Renovation_Budget_AiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.renovation-budget-ai/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'renovation-budget-ai',
