...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REINVESTMENT_PERIOD_CONTROLLER_NEW_ADDRESS || '';

export default function Reinvestment_Period_Controller_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'reinvestment-period-controller-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.reinvestment-period-controller-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'reinvestment-period-controller-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>reinvestment-period-controller-new</h1>
