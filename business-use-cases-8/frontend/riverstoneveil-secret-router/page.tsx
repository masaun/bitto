...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RIVERSTONEVEIL_SECRET_ROUTER_ADDRESS || '';

export default function Riverstoneveil_Secret_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'riverstoneveil-secret-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.riverstoneveil-secret-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'riverstoneveil-secret-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>riverstoneveil-secret-router</h1>
