...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PUBLIC_BUDGET_FRAMEWORK_ADDRESS || '';

export default function Public_Budget_FrameworkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.public-budget-framework/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'public-budget-framework',
