...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PUBLIC_EXPENDITURE_CORE_ADDRESS || '';

export default function Public_Expenditure_CorePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.public-expenditure-core/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'public-expenditure-core',
