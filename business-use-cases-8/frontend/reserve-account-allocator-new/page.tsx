...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RESERVE_ACCOUNT_ALLOCATOR_NEW_ADDRESS || '';

export default function Reserve_Account_Allocator_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'reserve-account-allocator-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.reserve-account-allocator-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'reserve-account-allocator-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>reserve-account-allocator-new</h1>
