...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RECEIVABLE_FINANCING_CORE_NEW_ADDRESS || '';

export default function Receivable_Financing_Core_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'receivable-financing-core-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.receivable-financing-core-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'receivable-financing-core-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>receivable-financing-core-new</h1>
