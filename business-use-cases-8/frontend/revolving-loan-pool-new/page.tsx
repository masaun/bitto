...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REVOLVING_LOAN_POOL_NEW_ADDRESS || '';

export default function Revolving_Loan_Pool_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.revolving-loan-pool-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'revolving-loan-pool-new',
