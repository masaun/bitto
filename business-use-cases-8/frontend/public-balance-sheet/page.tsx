...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PUBLIC_BALANCE_SHEET_ADDRESS || '';

export default function Public_Balance_SheetPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.public-balance-sheet/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'public-balance-sheet',
