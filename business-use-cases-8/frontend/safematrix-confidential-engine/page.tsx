...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SAFEMATRIX_CONFIDENTIAL_ENGINE_ADDRESS || '';

export default function Safematrix_Confidential_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'safematrix-confidential-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.safematrix-confidential-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'safematrix-confidential-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>safematrix-confidential-engine</h1>
