...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REVERSE_SPLIT_LOGIC_ADDRESS || '';

export default function Reverse_Split_LogicPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.reverse-split-logic/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'reverse-split-logic',
