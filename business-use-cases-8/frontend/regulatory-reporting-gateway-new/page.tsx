...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REGULATORY_REPORTING_GATEWAY_NEW_ADDRESS || '';

export default function Regulatory_Reporting_Gateway_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'regulatory-reporting-gateway-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.regulatory-reporting-gateway-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'regulatory-reporting-gateway-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>regulatory-reporting-gateway-new</h1>
