...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PUBLIC_TRANSFER_SYSTEM_ADDRESS || '';

export default function Public_Transfer_SystemPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.public-transfer-system/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'public-transfer-system',
