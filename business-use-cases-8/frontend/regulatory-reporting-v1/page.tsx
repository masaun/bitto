...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REGULATORY_REPORTING_V1_ADDRESS || '';

export default function Regulatory_Reporting_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.regulatory-reporting-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'regulatory-reporting-v1',
