...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_QUICKNODE_ENDPOINT_ADDRESS || '';

export default function Quicknode_EndpointPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.quicknode-endpoint/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'quicknode-endpoint',
