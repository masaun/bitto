...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RAINY_DAY_PROTOCOL_ADDRESS || '';

export default function Rainy_Day_ProtocolPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.rainy-day-protocol/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'rainy-day-protocol',
