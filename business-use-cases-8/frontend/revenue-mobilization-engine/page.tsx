...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REVENUE_MOBILIZATION_ENGINE_ADDRESS || '';

export default function Revenue_Mobilization_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'revenue-mobilization-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.revenue-mobilization-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'revenue-mobilization-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>revenue-mobilization-engine</h1>
