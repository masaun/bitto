...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RESERVE_VALIDATOR_CORE_ADDRESS || '';

export default function Reserve_Validator_CorePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.reserve-validator-core/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'reserve-validator-core',
