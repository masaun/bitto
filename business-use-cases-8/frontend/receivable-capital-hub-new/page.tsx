...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RECEIVABLE_CAPITAL_HUB_NEW_ADDRESS || '';

export default function Receivable_Capital_Hub_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'receivable-capital-hub-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.receivable-capital-hub-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'receivable-capital-hub-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>receivable-capital-hub-new</h1>
