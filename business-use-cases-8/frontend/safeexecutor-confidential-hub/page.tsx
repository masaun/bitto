...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SAFEEXECUTOR_CONFIDENTIAL_HUB_ADDRESS || '';

export default function Safeexecutor_Confidential_HubPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'safeexecutor-confidential-hub',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.safeexecutor-confidential-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'safeexecutor-confidential-hub',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>safeexecutor-confidential-hub</h1>
