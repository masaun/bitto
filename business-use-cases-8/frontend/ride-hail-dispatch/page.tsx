...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RIDE_HAIL_DISPATCH_ADDRESS || '';

export default function Ride_Hail_DispatchPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ride-hail-dispatch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ride-hail-dispatch',
