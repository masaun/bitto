...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RANDOM_SAMPLING_BOT_ADDRESS || '';

export default function Random_Sampling_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.random-sampling-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'random-sampling-bot',
