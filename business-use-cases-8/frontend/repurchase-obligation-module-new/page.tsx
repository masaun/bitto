...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REPURCHASE_OBLIGATION_MODULE_NEW_ADDRESS || '';

export default function Repurchase_Obligation_Module_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'repurchase-obligation-module-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.repurchase-obligation-module-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'repurchase-obligation-module-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>repurchase-obligation-module-new</h1>
