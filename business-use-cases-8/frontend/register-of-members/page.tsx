...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REGISTER_OF_MEMBERS_ADDRESS || '';

export default function Register_Of_MembersPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.register-of-members/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'register-of-members',
