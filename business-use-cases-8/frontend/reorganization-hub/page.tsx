...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REORGANIZATION_HUB_ADDRESS || '';

export default function Reorganization_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.reorganization-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'reorganization-hub',
