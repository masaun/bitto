...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RATE_LIMIT_CONTROLLER_ADDRESS || '';

export default function Rate_Limit_ControllerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.rate-limit-controller/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'rate-limit-controller',
