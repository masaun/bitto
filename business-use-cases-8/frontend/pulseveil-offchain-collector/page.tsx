...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PULSEVEIL_OFFCHAIN_COLLECTOR_ADDRESS || '';

export default function Pulseveil_Offchain_CollectorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'pulseveil-offchain-collector',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.pulseveil-offchain-collector/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'pulseveil-offchain-collector',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>pulseveil-offchain-collector</h1>
