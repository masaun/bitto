...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ROCKET_POOL_MINIPOOL_ADDRESS || '';

export default function Rocket_Pool_MinipoolPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.rocket-pool-minipool/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'rocket-pool-minipool',
