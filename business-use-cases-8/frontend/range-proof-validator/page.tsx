...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RANGE_PROOF_VALIDATOR_ADDRESS || '';

export default function Range_Proof_ValidatorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.range-proof-validator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'range-proof-validator',
