...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SAFEJOURNAL_CONFIDENTIAL_FETCH_ADDRESS || '';

export default function Safejournal_Confidential_FetchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'safejournal-confidential-fetch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.safejournal-confidential-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'safejournal-confidential-fetch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>safejournal-confidential-fetch</h1>
