...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REDWOODCLOAK_SECRET_TRANSFORM_ADDRESS || '';

export default function Redwoodcloak_Secret_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'redwoodcloak-secret-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.redwoodcloak-secret-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'redwoodcloak-secret-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>redwoodcloak-secret-transform</h1>
