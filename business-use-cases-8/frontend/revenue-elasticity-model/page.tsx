...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REVENUE_ELASTICITY_MODEL_ADDRESS || '';

export default function Revenue_Elasticity_ModelPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.revenue-elasticity-model/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'revenue-elasticity-model',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>revenue-elasticity-model</h1>
