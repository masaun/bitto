...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PULSEVEILFORGE_SECRET_ENGINE_ADDRESS || '';

export default function Pulseveilforge_Secret_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'pulseveilforge-secret-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.pulseveilforge-secret-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'pulseveilforge-secret-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>pulseveilforge-secret-engine</h1>
