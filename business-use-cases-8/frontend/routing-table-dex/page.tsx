...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ROUTING_TABLE_DEX_ADDRESS || '';

export default function Routing_Table_DexPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.routing-table-dex/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'routing-table-dex',
