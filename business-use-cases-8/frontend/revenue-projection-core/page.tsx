...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REVENUE_PROJECTION_CORE_ADDRESS || '';

export default function Revenue_Projection_CorePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.revenue-projection-core/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'revenue-projection-core',
