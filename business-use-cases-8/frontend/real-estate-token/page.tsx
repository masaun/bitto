...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REAL_ESTATE_TOKEN_ADDRESS || '';

export default function Real_Estate_TokenPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.real-estate-token/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'real-estate-token',
