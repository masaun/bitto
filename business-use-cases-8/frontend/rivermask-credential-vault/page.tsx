...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RIVERMASK_CREDENTIAL_VAULT_ADDRESS || '';

export default function Rivermask_Credential_VaultPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'rivermask-credential-vault',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.rivermask-credential-vault/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'rivermask-credential-vault',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>rivermask-credential-vault</h1>
