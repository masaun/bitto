...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_QUANTUMSEAL_API_HARVESTER_ADDRESS || '';

export default function Quantumseal_Api_HarvesterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'quantumseal-api-harvester',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.quantumseal-api-harvester/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'quantumseal-api-harvester',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>quantumseal-api-harvester</h1>
