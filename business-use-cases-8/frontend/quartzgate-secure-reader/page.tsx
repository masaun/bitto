...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_QUARTZGATE_SECURE_READER_ADDRESS || '';

export default function Quartzgate_Secure_ReaderPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.quartzgate-secure-reader/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'quartzgate-secure-reader',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>quartzgate-secure-reader</h1>
