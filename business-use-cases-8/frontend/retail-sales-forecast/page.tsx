...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RETAIL_SALES_FORECAST_ADDRESS || '';

export default function Retail_Sales_ForecastPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.retail-sales-forecast/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'retail-sales-forecast',
