...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_QUARTZVEILFORGE_PRIVATE_ENGINE_ADDRESS || '';

export default function Quartzveilforge_Private_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'quartzveilforge-private-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.quartzveilforge-private-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'quartzveilforge-private-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>quartzveilforge-private-engine</h1>
