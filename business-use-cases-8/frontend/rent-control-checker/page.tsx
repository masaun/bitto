...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RENT_CONTROL_CHECKER_ADDRESS || '';

export default function Rent_Control_CheckerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.rent-control-checker/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'rent-control-checker',
