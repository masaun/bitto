...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_QUERY_INTERFACE_V1_ADDRESS || '';

export default function Query_Interface_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.query-interface-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'query-interface-v1',
