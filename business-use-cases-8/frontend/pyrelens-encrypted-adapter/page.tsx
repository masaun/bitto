...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PYRELENS_ENCRYPTED_ADAPTER_ADDRESS || '';

export default function Pyrelens_Encrypted_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'pyrelens-encrypted-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.pyrelens-encrypted-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'pyrelens-encrypted-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>pyrelens-encrypted-adapter</h1>
