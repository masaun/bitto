...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REVENUE_SHARE_MGR_ADDRESS || '';

export default function Revenue_Share_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.revenue-share-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'revenue-share-mgr',
