...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REPRO_RATE_ADJUSTER_ADDRESS || '';

export default function Repro_Rate_AdjusterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.repro-rate-adjuster/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'repro-rate-adjuster',
