...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REDEMPTION_PRICE_CALCULATOR_NEW_ADDRESS || '';

export default function Redemption_Price_Calculator_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'redemption-price-calculator-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.redemption-price-calculator-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'redemption-price-calculator-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>redemption-price-calculator-new</h1>
