...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RISK_WEIGHT_ENGINE_NEW_ADDRESS || '';

export default function Risk_Weight_Engine_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.risk-weight-engine-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'risk-weight-engine-new',
