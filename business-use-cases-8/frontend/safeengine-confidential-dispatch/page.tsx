...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SAFEENGINE_CONFIDENTIAL_DISPATCH_ADDRESS || '';

export default function Safeengine_Confidential_DispatchPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'safeengine-confidential-dispatch',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.safeengine-confidential-dispatch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'safeengine-confidential-dispatch',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>safeengine-confidential-dispatch</h1>
