...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RATING_CONFIRMATION_TRIGGER_NEW_ADDRESS || '';

export default function Rating_Confirmation_Trigger_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'rating-confirmation-trigger-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.rating-confirmation-trigger-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'rating-confirmation-trigger-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>rating-confirmation-trigger-new</h1>
