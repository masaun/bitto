...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ROOT_HASH_MISMATCH_ADDRESS || '';

export default function Root_Hash_MismatchPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.root-hash-mismatch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'root-hash-mismatch',
