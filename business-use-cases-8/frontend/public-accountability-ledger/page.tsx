...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PUBLIC_ACCOUNTABILITY_LEDGER_ADDRESS || '';

export default function Public_Accountability_LedgerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'public-accountability-ledger',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.public-accountability-ledger/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'public-accountability-ledger',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>public-accountability-ledger</h1>
