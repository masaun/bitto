...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RECOVERY_FUND_VAULT_ADDRESS || '';

export default function Recovery_Fund_VaultPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.recovery-fund-vault/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'recovery-fund-vault',
