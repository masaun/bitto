...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_QUANTUMGUARD_OFFCHAIN_STREAM_ADDRESS || '';

export default function Quantumguard_Offchain_StreamPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'quantumguard-offchain-stream',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.quantumguard-offchain-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'quantumguard-offchain-stream',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>quantumguard-offchain-stream</h1>
