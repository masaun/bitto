...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RISK_PARAMETER_DB_ADDRESS || '';

export default function Risk_Parameter_DbPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.risk-parameter-db/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'risk-parameter-db',
