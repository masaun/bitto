...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REIT_DIVIDEND_LOGIC_ADDRESS || '';

export default function Reit_Dividend_LogicPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.reit-dividend-logic/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'reit-dividend-logic',
