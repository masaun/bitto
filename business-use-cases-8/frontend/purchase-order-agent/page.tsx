...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PURCHASE_ORDER_AGENT_ADDRESS || '';

export default function Purchase_Order_AgentPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.purchase-order-agent/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'purchase-order-agent',
