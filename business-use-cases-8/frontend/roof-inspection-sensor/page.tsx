...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ROOF_INSPECTION_SENSOR_ADDRESS || '';

export default function Roof_Inspection_SensorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.roof-inspection-sensor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'roof-inspection-sensor',
