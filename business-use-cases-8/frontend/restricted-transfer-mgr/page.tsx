...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RESTRICTED_TRANSFER_MGR_ADDRESS || '';

export default function Restricted_Transfer_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.restricted-transfer-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'restricted-transfer-mgr',
