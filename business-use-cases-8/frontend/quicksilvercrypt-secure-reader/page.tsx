...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_QUICKSILVERCRYPT_SECURE_READER_ADDRESS || '';

export default function Quicksilvercrypt_Secure_ReaderPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'quicksilvercrypt-secure-reader',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.quicksilvercrypt-secure-reader/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'quicksilvercrypt-secure-reader',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>quicksilvercrypt-secure-reader</h1>
