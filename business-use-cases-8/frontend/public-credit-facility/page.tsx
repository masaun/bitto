...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PUBLIC_CREDIT_FACILITY_ADDRESS || '';

export default function Public_Credit_FacilityPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.public-credit-facility/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'public-credit-facility',
