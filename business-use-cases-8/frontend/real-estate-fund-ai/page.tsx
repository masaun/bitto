...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REAL_ESTATE_FUND_AI_ADDRESS || '';

export default function Real_Estate_Fund_AiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.real-estate-fund-ai/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'real-estate-fund-ai',
