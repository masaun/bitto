...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_REFERRAL_INCENTIVE_SYS_ADDRESS || '';

export default function Referral_Incentive_SysPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.referral-incentive-sys/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'referral-incentive-sys',
