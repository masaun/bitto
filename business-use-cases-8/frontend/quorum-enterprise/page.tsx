...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_QUORUM_ENTERPRISE_ADDRESS || '';

export default function Quorum_EnterprisePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.quorum-enterprise/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'quorum-enterprise',
