...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RECURRING_TASK_MGR_ADDRESS || '';

export default function Recurring_Task_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.recurring-task-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'recurring-task-mgr',
