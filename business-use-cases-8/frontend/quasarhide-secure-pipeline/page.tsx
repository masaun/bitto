...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_QUASARHIDE_SECURE_PIPELINE_ADDRESS || '';

export default function Quasarhide_Secure_PipelinePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'quasarhide-secure-pipeline',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.quasarhide-secure-pipeline/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'quasarhide-secure-pipeline',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>quasarhide-secure-pipeline</h1>
