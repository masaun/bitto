...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_SAFEARCHIVE_CONFIDENTIAL_ADAPTER_ADDRESS || '';

export default function Safearchive_Confidential_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'safearchive-confidential-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.safearchive-confidential-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'safearchive-confidential-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>safearchive-confidential-adapter</h1>
