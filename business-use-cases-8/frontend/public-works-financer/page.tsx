...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PUBLIC_WORKS_FINANCER_ADDRESS || '';

export default function Public_Works_FinancerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.public-works-financer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'public-works-financer',
