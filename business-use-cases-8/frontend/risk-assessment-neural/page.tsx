...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_RISK_ASSESSMENT_NEURAL_ADDRESS || '';

export default function Risk_Assessment_NeuralPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.risk-assessment-neural/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'risk-assessment-neural',
