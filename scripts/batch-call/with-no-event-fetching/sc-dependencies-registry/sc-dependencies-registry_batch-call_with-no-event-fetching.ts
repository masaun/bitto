...
const contractDetails = parseContractIdentifier(process.env.SC_DEPENDENCIES_REGISTRY_CONTRACT_ADDRESS, 'sc-dependencies-registry');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const addContractTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'add-contract',
      functionArgs: [stringAsciiCV(`contract-${cycle}`), standardPrincipalCV(senderAddress)],
    ...
    const addContractResult = await broadcastTransaction({ transaction: addContractTx, network: networkForBroadcast });
    console.log(`add-contract tx: ${addContractResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const addProxyContractTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'add-proxy-contract',
      functionArgs: [stringAsciiCV(`proxy-${cycle}`), standardPrincipalCV(senderAddress)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const addProxyContractResult = await broadcastTransaction({ transaction: addProxyContractTx, network: networkForBroadcast });
    console.log(`add-proxy-contract tx: ${addProxyContractResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const injectDependenciesTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'inject-dependencies',
      functionArgs: [stringAsciiCV(`contract-${cycle}`), listCV([stringAsciiCV('dependency1')])],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const injectDependenciesResult = await broadcastTransaction({ transaction: injectDependenciesTx, network: networkForBroadcast });
    console.log(`inject-dependencies tx: ${injectDependenciesResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const upgradeContractTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const upgradeContractResult = await broadcastTransaction({ transaction: upgradeContractTx, network: networkForBroadcast });
    console.log(`upgrade-contract tx: ${upgradeContractResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
  }
