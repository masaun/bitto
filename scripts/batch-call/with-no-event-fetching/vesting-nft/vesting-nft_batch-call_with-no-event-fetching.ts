...
const contractDetails = parseContractIdentifier(
  process.env.VESTING_NFT_CONTRACT_ADDRESS,
  'vesting-nft'
);
  ...
  return [
    {
      ...
      args: (cycle: number, senderAddress: string, _tokenId: number) => [
        ...
        principalCV(senderAddress),
        uintCV(10000 + cycle * 1000),
        uintCV(currentTime + 3600),
        uintCV(currentTime + 86400),
      ],
    },
    {
      name: 'transfer',
    ...
    },
    {
      ...
      args: (cycle: number, _senderAddress: string, tokenId: number) => [
        uintCV(tokenId),
        stringAsciiCV(`https://api.bitto.io/vesting/${tokenId}-${cycle}`),
      ],
    },
    {
      name: 'burn',
...
async function executeAllCycles(): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Vesting NFT Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Vesting NFT Contract - Batch Call Script');
  ...
  console.log(`Cycles: ${CONFIG.cycles}`);
  console.log(`Environment Variable: VESTING_NFT_CONTRACT_ADDRESS`);

  if (!process.env.VESTING_NFT_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: VESTING_NFT_CONTRACT_ADDRESS not set in environment.');
