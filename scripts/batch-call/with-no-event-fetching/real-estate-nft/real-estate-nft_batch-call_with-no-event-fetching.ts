...
const contractDetails = parseContractIdentifier(process.env.REAL_ESTATE_NFT_CONTRACT_ADDRESS, 'real-estate-nft');
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const mintTx = await makeContractCall({
      ...
      functionArgs: [
        ...
        stringAsciiCV('Real Estate Property'),
    ...
    const setDebtTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'set-debt',
      functionArgs: [uintCV(tokenId), contractPrincipalCV(CONFIG.contractAddress, CONFIG.contractName), uintCV(50000)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const setDebtResult = await broadcastTransaction({ transaction: setDebtTx, network: networkForBroadcast });
    console.log(`set-debt tx: ${setDebtResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const forecloseTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const forecloseResult = await broadcastTransaction({ transaction: forecloseTx, network: networkForBroadcast });
    console.log(`foreclose tx: ${forecloseResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
