...
const contractDetails = parseContractIdentifier(process.env.CULTURAL_HISTORICAL_TOKEN_CONTRACT_ADDRESS, 'cultural-historical-token');
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const mintTx = await makeContractCall({
      ...
      functionArgs: [
        ...
        stringAsciiCV('Cultural Artifact'),
        stringAsciiCV('Ancient'),
        uintCV(1000),
        stringAsciiCV('Museum')
      ],
    ...
    const setExtendedTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const setExtendedResult = await broadcastTransaction({ transaction: setExtendedTx, network: networkForBroadcast });
    console.log(`set-extended tx: ${setExtendedResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
