...
const contractDetails = parseContractIdentifier(process.env.NON_ADDR_HELD_DEVICE_SIG_VERIFIER_CONTRACT_ADDRESS, 'non-addr-held-device-sig-verifier');
...
async function batchCall(): Promise<void> {
  ...
  const publicKey = bufferCV(Buffer.from('0123456789012345678901234567890ab', 'hex'));

  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    console.log(`\nCycle ${cycle + 1}/${CONFIG.batchCycles}`);

    const registerKeyTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'register-key',
      functionArgs: [publicKey],
    ...
    const registerKeyResult = await broadcastTransaction({ transaction: registerKeyTx, network: networkForBroadcast });
    console.log(`register-key tx: ${registerKeyResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const unregisterKeyTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const unregisterKeyResult = await broadcastTransaction({ transaction: unregisterKeyTx, network: networkForBroadcast });
    console.log(`unregister-key tx: ${unregisterKeyResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
  }
