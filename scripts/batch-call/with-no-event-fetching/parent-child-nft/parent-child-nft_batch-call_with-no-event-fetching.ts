...
const contractDetails = parseContractIdentifier(process.env.PARENT_CHILD_NFT_CONTRACT_ADDRESS, 'parent-child-nft');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const mintParentTx = await makeContractCall({
    ...
    const mintParentResult = await broadcastTransaction({ transaction: mintParentTx, network: networkForBroadcast });
    console.log(`mint-parent tx: ${mintParentResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const mintChildTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const mintChildResult = await broadcastTransaction({ transaction: mintChildTx, network: networkForBroadcast });
    console.log(`mint-child tx: ${mintChildResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const proposeTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'propose-add-child',
      functionArgs: [uintCV(parentId), uintCV(childId)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const proposeResult = await broadcastTransaction({ transaction: proposeTx, network: networkForBroadcast });
    console.log(`propose-add-child tx: ${proposeResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const acceptTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const acceptResult = await broadcastTransaction({ transaction: acceptTx, network: networkForBroadcast });
    console.log(`accept-child tx: ${acceptResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
    ...
    const removeResult = await broadcastTransaction({ transaction: removeTx, network: networkForBroadcast });
    console.log(`remove-child tx: ${removeResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
