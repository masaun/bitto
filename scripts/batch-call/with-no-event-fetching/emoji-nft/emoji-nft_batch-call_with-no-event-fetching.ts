...
const contractDetails = parseContractIdentifier(process.env.EMOJI_NFT_CONTRACT_ADDRESS, 'emoji-nft');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const emoteTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'emote',
      functionArgs: [contractPrincipalCV(CONFIG.contractAddress, CONFIG.contractName), uintCV(1), stringUtf8CV(emoji)],
    ...
    const emoteResult = await broadcastTransaction({ transaction: emoteTx, network: networkForBroadcast });
    console.log(`emote tx: ${emoteResult.txid}`);
    nonce++;
    ...
    console.log(`remove-emote tx: ${removeResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
  }
