...
const contractDetails = parseContractIdentifier(process.env.ESCROW_REFUNDABLE_TOKEN_CONTRACT_ADDRESS, 'escrow-refundable-token');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const initEscrowTx = await makeContractCall({
      ...
      functionArgs: [
        ...
        standardPrincipalCV(senderAddress),
        contractPrincipalCV(CONFIG.contractAddress, 'refundable-token'),
        uintCV(1000)
      ],
    ...
    const initEscrowResult = await broadcastTransaction({ transaction: initEscrowTx, network: networkForBroadcast });
    console.log(`init-escrow tx: ${initEscrowResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const escrowFundTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const escrowFundResult = await broadcastTransaction({ transaction: escrowFundTx, network: networkForBroadcast });
    console.log(`escrow-fund tx: ${escrowFundResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const escrowWithdrawTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const escrowWithdrawResult = await broadcastTransaction({ transaction: escrowWithdrawTx, network: networkForBroadcast });
    console.log(`escrow-withdraw tx: ${escrowWithdrawResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const escrowRefundTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const escrowRefundResult = await broadcastTransaction({ transaction: escrowRefundTx, network: networkForBroadcast });
    console.log(`escrow-refund tx: ${escrowRefundResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
  }
