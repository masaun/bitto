...
const contractDetails = parseContractIdentifier(
  process.env.DIAMONDS_CONTRACT_ADDRESS,
  'diamonds'
);
...
function getWriteFunctions(senderAddress: string, recipientAddress: string): FunctionCall[] {
  const sampleSelector = bufferCV(Buffer.from([0x01, 0x02, 0x03, 0x04]));
  const sampleSig = bufferCV(Buffer.alloc(64, 0xab));
  const samplePubKey = bufferCV(Buffer.alloc(33, 0x02));

  return [
    {
      name: 'add-facet',
      args: () => [principalCV(recipientAddress), sampleSelector, boolCV(false)],
      description: 'Add a new facet with selector',
    },
    {
      name: 'replace-facet',
      args: () => [sampleSelector, principalCV(recipientAddress)],
      description: 'Replace existing facet for selector',
    },
    {
      name: 'remove-facet',
      args: () => [sampleSelector],
      description: 'Remove a facet selector',
    },
    {
      name: 'authorize-signer',
      args: () => [principalCV(recipientAddress)],
      description: 'Authorize a new signer for diamond cuts',
    },
    {
      name: 'revoke-signer',
      args: () => [principalCV(recipientAddress)],
      description: 'Revoke signer authorization',
    },
    {
      name: 'transfer-ownership',
      args: () => [principalCV(recipientAddress)],
      description: 'Transfer diamond ownership',
    },
    {
    ...
    },
    {
      name: 'freeze-diamond',
      args: () => [],
      description: 'Freeze the diamond permanently',
    },
  ];
}
...
async function executeBatchCycles(): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for diamonds ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Diamonds - Batch Call Script');
  console.log('==========================================================');
