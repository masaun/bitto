...
const contractDetails = parseContractIdentifier(process.env.PHYSICAL_ASSET_BACKED_NFT_CONTRACT_ADDRESS, 'physical-asset-backed-nft');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const location = `storage-${cycle}`;

    const mintTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'mint',
      functionArgs: [standardPrincipalCV(senderAddress), stringUtf8CV(assetId), stringUtf8CV(location)],
    ...
    const requestRedemptionTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const requestRedemptionResult = await broadcastTransaction({ transaction: requestRedemptionTx, network: networkForBroadcast });
    console.log(`request-redemption tx: ${requestRedemptionResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const approveRedemptionTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const approveRedemptionResult = await broadcastTransaction({ transaction: approveRedemptionTx, network: networkForBroadcast });
    console.log(`approve-redemption tx: ${approveRedemptionResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const newLocation = `new-storage-${cycle}`;
    const updateLocationTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'update-storage-location',
      functionArgs: [uintCV(tokenId), stringUtf8CV(newLocation)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const updateLocationResult = await broadcastTransaction({ transaction: updateLocationTx, network: networkForBroadcast });
    console.log(`update-storage-location tx: ${updateLocationResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
