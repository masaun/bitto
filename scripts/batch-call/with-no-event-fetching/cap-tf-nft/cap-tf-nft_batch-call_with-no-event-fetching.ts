...
const contractDetails = parseContractIdentifier(process.env.CAP_TF_NFT_CONTRACT_ADDRESS, 'cap-tf-nft');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const mintTx = await makeContractCall({
      ...
      functionName: 'mint',
      functionArgs: [standardPrincipalCV(senderAddress), uintCV(10)],
    ...
    const setTransferLimitTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'set-transfer-limit',
      functionArgs: [uintCV(tokenId), uintCV(20)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const setTransferLimitResult = await broadcastTransaction({ transaction: setTransferLimitTx, network: networkForBroadcast });
    console.log(`set-transfer-limit tx: ${setTransferLimitResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
