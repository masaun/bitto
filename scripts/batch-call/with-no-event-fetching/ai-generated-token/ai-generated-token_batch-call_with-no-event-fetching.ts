...
const contractDetails = parseContractIdentifier(
  process.env.AI_GENERATED_TOKEN_CONTRACT_ADDRESS,
  'ai-generated-token'
);
...
function getWriteFunctions(senderAddress: string, recipientAddress: string): FunctionCall[] {
  const prompt = Buffer.from('Generate an image of a sunset', 'utf8');
  const aigcData = Buffer.from('image-data-here', 'utf8');
  const proof = Buffer.from('01'.repeat(32), 'hex');
  
  return [
    {
      name: 'mint',
      args: () => [principalCV(recipientAddress), bufferCV(prompt)],
      description: 'Mint new AI-generated token',
    },
    {
      name: 'transfer',
      args: () => [uintCV(1), principalCV(senderAddress), principalCV(recipientAddress)],
      description: 'Transfer token to new owner',
    },
    {
      name: 'add-aigc-data',
      args: () => [uintCV(1), bufferCV(prompt), bufferCV(aigcData), bufferCV(proof)],
      description: 'Add AIGC data with proof',
    },
    {
      name: 'update-aigc-data',
      args: () => [uintCV(1), bufferCV(prompt), bufferCV(aigcData)],
      description: 'Update AIGC data',
    },
    {
      name: 'set-proof-type',
      args: () => [stringAsciiCV('zkml')],
      description: 'Set proof type to zkML',
    },
  ];
}
...
async function executeBatchCycles(): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for ai-generated-token ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  AI-Generated Token - Batch Call Script');
  console.log('==========================================================');
