...
const contractDetails = parseContractIdentifier(process.env.SHARED_OWNERSHIP_NFT_CONTRACT_ADDRESS, 'shared-ownership-nft');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const mintTx = await makeContractCall({
      ...
      functionName: 'mint',
      functionArgs: [stringAsciiCV(`ipfs://token${tokenId}`)],
    ...
    const archiveTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const archiveResult = await broadcastTransaction({ transaction: archiveTx, network: networkForBroadcast });
    console.log(`archive tx: ${archiveResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const setTransferValueTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'set-transfer-value',
      functionArgs: [uintCV(tokenId), uintCV(1000)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const setTransferValueResult = await broadcastTransaction({ transaction: setTransferValueTx, network: networkForBroadcast });
    console.log(`set-transfer-value tx: ${setTransferValueResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
