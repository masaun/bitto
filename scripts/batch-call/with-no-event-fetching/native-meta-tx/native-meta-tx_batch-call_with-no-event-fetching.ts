...
const contractDetails = parseContractIdentifier(process.env.NATIVE_META_TX_CONTRACT_ADDRESS, 'native-meta-tx');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const setTrustedForwarderTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'set-trusted-forwarder',
    ...
    const setTrustedForwarderResult = await broadcastTransaction({ transaction: setTrustedForwarderTx, network: networkForBroadcast });
    console.log(`set-trusted-forwarder tx: ${setTrustedForwarderResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const incrementNonceTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const incrementNonceResult = await broadcastTransaction({ transaction: incrementNonceTx, network: networkForBroadcast });
    console.log(`increment-nonce tx: ${incrementNonceResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const executeMetaTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'execute-meta-tx',
      functionArgs: [bufferCV(Buffer.alloc(32)), bufferCV(Buffer.alloc(65)), bufferCV(Buffer.alloc(33))],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const executeMetaTxResult = await broadcastTransaction({ transaction: executeMetaTx, network: networkForBroadcast });
    console.log(`execute-meta-tx tx: ${executeMetaTxResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
  }
