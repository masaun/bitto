...
const contractDetails = parseContractIdentifier(process.env.SHAREABLE_RIGHTS_NFT_CONTRACT_ADDRESS, 'shareable-rights-nft');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const setPrivilegeTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'set-privilege',
      functionArgs: [uintCV(tokenId), bufferCV(Buffer.from('readwrite')), standardPrincipalCV(senderAddress), uintCV(2000)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const setPrivilegeResult = await broadcastTransaction({ transaction: setPrivilegeTx, network: networkForBroadcast });
    console.log(`set-privilege tx: ${setPrivilegeResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
