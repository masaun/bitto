...
const contractDetails = parseContractIdentifier(process.env.GENERIC_SERVICES_FACTORY_CONTRACT_ADDRESS, 'generic-services-factory');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const createTx = await makeContractCall({
      ...
      functionName: 'create',
      functionArgs: [standardPrincipalCV(senderAddress), stringAsciiCV(`service-${cycle}`)],
    ...
    console.log(`create tx: ${createResult.txid}`);
    nonce++;
