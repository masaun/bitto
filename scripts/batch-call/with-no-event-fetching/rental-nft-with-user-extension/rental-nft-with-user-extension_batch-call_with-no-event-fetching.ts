...
const contractDetails = parseContractIdentifier(
  process.env.RENTAL_NFT_WITH_USER_EXTENSION_CONTRACT_ADDRESS,
...
function getWriteFunctions(senderAddress: string, recipientAddress: string, cycle: number): FunctionCall[] {
  const futureExpiry = Math.floor(Date.now() / 1000) + 86400 * 30 + (cycle * 86400);
  const tokenId = cycle;
  const recordId = cycle;
  ...
  return [
    {
      ...
      args: () => [
        ...
        uintCV(1000),
        stringAsciiCV(`https://example.com/token/${cycle}`),
      ],
      description: 'Mint new semi-fungible tokens',
    },
    {
      name: 'transfer',
      args: () => [
    ...
    },
    {
      ...
      args: () => [principalCV(recipientAddress), boolCV(true)],
      description: 'Approve operator for all tokens',
    },
    {
      ...
      description: 'Create rental user record',
    },
    {
      name: 'extend-rental',
      args: () => [uintCV(recordId), uintCV(futureExpiry + 86400 * 7)],
      description: 'Extend rental period',
    },
    {
      name: 'delete-user-record',
      args: () => [uintCV(recordId)],
      description: 'Delete user record',
    },
    {
      name: 'batch-delete-expired-records',
      args: () => [listCV([])],
      description: 'Batch delete expired records',
    },
    {
      name: 'set-asset-restriction',
      args: () => [boolCV(false)],
...
async function executeBatchCycles(): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for rental-nft-with-user-extension ===');
  ...
  const sampleFunctions = getWriteFunctions(senderAddress, recipientAddress, 1);

  console.log(`Sender: ${senderAddress}`);
  console.log(`Functions: ${sampleFunctions.length}`);
  console.log(`Total transactions: ${sampleFunctions.length * CONFIG.batchCycles}`);
  ...
  for (let cycle = 1; cycle <= CONFIG.batchCycles; cycle++) {
    ...
    const functions = getWriteFunctions(senderAddress, recipientAddress, cycle);
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Rental NFT with User Extension - Batch Call Script');
  console.log('==========================================================');
