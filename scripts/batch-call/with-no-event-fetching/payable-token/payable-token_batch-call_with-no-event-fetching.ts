...
const contractDetails = parseContractIdentifier(process.env.PAYABLE_TOKEN_CONTRACT_ADDRESS, 'payable-token');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const mintTx = await makeContractCall({
      ...
      functionName: 'mint',
      functionArgs: [standardPrincipalCV(senderAddress), uintCV(1000)],
    ...
    const approveTx = await makeContractCall({
      ...
      functionArgs: [standardPrincipalCV(senderAddress), uintCV(500)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const transferTx = await makeContractCall({
      ...
      functionArgs: [standardPrincipalCV(senderAddress), standardPrincipalCV(senderAddress), uintCV(100)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const transferFromTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'transfer-from',
      functionArgs: [standardPrincipalCV(senderAddress), standardPrincipalCV(senderAddress), uintCV(50)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const transferFromResult = await broadcastTransaction({ transaction: transferFromTx, network: networkForBroadcast });
    console.log(`transfer-from tx: ${transferFromResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const onTransferReceivedTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'on-transfer-received',
      functionArgs: [standardPrincipalCV(senderAddress), standardPrincipalCV(senderAddress), uintCV(100), noneCV()],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const onTransferReceivedResult = await broadcastTransaction({ transaction: onTransferReceivedTx, network: networkForBroadcast });
    console.log(`on-transfer-received tx: ${onTransferReceivedResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const onApprovalReceivedTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'on-approval-received',
      functionArgs: [standardPrincipalCV(senderAddress), uintCV(100), noneCV()],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const onApprovalReceivedResult = await broadcastTransaction({ transaction: onApprovalReceivedTx, network: networkForBroadcast });
    console.log(`on-approval-received tx: ${onApprovalReceivedResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
  }
