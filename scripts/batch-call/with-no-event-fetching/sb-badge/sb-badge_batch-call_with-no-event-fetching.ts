...
const contractDetails = parseContractIdentifier(process.env.SB_BADGE_CONTRACT_ADDRESS, 'sb-badge');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const mintTx = await makeContractCall({
      ...
      functionName: 'mint',
      functionArgs: [contractPrincipalCV(CONFIG.contractAddress, CONFIG.contractName), uintCV(1), stringAsciiCV('ipfs://badge1')],
    ...
    const transferTx = await makeContractCall({
      ...
      functionArgs: [uintCV(badgeId), standardPrincipalCV(senderAddress), standardPrincipalCV(senderAddress)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
