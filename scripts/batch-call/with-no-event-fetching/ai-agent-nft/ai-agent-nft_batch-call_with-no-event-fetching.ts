...
const contractDetails = parseContractIdentifier(process.env.AI_AGENT_NFT_CONTRACT_ADDRESS, 'ai-agent-nft');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const mintTx = await makeContractCall({
      ...
      functionName: 'mint',
      functionArgs: [bufferCV(Buffer.from('datahash12345678901234567890123')), stringAsciiCV('AI Agent NFT'), standardPrincipalCV(senderAddress)],
    ...
    const authorizeUsageTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'authorize-usage',
      functionArgs: [uintCV(tokenId), standardPrincipalCV(senderAddress)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const authorizeUsageResult = await broadcastTransaction({ transaction: authorizeUsageTx, network: networkForBroadcast });
    console.log(`authorize-usage tx: ${authorizeUsageResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
    ...
    const approveTx = await makeContractCall({
      ...
      functionArgs: [standardPrincipalCV(senderAddress), uintCV(tokenId)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const approveResult = await broadcastTransaction({ transaction: approveTx, network: networkForBroadcast });
    console.log(`approve tx: ${approveResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const setApprovalForAllTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'set-approval-for-all',
      functionArgs: [standardPrincipalCV(senderAddress), boolCV(true)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const setApprovalForAllResult = await broadcastTransaction({ transaction: setApprovalForAllTx, network: networkForBroadcast });
    console.log(`set-approval-for-all tx: ${setApprovalForAllResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const delegateAccessTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const delegateAccessResult = await broadcastTransaction({ transaction: delegateAccessTx, network: networkForBroadcast });
    console.log(`delegate-access-to tx: ${delegateAccessResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const revokeAuthorizationTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const revokeAuthorizationResult = await broadcastTransaction({ transaction: revokeAuthorizationTx, network: networkForBroadcast });
    console.log(`revoke-authorization tx: ${revokeAuthorizationResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
