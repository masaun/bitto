...
const contractDetails = parseContractIdentifier(process.env.REFUNDABLE_TOKEN_CONTRACT_ADDRESS, 'refundable-token');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const setRefundConfigTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'set-refund-config',
      functionArgs: [uintCV(50), uintCV(999999999)],
    ...
    const setRefundConfigResult = await broadcastTransaction({ transaction: setRefundConfigTx, network: networkForBroadcast });
    console.log(`set-refund-config tx: ${setRefundConfigResult.txid}`);
    nonce++;
    ...
    const mintTx = await makeContractCall({
      ...
      contractName: CONFIG.contractName,
      functionName: 'mint',
      functionArgs: [uintCV(1000), standardPrincipalCV(senderAddress)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const refundResult = await broadcastTransaction({ transaction: refundTx, network: networkForBroadcast });
    console.log(`refund tx: ${refundResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const refundFromTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'refund-from',
      functionArgs: [standardPrincipalCV(senderAddress), uintCV(25)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const refundFromResult = await broadcastTransaction({ transaction: refundFromTx, network: networkForBroadcast });
    console.log(`refund-from tx: ${refundFromResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
  }
