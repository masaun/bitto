...
const contractDetails = parseContractIdentifier(
  process.env.CONSUMABLE_NFT_CONTRACT_ADDRESS,
  'consumable-nft'
);
  ...
  return [
    {
      ...
      args: (cycle: number, senderAddress: string, _tokenId: number) => [
        ...
        stringUtf8CV(`https://api.consumable.io/ticket/${cycle}-${Date.now()}`),
      ],
    },
    {
      name: 'set-authorized-consumer',
      args: (cycle: number, senderAddress: string, _tokenId: number) => [
        principalCV(senderAddress),
        boolCV(cycle % 2 === 0),
      ],
    },
    {
      ...
      args: (cycle: number, _senderAddress: string, _tokenId: number) => [
        stringAsciiCV(`https://api.consumable.io/v${cycle + 1}/`),
      ],
    },
    {
      name: 'mint',
      args: (cycle: number, senderAddress: string, _tokenId: number) => [
        principalCV(senderAddress),
        stringUtf8CV(`https://api.consumable.io/ticket/extra-${cycle}-${Date.now()}`),
      ],
    },
    {
      name: 'transfer',
    ...
    },
    {
      ...
      args: (_cycle: number, senderAddress: string, tokenId: number) => [
        ...
        uintCV(tokenId),
        uintCV(1),
        bufferCV(Buffer.from(`consumed-at-cycle-${_cycle}`)),
      ],
    },
    {
...
async function executeAllCycles(): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Consumable NFT Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Consumable NFT Contract - Batch Call Script');
  ...
  console.log(`Cycles: ${CONFIG.cycles}`);
  console.log(`Environment Variable: CONSUMABLE_NFT_CONTRACT_ADDRESS`);

  if (!process.env.CONSUMABLE_NFT_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CONSUMABLE_NFT_CONTRACT_ADDRESS not set in environment.');
