...
const contractDetails = parseContractIdentifier(process.env.CAPPABLE_TOKEN_CONTRACT_ADDRESS, 'cappable-token');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const setMaxSupplyTx = await makeContractCall({
    ...
    const setMaxSupplyResult = await broadcastTransaction({ transaction: setMaxSupplyTx, network: networkForBroadcast });
    console.log(`set-max-supply tx: ${setMaxSupplyResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const setTransferFeeTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const setTransferFeeResult = await broadcastTransaction({ transaction: setTransferFeeTx, network: networkForBroadcast });
    console.log(`set-transfer-fee tx: ${setTransferFeeResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
    ...
    const transferTx = await makeContractCall({
      ...
      functionArgs: [uintCV(100), standardPrincipalCV(senderAddress), standardPrincipalCV(senderAddress), noneCV()],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const withdrawFeesTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const withdrawFeesResult = await broadcastTransaction({ transaction: withdrawFeesTx, network: networkForBroadcast });
    console.log(`withdraw-fees tx: ${withdrawFeesResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
  }
