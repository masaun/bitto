...
const contractDetails = parseContractIdentifier(
  process.env.NESTABLE_NFT_CONTRACT_ADDRESS,
  'nestable-nft'
);
  ...
  return [
    {
      ...
      description: 'Mint new nestable NFT',
    },
    {
      name: 'nest-mint',
      args: () => [uintCV(1), principalCV(recipientAddress), stringAsciiCV('ipfs://nested-uri')],
      description: 'Mint NFT as child of parent token',
    },
    {
      name: 'transfer',
    ...
    {
      name: 'nest-transfer',
      args: () => [uintCV(1), principalCV(senderAddress), uintCV(2)],
      description: 'Transfer NFT to parent token',
    },
    {
      name: 'accept-child',
      args: () => [uintCV(1), uintCV(0), uintCV(2), principalCV(senderAddress)],
      description: 'Accept pending child token',
    },
    {
      name: 'reject-all-children',
      args: () => [uintCV(1), uintCV(10)],
      description: 'Reject all pending children',
    },
    {
      name: 'burn',
      args: () => [uintCV(1), uintCV(10)],
...
async function executeBatchCycles(): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for nestable-nft ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Nestable NFT - Batch Call Script');
  console.log('==========================================================');
