...
const contractDetails = parseContractIdentifier(
  process.env.MULTI_REDEMPTION_NFT_CONTRACT_ADDRESS,
...
function getWriteFunctions(senderAddress: string, recipientAddress: string): FunctionCall[] {
  const redemptionId = Buffer.from('01'.repeat(32), 'hex');
  ...
  return [
    ...
    {
      name: 'redeem',
      args: () => [bufferCV(redemptionId), uintCV(1), stringUtf8CV('redeem memo')],
      description: 'Redeem NFT with ID',
    },
    {
      name: 'cancel',
      args: () => [bufferCV(redemptionId), uintCV(1), stringUtf8CV('cancel memo')],
...
async function executeBatchCycles(): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for multi-redemption-nft ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Multi-Redemption NFT - Batch Call Script');
  console.log('==========================================================');
