...
const contractDetails = parseContractIdentifier(process.env.DESIGNATE_EXECUTOR_CONTRACT_ADDRESS, 'designate-executor');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const setWillTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'set-will',
      functionArgs: [uintCV(tokenId), listCV([standardPrincipalCV(senderAddress)]), uintCV(1000)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const setWillResult = await broadcastTransaction({ transaction: setWillTx, network: networkForBroadcast });
    console.log(`set-will tx: ${setWillResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const announceObitTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const announceObitResult = await broadcastTransaction({ transaction: announceObitTx, network: networkForBroadcast });
    console.log(`announce-obit tx: ${announceObitResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const cancelObitTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const cancelObitResult = await broadcastTransaction({ transaction: cancelObitTx, network: networkForBroadcast });
    console.log(`cancel-obit tx: ${cancelObitResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const bequeathTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const bequeathResult = await broadcastTransaction({ transaction: bequeathTx, network: networkForBroadcast });
    console.log(`bequeath tx: ${bequeathResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
