...
const contractDetails = parseContractIdentifier(
  process.env.KEY_BOUND_NFT_CONTRACT_ADDRESS,
  'key-bound-nft'
);
...
function getWriteFunctions(senderAddress: string, recipientAddress: string): FunctionCall[] {
  return [
    ...
    {
      name: 'add-bindings',
      args: () => [principalCV(recipientAddress), principalCV(senderAddress)],
      description: 'Add key wallet bindings',
    },
    {
      name: 'reset-bindings',
      args: () => [principalCV(senderAddress)],
      description: 'Reset key wallet bindings',
    },
    {
      name: 'safe-fallback',
      args: () => [principalCV(senderAddress)],
      description: 'Safe fallback transfer',
    },
    {
      name: 'allow-transfer',
      args: () => [uintCV(1), uintCV(0), principalCV(recipientAddress), boolCV(false)],
      description: 'Allow transfer',
    },
    {
      name: 'allow-approval',
      args: () => [uintCV(0), uintCV(5)],
      description: 'Allow approval',
    },
  ];
}
...
async function executeBatchCycles(): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for key-bound-nft ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Key-Bound NFT - Batch Call Script');
  console.log('==========================================================');
