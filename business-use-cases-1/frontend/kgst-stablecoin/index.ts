...
const CONTRACT_ADDRESS = process.env.KGST_STABLECOIN_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'kgst-stablecoin';
...
export async function add_minter() {
  const functionArgs = [
...
export async function remove_minter() {
  const functionArgs = [
...
export async function is_minter() {
  const response = await fetch(
    `https://api.mainnet.hiro.so/v2/contracts/call-read/${CONTRACT_ADDRESS.split('.')[0]}/${CONTRACT_NAME}/is-minter`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
