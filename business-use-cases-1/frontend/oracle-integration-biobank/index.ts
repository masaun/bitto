...
const CONTRACT_ADDRESS = process.env.ORACLE_INTEGRATION_BIOBANK_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'oracle-integration-biobank';

export async function connectWallet() {
  showConnect({
    appDetails: {
      name: 'Oracle Integration Biobank',
...
export async function submit_data() {
  const functionArgs = [
...
export async function verify_feed() {
  const functionArgs = [
...
export async function get_feed() {
  const response = await fetch(
    `https://api.mainnet.hiro.so/v2/contracts/call-read/${CONTRACT_ADDRESS.split('.')[0]}/${CONTRACT_NAME}/get-feed`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
