...
const CONTRACT_ADDRESS = process.env.TOKENIZED_ARENA_MARKETPLACE_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'tokenized-arena-marketplace';

export async function connectWallet() {
  showConnect({
    appDetails: {
      name: 'Tokenized Arena Marketplace',
...
export async function list_shares() {
  const functionArgs = [
...
export async function get_shares() {
  const response = await fetch(
    `https://api.mainnet.hiro.so/v2/contracts/call-read/${CONTRACT_ADDRESS.split('.')[0]}/${CONTRACT_NAME}/get-shares`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
