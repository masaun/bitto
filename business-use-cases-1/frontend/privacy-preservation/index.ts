...
const CONTRACT_ADDRESS = process.env.PRIVACY_PRESERVATION_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'privacy-preservation';
...
export async function toggle_enforcement() {
  const functionArgs = [
  ...
  const options = {
    ...
    contractAddress: CONTRACT_ADDRESS.split('.')[0],
    contractName: CONTRACT_NAME,
    functionName: 'toggle-enforcement',
    functionArgs,
    postConditionMode: PostConditionMode.Deny,
    postConditions: [],
    onFinish: (data: any) => {
...
export async function get_policy() {
  const response = await fetch(
    `https://api.mainnet.hiro.so/v2/contracts/call-read/${CONTRACT_ADDRESS.split('.')[0]}/${CONTRACT_NAME}/get-policy`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
