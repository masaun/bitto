...
const CONTRACT_ADDRESS = process.env.FORM_BASED_CODES_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'form-based-codes';
...
export async function create_form_code() {
  const functionArgs = [
...
export async function review_application() {
  const functionArgs = [
...
export async function get_code() {
  const response = await fetch(
    `https://api.mainnet.hiro.so/v2/contracts/call-read/${CONTRACT_ADDRESS.split('.')[0]}/${CONTRACT_NAME}/get-code`,
    {
      method: 'POST',
      headers: {
