...
const CONTRACT_ADDRESS = process.env.INCENTIVE_ZONING_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'incentive-zoning';
...
export async function create_zone() {
  const functionArgs = [
...
export async function get_zone() {
  const response = await fetch(
    `https://api.mainnet.hiro.so/v2/contracts/call-read/${CONTRACT_ADDRESS.split('.')[0]}/${CONTRACT_NAME}/get-zone`,
    {
      method: 'POST',
      headers: {
