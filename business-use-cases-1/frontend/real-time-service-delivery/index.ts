...
const CONTRACT_ADDRESS = process.env.REAL_TIME_SERVICE_DELIVERY_CONTRACT_ADDRESS || '';
const CONTRACT_NAME = 'real-time-service-delivery';
...
export async function assign_request() {
  const functionArgs = [
...
export async function complete_request() {
  const functionArgs = [
...
export async function rate_provider() {
  const functionArgs = [
...
export async function get_service_request() {
  const response = await fetch(
    `https://api.mainnet.hiro.so/v2/contracts/call-read/${CONTRACT_ADDRESS.split('.')[0]}/${CONTRACT_NAME}/get-service-request`,
    {
      method: 'POST',
      headers: {
