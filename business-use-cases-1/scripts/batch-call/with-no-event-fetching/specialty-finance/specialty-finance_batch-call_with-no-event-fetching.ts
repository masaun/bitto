...
const contractDetails = parseContractIdentifier(
  process.env.SPECIALTY_FINANCE_CONTRACT_ADDRESS,
  'specialty-finance'
);
  ...
  return [
    {
      ...
      description: 'Create specialty finance product',
    },
    {
      ...
      description: 'Disburse product funds',
    },
    {
      name: 'make-payment',
      args: (index: number, sender: string) => [
      ...
      description: 'Make payment on product',
    },
    {
      name: 'restructure-product',
      args: (index: number, sender: string) => [
        uintCV(1000000),
        uintCV(1000000),
        uintCV(1000000),
      ],
      description: 'Restructure product terms',
    },
  ];
}
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Specialty Finance Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Specialty Finance Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log(`Environment Variable: SPECIALTY_FINANCE_CONTRACT_ADDRESS`);

  if (!process.env.SPECIALTY_FINANCE_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: SPECIALTY_FINANCE_CONTRACT_ADDRESS not set in environment.');
