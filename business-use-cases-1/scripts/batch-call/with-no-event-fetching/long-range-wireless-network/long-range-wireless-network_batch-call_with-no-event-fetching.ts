...
const contractDetails = parseContractIdentifier(
  process.env.LONG_RANGE_WIRELESS_NETWORK_CONTRACT_ADDRESS,
  ...
  return [
    {
      ...
      description: 'Deploy LoRaWAN gateway',
    },
    {
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Long Range Wireless Network Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Long Range Wireless Network Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log(`Environment Variable: LONG_RANGE_WIRELESS_NETWORK_CONTRACT_ADDRESS`);

  if (!process.env.LONG_RANGE_WIRELESS_NETWORK_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: LONG_RANGE_WIRELESS_NETWORK_CONTRACT_ADDRESS not set in environment.');
