...
const contractDetails = parseContractIdentifier(
  process.env.INCENTIVES_TO_DEPLOY_NEW_SOLAR_PANELS_CONTRACT_ADDRESS,
  'incentives-to-deploy-new-solar-panels'
);
  ...
  return [
    {
      ...
      description: 'Create incentive program',
    },
    {
      ...
      description: 'Register solar installation',
    },
    {
    ...
    },
    {
      ...
      description: 'Claim incentive reward',
    },
  ];
}
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Incentives To Deploy New Solar Panels Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Incentives To Deploy New Solar Panels Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log(`Environment Variable: INCENTIVES_TO_DEPLOY_NEW_SOLAR_PANELS_CONTRACT_ADDRESS`);

  if (!process.env.INCENTIVES_TO_DEPLOY_NEW_SOLAR_PANELS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: INCENTIVES_TO_DEPLOY_NEW_SOLAR_PANELS_CONTRACT_ADDRESS not set in environment.');
