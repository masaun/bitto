...
const contractDetails = parseContractIdentifier(
  process.env.SENIOR_DIRECT_LENDING_CONTRACT_ADDRESS,
  ...
  return [
    {
      ...
      args: (index: number, sender: string) => [
        principalCV(sender),
        uintCV(5000000 + (index * 100000)),
        uintCV(750 + (index * 25)),
        uintCV(8000000 + (index * 150000)),
        uintCV(75 + (index * 2)),
        uintCV(50000 + (index * 1000)),
      ],
      description: 'Create senior direct lending loan',
    },
    {
      name: 'fund-loan',
      args: (index: number) => [
    ...
    },
    {
      ...
      args: (index: number) => [
        uintCV(index + 1),
        uintCV(250000 + (index * 5000)),
      ],
      description: 'Make repayment on loan',
    },
    {
      name: 'close-loan',
      args: (index: number) => [
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Senior Direct Lending Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Senior Direct Lending Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log(`Environment Variable: SENIOR_DIRECT_LENDING_CONTRACT_ADDRESS`);

  if (!process.env.SENIOR_DIRECT_LENDING_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: SENIOR_DIRECT_LENDING_CONTRACT_ADDRESS not set in environment.');
