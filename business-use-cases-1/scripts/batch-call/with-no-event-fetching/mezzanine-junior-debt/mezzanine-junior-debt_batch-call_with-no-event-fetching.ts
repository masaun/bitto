...
const contractDetails = parseContractIdentifier(
  process.env.MEZZANINE_JUNIOR_DEBT_CONTRACT_ADDRESS,
  ...
  return [
    {
      ...
      args: (index: number, sender: string) => [
        principalCV(sender),
        uintCV(index),
        uintCV(10000000 + (index * 200000)),
        uintCV(1000 + (index * 50)),
        uintCV(144000 + (index * 1000)),
      ],
      description: 'Create debt tranche with subordination',
    },
    {
      name: 'invest',
      args: (index: number) => [
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Mezzanine Junior Debt Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Mezzanine Junior Debt Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log(`Environment Variable: MEZZANINE_JUNIOR_DEBT_CONTRACT_ADDRESS`);

  if (!process.env.MEZZANINE_JUNIOR_DEBT_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: MEZZANINE_JUNIOR_DEBT_CONTRACT_ADDRESS not set in environment.');
