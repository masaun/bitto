...
const contractDetails = parseContractIdentifier(
  process.env.SMALL_MODULAR_NUCLEAR_REACTORS_CONTRACT_ADDRESS,
  ...
  return [
    {
      ...
      description: 'Deploy SMR',
    },
    {
      name: 'create-power-contract',
      args: (index: number, sender: string) => [
        uintCV(1000000),
        principalCV(sender),
        uintCV(1000000),
      ...
      description: 'Create power contract',
    },
    {
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Small Modular Nuclear Reactors Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Small Modular Nuclear Reactors Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log(`Environment Variable: SMALL_MODULAR_NUCLEAR_REACTORS_CONTRACT_ADDRESS`);

  if (!process.env.SMALL_MODULAR_NUCLEAR_REACTORS_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: SMALL_MODULAR_NUCLEAR_REACTORS_CONTRACT_ADDRESS not set in environment.');
