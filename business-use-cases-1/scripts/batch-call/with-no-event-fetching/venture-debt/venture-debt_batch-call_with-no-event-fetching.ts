...
const contractDetails = parseContractIdentifier(
  process.env.VENTURE_DEBT_CONTRACT_ADDRESS,
  'venture-debt'
);
  ...
  return [
    {
      ...
      description: 'Issue venture debt with warrants',
    },
    {
      name: 'disburse-funds',
      args: (index: number, sender: string) => [
      ...
      description: 'Disburse debt funds',
    },
    {
      name: 'issue-warrant',
      args: (index: number, sender: string) => [
      ...
      description: 'Issue warrant to holder',
    },
    {
      name: 'repay',
      args: (index: number, sender: string) => [
    ...
    {
      name: 'exercise-warrant',
      args: (index: number, sender: string) => [
        uintCV(1000000),
        uintCV(1000000),
        uintCV(1000000),
      ],
      description: 'Exercise warrant',
    },
  ];
}
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Venture Debt Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Venture Debt Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log(`Environment Variable: VENTURE_DEBT_CONTRACT_ADDRESS`);

  if (!process.env.VENTURE_DEBT_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: VENTURE_DEBT_CONTRACT_ADDRESS not set in environment.');
