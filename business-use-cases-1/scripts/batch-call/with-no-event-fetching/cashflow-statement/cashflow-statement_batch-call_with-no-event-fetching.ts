...
const contractDetails = parseContractIdentifier(
  process.env.CASHFLOW_STATEMENT_CONTRACT_ADDRESS,
  'cashflow-statement'
);
...
function getWriteFunctions(senderAddress: string): FunctionCall[] {
  return [
    ...
    },
    {
      ...
      args: (index: number, sender: string) => [
        principalCV(sender),  // company
        uintCV(index + 1),  // period
        intCV(50000 + (index * 1000)),  // operating
        intCV(-20000 - (index * 500)),  // investing
        intCV(15000 + (index * 750)),  // financing
        uintCV(100000 + (index * 5000)),  // beginning
        uintCV(145000 + (index * 6250)),  // ending
      ],
      description: 'Submit cashflow statement',
    },
    {
      name: 'update-operating-activities',
      args: (index: number, sender: string) => [
        principalCV(sender),  // company
        uintCV(index + 1),  // period
        intCV(55000 + (index * 1100)),  // new-amount
      ],
      description: 'Update operating activities',
    },
    {
      name: 'update-investing-activities',
      args: (index: number, sender: string) => [
        principalCV(sender),  // company
        uintCV(index + 1),  // period
        intCV(-22000 - (index * 550)),  // new-amount
      ],
      description: 'Update investing activities',
    },
    {
      name: 'update-financing-activities',
      args: (index: number, sender: string) => [
        principalCV(sender),  // company
        uintCV(index + 1),  // period
        intCV(16500 + (index * 825)),  // new-amount
      ],
      description: 'Update financing activities',
    },
  ];
}
...
async function executeAllBatchCalls(dryRun: boolean = false, specificFunction?: string): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Cashflow Statement Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Cashflow Statement Contract - Batch Call Script');
  ...
  console.log(`Batch Size: ${CONFIG.batchSize} calls per function`);
  console.log('Environment Variable: CASHFLOW_STATEMENT_CONTRACT_ADDRESS');

  if (!process.env.CASHFLOW_STATEMENT_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: CASHFLOW_STATEMENT_CONTRACT_ADDRESS not set in environment.');
