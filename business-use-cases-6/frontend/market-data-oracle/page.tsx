...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MARKET_DATA_ORACLE_ADDRESS || '';

export default function Market_Data_OraclePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.market-data-oracle/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'market-data-oracle',
