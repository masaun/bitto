...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NIGHTCORE_SECRET_BUFFER_ADDRESS || '';

export default function Nightcore_Secret_BufferPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.nightcore-secret-buffer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'nightcore-secret-buffer',
