...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MINERAL_RIGHTS_MGR_ADDRESS || '';

export default function Mineral_Rights_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.mineral-rights-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'mineral-rights-mgr',
