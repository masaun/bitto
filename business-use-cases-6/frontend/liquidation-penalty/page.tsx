...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LIQUIDATION_PENALTY_ADDRESS || '';

export default function Liquidation_PenaltyPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.liquidation-penalty/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'liquidation-penalty',
