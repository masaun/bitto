...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MARBLESEAL_CONFIDENTIAL_BRIDGE_ADDRESS || '';

export default function Marbleseal_Confidential_BridgePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'marbleseal-confidential-bridge',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.marbleseal-confidential-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'marbleseal-confidential-bridge',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>marbleseal-confidential-bridge</h1>
