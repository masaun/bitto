...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LATENCY_TRACKER_BOT_ADDRESS || '';

export default function Latency_Tracker_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.latency-tracker-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'latency-tracker-bot',
