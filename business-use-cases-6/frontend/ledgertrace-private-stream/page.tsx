...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LEDGERTRACE_PRIVATE_STREAM_ADDRESS || '';

export default function Ledgertrace_Private_StreamPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'ledgertrace-private-stream',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ledgertrace-private-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ledgertrace-private-stream',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>ledgertrace-private-stream</h1>
