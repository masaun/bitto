...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MULTI_PARTY_COMPUTE_ADDRESS || '';

export default function Multi_Party_ComputePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.multi-party-compute/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'multi-party-compute',
