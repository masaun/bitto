...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INTERGOVERNMENTAL_TRANSFER_ADDRESS || '';

export default function Intergovernmental_TransferPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'intergovernmental-transfer',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.intergovernmental-transfer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'intergovernmental-transfer',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>intergovernmental-transfer</h1>
