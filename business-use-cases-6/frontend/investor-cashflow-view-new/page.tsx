...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INVESTOR_CASHFLOW_VIEW_NEW_ADDRESS || '';

export default function Investor_Cashflow_View_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'investor-cashflow-view-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.investor-cashflow-view-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'investor-cashflow-view-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>investor-cashflow-view-new</h1>
