...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LITIGATION_RISK_MOD_ADDRESS || '';

export default function Litigation_Risk_ModPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.litigation-risk-mod/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'litigation-risk-mod',
