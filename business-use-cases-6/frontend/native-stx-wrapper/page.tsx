...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NATIVE_STX_WRAPPER_ADDRESS || '';

export default function Native_Stx_WrapperPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.native-stx-wrapper/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'native-stx-wrapper',
