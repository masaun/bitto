...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MORTGAGE_BACKED_MODULE_NEW_ADDRESS || '';

export default function Mortgage_Backed_Module_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'mortgage-backed-module-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.mortgage-backed-module-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'mortgage-backed-module-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>mortgage-backed-module-new</h1>
