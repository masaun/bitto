...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_IOTA_ROYALTY_STREAM_ADDRESS || '';

export default function Iota_Royalty_StreamPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.iota-royalty-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'iota-royalty-stream',
