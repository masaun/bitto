...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_KYC_AML_GATEKEEPER_ADDRESS || '';

export default function Kyc_Aml_GatekeeperPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.kyc-aml-gatekeeper/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'kyc-aml-gatekeeper',
