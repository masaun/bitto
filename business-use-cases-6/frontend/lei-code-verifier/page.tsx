...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LEI_CODE_VERIFIER_ADDRESS || '';

export default function Lei_Code_VerifierPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.lei-code-verifier/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'lei-code-verifier',
