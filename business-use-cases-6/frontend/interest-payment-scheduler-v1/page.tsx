...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INTEREST_PAYMENT_SCHEDULER_V1_ADDRESS || '';

export default function Interest_Payment_Scheduler_V1Page() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'interest-payment-scheduler-v1',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.interest-payment-scheduler-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'interest-payment-scheduler-v1',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>interest-payment-scheduler-v1</h1>
