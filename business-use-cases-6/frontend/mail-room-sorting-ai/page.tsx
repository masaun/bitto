...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MAIL_ROOM_SORTING_AI_ADDRESS || '';

export default function Mail_Room_Sorting_AiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.mail-room-sorting-ai/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'mail-room-sorting-ai',
