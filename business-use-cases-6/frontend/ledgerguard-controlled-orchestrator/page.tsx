...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LEDGERGUARD_CONTROLLED_ORCHESTRATOR_ADDRESS || '';

export default function Ledgerguard_Controlled_OrchestratorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'ledgerguard-controlled-orchestrator',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ledgerguard-controlled-orchestrator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ledgerguard-controlled-orchestrator',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>ledgerguard-controlled-orchestrator</h1>
