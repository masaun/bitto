...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NIGHTVEILFORGE_SECRET_ROUTER_ADDRESS || '';

export default function Nightveilforge_Secret_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'nightveilforge-secret-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.nightveilforge-secret-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'nightveilforge-secret-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>nightveilforge-secret-router</h1>
