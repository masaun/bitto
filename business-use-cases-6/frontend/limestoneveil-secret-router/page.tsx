...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LIMESTONEVEIL_SECRET_ROUTER_ADDRESS || '';

export default function Limestoneveil_Secret_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'limestoneveil-secret-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.limestoneveil-secret-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'limestoneveil-secret-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>limestoneveil-secret-router</h1>
