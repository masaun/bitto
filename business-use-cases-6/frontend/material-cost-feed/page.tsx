...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MATERIAL_COST_FEED_ADDRESS || '';

export default function Material_Cost_FeedPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.material-cost-feed/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'material-cost-feed',
