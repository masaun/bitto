...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MISTCLOAK_CONFIDENTIAL_ROUTER_ADDRESS || '';

export default function Mistcloak_Confidential_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'mistcloak-confidential-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.mistcloak-confidential-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'mistcloak-confidential-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>mistcloak-confidential-router</h1>
