...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INVESTOR_ONBOARDING_ADDRESS || '';

export default function Investor_OnboardingPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.investor-onboarding/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'investor-onboarding',
