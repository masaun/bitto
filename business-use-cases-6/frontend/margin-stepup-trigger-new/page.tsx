...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MARGIN_STEPUP_TRIGGER_NEW_ADDRESS || '';

export default function Margin_Stepup_Trigger_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'margin-stepup-trigger-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.margin-stepup-trigger-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'margin-stepup-trigger-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>margin-stepup-trigger-new</h1>
