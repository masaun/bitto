...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LOBBY_KIOSK_INTERFACE_ADDRESS || '';

export default function Lobby_Kiosk_InterfacePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.lobby-kiosk-interface/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'lobby-kiosk-interface',
