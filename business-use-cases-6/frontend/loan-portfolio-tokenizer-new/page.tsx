...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LOAN_PORTFOLIO_TOKENIZER_NEW_ADDRESS || '';

export default function Loan_Portfolio_Tokenizer_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'loan-portfolio-tokenizer-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.loan-portfolio-tokenizer-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'loan-portfolio-tokenizer-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>loan-portfolio-tokenizer-new</h1>
