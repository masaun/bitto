...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LINEA_CONSENSYS_V1_ADDRESS || '';

export default function Linea_Consensys_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.linea-consensys-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'linea-consensys-v1',
