...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LIGHT_CLIENT_NODE_ADDRESS || '';

export default function Light_Client_NodePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.light-client-node/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'light-client-node',
