...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MACROECONOMIC_POLICY_LINK_ADDRESS || '';

export default function Macroeconomic_Policy_LinkPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'macroeconomic-policy-link',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.macroeconomic-policy-link/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'macroeconomic-policy-link',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>macroeconomic-policy-link</h1>
