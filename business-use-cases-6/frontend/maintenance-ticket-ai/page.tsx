...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MAINTENANCE_TICKET_AI_ADDRESS || '';

export default function Maintenance_Ticket_AiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.maintenance-ticket-ai/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'maintenance-ticket-ai',
