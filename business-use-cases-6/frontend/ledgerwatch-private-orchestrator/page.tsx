...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LEDGERWATCH_PRIVATE_ORCHESTRATOR_ADDRESS || '';

export default function Ledgerwatch_Private_OrchestratorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'ledgerwatch-private-orchestrator',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ledgerwatch-private-orchestrator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ledgerwatch-private-orchestrator',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>ledgerwatch-private-orchestrator</h1>
