...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INVESTOR_PAYMENT_AGENT_NEW_ADDRESS || '';

export default function Investor_Payment_Agent_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'investor-payment-agent-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.investor-payment-agent-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'investor-payment-agent-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>investor-payment-agent-new</h1>
