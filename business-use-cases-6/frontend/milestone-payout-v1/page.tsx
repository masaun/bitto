...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MILESTONE_PAYOUT_V1_ADDRESS || '';

export default function Milestone_Payout_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.milestone-payout-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'milestone-payout-v1',
