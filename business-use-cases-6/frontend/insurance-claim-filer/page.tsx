...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INSURANCE_CLAIM_FILER_ADDRESS || '';

export default function Insurance_Claim_FilerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.insurance-claim-filer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'insurance-claim-filer',
