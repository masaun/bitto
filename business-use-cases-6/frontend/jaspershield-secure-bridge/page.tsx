...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_JASPERSHIELD_SECURE_BRIDGE_ADDRESS || '';

export default function Jaspershield_Secure_BridgePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'jaspershield-secure-bridge',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.jaspershield-secure-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'jaspershield-secure-bridge',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>jaspershield-secure-bridge</h1>
