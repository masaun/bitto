...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_KEPLERHIDE_TRUSTED_PIPELINE_ADDRESS || '';

export default function Keplerhide_Trusted_PipelinePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'keplerhide-trusted-pipeline',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.keplerhide-trusted-pipeline/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'keplerhide-trusted-pipeline',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>keplerhide-trusted-pipeline</h1>
