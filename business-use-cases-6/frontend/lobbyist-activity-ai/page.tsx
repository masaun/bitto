...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LOBBYIST_ACTIVITY_AI_ADDRESS || '';

export default function Lobbyist_Activity_AiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.lobbyist-activity-ai/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'lobbyist-activity-ai',
