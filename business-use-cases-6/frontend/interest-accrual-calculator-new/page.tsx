...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INTEREST_ACCRUAL_CALCULATOR_NEW_ADDRESS || '';

export default function Interest_Accrual_Calculator_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'interest-accrual-calculator-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.interest-accrual-calculator-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'interest-accrual-calculator-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>interest-accrual-calculator-new</h1>
