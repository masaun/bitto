...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INVESTOR_ONBOARDING_REGISTRY_NEW_ADDRESS || '';

export default function Investor_Onboarding_Registry_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'investor-onboarding-registry-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.investor-onboarding-registry-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'investor-onboarding-registry-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>investor-onboarding-registry-new</h1>
