...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_IRONHIDE_SECRET_RELAY_ADDRESS || '';

export default function Ironhide_Secret_RelayPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ironhide-secret-relay/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ironhide-secret-relay',
