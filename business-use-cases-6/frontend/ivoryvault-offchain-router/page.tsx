...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_IVORYVAULT_OFFCHAIN_ROUTER_ADDRESS || '';

export default function Ivoryvault_Offchain_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'ivoryvault-offchain-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ivoryvault-offchain-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ivoryvault-offchain-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>ivoryvault-offchain-router</h1>
