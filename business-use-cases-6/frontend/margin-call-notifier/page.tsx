...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MARGIN_CALL_NOTIFIER_ADDRESS || '';

export default function Margin_Call_NotifierPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.margin-call-notifier/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'margin-call-notifier',
