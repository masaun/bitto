...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_IOT_GATEWAY_CONTROLLER_ADDRESS || '';

export default function Iot_Gateway_ControllerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.iot-gateway-controller/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'iot-gateway-controller',
