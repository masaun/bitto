...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_KAPPA_REVENUE_SHARE_ADDRESS || '';

export default function Kappa_Revenue_SharePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.kappa-revenue-share/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'kappa-revenue-share',
