...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INSTITUTIONAL_TRANCHE_HUB_NEW_ADDRESS || '';

export default function Institutional_Tranche_Hub_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'institutional-tranche-hub-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.institutional-tranche-hub-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'institutional-tranche-hub-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>institutional-tranche-hub-new</h1>
