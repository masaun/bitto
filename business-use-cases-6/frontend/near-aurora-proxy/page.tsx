...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NEAR_AURORA_PROXY_ADDRESS || '';

export default function Near_Aurora_ProxyPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.near-aurora-proxy/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'near-aurora-proxy',
