...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MAP_LAYER_INTEGRATOR_ADDRESS || '';

export default function Map_Layer_IntegratorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.map-layer-integrator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'map-layer-integrator',
