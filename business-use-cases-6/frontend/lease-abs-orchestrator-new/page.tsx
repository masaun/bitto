...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LEASE_ABS_ORCHESTRATOR_NEW_ADDRESS || '';

export default function Lease_Abs_Orchestrator_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'lease-abs-orchestrator-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.lease-abs-orchestrator-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'lease-abs-orchestrator-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>lease-abs-orchestrator-new</h1>
