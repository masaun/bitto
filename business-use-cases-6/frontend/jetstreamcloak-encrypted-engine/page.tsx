...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_JETSTREAMCLOAK_ENCRYPTED_ENGINE_ADDRESS || '';

export default function Jetstreamcloak_Encrypted_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'jetstreamcloak-encrypted-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.jetstreamcloak-encrypted-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'jetstreamcloak-encrypted-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>jetstreamcloak-encrypted-engine</h1>
