...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INFRASTRUCTURE_FUNDING_CORE_ADDRESS || '';

export default function Infrastructure_Funding_CorePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'infrastructure-funding-core',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.infrastructure-funding-core/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'infrastructure-funding-core',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>infrastructure-funding-core</h1>
