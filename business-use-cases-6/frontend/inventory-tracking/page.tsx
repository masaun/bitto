...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INVENTORY_TRACKING_ADDRESS || '';

export default function Inventory_TrackingPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.inventory-tracking/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'inventory-tracking',
