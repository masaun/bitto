...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INSURANCE_PREMIUM_FINANCE_NEW_ADDRESS || '';

export default function Insurance_Premium_Finance_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'insurance-premium-finance-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.insurance-premium-finance-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'insurance-premium-finance-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>insurance-premium-finance-new</h1>
