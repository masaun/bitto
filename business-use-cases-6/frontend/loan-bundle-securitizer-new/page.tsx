...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LOAN_BUNDLE_SECURITIZER_NEW_ADDRESS || '';

export default function Loan_Bundle_Securitizer_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'loan-bundle-securitizer-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.loan-bundle-securitizer-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'loan-bundle-securitizer-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>loan-bundle-securitizer-new</h1>
