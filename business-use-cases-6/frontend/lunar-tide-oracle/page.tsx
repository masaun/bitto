...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LUNAR_TIDE_ORACLE_ADDRESS || '';

export default function Lunar_Tide_OraclePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.lunar-tide-oracle/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'lunar-tide-oracle',
