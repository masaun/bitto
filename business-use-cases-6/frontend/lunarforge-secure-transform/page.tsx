...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LUNARFORGE_SECURE_TRANSFORM_ADDRESS || '';

export default function Lunarforge_Secure_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'lunarforge-secure-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.lunarforge-secure-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'lunarforge-secure-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>lunarforge-secure-transform</h1>
