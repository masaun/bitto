...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LOGIFORGE_PRIVATE_EXEC_ADDRESS || '';

export default function Logiforge_Private_ExecPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.logiforge-private-exec/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'logiforge-private-exec',
