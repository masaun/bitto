...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INVESTOR_ALLOCATION_TRACKER_NEW_ADDRESS || '';

export default function Investor_Allocation_Tracker_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'investor-allocation-tracker-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.investor-allocation-tracker-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'investor-allocation-tracker-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>investor-allocation-tracker-new</h1>
