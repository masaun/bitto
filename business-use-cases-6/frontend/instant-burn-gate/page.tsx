...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INSTANT_BURN_GATE_ADDRESS || '';

export default function Instant_Burn_GatePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.instant-burn-gate/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'instant-burn-gate',
