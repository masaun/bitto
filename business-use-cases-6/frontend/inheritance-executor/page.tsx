...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INHERITANCE_EXECUTOR_ADDRESS || '';

export default function Inheritance_ExecutorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.inheritance-executor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'inheritance-executor',
