...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_IRONVEILFORGE_PRIVATE_ROUTER_ADDRESS || '';

export default function Ironveilforge_Private_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'ironveilforge-private-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ironveilforge-private-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ironveilforge-private-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>ironveilforge-private-router</h1>
