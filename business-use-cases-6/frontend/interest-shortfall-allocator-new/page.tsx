...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INTEREST_SHORTFALL_ALLOCATOR_NEW_ADDRESS || '';

export default function Interest_Shortfall_Allocator_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'interest-shortfall-allocator-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.interest-shortfall-allocator-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'interest-shortfall-allocator-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>interest-shortfall-allocator-new</h1>
