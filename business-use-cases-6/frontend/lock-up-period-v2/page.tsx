...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LOCK_UP_PERIOD_V2_ADDRESS || '';

export default function Lock_Up_Period_V2Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.lock-up-period-v2/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'lock-up-period-v2',
