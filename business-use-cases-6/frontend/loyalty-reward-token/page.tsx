...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LOYALTY_REWARD_TOKEN_ADDRESS || '';

export default function Loyalty_Reward_TokenPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.loyalty-reward-token/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'loyalty-reward-token',
