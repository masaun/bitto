...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LOAN_LEVEL_REPORTING_NEW_ADDRESS || '';

export default function Loan_Level_Reporting_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.loan-level-reporting-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'loan-level-reporting-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>loan-level-reporting-new</h1>
