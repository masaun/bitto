...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LOCAL_GOVERNMENT_TRANSFER_ADDRESS || '';

export default function Local_Government_TransferPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'local-government-transfer',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.local-government-transfer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'local-government-transfer',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>local-government-transfer</h1>
