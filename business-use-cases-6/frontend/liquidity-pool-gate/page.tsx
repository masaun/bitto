...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LIQUIDITY_POOL_GATE_ADDRESS || '';

export default function Liquidity_Pool_GatePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.liquidity-pool-gate/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'liquidity-pool-gate',
