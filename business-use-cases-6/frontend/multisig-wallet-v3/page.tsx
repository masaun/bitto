...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MULTISIG_WALLET_V3_ADDRESS || '';

export default function Multisig_Wallet_V3Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.multisig-wallet-v3/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'multisig-wallet-v3',
