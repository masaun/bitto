...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INNOVATION_GRANT_MANAGER_ADDRESS || '';

export default function Innovation_Grant_ManagerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.innovation-grant-manager/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'innovation-grant-manager',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>innovation-grant-manager</h1>
