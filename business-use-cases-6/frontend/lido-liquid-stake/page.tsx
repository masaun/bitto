...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LIDO_LIQUID_STAKE_ADDRESS || '';

export default function Lido_Liquid_StakePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.lido-liquid-stake/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'lido-liquid-stake',
