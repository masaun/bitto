...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_KEY_SHREDDER_TOOL_ADDRESS || '';

export default function Key_Shredder_ToolPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.key-shredder-tool/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'key-shredder-tool',
