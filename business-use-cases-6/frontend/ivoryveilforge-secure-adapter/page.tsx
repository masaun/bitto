...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_IVORYVEILFORGE_SECURE_ADAPTER_ADDRESS || '';

export default function Ivoryveilforge_Secure_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'ivoryveilforge-secure-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ivoryveilforge-secure-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ivoryveilforge-secure-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>ivoryveilforge-secure-adapter</h1>
