...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_KYC_COMPLIANCE_GATE_ADDRESS || '';

export default function Kyc_Compliance_GatePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.kyc-compliance-gate/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'kyc-compliance-gate',
