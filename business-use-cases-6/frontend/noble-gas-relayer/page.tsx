...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NOBLE_GAS_RELAYER_ADDRESS || '';

export default function Noble_Gas_RelayerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.noble-gas-relayer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'noble-gas-relayer',
