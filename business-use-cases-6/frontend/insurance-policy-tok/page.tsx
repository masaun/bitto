...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INSURANCE_POLICY_TOK_ADDRESS || '';

export default function Insurance_Policy_TokPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.insurance-policy-tok/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'insurance-policy-tok',
