...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LIFECYCLE_EVENT_LOGGER_ADDRESS || '';

export default function Lifecycle_Event_LoggerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.lifecycle-event-logger/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'lifecycle-event-logger',
