...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MICROFINANCE_BACKED_NOTE_NEW_ADDRESS || '';

export default function Microfinance_Backed_Note_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'microfinance-backed-note-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.microfinance-backed-note-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'microfinance-backed-note-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>microfinance-backed-note-new</h1>
