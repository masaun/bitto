...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_KYC_VERIFICATION_ADAPTER_NEW_ADDRESS || '';

export default function Kyc_Verification_Adapter_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'kyc-verification-adapter-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.kyc-verification-adapter-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'kyc-verification-adapter-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>kyc-verification-adapter-new</h1>
