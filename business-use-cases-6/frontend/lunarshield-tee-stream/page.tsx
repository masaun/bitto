...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LUNARSHIELD_TEE_STREAM_ADDRESS || '';

export default function Lunarshield_Tee_StreamPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.lunarshield-tee-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'lunarshield-tee-stream',
