...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LABOR_MARKET_TRACKER_ADDRESS || '';

export default function Labor_Market_TrackerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.labor-market-tracker/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'labor-market-tracker',
