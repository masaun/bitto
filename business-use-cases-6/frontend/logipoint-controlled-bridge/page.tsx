...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LOGIPOINT_CONTROLLED_BRIDGE_ADDRESS || '';

export default function Logipoint_Controlled_BridgePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'logipoint-controlled-bridge',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.logipoint-controlled-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'logipoint-controlled-bridge',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>logipoint-controlled-bridge</h1>
