...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LEGACY_TOKEN_SWAP_ADDRESS || '';

export default function Legacy_Token_SwapPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.legacy-token-swap/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'legacy-token-swap',
