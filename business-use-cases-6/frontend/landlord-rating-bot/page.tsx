...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LANDLORD_RATING_BOT_ADDRESS || '';

export default function Landlord_Rating_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.landlord-rating-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'landlord-rating-bot',
