...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MEV_PROTECTION_V1_ADDRESS || '';

export default function Mev_Protection_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.mev-protection-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'mev-protection-v1',
