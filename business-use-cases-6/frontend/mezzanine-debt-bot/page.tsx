...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MEZZANINE_DEBT_BOT_ADDRESS || '';

export default function Mezzanine_Debt_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.mezzanine-debt-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'mezzanine-debt-bot',
