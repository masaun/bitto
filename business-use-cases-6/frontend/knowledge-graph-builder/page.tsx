...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_KNOWLEDGE_GRAPH_BUILDER_ADDRESS || '';

export default function Knowledge_Graph_BuilderPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.knowledge-graph-builder/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'knowledge-graph-builder',
