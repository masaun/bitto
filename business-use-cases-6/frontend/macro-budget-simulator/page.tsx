...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MACRO_BUDGET_SIMULATOR_ADDRESS || '';

export default function Macro_Budget_SimulatorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.macro-budget-simulator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'macro-budget-simulator',
