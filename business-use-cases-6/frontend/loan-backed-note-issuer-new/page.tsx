...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LOAN_BACKED_NOTE_ISSUER_NEW_ADDRESS || '';

export default function Loan_Backed_Note_Issuer_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'loan-backed-note-issuer-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.loan-backed-note-issuer-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'loan-backed-note-issuer-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>loan-backed-note-issuer-new</h1>
