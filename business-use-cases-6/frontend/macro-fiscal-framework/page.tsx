...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MACRO_FISCAL_FRAMEWORK_ADDRESS || '';

export default function Macro_Fiscal_FrameworkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.macro-fiscal-framework/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'macro-fiscal-framework',
