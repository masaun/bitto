...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LOGISEC_TRUSTED_RUNNER_ADDRESS || '';

export default function Logisec_Trusted_RunnerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.logisec-trusted-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'logisec-trusted-runner',
