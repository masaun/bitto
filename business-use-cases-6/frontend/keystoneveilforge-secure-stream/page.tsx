...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_KEYSTONEVEILFORGE_SECURE_STREAM_ADDRESS || '';

export default function Keystoneveilforge_Secure_StreamPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'keystoneveilforge-secure-stream',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.keystoneveilforge-secure-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'keystoneveilforge-secure-stream',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>keystoneveilforge-secure-stream</h1>
