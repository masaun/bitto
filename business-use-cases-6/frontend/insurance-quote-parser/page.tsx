...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INSURANCE_QUOTE_PARSER_ADDRESS || '';

export default function Insurance_Quote_ParserPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.insurance-quote-parser/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'insurance-quote-parser',
