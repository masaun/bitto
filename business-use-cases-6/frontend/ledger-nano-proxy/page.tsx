...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LEDGER_NANO_PROXY_ADDRESS || '';

export default function Ledger_Nano_ProxyPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ledger-nano-proxy/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ledger-nano-proxy',
