...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NEBULARGUARD_OFFCHAIN_HARVESTER_ADDRESS || '';

export default function Nebularguard_Offchain_HarvesterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'nebularguard-offchain-harvester',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.nebularguard-offchain-harvester/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'nebularguard-offchain-harvester',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>nebularguard-offchain-harvester</h1>
