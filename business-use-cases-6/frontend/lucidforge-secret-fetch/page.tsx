...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LUCIDFORGE_SECRET_FETCH_ADDRESS || '';

export default function Lucidforge_Secret_FetchPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.lucidforge-secret-fetch/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'lucidforge-secret-fetch',
