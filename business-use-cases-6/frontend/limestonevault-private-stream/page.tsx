...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LIMESTONEVAULT_PRIVATE_STREAM_ADDRESS || '';

export default function Limestonevault_Private_StreamPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'limestonevault-private-stream',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.limestonevault-private-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'limestonevault-private-stream',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>limestonevault-private-stream</h1>
