...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LANDSCAPING_SCHEDULE_ADDRESS || '';

export default function Landscaping_SchedulePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.landscaping-schedule/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'landscaping-schedule',
