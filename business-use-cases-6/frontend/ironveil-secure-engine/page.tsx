...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_IRONVEIL_SECURE_ENGINE_ADDRESS || '';

export default function Ironveil_Secure_EnginePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ironveil-secure-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ironveil-secure-engine',
