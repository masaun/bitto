...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LOAN_TO_VALUE_GUARD_ADDRESS || '';

export default function Loan_To_Value_GuardPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.loan-to-value-guard/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'loan-to-value-guard',
