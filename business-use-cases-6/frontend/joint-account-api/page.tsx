...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_JOINT_ACCOUNT_API_ADDRESS || '';

export default function Joint_Account_ApiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.joint-account-api/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'joint-account-api',
