...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LEAD_PAINT_RECORD_ADDRESS || '';

export default function Lead_Paint_RecordPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.lead-paint-record/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'lead-paint-record',
