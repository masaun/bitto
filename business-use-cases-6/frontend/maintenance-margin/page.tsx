...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MAINTENANCE_MARGIN_ADDRESS || '';

export default function Maintenance_MarginPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.maintenance-margin/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'maintenance-margin',
