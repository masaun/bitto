...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NOISE_COMPLAINT_BOT_ADDRESS || '';

export default function Noise_Complaint_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.noise-complaint-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'noise-complaint-bot',
