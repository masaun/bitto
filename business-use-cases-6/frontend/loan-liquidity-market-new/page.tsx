...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LOAN_LIQUIDITY_MARKET_NEW_ADDRESS || '';

export default function Loan_Liquidity_Market_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'loan-liquidity-market-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.loan-liquidity-market-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'loan-liquidity-market-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>loan-liquidity-market-new</h1>
