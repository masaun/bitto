...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MAKER_DAO_SURPLUS_ADDRESS || '';

export default function Maker_Dao_SurplusPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.maker-dao-surplus/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'maker-dao-surplus',
