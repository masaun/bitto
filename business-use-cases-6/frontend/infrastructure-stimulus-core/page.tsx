...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INFRASTRUCTURE_STIMULUS_CORE_ADDRESS || '';

export default function Infrastructure_Stimulus_CorePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'infrastructure-stimulus-core',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.infrastructure-stimulus-core/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'infrastructure-stimulus-core',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>infrastructure-stimulus-core</h1>
