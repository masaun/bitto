...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_JANITORIAL_SERVICE_AI_ADDRESS || '';

export default function Janitorial_Service_AiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.janitorial-service-ai/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'janitorial-service-ai',
