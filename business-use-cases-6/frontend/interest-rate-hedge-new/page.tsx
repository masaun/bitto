...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INTEREST_RATE_HEDGE_NEW_ADDRESS || '';

export default function Interest_Rate_Hedge_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.interest-rate-hedge-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'interest-rate-hedge-new',
