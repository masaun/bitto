...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MIRAGEVAULT_TRUSTED_ADAPTER_ADDRESS || '';

export default function Miragevault_Trusted_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'miragevault-trusted-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.miragevault-trusted-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'miragevault-trusted-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>miragevault-trusted-adapter</h1>
