...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MOONVEILFORGE_SECURE_ROUTER_ADDRESS || '';

export default function Moonveilforge_Secure_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'moonveilforge-secure-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.moonveilforge-secure-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'moonveilforge-secure-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>moonveilforge-secure-router</h1>
