...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LUCIDVEIL_SECURE_PIPELINE_ADDRESS || '';

export default function Lucidveil_Secure_PipelinePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'lucidveil-secure-pipeline',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.lucidveil-secure-pipeline/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'lucidveil-secure-pipeline',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>lucidveil-secure-pipeline</h1>
