...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LOAN_MODIFICATION_REGISTRY_NEW_ADDRESS || '';

export default function Loan_Modification_Registry_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'loan-modification-registry-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.loan-modification-registry-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'loan-modification-registry-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>loan-modification-registry-new</h1>
