...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_MULTI_CURRENCY_TRANCHE_NEW_ADDRESS || '';

export default function Multi_Currency_Tranche_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'multi-currency-tranche-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.multi-currency-tranche-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'multi-currency-tranche-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>multi-currency-tranche-new</h1>
