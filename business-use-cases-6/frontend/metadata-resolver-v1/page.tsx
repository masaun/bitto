...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_METADATA_RESOLVER_V1_ADDRESS || '';

export default function Metadata_Resolver_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.metadata-resolver-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'metadata-resolver-v1',
