...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_IP_RIGHTS_PROTECTION_ADDRESS || '';

export default function Ip_Rights_ProtectionPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ip-rights-protection/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ip-rights-protection',
