...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_IPFS_CONTENT_HASH_ADDRESS || '';

export default function Ipfs_Content_HashPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.ipfs-content-hash/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'ipfs-content-hash',
