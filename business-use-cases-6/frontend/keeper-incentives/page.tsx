...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_KEEPER_INCENTIVES_ADDRESS || '';

export default function Keeper_IncentivesPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.keeper-incentives/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'keeper-incentives',
