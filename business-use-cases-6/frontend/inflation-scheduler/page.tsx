...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_INFLATION_SCHEDULER_ADDRESS || '';

export default function Inflation_SchedulerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.inflation-scheduler/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'inflation-scheduler',
