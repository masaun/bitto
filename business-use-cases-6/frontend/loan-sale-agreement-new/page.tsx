...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LOAN_SALE_AGREEMENT_NEW_ADDRESS || '';

export default function Loan_Sale_Agreement_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.loan-sale-agreement-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'loan-sale-agreement-new',
