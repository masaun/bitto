...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LLM_ACCESS_CONTROLLER_ADDRESS || '';

export default function Llm_Access_ControllerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.llm-access-controller/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'llm-access-controller',
