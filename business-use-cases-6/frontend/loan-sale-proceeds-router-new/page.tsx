...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_LOAN_SALE_PROCEEDS_ROUTER_NEW_ADDRESS || '';

export default function Loan_Sale_Proceeds_Router_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'loan-sale-proceeds-router-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.loan-sale-proceeds-router-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'loan-sale-proceeds-router-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>loan-sale-proceeds-router-new</h1>
