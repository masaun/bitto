...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OPAQUE_RESERVE_CHK_ADDRESS || '';

export default function Opaque_Reserve_ChkPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.opaque-reserve-chk/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'opaque-reserve-chk',
