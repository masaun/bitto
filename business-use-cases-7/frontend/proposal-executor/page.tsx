...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROPOSAL_EXECUTOR_ADDRESS || '';

export default function Proposal_ExecutorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.proposal-executor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'proposal-executor',
