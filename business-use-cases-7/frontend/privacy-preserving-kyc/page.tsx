...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PRIVACY_PRESERVING_KYC_ADDRESS || '';

export default function Privacy_Preserving_KycPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.privacy-preserving-kyc/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'privacy-preserving-kyc',
