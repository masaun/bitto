...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NORTHVEIL_SECURE_FETCHER_ADDRESS || '';

export default function Northveil_Secure_FetcherPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.northveil-secure-fetcher/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'northveil-secure-fetcher',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>northveil-secure-fetcher</h1>
