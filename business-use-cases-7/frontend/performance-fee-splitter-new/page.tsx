...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PERFORMANCE_FEE_SPLITTER_NEW_ADDRESS || '';

export default function Performance_Fee_Splitter_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'performance-fee-splitter-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.performance-fee-splitter-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'performance-fee-splitter-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>performance-fee-splitter-new</h1>
