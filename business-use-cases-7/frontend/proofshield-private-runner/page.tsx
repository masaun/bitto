...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROOFSHIELD_PRIVATE_RUNNER_ADDRESS || '';

export default function Proofshield_Private_RunnerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'proofshield-private-runner',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.proofshield-private-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'proofshield-private-runner',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>proofshield-private-runner</h1>
