...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PARKING_LOT_MANAGER_ADDRESS || '';

export default function Parking_Lot_ManagerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.parking-lot-manager/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'parking-lot-manager',
