...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NOTE_AUCTION_ENGINE_NEW_ADDRESS || '';

export default function Note_Auction_Engine_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.note-auction-engine-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'note-auction-engine-new',
