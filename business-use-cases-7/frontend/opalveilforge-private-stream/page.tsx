...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OPALVEILFORGE_PRIVATE_STREAM_ADDRESS || '';

export default function Opalveilforge_Private_StreamPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'opalveilforge-private-stream',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.opalveilforge-private-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'opalveilforge-private-stream',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>opalveilforge-private-stream</h1>
