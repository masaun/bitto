...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OMICRON_OPTION_VAULT_ADDRESS || '';

export default function Omicron_Option_VaultPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.omicron-option-vault/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'omicron-option-vault',
