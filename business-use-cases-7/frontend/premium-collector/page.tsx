...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PREMIUM_COLLECTOR_ADDRESS || '';

export default function Premium_CollectorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.premium-collector/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'premium-collector',
