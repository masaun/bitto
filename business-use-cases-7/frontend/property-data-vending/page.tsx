...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROPERTY_DATA_VENDING_ADDRESS || '';

export default function Property_Data_VendingPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.property-data-vending/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'property-data-vending',
