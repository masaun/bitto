...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PEER_DISCOVERY_SVC_ADDRESS || '';

export default function Peer_Discovery_SvcPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.peer-discovery-svc/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'peer-discovery-svc',
