...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PRINCIPAL_DEFERRAL_ENGINE_NEW_ADDRESS || '';

export default function Principal_Deferral_Engine_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'principal-deferral-engine-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.principal-deferral-engine-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'principal-deferral-engine-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>principal-deferral-engine-new</h1>
