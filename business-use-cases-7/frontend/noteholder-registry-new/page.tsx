...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NOTEHOLDER_REGISTRY_NEW_ADDRESS || '';

export default function Noteholder_Registry_NewPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.noteholder-registry-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'noteholder-registry-new',
