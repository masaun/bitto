...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PARTIAL_FILL_LOGIC_ADDRESS || '';

export default function Partial_Fill_LogicPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.partial-fill-logic/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'partial-fill-logic',
