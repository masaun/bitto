...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NOTARY_DIGITAL_GATE_ADDRESS || '';

export default function Notary_Digital_GatePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.notary-digital-gate/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'notary-digital-gate',
