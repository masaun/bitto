...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROOFSIGNAL_PRIVATE_ROUTER_ADDRESS || '';

export default function Proofsignal_Private_RouterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'proofsignal-private-router',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.proofsignal-private-router/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'proofsignal-private-router',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>proofsignal-private-router</h1>
