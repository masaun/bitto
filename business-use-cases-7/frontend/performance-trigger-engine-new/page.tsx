...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PERFORMANCE_TRIGGER_ENGINE_NEW_ADDRESS || '';

export default function Performance_Trigger_Engine_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'performance-trigger-engine-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.performance-trigger-engine-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'performance-trigger-engine-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>performance-trigger-engine-new</h1>
