...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROOFCREST_SECURE_HUB_ADDRESS || '';

export default function Proofcrest_Secure_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.proofcrest-secure-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'proofcrest-secure-hub',
