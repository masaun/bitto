...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PERMIT_TRACKING_AI_ADDRESS || '';

export default function Permit_Tracking_AiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.permit-tracking-ai/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'permit-tracking-ai',
