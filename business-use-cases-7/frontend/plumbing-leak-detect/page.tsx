...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PLUMBING_LEAK_DETECT_ADDRESS || '';

export default function Plumbing_Leak_DetectPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.plumbing-leak-detect/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'plumbing-leak-detect',
