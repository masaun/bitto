...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OFFSHORE_TAX_ROUTING_ADDRESS || '';

export default function Offshore_Tax_RoutingPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.offshore-tax-routing/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'offshore-tax-routing',
