...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PASSKEY_AUTHENTIC_ADDRESS || '';

export default function Passkey_AuthenticPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.passkey-authentic/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'passkey-authentic',
