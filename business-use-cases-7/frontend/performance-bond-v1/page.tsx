...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PERFORMANCE_BOND_V1_ADDRESS || '';

export default function Performance_Bond_V1Page() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.performance-bond-v1/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'performance-bond-v1',
