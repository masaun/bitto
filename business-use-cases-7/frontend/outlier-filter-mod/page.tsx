...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OUTLIER_FILTER_MOD_ADDRESS || '';

export default function Outlier_Filter_ModPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.outlier-filter-mod/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'outlier-filter-mod',
