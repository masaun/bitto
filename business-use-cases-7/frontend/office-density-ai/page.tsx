...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OFFICE_DENSITY_AI_ADDRESS || '';

export default function Office_Density_AiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.office-density-ai/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'office-density-ai',
