...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PRICING_SPREAD_CALCULATOR_NEW_ADDRESS || '';

export default function Pricing_Spread_Calculator_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'pricing-spread-calculator-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.pricing-spread-calculator-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'pricing-spread-calculator-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>pricing-spread-calculator-new</h1>
