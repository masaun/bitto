...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OCCUPANCY_SENSOR_FEED_ADDRESS || '';

export default function Occupancy_Sensor_FeedPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.occupancy-sensor-feed/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'occupancy-sensor-feed',
