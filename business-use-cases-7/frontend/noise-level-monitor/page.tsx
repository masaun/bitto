...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NOISE_LEVEL_MONITOR_ADDRESS || '';

export default function Noise_Level_MonitorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.noise-level-monitor/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'noise-level-monitor',
