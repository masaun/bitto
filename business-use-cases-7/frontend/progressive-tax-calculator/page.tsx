...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROGRESSIVE_TAX_CALCULATOR_ADDRESS || '';

export default function Progressive_Tax_CalculatorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.progressive-tax-calculator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'progressive-tax-calculator',
