...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PERFORMANCE_METRIC_ADDRESS || '';

export default function Performance_MetricPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.performance-metric/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'performance-metric',
