...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PREMIUM_CALCULATOR_ADDRESS || '';

export default function Premium_CalculatorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.premium-calculator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'premium-calculator',
