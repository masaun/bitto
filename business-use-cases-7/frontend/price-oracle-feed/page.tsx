...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PRICE_ORACLE_FEED_ADDRESS || '';

export default function Price_Oracle_FeedPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.price-oracle-feed/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'price-oracle-feed',
