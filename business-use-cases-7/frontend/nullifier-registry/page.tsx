...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NULLIFIER_REGISTRY_ADDRESS || '';

export default function Nullifier_RegistryPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.nullifier-registry/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'nullifier-registry',
