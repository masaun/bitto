...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PINEVEILFORGE_SECRET_STREAM_ADDRESS || '';

export default function Pineveilforge_Secret_StreamPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'pineveilforge-secret-stream',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.pineveilforge-secret-stream/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'pineveilforge-secret-stream',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>pineveilforge-secret-stream</h1>
