...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PREPAYMENT_SPEED_MODEL_NEW_ADDRESS || '';

export default function Prepayment_Speed_Model_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'prepayment-speed-model-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.prepayment-speed-model-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'prepayment-speed-model-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>prepayment-speed-model-new</h1>
