...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OCEANFORGE_SECURE_FETCHER_ADDRESS || '';

export default function Oceanforge_Secure_FetcherPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'oceanforge-secure-fetcher',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.oceanforge-secure-fetcher/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'oceanforge-secure-fetcher',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>oceanforge-secure-fetcher</h1>
