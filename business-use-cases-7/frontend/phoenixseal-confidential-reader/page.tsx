...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PHOENIXSEAL_CONFIDENTIAL_READER_ADDRESS || '';

export default function Phoenixseal_Confidential_ReaderPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'phoenixseal-confidential-reader',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.phoenixseal-confidential-reader/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'phoenixseal-confidential-reader',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>phoenixseal-confidential-reader</h1>
