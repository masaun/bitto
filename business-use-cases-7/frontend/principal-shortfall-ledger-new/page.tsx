...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PRINCIPAL_SHORTFALL_LEDGER_NEW_ADDRESS || '';

export default function Principal_Shortfall_Ledger_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'principal-shortfall-ledger-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.principal-shortfall-ledger-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'principal-shortfall-ledger-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>principal-shortfall-ledger-new</h1>
