...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PORTFOLIO_BACKED_SECURITY_NEW_ADDRESS || '';

export default function Portfolio_Backed_Security_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'portfolio-backed-security-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.portfolio-backed-security-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'portfolio-backed-security-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>portfolio-backed-security-new</h1>
