...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ORDER_BOOK_MATCHER_ADDRESS || '';

export default function Order_Book_MatcherPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.order-book-matcher/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'order-book-matcher',
