...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PENSION_LIABILITY_TRACKER_ADDRESS || '';

export default function Pension_Liability_TrackerPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'pension-liability-tracker',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.pension-liability-tracker/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'pension-liability-tracker',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>pension-liability-tracker</h1>
