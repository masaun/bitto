...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OPALVEIL_CONFIDENTIAL_ENGINE_ADDRESS || '';

export default function Opalveil_Confidential_EnginePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'opalveil-confidential-engine',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.opalveil-confidential-engine/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'opalveil-confidential-engine',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>opalveil-confidential-engine</h1>
