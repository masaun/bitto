...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NOTEHOLDER_MEETING_MODULE_NEW_ADDRESS || '';

export default function Noteholder_Meeting_Module_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'noteholder-meeting-module-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.noteholder-meeting-module-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'noteholder-meeting-module-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>noteholder-meeting-module-new</h1>
