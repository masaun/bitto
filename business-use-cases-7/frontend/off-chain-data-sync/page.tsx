...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OFF_CHAIN_DATA_SYNC_ADDRESS || '';

export default function Off_Chain_Data_SyncPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.off-chain-data-sync/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'off-chain-data-sync',
