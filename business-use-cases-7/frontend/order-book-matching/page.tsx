...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ORDER_BOOK_MATCHING_ADDRESS || '';

export default function Order_Book_MatchingPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.order-book-matching/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'order-book-matching',
