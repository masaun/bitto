...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PAYMENT_DATE_SCHEDULER_NEW_ADDRESS || '';

export default function Payment_Date_Scheduler_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'payment-date-scheduler-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.payment-date-scheduler-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'payment-date-scheduler-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>payment-date-scheduler-new</h1>
