...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NORTHSTARVEIL_CONFIDENTIAL_TRANSFORM_ADDRESS || '';

export default function Northstarveil_Confidential_TransformPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'northstarveil-confidential-transform',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.northstarveil-confidential-transform/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'northstarveil-confidential-transform',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>northstarveil-confidential-transform</h1>
