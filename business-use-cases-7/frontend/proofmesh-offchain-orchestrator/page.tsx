...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROOFMESH_OFFCHAIN_ORCHESTRATOR_ADDRESS || '';

export default function Proofmesh_Offchain_OrchestratorPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'proofmesh-offchain-orchestrator',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.proofmesh-offchain-orchestrator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'proofmesh-offchain-orchestrator',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>proofmesh-offchain-orchestrator</h1>
