...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OVERSIGHT_SECURE_RUNNER_ADDRESS || '';

export default function Oversight_Secure_RunnerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.oversight-secure-runner/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'oversight-secure-runner',
