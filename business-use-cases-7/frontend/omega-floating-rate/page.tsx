...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OMEGA_FLOATING_RATE_ADDRESS || '';

export default function Omega_Floating_RatePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.omega-floating-rate/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'omega-floating-rate',
