...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PAXOS_RESERVE_MGR_ADDRESS || '';

export default function Paxos_Reserve_MgrPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.paxos-reserve-mgr/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'paxos-reserve-mgr',
