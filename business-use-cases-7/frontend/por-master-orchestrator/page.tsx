...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_POR_MASTER_ORCHESTRATOR_ADDRESS || '';

export default function Por_Master_OrchestratorPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.por-master-orchestrator/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'por-master-orchestrator',
