...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PATENT_FILING_AGENT_ADDRESS || '';

export default function Patent_Filing_AgentPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.patent-filing-agent/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'patent-filing-agent',
