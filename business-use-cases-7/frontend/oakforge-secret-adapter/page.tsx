...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OAKFORGE_SECRET_ADAPTER_ADDRESS || '';

export default function Oakforge_Secret_AdapterPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.oakforge-secret-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'oakforge-secret-adapter',
