...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_POOLED_LOAN_LIQUIDITY_NEW_ADDRESS || '';

export default function Pooled_Loan_Liquidity_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'pooled-loan-liquidity-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.pooled-loan-liquidity-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'pooled-loan-liquidity-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>pooled-loan-liquidity-new</h1>
