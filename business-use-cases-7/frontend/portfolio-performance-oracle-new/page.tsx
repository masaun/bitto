...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PORTFOLIO_PERFORMANCE_ORACLE_NEW_ADDRESS || '';

export default function Portfolio_Performance_Oracle_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'portfolio-performance-oracle-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.portfolio-performance-oracle-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'portfolio-performance-oracle-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>portfolio-performance-oracle-new</h1>
