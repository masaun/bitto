...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OPERATING_BUDGET_TRACKER_ADDRESS || '';

export default function Operating_Budget_TrackerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.operating-budget-tracker/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'operating-budget-tracker',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>operating-budget-tracker</h1>
