...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OAKVEILFORGE_SECURE_ADAPTER_ADDRESS || '';

export default function Oakveilforge_Secure_AdapterPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'oakveilforge-secure-adapter',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.oakveilforge-secure-adapter/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'oakveilforge-secure-adapter',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>oakveilforge-secure-adapter</h1>
