...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PAILLIER_ENCRYPTION_ADDRESS || '';

export default function Paillier_EncryptionPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.paillier-encryption/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'paillier-encryption',
