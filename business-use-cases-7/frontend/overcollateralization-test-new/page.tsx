...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OVERCOLLATERALIZATION_TEST_NEW_ADDRESS || '';

export default function Overcollateralization_Test_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'overcollateralization-test-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.overcollateralization-test-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'overcollateralization-test-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>overcollateralization-test-new</h1>
