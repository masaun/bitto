...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OFFER_ACCEPTANCE_LOG_ADDRESS || '';

export default function Offer_Acceptance_LogPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.offer-acceptance-log/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'offer-acceptance-log',
