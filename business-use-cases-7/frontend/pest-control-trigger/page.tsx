...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PEST_CONTROL_TRIGGER_ADDRESS || '';

export default function Pest_Control_TriggerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.pest-control-trigger/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'pest-control-trigger',
