...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_NOTARISE_SECURE_DISPATCHER_ADDRESS || '';

export default function Notarise_Secure_DispatcherPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'notarise-secure-dispatcher',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.notarise-secure-dispatcher/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'notarise-secure-dispatcher',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>notarise-secure-dispatcher</h1>
