...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OPALGUARD_CONFIDENTIAL_BRIDGE_ADDRESS || '';

export default function Opalguard_Confidential_BridgePage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'opalguard-confidential-bridge',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.opalguard-confidential-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'opalguard-confidential-bridge',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>opalguard-confidential-bridge</h1>
