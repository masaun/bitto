...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PORTFOLIO_REBALANCER_AI_ADDRESS || '';

export default function Portfolio_Rebalancer_AiPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.portfolio-rebalancer-ai/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'portfolio-rebalancer-ai',
