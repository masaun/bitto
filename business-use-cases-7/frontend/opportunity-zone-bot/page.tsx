...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_OPPORTUNITY_ZONE_BOT_ADDRESS || '';

export default function Opportunity_Zone_BotPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.opportunity-zone-bot/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'opportunity-zone-bot',
