...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PERFORMANCE_FEE_POOL_ADDRESS || '';

export default function Performance_Fee_PoolPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.performance-fee-pool/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'performance-fee-pool',
