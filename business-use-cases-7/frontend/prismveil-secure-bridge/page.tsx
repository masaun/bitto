...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PRISMVEIL_SECURE_BRIDGE_ADDRESS || '';

export default function Prismveil_Secure_BridgePage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.prismveil-secure-bridge/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'prismveil-secure-bridge',
