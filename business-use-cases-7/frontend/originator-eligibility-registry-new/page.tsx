...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_ORIGINATOR_ELIGIBILITY_REGISTRY_NEW_ADDRESS || '';

export default function Originator_Eligibility_Registry_NewPage() {
  ...
  const handleConnect = () => {
    ...
    showConnect({
      appDetails: {
        name: 'originator-eligibility-registry-new',
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.originator-eligibility-registry-new/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'originator-eligibility-registry-new',
  ...
  return (
    <div style={ padding: '20px', fontFamily: 'Arial' }>
      <h1>originator-eligibility-registry-new</h1>
