...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PROOFVAULTX_PRIVATE_HUB_ADDRESS || '';

export default function Proofvaultx_Private_HubPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.proofvaultx-private-hub/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'proofvaultx-private-hub',
