...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PHI_WARRANT_ISSUER_ADDRESS || '';

export default function Phi_Warrant_IssuerPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.phi-warrant-issuer/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'phi-warrant-issuer',
