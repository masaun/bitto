...
const CONTRACT_ADDRESS = process.env.NEXT_PUBLIC_PERMIT_APPROVAL_PRED_ADDRESS || '';

export default function Permit_Approval_PredPage() {
  ...
  const getCounter = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.mainnet.hiro.so/v1/contract/{CONTRACT_ADDRESS}.permit-approval-pred/read-only/get-counter`,
        {
          method: 'GET',
    ...
    try {
      const response = await fetch('/api/call-contract', {
        ...
        body: JSON.stringify({
          ...
          contractName: 'permit-approval-pred',
