...
const contractDetails = parseContractIdentifier(process.env.FIXED_INCOME_BOND_CONTRACT_ADDRESS, 'fixed-income-bond');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const bondId = `BOND${cycle.toString().padStart(8, '0')}`;
    const issueTx = await makeContractCall({
    ...
    const issueResult = await broadcastTransaction({ transaction: issueTx, network: networkForBroadcast });
    console.log(`issue-bond tx: ${issueResult.txid}`);
    nonce++;
    ...
    const purchaseResult = await broadcastTransaction({ transaction: purchaseTx, network: networkForBroadcast });
    console.log(`purchase-bond tx: ${purchaseResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
    ...
    console.log(`redeem-bond tx: ${redeemResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
  }
