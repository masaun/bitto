...
const contractDetails = parseContractIdentifier(
  process.env.META_PROXY_CONTRACT_ADDRESS,
  'meta-proxy'
);
...
interface FunctionCall {
  name: string;
  args: (cycle: number, senderAddress: string, proxyId: number, contractPrincipal: string) => any[];
}
...
function getWriteFunctions(): FunctionCall[] {
  return [
    {
      name: 'create-proxy',
      args: (cycle: number, _senderAddress: string, _proxyId: number, contractPrincipal: string) => [
        principalCV(contractPrincipal),
        bufferCV(Buffer.from(`metadata-cycle-${cycle}-${Date.now()}`)),
      ],
    },
    {
      name: 'initialize-proxy',
      args: (_cycle: number, _senderAddress: string, proxyId: number, _contractPrincipal: string) => [
        uintCV(proxyId),
      ],
    },
    {
      ...
      args: (cycle: number, _senderAddress: string, _proxyId: number, contractPrincipal: string) => [
        principalCV(contractPrincipal),
        bufferCV(Buffer.from(`metadata-v2-cycle-${cycle}-${Date.now()}`)),
      ],
    },
    {
      name: 'update-implementation',
      args: (_cycle: number, _senderAddress: string, proxyId: number, contractPrincipal: string) => [
        uintCV(proxyId + 1),
        principalCV(contractPrincipal),
      ],
    },
    {
      name: 'set-authorized-deployer',
      args: (cycle: number, senderAddress: string, _proxyId: number, _contractPrincipal: string) => [
    ...
    },
    {
      ...
      args: (_cycle: number, senderAddress: string, _proxyId: number, _contractPrincipal: string) => [
        principalCV(senderAddress),
      ],
    },
  ];
...
async function executeCycle(
  ...
  const proxyId = cycle * 2 + 1;
  const functions = getWriteFunctions();
  const contractPrincipal = `${CONFIG.contractAddress}.${CONFIG.contractName}`;

  for (const func of functions) {
    const args = func.args(cycle, senderAddress, proxyId, contractPrincipal);
...
async function executeAllCycles(): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Meta Proxy Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Meta Proxy Contract - Batch Call Script');
  ...
  console.log(`Cycles: ${CONFIG.cycles}`);
  console.log(`Environment Variable: META_PROXY_CONTRACT_ADDRESS`);

  if (!process.env.META_PROXY_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: META_PROXY_CONTRACT_ADDRESS not set in environment.');
