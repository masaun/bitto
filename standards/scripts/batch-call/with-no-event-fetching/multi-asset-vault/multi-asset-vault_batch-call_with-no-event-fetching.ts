...
const contractDetails = parseContractIdentifier(process.env.MULTI_ASSET_VAULT_CONTRACT_ADDRESS, 'multi-asset-vault');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const assetAddress = standardPrincipalCV(senderAddress);

    const addAssetTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'add-asset',
      functionArgs: [assetAddress],
    ...
    const addAssetResult = await broadcastTransaction({ transaction: addAssetTx, network: networkForBroadcast });
    console.log(`add-asset tx: ${addAssetResult.txid}`);
    nonce++;
    ...
    const depositTx = await makeContractCall({
      ...
      functionArgs: [assetAddress, uintCV(1000)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const depositResult = await broadcastTransaction({ transaction: depositTx, network: networkForBroadcast });
    ...
    const withdrawTx = await makeContractCall({
      ...
      functionArgs: [assetAddress, uintCV(100)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const withdrawResult = await broadcastTransaction({ transaction: withdrawTx, network: networkForBroadcast });
    console.log(`withdraw tx: ${withdrawResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const transferSharesTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'transfer-shares',
      functionArgs: [assetAddress, uintCV(50), standardPrincipalCV(senderAddress)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const transferSharesResult = await broadcastTransaction({ transaction: transferSharesTx, network: networkForBroadcast });
    console.log(`transfer-shares tx: ${transferSharesResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
  }
