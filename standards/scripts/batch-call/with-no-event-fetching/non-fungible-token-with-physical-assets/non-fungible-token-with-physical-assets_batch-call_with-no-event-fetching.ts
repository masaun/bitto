...
const contractDetails = parseContractIdentifier(
  process.env.NON_FUNGIBLE_TOKEN_WITH_PHYSICAL_ASSETS_CONTRACT_ADDRESS,
  'non-fungible-token-with-physical-assets'
);
...
function getWriteFunctions(senderAddress: string, recipientAddress: string): FunctionCall[] {
  const sampleHash = bufferCV(Buffer.alloc(32, 0xab));
  const sampleEngagementData = bufferCV(Buffer.alloc(33, 0x02));
  ...
  return [
    {
      ...
      args: () => [
        ...
        someCV(principalCV(senderAddress)),
        stringAsciiCV('https://example.com/token/1'),
      ],
      description: 'Mint a new NFT tied to physical asset',
    },
    {
      name: 'transfer',
      args: () => [uintCV(1), principalCV(recipientAddress)],
      description: 'Transfer NFT to recipient',
    },
    {
      name: 'set-user',
      args: () => [uintCV(1), principalCV(recipientAddress)],
      description: 'Assign user to NFT',
    },
    {
      name: 'start-owner-engagement',
      args: () => [uintCV(1), sampleEngagementData, sampleHash],
      description: 'Start owner engagement process',
    },
    {
      name: 'owner-engagement',
      args: () => [uintCV(1), sampleHash],
      description: 'Complete owner engagement',
    },
    {
      name: 'start-user-engagement',
      args: () => [uintCV(1), sampleEngagementData, sampleHash],
      description: 'Start user engagement process',
    },
    {
      name: 'user-engagement',
      args: () => [uintCV(1), sampleHash],
      description: 'Complete user engagement',
    },
    {
      name: 'update-timestamp',
      args: () => [uintCV(1)],
      description: 'Update asset timestamp',
    },
    {
      name: 'set-timeout',
      args: () => [uintCV(1), uintCV(172800)],
      description: 'Set timeout for token',
    },
    {
      name: 'check-timeout',
      args: () => [uintCV(1)],
      description: 'Check if timeout expired',
    },
    {
      name: 'unassign-user',
      args: () => [uintCV(1)],
...
async function executeBatchCycles(): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for non-fungible-token-with-physical-assets ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Non-Fungible Token with Physical Assets - Batch Call Script');
  console.log('==========================================================');
