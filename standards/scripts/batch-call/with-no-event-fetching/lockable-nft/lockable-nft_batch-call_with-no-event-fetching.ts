...
const contractDetails = parseContractIdentifier(process.env.LOCKABLE_NFT_CONTRACT_ADDRESS, 'lockable-nft');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const setDefaultTx = await makeContractCall({
    ...
    const setDefaultResult = await broadcastTransaction({ transaction: setDefaultTx, network: networkForBroadcast });
    console.log(`set-default-locked tx: ${setDefaultResult.txid}`);
    nonce++;
    ...
    const lockResult = await broadcastTransaction({ transaction: lockTx, network: networkForBroadcast });
    ...
    const unlockResult = await broadcastTransaction({ transaction: unlockTx, network: networkForBroadcast });
