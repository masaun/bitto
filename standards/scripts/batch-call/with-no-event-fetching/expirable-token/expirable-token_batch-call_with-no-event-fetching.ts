...
const contractDetails = parseContractIdentifier(process.env.EXPIRABLE_TOKEN_CONTRACT_ADDRESS, 'expirable-token');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const transferAtEpochTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'transfer-at-epoch',
      functionArgs: [uintCV(2), uintCV(50), standardPrincipalCV(senderAddress), standardPrincipalCV(senderAddress)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const transferAtEpochResult = await broadcastTransaction({ transaction: transferAtEpochTx, network: networkForBroadcast });
    console.log(`transfer-at-epoch tx: ${transferAtEpochResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
  }
