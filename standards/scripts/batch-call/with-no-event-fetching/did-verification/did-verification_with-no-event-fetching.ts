...
const contractDetails = parseContractIdentifier(process.env.DID_VERIFICATION_CONTRACT_ADDRESS, 'did-verification');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    console.log(`authorize-verifier tx: ${authorizeResult.txid}`);
    nonce++;
    ...
    console.log(`revoke-verifier tx: ${revokeResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const identityHash = Buffer.from(cycle.toString().repeat(32).slice(0, 64), 'hex');
    const verificationHash = Buffer.from((cycle + 1).toString().repeat(32).slice(0, 64), 'hex');

    const createTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'create-identity',
      functionArgs: [bufferCV(identityHash), bufferCV(verificationHash)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const createResult = await broadcastTransaction({ transaction: createTx, network: networkForBroadcast });
    console.log(`create-identity tx: ${createResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const publicKey = Buffer.from('0'.repeat(66), 'hex');
    const signature = Buffer.from('1'.repeat(128), 'hex');

    const verifyTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'verify-identity',
      functionArgs: [standardPrincipalCV(senderAddress), bufferCV(verificationHash), bufferCV(publicKey), bufferCV(signature)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    console.log(`verify-identity tx: ${verifyResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const newIdentityHash = Buffer.from((cycle + 2).toString().repeat(32).slice(0, 64), 'hex');
    const newVerificationHash = Buffer.from((cycle + 3).toString().repeat(32).slice(0, 64), 'hex');

    const updateTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'update-identity',
      functionArgs: [bufferCV(newIdentityHash), bufferCV(newVerificationHash)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    console.log(`update-identity tx: ${updateResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const revokeIdentityTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const revokeIdentityResult = await broadcastTransaction({ transaction: revokeIdentityTx, network: networkForBroadcast });
    console.log(`revoke-identity tx: ${revokeIdentityResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
  }
