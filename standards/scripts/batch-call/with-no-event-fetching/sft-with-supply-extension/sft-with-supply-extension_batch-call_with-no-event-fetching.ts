...
const contractDetails = parseContractIdentifier(
  process.env.SFT_WITH_SUPPLY_EXTENSION_CONTRACT_ADDRESS,
  ...
  return [
    ...
    },
    {
      ...
      args: (cycle: number, _senderAddress: string, tokenId: number) => [
        uintCV(tokenId),
        stringAsciiCV(`https://api.bitto.io/sft/${tokenId}-${cycle}`),
      ],
    },
    {
      name: 'batch-transfer',
...
async function executeAllCycles(): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for SFT with Supply Extension Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  SFT with Supply Extension Contract - Batch Call Script');
  ...
  console.log(`Cycles: ${CONFIG.cycles}`);
  console.log(`Environment Variable: SFT_WITH_SUPPLY_EXTENSION_CONTRACT_ADDRESS`);

  if (!process.env.SFT_WITH_SUPPLY_EXTENSION_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: SFT_WITH_SUPPLY_EXTENSION_CONTRACT_ADDRESS not set in environment.');
