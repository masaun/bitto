...
const contractDetails = parseContractIdentifier(
  process.env.STEALTH_ADDRESS_REGISTRY_CONTRACT_ADDRESS,
...
function getWriteFunctions(senderAddress: string): FunctionCall[] {
  const metaAddress = Buffer.from('02'.repeat(66), 'hex');
  const signature = Buffer.from('03'.repeat(65), 'hex');
  
  return [
    {
      name: 'register-keys',
      args: () => [uintCV(0), bufferCV(metaAddress)],
      description: 'Register stealth address keys',
    },
    {
      name: 'register-keys-on-behalf',
      args: () => [principalCV(senderAddress), uintCV(0), bufferCV(metaAddress), bufferCV(signature)],
      description: 'Register keys on behalf of another user',
    },
    {
      name: 'increment-nonce',
      args: () => [],
      description: 'Increment nonce for key rotation',
    },
  ];
}
...
async function executeBatchCycles(): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for stealth-address-registry ===');
  ...
  const functions = getWriteFunctions(senderAddress);
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Stealth Address Registry - Batch Call Script');
  console.log('==========================================================');
