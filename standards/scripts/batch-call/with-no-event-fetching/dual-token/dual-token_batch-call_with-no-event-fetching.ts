...
const contractDetails = parseContractIdentifier(process.env.DUAL_TOKEN_CONTRACT_ADDRESS, 'dual-token');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const setSkipTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'set-skip-nft',
      functionArgs: [boolCV(cycle % 2 === 0)],
    ...
    const setSkipResult = await broadcastTransaction({ transaction: setSkipTx, network: networkForBroadcast });
    console.log(`set-skip-nft tx: ${setSkipResult.txid}`);
    nonce++;
    ...
    const transferTx = await makeContractCall({
      ...
      functionArgs: [uintCV(100), standardPrincipalCV(senderAddress), standardPrincipalCV(senderAddress)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const mintNftTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const mintNftResult = await broadcastTransaction({ transaction: mintNftTx, network: networkForBroadcast });
    console.log(`mint-nft tx: ${mintNftResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const transferNftTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const transferNftResult = await broadcastTransaction({ transaction: transferNftTx, network: networkForBroadcast });
    console.log(`transfer-nft tx: ${transferNftResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
    ...
    const burnResult = await broadcastTransaction({ transaction: burnTx, network: networkForBroadcast });
    console.log(`burn tx: ${burnResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
