...
const contractDetails = parseContractIdentifier(
  process.env.SFT_WITH_SUPPLY_EXTENSION_V2_CONTRACT_ADDRESS,
  'sft-with-supply-extension-v2'
);
  ...
  return [
    ...
    },
    {
      ...
      args: (cycle: number, _senderAddress: string, tokenId: number) => [
        uintCV(tokenId),
        stringAsciiCV(`https://api.bitto.io/sft-v2/${tokenId}-${cycle}`),
      ],
    },
    {
      name: 'batch-transfer',
      args: (cycle: number, senderAddress: string, tokenId: number) => [
        listCV([
          tupleCV({
            ...
            'amount': uintCV(5),
            'recipient': principalCV(senderAddress),
          }),
        ]),
      ],
    },
...
async function executeAllCycles(): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for SFT with Supply Extension V2 Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  SFT with Supply Extension V2 Contract - Batch Call Script');
  ...
  console.log(`Cycles: ${CONFIG.cycles}`);
  console.log(`Environment Variable: SFT_WITH_SUPPLY_EXTENSION_V2_CONTRACT_ADDRESS`);

  if (!process.env.SFT_WITH_SUPPLY_EXTENSION_V2_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: SFT_WITH_SUPPLY_EXTENSION_V2_CONTRACT_ADDRESS not set in environment.');
