...
const contractDetails = parseContractIdentifier(
  process.env.COMPOSABLE_NON_FUNGIBLE_TOKEN_CONTRACT_ADDRESS,
  ...
  return [
    {
      ...
      description: 'Mint new composable NFT',
    },
    {
      name: 'transfer',
    ...
    {
      name: 'add-asset',
      args: () => [uintCV(1), uintCV(100), principalCV(senderAddress), listCV([uintCV(1), uintCV(2)]), uintCV(1)],
      description: 'Add asset to NFT',
    },
    {
      name: 'equip',
      args: () => [uintCV(1), uintCV(0), uintCV(100), uintCV(1), uintCV(200)],
      description: 'Equip child asset',
    },
    {
      name: 'unequip',
      args: () => [uintCV(1), uintCV(100), uintCV(1)],
      description: 'Unequip asset',
    },
    {
      name: 'add-slot-part',
      args: () => [uintCV(1), uintCV(10), listCV([principalCV(senderAddress)])],
      description: 'Add slot part to catalog',
    },
    {
      name: 'add-fixed-part',
      args: () => [uintCV(2), uintCV(5), stringAsciiCV('fixed-part-metadata')],
      description: 'Add fixed part to catalog',
    },
  ];
}
...
async function executeBatchCycles(): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for composable-non-fungible-token ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Composable Non-Fungible Token - Batch Call Script');
  console.log('==========================================================');
