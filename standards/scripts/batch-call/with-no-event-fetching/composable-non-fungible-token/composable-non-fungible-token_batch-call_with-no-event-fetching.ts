...
function getWriteFunctions(senderAddress: string, recipientAddress: string): FunctionCall[] {
  return [
    ...
    },
    {
      ...
      description: 'Add asset to token',
    },
    {
      name: 'equip',
      args: () => [uintCV(1), uintCV(0), uintCV(1), uintCV(1), uintCV(1)],
      description: 'Equip child NFT to parent',
    },
    {
      name: 'unequip',
      args: () => [uintCV(1), uintCV(1), uintCV(1)],
      description: 'Unequip child NFT from parent',
    },
    {
      name: 'add-slot-part',
      args: () => [uintCV(1), uintCV(1), listCV([principalCV(senderAddress)])],
      description: 'Add slot part to catalog',
    },
    {
      name: 'add-fixed-part',
      args: () => [uintCV(2), uintCV(2), stringAsciiCV('fixed-metadata')],
