...
const contractDetails = parseContractIdentifier(
  process.env.MULTI_ASSET_TOKEN_CONTRACT_ADDRESS,
  'multi-asset-token'
);
  ...
  return [
    ...
    },
    {
      ...
      args: (cycle: number, _senderAddress: string, tokenId: number) => [
        uintCV(tokenId),
        stringAsciiCV(`https://api.bitto.io/asset/${tokenId}-${cycle}`),
        uintCV(0),
      ],
    },
    {
    ...
    },
    {
      ...
      args: (cycle: number, _senderAddress: string, tokenId: number) => [
        uintCV(tokenId),
        listCV([uintCV(cycle + 1), uintCV(cycle + 2)]),
      ],
    },
    {
    ...
    },
    {
      ...
      args: (cycle: number, _senderAddress: string, tokenId: number) => [
        uintCV(tokenId),
        stringAsciiCV(`https://api.bitto.io/multi-asset/${tokenId}-${cycle}`),
      ],
    },
    {
      name: 'burn',
...
async function executeAllCycles(): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for Multi-Asset Token Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Multi-Asset Token Contract - Batch Call Script');
  ...
  console.log(`Cycles: ${CONFIG.cycles}`);
  console.log(`Environment Variable: MULTI_ASSET_TOKEN_CONTRACT_ADDRESS`);

  if (!process.env.MULTI_ASSET_TOKEN_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: MULTI_ASSET_TOKEN_CONTRACT_ADDRESS not set in environment.');
