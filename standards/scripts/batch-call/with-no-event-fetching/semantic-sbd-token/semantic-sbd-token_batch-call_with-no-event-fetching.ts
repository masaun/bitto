...
const contractDetails = parseContractIdentifier(
  process.env.SEMANTIC_SBD_TOKEN_CONTRACT_ADDRESS,
  'semantic-sbd-token'
);
...
function getWriteFunctions(senderAddress: string, recipientAddress: string): FunctionCall[] {
  return [
    {
      name: 'mint',
      args: () => [principalCV(recipientAddress), stringAsciiCV('ipfs://test-uri'), stringUtf8CV('<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"><rdf:Description rdf:about="token">test</rdf:Description></rdf:RDF>')],
      description: 'Mint new semantic soulbound token',
    },
    {
      name: 'update-rdf',
      args: () => [uintCV(1), stringUtf8CV('<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"><rdf:Description rdf:about="token">updated</rdf:Description></rdf:RDF>')],
      description: 'Update RDF metadata of token',
    },
    {
      name: 'burn',
      args: () => [uintCV(1)],
      description: 'Burn soulbound token',
    },
    {
      name: 'set-schema-uri',
      args: () => [stringAsciiCV('https://schema.org/SBT')],
      description: 'Set schema URI for tokens',
    },
    {
      name: 'transfer',
      args: () => [uintCV(1), principalCV(senderAddress), principalCV(recipientAddress)],
      description: 'Attempt to transfer soulbound token',
    },
  ];
}
...
async function executeBatchCycles(): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for semantic-sbd-token ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Semantic SBD Token - Batch Call Script');
  console.log('==========================================================');
