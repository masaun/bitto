...
const contractDetails = parseContractIdentifier(process.env.ASYNC_VAULT_TOKEN_CONTRACT_ADDRESS, 'async-vault-token');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const requestDepositTx = await makeContractCall({
    ...
    const requestDepositResult = await broadcastTransaction({ transaction: requestDepositTx, network: networkForBroadcast });
    console.log(`request-deposit tx: ${requestDepositResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const fulfillDepositTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'fulfill-deposit',
      functionArgs: [standardPrincipalCV(senderAddress), uintCV(requestId)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const fulfillDepositResult = await broadcastTransaction({ transaction: fulfillDepositTx, network: networkForBroadcast });
    console.log(`fulfill-deposit tx: ${fulfillDepositResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const requestRedeemTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const requestRedeemResult = await broadcastTransaction({ transaction: requestRedeemTx, network: networkForBroadcast });
    console.log(`request-redeem tx: ${requestRedeemResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const fulfillRedeemTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const fulfillRedeemResult = await broadcastTransaction({ transaction: fulfillRedeemTx, network: networkForBroadcast });
    console.log(`fulfill-redeem tx: ${fulfillRedeemResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
