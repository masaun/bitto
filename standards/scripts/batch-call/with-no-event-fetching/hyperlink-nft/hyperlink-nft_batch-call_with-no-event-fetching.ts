...
const contractDetails = parseContractIdentifier(process.env.HYPERLINK_NFT_CONTRACT_ADDRESS, 'hyperlink-nft');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const mintWithoutHyperlinkTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'mint',
      functionArgs: [standardPrincipalCV(senderAddress), uintCV(tokenId), noneCV()],
    ...
    const mintWithoutHyperlinkResult = await broadcastTransaction({ transaction: mintWithoutHyperlinkTx, network: networkForBroadcast });
    console.log(`mint (no hyperlink) tx: ${mintWithoutHyperlinkResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const setHyperlinkTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'set-hyperlink',
      functionArgs: [uintCV(tokenId), bufferCV(Buffer.alloc(64, cycle))],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const setHyperlinkResult = await broadcastTransaction({ transaction: setHyperlinkTx, network: networkForBroadcast });
    console.log(`set-hyperlink tx: ${setHyperlinkResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    tokenId++;

    const mintWithHyperlinkTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'mint',
      functionArgs: [standardPrincipalCV(senderAddress), uintCV(tokenId), someCV(bufferCV(Buffer.alloc(32, cycle)))],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const mintWithHyperlinkResult = await broadcastTransaction({ transaction: mintWithHyperlinkTx, network: networkForBroadcast });
    console.log(`mint (with hyperlink) tx: ${mintWithHyperlinkResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
