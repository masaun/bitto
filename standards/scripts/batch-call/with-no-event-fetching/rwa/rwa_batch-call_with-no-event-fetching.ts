...
const contractDetails = parseContractIdentifier(process.env.RWA_CONTRACT_ADDRESS, 'rwa');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const setFrozenTokensTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'set-frozen-tokens',
      functionArgs: [standardPrincipalCV(senderAddress), uintCV(25), boolCV(true)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const setFrozenTokensResult = await broadcastTransaction({ transaction: setFrozenTokensTx, network: networkForBroadcast });
    console.log(`set-frozen-tokens tx: ${setFrozenTokensResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const setCanTransactTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const setCanTransactResult = await broadcastTransaction({ transaction: setCanTransactTx, network: networkForBroadcast });
    console.log(`set-can-transact tx: ${setCanTransactResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
  }
