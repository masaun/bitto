...
const contractDetails = parseContractIdentifier(
  process.env.NFT_WITH_STATE_FINGERPRINT_CONTRACT_ADDRESS,
  ...
  return [
    {
      ...
      args: (cycle: number, senderAddress: string, _tokenId: number) => [
        ...
        uintCV(1000 + cycle * 100),
        stringAsciiCV(`https://api.bitto.io/nft-state/${cycle}`),
      ],
    },
    {
      name: 'transfer',
    ...
    },
    {
      ...
      args: (cycle: number, _senderAddress: string, tokenId: number) => [
        ...
        uintCV(2000 + cycle * 100),
        stringAsciiCV(`https://api.bitto.io/nft-state/${tokenId}-updated-${cycle}`),
      ],
    },
    {
      name: 'set-token-uri',
      args: (cycle: number, _senderAddress: string, tokenId: number) => [
        uintCV(tokenId),
        stringAsciiCV(`https://api.bitto.io/nft-uri/${tokenId}-${cycle}`),
      ],
    },
    {
      name: 'burn',
...
async function executeAllCycles(): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for NFT with State Fingerprint Contract ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  NFT with State Fingerprint Contract - Batch Call Script');
  ...
  console.log(`Cycles: ${CONFIG.cycles}`);
  console.log(`Environment Variable: NFT_WITH_STATE_FINGERPRINT_CONTRACT_ADDRESS`);

  if (!process.env.NFT_WITH_STATE_FINGERPRINT_CONTRACT_ADDRESS) {
    console.warn('\n⚠ Warning: NFT_WITH_STATE_FINGERPRINT_CONTRACT_ADDRESS not set in environment.');
