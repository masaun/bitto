...
const contractDetails = parseContractIdentifier(process.env.AUTHORIZED_RESELLABLE_NFT_CONTRACT_ADDRESS, 'authorized-resellable-nft');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    console.log(`authorize-reseller tx: ${authorizeResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
    ...
    console.log(`revoke-reseller tx: ${revokeResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const resellTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'resell',
      functionArgs: [uintCV(tokenId), standardPrincipalCV(senderAddress), uintCV(1000)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const resellResult = await broadcastTransaction({ transaction: resellTx, network: networkForBroadcast });
    console.log(`resell tx: ${resellResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const statuses = ['active', 'inactive', 'pending', 'sold'];
    const status = statuses[cycle % statuses.length];

    const changeStatusTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'change-status',
      functionArgs: [uintCV(tokenId), stringAsciiCV(status)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const changeStatusResult = await broadcastTransaction({ transaction: changeStatusTx, network: networkForBroadcast });
    console.log(`change-status tx: ${changeStatusResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const redeemTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'redeem',
    ...
    const redeemResult = await broadcastTransaction({ transaction: redeemTx, network: networkForBroadcast });
    console.log(`redeem tx: ${redeemResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
