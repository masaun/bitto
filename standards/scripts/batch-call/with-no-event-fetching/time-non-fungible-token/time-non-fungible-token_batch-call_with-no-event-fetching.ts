...
const contractDetails = parseContractIdentifier(
  process.env.TIME_NON_FUNGIBLE_TOKEN_CONTRACT_ADDRESS,
  'time-non-fungible-token'
);
...
function getWriteFunctions(senderAddress: string, recipientAddress: string): FunctionCall[] {
  ...
  const splitTime = currentTime + 86400 * 180;
  ...
  return [
    {
      ...
      args: () => [
        ...
        uintCV(startTime),
        uintCV(endTime),
        stringAsciiCV('https://example.com/time-nft/1'),
      ],
      description: 'Mint a new time-bounded NFT',
    },
    {
      name: 'mint-with-asset-id',
      args: () => [
        principalCV(recipientAddress),
        uintCV(1),
        uintCV(startTime),
        uintCV(endTime),
        stringAsciiCV('https://example.com/time-nft/2'),
      ],
      description: 'Mint with existing asset ID',
    },
    {
      name: 'transfer',
    ...
    },
    {
      name: 'approve',
      args: () => [uintCV(1), principalCV(recipientAddress)],
      description: 'Approve spender for token',
    },
    {
    ...
    },
    {
      ...
      description: 'Split time NFT into two periods',
    },
    {
      name: 'merge-tokens',
      args: () => [
      ...
      description: 'Merge two time NFTs',
    },
    {
      name: 'burn',
      args: () => [uintCV(1)],
      description: 'Burn a time NFT',
    },
    {
...
async function executeBatchCycles(): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for time-non-fungible-token ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Time Non-Fungible Token - Batch Call Script');
  console.log('==========================================================');
