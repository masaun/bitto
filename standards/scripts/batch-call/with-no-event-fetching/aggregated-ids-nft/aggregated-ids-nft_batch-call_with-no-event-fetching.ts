...
const contractDetails = parseContractIdentifier(
  process.env.AGGREGATED_IDS_NFT_CONTRACT_ADDRESS,
  'aggregated-ids-nft'
);
...
function getWriteFunctions(senderAddress: string, recipientAddress: string): FunctionCall[] {
  const identitiesRoot = Buffer.from('02'.repeat(32), 'hex');
  ...
  return [
    ...
    {
      name: 'set-identities-root',
      args: () => [uintCV(1), bufferCV(identitiesRoot)],
      description: 'Set identities root hash',
    },
    {
      name: 'verify-identities-binding',
      args: () => [
        uintCV(1),
        principalCV(senderAddress),
        listCV([stringUtf8CV('user1'), stringUtf8CV('user2')]),
        bufferCV(identitiesRoot),
        bufferCV(signature)
      ],
      description: 'Verify identities binding with signature',
    },
  ];
}
...
async function executeBatchCycles(): Promise<BatchCallResult[]> {
  console.log('\n=== Starting Batch Calls for aggregated-ids-nft ===');
...
async function main(): Promise<void> {
  console.log('==========================================================');
  console.log('  Aggregated IDs NFT - Batch Call Script');
  console.log('==========================================================');
