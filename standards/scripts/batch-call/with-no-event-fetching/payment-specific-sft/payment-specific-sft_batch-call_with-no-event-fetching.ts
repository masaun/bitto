...
const contractDetails = parseContractIdentifier(process.env.PAYMENT_SPECIFIC_SFT_CONTRACT_ADDRESS, 'payment-specific-sft');
...
async function batchCall(): Promise<void> {
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const complianceResult = await broadcastTransaction({ transaction: complianceTx, network: networkForBroadcast });
    console.log(`set-compliance-guard tx: ${complianceResult.txid}`);
    nonce++;
    ...
    const mintTx = await makeContractCall({
      ...
      functionArgs: [uintCV(tokenId), standardPrincipalCV(senderAddress), uintCV(100), uintCV(999999999999)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    console.log(`mint-pbm tx: ${mintResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
    ...
    const transferTx = await makeContractCall({
      ...
      functionArgs: [uintCV(tokenId), uintCV(10), standardPrincipalCV(senderAddress), standardPrincipalCV(senderAddress)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    console.log(`transfer-pbm tx: ${transferResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
    ...
    const redeemTx = await makeContractCall({
      ...
      functionArgs: [uintCV(tokenId), uintCV(5)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    console.log(`redeem-pbm tx: ${redeemResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
  }
