...
const contractDetails = parseContractIdentifier(process.env.AC_REGISTRY_CONTRACT_ADDRESS, 'ac-registry');
...
async function batchCall(): Promise<void> {
  ...
  const roleBuffer = bufferCV(Buffer.from('admin'));
  ...
  for (let cycle = 0; cycle < CONFIG.batchCycles; cycle++) {
    ...
    const registerResult = await broadcastTransaction({ transaction: registerTx, network: networkForBroadcast });
    console.log(`register-contract tx: ${registerResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const grantRoleTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'grant-role',
      functionArgs: [contractPrincipalCV(CONFIG.contractAddress, CONFIG.contractName), roleBuffer, standardPrincipalCV(senderAddress)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const grantRoleResult = await broadcastTransaction({ transaction: grantRoleTx, network: networkForBroadcast });
    console.log(`grant-role tx: ${grantRoleResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const grantRolesTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'grant-roles',
      functionArgs: [
        listCV([contractPrincipalCV(CONFIG.contractAddress, CONFIG.contractName)]),
        listCV([roleBuffer]),
    ...
    const grantRolesResult = await broadcastTransaction({ transaction: grantRolesTx, network: networkForBroadcast });
    console.log(`grant-roles tx: ${grantRolesResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const revokeRoleTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const revokeRoleResult = await broadcastTransaction({ transaction: revokeRoleTx, network: networkForBroadcast });
    console.log(`revoke-role tx: ${revokeRoleResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const revokeRolesTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
    ...
    const revokeRolesResult = await broadcastTransaction({ transaction: revokeRolesTx, network: networkForBroadcast });
    console.log(`revoke-roles tx: ${revokeRolesResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);

    const unregisterTx = await makeContractCall({
      contractAddress: CONFIG.contractAddress,
      contractName: CONFIG.contractName,
      functionName: 'unregister-contract',
      functionArgs: [contractPrincipalCV(CONFIG.contractAddress, CONFIG.contractName)],
      senderKey: CONFIG.senderKey,
      network: networkForBroadcast,
      anchorMode: AnchorMode.Any,
      postConditionMode: PostConditionMode.Allow,
    ...
    const unregisterResult = await broadcastTransaction({ transaction: unregisterTx, network: networkForBroadcast });
    console.log(`unregister-contract tx: ${unregisterResult.txid}`);
    nonce++;
    await sleep(CONFIG.delayBetweenCalls);
  }
